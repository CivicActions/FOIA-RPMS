BSTS2P08 ;GDIT/HS/BEE-Version 2.0 Patch 8 ENV,Pre and Post Install ; 19 Nov 2012  9:41 AM
...
ENV ;EP - Environmental Checking Routine
 ...
 I '$$INSTALLD("BSTS*2.0*7") D BMES^XPDUTL("Version 2.0 Patch 7 of BSTS is required!") S XPDQUIT=2 Q
 ;
...
PST ;EP
 ...
 L -^TMP("BSTSINSTALL")
 ;
 NEW TRIEN,EXEC,ERR,WIEN,WSIEN,PATCH,SIEN,STS,TR,CDST,VAR
 NEW SSLNAME,SSLFND,SYSXREF,SYSGL,CPORT,TWSIEN,PWSIEN,CIEN,NMID
 NEW CVAR,RVAR,SVAR,STS
 ;
 ;Make a log entry
 D LOG^BSTSAPIL("INST","","PATCH","BSTS*2.0*8")
 ;
 ;Query DTS for namespace list (pull down CVX and new subset codesets)
 S STS=$$CODESETS^BSTSAPI("VAR")
 ;
 ;Check for new CVX codeset
 S CIEN=$O(^BSTS(9002318.1,"B",5190,""))
 I 'CIEN D  Q
 . ;
 . D BMES^XPDUTL("The new CVX codeset could not be located. Please log an incident report with BSTS support") S XPDQUIT=2 Q
 . ;
 . ;Unlock installation entry
 . L -^TMP("BSTSINSTALL")
 ;
 ;Check for new 32781 codeset
 S CIEN=$O(^BSTS(9002318.1,"B",32781,""))
 I 'CIEN D  Q
 . ;
 . D BMES^XPDUTL("The new 32781 codeset could not be located. Please log an incident report with BSTS support") S XPDQUIT=2 Q
 . ;
 . ;Unlock installation entry
 . L -^TMP("BSTSINSTALL")
 ;
 ...
 ;
 ;Check for new 32782 codeset
 S CIEN=$O(^BSTS(9002318.1,"B",32782,""))
 I 'CIEN D  Q
 . ;
 . D BMES^XPDUTL("The new 32782 codeset could not be located. Please log an incident report with BSTS support") S XPDQUIT=2 Q
 . ;
 . ;Unlock installation entry
 . L -^TMP("BSTSINSTALL")
 ;
 ;Check for new 32783 codeset
 S CIEN=$O(^BSTS(9002318.1,"B",32783,""))
 I 'CIEN D  Q
 . ;
 . D BMES^XPDUTL("The new 32783 codeset could not be located. Please log an incident report with BSTS support") S XPDQUIT=2 Q
 . ;
 . ;Unlock installation entry
 . L -^TMP("BSTSINSTALL")
 ;
 ...
 ;
 ;Clear out any current update in progress
 K ^XTMP("BSTSLCMP")
 ;
 ;Force an update of the codesets
 F NMID=5190,1552,36 D QUEUE^BSTSVOFL(NMID)
 ;
 ;Kick of now
 D JOBNOW^BSTSVOFL
 ;
