APSSIN03 ;IHS/MSC/PLS - ScriptPro Interface;09-Mar-2023 11:17;DU
 ;;1.0;IHS SCRIPTPRO INTERFACE;**3**;January 11, 2006;Build 14
...
DATA ; This module holds the data that will be put into the database.
 ;
 ;Fill Type^7.1^^Z~S VAL=$S($G(RXF):"Refill",1:"Original fill")~1
 ;Returns the fill type ('O'riginal/'R'efill) for the dispense.
 ;FDA Med Guide^12.9^^Z~S VAL=''$L($$GET1^DIQ(50.68,$$GET1^DIQ(50,$$GETP(RX0,6),22,"I"),100))~1
 ;Returns boolean flag indicating if FDA Med Guide is available for the VA PRODUCT entry linked to the dispensed drug.
 ;Patient Temp Address^3.7^^Z~S VAL=$$PTMPADD^APSSLIC($P(RX0,U,2))~1
 ;Patient Temporary Address
 ;Prescription Expiration Date^35.1^52,26^Z~S VAL=$$FMTE^XLFDT($P(RX2,U,6),"5Z")
 ;Patient Mail Indicator^5.1^^Z~S VAL=$$GET1^DIQ(55,$P(RX0,U,2),.03)~1
 ;Sends value in Pharmacy Patient Mail field
 ;Dispensed Drug IEN^12.5^52,6^I
 ;Dispense Drug Tradename^12.6^^Z~N DATA,ERR D LIST^DIC(50.1,","_$P(RX0,U,6)_",",".01E","P",,,,,"I $P(^(0),U,3)=0",,.DATA,.ERR) S:$G(DATA("ORDER")) VAL=$P($G(@DATA@(1,0)),U,2)~1
 ;Returns the first alphabetically sorted Tradename for the drug.
 ;Prescription Remarks^12.7^52,12^~~1
 ;Returns the remarks field for prescription.
 ;Patient Preferred Name^3.8^^Z~S VAL=$$GETPREF^AUPNSOGI($$GETP(RX0,2),,1)
 ;Prescription IEN^1.7^^Z~S VAL=$G(RXIEN)~1
 ;Returns the internal entry number of the Prescription File entry.
 ;Station Number^1.6^^Z~S VAL=$P($$SITE^VASITE(),U,3)~1
 ;Returns the Station Number from the $$SITE^VASITE call.
 ;EPCS Digitally Signed^30.2^52,310^~~1
 ;Returns flag indicating if prescription order was digitally signed.
 ;Patient Cell Phone^3.9^^Z~S VAL=$$GET1^DIQ(2,$P(RX0,U,2),.134)
 ;
 ;
 ; Parameter 1 passed to APSSLIC routine represents the Provider Internal Entry Number
 ; Parameter 2 represents the processing method which is described below..
 ;
 ; Method 1
