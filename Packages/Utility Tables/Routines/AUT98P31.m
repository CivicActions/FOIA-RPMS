AUT98P31 ; IHS/OIT/FBD&NKD - AUT V98.1 PATCH 31 ENVIRONMENT CHECKS AND POST-INIT PROCESS ; 05/21/2021
 ;;98.1;IHS DICTIONARIES (POINTERS);**31**;NOV 25,1997;Build 6
 ;
 ;
 D:'$D(IOM) HOME^%ZIS
 ;
 ...
 S:'$$VCHK("AUT","98.1","30") XPDQUIT=2          ; IHS STANDARD DICTIONARIES
...
LAST(PKG,VER) ; EP - returns last patch applied for a Package, PATCH^DATE
 ...
 ;
PRE ; EP FR KIDS
 D BMES^XPDUTL("Beginning pre-install for AUT v98.1 patch 31.")
 ;
 ;N DIU,AUTDD
 ;D BMES^XPDUTL("Deleting data dictionaries of affected files...")
 ;K DIU S DIU(0)="T" F AUTDD=9999999.53 D MES^XPDUTL(" - File "_AUTDD) S DIU=AUTDD D EN^DIU2
 ;D ^XBFMK
 ;
 ;
 D BMES^XPDUTL("AUT v98.1 patch 31 pre-install complete.")
 Q
 ;
POST ; EP FR KIDS
 D BMES^XPDUTL("Beginning post-install for AUT v98.1 patch 31.")
 ;
 D BMES^XPDUTL("Delivering "_$G(XPDNM)_" install message to select users...")
 D MAIL
 D BMES^XPDUTL("AUT v98.1 patch 31 post-install complete.")
