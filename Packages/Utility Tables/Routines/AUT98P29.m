AUT98P29 ; IHS/OIT/FBD&NKD - AUT V98.1 PATCH 29 ENVIRONMENT CHECKS AND POST-INIT PROCESS ; 02/25/2020
 ;;98.1;IHS DICTIONARIES (POINTERS);**29**;NOV 25,1997;Build 1
 ;
 ;
 D:'$D(IOM) HOME^%ZIS
 ;
 ...
 S:'$$VCHK("XU","8.0","1018") XPDQUIT=2          ; KERNEL
 S:'$$VCHK("DI","22.0","1018") XPDQUIT=2         ; FILEMAN
 S:'$$VCHK("AUT","98.1","28") XPDQUIT=2   ; AUT
...
LAST(PKG,VER) ; EP - returns last patch applied for a Package, PATCH^DATE
 ...
 ;
PRE ; EP FR KIDS
 D BMES^XPDUTL("Beginning pre-install for AUT v98.1 patch 29.")
 ;
 N DIU,AUTDD
 D BMES^XPDUTL("Deleting data dictionaries of affected SOGI files...")
 K DIU S DIU(0)="T" F AUTDD=9999999.103,9999999.104,9999999.105 D MES^XPDUTL(" - File "_AUTDD) S DIU=AUTDD D EN^DIU2
 D ^XBFMK
 ;
 N DIK,DA
 D BMES^XPDUTL("Replacing SEXUAL ORIENTATION entries...")
 I $D(^AUTTSO) K ^AUTTSO("B"),^AUTTSO("C") S DIK="^AUTTSO(",DA=0 F  S DA=$O(^AUTTSO(DA)) Q:'DA  D ^DIK
 D BMES^XPDUTL("Replacing GENDER IDENTITY entries...")
 I $D(^AUTTGI) K ^AUTTGI("B"),^AUTTGI("C") S DIK="^AUTTGI(",DA=0 F  S DA=$O(^AUTTGI(DA)) Q:'DA  D ^DIK
 D BMES^XPDUTL("Replacing PRONOUNS entries...")
 I $D(^AUTTPRN) K ^AUTTPRN("B"),^AUTTPRN("C") S DIK="^AUTTPRN(",DA=0 F  S DA=$O(^AUTTPRN(DA)) Q:'DA  D ^DIK
 ;
 D BMES^XPDUTL("AUT v98.1 patch 29 pre-install complete.")
 Q
 ;
POST ; EP FR KIDS
 D BMES^XPDUTL("Beginning post-install for AUT v98.1 patch 29.")
 ;
 D BMES^XPDUTL("Delivering "_$G(XPDNM)_" install message to select users...")
 D MAIL
 D BMES^XPDUTL("AUT v98.1 patch 29 post-install complete.")
