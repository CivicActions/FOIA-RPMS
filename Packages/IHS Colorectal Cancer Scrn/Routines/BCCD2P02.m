BCCD2P02 ;GDIT/HS/GCD-Post-Install; 22 Oct 2022  3:02 PM
...
POST ;
 ...
 ;
 ; Check nightly upload time. If it's between 13:00 and 17:59 (inclusive) prompt user to change it.
 S DA=$O(^BCCDS(90310.01,0))
 I DA S TIME=$TR($P($G(^BCCDS(90310.01,DA,0)),"^",6),":")
 I $G(TIME)'="",TIME>1259,TIME<1800 D  I $D(Y) D ERRMSG("  Post-install aborted.") G Q
 . D BMES^XPDUTL("The nightly task is currently scheduled to run during business hours. It is recommended that this run after hours.")
 . S DR=".06"
 . S DIE="^BCCDS(90310.01," D ^DIE
 ;
 ; Delete obsolete classes
 F I="DischargeInstrucions","DischargeSummary","HistoryAndPhysical","HospitalCourse","ProcedureNote","Progress" D
 . S CLASS="BCCD.Xfer.Pop"_I
 . X "if ##class(%ClassDefinition).%ExistsId("""_CLASS_""") D BMES^XPDUTL(""  Removing "_CLASS_""") do ##class(%ClassDefinition).%DeleteId("""_CLASS_""")"
 ;
 ; Import BCCD classes
