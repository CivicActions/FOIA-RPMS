BCCD1P01 ;GDIT/HS/GCD-Post-Install; 18 Apr 2013  12:51 PM
 ;;1.0;CCDA;**1**;Feb 21, 2014;Build 13
 Q
POST ;
 ...
 I $G(DT)="" D DT^DICRW
 ;
 ; Clear provider class data from BCCD MESSAGE TYPE, if it exists
 D BMES^XPDUTL("  Removing obsolete PROVIDER CLASS data from BCCD MESSAGE TYPE record")
 S DA(1)=$O(^BCCDS(90310.02,"B","CCD",""))
 I 'DA(1) D BMES^XPDUTL("  Unable to find BCCD MESSAGE TYPE record.  Install aborted.") G Q
 S DIK="^BCCDS(90310.02,"_DA(1)_",3,",DA=0
 F  S DA=$O(^BCCDS(90310.02,DA(1),3,DA)) Q:'DA  D ^DIK
 ;
 ...
 . H 1
 I I=121 D BMES^XPDUTL("  Could not stop CCDA jobs.  Install aborted.") G Q
 ;
 ; Check for locks to verify that the tasks are stopped.
 I $$LOCK^BCCDPAT("^BCCDTSK")!$$LOCK^BCCDPAT("^BCCDPUSH") D BMES^XPDUTL("  Could not stop CCDA jobs.  Install aborted.") G Q
 ;
 ; Import BCCD classes
 D BMES^XPDUTL("  Import BCCD package")
 K ERR
 S TRNM=$G(@XPDGREF@("SENT REC"))
 I TRNM="" D BMES^XPDUTL("  Error occurred duing import.  Install aborted.") G Q
 S TRIEN=$O(^BCCDCLS("B",TRNM,""))
 I $G(TRIEN)'="" D IMPORT^BCCDCLAS(TRIEN,.ERR)
 I $G(ERR) D BMES^XPDUTL("  Error occurred duing import. Error: "_ERR_".  Install aborted.") G Q
 ;
 ; Exit Entry point
Q ;
