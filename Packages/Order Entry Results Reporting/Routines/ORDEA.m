ORDEA ;ISL/TC & JMH & JLC - DEA related items ;11-Jul-2019 17:14;PLS
 ;;3.0;ORDER ENTRY/RESULTS REPORTING;**306,1017**;Dec 17, 1997;Build 48
 ;
 ;
 ;IHS/MSC/MGH routine from CPRS 29 included for EPCS
 ;
...
SIGINFO(ORY,ORDFN,ORPROV) ;returns the provider/patient info that must be displayed when signing controlled substance orders
 ...
 ;S ORI=ORI+1,ORY(ORI)="DEA: "_$$DEA^XUSER(,ORPROV)
 S STR="DEA: "_$$DEA^XUSER(,ORPROV)
 I $L(ORDETOX)>0 S STR=STR_"  "_"DEA X: "_ORDETOX
 S ORI=ORI+1,ORY(ORI)=STR
...
BUILDFDA(ORIFN,ORDFDA,OROUT,HASH,OHD) ;
 ...
 . ;E  S OROUT(3)=OROUT(3)_A  ;IHS/MSC/PLS - 10/25/2017
 . E  S OROUT(3)=OROUT(3)_"~"_A
 ...
 S DFN=+$$GET1^DIQ(100,ORIFN_",",.02,"I")
 I DUZ("AG")'="I" S ORDFDA(101.52,IENS,20)=$$GETICN^MPIF001(DFN)
...
BUILD(ORIFN) ;Build ARCHIVE entry for CPRS order number
 N ORDFDA,OROUT,ERROR               ;IHS/GDIT/MSC/MGH new variables used in update call
