ORDEA01A ;ISP/RFR - DEA REPORTS 01;21-Sep-2018 17:25;DU
 ;;3.0;ORDER ENTRY/RESULTS REPORTING;**218,1017**;Dec 17, 1997;Build 48
 ;Modified - IHS/MSC/PLS - 03/26/2018 - Line INCOMPL+5
...
INCOMPL ;INCOMPLETE PROVIDER SETUP REPORT
 ...
 ;IHS/MSC/PLS - 03/26/2018
 ;W !,"By default, the report lists the prescribable schedules for each provider",!
 W !,"The report lists the prescribable schedules for each provider",!
...
INCOMPLQ ;TASKMAN ENTRY POINT
 ...
 .D VRFYPHSH^BEHOEP5(.CHK,Y)
 .I CHK=0 S RETURN("Provider hash is good")=""
 .I CHK=1 S RETURN("Provider hash does not exist or has been tampered")=""
 .I $D(RETURN)>9 D
 ..N REASON,NAME,LINE
...
DETOX ;DETOX/MAINTENANCE ID Report
 ...
 W !!,"This report identifies all users who have a DEA X number",!
...
DETOXQ ;TASKMAN ENTRY POINT
 ...
 ...S STOP=$$HEADER^ORUTL("PROVIDERS WITH A DEA X ID",.PGNUM,.COL)
 ..W !,$$CJ^XLFSTR("*** DISUSERED/TERMINATED USERS ***",$S(+$G(IOM)>0:(IOM-1),1:79)," "),!
 ...
 .I ($Y+2)>IOSL S STOP=$$HEADER^ORUTL("PROVIDERS WITH A DEA X ID",.PGNUM,.COL)
 .Q:$G(STOP)
 .W !,$$CJ^XLFSTR("[END OF REPORT]",$S(+$G(IOM)>0:(IOM-1),1:79)," ")
 K @DOCS,@DATA
 S:$D(ZTQUEUED) ZTREQ="@"
