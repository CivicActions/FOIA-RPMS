ACPT21RH ;IHS/SD/RNB - ACPT*2.10  2009 reload ; 04 Nov 2009 10:47 AM
 ;;2.10;CPT FILES;**2**;DEC 18, 2009
 ;
 Q  ;
IMPHCPCS ;  this tag will load the complete file into ^TMP("ACPT-IMP",$J) using the concept ID
 ...
 N POP D  Q:POP
 .D OPEN^%ZISH("CPTHFILE",ACPTPTH,"acpt2010.9h","R")  ; open read-only
 .U IO(0)  ; use terminal
...
CLNSTR ; Clean string of control characters
 D CLEAN^ACPT210A(.ACPTSTRX,.ACPTMAPV,0) ; strip out the control characters
 U IO(0) D MES^XPDUTL("From acpt2010.9h"_"="_ACPTMAPV_"...") W ! ; show the problem (safely)
 Q
LOADCODE ; load CPTs from ^TMP("ACPT-HCPCS",$J)
 ...
 ..;S ($P(^ICPT(ACPTIEN,0),U,8),$P(^ICPT(ACPTIEN,9999999),U,6))=ACPTYR  ; Date Added (7) to 3090000
 ..S DIC("DR")="8////"_ACPTYR_";9999999.06////"_ACPTYR
 ..;the below codes are effective 4/1/09
 ..;S:ACPTCODE="K0739"!(ACPTCODE="K0740")!(ACPTCODE="S3865")!(ACPTCODE="S3866")!(ACPTCODE="S3870") ($P(^ICPT(ACPTIEN,0),U,8),$P(^ICPT(ACPTIEN,9999999),U,6))=3090401
 ..S:ACPTCODE="K0739"!(ACPTCODE="K0740")!(ACPTCODE="S3865")!(ACPTCODE="S3866")!(ACPTCODE="S3870") DIC("DR")="8////3090401;9999999.06////3090401"
 ..S DIC="^ICPT("
 ..S DIC(0)="L"
 ..D ^DIC
 .;
 .;S ACPTNODE=$G(^ICPT(ACPTIEN,0))  ; get record's header node
 .S ACPTSHRT=$$CLEAN^ACPT210L($P($G(^TMP("ACPT-HCPCS",$J,ACPTACT,ACPTCODE)),U))  ; clean up the Short Name
 ...
 ..D ^DIE
 .;
 .;S ($P(ACPTNODE,U,7),$P(^ICPT(ACPTIEN,9999999),U,7))=""  ; clear Date Deleted field (8)
 ...
 .S ACPTDESC=$$CLEAN^ACPT210L($P($G(^TMP("ACPT-HCPCS",$J,ACPTACT,ACPTCODE)),U,2))  ; clean up the Description
 .D TEXT^ACPT210L(.ACPTDESC) ; convert string to WP array
 ...
 .;
 .S ACPTSHRT=$$CLEAN^ACPT210L($P($G(^TMP("ACPT-HCPCS",$J,"M",ACPTCODE)),U),1)  ;Short desc
 ...
 .S ACPTDESC=$$CLEAN^ACPT210L($P($G(^TMP("ACPT-HCPCS",$J,"M",ACPTCODE)),U,2))  ;Long Desc
 .D TEXT^ACPT210L(.ACPTDESC) ; convert string to WP array
...
DELCODE ;
 ...
 .; Date Deleted and Inactive Date to 3081231
 .S ($P(^ICPT(ACPTIEN,9999999),U,7),$P(^ICPT(ACPTIEN,0),U,7))=ACPTYR
 .S:ACPTCODE="S8190" ($P(^ICPT(ACPTIEN,0),U,7),$P(^ICPT(ACPTIEN,9999999),U,7))=3090401
 .;
