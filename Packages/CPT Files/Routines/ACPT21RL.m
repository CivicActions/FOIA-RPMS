ACPT21RL ;IHS/SD/RNB - ACPT*2.10 2009 reload ; 04 Nov 2009  10:47 AM
...
IMPCPT ;  this tag will load the complete file into ^TMP("ACPT-IMP",$J) using the concept ID
 ...
 N POP D  Q:POP
 .D OPEN^%ZISH("CPTHFILE",ACPTPTH,"acpt2010.9l","R")  ; open read-only
 .U IO(0)  ; use terminal
 ...
 .K ACPTX,ACPTY,ACPTZ S ACPTZ=0 F ACPTX=1:1:$L(ACPTLINE) S ACPTY=$E(ACPTLINE,ACPTX) I ($A(ACPTY)<32)!($A(ACPTY)>126) S ACPTZ=1 Q
 .I ACPTZ D  ;Clean control characters
 ..D CLEAN^ACPT210A(.ACPTLINE,.ACPTMAPV,0) ; strip out the control characters
 ..U IO(0) D MES^XPDUTL("From acpt2010.9l"_"="_ACPTMAPV_"...") W ! ; show the problem (safely)
 ..Q
 .;
...
LOADCODE ; load CPTs from ^TMP("ACPT-IMP",$J)
 ...
 .;S ($P(^ICPT(ACPTIEN,9999999),U,6),$P(^ICPT(ACPTIEN,0),U,8))=ACPTYR  ;Date Added (7) to 3090101
 ...
 S ACPTSHRT=$$UP^XLFSTR($TR($$CLEAN^ACPT210L($G(^TMP("ACPT-IMP",$J,ACPTFIEN,111,1))),";",","))  ; clean up the Short Name
 ...
 S ACPTDESC=$$UP^XLFSTR($$CLEAN^ACPT210L(ACPTDESC))  ; clean up the Description
 ...
 .S X="01/01/2010"  ; new entry for 1/1/2010
...
DELETE ;  this tag will load the complete file into ^TMP("ACPT-DEL",$J) using the concept ID
 ...
 N POP D  Q:POP
 .D OPEN^%ZISH("CPTHFILE",ACPTPTH,"acpt2010.9d","R")  ; open read-only
 .U IO(0)  ; use terminal
...
DELCODE ;
 ...
 .S X="12/31/2009"  ; value to lookup in the subfile
 ...
 .S DR="7////3091231;9999999.07////3091231"
 .D ^DIE
 .;
 Q
 ;
