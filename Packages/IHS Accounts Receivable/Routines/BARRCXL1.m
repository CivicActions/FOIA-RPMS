BARRCXL1 ; IHS/SD/LSL - Cancelled Bills Report - Gather Data ;05/05/2020
 ;;1.8;IHS ACCOUNTS RECEIVABLE;**7,19,29,30**;OCT 26, 2005;Build 55
 ;
 ; IHS/SD/PKD - 05/07/10 - BAR*1.8.19
 ; IHS/SD/LSL - 03/10/03 - Routine created
 ;      Called by BARRCXL
 ; MODIFIED XTMP FILE NAME TO TMP TO MEET SAC REQUIREMENTS;MRS:BAR*1.8*7 IM29892
 ; IHS/SD/CPC - 20200505 - BAR*1.8*30 CR9476
 Q
 ; *********************************************************************
 ;
 ;
...
DATA ; EP
 ...
 ;
 I BARY("OBAL")=1,(+$FN(BARBAL,"-")<.01) Q  ;IHS/SD/CPC - BAR*1.8*30 CR9476 Some sites have messed up "OBAL" indexes.
 I BARY("RTYP")=1 D DETDATA
 I BAR("XML")=1 D DETDATA       ;IHS/SD/CPC - BAR*1.8*30 CR9476
 D SUMDATA
 Q
 ; *********************************************************************
DETDATA ;
 ...
 N BARSORTNM S BARSORTNM=$S(BARY("SORT")="V":$$GET1^DIQ(9002274.8,BARSORT,.01,"E"),1:$$GET1^DIQ(40.7,BARSORT,.01,"E"))
 N HRN S HRN=$P(BARBILL,"-",3) I 'HRN S HRN=$P(BAR(1),"^",7)   ;//TODO - CONFIRM PIECE THREE OF THE BILL NUMBER IS ALWAYS HRN
 I HRN="" S HRN="***** "
 ...
 S ^TMP($J,"BAR-CXL XML DET",BARBCANC,BARLOC,BARSORT,BARPAT,BARBILL)=""
 S ^TMP($J,"BAR-CXL XML DET",BARBCANC,BARLOC,BARSORT,BARPAT,BARBILL,1)=BARBCANC
 S ^TMP($J,"BAR-CXL XML DET",BARBCANC,BARLOC,BARSORT,BARPAT,BARBILL,2)=BARLOC
 S ^TMP($J,"BAR-CXL XML DET",BARBCANC,BARLOC,BARSORT,BARPAT,BARBILL,3)=$$GET1^DIQ(9002274.8,BARBVSTY,.01) ;BARSORTNM
 S ^TMP($J,"BAR-CXL XML DET",BARBCANC,BARLOC,BARSORT,BARPAT,BARBILL,4)=BARPAT
 S ^TMP($J,"BAR-CXL XML DET",BARBCANC,BARLOC,BARSORT,BARPAT,BARBILL,5)=HRN
 S ^TMP($J,"BAR-CXL XML DET",BARBCANC,BARLOC,BARSORT,BARPAT,BARBILL,6)=BARACCT
 S ^TMP($J,"BAR-CXL XML DET",BARBCANC,BARLOC,BARSORT,BARPAT,BARBILL,7)=BARBILL
 S ^TMP($J,"BAR-CXL XML DET",BARBCANC,BARLOC,BARSORT,BARPAT,BARBILL,8)=BAR("D")
 S ^TMP($J,"BAR-CXL XML DET",BARBCANC,BARLOC,BARSORT,BARPAT,BARBILL,9)=BARBREAS
 S ^TMP($J,"BAR-CXL XML DET",BARBCANC,BARLOC,BARSORT,BARPAT,BARBILL,10)=BARBAMT
 S ^TMP($J,"BAR-CXL XML DET",BARBCANC,BARLOC,BARSORT,BARPAT,BARBILL,11)=BARBAL
 S ^TMP($J,"BAR-CXL XML DET",BARBCANC,BARLOC,BARSORT,BARPAT,BARBILL,12)=$$GET1^DIQ(40.7,BARBCLNC,.01)  ;CLINIC NAME
 S ^TMP($J,"BAR-CXL XML DET",BARBCANC,BARLOC,BARSORT,BARPAT,BARBILL,13)=BAR3PLOC  ; 3P DUZ(2),3P IEN 
 ; END 1.8*19
 Q
 ; ********************************************************************
 ;
