BARUFEX5 ; IHS/SD/TPF - CONTINUATION OF EXPORT -- BUILD BOB FILE ; 09/17/2008
 ;;1.8;IHS ACCOUNTS RECEIVABLE;**4,5,6,7,23,34**;OCT 26, 2005;Build 139
 ;IHS/SD/MRS 1.8*4 NEW ROUTINE; ITEM 1 OF SCR58 1/2/2008 DD ITEM 4.1.1
 ;IHS/SD/POT HEAT54733 APR 2012 fix <SUBSCRIPT>SETORIG+2^BARUFEX5
 ;IHS/SD/POT HEAT93190 DEC 2012 NOHEAT MARK/IGNORE DUPLICATE GHOST BILLS (SAME PAT, BUT MISSING CLAIM IN 3PB)
 ;IHS/SD/POT NOHEAT MAR 2013 ADDED NEW VA billing 
 ;IHS/SD/SDR 1.8*34 ADO78239 Made it to look at the first transaction that's not IGNORED when finding the oldest
 ;   transaction for all the pairing that's done afterwards
 ;
PRE(BARBG,BARDUZ) ;EP;
 ;
...
BARBEG(X) ;SET BEGINING DATE FROM NOT SET CROSS-REFERENCE
 ;
 N Z
 ;S:BARBG="" BARBG=3070930  ;bar*1.8*34 IHS/SD/SDR ADO78239
 ;S Z=$O(^BARSESS(X,"NS",""))  ;bar*1.8*34 IHS/SD/SDR ADO78239
 ;I Z=""!(Z>BARBG) S Z=BARBG  ;IF NO NS X-REF, OR BARBG<NS, USE SESS DATE  ;bar*1.8*34 IHS/SD/SDR ADO78239
 ;start new bar*1.8*34 IHS/SD/SDR ADO78239
 S:BARBG="" BARBG=3080930
 N BFLG
 S BFLG=0
 S Z=0
 F  S Z=$O(^BARSESS(X,"NS",Z)) Q:'Z  D  Q:BFLG
 .I $P($G(^BARTR(DUZ(2),Z,1)),U,12)'="I" S BFLG=1  ;transaction isn't ignored, so use this as starting point for reversal matching
 I Z=""!(Z<BARBG) S Z=BARBG  ;IF NO NS X-REF, OR BARBG<NS, USE SESS DATE
 ;end new bar*1.8*34 IHS/SD/SDR ADO78239
 Q Z-1
