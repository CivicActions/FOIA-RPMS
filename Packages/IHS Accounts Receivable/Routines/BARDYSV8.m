BARDYSV8 ; IHS/SD/TPF - DAYS IN A/R REPORT CALLS ; 
 ...
 ;IHS/SD/SDR 1.8*33 ADO60817 Changed to use .011 for date/time instead of .01 which is now date/time/counter
 Q
FIRLSTPY(BILL3P) ;EP - FIND FIRST-LAST PAYMENT MADE IN TRANS FILE
 ...
 ..;S FIRST=$$GET1^DIQ(90050.03,TRANIEN_",",.01,"I")  ;bar*1.8*33 IHS/SD/SDR ADO60817
 ..S FIRST=$$GET1^DIQ(90050.03,TRANIEN_",",.011,"I")  ;bar*1.8*33 IHS/SD/SDR ADO60817
 ...
 ..;S LAST=$$GET1^DIQ(90050.03,TRANIEN_",",.01,"I")  ;last pymt wasn't populated if only one bill with one pymt  ;bar*1.8*28 CR8345 HEAT224215  ;bar*1.8*33 IHS/SD/SDR ADO60817
 ..S LAST=$$GET1^DIQ(90050.03,TRANIEN_",",.011,"I")  ;last pymt wasn't populated if only one bill with one pymt  ;bar*1.8*33 IHS/SD/SDR ADO60817
 ..;
 ...
 ..;end old bar*1.8*28 CR8345 HEAT224215
 ..;S LAST=$$GET1^DIQ(90050.03,TRANIEN_",",.01,"I")  ;bar*1.8*33 IHS/SD/SDR ADO60817
 ..S LAST=$$GET1^DIQ(90050.03,TRANIEN_",",.011,"I")  ;bar*1.8*33 IHS/SD/SDR ADO60817
 S:$G(FIRST)="" FIRST=DT
 S:$G(LAST)="" LAST=DT
