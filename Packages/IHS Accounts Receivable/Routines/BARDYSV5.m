BARDYSV5 ; IHS/SD/MAS,TPF - OMB - DAYS TO COLLECTION ; 02/09/2009
 ;;1.8;IHS ACCOUNTS RECEIVABLE;**12,13,14,16,23,33**;OCT 26, 2005;Build 133
 ;
 ...
 ;IHS/SD/TMM 01/29/2010 M3 Tag TRANS moved from ^BARDYSV3 to ^BARDYSV5 due to routine size of ^BARDYSV3 and SAC checker requirement
 ...
 ;IHS/SD/SDR 1.8*33 ADO60817 Updated loop thru A/R Trans file to use 'C' xref instead of relying on DINUM
 Q
 ;
...
TRANS(BARBEG,BAREND) ;EP - Find the A/R Transactions for this period   ;M3*ADD*TMM
 ...
 N CNT
 ;S BARTRIEN=BEGIN F CNT=1:1 S BARTRIEN=$O(^BARTR(DUZ(2),BARTRIEN)) Q:('BARTRIEN)!(BARTRIEN>END)  D  ;A/R TRANS DT/TM  ;bar*1.8*33 IHS/SD/SDR ADO60817
 ;start new bar*1.8*33 IHS/SD/SDR ADO60817
 ;note - new code includes new line tag TRANS2 to minimize amount of changes
 S BARBDT=BEGIN
 F CNT=1:1 S BARBDT=$O(^BARTR(DUZ(2),"C",BARBDT)) Q:('BARBDT)!(BARBDT>END)  D TRANS2
 Q
TRANS2 ;EP
 S BARTRIEN=0
 F  S BARTRIEN=$O(^BARTR(DUZ(2),"C",BARBDT,BARTRIEN)) Q:'BARTRIEN  D
