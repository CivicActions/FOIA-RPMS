BARRUTL ; IHS/SD/LSL - Report Utility ; 07/26/2010
 ;;1.8;IHS ACCOUNTS RECEIVABLE;**6,19,33**;OCT 26, 2005;Build 133
 ;
 ;IHS/ASDS/LSL 08/29/00 Routine created
 ;IHS/SD/LSL 1.6*2 04/12/02 Modified LOOP line tag to allow it to be used for new Age Summary Report
 ;
 ;IHS/SD/LSL 1.7 10/03/02 NDA-0902-180080 Modified LOOP lne tag to allow reports to sort by date correctly.
 ;   Reports show ** NO DATA TO PRINT **
 ;IHS/SD/LSL 1.7 12/06/02 NHA-0601-180049 Modified to find bill in 3P correctly.
 ;      
 ;IHS/SD/TMM 1.8*19 7/26/10 Select by Group Plans
 ;IHS/SD/SDR 1.8*33 ADO60817 Use 'C' xref to check date range; 'B' xref is now DATE/TIME/CNTR
 ;
 Q
 ;***************************************************
 ;
...
TRANS ;EP for Looping thru Transaction File
 ;S BARP("X")=$S($G(BARY("DT"))="T":"B",1:1)  ;bar*1.8*33 IHS/SD/SDR ADO60817
 S BARP("X")=$S($G(BARY("DT"))="T":"C",1:1)  ;bar*1.8*33 IHS/SD/SDR ADO60817
