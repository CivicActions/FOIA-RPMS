BAREDP00 ; IHS/SD/LSL - AR ERA AUTO-POSTIEG ; 01/30/2009
 ;;1.8;IHS ACCOUNTS RECEIVABLE;**1,5,6,7,10,17,20,21,23,29,31,37**;OCT 26,2005;Build 210
 ;IHS/SD/POT 1.8*23 JAN2013 AUTO-Transport TYPE
 ;IHS/SD/FCJ 1.8*31 CR#6984 REQ2;display claim info OR "."
 ;IHS/SD/SDR 1.8*37 ADO60825 Make sure cashiering session stays open; split to BAREDP0B due to routine size
 Q
TSEL ;EP TRANSPORT SELECT
 W !
 K DIC,DA
 S DIC=$$DIC^XBDIQ1(90056.01),DIC(0)="AEQM"
 K DD,DO
 D ^DIC
 S TRDA=+Y,TRNAME=$P(Y,U,2)
 Q
INSTALL ;EP -Load New file;4010-5010 TYPES
 D SIG^XUSESIG
 Q:X1=""
 I $G(DUZ(2))="" D  Q
 .W !!,"Check your DUZ setup." H 4
 I '$D(^BAREDI("I",DUZ(2),0)) S ^BAREDI("I",DUZ(2),0)="A/R EDI IMPORT^90056.02^^^"
 K ^TMP($J,"ERA")
 S DIR(0)="F",DIR("A")="Enter the directory path for the transport file"
 S BARPATH=$P($G(^BAR(90052.06,DUZ(2),DUZ(2),0)),U,17)
 S DIR("B")=BARPATH,DIR("?")="For example enter '/usr/mydir/'"
 D ^DIR
 K DIR
 Q:$D(DIRUT)
 ;Path
 ...
 S BARCTRL=0
 S POP=0 D READ(XBDIR,HSTFILE) I POP Q
 S BARRET=$$FORMAT(BARREC) I +BARRET D  Q
 ...
 Q
4010 ;
 D 4010^BAREDP0B  ;split routine due to size bar*1.8*37
 I X'>0 G INSTALL
 K DIC
 S DIC=$$DIC^XBDIQ1(90056.02)
 S DIC(0)="EL"
 S X=HSTFILE
 S:$G(FILE) DINUM=($P(FILE,"_")-1000)
 K DD,DO D FILE^DICN
 S IMPDA=+Y
 K DIC
 I +Y<1 D  Q
...
REDO ;EP for mid-stream testing
 S SUCC=""
...
PLB ;Chk for PLB/Pymt Reversals
 S BARPTYP="M"  ;BAR*1.8*31;OIT.IHS.FCJ CR#6984 REQ5;TYPE MATCH
 D SELFL  ;Select file
 I Y'>0 Q
 ;BAR*1.8*31;OIT.IHS.FCJ CR#6984 REQ2 ST OF CHG
 S BARDSP=1,BARDSPC=1
 K DIR
 S DIR(0)="Y^A",DIR("B")="YES",DIR("A")="Display Bill Matching Details"
 D ^DIR K DIR
 S:Y=0 BARDSP=0
 ;BAR*1.8*31;CR#6984 REQ2 END OF CHG
 ...
 D ^DIE
 K DIR
 S DIR(0)="E",DIR("A")="<CR> - Continue"
 D ^DIR
 D CLEAR^VALM1
 D CLNUP
 Q
FORMAT(BARREC) ;
 ;Verify file 835 if Transport HIPAA 835
 ...
 S BARTMP=BARREC
 I $E(BARTMP,1,3)'="ISA" S BARERR=1 Q BARERR  ;X12 msg st w/ISA
...
REVIEW ;EP
 ...
 .D EOP^BARUTL(1)
 S BARDSP=1,BARPTYP="M"  ;BAR*1.8*31;OIT.IHS.FCJ CR#6984 REQ2-REQ5
 D SELFL
 I Y'>0 Q
 ...
 .S DR=".05///^S X=BARDTREV",DR=DR_";.06////^S X=DUZ"
 .D ^DIE
 D CLNUP
 Q
POST ; EP
 ...
 S BARDSP=1,BARPTYP="P"  ;BAR*1.8*31;OIT.IHS.FCJ CR#6984 REQ5;TYPE POST
 D SELFL
 ;I Y'>0 Q  ;bar*1.8*37 IHS/SD/SDR ADO60825
 I Y'>0 S ^BARTMP("UFMSPST",DUZ(2),DUZ,UFMSESID)=+$G(^BARTMP("UFMSPST",DUZ(2),DUZ,UFMSESID))-1 Q  ;bar*1.8*37 IHS/SD/SDR ADO60825
 ;I TRNAME[("5010") D POST^BAR50P00 Q  ;bar*1.8*37 IHS/SD/SDR ADO60825
 I TRNAME[("5010") D POST^BAR50P00 S ^BARTMP("UFMSPST",DUZ(2),DUZ,UFMSESID)=+$G(^BARTMP("UFMSPST",DUZ(2),DUZ,UFMSESID))-1 Q  ;bar*1.8*37 IHS/SD/SDR ADO60825
 I TRNAME["HIPAA" D  Q
 ...
 .D BTCHDISP
 .W !
 .S DIR(0)="Y",DIR("A")="Do you want to select a different batch",DIR("B")="N"
 ...
 I +$G(BARCOL),+$G(BARITM)
 E  D
 .W !,"Batch & Item not selected ",!,"Adjustments only will be made,",!! H 2
 .K BARCOL,BARITM
 .K DR,DIE,DA,DIC
 .S DIE=$$DIC^XBDIQ1(90056.02)
 .S DA=IMPDA
...
POSTA ;EP  POST
 ...
 S DIR("B")="N",DIR("?")="Enter 'Y' to load matched claims: "
 D ^DIR K DIR
 Q:$D(DIRUT)
 I X="Y" D
...
SELFL ;Select file
 ...
 W !
 S DIC(0)="AEZQM",DIC("A")="Select file: "
 ;S DIC("S")="I $$NEWFILE^BAREDP00(Y)" ;only show files newer than 12/20/2013 P.OTT TEST
 K DD,DO
 D ^DIC
 I Y'>0 Q
 ...
 S TRDA=$P(Y(0),U,3)
 S HSTIME=$P(Y(0),U,2)
 ;RPTLOOK set in REPORT tag; allow rpts on batches > 3qtrs old
 S HSTFILE=$P(Y(0),U,5)
...
NEWFILE(Y) ;
 ...
 S X=$P(X,"^",1) I X="" Q 1
 S %DT="" D ^%DT
 S X1=DT,X2=Y D ^%DTC I X>365 Q 0
 Q 1
SELCK ;
 D SELCK^BAREDP0A
 Q
...
READ(BARPATH,BARFILE) ; EP
 ...
 .S BARDONE=1
 .D EOP^BARUTL(1)
 S BARCNT=0,BARI=0 ;# DELIMITERS
 F I=1:1 Q:BARCNT=3  D
 .U IO READ *CH
 .I CH=10 Q
 .I CH=13 Q
 .S BARI=BARI+1 I BARI=106 S BARDEL=$C(CH),BARCNT=1 ;DELIMITER
