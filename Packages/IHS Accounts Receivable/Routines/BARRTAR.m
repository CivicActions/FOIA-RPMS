BARRTAR ; IHS/SD/LSL - Transaction report ; 08/20/2008
 ;;1.8;IHS ACCOUNTS RECEIVABLE;**6,7,21,35**;OCT 26, 2005;Build 187
 ;IHS/ASDS/LSL 10/02/00 Routine created
 ;IHS/SD/LSL 1.6*2 04/19/02 Modified to accomodate new "Location to sort report by" parameter
 ;
 ;IHS/SD/LSL 1.7 10/24/02 PAB-1002-90130 Modified to insert DUZ(2) as subscript in data global. Needed to
 ;   pull correct Collection Batch and Item from A/R Global during print due to "location to sort report by" parameter edits.
 ;IHS/SD/LSL 1.7*2 06/20/03 IM10890 Remove tran type 115 (coll batch to acct post) from report
 ;
 ;IHS/SD/MRS 1.8*7 IM29892 MODIFIED XTMP FILE NAME TO TMP TO MEET SAC REQUIREMENTS
 ;IHS/SD/SDR 1.8*35 ADO76628 Added code to populate filename if tasked from RTSK
 Q
 ;*********************************************************************
 ;
EN ; EP
