BARRPAY2   ; IHS/SD/PKD - TOP PAYERS REPORT ; 07/2/2010
 ...
 ;IHS/SD/CPC - BAR*1.8*29 CR9027 Change Allowable to allowed adjustments + payments 
 Q
 ; *******************************
PRINT ; EP   - 
...
GRTOT S AMT="" F LIMIT=1:1:BAR("NBR TO PRINT") S AMT=$O(^TMP($J,"BAR-PAYS",AMT)) Q:AMT=""  D  Q:$G(BAR("F1"))
 ...
 ...W !,$P(PAYER,U),?32,$J($P(DATA,U),7),?40,$J($FN(-AMT,",",2),16),?59,$J($FN(($P(DATA,U,2)-AMT),",",2),16) ;IHS/SD/CPC - CR9027 Change Allowable to allowed adjustments + payments ;
 ...I $I(SUBTOT("CNT"),$P(DATA,U))  ; Increment totals
 ...I $I(SUBTOT("AMT"),-AMT)
 ...;I $I(SUBTOT("ALLOW"),$P(DATA,U,2))
 ...I $I(SUBTOT("ALLOW"),$P(DATA,U,2)-AMT) ;IHS/SD/CPC - CR9027 Change Allowable to allowed adjustments + payments ; 
 ...I $Y>(IOSL-5) D HD^BARRPSRB Q:$G(BAR("F1"))
 Q:$G(BAR("F1"))
 I $D(SUBTOT) D
 .W !!,"GRAND TOTALS",?33,$J($FN($G(SUBTOT("CNT")),","),6)
 .;W ?40,$J($FN($G(SUBTOT("AMT")),",",2),16),?59,$J($FN($G(SUBTOT("ALLOW")),",",2),16),!!
 .W ?40,$J($FN($G(SUBTOT("AMT")),",",2),16),?59,$J($FN(($G(SUBTOT("ALLOW"))-AMT),",",2),16),!! ;IHS/SD/CPC - CR9027 Change Allowable to allowed adjustments + payments ; 
...
PRTSBTL  ;
 ...
 ....W !,$P(PAYER,U),?32,$J($FN($P(DATA,U),","),7),?40,$J($FN(-AMT,",",2),16),?59,$J($FN(($P(DATA,U,2)-AMT),",",2),16)  ;IHS/SD/CPC - CR9027 Change Allowable to allowed adjustments + payments ;
 ...
 .W ?40,$J($FN($G(SUBTOTG("GRAMT")),",",2),16),?59,$J($FN(($G(SUBTOTG("GRALLOW"))+$G(SUBTOTG("GRAMT"))),",",2),16),!!  ;IHS/SD/CPC - CR9027 Change Allowable to allowed adjustments + payments ;
...
SORTSUB  ;
 ...
 W !,?5,"SUB-TOTALS",?33,$J($FN($G(SUBTOT("CNT")),","),6),?40,$J($FN($G(SUBTOT("AMT")),",",2),16),?59,$J($FN(($G(SUBTOT("ALLOW"))+$G(SUBTOT("AMT"))),",",2),16),! ;IHS/SD/CPC - CR9027 Change Allowable to allowed adjustments + payments ;
 K SUBTOT
 I $Y>(IOSL-5) D HD^BARRPSRB Q:$G(BAR("F1"))
 Q
