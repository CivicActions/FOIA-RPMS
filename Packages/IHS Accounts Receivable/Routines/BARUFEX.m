BARUFEX ; IHS/SD/TPF - MAIN EXTRACT RTN FOR UFMS ; 09/19/2008
 ;;1.8;IHS ACCOUNTS RECEIVABLE;**3,4,6,7,33**;OCT 26, 2005;Build 133
 ;Heavily modified to prevent duplicate records ;MRS:BAR*1.8*7 IM30562
 ;IHS/SD/MRS 1.8*7 GETDATA MOVED TO NEW ROUTINE BARUFEX1 created to meet SAC size requirements
 ;IHS/SD/MRS 1.8*7 IM30562 New functionality only enters through single PULLSESS EP
 ;IHS/SD/SDR 1.8*33 ADO60817 Updated to use 'C' xref since file is no longer DINUMed by date/time
 Q
 ;
...
EN(ARFROM,ARTO) ;EP - ENTRY TO MAIN UFMS EXTRACT. PULL ALL TRANSACTIONS W/IN DATE RANGE
 ...
 S RECORD=1,TOTAMT=0
 ;F  S TRDATE=$O(^BARTR(DUZ(2),TRDATE)) Q:'TRDATE!(TRDATE>ENDDATE)  D  ;bar*1.8*33 IHS/SD/SDR ADO60817
 ;.;D GETDATA(TRDATE,.RECORD,.TOTAMT)               ;MRS:BAR*1.8*7 IM30562
 ;.D GETDATA^BARUFEX1(TRDATE,.RECORD,.TOTAMT)  ;bar*1.8*33 IHS/SD/SDR ADO60817
 ;start new bar*1.8*33 IHS/SD/SDR ADO60817
 F  S TRDATE=$O(^BARTR(DUZ(2),"C",TRDATE)) Q:'TRDATE!(TRDATE>ENDDATE)  D
 .S BARTRDFN=0
 .F  S BARTRDFN=$O(^BARTR(DUZ(2),"C",TRDATE,BARTRDFN)) Q:'BARTRDFN  D
 ..D GETDATA^BARUFEX1(BARTRDFN,.RECORD,.TOTAMT)
