BARUFUT ; IHS/SD/TPF - UTILITIES FOR UFMS ; 09/19/2007
 ;;1.8;IHS ACCOUNTS RECEIVABLE;**3,4,6,7,13,21,22,23,24,26,29,37,38**;OCT 26,2005;Build 211
 ;IHS/SD/POT OCT2012 NOHEAT ALLOW RE-CONNECT TO LAST OPEN SESSION AFTER LOG-OFF OR SWITCHING FALICITIES
 ;IHS/SD/POT HEAT147572 ALLOW TRIBAL SITES ERA POSTING OF NEG BAL & CANCELLED BILLS
 ;IHS/SD/SDR 1.8*26 HEAT170856 Made correction to cancelled bills check
 ;IHS/SD/SDR 1.8*37 ADO60825 Moved where check is done for open/posting session
 ;IHS/SD/SDR 1.8*38 ADO118505 Switched line back from p37; it was causing tribal sites to have to open a cashiering session
 Q
 ;
NOTLOGIN() ;EP - DISPLAY NOT LOGGED IN MSG
 NEW BARTMP ;P.OTT
 S BARTMP=$$OPENSTAT^BARUFUT(DUZ) I BARTMP]"" S UFMSESID=BARTMP Q 0
 ;SET UFMSESID=0 FOR TRIBAL SITES W/SESSION TURNED OFF
 I $$GET1^DIQ(90052.06,DUZ(2)_",",1503,"I") S UFMSESID=0 Q 0
...
OUTORREC(SESSID) ;EP - ASK USER IF THEY WANT TO LOG OUT OR RECONCILE TRUE=LOGOUT
 ...
 D ^DIR
 S ANSWER=Y
 I +$G(^BARTMP("UFMSPST",DUZ(2),DUZ,UFMSESID))=0 K ^BARTMP("UFMSPST",DUZ(2),DUZ,UFMSESID)  ;bar*1.8*37 IHS/SD/SDR ADO60825
 I 'ANSWER D  Q 'ANSWER
 .S X=$$DELFLG^BARUFUT1(DUZ,UFMSESID)
 Q 0
 ;
FINDTRAN(TPBIEN,TRARRAY) ;EP - GIVEN A 3PBIEN FROM UFMS ERROR REPORT
 ...
 Q
 ;
ASKACT(UDUZ,SESSID,ACTION,MODE,ERASTAT) ;EP -ASK FOR ACTION TO TAKE. CALLED FROM BARUFLOG. ONLY ONE SESSION
ASKACT1 ;EP -
 N CURSTAT
 ...
 I CURSTAT="OPEN" I $$STILPOST^BARUFUT1(UDUZ) D  Q
 .I UDUZ=DUZ D
 ..W $$EN^BARVDF("HIN")
 ..W !,"It appears you are STILL LOGGED IN and may still be posting",!
 ..W $$EN^BARVDF("HIF")
 .I UDUZ'=DUZ D
 ..W $$EN^BARVDF("HIN")
 ..W !,"It appears the cashier is STILL LOGGED IN and may still be posting",!
 ..W $$EN^BARVDF("HIF")
 .D PAZ^BARRUTL
 ...
 .;start old bar*1.8*37 IHS/SD/SDR ADO60825
 ...
 .;end old bar*1.8*37 IHS/SD/SDR ADO60825
 .S DIR("A")="D/Delete"
 ...
 ..S VALIDCHK="I (U_""V""_U_""S""_U)'[(U_Y_U) W !,""Invalid Option. Try Again"" G ASKACT1"
 .S DIR("A")="Enter Action"
 ;IF MODE="OUT" THEN THIS WAS CALLED FROM SIGN OUT AND CASHIER IS
 ;IN PROCESS OF SIGNING OUT. JUST DISPLAY COUNTS AND LEAVE.
 I MODE="OUT" Q
 ...
 I +POSTING=1,(ACTION="RC") D  Q  ;IS USER STILL LOGGED ON OR POSTING?
 ...
 .D TRANSMIT(UDUZ,SESSID,ACTION)  ;MAKE SURE IT IS "T"
...
ASKFORRT ;EP - ASK FOR <RETURN>
 ...
 Q
 ;
SELTRAN(UDUZ,SESSID) ;EP - SELECT AND TRANSMIT ONE OR MORE TRANSACTIONS FROM SESSION
 ...
 D ONETRAN^BARUFEX(.TRANS,SESSID)
...
TRANSMIT(UDUZ,SESSID,ACTION) ;EP - TRANSMIT/RETRANSMIT SESSION
 ...
 I CURSTAT'="REVIEWED/APPROVED" D  Q
 .W !!,"SESSION STATUS MUST BE REVIEWED/APPROVED"
 .W !,"TO BE ABLE TO TRANSMIT THE SESSION!"
 .H 2
...
SETSESS(UDUZ,SESSID,BARSTAT) ;EP - SET SESSION STATUS 'BARSTAT'
 Q:$G(BARSTAT)=""
 S STATCHG=$$ADDSTAT(UDUZ,SESSID)  ;CREATE NEW STATUS CHANGE DATE/TIME
 I +STATCHG<1 D  Q 0
 ...
 Q 1
 ;
ADDSTAT(UDUZ,SESSID) ; EP - CREATE NEW STATUS CHANGE DATE/TIME
 K DIC,DIE,DR,DA,DIR
 D NOW^%DTC
 S X=%
 S DA(2)=UDUZ
...
TRANTRIG(UDUZ,SESSID,BARTRAN) ;EP - CREATE TRANSACTION ENTRY IN SESSION
 ...
 I $$CURSTAT(UDUZ,SESSID,"E")'="OPEN" Q 0  ;Check current session status
 ...
 Q:Y<0 0
 S RET=+Y
 ;PLACE ASUFACASUFAC3PIEN IN ITS FIELD
...
IHS(DUZ2) ;EP - RETURN TRUE IF IHS AFFILIATION
 ...
 ;NEW CODE :HEAT147572 
IHSNEGB(DUZ2) ;EP - Returns True if negative balance posting is NOT ALLOWED.  
 ;RETURNS 1 IF:
 ; (1) AFFILIATION=IHS
 ...
 ;;NEW CODE :HEAT147572 
 ;~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
IHSCANCB(DUZ2) ;EP - CHECK IF ERA POSTING CANCELLED BILLS IS ALLOWED RETURNS TRUE IF ALLOWED FALSE IF NOT
...
NOTOPEN(DUZ,UFMSESID) ;EP - IHS/SD/TPF BAR*1.8*21 8/3/2011 HEAT20490 EXIT CASHIER IF SESSION NOT OPEN
 ...
 Q:$G(UFMSESID)="" 1  ;UFMSESID NOT DEFINED THEN SESSION IS NOT OPEN  ;bar*1.8*37 IHS/SD/SDR ADO60825  ;bar*1.8*38 IHS/SD/SDR ADO118505 put this line back
 ;Q:(+$G(UFMSESID)=0) 1  ;UFMSESID NOT DEFINED THEN SESSION IS NOT OPEN  ;bar*1.8*37 IHS/SD/SDR ADO60825  ;bar*1.8*38 IHS/SD/SDR ADO118505 removed this line
