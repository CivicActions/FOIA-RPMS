BARPUCU ; IHS/SD/LSL - UNALLOCATED COMMAND PROCESSOR ; 06/09/2008
 ;;1.8;IHS ACCOUNTS RECEIVABLE;**4,6,19,21,33**;OCT 26, 2005;Build 133
 ;** posting utilities
 ;
 ;IHS/SD/LSL 09/23/02 1.6*3 HIPAA - Don't allow updating of other files if Adjustment Category
 ;   is PENDING or GENERAL INFORMATION.
 ;
 ;IHS/SD/LSL 11/27/02 V1.7 QAA-1200-130051 - Added quit logic if error in creating a transaction
 ;
...
FINISH ;
 ...
 .S DIC(0)="L"
 .S DLAYGO=90050
 .;L +^BARTR(DUZ(2)):2 F  D NOW^%DTC S X=% I '$D(^BARTR(DUZ(2),"B",X)) L -^BARTR(DUZ(2)) D ^DIC K DLAYGO Q  ;IHS/SD/SDR bar*1.8*33 ADO60909
 .S Y=$$NEW^BARTR()  ;bar*1.8*33 IHS/SD/SDR ADO60909
 .S BARSIB=+Y
