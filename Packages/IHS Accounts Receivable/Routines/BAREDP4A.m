BAREDP4A ; IHS/SD/SDR - MATCH REASONS AND CLAIMS - 2 ; 01/09/2009
 ;;1.8;IHS ACCOUNTS RECEIVABLE;**20,21,23,24,29**;OCT 26,2005;Build 66
 ...
 ;IHS/SD/CPC CR 6976 Match ERA RX numbers to RPMS Bills - BAR*1.8*29
 ;;
PAYER    ;EP
...
CLM(BAREIENS,BARBILL,BARX,BARMMFLG) ;EP
 ...
 .S BARBILL=$$GET1^DIQ(90050.01,BARBIEN,.01)  ;IHS/DIT/CPC BAR*1.8*29 CR 6982 & 10777
 .S BARFND=10
 .S BARX="B"
 .S BARTMP="1^"_BARBILL
 .;IHS/DIT/CPC BAR*1.8*29 CR 6982 & 10777
 .; I $D(^BARBL(DUZ(2),"G",BARBILL))!$D(^BARBL(DUZ(2),"G",+BARBILL)) S BARX="G" 
 .; S BARTMP=$$FINDING(BARBILL) I +BARTMP S BARX="G",(BARBILL)=$P(BARTMP,"^",2)
 .; S BARTMP=$$FINDING(BARBILL) I +BARTMP S BARBILL=$P(BARTMP,"^",2)
 ...
 ;
 ;S BARSAVE=+BARBILL
 S BARSAVE=BARBILL  ;IHS/DIT/CPC BAR*1.8*29 CR 6982 & 10777
 S BARBILL=+BARBILL_"-" ;2/12/2014  ;BAR*1.8*29 CR6976
...
CLM2(BARX,BARBILL,BARMMFLG) ;
 ...
 .I BARMMCNT(3) S BARMMFLG=3 ;Neither Amount or DOS match ;IHS/SD/CPC 11/11/19 CR 6979
 .Q
 Q
CLM3(BARFROM,BARBIEN,IMPDA,CLMDA,BARMMTYP) ;
 ...
 S BARDOS=$$Y2KD2^BARDUTL(BARDOS)  ;IHS/SD/CPC 11/11/19 CR 10407
 ...
 .QUIT  ;OK
 I ERADOSE]"",(BARDOSE]""),(+ERABILL=+BARAMT),(ERADOSE=BARDOSE) D  Q 1  ;Check for DOS end match IHS/SD/CPC 11/11/19 CR 10407
 .S BILMATCH=BILMATCH+1,BILMATCH(BARBIEN)=""
 .I $G(BARDBG) D INS^BAR50DET("  MATCHED!",1)
 .QUIT  ;OK
 ; ---- NOT MATCHED - OTHER REASON FALL THRU
 I BARFND=10,$D(BARMMTCH) D  Q 1  ;IHS/DIT/CPC BAR*1.8*29 CR 6982 & 10777 MANUAL MATCH
 .S BILMATCH=BILMATCH+1,BILMATCH(BARBIEN)=""
 .I $G(BARDBG) D INS^BAR50DET("  MATCHED!",1)
 .QUIT
 ;BARMMTYP=0 - MATCH
 ...
 ;
 I BARFND=10,'$D(BARMMTCH) D  Q 1  ;;IHS/DIT/CPC BAR*1.8*29 CR 6982 & 10777 Not MANUAL MATCH
 .I (+ERABILL'=+BARAMT),(ERADOS'=BARDOS) S BARMMTYP=3 D  ;REASON BA & CD AMOUNT AND DOS DON'T MATCH
 ..I $G(BARDBG) D INS^BAR50DET(" NOT MATCHED",1) Q  ;
 .I (+ERABILL'=+BARAMT),(ERADOS=BARDOS) S BARMMTYP=1 D   ;REASON BA BILL AMOUNT DOESN'T MATCH
 ..I $G(BARDBG) D INS^BAR50DET(" PARTIAL MATCH: DOS OK. AMT DIFF: "_$J(ERABILL-BARAMT,5,2)_"$",1) S BARMMVAL=BARDOS
 .I (ERADOS'=BARDOS),(+ERABILL=+BARAMT) S BARMMTYP=2 D   ;REASON CD DOS DOESN'T MATCH
...
TRANSCK  ;EP
 ...
 S BARTFLG=0
 I BARCKIEN']"," S BARCKIEN=BARCKIEN_","_IMPDA
 ...
 .;I BARTBTCH'="",BARITEM'="",BARBTCH=BARTBTCH,BARITEM=BARTITEM S BARTFLG=1  ;IHS/SD/CPC -20200320 BAR*1.8*29 Typo - BARTITEM
 .I BARTBTCH'="",BARTITEM'="",BARBTCH=BARTBTCH,BARITEM=BARTITEM S BARTFLG=1
 I BARTFLG S ERRORS("HTRN")=""
 I $D(ERRORS) D ADDREAS^BAR50P04(IMPDA,CLMDA,.ERRORS)
 Q
FINDING(BARBILL) ;RETURNS 1^BARXBILL (WITH MISSED OR ADDED LEADING ZERORS); OR 0
 ...
 ..S BARX="G"  ;IHS/DIT/CPC BAR*1.8*29 CR 6982 & 10777
 ..Q
 .Q
 I $E(BARBILL)="0" D  Q BARRET
 .S BARRET=0
