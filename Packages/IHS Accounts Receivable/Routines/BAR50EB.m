BAR50EB ; IHS/SD/TPF - AR ERA BALANCE CHECKER ; 01/28/2009
 ;;1.8;IHS ACCOUNTS RECEIVABLE;**5,6,10,20,21,22,23,24,26,28,29,30,31**;OCT 26,2005;Build 90
 ;
 ;THIS RTN CONTAINS THE BAL CHECKING NEED SO ERA POSTING DOES NOT ALLOW
 ;POSTING WHICH WILL NOT BE ACCEPTED BY UFMS
 ;
 ;BATCH - CHECKS THE TRANS WITHIN A SINGLE ERA FILE BEING POSTED
 ;AGAINST A BATCH. PAY/ADJ FROM THE ERA FILE MUST BE <= THE 
 ;BATCH AMT (#29) AND THE BATCH AMT (#29) MUST BE >=0
 ...
 ;IHS/DIT/CPC 1.8*29 - 
 ;IHS/DIT/CPC 1.8*30 - CR6977
 ;BAR*1.8*31;OIT.IHS.FCJ CR#6984 REQ 5 MOD HEADINGS;REQ 2 prt detail to screen
 Q
 ;
 ;TYPE = "ERA" FOR INTERNAL ERA FILE CHECK, RPMS = COMPARE ERA TO RPMS BILL
NEGBAL(IMPDA,TYPE) ;EP - CHECK FOR NEG BAL W/IN ERA AND IF POSTED AGAINST RPMS
 I $G(BARREPRC)]"" Q
 Q:$G(TYPE)=""
 I '$$IHSNEGB^BARUFUT(DUZ(2)) D  Q  ;IHSNEGB Returns FALSE if neg bal IS ALLOWED ;BAR*1.8*29 Comment added.
 .;BAR*1.8*31;OIT.IHS.FCJ CR#6984 REQ 2 ADDED $G(BARDSP) TO NXT 2 LINES
 .W:$G(BARDSP) !,"The 'Allow Negative Balance' parameter is set to yes.  Aborting Negative Balance Check.",!   ;BAR*1.8*31
 I $G(BARDBG),$G(BARDSP) W !!!,"CHECKING FOR NEGATIVE BALANCE IF MATCHED ERA CLAIMS ARE POSTED..."  ;bar*1.8*20;BAR*1.8*31
 ...
 ..;S CLMCNT=CLMCNT+1  ;BAR*1.8*30 CR6977  IHS/SD/CPC - 20200518 COUNTING 2X
 ...
 ..;I $G(BARDBG) W !!,"ERA BILL: ",BARBILL  ;bar*1.8*30 CR6977;BAR*1.8*31
 ..I $G(BARDBG),$G(BARDSP) W !!,"ERA BILL: ",BARBILL  ;bar*1.8*30 CR6977;BAR*1.8*31;IHS.OIT.FCJ CR#6984 REQ 2 
 ...
 ....S ERRORS("BL NF")=""  ;BILL NOT FOUND IN RPMS. SET IF ERROR 11 NOT SET
 ..I BARBLIEN="DUPLICATE BILLS FOUND" D
 ...S ERRORS("DUPB")=""  ;DUPLICATE BILLS FOUND IN RPMS
 ..I BARBLIEN S BARBAL=$$GET1^DIQ(90050.01,BARBLIEN_",",15,"I")  ;BILL AMT
 ..E  S BARBAL=0
 ..;I $G(BARDBG) W ?41,"CURRENT BILL AMT(RPMS): ",$J($FN(BARBAL,",",2),14)  ;bar*1.8*20 REQ4;BAR*1.8*31
 ..I $G(BARDBG),$G(BARDSP) W ?41,"CURRENT BILL AMT(RPMS): ",$J($FN(BARBAL,",",2),14)  ;bar*1.8*20 REQ4;BAR*1.8*31;IHS.OIT.FCJ CR#6984 REQ 2
 ..;
 ..S BARADJ=0
 ..I $$DONOTPR(IMPDA,CLMDA) Q  ;new code
 ..;Q:$$OVERIDE^BAR50EP1(CLMDA)  ;MRS:BAR*1.8*10 D159-1 AND 2
 ..;Q:$P($G(^BAREDI("I",DUZ(2),IMPDA,30,CLMDA,0)),U,2)="P"  ;DON'T PROC POSTED CLAIMS
 ...
 ..;DENIAL ADJ CLP02=4 WERE CREATING NEG BAL
 ..S ERACHECK=$P($G(^BAREDI("I",DUZ(2),IMPDA,30,CLMDA,2)),U)
 ..S BARAMT=$P($G(^BAREDI("I",DUZ(2),IMPDA,30,CLMDA,0)),U,4)
 ..S BARPAY=BARPAY+BARAMT
 ..;I $G(BARDBG) W !?3,$S(BARBILL'=$$GET1^DIQ(90050.01,BILLCHOS,".01"):"("_$$GET1^DIQ(90050.01,BILLCHOS,".01")_")",1:""),?59,"PYMT: ",$J($FN(BARAMT,",",2),14)  ;bar*1.8*20 REQ4;BAR*1.8*31
 ..I $G(BARDBG),$G(BARDSP) W !?3,$S(BARBILL'=$$GET1^DIQ(90050.01,BILLCHOS,".01"):"("_$$GET1^DIQ(90050.01,BILLCHOS,".01")_")",1:""),?59,"PYMT: ",$J($FN(BARAMT,",",2),14)  ;BAR*1.8*31;IHS.OIT.FCJ CR#6984 REQ 2
 ..;
 ...
 ..I ((BARSTAT="M")&('$$CHKMM^BAR50P08(IMPDA,CLMDA))) S CHECKTOT(ERACHECK)=$G(CHECKTOT(ERACHECK))+BARAMT  ;Allow manual match rntp ;bar*1.8*29 IHS/DIT/CPC Internal testing
 ..S ADJDA=0
 ...
 ...;BAR*1.8*31;OIT.IHS.FCJ CR#6984 REQ 2 ADDED $G(BARDSP) TO NXT 3 LINES
 ...I $G(BARDBG),$G(BARDSP) W !?3,"A/R CAT:",$E(BARCAT,1,11) ;,"  (",BARCAT,")" R ASD ;BAR*1.8*31
 ...I $G(BARDBG),$G(BARDSP) W ?23,"A/R RSN:",$E(BARREAS,1,27)  ;BAR*1.8*31
 ...I $G(BARDBG),$G(BARDSP) W ?60,"ADJ:",$J($FN(BARAMT,",",2),15)  ;BAR*1.8*31
 ...
 ..;I $G(BARDBG) W !,?28,"BILL BALANCE IF ERA CLAIM IS POSTED: ",$J($FN(BARBAL-BARTOT,",",2),14) ;BAR*1.8*31
 ..I $G(BARDBG),$G(BARDSP) W !,?28,"BILL BALANCE IF ERA CLAIM IS POSTED: ",$J($FN(BARBAL-BARTOT,",",2),14) ;BAR*1.8*31;OIT.IHS.FCJ CR#6984 REQ 2
 ..;---chk for neg bal
 ..;I ((BARSTAT="M")&(+$O(^BAREDI("I",DUZ(2),IMPDA,30,CLMDA,4,0))'=0)) Q  ;bar*1.8*28 IHS/SD/SDR CR8346 HEAT275351
 ..;I ((BARSTAT="M")&($$CHKMM^BAR50P08(IMPDA,CLMDA))) Q  ;BAR*1.8*29 Allow Manual Match RNTP IHS/DIT/CPC Internal Testing
 ..I BARBAL-BARTOT<0 D
 ...Q:BARSTAT'="M"!($D(ERRORS("DUPB")))  ;don't put error on claim if not matched or if duplicate bills  bar*1.8*20 REQ5
 ...I '$$IHSNEGB^BARUFUT(DUZ(2)) Q  ;P.OTT 2/5 2014 HEAT147572  ;IHSNEGB Returns FALSE if negative balance IS ALLOWED ;BAR*1.8*29 Comment added.
 ...S ERRORS("NEGR")=""
 ...
 ..K ERRORS
 ..; ------
 D CHECKTOT(.CHECKTOT) ;CHECK ERA CHECK TOT AGAINST BATCH/ITEM TOT
 Q
 ;
CHECKTOT(CHECKTOT) ;CHECK ERA CHECK TOT AGAINST BATCH/ITEM TOT
 N ERRORS,CLMDA,ALLPAY,ERACHECK,CLSTATUS
 N BPR02  ;BAR*1.8*6 SCR119 IHS/SD/TPF
 ;BAR*1.8*31;OIT.IHS.FCJ CR#6984 REQ 2 ADD BARDSP TEST;ST OF CHG
 W:$G(BARDSP) !
 ;BAR*1.8*31;OIT.IHS.FCJ CR#6984 REQ 5 MOD HEADINGS TO PRINT DEPENDING ON PROC TYPE MATCH/POST;ST OF CHG
 ...
 I BARPTYP="M" D
 .W:$G(BARDSP) !?2,"  Matched Bills: ",$J(+$G(BARCNT("MATCHED")),5)," for $",$J($FN(+$G(BARAMT("MATCHED")),",",2),12)
 .W:$G(BARDSP) !?2,"Unmatched Bills: ",$J(+$G(BARCNT("UNMATCHED")),5)," for $",$J($FN(+$G(BARAMT("UNMATCHED")),",",2),12)
 .W:$G(BARDSP) !?2,"    Total Bills: ",$J(+$G(BARTOT("CNT")),5)," for $",$J($FN(+$G(BARTOT("AMT")),",",2),12)
 I BARPTYP="P" D
 .W:$G(BARDSP) !?2,"Matched Unposted Bills: ",$J(+$G(BARCNT("UNMATCHED")),5)," for $",$J($FN(+$G(BARAMT("UNMATCHED")),",",2),12)
 .W:$G(BARDSP) !?2,"  Matched Posted Bills: ",$J(+$G(BARCNT("MATCHED")),5)," for $",$J($FN(+$G(BARAMT("MATCHED")),",",2),12)
 .W:$G(BARDSP) !?2,"   Total Matched Bills: ",$J(+$G(BARTOT("CNT")),5)," for $",$J($FN(+$G(BARTOT("AMT")),",",2),12)
 ;BAR*1.8*31;OIT.IHS.FCJ CR#6984 REQ 5;END OF CHG
 W:$G(BARDSP) !
 ...
 .I $G(BARDBG) W:$G(BARDSP) !?2,"CHECKING ERA CHECK TOTALS FOR ",ERACHECK," TOTAL ",$J($FN(CHECKTOT(ERACHECK),",",2),15)  ;bar*1.8*20 REQ4
 .;BAR*1.8*31;CR#6984 REQ 2;END OF CHG
...
GETITMTO(ERACHECK,ERATOTAL,ERRORS,BRP02) ;EP - GIVEN ERACHECK GET BATCH ITEM TOTALS FOR ERA CHECK ;BAR*1.8*6 SCR119 IHS/SD/TPF
 ...
 .S ERRORS("NB")=""  ;NO BATCH FOUND-ERA CHECK
 .;I $G(BARDBG) W !,"NO BATCH FOUND FOR ERA CHECK"  ;BAR*1.8*31
 .I $G(BARDBG),$G(BARDSP) W !,"NO BATCH FOUND FOR ERA CHECK" ;BAR*1.8*31;OIT.IHS.FCJ CR#6984 REQ 2
 W:$G(BARDSP) !  ;BAR*1.8*31;OIT.IHS.FCJ CR#6984 REQ 2 ADD BARDSP TEST
 S BPOSTBAL=0 ;BATCH POSTING BAL
 ...
 .;;;. I $G(BARDBG) W !,BARMSG
 .;
 .I $G(BARDBG),$G(BARDSP) W !,?2,"CHECKING BATCH: ",BARCB ;P.OTT SHOW ONLY 'NEW' CB 10/10/2013;BAR*1.8*31;OIT.IHS.FCJ CR#6984 REQ 2 ADDED BARDSP TEST
 .S BPOSTBAL=BPOSTBAL+$$GET1^DIQ(90051.01,BARCOLDA_",",17,"E")  ;BAR*1.8*6 LOOK AT BATCH POSTING TOTAL NOT ITEM CREDIT TOTALS
 .I $G(BARDBG),$G(BARDSP) W " POSTING BALANCE: ",BPOSTBAL    ;BAR*1.8*31;OIT.IHS.FCJ CR#6984 REQ 2 ADDED BARDSP TEST
 I ERATOTAL>BPOSTBAL D
 .I '$$IHSNEGB^BARUFUT(DUZ(2)) Q  ;P.OTT 2/6 2014 HEAT147572 ;IHSNEGB Returns FALSE if negative balance IS ALLOWED ;BAR*1.8*29 Comment added.
 .S ERRORS("ERA > ITM")=""  ;ERA TOTAL GREATER THAN BATCH/ITEM TOTAL
 .I $G(BARDBG) D  ;
 ..W:$G(BARDSP) !,"ERA TOTAL PAYMENTS AND ADJUSTMENTS OF ",ERATOTAL    ;BAR*1.8*31;OIT.IHS.FCJ CR#6984 REQ 2 ADDED BARDSP TEST
 ..W:$G(BARDSP) !,"IS GREATER THAN BATCH POSTING BALANCE OF ",BPOSTBAL  ;BAR*1.8*31;OIT.IHS.FCJ CR#6984 REQ 2 ADDED BARDSP TEST
 Q
