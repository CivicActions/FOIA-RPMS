BARRADJ ; IHS/SD/TPF - TRANSACTION/ADJUSTMENT REPORT ; 08/20/2008
 ;;1.8;IHS ACCOUNTS RECEIVABLE;**6,7,19,20,24,29,31**;OCT 26, 2005;Build 90
 ...
 ;BAR*1.8.*31 IHS.OIT.FCJ CR#8938 HEAT284261 ADD PRIM PRV TO DELIM REPORT;ADD XML REPORT
 ;
 Q
EN ; EP
 K BARY,BAR
 ...
 N BARTEXT
 ;BAR*1.8.*31 IHS.OIT.FCJ CR#8938 ST CHANGE
SEL ;TEXT DEL OR XML
 K DIR
 ;S DIR("A")="Text-delimited? "
 ;S DIR(0)="Y;;"
 ;S DIR("B")="N"
 S DIR(0)="S^D:Text-Delimited;X:XML;P:Print"
 S DIR("A")="Select Text-Delimited, Spreadsheet XML file or Print"
 S DIR("B")="Print"
 D ^DIR
 I $D(DTOUT)!$D(DUOUT)!$D(DIRUT) Q
 S BARTEXT=Y
 W:BARTEXT'="P" !!,"NOTE: the Delimited and XML Format should be written to a HFS host file. ",!
 W:BARTEXT="X" "The report is in spreadsheet XML format so please use an xml ",!
 W:BARTEXT="X" "suffix on the file name.",!
 W:BARTEXT="X" "i.e. 'c:\temp\daysreport.xml'",!!
 ;D:BARTEXT="X" PAZ^BARRUTL
 ;BAR*1.8.*31 IHS.OIT.FCJ CR#8938 END CHANGE
 ;IHS/SD/AR bar*1.8*19 RQMNT
 D ^BARRHD  ;Rpt hdr
 S %ZIS="Q"
 D ^%ZIS
 ...
 U IO ;:140 ;4/1/2014
 D COMPUTE
 I BARTEXT="X" D PRTXML^BARRADJ4   ;BAR*1.8.*31 IHS.OIT.FCJ CR#8938
 D ^%ZISC
...
QUE ;EP
 ...
 Q
COMPUTE ;
 S BARROW=0   ;BAR*1.8.*31 IHS.OIT.FCJ CR#8938
 S BAR("SUBR")="BAR-TSR"
 ...
 . ;IHS/SD/POT BAR*1.8*24-ADDED DOS;BAR*1.8.*31 IHS.OIT.FCJ CR#8938 ADD PRIM PRV and BARTEXT="D" TEST
 ...
 . W:(BARTEXT="D")&(BARY("SORT")'="N") "LOCATION^CL/VI^ADJ CATEGORY^BILL NUMBER^DOS^TRXN DATE^INSURER^BILL AMOUNT^TRXN AMOUNT^ADJ TYPE^PRIMARY PROVIDER"
 . W:(BARTEXT="D")&(BARY("SORT")="N") "LOCATION^ADJ CATEGORY^BILL NUMBER^DOS^TRXN DATE^INSURER^BILL AMOUNT^TRXN AMOUNT^ADJ TYPE^PRIMARY PROVIDER"
 E  D
 . ;IHS/SD/POT BAR*1.8*24-ADDED DOS;BAR*1.8.*31 IHS.OIT.FCJ CR#8938 ADD PRIM PRV and BARTEXT="D" TEST
 ...
 . W:(BARTEXT="D")&(BARY("SORT")'="N") "LOCATION^CL/VI^BILL NUMBER^DOS^TRXN DATE^INSURER^BILL AMOUNT^TRXN AMOUNT^PRIMARY PROVIDER"
 . W:(BARTEXT="D")&(BARY("SORT")="N") "LOCATION^BILL NUMBER^DOS^TRXN DATE^INSURER^BILL AMOUNT^TRXN AMOUNT^PRIMARY PROVIDER"
...
DATA ; EP
 ...
 ;IHS.OIT.FCJ BECUASE OF RTN SIZE MODIFIED THE COMMENTED LINES FROM PRV PATCH S DATA VAR INSTEAD OF EACH LINE
 ;S BARHLD=$G(^TMP($J,"BAR-TSR",BARTR("AR"),DUZ(2)_U_BARTR("L")_U_BAR("TRANS")_U_BAR("ADJCAT")_U_BAR("SORT")_U_BAR_U_BARTR))
 ;S DATA=BAR(1)_U_$P(BARHLD,U,2)+BAR(2)_U_$P(BARHLD,U,3)+BAR(3)_U_$P(BARHLD,U,4)+BAR(4)_U_$P(BARHLD,U,5)+BAR(5)_U_BAR(6)_U_$P(BARHLD,U,7)+BAR(7)
 ;S DATA=DATA_U_$$GET1^DIQ(90052.02,BARTR("T")_",",.01,"E")_U_$$GET1^DIQ(90050.02,BARTR("I")_",",.01,"E")_U_$$SDT^BARDUTL($P(BARTR("DT"),"."))_U_BARTR("I")_U_BAR("ADJCAT")_U_BAR("ADJTYPE")
 ;S ^TMP($J,"BAR-TSR",BARTR("AR"),DUZ(2)_U_BARTR("L")_U_BAR("TRANS")_U_BAR("ADJCAT")_U_BAR("SORT")_U_BAR_U_BARTR)=DATA
 ; For summary
 ...
 ;BAR*1.8.*31 IHS.OIT.FCJ BUG SHOULD BE 12 AND 13 AND WAS SET AS 11 AND 12 NODES
 S $P(^TMP($J,"BAR-TSR",BARTR("AR"),DUZ(2)_U_BARTR("L")_U_BAR("TRANS")_U_BAR("ADJCAT")_U_BARTR("DATA SRC")_U_BAR("SORT")_U_BAR_U_BARTR),U,12)=BAR("ADJCAT")
 S $P(^TMP($J,"BAR-TSR",BARTR("AR"),DUZ(2)_U_BARTR("L")_U_BAR("TRANS")_U_BAR("ADJCAT")_U_BARTR("DATA SRC")_U_BAR("SORT")_U_BAR_U_BARTR),U,13)=BAR("ADJTYPE")
 ;BAR*1.8.*31 IHS.OIT.FCJ CR#8938 NEW LINE FOR PRIM PROV,VISIT TYPE AND CLINIC TYPE
 S $P(^TMP($J,"BAR-TSR",BARTR("AR"),DUZ(2)_U_BARTR("L")_U_BAR("TRANS")_U_BAR("ADJCAT")_U_BARTR("DATA SRC")_U_BAR("SORT")_U_BAR_U_BARTR),U,14,16)=BAR("PV")_U_BAR("V")_U_BAR("C")
 ; For summary
