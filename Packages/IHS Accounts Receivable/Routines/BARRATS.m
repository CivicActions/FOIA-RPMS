BARRATS ; IHS/SD/LSL - File Synchronization ;09/15/2008
 ;;1.8;IHS ACCOUNTS RECEIVABLE;**7,19,20,24,33,35**;OCT 26, 2005;Build 187
 ;
 ;IHS/SD/LSL 06/04/02 1.6*2 Routine created
 ;
 ;IHS/SD/MRS 1.8*7 IM29892 MODIFIED XTMP FILE NAME TO TMP TO MEET SAC REQUIREMENTS
 ;
 ;IHS/SD/POT 1.8*24 HEAT141692 CHANGE DEFAULT VALUE (detail or summary)
 ;IHS/SD/SDR 1.8*33 ADO60817 Updated detail subheader from TRAN DATE to TR IEN
 ;IHS/SD/SDR 1.8*35 ADO76628 Updated to run when tasked in RTSK option
 ;
 ;*********************************************************************
 ;  This routine will compare the Current Bill Amount of the A/R Bill File to the A/R Transaction 
 ;  history balance.  If they are not equal, relevant data will be stored in a temp global.  A report
 ;  (A/R BILL AND TRANSACTION SYNCHRONIZATION REPORT) will be generated containing A/R Bill,
 ;  Date of Service, A/R Account, Current Bill Amount, Transaction History Balance, and the
 ;  difference between the two.
 ;
 ;  The user may select a visit location for the report.  The user may choose to print
 ;  a detail report.  If detail, then the transaction history will show as well (Transaction Date,
 ;  Transaction Type, A/R Account, Transaction Amount, and Transaction Balance)
 ;
 ;*********************************************************************
 ;
 Q
...
HIST ;
 ...
 ;W !!?6,"TR DATE",?17,"TR TYPE",?33,"A/R ACCOUNT",?50,"TR AMOUNT",?65,"TR BALANCE"  ;bar*1.8*33 IHS/SD/SDR ADO60817
 W !!?6,"TR IEN",?17,"TR TYPE",?33,"A/R ACCOUNT",?50,"TR AMOUNT",?65,"TR BALANCE"  ;bar*1.8*33 IHS/SD/SDR ADO60817
