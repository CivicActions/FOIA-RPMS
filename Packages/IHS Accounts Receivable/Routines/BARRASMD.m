BARRASMD ; IHS/SD/LSL - Age Summary Report Print Logic ;07/28/2020
 ;;1.8;IHS ACCOUNTS RECEIVABLE;**7,28,30**;OCT 26, 2005;Build 55
 ;COPIED FROM BARRASMB
...
PRINT ; EP
 ...
 I $G(BARY("RTYP"))=3,($G(BARY("STCR"))=5) D
 .S BARDASH="----------"
 .S BAREQUAL="=========="
 .S BAR("COL")="W !,BARY(""STCR"",""NM""),!,?3,""UFMS INVOICE NUMBER"",?30,""RPMS BILL NUMBER"",?52,""BALANCE"",?65,""CATEGORY"""
...
STANDARD ;
 ...
 .I '$D(BARY("LOC")) W !,"*** VISIT Location: ",BAR("SUB0"),?40,"ASUFAC: ",^TMP($J,BAR("SUB0")),!
 .S BAR("SUB1")=""
...
BILL ;
 ...
 F  S BAR("SUB0")=$O(^TMP($J,"BAR-ASMT",BAR("SUB0"))) Q:BAR("SUB0")=""  D  Q:$G(BAR("F1"))
 .I BAR("SUB0")'=BARHOLD("SUB0") D HD
 .S BARCNT("SUB0")=^TMP($J,"BAR-ASMC",BAR("SUB0"))
 .Q:$G(BAR("F1"))
 .S BARHOLD("SUB0")=BAR("SUB0")
 .S LARSUFAC=$$GET1^DIQ(9999999.06,$P(^TMP($J,"BAR-ASMT",BAR("SUB0")),U,7),.0799)
 .I '$D(BARY("LOC")) W !,"*** VISIT Location: ",BAR("SUB0"),?50,"ASUFAC: ",LARSUFAC,!
 .S BAR("SUB1")=""
 .F  S BAR("SUB1")=$O(^TMP($J,"BAR-ASMT",BAR("SUB0"),BAR("SUB1"))) Q:BAR("SUB1")=""  D  Q:$G(BAR("F1"))
 ..S BARCNT("SUB1")=^TMP($J,"BAR-ASMC",BAR("SUB0"),BAR("SUB1"))
 ..S BARTMP=$G(^TMP($J,"BAR-ASMT",BAR("SUB0"),BAR("SUB1")))
 ..W !!,?45,BAR("SUB1"),!        ; Billing Entity/Allowance Category
 ..S BAR("SUB2")=""
 ..F  S BAR("SUB2")=$O(^TMP($J,"BAR-ASMT",BAR("SUB0"),BAR("SUB1"),BAR("SUB2"))) Q:BAR("SUB2")=""  D  Q:$G(BAR("F1"))
 ...S BARTMPS=$G(^TMP($J,"BAR-ASMT",BAR("SUB0"),BAR("SUB1"),BAR("SUB2")))
 ...I $Y>(IOSL-8) D HD Q:$G(BAR("F1"))
 ...W !,BAR("SUB2")      ; A/R Account
 ...S BAR("SUB3")=""
 ...
 ....S BARDEMO=$G(^TMP($J,"BAR-DEMO",BAR("SUB0"),BAR("SUB1"),BAR("SUB2"),BAR("SUB3")))
 ....;W !?2,$E(BAR("SUB3"),1,17)                  ;bill number
 ....W !?2,$E($P(BARDEMO,U,2),1,25)               ;apply to (ufms invoice number)
 ....W ?29,$E(BAR("SUB3"),1,17)                   ;bill number
 ....W ?50,$J($FN($P(BARTMPS2,U,6),"p,",2),15)  ;balance
 ....I $P(BARTMPS2,U)>0 W ?70,$J("CURRENT",9) S BARTMPS2("SUBCNT")=$G(BARTMPS2("SUBCNT"))+1
 ....I $P(BARTMPS2,U,2)>0 W ?70,$J("31-60",9) S BARTMPS2("SUBCNT")=$G(BARTMPS2("SUBCNT"))+1
 ....I $P(BARTMPS2,U,3)>0 W ?70,$J("61-90",9) S BARTMPS2("SUBCNT")=$G(BARTMPS2("SUBCNT"))+1
 ....I $P(BARTMPS2,U,4)>0 W ?70,$J("90-120",9) S BARTMPS2("SUBCNT")=$G(BARTMPS2("SUBCNT"))+1
 ....I $P(BARTMPS2,U,5)>0 W ?70,$J("120+",9) S BARTMPS2("SUBCNT")=$G(BARTMPS2("SUBCNT"))+1
 ....;W ?70,$J($P(BARTMPS2,U,6),10,2)   ; BALANCE
 ...Q:$G(BAR("F1"))
 ...W !,?50,$J(BARDASH,15),!
 ...W "A/R ACCOUNT TOTAL  "_BARTMPS2("SUBCNT")_$S(BARTMPS2("SUBCNT")>1:" BILLS",1:" BILL")
 ...S BARTMPS2("SUBCNT")=0
 ...S BARTMPCT=$G(^TMP($J,"BAR-ASMC",BAR("SUB0"),BAR("SUB1")))
 ...D ACCOUNT
 ...W !
 ..Q:$G(BAR("F1"))
 ..W BARDASH,!
 ...
 .Q:$G(BAR("F1"))
 .S BARTMP=$G(^TMP($J,"BAR-ASMT",BAR("SUB0")))
 .S BARTMPCT=$G(^TMP($J,"BAR-ASMC",BAR("SUB0")))
 .W !,BARDASH,!,"*** VISIT Loc Total"
 .D SUM2
 Q:$G(BAR("F1"))
 W !
 S BARTMPCT=$G(^TMP($J,"BAR-ASMC"))
 D TOTAL               ; Report Totals
 I '$D(BARA),(XQY0'["UFMS") D EOP^BARUTL(0)  ;If not USM Pause IHS/DIT/CPC - 20180502 BAR*1.8*28 CR8350
 Q
 ; ********************************************************************
...
ACCOUNT ;
 ...
 ;W:$P(BARTMPS,U)>0 ?70,$J($P(BARTMPS,U),19,2)     ; CURRENT
 ;W:$P(BARTMPS,U)>0 ?70,$J($P(BARTMPS,U),19,2)    ; 31-60
 ;W:$P(BARTMPS,U)>0 ?70,$J($P(BARTMPS,U),19,2)   ; 61-90
 ;W:$P(BARTMPS,U)>0 ?70,$J($P(BARTMPS,U),19,2)    ; 90-120
 ;W:$P(BARTMPS,U)>0 ?70,$J($P(BARTMPS,U),19,2)    ; 120+
 ;W ?20,$P(BARTMPCT,U,6),$S($P(BARTMPCT,U,6)>1:" BILLS",1:" BILL")
 W ?50,$J($FN($P(BARTMPS,U,6),"p,",2),15)   ; BALANCE
 Q
 ; ********************************************************************
 ;
SUM2 ;
 ...
 W ?20,$P(BARTMPCT,U,6),$S($P(BARTMPCT,U,6)>1:" BILLS",1:" BILL")
 W ?50,$J($FN($P(BARTMP,U,6),"p,",2),15)   ; BALANCE
 Q
 ; ********************************************************************
 ;
TOTAL ;
