BARMPAS3 ; IHS/SD/LSL - Patient Account Statement Print ;
 ;;1.8;IHS ACCOUNTS RECEIVABLE;**19,20,21,23,36**;OCT 26, 2005;Build 199
 ;IHS/SD/LSL 1.7*2 05/13/03
 ;
 ;IHS/SD/POT FEB 2012 LINE PRINT+1 ADDED TO GET REPORT HEADER WHEN PRINTED (QUEUED)
 ;IHS/SD/POT HEAT#80718 21-AUG-2012 ADDED SORTING OPTION BY PATNAME
 ;HEAT91646 DROPPED KILLING COLLECTED DATA (RETAIN DISFUNCT) - WILL BE HANDLED BY THE NEW 'PUR' OPTION
 ;IHS/SD/SDR 1.8*36 ADO74825 Fixed divider header to not wrap
 ;IHS/SD/SDR 1.8*36 ADO74824 When selecting stmt runs, check for runs for logged into facility
 ;********************************************************************
 Q
 ;
GETHDR  ;EP
...
SELECT ;
 ...
 I BARCNT=0 D  Q
 .W !!!,$$CJ^XLFSTR("NO PATIENT ACCOUNT STATEMENT RUNS FOR THIS FACILITY TO CHOOSE FROM",IOM)
 .D EOP^BARUTL(0)
