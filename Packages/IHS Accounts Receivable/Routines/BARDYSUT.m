BARDYSUT ; IHS/SD/TPF - DAYS IN A/R REPORT UTILS ; 
 ;;1.8;IHS ACCOUNTS RECEIVABLE;**1,33**;OCT 26, 2005;Build 133
 ;IHS/SD/SDR 1.8*33 ADO60817 Updated to use 'C' xref since DINUM has been removed and the IEN
 ;   won't be the date/time anymore
 ;
 Q
...
CHKARTR(FROM,TO) ;EP -
 ...
 ;S:$G(FROM)'="" TRANIEN=FROM-.01  ;bar*1.8*33 IHS/SD/SDR ADO60817
 S:$G(FROM)'="" TRANFDT=FROM-.01  ;bar*1.8*33 IHS/SD/SDR ADO60817
 S:$G(TO)="" TO=99999999999999999
 ;F  S TRANIEN=$O(^BARTR(DUZ(2),TRANIEN)) Q:'TRANIEN!(TRANIEN>TO)  D  ;bar*1.8*33 IHS/SD/SDR ADO60817
 ;start new bar*1.8*33 IHS/SD/SDR ADO60817
 ;note - new code also includes new tag CHKARTR2 to minimize changes
 F  S TRANFDT=$O(^BARTR(DUZ(2),"C",TRANFDT)) Q:'TRANFDT!(TRANFDT>TO)  D CHKARTR2
 Q
CHKARTR2 ;EP
 S TRANIEN=0
 F  S TRANIEN=$O(^BARTR(DUZ(2),"C",TRANFDT,TRANIEN)) Q:'TRANIEN  D
