BARDYSPR ; IHS/SD/TPF - DAYS IN A/R REPORT PRINT DRIVER; 
 ;;1.8;IHS ACCOUNTS RECEIVABLE;**16,28,33**;OCT 26, 2005;Build 133
 ;IHS/SD/SDR 1.8*28 HEAT224215 Added write comma-delmtd or detl rpt
 ;IHS/DIT/CPC 1.8*28 CR8345 Changed delim file format, added spreadsheet XML option
 ;IHS/SD/SDR 1.8*33 ADO60817 Updated to use 'C' xref, not 'B' after A/R Trans .01 field change that included counter in 'B' xref
 Q
PRINTAPP ;EP
 N MONTHYR
 S BAR("PG")=0
...
PRINTVIS ;EP
 ...
 S DECIMALS=2  ;HOW MANY DECIMALS FOR AVGS
 S BAR("PG")=0
...
FOOTNOTE ;EP
 Q
 ;TOT COLLECTIONS FOR DATE RANGE
COLLECT(START,END,COLLAMT) ;EP
 N ENTRY,DEBIT,CREDIT
 S ENTRY=START-.01
 S END=END_".24"
 ;F  S ENTRY=$O(^BARTR(DUZ(2),"B",ENTRY)) Q:ENTRY=""!(ENTRY>END)  D  ;bar*1.8*33 IHS/SD/SDR ADO60817
 F  S ENTRY=$O(^BARTR(DUZ(2),"C",ENTRY)) Q:ENTRY=""!(ENTRY>END)  D  ;bar*1.8*33 IHS/SD/SDR ADO60817
 .S MONTHYR=$E(ENTRY,4,5)_$E(ENTRY,2,3)
 .S THISYR=$E(DT,1,3)
 .S REC=""
 .;F  S REC=$O(^BARTR(DUZ(2),"B",ENTRY,REC)) Q:'REC  D  ;bar*1.8*33 IHS/SD/SDR ADO60817
 .F  S REC=$O(^BARTR(DUZ(2),"C",ENTRY,REC)) Q:'REC  D  ;bar*1.8*33 IHS/SD/SDR ADO60817
