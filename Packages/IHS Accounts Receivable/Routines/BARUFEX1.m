BARUFEX1 ; IHS/SD/TPF - MAIN EXTRACT RTN FOR UFMS (CONTINUED FROM BARUFEX) ; 10/31/2008
 ;;1.8;IHS ACCOUNTS RECEIVABLE;**7,9,16,19,20,23,28,33**;OCT 26,2005;Build 133
 ;IHS/SD/MRS 1.8*7 NEW ROUTINE -- to meet SAC size requirements
 ;IHS/SD/MRS 1.8*7 IM30562 Heavily modified to prevent duplicate records
 ;IHS/SD/MRS 1.8*7 IM30562 New functionality; only enters from single PULLSESS EP
 ...
 ;IHS/SD/SDR 1.8*33 ADO60817 Updated for A/R Transaction Date/Time/Counter format; updated the transaction date to use date field
 Q
 ;
...
ERR1 .D ERR^BARUFEXU(1)
 ...
 ;S UFMSTRDT=$$FMTE^XLFDT($P(TRDATE,"."),"5DZ")  ;bar*1.8*33 IHS/SD/SDR ADO60817
 S UFMSTRDT=$$FMTE^XLFDT($P($G(^BARTR(DUZ(2),TRDATE,0)),U,12),"5DZ")  ;bar*1.8*33 IHS/SD/SDR ADO60817
...
CONT ; ALL QUIT CONDITIONS HAVE BEEN MET
 ...
 ;S UNIQUEID=$$FILLSTR^BARUFUT1(TRDATE,14,"L",0)  ;bar*1.8*33 IHS/SD/SDR ADO60817
 S UNIQUEID=$$FILLSTR^BARUFUT1($P($G(^BARTR(DUZ(2),TRDATE,0)),U),18,"L"," ")  ;bar*1.8*33 IHS/SD/SDR ADO60817
