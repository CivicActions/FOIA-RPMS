BARUFUT1 ; IHS/SD/TPF - UTILITIES 2 FOR UFMS ; 04/28/2008
 ;;1.8;IHS ACCOUNTS RECEIVABLE;**3,4,5,6,13,21,23,24,30,37**;OCT 26, 2005;Build 210
 ;;
 ;Modify patch number in filename so batches are formatted correctly by the HUB
 ;IHS/SD/POT 1.8*23 MAR 2012 HEAT62222  ADDED GETDAY0(FORM)
 ;IHS/SD/POT 1.8*23 APR 2013 HEAT89920
 ;IHS/SD/POT 1.8*24 JUN 2013 belcourt HEAT118656
 ;IHS/SD/SDR 1.8*37 ADO60825 Corrected logic checking if user is logged in and posting
 Q
 ;
...
STILPOST(UDUZ) ;EP -IS THE USER STILL LOGGED ON AND/OR POSTING?
 ...
 ;S LOGIN=$O(^XUSEC(0,"CUR",UDUZ,DT))  ;bar*1.8*37 IHS/SD/SDR ADO60825
 S LOGIN=+$G(^BARTMP("UFMSPST",DUZ(2),UDUZ,SESSID)) ;bar*1.8*37 IHS/SD/SDR ADO60825
 Q:'LOGIN 0_U_"NOT LOGGED IN"  ;THEREFORE NOT POSTING
 Q 1_U_"LOGGED IN AND POSTING"  ;bar*1.8*37 IHS/SD/SDR ADO60825
 ;
 S INMENUS=0
 ...
 .Q:'CURREC
 .;VALUE OF DEFAULT DIVISION TO THE USER NOT THE DIVISION THEY SIGN INTO
...
ASUFAC(DUZ2,TRANS) ;EP -GET ASUFACASUFAC3PIEN STRING
 ...
 S TPBIEN=$P($$FIND3PB^BARUTL(DUZ2,ARBILLIN),",",2)   ;3PIEN
...
DELETE(UDUZ,SESSID) ;EP - DELETE SESSION
 K DIR,DIE,DIC,DR,DA
 I +$G(^BARTMP("UFMSPST",DUZ(2),DUZ,SESSID))=0 K ^BARTMP("UFMSPST",DUZ(2),DUZ,SESSID)  ;bar*1.8*37 IHS/SD/SDR ADO60825
 S DA(1)=UDUZ
 S DA=SESSID
