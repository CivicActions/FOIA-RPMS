BAREDP04 ; IHS/SD/LSL - MATCH REASONS AND CLAIMS ; 01/09/2009
 ;;1.8;IHS ACCOUNTS RECEIVABLE;**5,6,10,20,21,22,23,24,28,29**;OCT 26,2005;Build 66
...
INDEX    ;EP
 ...
 S CLMCNT=0,BARBL=""
 ;F  S BARBL=$O(^XTMP("BAR-LIST",$J,DUZ(2),BARBL)) Q:BARBL=""  D  Q:QFLG=1
...
CLMFLG(CLMDA,ERRORS) ;EP
 ...
 .I '$G(CLMFLG),$G(CLMTYP)="BA" S STAT="BA"  ;BILL AMOUNT DOESN'T MATCH RPMS ;IHS/DIT/CPC - BAR*1.8*29 CR6982
 .I '$G(CLMFLG),$G(CLMTYP)="CD" S STAT="CD"  ;DOS DOESN'T MATCH RPMS  ;ADDED $G ;IHS/DIT/CPC - BAR*1.8*28
 ;RSNS FOUND FOR 'NOT TO POST' (OLD)
 I $D(ERRORS("CF")) K ERRORS S ERRORS("CF")=""
 I $D(ERRORS("CT")) K ERRORS S ERRORS("CT")=""
 I $D(ERRORS("DUPB")) K ERRORS S ERRORS("DUPB")=""
 ;START ;IHS/DIT/CPC BAR*1.8*29 CR 6982 & 10777
 I $D(ERRORS),'$D(BARMMTCH) D ADDREAS(IMPDA,CLMDA,.ERRORS)
 I $D(ERRORS),$D(BARMMTCH) D
 .S BARTER=""
 .F  S BARTER=$O(ERRORS(BARTER)) Q:BARTER=""  D
 ..S BARSUF=$S(BARTER="BA":"'s Bill Amount.",BARTER="CD":"'s Date of Service.",1:".")
 ..W !,"ERA bill "_$$GET1^DIQ(90056.0205,BAREIENS,.01)_" does not match A/R Bill "_$P($G(^BARBL(DUZ(2),BARBIEN,0)),U)_BARSUF
 ..W !,"Do you want to disregard the reason not to post?"
 ..K DIR
 ..S DIR(0)="Y",DIR("B")="YES"
 ..D ^DIR
 ..S:Y ERRORS("MM")=""  ;Manual Match
 ..K:Y ERRORS(BARTER)
 .D ADDREAS(IMPDA,CLMDA,.ERRORS)
 .K DIR,Y,BARTER,BARMMTCH,BARSUF
 ;END ;IHS/DIT/CPC BAR*1.8*29 CR 6982 & 10777
...
HIPAACLM(IMPDA,CLMDA,ERRORS) ;BAR*1.8*5 SRS-80 TPF ERRORS CAN BE MULTIPLE
 ...
 ;1st chk bill# "B" x-ref; if no, chk other identifier "G" x-ref for Pharmacy POS
 S BAREIENS=CLMDA_","_IMPDA_","
 S BARTEST=$$GET1^DIQ(90056.0205,BAREIENS,.01) ;full string
 S BARTEST=$TR(BARTEST,"E","A")  ;bar*1.8*26 IHS/SD/SDR HEAT233443 ; IHS/DIT/CPC - BAR*1.8*29
 ...
 I BARFND=0,BARCNT=0 S CLMTYP="CT",ERRORS("CT")=""  ;CLAIM NUMBER NOT FOUND IN RPMS
...
CLM2 ;
 ...
 S ERABILL=$$GET1^DIQ(90056.0205,BAREIENS,.05)
 S BARBILD=$$GET1^DIQ(90050.01,BARBIEN,13,"I")
 ...
 S BARBDT=$$GET1^DIQ(90050.01,BARBIEN,102,"I")  ;A/R BILL DOS BEGIN
 S BARBDTE=$$GET1^DIQ(90050.01,BARBIEN,103,"I")  ;A/R BILL DOS END
 S BARBDT=$$Y2KD2^BARDUTL(BARBDT)  ;CONVERT TO YYYYMMDD
 S BARBDTE=$$Y2KD2^BARDUTL(BARBDTE)
 ;I BARRADT'=BARBDT S CLMTYP="CD",ERRORS("CD")=""  ;DOS DOESN'T MATCH RPMS ; BAR*1.8*5 SRS-80
 I +BARBDTE>0 D
 .I BARRADT'=BARBDT,(BARRADTE'=BARBDTE) S CLMTYP="CD",ERRORS("CD")=""
 E  I BARRADT'=BARBDT S CLMTYP="CD",ERRORS("CD")=""
 ;END CR 10407
