BARDYSVZ ; IHS/SD/MAS,TPF - OMB - DAYS TO COLLECTION REPORT FOR SQL ; 02/09/2009
 ;;1.8;IHS ACCOUNTS RECEIVABLE;**12,13,23,33**;OCT 26, 2005;Build 133
 ;IHS/SD/TMM 07/02/09 M1 Routine too large.  Create 2nd routine ^BARDYSV2
 ;P.OTT FEB 2013 CHANGED PTR TO INSURER TYPE
 ;IHS/SD/SDR 1.8*33 ADO60817 Updated to use .011 for date/time and not IEN which is now date/time/counter
 Q
 ;ASK DATE RANGE; FIND VISITS FOR RANGE IN VISIT FILE
INTERACT ;EP
...
ASKEND ;EP - ASK END DATE
 ...
 Q
 ;
 ;IT IS EXPECTED THAT IF NO DATES ARE PASSED VIA TASKMAN THAT TASK WILL BE SCHEDULED
 ;FOR FIRST DAY OF MONTH AND RANGE WILL BE FROM 1ST TO LAST DAY OF THE MONTH BEFORE
...
LSTMNTH(TODAY) ;EP - COMPY
 ...
 Q BEGIN_U_END
 ;GIVEN A DATE RANGE FIND VISITS FOR THAT MONTH IN VISIT FILE
...
GETV(ENTRY) ;
 ...
 .S BARCS=$$CLINIC^APCLV(VISITIEN,"C")  ;CLINIC STOP
...
GETB(BARVDA) ;EP - GET BILL DATA 
 ...
 .;S INSPTR=$$GET1^DIQ(90050.03,BLLARIEN_",",6,"I")  ;'A/R ACCOUNT' PTR  ;bar*1.8*33 IHS/SD/SDR ADO60817
 .S INSPTR=$$GET1^DIQ(90050.01,BLLARIEN_",",3,"I")  ;'A/R ACCOUNT' PTR  ;bar*1.8*33 IHS/SD/SDR ADO60817
 .S INSTYP=$$GET1^DIQ(9999999.18,INSPTR,.211,"I")  ;RETURNS NUM PTR (I.E. 21) P.OTT GET INS TYP
 .;S ISNTYP=$P($G(^AUTTINTY(BARALLC,0)),U,2)  ;NUMBER-->"INDIAN PATIENT^I" P.OTT  ;bar*1.8*33 IHS/SD/SDR ADO60817
 .S INSTYP=$P($G(^AUTTINTY(BARALLC,0)),U,2)  ;NUMBER-->"INDIAN PATIENT^I" P.OTT  ;bar*1.8*33 IHS/SD/SDR ADO60817
...
GETTX(BILLIEN) ;EP - GET TRANSACTION DATA
 ...
 N BARTRDFN  ;bar*1.8*33 IHS/SD/SDR ADO60817
 S BILL3P=$P($G(^ABMDBILL(DUZ(2),BILLIEN,0)),U)_" "
 S BILLAR=$O(^BARBL(DUZ(2),"B",BILL3P))
 S:BILLAR'="" BLLARIEN=$O(^BARBL(DUZ(2),"B",BILLAR,""))
 Q:'$G(BLLARIEN)  ;CAN'T FIND CORRESPONDING A/R BILL
 ...
 S BARTRDFN=""
 F  S BARTRDFN=$O(^BARTR(DUZ(2),"AC",BLLARIEN,BARTRDFN)) Q:BARTRDFN=""  D
 .S IENS=BARTRDFN_","
 .S TRANDATE=$P($G(^BARTR(DUZ(2),BARTRDFN,0)),U,18)  ;A/R Trans DT/TM in FM format
