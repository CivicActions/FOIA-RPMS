BARCLU01 ; IHS/SD/LSL - Split out of BARCLU0 ;; 07/09/2010
 ;;1.8;IHS ACCOUNTS RECEIVABLE;**3,4,19,22,28,35**;OCT 26, 2005;Build 187
 ;;
 ;IHS/SD/TMM 1.8*19 06/18/2010 (M819), Add Prepayment functionality.
...
CACC ; EP
 ...
 .D ^DIE
 .I ($P($G(^BARCOL(DUZ(2),BARCLDA,1,DA,0)),U,7)'=0)&($P($G(^BARAC(DUZ(2),$P($G(^BARCOL(DUZ(2),BARCLDA,1,DA,0)),U,7),0)),U)["AUPNPAT") D
 ..I $$GETPREF^AUPNSOGI(+$P($G(^BARAC(DUZ(2),$P($G(^BARCOL(DUZ(2),BARCLDA,1,DA,0)),U,7),0)),U),"I",1)'="" W ?40," - "_$$GETPREF^AUPNSOGI(+$P($G(^BARAC(DUZ(2),$P($G(^BARCOL(DUZ(2),BARCLDA,1,DA,0)),U,7),0)),U),"I",1)_"*"
 .;end new bar*1.8*35 IHS/SD/SDR ADO60910
 .;I BARSPAR(2,"I") S DR=DR_"8//^S X=$G(BARITLC);"       ;8=Visit Location (A/R Satelite)  ;bar*1.8*35 IHS/SD/SDR ADO60910
 .I BARSPAR(2,"I") S DR="8//^S X=$G(BARITLC);"       ;8=Visit Location (A/R Satelite)  ;bar*1.8*35 IHS/SD/SDR ADO60910
 .;E  S DR=DR_"8///^S X=BARSPAR(.01);" ;location  ;bar*1.8*35 IHS/SD/SDR ADO60910
 .E  S DR="8///^S X=BARSPAR(.01);" ;location  ;bar*1.8*35 IHS/SD/SDR ADO60910
