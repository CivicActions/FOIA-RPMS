BAREDEP ; IHS/SD/TPF - AR ERA PAYMENT CHECKER ; 01/30/2009
 ;;1.8;IHS ACCOUNTS RECEIVABLE;**5,6,10,20,23,24,33**;OCT 26,2005;Build 133
 ;BAR*1.8*6 IHS/SD/TPF MOVE REVERSAL CHECK TO A FULL CLMDA LOOP
 ;IHS/SD/POT 1.8*23 HEAT#82698 NOV 2012  ACCEPT LEADING ZEROES IN CHKECK # (POS)
 ;IHS/SD/POT 1.8*24 HEAT148388 1/10/2014 ACCEPT REVERSALS FOR TYPE= 1 IF NEG PAYMENT
 ;IHS/SD/POT 1.8*24 HEAT147572 ALLOW TRIBAL SITES ERA POSTING OF NEG BAL & CANCELLED BILLS
 ;IHS/SD/SDR 1.8*33 ADO60817 Changed to use .011 field (date/time) instead of .01 (date/time/counter)
 Q
...
INRPMS(IMPDA,REVDA,MATCHES,NOMATCH) ;EP - LOOK FOR MATCHING PAYMENT IN A/R BILL
 ...
 ..S FOUND=FOUND+1
 ..;S REVDATE=$$GET1^DIQ(90050.03,TRANS_",",.01,"I")  ;bar*1.8*33 IHS/SD/SDR ADO60817
 ..S REVDATE=$$GET1^DIQ(90050.03,TRANS_",",.011,"I")  ;bar*1.8*33 IHS/SD/SDR ADO60817
