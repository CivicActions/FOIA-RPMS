BARFPST5 ; IHS/SD/LSL - A/R FLAT RATE POSTING ; 12/22/2008
 ;;1.8;IHS ACCOUNTS RECEIVABLE;**6,10,21,23,37**;OCT 26, 2005;Build 210
 ;;
DOC ;
 ;IHS/SD/LSL 01/01/2000 Created routine; Contains code for POSTING, EDITING, CANCELLING, or QUITING
 ;  Bills from A/R FLAT RATE POSTING
 ;
 ;IHS/ASDS/LSL 1.3 06/29/00 Mark bills for rollback capabilities.  Set BARROLL array 
 ;  during posting process.  Call EN^BARROLL after posting complete.  BARRAYGO needs to be defined as well. (Currently,
 ;  it will be set to 0 as we don't allow "Roll over as you go".)
 ;IHS/ASDS/LSL 1.3 06/29/00 Added tag ROLFIX to mark bills for rollback that were posted
 ;  before the above change.
 ;
 ;IHS/SD/LSL 1.7 10/21/02 NOIS QAA-1200-130051 Added quit logic in PSTBIL if error getting A/R Transaction
 ;
 ;IHS/SD/SDR 1.8*37 ADO60825 Added logic to make sure cashiering stays open as long as option is in use
 ;;
 Q
 ; *********************************************************************
ACTION ; EP
