BARUP ; IHS/SD/LSL - UPLOAD BILL FROM 3P ;
 ;;1.8;IHS ACCOUNTS RECEIVABLE;**19,23,24,30,34**;OCT 26, 2005;Build 139
 ;
 ;IHS/SD/LSL v1.7 11/27/02 QAA-1200-130051 Inserted documentation.
 ;IHS/SD/LSL 1.7*1 06/09/03 Don't new BAROPT.  Needed to determine if path to this routine originated from an AR Upload menu option.
 ;
 ;IHS/SD/POT 1.8*23 08/21/2012 HEAT73895 DISPLAY REASON OF NOT UPLOADING
 ;IHS/SD/SDR 1.8*24 HEAT118656 BARERR
 ;IHS/SD/SDR 1.8*34 ADO80817 updated call to MSGTX from BARUP1 to BARUP2 (routine was split)
 ;*********************************************************************
 ;
...
CHECK ;
 ...
 .;D MSGTX^BARUP1  ;CREATE MESSAGE TRANSACTION  ;bar*1.8*34 IHS/SD/SDR ADO80817
 .D MSGTX^BARUP2  ;CREATE MESSAGE TRANSACTION  ;bar*1.8*34 IHS/SD/SDR ADO80817
 S DUZ(2)=BARDUZ2
 K BARDUZ2
 Q
 ;*********************************************************************
...
GETPAR ;** visit location comes from #.03 of 3p bill file
 ...
 ;------------------------------------------------------
 ;start new IHS/SD/SDR bar*1.8*24 Belcourt HEAT118656 BARERR 
 ...
 .W !,BARERR
 ;end new IHS/SD/SDR bar*1.8*24 Belcourt HEAT118656 
 ;------------------------------------------------------ 
 S DUZ(2)=BARPAR
 ;
 ;-------------------------------
...
SETACC(BARACC) ;EP - establish record in A/R FACILITY ACCOUNT file
 ...
 Q BARACODA
ERRMSG(BARERR) ;IHS/SD/POT bar*1.8*23 HEAT73895 DISPLAY BARERR TYPE
