...
LST I LRPAT W @IOF S PNM="" F  S PNM=$O(^LAR("NAME",PNM)) Q:PNM=""  S LRDFN=0 F  S LRDFN=$O(^LAR("NAME",PNM,LRDFN)) Q:LRDFN<1  D
 ...
 . I $D(^LR(LRDFN,0))#2 N PNM S LRDPF=$P(^LR(LRDFN,0),"^",2),DFN=$P(^(0),"^",3) D DEM^LRX W !,PNM,?30,HRCN  ;IHS/ANMC/CLS 11/1/95
 .; I '$D(^LR(LRDFN,0))#2 W !!,PNM," LRDFN # "_LRDFN_" Has Been Deleted from ^LR( ",!,$C(7),"SSN = Unknown",!
 . I '$D(^LR(LRDFN,0))#2 W !!,PNM," LRDFN # "_LRDFN_" Has Been Deleted from ^LR( ",!,$C(7),!  ;IHS/ANMC/CLS 11/1/95
QUIT ;D KILL D ^%ZISC,KVAR^BLRDPT K F1,C1,C2,C3 U IO(0) Q
 ;----- BEGIN IHS MODIFICATIONS LR*5.2*1018
 D KILL D ^%ZISC D @$S($$ISPIMS^BLRUTIL:"KVAR^VADPT",1:"KVAR^BLRDPT") K F1,C1,C2,C3 U IO(0) Q
...
DQ2 I $P(^LAB(69.9,1,6,P1,0),U,4)'=3 W !!,"Not in the right state.",!! G QUIT
 ...
 K ^LAR("NAME"),^LAR("SSN"),^LAR("HRCN"),^LAR("Z"),^LAB(69.9,1,"TAPE"),^LAB(69.9,1,"LRDFN"),^LAB(69.9,1,"PURGE LRDFN") S ^LAR("Z",0)="ARCHIVED LR DATA^63.9999" G QUIT  ;IHS/ANMC/CLS 11/1/95
...
KILL W ! W:$E(IOST,1,2)="P-" @IOF
 ...
 S ZTQUE="@" D ^%ZISC K I,J,LRPAT,LRDAT,LRDPF,LRIDT,LRSS,LRSUB,P1,PNM,SSN,HRCN,X0,X1,X2,X3,ZTSK,ZTRTN,ZTIO,ZTDESC,ZTSAVE  ;IHS/ANMC/CLS 11/1/95
 Q
PRT ;
