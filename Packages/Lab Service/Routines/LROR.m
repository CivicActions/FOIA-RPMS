LROR ; IHS/DIR/AAB - LAB MODULE FOR OR 3/29/90 16:39 ; [ 11/19/2002  8:34 AM ]
 ;;5.2;LR;**1003,1011,1013,1015,1018**;Nov 18, 2004
...
V ;;from LRVER3A
 ...
 . ;I $L($P(X,"^",14)) S X=$P(X,"^",14) I $P($G(^LRO(69,+X,1,+$P(X,";",2),2,+$P(X,";",3),0)),"^",7) S ORIFN=$P(^(0),"^",7),ORSTS=2 D ST^BLRORX  ;IHS/DIR TUC/AAB 06/15/98
 . ;BEGIN IHS MODIFICATIONS LR*5.2*1018
 . ;RESTORE CALL TO ORX
 . I $L($P(X,"^",14)) S X=$P(X,"^",14) I $P($G(^LRO(69,+X,1,+$P(X,";",2),2,+$P(X,";",3),0)),"^",7) S ORIFN=$P(^(0),"^",7),ORSTS=2 ;IHS/ITSC/TPF 07/30/01 **1013**
 . N I I $D(LRNOW)!$D(LRNOWM) S ORETURN("ORSTOP")=$S($D(LRNOW):LRNOW,1:LRNOWM)
 . S ORIFN=LRIFN,ORETURN("ORSTS")=2 D RETURN^ORX S ORIFN=LRIFN
 . ;S ORIFN=LRIFN,ORETURN("ORSTS")=2 S ORIFN=LRIFN  ;IHS/DIR TUC/AAB 06/15/98
 S Y=LRY K LRY,LRNOWM,A,B,C Q
SET ;from LROW2,LRWLST
 N X,ORPK,ORL,ORNP,ORVP,ORPCL,ORPURG,ORSTRT,ORSTS,ORTX,ORIT,Y N:'$G(BLRGUI) LRURG
 ...
 S ORPK=LRODT_"^"_LRSN_"^"_LRTN,ORIT=+LRTEST(LRI)_";LAB(60," S:'$G(BLRGUI) LRURG=$P(LRTEST(LRI),"^",2) S X=$G(^LRO(69,+LRODT,1,+LRSN,0)),X1=$G(^(1))
...
SET1 S:'$D(ORPCL) ORPCL=$P(^LAB(69.9,1,1),U,6)_";ORD(101,"
 ...
 ;D FILE^BLRORX S LRORIFN=$S($D(ORIFN):ORIFN,1:""),LRODT=LRODTSV,LRSN=LRSNSV,LRTN=LRTNSV  ;IHS/DIR TUC/AAB 06/15/98
 ;S LRORIFN=$S($D(ORIFN):ORIFN,1:""),LRODT=LRODTSV,LRSN=LRSNSV,LRTN=LRTNSV  ;/IHS/ITSC/TPF 07/30/01 TOOK OUT DUMMY CALL TO BLRORX
...
EN(AREA,DATE,ACC) ;
 ...
 I $O(FLAG(0)) D MSG S ORBPMSG=MSG,ORNOTE(50)=1,ORVP=$S(LRDPF=2&($G(DFN)'=""):DFN,LRDPF=2&($G(LRDFN)'=""):LRDFN,1:"") Q:'ORVP  S ORVP=ORVP_";DPT(",II=0 D
