LRVR1 ;DALOI/CJS/JAH - LAB ROUTINE DATA VERIFICATION ; 06-Apr-2022 06:25 ; MKK
 ;;5.2;LAB SERVICE;**42,153,221,1018,286,1027,291,1031,1051**;NOV 1, 1997;Build 19
 ;
 ; MSC/MKK - LR*5.2*1051 - 06-Apr-2022 - Item 75913 - No Lab Data Error Fix
 ;
 ;
...
V2 K LRPRGSQ(LRSQ)
 ...
 ; 
 ; ----- BEGIN IHS/MSC/MKK - LR*5.2*1051 - 06-Apr-2022 - Item 75913
 NEW LRASTR
 S LRASTR=$P($G(^LRO(68,LRAA,1,LRAD,1,LRAN,.3)),U)    ; UID
 I $L(LRASTR) S LRASTR="UID:"_LRASTR
 E  S LRASTR=$G(^LRO(68,LRAA,1,LRAD,1,LRAN,.2))  ; Accession Number
 I LRIDT<1 W !,"NOTICE: NO POINTER TO FILE #63. ",LRASTR,!  Q
 I '$D(^LR(LRDFN,LRSS,LRIDT,0)) W !,"NOTICE: NO POINTER TO FILE #63. ",LRASTR,!  Q
 ; ----- END IHS/MSC/MKK - LR*5.2*1051 - 06-Apr-2022 - Item 75913
 ;
 S LRCW=8
LD S LRSS="CH"
