ADGPI ; IHS/ADC/PDW/ENM - PATIENT INQUIRY ; [ 09/17/2002  4:19 PM ]
 ;;5.3;PIMS;**1001,1003,1004,1005,1010,1011,1012,1021,1022**;MAY 28, 2004;Build 18
 ;
SP ; -- select patient
...
DM ; -- demographic data
 ...
 ;
 ;97823 maw p1022 PPN
 S DGDPTN0=$$GETPREF^AUPNSOGI(DFN,"E",1)
 ;S DGDPTN0=^DPT(DFN,0),
 S DGDPTN11=$G(^DPT(DFN,.11)),DGDPTN13=$G(^DPT(DFN,.13))
 S DGPATN0=$G(^AUPNPAT(DFN,0)),DGPATN11=$G(^AUPNPAT(DFN,11))
 W !!?13,"***Confidential Patient Data Covered by Privacy Act***"
 W !!,$P(DGDPTN0,U)
 W !,?32,"HRCN: ",$$HRCN^ADGF,?54,"DOB: ",$$DOB
 W !,$P(DGDPTN11,U),?31,"PHONE: ",$P(DGDPTN13,U),?53,"PROV: ",$$PCP
 W !,$P(DGDPTN11,U,4),"  ",$$ST,"  ",$P(DGDPTN11,U,6)
 ;W ?32,"ELIG: ",$$ELIG,?54,"SSN: ",$P(DGDPTN0,U,9) Q  ;20220512 orig
 W ?32,"ELIG: ",$$ELIG,?54,"SSN: XXX-XX-",$E($P(DGDPTN0,U,9),6,9) Q  ;20220512 p1021 84778
 ;
CS ; -- current status
 ;D INP^DGRPD Q
