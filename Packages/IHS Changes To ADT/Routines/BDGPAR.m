BDGPAR ; IHS/ANMC/LJF - CALLS FOR ADT PARAMETERS ;  [ 06/16/2003  2:41 PM ]
 ;;5.3;PIMS;**1011,1012,1023**;APR 26, 2002;Build 24
 ;
...
ACTHS(N) ;EP; returns 1 if hospital service active and clinical today
 ...
 Q 1                                           ;reopened
 ;
CRECMAX(DT) ;RETRIEVE CENSUS RECALC MAX BACKTRACK LIMIT  ;PIMS*5.3*1023
 ;RETURNED VALUES:
 ; >0 = FM-FORMATTED VALUE OF DATE CORRESPONDING TO UPPER LIMIT IDENTIFIED
 ;      IN IHS ADT PARAMETERS FILE FOR IDENTIFIED MEDICAL CENTER DIVISION
 ;  0 = NO MAX LIMIT DEFINED FOR IDENTIFIED MEDICAL CENTER DIVISION
 ; 
 N BDGDIV,LFROM,OLDDT,X,X1,X2,YEAR
 S YEAR=-365.2422  ;DECREMENTAL VALUE OF ONE YEAR, IN DAYS
 S BDGDIV=$$DIV^BDGPAR(DUZ(2))  ;MEDICAL CENTER DIVISION
 S LFROM=+$G(^BDGPAR(BDGDIV,4))  ;PARAMETER-SPECIFIED 'FROM' MAXIMUM LIMIT, IN YEARS
 I +LFROM D  ;FOLLOW THIS FORK IF MAX BACKTRACK VALUE SPECIFIED
 . S X1=DT
 . S X2=LFROM*YEAR  ;CONVERT MAX BACKTRACK YEAR SPEC INTO DAYS
 . D C^%DTC         ;AND CALCULATE CORRESPONDING FM-FORMATTED DATE VALUE
 . I 1
 E  S X=0  ;RETURN ZERO IF NO MAX BACKTRACK LIMIT SPECIFIED
 Q X
