...
PAC(PKG,VER) ; called from package init (DIFROM7 created this routine)
 ...
 ;
 N %,%I,%H,DATE,DIFROM,NOW,PACKAGE,RUN,SERVER,SITE,START,X,XMDUZ,XMSUB,XMTEXT,XMY,Y K ^TMP("APC5INIS",$J)
 ;
 ...
 ;
 ; Message for server
 S ^TMP("APC5INIS",$J,1,0)="PACKAGE INSTALL"
 S ^TMP("APC5INIS",$J,2,0)="SITE: "_SITE
 S ^TMP("APC5INIS",$J,3,0)="PACKAGE: "_PACKAGE
 S ^TMP("APC5INIS",$J,4,0)="VERSION: "_VER
 S ^TMP("APC5INIS",$J,5,0)="Start time: "_START
 S ^TMP("APC5INIS",$J,6,0)="Completion time: "_NOW
 S ^TMP("APC5INIS",$J,7,0)="Run time: "_RUN
 S ^TMP("APC5INIS",$J,8,0)="DATE: "_DATE
 ;
 ; Data is sent to server on FORUM - S.A5CSTS
 S XMY(SERVER)="",XMDUZ=.5,XMTEXT="^TMP(""APC5INIS"",$J,",XMSUB=PACKAGE_" VERSION "_VER_" INSTALLATION"
 D ^XMD
 K ^TMP("APC5INIS",$J)
 Q
