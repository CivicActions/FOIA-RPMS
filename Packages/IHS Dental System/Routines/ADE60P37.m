ADE60P37 ;IHS/OIT/GAB - ADE V 6.0 PATCH 37 [ 12/06/2022  2:35 PM ]
 ;;6.0;ADE IHS DENTAL;**37**;March 25, 1999;Build 6
 ;;2023 ADA-CDT Code Updates /IHS/OIT/GAB 12/2022
 ;
ENV ;Environment check
 ...
 I '$$CHKPAT("ADE*6.0*36") S XPDQUIT=1
 D OK
 ;
 Q
 ;
...
PRE ;EP -- PRE-INSTALL FROM KIDS.
 ...
 D ^XBKVAR
 ;
 D BMES^XPDUTL("Beginning pre-install routine (PRE^ADE60P37)...")
 ;
...
POST ;EP Post-Install
 ; only post for patch 37 - /IHS/OIT/GAB *37* 12/2022
 ;
 N ADED,ADECNT,ADEVALUE
 D BMES^XPDUTL("Adding new Dental Codes for 2023...")
 D ADDCDT37^ADE6P371
 D BMES^XPDUTL(" ...DONE")
 D BMES^XPDUTL("Inactivating Dental Codes for 2023...")
 D DELCDT37^ADE6P373
 D BMES^XPDUTL("...DONE")
 D MAIL
 Q
...
MAIL ;SEND OUT A MAILMAN MESSAGE 
 ...
 K ^TMP("ADEP37",$J)
 ;I $G(ADEQUIT)=1 S ^TMP("ADEP37",$J,1)=" --- ADE v6.0, Patch 36 HAS NOT BEEN INSTALLED into this namespace ---"
 S ^TMP("ADEP37",$J,1)=" --- ADE v6.0, Patch 37 has been installed into this namespace ---"
 I $G(XPDA) D
 . S %=0
 . F  S %=$O(^XTMP("XPDI",XPDA,"BLD",XPDBLD,1,%)) Q:'%   D
 . . S ^TMP("ADEP37",$J,(%+1))=" "_^(%,0)
 ;
 S XMSUB=$P($P($T(+1),";",2)," ",3,99)
 S XMDUZ=$S($G(DUZ):DUZ,1:.5)
 S XMTEXT="^TMP(""ADEP37"",$J,"
 S XMY(1)=""
 S XMY(DUZ)=""
 ;
