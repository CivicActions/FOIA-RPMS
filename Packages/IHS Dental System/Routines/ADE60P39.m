ADE60P39 ;IHS/OIT/GAB - ADE V 6.0 PATCH 39 [ 08/06/2023  2:35 PM ]
 ;;6.0;ADE IHS DENTAL;**39**;March 25, 1999;Build 7
 ;;/IHS/OIT/GAB October 2023 Patch 39
 ;
ENV ;Environment check
 ...
 I '$$CHKPAT("ADE*6.0*38") S XPDQUIT=1
 D OK
 ;
 Q
 ;
...
PRE ;EP -- PRE-INSTALL FROM KIDS.
 ...
 D ^XBKVAR
 ;
 D BMES^XPDUTL("Beginning pre-install routine (PRE^ADE60P39)...")
 ;
...
POST ;EP Post-Install
 ; only post for patch 39 - /IHS/OIT/GAB *39*
 ;
 D MAIL
 Q
 ;
...
MAIL ;SEND OUT A MAILMAN MESSAGE 
 ...
 K ^TMP("ADEP39",$J)
 S ^TMP("ADEP39",$J,1)=" --- ADE v6.0, Patch 39 has been installed into this namespace ---"
 I $G(XPDA) D
 . S %=0
 . F  S %=$O(^XTMP("XPDI",XPDA,"BLD",XPDBLD,1,%)) Q:'%   D
 . . S ^TMP("ADEP39",$J,(%+1))=" "_^(%,0)
 ;
 S XMSUB=$P($P($T(+1),";",2)," ",3,99)
 S XMDUZ=$S($G(DUZ):DUZ,1:.5)
 S XMTEXT="^TMP(""ADEP39"",$J,"
 S XMY(1)=""
 S XMY(DUZ)=""
 ;
