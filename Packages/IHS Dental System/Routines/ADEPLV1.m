ADEPLV1 ; IHS/HQT/MJL - DENTAL LVL OF SVC ;  [ 03/24/1999   9:04 AM ]
 ;;6.0;ADE;**15,38**;MAR 25, 1999;Build 158
...
RO1 S ADEDFN=$O(^ADEPCD("AC",ADEDT,ADEDFN)) G:ADEDFN="" ROLL
 ...
 ;/IHS/OIT/GAB PATCH 38 - Commented above line and added below line for File 3,6,16 removal, File200 update PROVIDER CLASS - CK ABOVE FOR $P is this correct?
 I ADEPDET S ADERPD=$P(ADENOD,U,ADEPRO) G:ADERPD="" RO1 G:$P(^VA(200,ADERPD,"PS"),U,5)']"" RO1 I ADEPRO=5,$P(^DIC(7,$P(^VA(200,ADERPD,"PS"),U,5),9999999),U)=52 G RO1
...
P2 ;I ADEPDET S ADERPD=$O(@ADEGBL@(ADELOE,ADERPD)) I '+ADERPD S ADEDAT=@ADEGBL@(ADELOE,0) W !,"TOTAL FOR ",ADEFAC,":" D CALC G:ADEQIT END G P1
 ...
 ;/IHS/OIT/GAB PATCH 38 - Commented above line and added below line for File 3,6,16 removal, File200 update PRINT PROV NAME
 I ADEPDET W !,$P(^VA(200,ADERPD,0),U) S ADEDAT=@ADEGBL@(ADELOE,ADERPD),ADERVUD=@ADEGBL@(ADELOE,ADERPD,"RVU") D CALC G:ADEQIT END G P2
 ;----- END IHS MODIFICATIONS
...
CALC ;
 ...
 . D EOP
 ;F Z=1:1:8 S ADELV=Z-1 S:ADELV=7 ADELV=9 W !,"LEVEL ",ADELV,": ",?20,$J($P(ADEDAT,U,(Z*2-1)),4),?30,$J($P(ADEDAT,U,Z*2),6),?40,$J(+$P($G(ADERVUD),U,Z*2),6) S:ADELV>0&(ADELV<7) ADESVRT=ADESVRT+$P(ADEDAT,U,(Z*2-1)),
 ;ADESMRT=ADESMRT+$P(ADEDAT,U,(Z*2)),ADERVUTO=ADERVUTO+$P($G(ADERVUD),U,Z*2) D EOP Q:ADEQIT
 ;
