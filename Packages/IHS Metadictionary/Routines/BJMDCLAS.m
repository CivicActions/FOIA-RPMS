BJMDCLAS ;VNGT/HS/AM-Post Install ; 06 May 2010  12:51 PM
...
EXPORT ;
 ...
 F  R !,"Class: ",MASK:60 Q:MASK=""  S:MASK'?1.E1".CLS"&(MASK'?1.E1".cls") MASK=MASK_".CLS" S CLASSES(MASK)=""
 Q:'$D(CLASSES)
 ; Create a new global-based character stream
EXPGO ;
 ; Create new record in 90608 with IEN of REC
 ; Populate Name (not sure what it should be), Date/Time, and set status to I
 K DO,DA S DIC=90608,DLAYGO=90608,DIC(0)="L",X=$S($G(NAME)'="":NAME,1:"New Record for "_$H)
 ...
 . S B64=$TR(B64,$C(10))
 . F I=1:1 SET XML=$P(B64,$C(13),I) Q:XML=""  D POPULATE^BJMDCLAS(REC,XML) Q:ERR
 . I ERR D ERROR
 . D POPULATE^BJMDCLAS(REC,"------------------------- SEGMENT END ------------------------")
 S DIE="^BJMDCLS("
 K DA S DA=REC
 ; Set Status to READY
 S DR="1.02////R"
...
IMPORT(REC,ERR) ;
 ...
 ; Change the value of field RPMS STATUS in 90608 to "I"
 K DA S DA=REC,DIE="^BJMDCLS(",DR="1.02////I" D ^DIE
 ; Erase old errors
 ...
 F  D  Q:B64=""
 . S B64=""
 . F  S I=$O(^BJMDCLS(REC,10,I)) Q:'I  S STR=^BJMDCLS(REC,10,I,0) Q:STR["SEGMENT END"  S B64=B64_STR_$C(13,10)
 . I B64="" Q
 ...
 ; 7/26/12 DMB SCCB-P04979 Add /journal=0 for Ensemble 2012.1 as the default changed from journal off to 
 ;                         journal on in 2012.1
 S EXEC="D $system.OBJ.LoadStream(STREAM,""ck/lock=0/journal=0"",.ERR,.LOADED,1)"
 X EXEC
 ...
 ; 7/26/12 DMB SCCB-P04979 Add /journal=0 for Ensemble 2012.1 as the default changed from journal off to 
 ;                         journal on in 2012.1
 S EXEC="D $system.OBJ.LoadStream(STREAM,""ck/lock=0/journal=0"",.ERR,.LOADED)"
 X EXEC
 ; Error processing after the actual load
 I ERR D BGERROR G EXIT
 ;
 S CLASS="" F  S CLASS=$O(LOADED(CLASS)) Q:CLASS=""  D
 . S DIC="^BJMDCLS("_REC_",11,",DIC(0)="L",DLAYGO=1
 . S DA(1)=REC
 . S X=CLASS
 . D ^DIC
 ; Change the value of the field RPMS STATUS in 90608 to "R"
 ; and populate RPMS DATE/TIME INSTALLED
 S DIE="^BJMDCLS("
 K DA S DA=REC
 ; Set Status to READY
 W !,"Updating 90608 Record"
 D NOW^%DTC
 S DR="1.02////C;1.03////"_%
 D ^DIE
EXIT ;
...
POPULATE(REC,XML) ;
 S DA(1)=REC
 K DIC S DIC="^BJMDCLS("_DA(1)_",10," ;XML Subfile
...
BGERROR ;
 ...
 ; Change the value of the field RPMS STATUS in 90608 to "E"
 ; S ERRTEXT=""
 S DIE="^BJMDCLS("
 I '$G(REC) Q
 I '$D(^BJMDCLS(REC)) Q
 ...
 S DR="1.02////E"
 D ^DIE
 ; If this is in the foreground, display the error to the user. Only BFMC errors belong in 90607. 6/08/12 GCD SCCB-P03461
 I $G(LOAD)'="BFMC" D FGERROR Q
 ; 6/08/12 GCD SCCB-P03461 Moved this code down since we don't need to do it twice for foreground error reporting.
 ...
 ;
FGERROR ;
 ; 6/08/12 GCD SCCB-P03461
 I $D(ERR)=1 S ERR(1)=ERR,ERR=1
 I $D(ERR)#10 S I="" D
 . F  S I=$O(ERR(I)) Q:'I  W !,$TR(ERR(I),$C(10,13),"  ")
