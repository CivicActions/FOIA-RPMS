AUPN9929 ; IHS/OIT/FBD&NKD - AUPN V99.1 PATCH 29 ENVIRONMENT CHECKS AND POST-INIT PROCESS ; 11/23/2021
 ;;99.1;IHS DICTIONARIES (PATIENT);**29**;MAR 9, 1999;Build 1
 ;
 ;
 D:'$D(IOM) HOME^%ZIS
 ;
 ...
 S:'$$VCHK("AUPN","99.1","28") XPDQUIT=2  ; AUPN
...
MAIL ;----- SEND INSTALL MAIL MESSAGE 
 ...
 KILL ^TMP("AUPN9929",$J)
 S ^TMP("AUPN9929",$J,1)=" --- AUPN v 99.1, Patch 29, has been installed into this uci ---"
 S %=0
 F  S %=$O(^XTMP("XPDI",XPDA,"BLD",XPDBLD,1,%)) Q:'%   S ^TMP("AUPN9928",$J,(%+1))=" "_^(%,0)
 S XMSUB=$P($P($T(+1),";",2)," ",3,99),XMDUZ=$S($G(DUZ):DUZ,1:.5),XMTEXT="^TMP(""AUPN9929"",$J,",XMY(1)="",XMY(DUZ)=""
