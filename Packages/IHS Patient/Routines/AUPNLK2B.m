AUPNLK2B ; IHS/CMI/LAB - Broke up AUPNLK2 because of size ;
 ;;99.1;IHS DICTIONARIES (PATIENT);**12,28**;MAR 09, 1999;Build 1
 ;IHS/OIT/FBD&NKD AUPN*99.1*28 BIRTH SEX
 ;
...
CHKID ; CHECK IDENTIFIERS
 ...
 ;
CHKID1 ;
 ;S AUP("DR")=$S('$D(AUP("DR")):AUPID,1:AUP("DR")_";"_AUPID) I $D(^DD(2,AUPID,0)) S AUPID0=^(0) D ASKID S:'$D(X) AUPQF2=4  ;IHS/OIT/FBD&NKD AUPN*99.1*28 BIRTH SEX
 S AUP("DR")=$S('$D(AUP("DR")):AUPID,1:AUP("DR")_";"_AUPID) I $D(^DD(2,AUPID,0)) S AUPID0=^(0) S:$P(AUPID0,U)="SEX" $P(AUPID0,U)="BIRTH SEX" D ASKID S:'$D(X) AUPQF2=4
 Q
 ;
