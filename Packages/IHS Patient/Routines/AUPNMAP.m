AUPNMAP ; IHS/OIT/FBD - MAPPER API ; 27 Jul 2018  9:38 AM
 ;;2.0;IHS PCC SUITE;**10,22,24,31**;MAY 14, 2009;Build 10
 ;
 ;IHS/CMI/LAB - PATCH 24, ROUTINE REPLACED WITH ROUTINE MODIFIED BY ARLIS FOR ECQM
...
RAD1 ;
 ...
 S LK=$$GET1^DIQ(71,LK,9,"I")
 I LK="" Q  ;no cpt code
 S V2=$$GET1^DIQ(AUPNFILE,AUPNDA,.03,"I")  ;VISIT IEN
 ;CALL CQM MAP API TO GET CODES
 I $T(MM^BCQMAPI)="" Q  ; no mapper so don't bother
 S X=$$MM^BCQMAPI(AUPNTF,LK,"I",V1,V2,V3,V4,V5,V6,$$VD^APCLV($$VALI^XBDIQ1(AUPNFILE,AUPNDA,.03)),"CODES")
 I 'X Q  ;NO CODES to stuff
 ;now set snomed and loinc multiples in entry DA
 S AUPNX=0 F  S AUPNX=$O(CODES(AUPNX)) Q:AUPNX=""  D
 .S AUPNT=$O(CODES(AUPNX,""))  ;this will be the type of code
...
HF1 ;
 ...
 S (V1,V2,V3,V4,V5,V6)=""
 S LK=$$GET1^DIQ(AUPNFILE,AUPNDA,.01)
 S V1=$$GET1^DIQ(AUPNFILE,AUPNDA,.07,"I")
 S V2=$$GET1^DIQ(AUPNFILE,AUPNDA,.08,"I")
 S V3=AUPNDA
 ;S V1=$$GET1^DIQ(AUPNFILE,AUPNDA,.04,"I")
 D MAP
 Q
...
CPT ;EP - CALLED FROM XREF ON V CPT NEW STYLE XREF
 NEW AUPNDA,AUPNFILE,AUPNTF,AUPNVV
 S AUPNDA=DA
 S AUPNFILE=9000010.18
 S AUPNTF=81
 S AUPNVV=$G(APCDVSIT)
 D EN^XBNEW("CPT1^AUPNMAP","AUPNDA;AUPNFILE;AUPNTF;AUPNVV")
 Q
CPT1 ;
 D DELGEN
 S (V1,V2,V3,V4,V5,V6)=""
 S LK=$$GET1^DIQ(AUPNFILE,AUPNDA,.01,"I")
 S V2=$$GET1^DIQ(AUPNFILE,AUPNDA,.03,"I")  ;VISIT IEN
 I V2="" S V2=$G(AUPNVV)  ;FOR DATA ENTRY
 ;D MAP
 ;CALL CQM MAP API TO GET CODES
 I $T(MM^BCQMAPI)="" Q  ; no mapper so don't bother
 S X=$$MM^BCQMAPI(AUPNTF,LK,"I",V1,V2,V3,V4,V5,V6,$$VD^APCLV($$VALI^XBDIQ1(AUPNFILE,AUPNDA,.03)),"CODES")
