AUPN9930 ; IHS/OIT/FBD&NKD - AUPN V99.1 PATCH 30 ENVIRONMENT CHECKS AND POST-INIT PROCESS ; 09/16/2024
 ;;99.1;IHS DICTIONARIES (PATIENT);**30**;MAR 9, 1999;Build 1
 ;
 ;
 D:'$D(IOM) HOME^%ZIS
 ;
 ...
 S:'$$VCHK("AUT","98.1","33") XPDQUIT=2   ; AUT
 S:'$$VCHK("AUPN","99.1","29") XPDQUIT=2  ; AUPN
...
MAIL ;----- SEND INSTALL MAIL MESSAGE 
 ...
 KILL ^TMP("AUPN9930",$J)
 S ^TMP("AUPN9930",$J,1)=" --- AUPN v 99.1, Patch 30, has been installed into this uci ---"
 S %=0
 F  S %=$O(^XTMP("XPDI",XPDA,"BLD",XPDBLD,1,%)) Q:'%   S ^TMP("AUPN9930",$J,(%+1))=" "_^(%,0)
 S XMSUB=$P($P($T(+1),";",2)," ",3,99),XMDUZ=$S($G(DUZ):DUZ,1:.5),XMTEXT="^TMP(""AUPN9930"",$J,",XMY(1)="",XMY(DUZ)=""
