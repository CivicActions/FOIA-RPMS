AUPN9928 ; IHS/OIT/FBD&NKD - AUPN V99.1 PATCH 28 ENVIRONMENT CHECKS AND POST-INIT PROCESS ; 03/04/2020
 ...
 S:'$$VCHK("AUT","98.1","29") XPDQUIT=2   ; AUT
 S:'$$VCHK("AUPN","99.1","27") XPDQUIT=2  ; AUPN
...
LAST(PKG,VER) ; EP - returns last patch applied for a Package, PATCH^DATE
 ...
 ;
PRE ; EP FR KIDS
 D BMES^XPDUTL("Preparing Eligibility Modifiers File for update...")
 N DIK,DA
 S DIK="^AUPNELM(" F DA=24,28,30 D ^DIK
 Q
 ;
POST ; EP FR KIDS
...
MAIL ;----- SEND INSTALL MAIL MESSAGE 
 ...
 KILL ^TMP("AUPN9928",$J)
 S ^TMP("AUPN9928",$J,1)=" --- AUPN v 99.1, Patch 28, has been installed into this uci ---"
 S %=0
 F  S %=$O(^XTMP("XPDI",XPDA,"BLD",XPDBLD,1,%)) Q:'%   S ^TMP("AUPN9928",$J,(%+1))=" "_^(%,0)
 S XMSUB=$P($P($T(+1),";",2)," ",3,99),XMDUZ=$S($G(DUZ):DUZ,1:.5),XMTEXT="^TMP(""AUPN9928"",$J,",XMY(1)="",XMY(DUZ)=""
