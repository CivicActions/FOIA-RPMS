AUPNLK ; IHS/CMI/LAB - IHS PATIENT LOOKUP MAIN ROUTINE 24-MAY-1993 ; [ 03/16/2004  7:43 AM ]
 ;;99.1;IHS DICTIONARIES (PATIENT);**5,6,9,10,12,13,28**;MAR 09, 1999;Build 1
 ;patch 5 - fm v22
...
WRT ;
 ...
 ;D EN^DDIOL($S($D(AUPSEL)!($P(AUPS(AUPDFN),U)'=$P(AUPS(AUPDFN),U,2)):"  "_$P(AUPS(AUPDFN),U)_"  ",1:"  "))  ;IHS/OIT/FBD&NKD AUPN*99.1*28 PREFERRED NAME LOOKUP DISPLAY
 D EN^DDIOL($S($D(AUPSEL)!($P(AUPS(AUPDFN),U)'=$P(AUPS(AUPDFN),U,2)):"  "_$$GETPREF^AUPNSOGI(AUPDFN,"E")_"  ",1:"  "))
 S Y=+AUPDFN X:$D(^DPT(AUPDFN,0)) DIC("W")
 Q
 ;
WRT2 ;
 ;D EN^DDIOL("  "_$P(^DPT(AUPDFN,0),U)),EN^DDIOL("OK","","!?8")  ;IHS/OIT/FBD&NKD AUPN*99.1*28 PREFERRED NAME LOOKUP DISPLAY
 D EN^DDIOL("  "_$$GETPREF^AUPNSOGI(AUPDFN,"E")),EN^DDIOL("OK","","!?8")
 S %=1 D YN^DICN
