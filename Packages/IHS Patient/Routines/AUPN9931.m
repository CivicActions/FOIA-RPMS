AUPN9931 ; IHS/OIT/NKD - AUPN V99.1 PATCH 31 ENVIRONMENT CHECKS AND POST-INIT PROCESS ; 05/14/2025
 ;;99.1;IHS DICTIONARIES (PATIENT);**31**;MAR 9, 1999;Build 1
 ;
 ;
 D:'$D(IOM) HOME^%ZIS
 ;
 ...
 S:'$$VCHK("AUPN","99.1","30") XPDQUIT=2  ; AUPN
...
MAIL ;----- SEND INSTALL MAIL MESSAGE 
 ...
 KILL ^TMP("AUPN9931",$J)
 S ^TMP("AUPN9931",$J,1)=" --- AUPN v 99.1, Patch 31, has been installed into this uci ---"
 S %=0
 F  S %=$O(^XTMP("XPDI",XPDA,"BLD",XPDBLD,1,%)) Q:'%   S ^TMP("AUPN9931",$J,(%+1))=" "_^(%,0)
 S XMSUB=$P($P($T(+1),";",2)," ",3,99),XMDUZ=$S($G(DUZ):DUZ,1:.5),XMTEXT="^TMP(""AUPN9931"",$J,",XMY(1)="",XMY(DUZ)=""
