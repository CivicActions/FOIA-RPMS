...
SEND(DIALOUT) ;EP - from ABSPOSQ3
 S ABSPECT2=0
 ;
 ; /IHS/OIT/RAM ; 25 MAR 24 ; DEBUG TO SEE IF THIS EVEN GETS 'HIT'... COMMENT OUT WHEN WORKING.
 S ABSPNEXT=$O(^ABSPDEBUG(9999999),-1)+1
 S ^ABSPDEBUG(ABSPNEXT)=$H
...
S12 ; 
 ...
 ;
 ; /IHS/OIT/RAM ; 25 MAR 24 ; REORDERING THIS EARLIER TO ACCOUNT FOR NEW CONNECTIVITY VIA HTTP POST
 ...
 N IO ; /IHS/OIT/RAM ; 25 MAR 24 ; WE CAN STILL NEW IO - BUT IF HTTP POST WE DON'T NEED IT ANY LONGER.
 I 'HTTPPOST S IO=$$IO^ABSPOSA(DIALOUT) I IO="" G S12:$$IMPOSS^ABSPOSUE("DB","TRI","IO field missing in DIALOUT="_DIALOUT,,"S12",$T(+0))
...
LOOP0 ; Wait for host to send ENQ
 ;
 I '(T1LINE!HTTPPOST) D  I RET Q RET ; /IHS/OIT/RAM ; 21 MAR 24 ; ALSO DON'T INITIATE IF HTTP POST.
...
LOOP1A D LOG("2 - Sending message #"_CLAIMIEN)
 ...
 ; /IHS/OIT/RAM; 21 MAR 24 ; NEW BRANCH AS SEND/RECEIVE FOR HTTP POST IS A SINGLE OPERATION.
 I 'HTTPPOST D SENDREQ^ABSPOSAS(DIALOUT,.SENDMSG)
...
LOOP1B I T1LINE S HMSG="ACK" G LOOP1C
 I HTTPPOST S HMSG="ACK" G LOOP1C ; /IHS/OIT/RAM ; 21 MAR 24 ; ALSO SKIP THE HANDSHAKES IF HTTP POST.
...
LOOP1C I HMSG="ACK" D
 ...
 . I HTTPPOST S HMSG="STX" Q  ; /IHS/OIT/RAM ; 21 MAR 24 ; ALSO 'ASSUME' WE RECEIVE STX IF HTTP POST.
...
LOOP3 S (GETMSG,LRC)=""
 ...
 I HTTPPOST D  ; /IHS/OIT/RAM ; 25 MAR 24 ; FOLLOWED ALL OTHER LOGIC (MOSTLY) TILL NOW - IF HTTP POST LET'S PROCESS MESSAGE.
 . S ABSPNEXT=$O(^ABSPDEBUG(9999999),-1)+1
 . S ^ABSPDEBUG(ABSPNEXT)=SENDMSG
 . S RESPMSG=$$SENDMSG^ABSPHTTP(DIALOUT,SENDMSG)
 . S ABSPNEXT=$O(^ABSPDEBUG(9999999),-1)+1
 . S ^ABSPDEBUG(ABSPNEXT)=RESPMSG
 . I +RESPMSG=0 S HMSG="ETX",RESPLRC="HTTPOK" ; /IHS/OIT/RAM ; 25 MAR 24 ; SENT MESSAGE, IF RESPONSE STARTS WITH 'D0' IT'S A SUCCESS.
 . E  S HMSG="HTTPERR",LRC=""
 E  S HMSG=$$GETMSG^ABSPOSAR(DIALOUT,.RESPMSG,.RESPLRC,60)
 ;
 ;         HMSG="ETX" or "EOT" or "" (if timed out)
 ;
 S ABSPNEXT=$O(^ABSPDEBUG(9999999),-1)+1
 S ^ABSPDEBUG(ABSPNEXT)="MADE IT PAST THE GAUNTLET... "_HMSG_" "_RESPLRC
 ;
 I HMSG="ETX" D
 . S ABSPNEXT=$O(^ABSPDEBUG(9999999),-1)+1
 . S ^ABSPDEBUG(ABSPNEXT)="ABOUT TO LOG "_RESPLRC
 ...
 . D CLAIMEND
 . I HMSG'="+++" D HANGUP
 . S RET=$S(HMSG="+++":30261,HMSG="":30262,HTTPPOST:RESPMSG,1:30263)
 ;
 ; Test LRC character.  If we agree, we send ACK.  If not, we NAK.
 ; And if we send ACK, get the next message to send, if any.
 S ABSPNEXT=$O(^ABSPDEBUG(9999999),-1)+1
 S ^ABSPDEBUG(ABSPNEXT)="PAST ETX CHECKING CODE"
 ;
 I T1LINE!HTTPPOST S LRCOK=1 ; G PASTLRC ; if  T1 connection, LRC is n/a ; /IHS/OIT/RAM and also account for HTTP Post
 ...
 . . Q:T1LINE!HTTPPOST  ; /IHS/OIT/RAM ; 25 MAR 24 ; ACCOUNT FOR HTTP POST
...
LR L +^ABSPECX("POS",DIALOUT,"R",CLAIMIEN):300 ; lock the response
 ...
 I T1LINE!HTTPPOST Q 0  ; /IHS/OIT/RAM ; 25 MAR 24 ; ACCOUNT FOR HTTP POST
