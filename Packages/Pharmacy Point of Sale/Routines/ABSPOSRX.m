ABSPOSRX ; IHS/FCS/DRS - callable from RPMS pharm ;     [ 01/21/2003  8:40 AM ]
 ;;1.0;PHARMACY POINT OF SALE;**3,4,5,31,40,44,56**;JUN 1, 2001;Build 131
 Q
 ;
 ;Also used by other ABSPOSR* routines to find transactions that need to be submitted to Point of Sale.
 ;------------------------------------------------------
 ;IHS/SD/lwj 11/25/02  change in the task subroutine. There was a strange problem at Benewah only where the
 ;tasking to task man from "claim" was not functioning properly. The claim would task, but nothing would
 ;happen, and the task would disappear from taskman. If the same site deleted or editted the prescription,
 ;the same task code would work perfect. No other site has ever reported this problem and the biggest
 ;difference is ILC software running under the same name space at Benewah. To get around the problem
 ;we altered TASK to new everything. This worked, so we are keeping the fix in the package. This fix
 ;was generated by Patrick Cox of Oklahoma.
 ;-------
 ;IHS/SD/lwj 1/9/03 the new we added in November caused our "other" sites (i.e. not Benewah) problems
 ;with their posting. It turns out that the DUZ wasn't always being submitted with the task. The DUZ
 ;is required for the posting to 3rd party, so when it wasn't there, it didn't post correctly.  Altered
 ;the new to do everything BUT the DUZ.
 ;------------------------------------------------------------
 ;IHS/OIT/SCR 1.0*31 4/17/09MOREDATA("RXREASON") is defined and set in Outpatient Pharmacy 7.0
 ;when an prescription has been reversed and provides one of three possible reason for the reversal:
 ;   Prescription logically deleted
 ;   Returned to stock
 ;   Reversal caused by edit
 ; MOREDATA("RXREASON") is only defined for 'UNCLAIM' transaction types
 ;IHS/SD/SDR 1.0*56 ADO122824 Added message to screen so if ABSP lock fails user knows right away
 ; 
...
DOCU1 ; There are only four callable entry points!
 ...
 ;
 ;  Note: If the claim has already been processed, and it's resubmitted,
 ;    then a reversal will be done first,and then the resubmit will be done.
 ;    Intervening calls to $$STATUS may show progress of the reversal before
 ...
 ;                 = 0^reason = failure (should never happen)
 ;
 ;    Note: The reversal will actually be done ONLY if the most recent processing
 ;      of the claim resulted in something reversible, namely  E PAYABLE or PAPER
 ; = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = =
 ; $$STATUS^ABSPOSRX    inquire about a claim's status
 ;
 ...
 ;    E PAYABLE, E REJECTED, E CAPTURED, E DUPLICATE, E REVERSAL ACCEPTED,  E REVERSAL REJECTED
 ;    E OTHER, PAPER, PAPER REVERSAL
 ;      (PAPER categories include uninsured patients,
 ;      even beneficiaries, as well as non-electronic insurances)
 ;
 ;  "time" is the Fileman date and time of the last update in the status of this claim.
 ;
...
CLAIM(RXI,RXR,MOREDATA)     ;EP - ABSPOSR1
 ...
 I '$D(RXR) S RXR=0
 ;I '$$LOCK("SUBMIT") Q 0  ;absp*1.0*56 IHS/SD/SDR ADO122824
 ;I '$$LOCK("SUBMIT") U 0 W !!,$$EN^ABSPVDF("RVN"),"ABSP Pharmacy Point Of Sale claim did NOT generate",$$EN^ABSPVDF("RVF"),! Q 0  ;absp*1.0*56 IHS/SD/SDR ADO122824  ;absp*1.0*57 IHS/SD/SDR ADO125991
 I '$$LOCK("SUBMIT") U 0 W:($G(IO)'="") !!,$$EN^ABSPVDF("RVN"),"ABSP Pharmacy Point Of Sale claim did NOT generate",$$EN^ABSPVDF("RVF"),! Q 0  ;absp*1.0*57 IHS/SD/SDR ADO125991
...
STATUS59(RXI,RXR)  N IEN59,STAT
 ...
 ; The background job
 ;
 ;IHS/SD/lwj 11/25/02  details in top of program - N command added on next line.
