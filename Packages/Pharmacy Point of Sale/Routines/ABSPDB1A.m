ABSPDB1A ; IHS/OIT/CASSevern/Pieran ran 1/19/2011 - Handling of outgoing NCPDP Billing "B1" Claims for D.0 (Claim Segment)
 ;;1.0;PHARMACY POINT OF SALE;**42,46,51,52**;JUN 01, 2001;Build 131
 ;
CLAIM ;EP CALLED FROM ABSPDB1 to set up CLAIM SEGMENT
 N FIELD,ZIO,ZFN,F460,DD354
 S RECORD=$G(RECORD),DD354=0
 ; /IHS/OIT/RAM ; PATCH 51; ADD ABILITY TO OVERRIDE SPECIAL CODES 420.[123] AND ADD 460-ET AS WELL.
 F FIELD="111",455,402,436,407,456,457,458,459,442,403,405,406,408,414,415,419,354.2,420.1,420.2,420.3,354,460,308,429,453,445,446,454,600,418,461,462,463,464,343,344,345,357,391,995,996,147 D
 . Q:$D(SUPRESF(FIELD))
 . I (ACTION["CLAIM"),(FIELD'=111) D
 . . I FIELD=354.2 Q  ; /IHS/OIT/RAM ; P51 ; Segment 354 needs to count the 420.x before being populated, skip this placeholder.
 . . S FIELD=$TR(FIELD,".","0") ; P51 ; The overrides allow for dots in the codes, but M doesn't allow for dots in subroutine labels.
 . . D @(FIELD_"GET")
 . . D @(FIELD_"FMT")
 . . D @(FIELD_"SET")
 . ELSE  D APPEND(FIELD)
...
442GET I '$D(SPECIAL(442)) D
 . S F460=+$$GET1^DIQ(9002313.99,"1,",2.02,"I") ; /IHS/OIT/RAM ; P51 ; THERE'S A FLAG IN ABSP SETUP IF WE'RE GOING TO SEND 460 OR NOT.
 . ; /IHS/OIT/RAM ; P52 ; 20 AUG 20 ; REOPENED CR10863 - SEEMS BOTH 460 AND 442 NEED TO BE SENT FOR CII's. EASIEST WAY FOR THAT TO HAPPEN WOULD BE COMMENT OUT NEXT LINE.
 . ; I (ABSP("RX",ABSP(9002313.0201),"Drug Schedule")=2)&F460 S ABSP("X")="" Q  ; /IHS/OIT/RAM ; PATCH 51 ; If Schedule II, populate 460-ET instead of 442-E7.
 ...
 . ; S ZFN="D:\Z\ABSP\NOT2_"_$H_".TXT" O ZFN:("NW"):10 U ZFN ZW ABSP,FIELD,SUPRESF,SUPRESSG,SPECIAL,ACTION W ! U 0 C ZFN 
 ELSE  X SPECIAL(442)
 Q
442FMT S:ABSP("X")'="" ABSP("X")="E7"_$$NFF^ABSPECFM(ABSP("X"),10) ;Spec says length is 7
 Q
442SET ; S:ABSP("RX",ABSP(9002313.0201),"Drug Schedule")='2 $P(^ABSPC(ABSP(9002313.02),400,ABSP(9002313.0201),440),U,2)=ABSP("X")
 S:ABSP("X")'="" $P(^ABSPC(ABSP(9002313.02),400,ABSP(9002313.0201),440),U,2)=ABSP("X")
 Q
 ;Fill Number
...
419SET S $P(^ABSPC(ABSP(9002313.02),400,ABSP(9002313.0201),400),U,19)=ABSP("X")
 Q
 ; MOVED Submission Clarification Code Count LOWER
 ; 354GET I '$D(SPECIAL(354)) S ABSP("X")=$G(ABSP("RX",ABSP(9002313.0201),"Subm Clar Count"))
 ;  E  X SPECIAL(354)
 ;  Q
...
42001GET I '$D(SPECIAL(420.1)) D   ; WAS FOR TESTING: I '($D(SPECIAL(420.1))!($D(SPECIAL(42001)))) D
 . S ABSP("X")="" I $G(ABSP("RX",ABSP(9002313.0201),"Subm Clar Code 1"))]"" S ABSP("X")=ABSP("RX",ABSP(9002313.0201),"Subm Clar Code 1")
 E  X SPECIAL(420.1) ; S ZFN="D:\Z\ABSP\ZFLD42001_"_$H_".TXT" O ZFN:("NW"):10 U ZFN ZW ABSP,FIELD,SUPRESF,SUPRESSG,SPECIAL,ACTION W ! U 0 C ZFN 
 ; 
 Q
...
42001SET S $P(^ABSPC(ABSP(9002313.02),400,ABSP(9002313.0201),421),U,1)=ABSP("X")
 ; /IHS/OIT/RAM ; P51 ; "BACKUP PLAN" FOR COUNTING # OF 420.X SEGMENTS
 S:ABSP("X")'="" ABSP("COUNT",420)=$G(ABSP("COUNT",420))+1
 ;
 Q
 ;Submission Clarification Code 2
42002GET I '$D(SPECIAL(420.2)) D   ;I '($D(SPECIAL(420.2))!($D(SPECIAL(42002)))) D
 . S ABSP("X")="" I $G(ABSP("RX",ABSP(9002313.0201),"Subm Clar Code 2"))]"" S ABSP("X")=ABSP("RX",ABSP(9002313.0201),"Subm Clar Code 2")
 E  X SPECIAL(420.2) ; S ZFN="D:\Z\ABSP\ZFLD42002_"_$H_".TXT" O ZFN:("NW"):10 U ZFN ZW ABSP,FIELD,SUPRESF,SUPRESSG,SPECIAL,ACTION W ! U 0 C ZFN 
 ;
 Q
...
42003GET I '$D(SPECIAL(420.3)) D   ; I '($D(SPECIAL(420.3))!($D(SPECIAL(42003)))) D
 . S ABSP("X")="" I $G(ABSP("RX",ABSP(9002313.0201),"Subm Clar Code 3"))]"" S ABSP("X")=ABSP("RX",ABSP(9002313.0201),"Subm Clar Code 3")
 E  X SPECIAL(420.3) ; S ZFN="D:\Z\ABSP\ZFLD42003_"_$H_".TXT" O ZFN:("NW"):10 U ZFN ZW ABSP,FIELD,SUPRESF,SUPRESSG,SPECIAL,ACTION W ! U 0 C ZFN 
 ;
 Q
...
42003SET S $P(^ABSPC(ABSP(9002313.02),400,ABSP(9002313.0201),421),U,3)=ABSP("X")
 ...
 ;
 Q
 ;Submission Clarification Code Count - MOVED AFTER 420.X FOR AUTO CALCULATION
354GET I '$D(SPECIAL(354)) S ABSP("X")=$G(ABSP("RX",ABSP(9002313.0201),"Subm Clar Count")) D
 . ; /IHS/OIT/RAM ; P51 ; "BACKUP PLAN" FOR DOCUMENTING # OF 420.X SEGMENTS
 . I ABSP("X")=""&($G(ABSP("COUNT",420))>0) S ABSP("X")=$G(ABSP("COUNT",420))
 . ; S ZFN="D:\Z\ABSP\IS354_"_$H_".TXT" O ZFN:("NW"):10 U ZFN ZW ABSP,FIELD,SUPRESF,SUPRESSG,SPECIAL,ACTION W ! U 0 C ZFN 
 E  X SPECIAL(354)
 Q
354FMT S:ABSP("X")'="" ABSP("X")="NX"_$$NFF^ABSPECFM($G(ABSP("X")),1)
 Q
354SET S $P(^ABSPC(ABSP(9002313.02),400,ABSP(9002313.0201),350),U,4)=ABSP("X")
 Q
 ; Quantity Prescribed
 ; /IHS/OIT/RAM/ ; PATCH 51 ; ADD ABILITY TO POPULATE SEGMENT 460-ET FOR SCHEDULE II DRUGS ONLY.
460GET I '$D(SPECIAL(460)) D
 . S F460=+$$GET1^DIQ(9002313.99,"1,",2.02,"I") ; /IHS/OIT/RAM ; P51 ; THERE'S A FLAG IN ABSP SETUP IF WE'RE GOING TO SEND 460 OR NOT.
 . I '((ABSP("RX",ABSP(9002313.0201),"Drug Schedule")=2)&(F460)) S ABSP("X")="" Q  ; /IHS/OIT/RAM ; PATCH 51 ; ONLY SET THIS IF SCHEDULE II.
 . ; ; DEBUGONLY S ZFN="D:\Z\ABSP\IS2_"_$H_".TXT" O ZFN:("NW"):10 U ZFN ZW ABSP,FIELD,SUPRESF,SUPRESSG,SPECIAL,ACTION W ! U 0 C ZFN
 . S ABSP("X")=$G(ABSP("RX",ABSP(9002313.0201),"Quantity"))*1000
 . S:$D(ABSP("RX",ABSP(9002313.0201),"Metric Decimal Quantity")) ABSP("X")=ABSP("RX",ABSP(9002313.0201),"Metric Decimal Quantity")
 ELSE  X SPECIAL(460)
 Q
460FMT S:ABSP("X")'="" ABSP("X")="ET"_$$NFF^ABSPECFM(ABSP("X"),10) ;
 Q
460SET ; S:ABSP("RX",ABSP(9002313.0201),"Drug Schedule")=2 $P(^ABSPC(ABSP(9002313.02),400,ABSP(9002313.0201),450),U,10)=ABSP("X")
 S:ABSP("X")'="" $P(^ABSPC(ABSP(9002313.02),400,ABSP(9002313.0201),450),U,10)=ABSP("X")
 Q
 ;Other Coverage Code
...
APPEND(FIELD) ;This is where the record is built field by field
 ...
 ELSE  D
 . I FIELD=354 Q  ; /IHS/OIT/RAM ; P51 ; Segment 354 already sent due to placeholder, don't send twice.
 . I FIELD=354.2 S FIELD=354 ; /IHS/OIT/RAM ; P51 ; Segment 354 needs to go first (per SSM) transform placeholder into segment 354.
 . ; W "PREFIELD: ",FIELD,!
 . I $E(FIELD,1,3)=420 S FIELD="420."_$E(FIELD,5)
 . ; W "VALUE? ",$G(ABSP(9002313.0201,IEN(9002313.01),FIELD,"I")),!
 . ; S ZFN="D:\Z\ABSP\FIELD_"_FIELD_"_"_$H_".TXT" O ZFN:("NW"):10 U ZFN ZW ABSP,FIELD,SUPRESF,SUPRESSG,SPECIAL,ACTION W ! U 0 C ZFN
