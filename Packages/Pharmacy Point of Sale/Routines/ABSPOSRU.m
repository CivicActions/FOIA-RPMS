ABSPOSRU ;IHS/OIT/SCR - Utilities for POS Insurance Report ;
 ;;1.0;PHARMACY POINT OF SALE;**38,46,50,52,54**;JUN 01, 2001  ;Build 131
...
BDT(RDATE)  ;ENTER BEGINING DATE
 ; /IHS/OIT/RAM ; P52 ; ADD WHICH SEARCH DATE TYPE TO PROMPT.
 N ABSPBDT,DIR,X1,X,DUOUT,DIROUT,DIRUT,DTOUT
 W !
 K DIR
 S DIR(0)="DEX"
 S DIR("A")="Enter Beginning Prescription "_RDATE_" Date"
 D ^DIR
 Q:$D(DUOUT)!$D(DIROUT)!$D(DIRUT)!$D(DTOUT) -1
 S ABSPBDT=+Y
 ; /IHS/OIT/RAM ; P52 ; ALLOW A HUMAN READABLE FORMAT DATE TO BE PRINTED.
 ; S X1=ABSPBDT D C^%DTC ; OLD ROUTINE
 S X1=ABSPBDT D C^%DTC,YX^%DTC ; ADD ROUTINE TO CONVERT TO HUMAN READABLE
 W *9,Y ; DISPLAY THE DATE
 ; /IHS/OIT/RAM ; P52 ; END OF CHANGES.
 Q X
EDT(RDATE)  ;ENTER END DATE
 ; /IHS/OIT/RAM ; P52 ; ADD WHICH SEARCH DATE TYPE TO PROMPT.
 N ABSPBDT,DIR,X1,X,DUOUT,DIROUT,DIRUT,DTOUT
 W !
 K DIR
 S DIR(0)="DEX"
 S DIR("A")="Enter Ending Prescription "_RDATE_" Date"
 D ^DIR
 Q:$D(DUOUT)!$D(DIROUT)!$D(DIRUT)!$D(DTOUT) -1
 S ABSPEDT=+Y
 ; /IHS/OIT/RAM ; P52 ; ALLOW A HUMAN READABLE FORMAT DATE TO BE PRINTED.
 ; S X1=ABSPEDT D C^%DTC ; OLD ROUTINE
 S X1=ABSPEDT D C^%DTC,YX^%DTC ; ADD ROUTINE TO CONVERT TO HUMAN READABLE
 W *9,Y ; DISPLAY THE DATE
 ; /IHS/OIT/RAM ; P52 ; END OF CHANGES.
 Q X
...
DEVSEL() ; SELECT DEVICE
 N ABSPSTOP,%ZIS,POP
 ;S POP=0,ABSPSTOP=0,%ZIS="M"
 S POP=0,ABSPSTOP=0,%ZIS="MQ"  ;IHS/GDIT/AEF 3231009 patch 54 FID 77369 
 D ^%ZIS
 I POP=1 Q "^"
 ; W "|"_$G(POP)_"|",*9,"|"_$G(IO)_"|",*9,"|"_$G(ION)_"|",*9,"|"_$G(IOST)_"|",*9,"|"_$G(X)_"|",*9,"|"_$G(Y)_"|",!
 Q:$G(IO)="" 0
 Q IO
 ;
