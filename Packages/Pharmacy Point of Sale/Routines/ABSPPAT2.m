ABSPPAT2 ; IHS/SD/SDR - PATIENT ELIGIBILITY EXTRINSICS ; [ 02/14/2005  11:00 AM ]
...
MCR(P,D) ;EP - Is patient P medicare eligible on date D.  1 = yes, 0 = no.
 ...
 I '$G(P) Q 0
 I '$G(D) Q 0
 I $P($G(^AUTNINS(P,2)),U,3)'="P" Q 0  ;BILL POINT OF SALE ONLY
 NEW I,Y
 S Y=0,U="^"
 I '$D(^DPT(P,0)) G MCRX
...
MCD(P,D) ;EP - Is patient P medicaid eligible on date D.
 ...
 .;
 .S ABSPRXB=""
 .I $P($G(^AUPNMCD(I,0)),U,10)'="" D
 ..S ABSPRXB=$P($G(^AUTNINS($P($G(^AUPNMCD(I,0)),U,10),2)),U,3)
 .I ABSPRXB'="P" D
 ..S ABSPINS=$P($G(^AUPNMCD(I,0)),U,2)
 ..Q:'$P($G(^AUPNMCD(I,0)),U,4)
 ..S ABSP("STATE")=$P($G(^AUPNMCD(I,0)),U,4)
 ..I $D(^AUTNINS(ABSPINS,13,ABSP("STATE"),0)) D
 ...S ABSPINS=$P($G(^AUTNINS(ABSPINS,13,ABSP("STATE"),0)),U,2)
 ...S ABSPRXB=$P($G(^AUTNINS(ABSPINS,2)),U,3)
 .I ABSPRXB'="P" Q  ;BILL POINT OF SALE ONLY
 .;
 .Q:'$D(^AUPNMCD(I,11))
...
PI(P,D) ;EP - Is patient P private insurance eligible on date D. 1= yes, 0=no.
 ...
 .I $P($G(^AUTNINS(X,2)),U,3)'="P" Q  ;BILL POINT OF SALE ONLY
...
PIN(P,D,F) ;EP - return private insurer name for patient P on date D in form F
 ...
 .I $P($G(^AUTNINS(Y,2)),U,3)'="P" Q  ;BILL POINT OF SALE ONLY
