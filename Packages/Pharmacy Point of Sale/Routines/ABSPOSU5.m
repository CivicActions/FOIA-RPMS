ABSPOSU5 ; IHS/FCS/DRS - utilities ;     [ 09/12/2002  10:19 AM ]
...
PRESSANY(NLF,TIMEOUT) ;EP
 ...
 F I=1:1:NLF W !
 ;W "Press any key to continue:  " R *X:TIMEOUT
 I $$FREETEXT^ABSPOSU2("Press ENTER to continue: ",,1,1,1,300)
 Q
 ;----------------------------------------------------------------------
 ;'Press the return key to continue of ^ to exit:' PROMPT
 ; See also $$EOPQ^ABSPOSU8
ENDPAGE(NLF,TIMEOUT) ;EP
 ...
 ;----------------------------------------------------------------------
ENDRPT() ; EP
 I $Q Q $$ENDRPT^ABSPOSU8
 D ENDRPT^ABSPOSU8() Q
 ;=====================================================================
BYE ; EP
 ; Most routines should come here when they exit.
 ; Example: GOTO BYE^ABSPOSU3
