ABSPDE1 ; IHS/OIT/CASSevern/Pieran ran 2/23/2011 - Handling of outgoing NCPDP Eligibility "E1" transactions for D.0
 ;;1.0;PHARMACY POINT OF SALE;**42,48**;JUN 21, 2001;Build 131
 ;
 ;IHS/OIT/CASSevern/Pieran ran - Started with copy of ABSPOSE2 and modified for D.0
 Q
MAIN ;EP
 N POP
 S POP=0
...
HEADER ; Header Seg
 ...
 ;
 ;101 BIN (Emdeon plan # hard coded to 610144) + 102 Version (always D0) +
 ;103 Trans Code (always E1)
 S TDATA="610144D0E1"
 ;104 Processor control number ("CARDFINDER" for Commercial AND Part D)
 S TDATA=TDATA_"CARDFINDER"
 ;109 Transaction Count (1 for the E1)+202 Service Prov ID Qual (always 01)
 S TDATA=TDATA_101
 ;
 ;201 Service Provider ID
 S TDATA=TDATA_$$ANFF^ABSPECFM($$GET1^DIQ(9002313.5601,"1,1,",.02),15)    ;NPI #
 ;401 Date of Service
 ;Date can be -90 to +90 of current date.
 S TDATA=TDATA_$$DTF1^ABSPECFM(E1DATE)
 ;
 ;110 Software Vendor/Certification ID
 S TDATA=TDATA_$$ANFF^ABSPECFM(" ",10)  ;Will be assigned After we pass certification??
...
PATIENT ; Patient Seg
 ...
 S TDATA=TDATA_"CP"_ABSP325_FS
 S DR=DR_"325////"_ABSP325_";"
 ;
 ;307 Place of Service
 S ABSP307=1 ;Pharmacy
 S TDATA=TDATA_"C7"_ABSP307_SS_FS
 ;
 Q
INSURER ; Insurance Seg
 ...
 ;
 ;302 Cardholder ID - last 4 of SSN
...
RAWTRANS ; Raw trans in ^ABSPE
 ...
 I $D(ZERR) D LOG^ABSPOSL2("RAWTRANS^ABSPDE1",.ZERR) ; /IHS/OIT/RAM ; 12 JUN 17 ; AND LOG IT IF AN ERROR OCCURS.
 ;
 Q
