ABSPP49 ; /IHS/OIT/RAM - PRE & POST INSTALL ROUTINES FOR ABSP PATCH 49 ; 13 OCT 2017 ;
 ;;1.0;PHARMACY POINT OF SALE;**49**;13 OCT 2017;Build 131
 ; =-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
 ...
 */
 ...
 I +LASTPATCH>52 {
  D MES^XPDUTL("Current patch revision of ABSP 1.0 package: "_+LASTPATCH_" -- is newer than Patch 52.")
 ...
 ;
ENVBYE   ; CHANGE SPACE AT END OF LINE.
...
ENVABORT  ; "SOMETHING BROKE. ABORT INSTALL AND UNLOAD DISTRIBUTION."
 ...
 R PLOP#1:120
 S XPDABORT=1,XPDQUIT=1,XPDQUIT("ABSP*1.0*51")=1
 ;;1.0;PHARMACY POINT OF SALE;**49**;13 OCT 2017
 ;
...
POSTBYE ;
 Q
 ;
GETPATCH(PKG,ALLPATCH) ; GET ALL ABSP PATCH DATA FROM THE INSTALL FILE.
 ;
 ...
 Q
 ;
GETPERS(IEN) ; TAKE THE 'INSTALLED BY' IEN AND TURN IT INTO A NAME.
 ;
 N NAME
 N U S U="^"
...
PREREQ(PKG,VER,PAT) ;
 ...
 ;
NOTOUCH ;; LEFTOVER CODE I COULDN'T BRING MYSELF TO DELETE... MIGHT STILL NEED THE LOGIC SOMEDAY. 
 ;
 ; MORE NOTOUCHIE... =-=-=-=
...
SUP(YUP) ; UTILITY TO GRAB JUST THE FIRST CHARACTER AND UPPERCASE IT.
 Q $TR($E($G(YUP)),"abcdefghijklmnopqrstuvwxyz","ABCDEFGHIJKLMNOPQRSTUVWXYZ")
 ;
