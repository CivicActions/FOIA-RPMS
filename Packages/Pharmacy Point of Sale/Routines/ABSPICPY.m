ABSPICPY ; IHS/OIT/CASSevern/Pieran ran 9/19/2011 - Copy Insurance default Profiles
 ;;1.0;PHARMACY POINT OF SALE;**42,46,48,49,54,56**;JUN 01, 2001;Build 131
 ;
 ...
 ;IHS/OIT/RAM 4.18.24 FOUND THAT DEFAULT INSURER CONNECTIVITY ISN'T BEING USED FROM THE ABSP SETUP FILE, BUT HARDCODED HERE
 ;IHS/SD/SDR 1.0*56 ADO113735 Stop SUPPRESSS NCPDP FIELD from auto-populating for 354, 420
EN(ABSPINS) ;EP
 N OK,TYP
 S OK=0
...
PROCESS(ABSPINS,TYP) ;Process the INSURER here
 N INS,ABSPSPEC,ABSPSPSG,ABSPSPFL,ZERR  ;IHS/OIT/RAM 6.9.17 ADD DBS CALL ERROR RETURN VARIABLE
 ...
 D:$D(INS(1)) UPDATE^DIE("E","INS(1)",,"ZERR")  ;IHS/OIT/RAM 6.9.17 UPDATE DBS CALL TO ALLOW FOR ERROR RETURN
 I $D(ZERR) D LOG^ABSPOSL2("PROCESS^ABSPICPY",.ZERR)  ;IHS/OIT/RAM 6.9.17 AND LOG IT IF AN ERROR OCCURS
...
DEFTAB ;;TYPE;NCPDP FIELD #;FIELD NAME;VALUE
 ...
 ;;TOP;100.07;DIAL OUT TO;HTTP POST
 ;;TOP;100.14;Insurer NPI Flag;BOTH
 ;;TOP;100.06;RX-Pricing Method;STANDARD
 ;;***
 Q
 ;start old absp*1.0*56 IHS/SD/SDR ADO113735
 ;;FLD;354;SUPPRESS FIELD 354;  moved here from top line in above tag
 ;;FLD;420;SUPPRESS FIELD 420;  moved here; was between 996 and 458
 ;end old absp*1.0*56 IHS/SD/SDR ADO113735
...
POPSPEC(ABSPINS,ABSPSPEC) ;Now populate the Special Code stuff
 ...
 N NCPDPCD,INS,STRING,ZERR  ;IHS/OIT/RAM 6.9.17 ADD DBS CALL ERROR RETURN VARIABLE
 S NCPDPCD=""
 ...
 .D UPDATE^DIE("E","INS(1)",,"ZERR")  ;IHS/OIT/RAM 6.9.17 UPDATE DBS CALL TO ALLOW FOR ERROR RETURN
 .I $D(ZERR) D LOG^ABSPOSL2("POPSPEC^ABSPICPY",.ZERR)  ;IHS/OIT/RAM 6.9.17 AND LOG IT IF AN ERROR OCCURS
 Q
POPSEG(ABSPINS,ABSPSPSG) ;Next we populate the suppressed segments
 ;This has already been run...don't add duplicate entries
 Q:$D(^ABSPEI(ABSPINS,221))
 N SEGCD,INS,ZERR  ;IHS/OIT/RAM 6.9.17 ADD DBS CALL ERROR RETURN VARIABLE
 S SEGCD=""
 ...
 .D UPDATE^DIE("","INS(1)",,"ZERR")  ;IHS/OIT/RAM 6.9.17 UPDATE DBS CALL TO ALLOW FOR ERROR RETURN
 .I $D(ZERR) D LOG^ABSPOSL2("POPSEG^ABSPICPY",.ZERR)  ;IHS/OIT/RAM 6.9.17 AND LOG IT IF AN ERROR OCCURS
 Q
POPFLD(ABSPINS,ABSPSPFL) ;Next we populate the suppressed fields
 ;This has already been run...don't add duplicate entries
 Q:$D(^ABSPEI(ABSPINS,220))
 N NCPDPCD,ZERR  ;IHS/OIT/RAM 6.9.17 ADD DBS CALL ERROR RETURN VARIABLE
 S NCPDPCD=""
 F  S NCPDPCD=$O(ABSPSPFL(NCPDPCD)) Q:NCPDPCD=""  D
 .S INS(1,9002313.46,"+1,"_ABSPINS_",",.01)=NCPDPCD
 .;D UPDATE^DIE("E","INS(1)")
 .D UPDATE^DIE("E","INS(1)",,"ZERR") ;IHS/OIT/RAM 6.9. UPDATE DBS CALL TO ALLOW FOR ERROR RETURN
 .I $D(ZERR) D LOG^ABSPOSL2("POPFLD^ABSPICPY",.ZERR) ;IHS/OIT/RAM 6.9.17 LOG IT IF AN ERROR OCCURS
 Q
