...
IO(DIALOUT) ;EP -
 ...
 ;
 ; /IHS/OIT/RAM 20 MAR 2024 - ADDED NEW FORM OF CONNECTIVITY, BUT DID _NOT_
 ;          ADD IT TO THE $$TCP() FUNCTION - DUE TO LIMITATIONS IN THE
 ;          ABSP DIAL OUT FILE, THE DOMAIN FOR THE HTTP POST FUNCTION
 ;          IS TOO LONG.
 N ABSPOFLD
 ;
 ...
 ;
 ; /IHS/OIT/RAM ; 20 MAR 2024 ; ADDED FOURTH CONNECTIVITY TYPE TO CHECK FOR.
TCP(DIALOUT) ;EP - 
 N X S X=$$GET55FLD(DIALOUT,420.02) Q X=2!(X=3) ; !(X=4) ; /IHS/OIT/RAM ; SEE COMMENT ABOVE.
TCPSERV(DIALOUT)   Q $$GET55FLD(DIALOUT,420.02)=2
T1DIRECT(DIALOUT) ;EP -
 Q $$GET55FLD(DIALOUT,420.02)=3
 ; /IHS/OIT/RAM ; 20 MAR 2024 ; ADDED NEW FORM OF CONNECTIVITY TEST.
HTTPPOST(DIALOUT) ;EP -
 Q $$GET55FLD(DIALOUT,420.02)=4
 ;
MODEMTYP(DIALOUT) ;EP - 
