ABSPOS0 ;IHS/GDIT/AEF - continuation of ABSPOS [ 09/16/2000  5:17 PM ]
 ;;1.0;PHARMACY POINT OF SALE;**11,54**;JUN 01, 2001;Build 131
 ;ZT
DEFTIME() I DUZ=9598 Q .0005 ; my default time
 Q .0015 ; fifteen minutes is the default default
UPDFREQ() I DUZ=9598 Q 5   ; I get it every 5 seconds
 Q 15 ; every fifteen seconds is the usual case
NEW D FULL^VALM1 W:$D(IOF) @IOF
 D ^ABSPOSI
 ...
 S VALMBCK="R"
 Q
MYPARAMS S ^TMP("ABSPOS",$J,"USER")=$S($D(USER):USER,$G(DUZ):DUZ,1:0)
 ...
 Q
HDR ; -- header code
 N USER,ONEPAT,% S USER=^TMP("ABSPOS",$J,"USER"),ONEPAT=^("PATIENT")
 I USER S %="Transmitted by "_$P($G(^VA(200,USER,0)),U)
 E  D
 . S %="All prescriptions"
 . ;I ONEPAT S %=%_" for patient "_$P(^DPT(ONEPAT,0),U)
 . I ONEPAT S %=%_" for patient "_$P(^DPT(ONEPAT,0),U)_$$PPN1^ABSPUTL(ONEPAT)  ;IHS/GDIT/AEF 3240110 - ABSP*1.0*54 FID 77888
 S VALMHDR(1)=%
 ...
 Q
ONEPAT() ; overflow from ABSPOS - extra date & time info printed in onepat mode
 ; POS time and FILL time
 ;start with kludgey var name machinations (sigh)
 N IEN59 S IEN59=RXI N RXI S RXI=$P(^ABSPEC(9002335.59,IEN59,1),U,11)
 N POS S POS=$P(^ABSPEC(9002335.59,IEN59,0),U,8)
 N RXR S RXR=$P(^ABSPEC(9002335.59,IEN59,1),U)
