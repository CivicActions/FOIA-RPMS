ABSPOSH4 ; IHS/FCS/DRS - Parse Claim 5.1,D.0 Response ;    [ 09/04/2002  12:56 PM ]
 ;;1.0;PHARMACY POINT OF SALE;**3,14,36,42,46,57**;JUN 01, 2001;Build 131
 ;----------------------------------------------------------------------
 ...
 ;NCPDP 5.1 response segments are completely different than the 3.2 response.  Of significant importance are:
 ;  In 3.2, there were 4 basic repsonse segments (header required, header option, information required, information optional.) 
 ;  In 5.1, there are 8 possible segments (header, message, insurance, status, claim, pricing, DUR/PPS, and prior authorization)
 ;
 ;  In 5.1, for all segments following the header, a segment separator is used.
 ;
 ;  In 5.1, field separators, and field identifiers are used for all fields not appearing on the header segment.
 ;
 ;This routine will be solely responsible for parsing the data for 5.1 claims. It is called by ABSPECA4.
 ;----------------------------------------------------------------------
 ;IHS/SD/lwj 10/27/05 patch 14 new subroutine added to parse out the E1 response
 ;IHS/SD/SDR 1.0*57 ADO105714 Fix for programming error <UNDEF>PARSETN+56^ABSPOSH4
 ;
 ;----------------------------------------------------------------------
...
PARSETM ;This subroutine will parse the variable portions of the transmission 
 ;level message. Keep in mind that most fields are optional so we have no idea what is coming back. We will parse based 
 ;on the field separators, and field identification. (tranmission level variable records are the message (ID=20) 
...
PARSETN ; This subroutine will parse the transaction level segments. For 
 ...
 ;record based on field separators and the value of the field identification.  
 ...
 ..Q:(FLDNUM=339)  ;skip this segment for now; causing prog error ;absp*1.0*57 IHS/SD/SDR ADO105714
