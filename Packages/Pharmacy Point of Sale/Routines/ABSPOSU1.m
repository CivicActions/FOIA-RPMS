...
DATE(PROMPT,DFLT,OPT,SDATE,EDATE,%DT,TIMEOUT) ;EP -
 ...
 ;
 ;Parmeters:    Y       - FileMan formatted date (YYYMMDD.HHMMSS)
...
NOWEXT() ;EP - External form of $$NOWFM
 ...
 ;              X2   - Number of days (pos = add, neg = subtract)
...
DTR1 ; -- Get beginning date
 ...
 S BEGDT=$$DATE^ABSPOSU1(PROMPT,$G(DEFAULT),1,1000101,3991231,%DT,$G(DTIME))
 I BEGDT<1 QUIT 0
 ;
 WRITE !
 S %DT="AE"_$G(T)
 ...
 S ENDDT=$$DATE^ABSPOSU1(PROMPT,$G(DEFAULT),1,BEGDT,3991231,%DT,$G(DTIME))
 I ENDDT["^" Q 0  ;user wants out if "^"
 ; -- Ensure END date is not earlier than BEG date
 I ENDDT<BEGDT WRITE *7,!!,"Ending date must not be less than beginning date!",!! HANG 2 GOTO DTR1
 QUIT BEGDT_U_ENDDT
 ;--------------------------------------------------------------------
 ;
 ;
DTP(AA,DEF) ;EP - *** GET A SINGLE PAST DATE, TIME NOT ALLOWED ***
 ...
 S U="^"
 ;
 ;S %DT(0)=-DT ;date may not be after TODAY
