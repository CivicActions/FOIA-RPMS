XUSHSHP ;SF/STAFF - HASHING ROUTINE FOR SIG BLOCK IN FILE 200 ;3/30/93  08:47 ; [ 04/02/2003  8:33 AM ]
 ;;8.0;KERNEL;**1002,1003,1004,1005,1007**;APR 1, 2003
 ;;8.0;KERNEL;**60**;Jul 10, 1995
HASH Q:'$D(X)  F I=1:1:$L(X) I $E(X,I)?1L S X=$E(X,1,I-1)_$C($A(X,I)-32)_$E(X,I+1,99)
 G EN1
EN ;
 Q:'$D(X)!('$D(X1))!('$D(X2))  G EN2
DE ;
 Q:'$D(X)!('$D(X1))!('$D(X2))  G EN3
 ;
EN1 Q:X=""  N PA,PI,PJ,PL,PR,PX,PY,PY1,PZ S PX=$E(X,1,20),PY=PX,X="",PL=$L(PX) F PI=1:1:PL S PY1=$E(PY),PZ=$A(PY1)#20+1,PY=$E(PY,2,20),PA(PI)=$S(PY[PY1:$P($T(Z+(PZ+PI#20+1)),";",3,9),1:$P($T(Z+PZ),";",3,9))
 F PZ=1:1 Q:$L(X)>19  D C S X=$S(PZ#2:PX_X,1:X_PX)
 S X=$E(X,1,20) G Q
EN2 Q:X=""  S PX=X,X="",PL=$L(PX) D CL S PR=X1*X2#94,PY="" F PI=1:1:PL S PA(PI)=$E(PA(PI),PR,999)_$E(PA(PI),1,PR-1),PY1=$E(PA(PI),$A(PX,PI)#94+1),PY=PY_PY1
 S X=PY G Q
EN3 Q:X=""  S PX=X,X="",PL=$L(PX) D CL S PR=X1*X2#94,PY="" F PI=1:1:PL S PA(PI)=$E(PA(PI),PR,999)_$E(PA(PI),1,PR-1),PY1=$F(PA(PI),$E(PX,PI))-2,PY1=$S(PY1<32:PY1+94,1:PY1),PY=PY_$C(PY1)
 S X=PY G Q
