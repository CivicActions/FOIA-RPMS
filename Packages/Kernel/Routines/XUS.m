...
CHECKAV(X1,XUAPP) ;Check A/V code return DUZ or Zero. (Called from XUSRB)
 ...
 Q:X'?1.20ANP 0
 ;S X=$$EN^ROUTINE(X) I '$D(^VA(200,"A",X)) D LBAV Q 0  ;IHS/OIT/FBD - XU*8.0*1018 - ORIGINAL LINE - COMMENTED OUT
 S X=$$EN^XUSHSH(X) I '$D(^VA(200,"A",X)) D LBAV Q 0  ;IHS/OIT/FBD - XU*8.0*1018 - REPLACED VA-DISTRIBUTED $$EN^ROUTINE CALL WITH $$EN^XUSHSH
 S %1="",IEN=$O(^VA(200,"A",X,0)),XUF(.3)=IEN D USER(IEN)
 ;S X=$P(X1,";",2) S:XUF %1="Verify: "_X S X=$$EN^ROUTINE(X)  ;IHS/OIT/FBD - XU*8.0*1018 - ORIGINAL LINE - COMMENTED OUT
...
UVALID() ;EF. Is it valid for this user to sign on?
 ...
 ;I '$L($P(XUSER(1),U,2)) Q 21 ;p419, p434  ;XU*8.0*1018 - IHS/OIT/FBD - 3/24/2011 - COMMENTED LINE TO RE-ENABLE NULL VERIFY CODE TOLERANCE FOR IHS
 Q 0
 ;
