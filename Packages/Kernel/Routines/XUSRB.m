XUSRB ;ISCSF/RWF - Request Broker ;02/03/10  16:07
 ;;8.0;KERNEL;**11,16,28,32,59,70,82,109,115,165,150,180,213,234,238,265,337,395,404,437,523,1020**;Jul 10, 1995;Build 8
 ;Per VHA Directive 2004-038, this routine should not be modified
 ;XU*8.0*1020 - IHS/OIT/FBD - added calling application specification to CHECKAV^XUS calls, ID of BMXNet and XWB RPC Broker calls
 Q  ;No entry from top
 ;
...
VALIDAV(RET,AVCODE) ;Check a users access
 ...
 N XUAPP  ;XU*8.0*1020
 S XUAPP="XUSRB - VALIDAV"  ;XU*8.0*1020 - DEVAULT VALUE, MAY BE OVERWRITTEN
 ...
 . I $$BROKER^XWBLIB S XUAPP="XWB login"  ;XU*8.0*1020 - IDENTIFY IF CALL IS PART OF RPC BROKER PROCESSING
 . I +$G(BMXPORT) S XUAPP=$$BMXCHK(BMXPORT)  ;XU*8.0*1020 - IDENTIFY IF CALL IS PART OF BMXNet BROKER PROCESSING
 . ;S DUZ=$$CHECKAV^XUS($$DECRYP^XUSRB1(AVCODE))  ;XU*8.0*1020 - COMMENTED OUT
 . S DUZ=$$CHECKAV^XUS($$DECRYP^XUSRB1(AVCODE),XUAPP) ;XU*8.0*1020 - ADDED CALLING APP SPECIFICATION
 . Q
...
VAX S:XUMSG>0 DUZ=0 ;Can't sign-on, Clear DUZ.
 ...
 Q
 ;
BMXCHK(PORT) ;XU*8.0*1020 - CHECK IF CURRENT PROCESS FLOW RESULTS FROM A BMXNET CALL
 N STATUS,IEN,DESC
 S STATUS="BMX login"
 I $D(^BMXMON("B",PORT)) D  ;
 . S IEN=$O(^BMXMON("B",PORT,""))
 . I +IEN D  ;
 . . S DESC=$P($G(^BMXMON(IEN,2)),U,1)
 . . I DESC]"" S STATUS=STATUS_" from "_DESC
 Q STATUS
 ;
...
OPTACCES(RET,USER,OPTIONS,MODE) ;Checks or sets user's access for passed in options
 ...
 Q
 ;
CHECKAV(AVC,XUAPP) ;SR. EF. to check an A/V code, Separate w/ ";", return IEN or 0
 N XUF,XUSER S XUF=0,U="^"
 S:XUAPP="" XUAPP="XUSRB - CHECKAV"  ;XU*8.0*1020 - IF NOT ALREADY IDENTIFIED, SPECIFY CALLING APP
 ;Q $$CHECKAV^XUS(AVC) ;XU*8.0*1020 - COMMENTED OUT
 Q $$CHECKAV^XUS(AVC,XUAPP) ;XU*8.0*1020 - ADDED CALLING APP SPECIFICATION
