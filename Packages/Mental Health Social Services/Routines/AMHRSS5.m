AMHRSS5 ; IHS/CMI/LAB - list refusals ;
...
INFORM ;
 W !,$$CTR($$USR)
 W !,$$LOC()
 W !!,"*Please Note: This SUICIDE SCREENING report is intended for advanced"
 W !,"RPMS users who are experienced in building search templates and using Q-MAN."
 W !!,$$CTR("TALLY AND LISTING OF ALL VISITS W/SUICIDE SCREENING",80)
 W !,$$CTR("ONLY PATIENTS WHO ARE MEMBERS OF A USER DEFINED SEARCH TEMPLATE",80)
 W !,$$CTR("ARE INCLUDED IN THIS REPORT",80)
 W !!,"This report will tally and optionally list all visits on which a"
 W !,"Suicide Screening or a refusal was documented in the"
 W !,"time frame specified by the user.  Suicide Screening is defined as:"
 W !?10,"- Suicide Screening (exam code 44)"
 W !?10,"- refusal of exam code 44"
 W !,"This report will tally the visits by age, gender, screening result, provider "
 W !,"(either exam provider, if available, or primary provider on the visit), "
 W !,"clinic, date of screening, designated PCP, MH Provider, SS Provider"
 W !,"and A/SA provider."
 W !,"  Notes:  "
 ...
 W !?10,"- this is a tally visits with a screening done, if a patient had"
...
EXCL ;
 S AMHREXPC=""
 W !!,"Would you like to include SUICIDE SCREENING Screenings documented in the PCC clinical"
...
ZIS ;
 ...
 S XBRP="PRINT^AMHRSS5P",XBRC="PROC^AMHRSS5",XBRX="XIT^AMHRSS5",XBNS="AMH"
 D ^XBDBQUE
 D XIT
 Q
BROWSE ;
 S XBRP="VIEWR^XBLM(""^AMHRSS5P"")"
 S XBNS="AMH",XBRC="PROC^AMHRSS5",XBRX="XIT^AMHRSS5",XBIOP=0 D ^XBDBQUE
 Q
XIT ;
 D EN^XBVK("AMHR")
 D ^XBFMK
...
PROC ;
 S AMHRCNT=0
 S AMHRH=$H,AMHRJ=$J
 K ^XTMP("AMHRSS5",AMHRJ,AMHRH)
 D XTMP^AMHUTIL("AMHRSS5","SUICIDE SCREENING REPORT")
 ...
 ..S ^XTMP("AMHRSS5",AMHRJ,AMHRH,"PTS",DFN)=""
 ..I $D(^XTMP("AMHRSS5",AMHRJ,AMHRH,"PTS",DFN,AMHRDATE)) Q
 ..S AMHRCNT=AMHRCNT+1
 ..S ^XTMP("AMHRSS5",AMHRJ,AMHRH,"PTS",DFN,AMHRDATE)=""
 ..S AMHRRES=$$VAL^XBDIQ1(9002011,AMHRBIEN,1409) S:AMHRRES["REFUSED" AMHRRES="REFUSED SCREENING"
 ..S ^XTMP("AMHRSS5",AMHRJ,AMHRH,"VSTS",AMHRCNT)="BH"_U_$$BHPPNAME(AMHRBIEN)_U_AMHRRES_U_$$VAL^XBDIQ1(9002011,AMHRBIEN,2001)_U_$$AGE^AUPNPAT(DFN,AMHRDATE)_U_$$VAL^XBDIQ1(2,DFN,.02)_U_AMHRDATE_U_AMHRBIEN_U_DFN
 ..S $P(^XTMP("AMHRSS5",AMHRJ,AMHRH,"VSTS",AMHRCNT),U,10)=$$VAL^XBDIQ1(9002011,AMHRBIEN,.25)
 ..S $P(^XTMP("AMHRSS5",AMHRJ,AMHRH,"VSTS",AMHRCNT),U,15)=AMHRBIEN
 ..S $P(^XTMP("AMHRSS5",AMHRJ,AMHRH,"VSTS",AMHRCNT),U,16)=$$VAL^XBDIQ1(9002011,AMHRBIEN,1410)
 ..S $P(^XTMP("AMHRSS5",AMHRJ,AMHRH,"VSTS",AMHRCNT),U,17)=$$VAL^XBDIQ1(9000001,DFN,.14)
 ..S $P(^XTMP("AMHRSS5",AMHRJ,AMHRH,"VSTS",AMHRCNT),U,25)=$$RACE^AMHUTIL2(DFN)
 ..S $P(^XTMP("AMHRSS5",AMHRJ,AMHRH,"VSTS",AMHRCNT),U,26)=$$ETHN^AMHUTIL2(DFN)
 ...
 .S ^XTMP("AMHRSS5",AMHRJ,AMHRH,"VSTS",AMHRCNT)="EX"_U_$$PPV(AMHRVIEN)_U_AMHRRES_U_$$VAL^XBDIQ1(9000010.13,AMHREIEN,81101)_U_$$AGE^AUPNPAT(DFN,AMHRDATE)_U_$$VAL^XBDIQ1(2,DFN,.02)_U_AMHRDATE_U_AMHREIEN_U_DFN
 .S $P(^XTMP("AMHRSS5",AMHRJ,AMHRH,"VSTS",AMHRCNT),U,10)=$$VAL^XBDIQ1(9000010,AMHRVIEN,.08)
 .S $P(^XTMP("AMHRSS5",AMHRJ,AMHRH,"VSTS",AMHRCNT),U,15)=AMHRVIEN
 .S $P(^XTMP("AMHRSS5",AMHRJ,AMHRH,"VSTS",AMHRCNT),U,16)=$$SPRV(AMHREIEN)
 .S $P(^XTMP("AMHRSS5",AMHRJ,AMHRH,"VSTS",AMHRCNT),U,17)=$$VAL^XBDIQ1(9000001,DFN,.14)
 .S $P(^XTMP("AMHRSS5",AMHRJ,AMHRH,"VSTS",AMHRCNT),U,25)=$$RACE^AMHUTIL2(DFN)
 .S $P(^XTMP("AMHRSS5",AMHRJ,AMHRH,"VSTS",AMHRCNT),U,26)=$$ETHN^AMHUTIL2(DFN)
 .S ^XTMP("AMHRSS5",AMHRJ,AMHRH,"PTS",DFN,AMHRDATE)=AMHRCNT
 ...
 .S AMHRCNT=AMHRCNT+1
 .S ^XTMP("AMHRSS5",AMHRJ,AMHRH,"VSTS",AMHRCNT)="REF"_U_"UNKNOWN"_U_AMHRRES_U_$$VAL^XBDIQ1(9000022,AMHRRIEN,1101)_U_$$AGE^AUPNPAT(DFN,AMHRDATE)_U_$$VAL^XBDIQ1(2,DFN,.02)_U_AMHRDATE_U_AMHRRIEN_U_DFN_U
 .S $P(^XTMP("AMHRSS5",AMHRJ,AMHRH,"VSTS",AMHRCNT),U,16)=$$PRVREF(AMHRRIEN)
 .S $P(^XTMP("AMHRSS5",AMHRJ,AMHRH,"VSTS",AMHRCNT),U,17)=$$VAL^XBDIQ1(9000001,DFN,.14)
 .S $P(^XTMP("AMHRSS5",AMHRJ,AMHRH,"VSTS",AMHRCNT),U,25)=$$RACE^AMHUTIL2(DFN)
 .S $P(^XTMP("AMHRSS5",AMHRJ,AMHRH,"VSTS",AMHRCNT),U,26)=$$ETHN^AMHUTIL2(DFN)
 .S ^XTMP("AMHRSS5",AMHRJ,AMHRH,"PTS",DFN,AMHRDATE)=AMHRCNT
