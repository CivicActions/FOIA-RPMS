AMHRTAPP ; IHS/CMI/LAB - list refusals ;
...
PRINT1 ;
 ...
 D HEADER
 W !,"Total Number of Patients screened:",?62,$J($$COM(AMHPTOT,0),7)
 W !,"Total Number of Patients with at least one positive screening:",?62,$J($$COM(AMHTOPO,0),7),?72,$J($$PER(AMHTOPO,AMHPTOT),7),!
TALLIES ;
 D GENDER
 Q:$$END
 D AGE
...
TOB ;
 W !!?5,"Tobacco",!
 S AMHRX="" F  S AMHRX=$O(AMHTOB(AMHRX)) Q:AMHRX=""!($D(AMHRQUIT))  D
 .Q:$$END
 .W !?20,AMHRX,?62,$J($$COM($G(AMHTOB(AMHRX)),0),7),?72,$J($$PER(AMHTOB(AMHRX),AMHPTOT),7)
 .Q
...
ALC ;
 W !!?5,"Alcohol",!
 S AMHRX="" F  S AMHRX=$O(AMHALC(AMHRX)) Q:AMHRX=""!($D(AMHRQUIT))  D
 .Q:$$END
 .W !?20,AMHRX,?62,$J($$COM($G(AMHALC(AMHRX)),0),7),?72,$J($$PER(AMHALC(AMHRX),AMHPTOT),7)
 .Q
 W !!?20,"Positive Alcohol Tally",?62,$J($$COM(AMHTOPO(2),0),7),?72,$J($$PER(AMHTOPO(2),AMHPTOT),7)
 Q
 ;
SED ;
 W !!?5,"Sedative",!
 S AMHRX="" F  S AMHRX=$O(AMHSED(AMHRX)) Q:AMHRX=""!($D(AMHRQUIT))  D
 .Q:$$END
 .W !?20,AMHRX,?62,$J($$COM($G(AMHSED(AMHRX)),0),7),?72,$J($$PER(AMHSED(AMHRX),AMHPTOT),7)
 .Q
 W !!?20,"Positive Sedative Tally",?62,$J($$COM(AMHTOPO(3),0),7),?72,$J($$PER(AMHTOPO(3),AMHPTOT),7)
 Q
 ;
CANNA ;
 W !!?5,"Cannabis",!
 S AMHRX="" F  S AMHRX=$O(AMHCANNA(AMHRX)) Q:AMHRX=""!($D(AMHRQUIT))  D
 .Q:$$END
 .W !?20,AMHRX,?62,$J($$COM($G(AMHCANNA(AMHRX)),0),7),?72,$J($$PER(AMHCANNA(AMHRX),AMHPTOT),7)
 .Q
 W !!?20,"Positive Cannabis Tally",?62,$J($$COM(AMHTOPO(4),0),7),?72,$J($$PER(AMHTOPO(4),AMHPTOT),7)
 Q
 ;
STIM ;
 W !!?5,"Stimulant",!
 S AMHRX="" F  S AMHRX=$O(AMHSTIM(AMHRX)) Q:AMHRX=""!($D(AMHRQUIT))  D
 .Q:$$END
 .W !?20,AMHRX,?62,$J($$COM($G(AMHSTIM(AMHRX)),0),7),?72,$J($$PER(AMHSTIM(AMHRX),AMHPTOT),7)
 .Q
 W !!?20,"Positive Stimulant Tally",?62,$J($$COM(AMHTOPO(5),0),7),?72,$J($$PER(AMHTOPO(5),AMHPTOT),7)
 Q
 ;
OPI ;
 W !!?5,"Opioid",!
 S AMHRX="" F  S AMHRX=$O(AMHOPI(AMHRX)) Q:AMHRX=""!($D(AMHRQUIT))  D
 .Q:$$END
 .W !?20,AMHRX,?62,$J($$COM($G(AMHOPI(AMHRX)),0),7),?72,$J($$PER(AMHOPI(AMHRX),AMHPTOT),7)
 .Q
 W !!?20,"Positive Opioid Tally",?62,$J($$COM(AMHTOPO(6),0),7),?72,$J($$PER(AMHTOPO(6),AMHPTOT),7)
 Q
 ;
HEROIN ;
 W !!?5,"Heroin",!
 S AMHRX="" F  S AMHRX=$O(AMHHERO(AMHRX)) Q:AMHRX=""!($D(AMHRQUIT))  D
 .Q:$$END
 .W !?20,AMHRX,?62,$J($$COM($G(AMHHERO(AMHRX)),0),7),?72,$J($$PER(AMHHERO(AMHRX),AMHPTOT),7)
 .Q
 W !!?20,"Positive Heroin Tally",?62,$J($$COM(AMHTOPO(7),0),7),?72,$J($$PER(AMHTOPO(7),AMHPTOT),7)
 Q
 ;
OTHS ;
 W !!?5,"Other Substance",!
 S AMHRX="" F  S AMHRX=$O(AMHOTHS(AMHRX)) Q:AMHRX=""!($D(AMHRQUIT))  D
 .Q:$$END
 .W !?20,AMHRX,?62,$J($$COM($G(AMHOTHS(AMHRX)),0),7),?72,$J($$PER(AMHOTHS(AMHRX),AMHPTOT),7)
 .Q
 W !!?20,"Positive Other Substance Tally",?62,$J($$COM(AMHTOPO(8),0),7),?72,$J($$PER(AMHTOPO(8),AMHPTOT),7)
 Q
 ;
...
HEADER1 ;
 ...
 W $$CTR("TOBACCO, ALCOHOL, PRESCRIPTION MEDICATION, AND OTHER SUBSTANCES",80),!
 W $$CTR("*** PATIENT TALLY "_$S($D(AMHRPLST):"AND PATIENT LISTING ",1:"")_"***",80),!
