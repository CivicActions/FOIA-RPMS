AMHRDV1P ; IHS/CMI/LAB - list refusals ;
...
PRINT1 ;
 ...
 K AMHRLSTP
 I '$D(^XTMP("AMHRDV1",AMHRJ,AMHRH)) D HEADER W !!,"No data to report.",! G DONE
 D HEADER
 S AMHRTOT=$$TOT
...
DATE ;
 ...
 K ^TMP($J) S X=0 F  S X=$O(^XTMP("AMHRDV1",AMHRJ,AMHRH,"PTS",X)) Q:X'=+X  D
 .S D=$O(^XTMP("AMHRDV1",AMHRJ,AMHRH,"PTS",X,""),-1) S AMHRY=^XTMP("AMHRDV1",AMHRJ,AMHRH,"PTS",X,D)
...
TOT() ;
 NEW C,X
 S C=0
 S X=0 F  S X=$O(^XTMP("AMHRDV1",AMHRJ,AMHRH,"PTS",X)) Q:X'=+X  S C=C+1
 Q C
HEADER ;EP
 G:'AMHRPG HEADER1
...
LIST ;EP - called from xbdbque
 ...
 K ^TMP($J)
 ;resort by sort item
 S DFN=0 F  S DFN=$O(^XTMP("AMHRDV1",AMHRJ,AMHRH,"PTS",DFN)) Q:DFN'=+DFN  S D=$O(^XTMP("AMHRDV1",AMHRJ,AMHRH,"PTS",DFN,""),-1) S AMHRY=^XTMP("AMHRDV1",AMHRJ,AMHRH,"PTS",DFN,D) D
 .D @AMHRSORT
