AMHRDV5P ; IHS/CMI/LAB - list refusals ;
...
PRINT1 ;
 ...
 I '$D(^XTMP("AMHRDV5",AMHRJ,AMHRH)) D HEADER W !!,"No data to report.",! G DONE
 D HEADER
 S AMHRTOT=AMHRCNT
 S AMHRPTOT=$$PTOT
...
RES ;
 ...
 K AMHRRES S X=0 F  S X=$O(^XTMP("AMHRDV5",AMHRJ,AMHRH,"VSTS",X)) Q:X'=+X  D
 .S AMHRY=^XTMP("AMHRDV5",AMHRJ,AMHRH,"VSTS",X)
...
DATE ;
 ...
 K ^TMP($J) S X=0 F  S X=$O(^XTMP("AMHRDV5",AMHRJ,AMHRH,"VSTS",X)) Q:X'=+X  D
 .S AMHRY=^XTMP("AMHRDV5",AMHRJ,AMHRH,"VSTS",X)
...
PTOT() ;
 NEW C,X
 S C=0
 S X=0 F  S X=$O(^XTMP("AMHRDV5",AMHRJ,AMHRH,"PTS",X)) Q:X'=+X  S C=C+1
 Q C
TOT() ;
 NEW C,X
 S C=0
 S X=0 F  S X=$O(^XTMP("AMHRDV5",AMHRJ,AMHRH,"VSTS",X)) Q:X'=+X  S C=C+1
 Q C
HEADER ;EP
 G:'AMHRPG HEADER1
...
HEADER1 ;
 ...
 W !,$$CTR("***  IPV SCREENING VISIT TALLY"_$S(AMHRLIST:" AND VISIT LISTING",1:"")_"  ***",80),!
...
LIST ;EP - called from xbdbque
 ...
 K ^TMP($J)
 ;resort by sort item
 S AMHRX=0 F  S AMHRX=$O(^XTMP("AMHRDV5",AMHRJ,AMHRH,"VSTS",AMHRX)) Q:AMHRX'=+AMHRX  S AMHRY=^XTMP("AMHRDV5",AMHRJ,AMHRH,"VSTS",AMHRX) D
 .S DFN=$P(AMHRY,U,9)
 .D @AMHRSORT
 ...
 .Q:$D(^XTMP("AMHRDV5",AMHRJ,AMHRH,"PTS",DFN))
 .Q:$$END
