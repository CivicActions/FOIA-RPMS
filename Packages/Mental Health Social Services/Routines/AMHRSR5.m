...
INFORM ;
 W !,$$CTR($$USR)
 W !,$$LOC()
 W !!,"*Please Note: This SUICIDE RISK ASSESSMENT report is intended for advanced"
 W !,"RPMS users who are experienced in building search templates and using Q-MAN."
 W !!,$$CTR("TALLY AND LISTING OF ALL VISITS W/SUICIDE RISK ASSESSMENT",80)
 ...
 W !,"Suicide Risk Assessment or a refusal was documented in the"
 W !,"time frame specified by the user.  Suicide Risk Assessment is defined as:"
...
ZIS ;
 ...
 S XBRP="PRINT^AMHRSR5P",XBRC="PROC^AMHRSR5",XBRX="XIT^AMHRSR5",XBNS="AMH"
 D ^XBDBQUE
 D XIT
 Q
BROWSE ;
 S XBRP="VIEWR^XBLM(""^AMHRSR5P"")"
 S XBNS="AMH",XBRC="PROC^AMHRSR5",XBRX="XIT^AMHRSR5",XBIOP=0 D ^XBDBQUE
 Q
XIT ;
 D EN^XBVK("AMHR")
 D ^XBFMK
...
PROC ;
 S AMHRCNT=0
 S AMHRH=$H,AMHRJ=$J
 K ^XTMP("AMHRSR5",AMHRJ,AMHRH)
 D XTMP^AMHUTIL("AMHRSR5","SUICIDE RISK ASSESSMENT REPORT")
 ...
 ..I $D(^XTMP("AMHRSR5",AMHRJ,AMHRH,"PTS",DFN,AMHRDATE)) Q
 ..S AMHRCNT=AMHRCNT+1
 ..S AMHRRES=$$VAL^XBDIQ1(9002011,AMHRBIEN,1407) S:AMHRRES["REFUSED" AMHRRES="REFUSED SCREENING"
 ..S ^XTMP("AMHRSR5",AMHRJ,AMHRH,"VSTS",AMHRCNT)="BH"_U_$$BHPPNAME(AMHRBIEN)_U_AMHRRES_U_$$VAL^XBDIQ1(9002011,AMHRBIEN,1901)_U_$$AGE^AUPNPAT(DFN,AMHRDATE)_U_$$VAL^XBDIQ1(2,DFN,.02)_U_AMHRDATE_U_AMHRBIEN_U_DFN
 ..S $P(^XTMP("AMHRSR5",AMHRJ,AMHRH,"VSTS",AMHRCNT),U,10)=$$VAL^XBDIQ1(9002011,AMHRBIEN,.25)
 ...
 .S ^XTMP("AMHRSR5",AMHRJ,AMHRH,"VSTS",AMHRCNT)="EX"_U_$$PPV(AMHRVIEN)_U_AMHRRES_U_$$VAL^XBDIQ1(9000010.13,AMHREIEN,81101)_U_$$AGE^AUPNPAT(DFN,AMHRDATE)_U_$$VAL^XBDIQ1(2,DFN,.02)_U_AMHRDATE_U_AMHREIEN_U_DFN
 .S $P(^XTMP("AMHRSR5",AMHRJ,AMHRH,"VSTS",AMHRCNT),U,10)=$$VAL^XBDIQ1(9000010,AMHRVIEN,.08)
