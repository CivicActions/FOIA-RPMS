AMHRSR4 ; IHS/CMI/LAB - list refusals ;
...
INFORM ;
 W !,$$CTR($$USR)
 W !,$$LOC()
 W !!,"*Please Note: This SUICIDE RISK ASSESSMENT report is intended for advanced RPMS users"
 W !,"who are experienced in building search templates and using Q-MAN."
 W !!,$$CTR("TALLY AND LISTING OF PATIENT RECEIVING SUICIDE RISK ASSESSMENT, INCLUDING REFUSALS",80)
 ...
 W !,"Suicide Risk Assessment or a refusal documented in the time frame specified"
...
EXCL ;
 S AMHREXPC=""
 W !!,"Would you like to include SUICIDE RISK ASSESSMENT Screenings documented in the PCC clinical"
...
ZIS ;
 ...
 S XBRP="PRINT^AMHRSR4P",XBRC="PROC^AMHRSR4",XBRX="XIT^AMHRSR4",XBNS="AMH"
 D ^XBDBQUE
 D XIT
 Q
BROWSE ;
 S XBRP="VIEWR^XBLM(""^AMHRSR4P"")"
 S XBNS="AMH",XBRC="PROC^AMHRSR4",XBRX="XIT^AMHRSR4",XBIOP=0 D ^XBDBQUE
 Q
XIT ;
 D EN^XBVK("AMHR")
 D ^XBFMK K DIR
...
PROC ;
 S AMHRCNT=0
 S AMHRH=$H,AMHRJ=$J
 K ^XTMP("AMHRSR4",AMHRJ,AMHRH)
 D XTMP^AMHUTIL("AMHRSR4","SUICIDE RISK ASSESSMENT REPORT")
 ...
 ..S X=$O(^XTMP("AMHRSR4",AMHRJ,AMHRH,"PTS",DFN,0))
 ..I X]"",X<AMHRDATE K ^XTMP("AMHRSR4",AMHRJ,AMHRH,"PTS",DFN,X)
 ..S AMHRRES=$$VAL^XBDIQ1(9002011,AMHRBIEN,1407) S:AMHRRES["REFUSED" AMHRRES="REFUSED SCREENING"
 ..S ^XTMP("AMHRSR4",AMHRJ,AMHRH,"PTS",DFN,AMHRDATE)="BH"_U_$$BHPPNAME(AMHRBIEN)_U_AMHRRES_U_$$VAL^XBDIQ1(9002011,AMHRBIEN,1901)_U_$$AGE^AUPNPAT(DFN,AMHRDATE)_U_$$VAL^XBDIQ1(2,DFN,.02)_U_AMHRDATE_U_AMHRBIEN
 ..S $P(^XTMP("AMHRSR4",AMHRJ,AMHRH,"PTS",DFN,AMHRDATE),U,9)=$$VAL^XBDIQ1(9002011,AMHRBIEN,.25)
 ..S $P(^XTMP("AMHRSR4",AMHRJ,AMHRH,"PTS",DFN,AMHRDATE),U,15)=AMHRBIEN
 ..S $P(^XTMP("AMHRSR4",AMHRJ,AMHRH,"PTS",DFN,AMHRDATE),U,16)=$$VAL^XBDIQ1(9002011,AMHRBIEN,1408)
 ..S $P(^XTMP("AMHRSR4",AMHRJ,AMHRH,"PTS",DFN,AMHRDATE),U,17)=$$VAL^XBDIQ1(9000001,DFN,.14)
 ..S $P(^XTMP("AMHRSR4",AMHRJ,AMHRH,"PTS",DFN,AMHRDATE),U,25)=$$RACE^AMHUTIL2(DFN)
 ..S $P(^XTMP("AMHRSR4",AMHRJ,AMHRH,"PTS",DFN,AMHRDATE),U,25)=$$ETHN^AMHUTIL2(DFN)
 ...
 .S ^XTMP("AMHRSR4",AMHRJ,AMHRH,"PTS",DFN,AMHRDATE)="EX"_U_$$PPV(AMHRVIEN)_U_AMHRRES_U_$$VAL^XBDIQ1(9000010.13,AMHREIEN,81101)_U_$$AGE^AUPNPAT(DFN,AMHRDATE)_U_$$VAL^XBDIQ1(2,DFN,.02)_U_AMHRDATE_U_AMHREIEN
 .S $P(^XTMP("AMHRSR4",AMHRJ,AMHRH,"PTS",DFN,AMHRDATE),U,9)=$$VAL^XBDIQ1(9000010,AMHRVIEN,.08)
 .S $P(^XTMP("AMHRSR4",AMHRJ,AMHRH,"PTS",DFN,AMHRDATE),U,15)=AMHRVIEN
 .S $P(^XTMP("AMHRSR4",AMHRJ,AMHRH,"PTS",DFN,AMHRDATE),U,16)=$$SPRV(AMHREIEN)
 .S $P(^XTMP("AMHRSR4",AMHRJ,AMHRH,"PTS",DFN,AMHRDATE),U,25)=$$RACE^AMHUTIL2(DFN)
 .S $P(^XTMP("AMHRSR4",AMHRJ,AMHRH,"PTS",DFN,AMHRDATE),U,25)=$$ETHN^AMHUTIL2(DFN)
 .S $P(^XTMP("AMHRSR4",AMHRJ,AMHRH,"PTS",DFN,AMHRDATE),U,17)=$$VAL^XBDIQ1(9000001,DFN,.14)
 ...
 .S ^XTMP("AMHRSR4",AMHRJ,AMHRH,"PTS",DFN,AMHRDATE)="REF"_U_"UNKNOWN"_U_AMHRRES_U_$$VAL^XBDIQ1(9000022,AMHRRIEN,1101)_U_$$AGE^AUPNPAT(DFN,AMHRDATE)_U_$$VAL^XBDIQ1(2,DFN,.02)_U_AMHRDATE_U_AMHRRIEN_U
 .S $P(^XTMP("AMHRSR4",AMHRJ,AMHRH,"PTS",DFN,AMHRDATE),U,16)=$$PRVREF(AMHRRIEN)
 .S $P(^XTMP("AMHRSR4",AMHRJ,AMHRH,"PTS",DFN,AMHRDATE),U,17)=$$VAL^XBDIQ1(9000001,DFN,.14)
 .S $P(^XTMP("AMHRSR4",AMHRJ,AMHRH,"PTS",DFN,AMHRDATE),U,25)=$$RACE^AMHUTIL2(DFN)
 .S $P(^XTMP("AMHRSR4",AMHRJ,AMHRH,"PTS",DFN,AMHRDATE),U,25)=$$ETHN^AMHUTIL2(DFN)
 Q
