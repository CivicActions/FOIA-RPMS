...
PROCESS ;
 ...
 K AMHPRAT,AMHTARG S AMHGRTA=0,AMHGRTT=0,AMHGTPA=0,AMHGTNS=0
 ...
 ..S AMHGRTA=AMHGRTA+1
 ..S AMHNS=$$VAL^XBDIQ1(9002011,AMHVIEN,.09) S:'X X=1 S AMHGTNS=AMHGTNS+AMHNS
 ...
 ...S $P(AMHPRAT(X),U,1)=$P($G(AMHPRAT(X)),U,1)+1
 ...S $P(AMHPRAT(X),U,2)=$P($G(AMHPRAT(X)),U,2)+AMHNS
 ...I AMHTAR]"" S AMHPRAT(X,AMHTAR)=$G(AMHPRAT(X,AMHTAR))+1,AMHPRAT(X,AMHTAR,"NS")=$G(AMHPRAT(X,AMHTAR,"NS"))+AMHNS
 ...I AMHTAR="" S AMHPRAT(X,"NOT RECORDED")=$G(AMHPRAT(X,"NOT RECORDED"))+1,AMHPRAT(X,"NOT RECORDED","NS")=$G(AMHPRAT(X,"NOT RECORDED","NS"))+1
 ...S AMHGTPA=AMHGTPA+1
 ..Q
 .Q
 S AMHET=$H
...
PRINT1 ;
 ...
 W !,"Total Number Served: ",?45,$$C(AMHGTNS,0)
 S AMHS="" F  S AMHS=$O(AMHPRAT(AMHS)) Q:AMHS=""!($D(AMHQUIT))  D
 .I $Y>(IOSL-3) D HEAD Q:$D(AMHQUIT)
 .S AMHX=AMHPRAT(AMHS)
 .W !!?1,AMHS,?45,$$C($P(AMHX,U,1),0),?57,$J((($P(AMHX,U,1)/AMHGRTA)*100),5,1),?70,$$C($P(AMHX,U,2),0)
...
HEAD1 ;
 ...
 W !?5,"PREVENTION ACTIVITY",?45,"# of visits",?57,"% of visits",?70,"# Served"
 W !,$$REPEAT^XLFSTR("-",80)
 Q
C(X,X2,X3) ;
 D COMMA^%DTC
