AMHRDE2P ; IHS/CMI/LAB - list refusals ;
...
PRINT1 ;
 ...
 K AMHRLSTP
 I '$D(^XTMP("AMHRDE2",AMHRJ,AMHRH)) D HEADER W !!,"No data to report.",! G DONE
 D HEADER
 S AMHRTOT=AMHRCNT
 S AMHRPTOT=$$PTOT
...
RES ;
 ...
 K AMHRRES S X=0 F  S X=$O(^XTMP("AMHRDE2",AMHRJ,AMHRH,"VSTS",X)) Q:X'=+X  D
 .S AMHRY=^XTMP("AMHRDE2",AMHRJ,AMHRH,"VSTS",X)
...
DATE ;
 ...
 K ^TMP($J) S X=0 F  S X=$O(^XTMP("AMHRDE2",AMHRJ,AMHRH,"VSTS",X)) Q:X'=+X  D
 .S AMHRY=^XTMP("AMHRDE2",AMHRJ,AMHRH,"VSTS",X)
...
PTOT() ;
 NEW C,X
 S C=0
 S X=0 F  S X=$O(^XTMP("AMHRDE2",AMHRJ,AMHRH,"PTS",X)) Q:X'=+X  S C=C+1
 Q C
TOT() ;
 NEW C,X
 S C=0
 S X=0 F  S X=$O(^XTMP("AMHRDE2",AMHRJ,AMHRH,"VSTS",X)) Q:X'=+X  S C=C+1
 Q C
HEADER ;EP
 G:'AMHRPG HEADER1
...
LIST ;EP - called from xbdbque
 ...
 K ^TMP($J)
 ;resort by sort item
 S AMHRX=0 F  S AMHRX=$O(^XTMP("AMHRDE2",AMHRJ,AMHRH,"VSTS",AMHRX)) Q:AMHRX'=+AMHRX  S AMHRY=^XTMP("AMHRDE2",AMHRJ,AMHRH,"VSTS",AMHRX) D
 .S DFN=$P(AMHRY,U,15)
 .D @AMHRSORT
