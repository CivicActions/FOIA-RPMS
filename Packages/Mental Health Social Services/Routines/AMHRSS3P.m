AMHRSS3P ; IHS/CMI/LAB - list refusals ;
...
PRINT1 ;
 ...
 K AMHRLSTP
 I '$D(^XTMP("AMHRSS3",AMHRJ,AMHRH)) D HEADER W !!,"No data to report.",! G DONE
 D COVPAGE
 Q:$$END
 D HEADER
 S AMHRTOT=AMHRCNT
...
PTOT() ;
 NEW C,X
 S C=0
 S X=0 F  S X=$O(^XTMP("AMHRSS3",AMHRJ,AMHRH,"PTS",X)) Q:X'=+X  S C=C+1
 Q C
TOT() ;
 NEW C,X
 S C=0
 S X=0 F  S X=$O(^XTMP("AMHRSS3",AMHRJ,AMHRH,"VSTS",X)) Q:X'=+X  S C=C+1
 Q C
HEADER ;EP
 G:'AMHRPG HEADER1
...
HEADER1 ;
 ...
 W !,$$CTR("***  SUICIDE SCREENING VISIT LISTING FOR SELECTED PATIENTS  ***",80),!
...
LIST ;EP - called from xbdbque
 ...
 K ^TMP($J)
 ;resort by sort item
 S AMHRX=0 F  S AMHRX=$O(^XTMP("AMHRSS3",AMHRJ,AMHRH,"VSTS",AMHRX)) Q:AMHRX'=+AMHRX  S AMHRY=^XTMP("AMHRSS3",AMHRJ,AMHRH,"VSTS",AMHRX) D
 .S DFN=$P(AMHRY,U,9)
 .D @AMHRSORT
 ...
 ..W !?3,"                     Race: ",?31,$$RACE^AMHUTIL2(DFN)
 ..W !?3,"                Ethnicity: ",?31,$$ETHN^AMHUTIL2(DFN)
 Q
H ;
...
COVPAGE ;EP
 W:$D(IOF) @IOF
 W !?20,"********** SUICIDE SCREENING SCREENING FOR SELECTED PATIENTS **********"
 W !!,"REPORT REQUESTED BY: ",$P(^VA(200,DUZ,0),U)
 W !!,"The following report contains an SUICIDE SCREENING screening report based on the",!,"following criteria:"
SHOW ;
 ...
 .I X=1 W ?8,"NORMAL/NEGATIVE"
 .I X=2 W "  ","ACUTE POSITIVE"
 .I X=3 W "  ","NON-ACUTE POSITIVE"
 .I X=4 W "  ","REFUSED"
 .I X=5 W "  ","UNABLE TO SCREEN"
 .;I X=6 W "  ","REFERRAL NEEDED"
