AMHRSS31 ; IHS/CMI/LAB - list SUICIDE SCREENING screenings ;
...
PROC ;
 ...
 D XTMP^AMHUTIL("AMHRSS3","SUICIDE SCREENING REPORT")
...
BH ;now go through BH
 ...
 ..;result check
 ..S AMHRRES=$$VAL^XBDIQ1(9002011,AMHRBIEN,1409)
 ..Q:AMHRRES=""
 ..S:AMHRRES["REFUSED" AMHRRES="REFUSED SCREENING"
 ..I AMHRRES="NEGATIVE",'$D(AMHRREST(1)) Q
 ..I AMHRRES="ACUTE POSITIVE",'$D(AMHRREST(2)) Q
 ..I AMHRRES="NON-ACUTE POSITIVE",'$D(AMHRREST(3)) Q
 ..I AMHRRES["REFUSED",'$D(AMHRREST(4)) Q  ;do not want refusals
 ..I AMHRRES["UNABLE",'$D(AMHRREST(5)) Q  ;do not want unables
 ..;I AMHRRES["REFERRAL",'$D(AMHRREST(6)) Q  ;do not want unables
 ...
 ..S X=$P($G(^AMHREC(AMHRBIEN,14)),U,10)
 ...
 ..S ^XTMP("AMHRSS3",AMHRJ,AMHRH,"PTS",DFN)=""
 Q:$D(^XTMP("AMHRSS3",AMHRJ,AMHRH,"PTS",DFN))   ;already got this patient so no need to go on
PCC ;
 Q:'AMHREXPC
...
REF ;now go through refusals in pcc
 Q:$D(^XTMP("AMHRSS3",AMHRJ,AMHRH,"PTS",DFN))  ;already got one so no need to go on
 ...
 ;
GETSCR ;
 K ^XTMP("AMHRSS3",AMHRJ,AMHRH,"VSTS")
 S DFN=0 F  S DFN=$O(^XTMP("AMHRSS3",AMHRJ,AMHRH,"PTS",DFN)) Q:DFN'=+DFN  D GETSCR1
 Q
GETSCR1 ;
 S AMHRSD=$$FMADD^XLFDT(AMHRBD,-1),AMHRSD=AMHRSD_".9999"
 F  S AMHRSD=$O(^AMHREC("AF",DFN,AMHRSD)) Q:AMHRSD'=+AMHRSD!($P(AMHRSD,".")>AMHRED)  D
 ...
 ..;I AMHRRES["REFERRAL",'$D(AMHRREST(6)) Q  ;do not want unables
 ..I AMHRRES="",'$D(AMHRREST(7)) Q
 ..S ^XTMP("AMHRSS3",AMHRJ,AMHRH,"VSTS",AMHRCNT)="BH"_U_$$BHPPNAME(AMHRBIEN)_U_AMHRRES_U_$$VAL^XBDIQ1(9002011,AMHRBIEN,2001)_U_$$AGE^AUPNPAT(DFN,AMHRDATE)_U_$$VAL^XBDIQ1(2,DFN,.02)_U_AMHRDATE_U_AMHRBIEN_U_DFN
 ..S $P(^XTMP("AMHRSS3",AMHRJ,AMHRH,"VSTS",AMHRCNT),U,10)=$$VAL^XBDIQ1(9002011,AMHRBIEN,.25)
 ..S $P(^XTMP("AMHRSS3",AMHRJ,AMHRH,"VSTS",AMHRCNT),U,15)=AMHRBIEN
 ..S $P(^XTMP("AMHRSS3",AMHRJ,AMHRH,"VSTS",AMHRCNT),U,16)=$$VAL^XBDIQ1(9002011,AMHRBIEN,1410)
 ..S $P(^XTMP("AMHRSS3",AMHRJ,AMHRH,"VSTS",AMHRCNT),U,17)=$$VAL^XBDIQ1(9000001,DFN,.14)
 ..S ^XTMP("AMHRSS3",AMHRJ,AMHRH,"PTS",DFN,AMHRDATE)=AMHRCNT
 Q:'AMHREXPC
 ;go through exam SRA, then through AUPNPREF for refusals
 S AMHREIEN=0 F  S AMHREIEN=$O(^AUPNVXAM("AC",DFN,AMHREIEN)) Q:AMHREIEN'=+AMHREIEN  D
 .Q:'$D(^AUPNVXAM(AMHREIEN,0))
 ...
 .Q:AMHRDATE>AMHRED
 .Q:AMHRDATE<AMHRBD
 .I $D(^XTMP("AMHRSS3",AMHRJ,AMHRH,"PTS",DFN,AMHRDATE)) Q
 .S AMHRCNT=AMHRCNT+1
 .S AMHRRES=$$VAL^XBDIQ1(9000010.13,AMHREIEN,.04) S:AMHRRES["REFUSED" AMHRRES="REFUSED SCREENING" S:AMHRRES["NEGATIVE" AMHRRES="NEGATIVE"
 .I AMHRRES="",'$D(AMHRREST(7)) Q  ;NO BLANKS
 .I AMHRRES="NEGATIVE",'$D(AMHRREST(1)) Q
 .I AMHRRES="ACUTE POSITIVE",'$D(AMHRREST(2)) Q
 .I AMHRRES="NON-ACUTE POSITIVE",'$D(AMHRREST(3)) Q
 .;I AMHRRES["REFERRAL",'$D(AMHRREST(6)) Q  ;do not want unables
 .I AMHRRES="",'$D(AMHRREST(7)) Q
 .S ^XTMP("AMHRSS3",AMHRJ,AMHRH,"VSTS",AMHRCNT)="EX"_U_$$PPV(AMHRVIEN)_U_AMHRRES_U_$$VAL^XBDIQ1(9000010.13,AMHREIEN,81101)_U_$$AGE^AUPNPAT(DFN,AMHRDATE)_U_$$VAL^XBDIQ1(2,DFN,.02)_U_AMHRDATE_U_AMHREIEN_U_DFN
 .S $P(^XTMP("AMHRSS3",AMHRJ,AMHRH,"VSTS",AMHRCNT),U,10)=$$VAL^XBDIQ1(9000010,AMHRVIEN,.08)
 .S $P(^XTMP("AMHRSS3",AMHRJ,AMHRH,"VSTS",AMHRCNT),U,15)=AMHRVIEN
 .S $P(^XTMP("AMHRSS3",AMHRJ,AMHRH,"VSTS",AMHRCNT),U,16)=$$SPRV(AMHREIEN)
 .S $P(^XTMP("AMHRSS3",AMHRJ,AMHRH,"VSTS",AMHRCNT),U,17)=$$VAL^XBDIQ1(9000001,DFN,.14)
 .S ^XTMP("AMHRSS3",AMHRJ,AMHRH,"PTS",DFN,AMHRDATE)=AMHRCNT
 ...
 .Q:AMHRDATE>AMHRED
 .Q:AMHRDATE<AMHRBD
 .I $P(^AUPNPREF(AMHRRIEN,0),U,7)="R",'$D(AMHRREST(4)) Q
 .I $P(^AUPNPREF(AMHRRIEN,0),U,7)="U",'$D(AMHRREST(5)) Q
 .I $D(^XTMP("AMHRSS3",AMHRJ,AMHRH,"PTS",DFN,AMHRDATE)) Q
 .S AMHRRES=$$VAL^XBDIQ1(9000022,AMHRRIEN,.07) S:AMHRRES["REFUSED" AMHRRES="REFUSED SCREENING"
 .S AMHRCNT=AMHRCNT+1
 .S ^XTMP("AMHRSS3",AMHRJ,AMHRH,"VSTS",AMHRCNT)="REF"_U_"UNKNOWN"_U_AMHRRES_U_$$VAL^XBDIQ1(9000022,AMHRRIEN,1101)_U_$$AGE^AUPNPAT(DFN,AMHRDATE)_U_$$VAL^XBDIQ1(2,DFN,.02)_U_AMHRDATE_U_AMHRRIEN_U_DFN_U
 .S $P(^XTMP("AMHRSS3",AMHRJ,AMHRH,"VSTS",AMHRCNT),U,16)=$$PRVREF(AMHRRIEN)
 .S $P(^XTMP("AMHRSS3",AMHRJ,AMHRH,"VSTS",AMHRCNT),U,17)=$$VAL^XBDIQ1(9000001,DFN,.14)
 .S ^XTMP("AMHRSS3",AMHRJ,AMHRH,"PTS",DFN,AMHRDATE)=AMHRCNT
 Q
BHPPIN(R) ;
