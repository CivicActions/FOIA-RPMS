...
GETPAT ;
 ...
 S (DFN,AMHPAT)=+Y
 D PFLAG^AMHUTIL(DFN)
 W !?25,"Ok" S %=1 D YN^DICN I %'=1 S DFN="",AMHPAT="" G GETPAT
...
PRINT ;EP - called from xbdbque
 ...
 .W !!,$$FMTE^XLFDT($P(^AMHREC(AMHR,0),U)),?21,$E($$PPNAME^AMHUTIL(AMHR),1,18)
 .S X=$$VALI^XBDIQ1(9002011,AMHR,.34)
 .W ?40,$S(X:"GRP",1:"IND"),?45,$E($$VAL^XBDIQ1(9002011,AMHR,.02),1,7)
...
HEADER1 ;
 ...
 S AMHTEXT="BEHAVIORAL HEALTH NO SHOW REPORT FOR"
 W !?(80-$L(AMHTEXT)/2),AMHTEXT,!
 W ?(80-$L($$GETPREF^AUPNSOGI(AMHPAT,"E",1))/2),$$GETPREF^AUPNSOGI(AMHPAT,"E",1),!
 S AMHTEXT="Appointment Dates:  "_AMHBDD_" and "_AMHEDD
 W ?(80-$L(AMHTEXT)/2),AMHTEXT,!
 ;S X=$P(^DPT(AMHPAT,0),U) W !,$$CTR(X,80),!
 W $TR($J(" ",80)," ","=")
 W !," DATE",?21,"PROVIDER",?40,"I/G",?45,"PROGRAM",?53,"POV"
 W !,$TR($J(" ",80)," ","-")
 Q
