...
PROC ;
 ...
 D XTMP^AMHUTIL("AMHRSR3","SUICIDE RISK ASSESSMENT REPORT")
...
BH ;now go through BH
 ...
 ..I AMHRRES="LOW",'$D(AMHRREST(1)) Q
 ..I AMHRRES="MODERATE",'$D(AMHRREST(2)) Q
 ..I AMHRRES="HIGH",'$D(AMHRREST(3)) Q
 ...
 ..S X=$P($G(^AMHREC(AMHRBIEN,14)),U,8)
 ...
 ..S ^XTMP("AMHRSR3",AMHRJ,AMHRH,"PTS",DFN)=""
 Q:$D(^XTMP("AMHRSR3",AMHRJ,AMHRH,"PTS",DFN))   ;already got this patient so no need to go on
PCC ;
 Q:'AMHREXPC
...
REF ;now go through refusals in pcc
 Q:$D(^XTMP("AMHRSR3",AMHRJ,AMHRH,"PTS",DFN))  ;already got one so no need to go on
 ...
 ;
GETSCR ;
 K ^XTMP("AMHRSR3",AMHRJ,AMHRH,"VSTS")
 S DFN=0 F  S DFN=$O(^XTMP("AMHRSR3",AMHRJ,AMHRH,"PTS",DFN)) Q:DFN'=+DFN  D GETSCR1
 Q
GETSCR1 ;
 S AMHRSD=$$FMADD^XLFDT(AMHRBD,-1),AMHRSD=AMHRSD_".9999"
 F  S AMHRSD=$O(^AMHREC("AF",DFN,AMHRSD)) Q:AMHRSD'=+AMHRSD!($P(AMHRSD,".")>AMHRED)  D
 ...
 ..I AMHRRES["REFERRAL",'$D(AMHRREST(6)) Q  ;do not want unables
 ..I AMHRRES="",'$D(AMHRREST(7)) Q
 ..S ^XTMP("AMHRSR3",AMHRJ,AMHRH,"VSTS",AMHRCNT)="BH"_U_$$BHPPNAME(AMHRBIEN)_U_AMHRRES_U_$$VAL^XBDIQ1(9002011,AMHRBIEN,1901)_U_$$AGE^AUPNPAT(DFN,AMHRDATE)_U_$$VAL^XBDIQ1(2,DFN,.02)_U_AMHRDATE_U_AMHRBIEN_U_DFN
 ..S $P(^XTMP("AMHRSR3",AMHRJ,AMHRH,"VSTS",AMHRCNT),U,10)=$$VAL^XBDIQ1(9002011,AMHRBIEN,.25)
 ..S $P(^XTMP("AMHRSR3",AMHRJ,AMHRH,"VSTS",AMHRCNT),U,15)=AMHRBIEN
 ..S $P(^XTMP("AMHRSR3",AMHRJ,AMHRH,"VSTS",AMHRCNT),U,16)=$$VAL^XBDIQ1(9002011,AMHRBIEN,1408)
 ..S $P(^XTMP("AMHRSR3",AMHRJ,AMHRH,"VSTS",AMHRCNT),U,17)=$$VAL^XBDIQ1(9000001,DFN,.14)
 ..S ^XTMP("AMHRSR3",AMHRJ,AMHRH,"PTS",DFN,AMHRDATE)=""
 Q:'AMHREXPC
 ;go through exam SRA, then through AUPNPREF for refusals
 S AMHREIEN=0 F  S AMHREIEN=$O(^AUPNVXAM("AC",DFN,AMHREIEN)) Q:AMHREIEN'=+AMHREIEN  D
 .Q:'$D(^AUPNVXAM(AMHREIEN,0))
 ...
 .Q:AMHRDATE>AMHRED
 .Q:AMHRDATE<AMHRBD
 .I $D(^XTMP("AMHRSR3",AMHRJ,AMHRH,"PTS",DFN,AMHRDATE)) Q
 .S AMHRCNT=AMHRCNT+1
 .S AMHRRES=$$VAL^XBDIQ1(9000010.13,AMHREIEN,.04) S:AMHRRES["REFUSED" AMHRRES="REFUSED SCREENING"
 ...
 .I AMHRRES["REFERRAL",'$D(AMHRREST(6)) Q  ;do not want unables
 .I AMHRRES="",'$D(AMHRREST(7)) Q
 .S ^XTMP("AMHRSR3",AMHRJ,AMHRH,"VSTS",AMHRCNT)="EX"_U_$$PPV(AMHRVIEN)_U_AMHRRES_U_$$VAL^XBDIQ1(9000010.13,AMHREIEN,81101)_U_$$AGE^AUPNPAT(DFN,AMHRDATE)_U_$$VAL^XBDIQ1(2,DFN,.02)_U_AMHRDATE_U_AMHREIEN_U_DFN
 .S $P(^XTMP("AMHRSR3",AMHRJ,AMHRH,"VSTS",AMHRCNT),U,10)=$$VAL^XBDIQ1(9000010,AMHRVIEN,.08)
 .S $P(^XTMP("AMHRSR3",AMHRJ,AMHRH,"VSTS",AMHRCNT),U,15)=AMHRVIEN
 .S $P(^XTMP("AMHRSR3",AMHRJ,AMHRH,"VSTS",AMHRCNT),U,16)=$$SPRV(AMHREIEN)
 .S $P(^XTMP("AMHRSR3",AMHRJ,AMHRH,"VSTS",AMHRCNT),U,17)=$$VAL^XBDIQ1(9000001,DFN,.14)
 .S ^XTMP("AMHRSR3",AMHRJ,AMHRH,"PTS",DFN,AMHRDATE)=AMHRCNT
 ...
 .S AMHRRES=$$VAL^XBDIQ1(9000022,AMHRRIEN,.07) S:AMHRRES["REFUSED" AMHRRES="REFUSED SCREENING"
 .S AMHRCNT=AMHRCNT+1
 .S ^XTMP("AMHRSR3",AMHRJ,AMHRH,"VSTS",AMHRCNT)="REF"_U_"UNKNOWN"_U_AMHRRES_U_$$VAL^XBDIQ1(9000022,AMHRRIEN,1101)_U_$$AGE^AUPNPAT(DFN,AMHRDATE)_U_$$VAL^XBDIQ1(2,DFN,.02)_U_AMHRDATE_U_AMHRRIEN_U_DFN_U
 .S $P(^XTMP("AMHRSR3",AMHRJ,AMHRH,"VSTS",AMHRCNT),U,16)=$$PRVREF(AMHRRIEN)
 .S $P(^XTMP("AMHRSR3",AMHRJ,AMHRH,"VSTS",AMHRCNT),U,17)=$$VAL^XBDIQ1(9000001,DFN,.14)
 .S ^XTMP("AMHRSR3",AMHRJ,AMHRH,"PTS",DFN,AMHRDATE)=AMHRCNT
 Q
BHPPIN(R) ;
