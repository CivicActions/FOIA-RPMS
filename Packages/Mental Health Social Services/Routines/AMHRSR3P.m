AMHRSR3P ; IHS/CMI/LAB - list refusals ;
 ;;4.0;IHS BEHAVIORAL HEALTH;**8,11,12**;JUN 02, 2010;Build 46
 ;
 ;
...
PRINT1 ;
 ...
 K AMHRLSTP
 I '$D(^XTMP("AMHRSR3",AMHRJ,AMHRH)) D HEADER W !!,"No data to report.",! G DONE
 D COVPAGE
 Q:$$END
 D HEADER
 S AMHRTOT=AMHRCNT
...
PTOT() ;
 NEW C,X
 S C=0
 S X=0 F  S X=$O(^XTMP("AMHRSR3",AMHRJ,AMHRH,"PTS",X)) Q:X'=+X  S C=C+1
 Q C
TOT() ;
 NEW C,X
 S C=0
 S X=0 F  S X=$O(^XTMP("AMHRSR3",AMHRJ,AMHRH,"VSTS",X)) Q:X'=+X  S C=C+1
 Q C
HEADER ;EP
 G:'AMHRPG HEADER1
...
HEADER1 ;
 ...
 W !,$$CTR("***  SUICIDE RISK ASSESSMENT VISIT LISTING FOR SELECTED PATIENTS  ***",80),!
...
LIST ;EP - called from xbdbque
 ...
 K ^TMP($J)
 ;resort by sort item
 S AMHRX=0 F  S AMHRX=$O(^XTMP("AMHRSR3",AMHRJ,AMHRH,"VSTS",AMHRX)) Q:AMHRX'=+AMHRX  S AMHRY=^XTMP("AMHRSR3",AMHRJ,AMHRH,"VSTS",AMHRX) D
 .S DFN=$P(AMHRY,U,9)
 .D @AMHRSORT
...
COVPAGE ;EP
 W:$D(IOF) @IOF
 W !?20,"********** SUICIDE RISK ASSESSMENT SCREENING FOR SELECTED PATIENTS **********"
 W !!,"REPORT REQUESTED BY: ",$P(^VA(200,DUZ,0),U)
 W !!,"The following report contains an SUICIDE RISK ASSESSMENT screening report based on the",!,"following criteria:"
SHOW ;
