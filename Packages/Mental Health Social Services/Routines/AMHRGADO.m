AMHRGADO ; IHS/CMI/LAB - BROWSE VISITS ;
...
START ;
 W:$D(IOF) @IOF
 D EN^XBVK("AMH")
 W !,$$CTR("GAD-2 and GAD-7 Generalized Anxiety Disorder Outcomes - Scores for One Patient",80),!!
 W !,"This option is used to list GAD2 and GAD7 Scores for one patient within",!,"a date range specified by the user.",!
 D DBHUSR^AMHUTIL
PAT ;
 S DFN=""
...
WHICH ;
 W !!,"Please note:  Only visits with GAD2 and GAD7 scores recorded will",!,"display on this list.",!
 S AMHQUIT=0
 S AMHW=""
...
BROWSE ;
 K ^TMP("AMHRGADO",$J)
 D GATHER
 D EN^VALM("AMH GAD SCORES ONE PATIENT")
 K ^TMP("AMHRGADO",$J)
 D CLEAR^VALM1
 D FULL^VALM1
END ;
...
PROV ;
 ...
 Q
HASGAD(V) ;EP - does this visit have a GAD measurement
 NEW X,Y,Z
 S (X,Z)=0
 F  S X=$O(^AMHRMSR("AD",V,X)) Q:X'=+X  S Y=$$VAL^XBDIQ1(9002011.12,X,.01) I Y="GAD2"!(Y="GAD7") S Z=1
 Q Z
HASGADV(V) ;EP
 NEW X,Y,Z
 S (X,Z)=0
 F  S X=$O(^AUPNVMSR("AD",V,X)) Q:X'=+X  S Y=$$VAL^XBDIQ1(9000010.01,X,.01) I Y="GAD2"!(Y="GAD7") S Z=1
 Q Z
PRINT ;EP - called from xbdbque
 S AMHQUIT=0
 ;gather up all visit records in ^TMP("AMHRGADO",$J
 D GATHER
 D PRINT1
 K ^TMP("AMHRGADO",$J)
 Q
...
PRINT1 ;
 W:$D(IOF) @IOF
 NEW AMHX
 S AMHX=0 F  S AMHX=$O(^TMP("AMHRGADO",$J,AMHX)) Q:AMHX'=+AMHX!(AMHQUIT)  D
 .I $Y>(IOSL-5) D FF Q:AMHQUIT
 .W !,^TMP("AMHRGADO",$J,AMHX,0)
 .Q
 Q
GATHER ;
 K ^TMP("AMHRGADO",$J)
 ...
 S X=$TR($J("",80)," ","*") D S(X)
 S X="Date",$E(X,11)="GAD2",$E(X,16)="GAD7",$E(X,26)="PROVIDER",$E(X,41)="CLINIC",$E(X,55)="Diagnosis/POV" D S(X)
 ...
 .....I Y="GAD2" S Z=Z_$P(^AMHRMSR(X,0),U,4)_" "
 .....I Y="GAD7" S N=N_$P(^AMHRMSR(X,0),U,4)_" "
 ....S $E(AMHX,11)=Z
 ....S $E(AMHX,16)=N
 ....S $E(AMHX,21)=J
 ...
 .....I Y="GAD2" S Z=Z_$P(^AUPNVMSR(X,0),U,4)_" "
 .....I Y="GAD7" S N=N_$P(^AUPNVMSR(X,0),U,4)_" "
 ....S $E(AMHX,11)=Z
 ....S $E(AMHX,16)=N
 ....S $E(AMHX,21)=J
 ...
 ....D S(AMHX)
 I AMHCNT=0 S X="No Visits with GAD2/GAD7 measurements in the specified time frame." D S(X,1)
 Q
...
FF ;EP
 ...
 .W H,?46,"DOB: ",$$FMTE^XLFDT($P(^DPT($P(AMHR0,U,8),0),U,3),"2D"),!   ;?59,"SSN: ",$$SSN^AMHUTIL($P(AMHR0,U,8)),! IHS/CMI/LAB REMOVED SSN P 11
 W:$D(IOF) @IOF
 Q
HDR ; -- header code
 Q
...
S1 ;
 S AMHC=AMHC+1
 S ^TMP("AMHRGADO",$J,AMHC,0)=X
 Q
INIT ; -- init variables and list array
 S VALMCNT=$O(^TMP("AMHRGADO",$J,""),-1)
 Q
 ;
HELP ; -- help code
 S X="?" D DISP^XQORM1 W !!
