AMHRSS1 ; IHS/CMI/LAB - list SUICIDE SCREENING 24 Aug 2009 6:21 PM ;
...
INFORM ;
 W !,$$CTR($$USR)
 W !,$$LOC()
 W !!,$$CTR("TALLY AND LISTING OF PATIENTS RECEIVING SUICIDE ",80),!,$$CTR("SCREENING, INCLUDING REFUSALS",80)
 W !!,"This report will tally and optionally list all patients who have had "
 W !,"a Suicide Screening or a refusal documented in the time frame"
 W !,"specified by the user.  Suicide Screening is defined as any of the"
 W !,"following documented: "
 W !?10,"- Suicide Screening (Exam code 44)"
 W !?10,"- refusal of exam code 44"
 ...
 D DBHUSR^AMHUTIL
 D XIT
 S AMHREXC=$O(^AUTTEXAM("C",44,0))
 I 'AMHREXC W !!,"Exam code 44 is missing from the EXAM table.  Cannot run report.",! H 3 D XIT Q
 ;
...
EXCL ;
 S AMHREXPC=""
 W !!,"Would you like to include Suicide Screenings documented in the PCC"
...
LIST1 ;
 S AMHRSORT=""
 W !
 S DIR(0)="S^H:Health Record Number;N:Patient Name;P:Provider who screened;C:Clinic;R:Result of Screening;D:Date Screened;A:Age of Patient at Screening;G:Gender of Patient;T:Terminal Digit HRN;Q:Race;E:Ethnicity"
...
ZIS ;CALL TO XBDBQUE
 ...
 S XBRP="PRINT^AMHRSS1P",XBRC="PROC^AMHRSS1",XBRX="XIT^AMHRSS1",XBNS="AMH"
 D ^XBDBQUE
 D XIT
 Q
BROWSE ;
 S XBRP="VIEWR^XBLM(""^AMHRSS1P"")"
 S XBNS="AMH",XBRC="PROC^AMHRSS1",XBRX="XIT^AMHRSS1",XBIOP=0 D ^XBDBQUE
 Q
XIT ;
 D EN^XBVK("AMHR")
 D ^XBFMK
...
PROC ;
 S AMHRCNT=0
 S AMHRH=$H,AMHRJ=$J
 K ^XTMP("AMHRSS1",AMHRJ,AMHRH)
 D XTMP^AMHUTIL("AMHRSS1","SUICIDE SCREENING REPORT")
 ;now go through BH
 ...
 ..;S X=$O(^XTMP("AMHRSS1",AMHRJ,AMHRH,"PTS",DFN,0))
 ..;I X]"",X<AMHRDATE K ^XTMP("AMHRSS1",AMHRJ,AMHRH,"PTS",DFN,X)
 ..S AMHRRES=$$VAL^XBDIQ1(9002011,AMHRBIEN,1409) S:AMHRRES["REFUSED" AMHRRES="REFUSED SCREENING"
 ..S ^XTMP("AMHRSS1",AMHRJ,AMHRH,"PTS",DFN,AMHRDATE)="BH"_U_$$BHPPNAME(AMHRBIEN)_U_AMHRRES_U_$$VAL^XBDIQ1(9002011,AMHRBIEN,2001)_U_$$AGE^AUPNPAT(DFN,AMHRDATE)_U_$$VAL^XBDIQ1(2,DFN,.02)_U_AMHRDATE_U_AMHRBIEN
 ..S $P(^XTMP("AMHRSS1",AMHRJ,AMHRH,"PTS",DFN,AMHRDATE),U,9)=$$VAL^XBDIQ1(9002011,AMHRBIEN,.25)
 ..S $P(^XTMP("AMHRSS1",AMHRJ,AMHRH,"PTS",DFN,AMHRDATE),U,15)=AMHRBIEN
 ..S $P(^XTMP("AMHRSS1",AMHRJ,AMHRH,"PTS",DFN,AMHRDATE),U,16)=$S($P($G(^AMHREC(AMHRBIEN,14)),U,10):$$VAL^XBDIQ1(9002011,AMHRBIEN,1410),1:"UNKNOWN")
 ..S $P(^XTMP("AMHRSS1",AMHRJ,AMHRH,"PTS",DFN,AMHRDATE),U,17)=$$VAL^XBDIQ1(9000001,DFN,.14)
 ..S $P(^XTMP("AMHRSS1",AMHRJ,AMHRH,"PTS",DFN,AMHRDATE),U,18)=$$VAL^XBDIQ1(9002011.55,DFN,.02)
 ..S $P(^XTMP("AMHRSS1",AMHRJ,AMHRH,"PTS",DFN,AMHRDATE),U,19)=$$VAL^XBDIQ1(9002011.55,DFN,.03)
 ..S $P(^XTMP("AMHRSS1",AMHRJ,AMHRH,"PTS",DFN,AMHRDATE),U,20)=$$VAL^XBDIQ1(9002011.55,DFN,.04)
 ..S $P(^XTMP("AMHRSS1",AMHRJ,AMHRH,"PTS",DFN,AMHRDATE),U,25)=$$RACE^AMHUTIL2(DFN)
 ..S $P(^XTMP("AMHRSS1",AMHRJ,AMHRH,"PTS",DFN,AMHRDATE),U,26)=$$ETHN^AMHUTIL2(DFN)
 ...
 .Q:AMHRDATE>AMHRED
 .Q:AMHRDATE<AMHRBD
 .Q:$D(^XTMP("AMHRSS1",AMHRJ,AMHRH,"PTS",DFN,AMHRDATE))  ;already got one from BH on this date
 .;S X=$O(^XTMP("AMHRSS1",AMHRJ,AMHRH,"PTS",DFN,0))
 .;I X]"",X<AMHRDATE K ^XTMP("AMHRSS1",AMHRJ,AMHRH,"PTS",DFN,X)
 ...
 .S ^XTMP("AMHRSS1",AMHRJ,AMHRH,"PTS",DFN,AMHRDATE)="EX"_U_$$PPV(AMHRVIEN)_U_AMHRRES_U_$$VAL^XBDIQ1(9000010.13,AMHREIEN,81101)_U_$$AGE^AUPNPAT(DFN,AMHRDATE)_U_$$VAL^XBDIQ1(2,DFN,.02)_U_AMHRDATE_U_AMHREIEN
 .S $P(^XTMP("AMHRSS1",AMHRJ,AMHRH,"PTS",DFN,AMHRDATE),U,9)=$$VAL^XBDIQ1(9000010,AMHRVIEN,.08)
 .S $P(^XTMP("AMHRSS1",AMHRJ,AMHRH,"PTS",DFN,AMHRDATE),U,15)=AMHRVIEN
 .S $P(^XTMP("AMHRSS1",AMHRJ,AMHRH,"PTS",DFN,AMHRDATE),U,16)=$$SPRV(AMHREIEN)
 .S $P(^XTMP("AMHRSS1",AMHRJ,AMHRH,"PTS",DFN,AMHRDATE),U,17)=$$VAL^XBDIQ1(9000001,DFN,.14)
 .S $P(^XTMP("AMHRSS1",AMHRJ,AMHRH,"PTS",DFN,AMHRDATE),U,18)=$$VAL^XBDIQ1(9002011.55,DFN,.02)
 .S $P(^XTMP("AMHRSS1",AMHRJ,AMHRH,"PTS",DFN,AMHRDATE),U,19)=$$VAL^XBDIQ1(9002011.55,DFN,.03)
 .S $P(^XTMP("AMHRSS1",AMHRJ,AMHRH,"PTS",DFN,AMHRDATE),U,20)=$$VAL^XBDIQ1(9002011.55,DFN,.04)
 ...
 .S ^XTMP("AMHRSS1",AMHRJ,AMHRH,"PTS",DFN,AMHRDATE)="REF"_U_"UNKNOWN"_U_AMHRRES_U_$$VAL^XBDIQ1(9000022,AMHRRIEN,1101)_U_$$AGE^AUPNPAT(DFN,AMHRDATE)_U_$$VAL^XBDIQ1(2,DFN,.02)_U_AMHRDATE_U_AMHRRIEN_U
 .S $P(^XTMP("AMHRSS1",AMHRJ,AMHRH,"PTS",DFN,AMHRDATE),U,16)=$$PRVREF(AMHRRIEN)
 .S $P(^XTMP("AMHRSS1",AMHRJ,AMHRH,"PTS",DFN,AMHRDATE),U,17)=$$VAL^XBDIQ1(9000001,DFN,.14)
 .S $P(^XTMP("AMHRSS1",AMHRJ,AMHRH,"PTS",DFN,AMHRDATE),U,18)=$$VAL^XBDIQ1(9002011.55,DFN,.02)
 .S $P(^XTMP("AMHRSS1",AMHRJ,AMHRH,"PTS",DFN,AMHRDATE),U,19)=$$VAL^XBDIQ1(9002011.55,DFN,.03)
 .S $P(^XTMP("AMHRSS1",AMHRJ,AMHRH,"PTS",DFN,AMHRDATE),U,20)=$$VAL^XBDIQ1(9002011.55,DFN,.04)
