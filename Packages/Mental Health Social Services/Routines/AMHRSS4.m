AMHRSS4 ; IHS/CMI/LAB - list refusals ;
...
INFORM ;
 W !,$$CTR($$USR)
 W !,$$LOC()
 W !!,"*Please Note: This SUICIDE SCREENING report is intended for advanced RPMS users"
 W !,"who are experienced in building search templates and using Q-MAN."
 W !!,$$CTR("TALLY AND LISTING OF PATIENT RECEIVING SUICIDE SCREENING, INCLUDING REFUSALS",80)
 ...
 W !,"Suicide Screening or a refusal documented in the time frame specified"
 W !,"by the user.  Suicide Screening is defined as any of the following"
 W !,"documented:"
...
ZIS ;
 ...
 S XBRP="PRINT^AMHRSS4P",XBRC="PROC^AMHRSS4",XBRX="XIT^AMHRSS4",XBNS="AMH"
 D ^XBDBQUE
 D XIT
 Q
BROWSE ;
 S XBRP="VIEWR^XBLM(""^AMHRSS4P"")"
 S XBNS="AMH",XBRC="PROC^AMHRSS4",XBRX="XIT^AMHRSS4",XBIOP=0 D ^XBDBQUE
 Q
XIT ;
 D EN^XBVK("AMHR")
 D ^XBFMK K DIR
...
PROC ;
 S AMHRCNT=0
 S AMHRH=$H,AMHRJ=$J
 K ^XTMP("AMHRSS4",AMHRJ,AMHRH)
 D XTMP^AMHUTIL("AMHRSS4","SUICIDE SCREENING REPORT")
 ...
 ..S X=$O(^XTMP("AMHRSS4",AMHRJ,AMHRH,"PTS",DFN,0))
 ..I X]"",X<AMHRDATE K ^XTMP("AMHRSS4",AMHRJ,AMHRH,"PTS",DFN,X)
 ..S AMHRRES=$$VAL^XBDIQ1(9002011,AMHRBIEN,1409) S:AMHRRES["REFUSED" AMHRRES="REFUSED SCREENING" S:AMHRRES["NEGATIVE" AMHRRES="NEGATIVE"
 ..S ^XTMP("AMHRSS4",AMHRJ,AMHRH,"PTS",DFN,AMHRDATE)="BH"_U_$$BHPPNAME(AMHRBIEN)_U_AMHRRES_U_$$VAL^XBDIQ1(9002011,AMHRBIEN,2001)_U_$$AGE^AUPNPAT(DFN,AMHRDATE)_U_$$VAL^XBDIQ1(2,DFN,.02)_U_AMHRDATE_U_AMHRBIEN
 ..S $P(^XTMP("AMHRSS4",AMHRJ,AMHRH,"PTS",DFN,AMHRDATE),U,9)=$$VAL^XBDIQ1(9002011,AMHRBIEN,.25)
 ..S $P(^XTMP("AMHRSS4",AMHRJ,AMHRH,"PTS",DFN,AMHRDATE),U,15)=AMHRBIEN
 ..S $P(^XTMP("AMHRSS4",AMHRJ,AMHRH,"PTS",DFN,AMHRDATE),U,16)=$$VAL^XBDIQ1(9002011,AMHRBIEN,1410)
 ..S $P(^XTMP("AMHRSS4",AMHRJ,AMHRH,"PTS",DFN,AMHRDATE),U,17)=$$VAL^XBDIQ1(9000001,DFN,.14)
 ..S $P(^XTMP("AMHRSS4",AMHRJ,AMHRH,"PTS",DFN,AMHRDATE),U,25)=$$RACE^AMHUTIL2(DFN)
 ..S $P(^XTMP("AMHRSS4",AMHRJ,AMHRH,"PTS",DFN,AMHRDATE),U,26)=$$ETHN^AMHUTIL2(DFN)
 ...
 .S ^XTMP("AMHRSS4",AMHRJ,AMHRH,"PTS",DFN,AMHRDATE)="EX"_U_$$PPV(AMHRVIEN)_U_AMHRRES_U_$$VAL^XBDIQ1(9000010.13,AMHREIEN,81101)_U_$$AGE^AUPNPAT(DFN,AMHRDATE)_U_$$VAL^XBDIQ1(2,DFN,.02)_U_AMHRDATE_U_AMHREIEN
 .S $P(^XTMP("AMHRSS4",AMHRJ,AMHRH,"PTS",DFN,AMHRDATE),U,9)=$$VAL^XBDIQ1(9000010,AMHRVIEN,.08)
 .S $P(^XTMP("AMHRSS4",AMHRJ,AMHRH,"PTS",DFN,AMHRDATE),U,15)=AMHRVIEN
 .S $P(^XTMP("AMHRSS4",AMHRJ,AMHRH,"PTS",DFN,AMHRDATE),U,16)=$$SPRV(AMHREIEN)
 .S $P(^XTMP("AMHRSS4",AMHRJ,AMHRH,"PTS",DFN,AMHRDATE),U,17)=$$VAL^XBDIQ1(9000001,DFN,.14)
 .S $P(^XTMP("AMHRSS4",AMHRJ,AMHRH,"PTS",DFN,AMHRDATE),U,25)=$$RACE^AMHUTIL2(DFN)
 .S $P(^XTMP("AMHRSS4",AMHRJ,AMHRH,"PTS",DFN,AMHRDATE),U,26)=$$ETHN^AMHUTIL2(DFN)
 ...
 .S ^XTMP("AMHRSS4",AMHRJ,AMHRH,"PTS",DFN,AMHRDATE)="REF"_U_"UNKNOWN"_U_AMHRRES_U_$$VAL^XBDIQ1(9000022,AMHRRIEN,1101)_U_$$AGE^AUPNPAT(DFN,AMHRDATE)_U_$$VAL^XBDIQ1(2,DFN,.02)_U_AMHRDATE_U_AMHRRIEN_U
 .S $P(^XTMP("AMHRSS4",AMHRJ,AMHRH,"PTS",DFN,AMHRDATE),U,16)=$$PRVREF(AMHRRIEN)
 .S $P(^XTMP("AMHRSS4",AMHRJ,AMHRH,"PTS",DFN,AMHRDATE),U,17)=$$VAL^XBDIQ1(9000001,DFN,.14)
 .S $P(^XTMP("AMHRSS4",AMHRJ,AMHRH,"PTS",DFN,AMHRDATE),U,25)=$$RACE^AMHUTIL2(DFN)
 .S $P(^XTMP("AMHRSS4",AMHRJ,AMHRH,"PTS",DFN,AMHRDATE),U,26)=$$ETHN^AMHUTIL2(DFN)
 Q
