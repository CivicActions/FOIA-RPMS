AMHRDV1 ; IHS/CMI/LAB - list IPV/DV 24 Aug 2009 6:21 PM ;
 ;;4.0;IHS BEHAVIORAL HEALTH;**11**;JUN 02, 2010;Build 27
 ;
 ;IHS/CMI/LAB - PATCH 11 ADDED RACE/ETHNICITY
INFORM ;
 W !,$$CTR($$USR)
 W !,$$LOC()
 ...
 W !,"exam provider, if available, or primary provider on the visit), clinic,"
 W !,"date of screening, designated PCP, MH Provider, SS Provider and A/SA"
 W !,"Provider."
 W !,"  Notes:  "
...
ZIS ;CALL TO XBDBQUE
 ...
 S XBRP="PRINT^AMHRDV1P",XBRC="PROC^AMHRDV1",XBRX="XIT^AMHRDV1",XBNS="AMH"
 D ^XBDBQUE
 D XIT
 Q
BROWSE ;
 S XBRP="VIEWR^XBLM(""^AMHRDV1P"")"
 S XBNS="AMH",XBRC="PROC^AMHRDV1",XBRX="XIT^AMHRDV1",XBIOP=0 D ^XBDBQUE
 Q
XIT ;
 D EN^XBVK("AMHR")
 D ^XBFMK
...
PROC ;
 S AMHRCNT=0
 S AMHRH=$H,AMHRJ=$J
 K ^XTMP("AMHRDV1",AMHRJ,AMHRH)
 D XTMP^AMHUTIL("AMHRDV1","IPV SCREENING REPORT")
 ;now go through BH
 ...
 ..;S X=$O(^XTMP("AMHRDV1",AMHRJ,AMHRH,"PTS",DFN,0))
 ..;I X]"",X<AMHRDATE K ^XTMP("AMHRDV1",AMHRJ,AMHRH,"PTS",DFN,X)
 ...
 ..S ^XTMP("AMHRDV1",AMHRJ,AMHRH,"PTS",DFN,AMHRDATE)="BH"_U_$$BHPPNAME(AMHRBIEN)_U_AMHRRES_U_$$VAL^XBDIQ1(9002011,AMHRBIEN,1501)_U_$$AGE^AUPNPAT(DFN,AMHRDATE)_U_$$VAL^XBDIQ1(2,DFN,.02)_U_AMHRDATE_U_AMHRBIEN
 ..S $P(^XTMP("AMHRDV1",AMHRJ,AMHRH,"PTS",DFN,AMHRDATE),U,9)=$$VAL^XBDIQ1(9002011,AMHRBIEN,.25)
 ..S $P(^XTMP("AMHRDV1",AMHRJ,AMHRH,"PTS",DFN,AMHRDATE),U,15)=AMHRBIEN
 ..S $P(^XTMP("AMHRDV1",AMHRJ,AMHRH,"PTS",DFN,AMHRDATE),U,16)=$S($P($G(^AMHREC(AMHRBIEN,14)),U,2):$$VAL^XBDIQ1(9002011,AMHRBIEN,1402),1:"UNKNOWN")
 ..S $P(^XTMP("AMHRDV1",AMHRJ,AMHRH,"PTS",DFN,AMHRDATE),U,17)=$$VAL^XBDIQ1(9000001,DFN,.14)
 ..S $P(^XTMP("AMHRDV1",AMHRJ,AMHRH,"PTS",DFN,AMHRDATE),U,18)=$$VAL^XBDIQ1(9002011.55,DFN,.02)
 ..S $P(^XTMP("AMHRDV1",AMHRJ,AMHRH,"PTS",DFN,AMHRDATE),U,19)=$$VAL^XBDIQ1(9002011.55,DFN,.03)
 ..S $P(^XTMP("AMHRDV1",AMHRJ,AMHRH,"PTS",DFN,AMHRDATE),U,20)=$$VAL^XBDIQ1(9002011.55,DFN,.04)
 ..S $P(^XTMP("AMHRDV1",AMHRJ,AMHRH,"PTS",DFN,AMHRDATE),U,25)=$$RACE^AMHUTIL2(DFN)  ;**LORI
 ..S $P(^XTMP("AMHRDV1",AMHRJ,AMHRH,"PTS",DFN,AMHRDATE),U,26)=$$ETHN^AMHUTIL2(DFN)
 ...
 .Q:AMHRDATE>AMHRED
 .Q:AMHRDATE<AMHRBD
 .Q:$D(^XTMP("AMHRDV1",AMHRJ,AMHRH,"PTS",DFN,AMHRDATE))  ;already got one from BH on this date
 .;S X=$O(^XTMP("AMHRDV1",AMHRJ,AMHRH,"PTS",DFN,0))
 .;I X]"",X<AMHRDATE K ^XTMP("AMHRDV1",AMHRJ,AMHRH,"PTS",DFN,X)
 ...
 .S ^XTMP("AMHRDV1",AMHRJ,AMHRH,"PTS",DFN,AMHRDATE)="EX"_U_$$PPV(AMHRVIEN)_U_AMHRRES_U_$$VAL^XBDIQ1(9000010.13,AMHREIEN,81101)_U_$$AGE^AUPNPAT(DFN,AMHRDATE)_U_$$VAL^XBDIQ1(2,DFN,.02)_U_AMHRDATE_U_AMHREIEN
 .S $P(^XTMP("AMHRDV1",AMHRJ,AMHRH,"PTS",DFN,AMHRDATE),U,9)=$$VAL^XBDIQ1(9000010,AMHRVIEN,.08)
 .S $P(^XTMP("AMHRDV1",AMHRJ,AMHRH,"PTS",DFN,AMHRDATE),U,15)=AMHRVIEN
 .S $P(^XTMP("AMHRDV1",AMHRJ,AMHRH,"PTS",DFN,AMHRDATE),U,16)=$$SPRV(AMHREIEN)
 .S $P(^XTMP("AMHRDV1",AMHRJ,AMHRH,"PTS",DFN,AMHRDATE),U,17)=$$VAL^XBDIQ1(9000001,DFN,.14)
 .S $P(^XTMP("AMHRDV1",AMHRJ,AMHRH,"PTS",DFN,AMHRDATE),U,18)=$$VAL^XBDIQ1(9002011.55,DFN,.02)
 .S $P(^XTMP("AMHRDV1",AMHRJ,AMHRH,"PTS",DFN,AMHRDATE),U,19)=$$VAL^XBDIQ1(9002011.55,DFN,.03)
 .S $P(^XTMP("AMHRDV1",AMHRJ,AMHRH,"PTS",DFN,AMHRDATE),U,20)=$$VAL^XBDIQ1(9002011.55,DFN,.04)
 .S $P(^XTMP("AMHRDV1",AMHRJ,AMHRH,"PTS",DFN,AMHRDATE),U,25)=$$RACE^AMHUTIL2(DFN)  ;**LORI
 .S $P(^XTMP("AMHRDV1",AMHRJ,AMHRH,"PTS",DFN,AMHRDATE),U,26)=$$ETHN^AMHUTIL2(DFN)  ;**LORI
 ...
 .S ^XTMP("AMHRDV1",AMHRJ,AMHRH,"PTS",DFN,AMHRDATE)="REF"_U_"UNKNOWN"_U_AMHRRES_U_$$VAL^XBDIQ1(9000022,AMHRRIEN,1101)_U_$$AGE^AUPNPAT(DFN,AMHRDATE)_U_$$VAL^XBDIQ1(2,DFN,.02)_U_AMHRDATE_U_AMHRRIEN_U
 .S $P(^XTMP("AMHRDV1",AMHRJ,AMHRH,"PTS",DFN,AMHRDATE),U,16)=$$PRVREF(AMHRRIEN)
 .S $P(^XTMP("AMHRDV1",AMHRJ,AMHRH,"PTS",DFN,AMHRDATE),U,17)=$$VAL^XBDIQ1(9000001,DFN,.14)
 .S $P(^XTMP("AMHRDV1",AMHRJ,AMHRH,"PTS",DFN,AMHRDATE),U,18)=$$VAL^XBDIQ1(9002011.55,DFN,.02)
 .S $P(^XTMP("AMHRDV1",AMHRJ,AMHRH,"PTS",DFN,AMHRDATE),U,19)=$$VAL^XBDIQ1(9002011.55,DFN,.03)
 .S $P(^XTMP("AMHRDV1",AMHRJ,AMHRH,"PTS",DFN,AMHRDATE),U,20)=$$VAL^XBDIQ1(9002011.55,DFN,.04)
