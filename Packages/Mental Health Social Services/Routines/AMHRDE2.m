AMHRDE2 ; IHS/CMI/LAB - list refusals ;
...
ZIS ;
 ...
 S XBRP="PRINT^AMHRDE2P",XBRC="PROC^AMHRDE2",XBRX="XIT^AMHRDE2",XBNS="AMH"
 D ^XBDBQUE
 D XIT
 Q
BROWSE ;
 S XBRP="VIEWR^XBLM(""^AMHRDE2P"")"
 S XBNS="AMH",XBRC="PROC^AMHRDE2",XBRX="XIT^AMHRDE2",XBIOP=0 D ^XBDBQUE
 Q
XIT ;
 D EN^XBVK("AMHR")
 D ^XBFMK
...
PROC ;
 S AMHRCNT=0
 S AMHRH=$H,AMHRJ=$J
 K ^XTMP("AMHRDE2",AMHRJ,AMHRH)
 D XTMP^AMHUTIL("AMHRDE2","DEPRESSION SCREENING REPORT")
 ;now go through BH
 ...
 ..Q:$$DEMO^AMHUTIL1(DFN,$G(AMHMENU))
 ..S AMHSCR=$$BHSCR(AMHRBIEN)
 ..Q:AMHSCR=""
 ..S AMHRCNT=AMHRCNT+1
 ..S ^XTMP("AMHRDE2",AMHRJ,AMHRH,"PTS",DFN,AMHRDATE)=""
 ..S ^XTMP("AMHRDE2",AMHRJ,AMHRH,"VSTS",AMHRCNT)=AMHSCR
 ..S $P(^XTMP("AMHRDE2",AMHRJ,AMHRH,"VSTS",AMHRCNT),U,25)=$$RACE^AMHUTIL2(DFN)
 ..S $P(^XTMP("AMHRDE2",AMHRJ,AMHRH,"VSTS",AMHRCNT),U,26)=$$ETHN^AMHUTIL2(DFN)
PCC ;
 Q:'AMHREXPC
 S AMHRSD=$$FMADD^XLFDT(AMHRBD,-1),AMHRSD=AMHRSD_".9999"
 ...
 .Q:$$DEMO^AMHUTIL1(DFN,$G(AMHDEMO))
 .Q:'$$ALLOWP^AMHUTIL(DUZ,DFN)
 .I $D(^XTMP("AMHRDE2",AMHRJ,AMHRH,"PTS",DFN,AMHRDATE)) Q
 .S AMHRCNT=AMHRCNT+1
 .S ^XTMP("AMHRDE2",AMHRJ,AMHRH,"PTS",DFN,AMHRDATE)=""
 .S T=AMHRDATE
 ...
 .S $P(T,U,15)=DFN
 .S $P(T,U,20)="PCC"
 .S ^XTMP("AMHRDE2",AMHRJ,AMHRH,"VSTS",AMHRCNT)=T
 .S $P(^XTMP("AMHRDE2",AMHRJ,AMHRH,"VSTS",AMHRCNT),U,25)=$$RACE^AMHUTIL2(DFN)
 .S $P(^XTMP("AMHRDE2",AMHRJ,AMHRH,"VSTS",AMHRCNT),U,26)=$$ETHN^AMHUTIL2(DFN)
 Q
 ;
BHSCR(V) ;EP - is there a screening?  return in R
 ...
 .I M="PHQ2"!(M="PHQ9")!(M="PHQT") S R=$$BHRT^AMHRDE1(V,M,$P(^AMHRMSR(X,0),U,4),P,$$VALI^XBDIQ1(9002011.12,X,1204))
 I R]"" Q R
 ;get exam
 S E=$P($G(^AMHREC(V,14)),U,3)
 I E]"" S R=$$BHRT^AMHRDE1(V,"DEPRESSION SCREENING",$$VAL^XBDIQ1(9002011,V,1403),P,$P($G(^AMHREC(V,14)),U,4),$P($G(^AMHREC(V,16)),U,1))
 I R]"" Q R
 ;get pov
 S X=0 F  S X=$O(^AMHRPRO("AD",V,X)) Q:X'=+X  D
 .S M=$$VAL^XBDIQ1(9002011.01,X,.01)
 .I M="14.1"!(M="V79.0") S R=$$BHRT^AMHRDE1(V,M,"",P,$$VALI^XBDIQ1(9002011.01,X,1204))
 I R]"" Q R
 ;get education
 S X=0 F  S X=$O(^AMHREDU("AD",V,X)) Q:X'=+X  D
 .S M=$$VAL^XBDIQ1(9002011.05,X,.01)
 .I M="DEP-SCR" S R=$$BHRT^AMHRDE1(V,M,"",P,$$VALI^XBDIQ1(9002011.05,X,.04),$P($G(^AMHREDU(V,11)),U,1))
 I R]"" Q R
 Q R  ;
