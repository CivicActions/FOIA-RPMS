AMHGD ; IHS/CMI/MAW - AMHG Get Data for GUI Front End 11/25/2008 9:38:11 AM ; 26 Jun 2017  3:55 PM
 ;;4.0;IHS BEHAVIORAL HEALTH;**1,4,8,11,12**;JUN 02, 2010;Build 46
 ;
 ;
 ;
...
VISITL(RETVAL,AMHSTR) ;-- get visit list for record selector screen
 ...
 N AMHI,AMHP,AMHE,AMHB,P,AMHIVB,AMHIVE,AMHDA,AMHIEN,AMHACT,AMHACTT
 S P="|"
 ...
 S @RETVAL@(AMHI)=@RETVAL@(AMHI)_"^T00001Group"_U_"T00080Program^T00005ActivityTime^T00030DOBI"_$C(30)
 S AMHB=$P(AMHSTR,P)
 S AMHE=$P(AMHSTR,P,2)
 S AMHP=$P(AMHSTR,P,3)
 ...
 .. N AMHDT,AMHPOV,AMHAXV,AMHCLN,AMHACT,AMHVT,AMHCT,AMHPRV,AMHPOVI,AMHPRVI,AMHACTI,AMHEHR,AMHDOBI
 .. N AMHPRVM,AMHPROG
 .. S AMHPRVM=0
 ...
 .. S AMHACTT=$$GET1^DIQ(9002011,AMHIEN,.12)  ;p11 CR04951
 .. S AMHDOBI=$P($G(^DPT(AMHP,0)),U,3)
 .. S AMHI=AMHI+1
 .. S @RETVAL@(AMHI)=AMHIEN_U_AMHDT_U_$$LVDT^AMHGU(AMHDT)_U_AMHPOVE_U_AMHAXV_U_AMHCLN_U_AMHACT_U_AMHVT_U_AMHCT_U_AMHPRV_U_AMHESIG_U_AMHEHR_U_AMHIDEL_U_AMHLOC_U_AMHGRP_U_AMHPROG_U_AMHACTT_$C(30)
 S @RETVAL@(AMHI+1)=$C(31)
 Q
 ;
VISITAP(RETVAL,AMHSTR) ;-- get visit list for record selector screen all patients
 S X="MERR^AMHGU",@^%ZOSF("TRAP") ; m error trap
 N AMHI,AMHP,AMHE,AMHB,P,AMHIVB,AMHIVE,AMHDA,R,AMHIEN,AMHACTT
 S P="|",R="~"
 S RETVAL="^AMHTMP("_$J_")"
 S AMHI=0
 K ^AMHTMP($J)
 ...
 S @RETVAL@(AMHI)=@RETVAL@(AMHI)_"^T00001DeleteIntakes^T00030LocationofEncounter^T00001Group^T00001Spt^T02500Message^T00080Program^T00005ActivityTime^T00030DOBI"_$C(30)
 S AMHB=$P(AMHSTR,P)
 S AMHE=$P(AMHSTR,P,2)
 S AMHP=$P(AMHSTR,P,3)
 ...
 .. N AMHPRVM,AMHSPT,AMHMSG,AMHPROG,AMHDOBI
 .. S AMHPRVM=0
 .. Q:'$$ALLOWVI^AMHUTIL(DUZ,AMHIEN)  ;screen on user and visit
 .. S AMHD0=$G(^AMHREC(AMHIEN,0))
 ...
 .. S AMHDOBI=$$GET1^DIQ(2,AMHPIEN,.03,"I")
 .. S AMHDOB=$$LVDT^AMHGU(AMHDOBI)
 ...
 .. S @RETVAL@(AMHI)=@RETVAL@(AMHI)_U_$G(AMHMSG)_U_AMHPROG_U_AMHACTT_U_AMHDOBI_$C(30)
 S @RETVAL@(AMHI+1)=$C(31)
 Q
 ;
...
TPL(RETVAL,AMHSTR) ;-- get treatment plans for record selector screen
 S X="MERR^AMHGU",@^%ZOSF("TRAP") ; m error trap
 N AMHI,AMHP,AMHE,AMHB,P,AMHIVB,AMHIVE,AMHDA,AMHIEN,AMHCD
 S P="|",R="~"
 S RETVAL="^AMHTMP("_$J_")"
 S AMHI=0
 K ^AMHTMP($J)
 S @RETVAL@(AMHI)="T00010BMXIEN^T00030SortDate^T00030DateEstablished^T00030Program^T00030Status^T00080Problem^T00030Provider^T00030ReviewDate^T00010Reviews^T00030ClosedDate"_$C(30)
 S AMHB=$P(AMHSTR,P)
 S AMHE=$P(AMHSTR,P,2)
 S AMHP=$P(AMHSTR,P,3)
 S AMHIVB=(9999999-AMHB)+.0001
 ...
 .. S AMHCD=$$GET1^DIQ(9002011.56,AMHIEN,.12,"I")  ;p11 CR06841
 .. S AMHI=AMHI+1
 .. S @RETVAL@(AMHI)=AMHIEN_U_AMHDE_U_$$LVDT^AMHGU(AMHDE)_U_AMHPRG_U_AMHST_U_AMHPRB_U_AMHPRV_U_$$LVDT^AMHGU(AMHRD)_U_AMHRCNT_U_$$LVDT^AMHGU(AMHCD)_$C(30)
 S @RETVAL@(AMHI+1)=$C(31)
 Q
 ;
TPLAP(RETVAL,AMHSTR) ;-- get treatment plans for record selector screen all patients
 ...
 S AMHI=0
 K ^AMHTMP($J)
 S @RETVAL@(AMHI)="T00010BMXIEN^T00030SortDate^T00030DateEstablished^T00040Patient^T00010Chart^T00001Sex^T00030DOB^T00030Program^T00030Status^T00080Problem^T00030Provider^T00030ReviewDate^T00010Reviews^T00001Spt^"
 S @RETVAL@(AMHI)=@RETVAL@(AMHI)_"T02500Message^T00007DateClosedInternal^T00030DateClosed_T00030DOBI"_$C(30)
 S AMHB=$P(AMHSTR,P)
 S AMHE=$P(AMHSTR,P,2)
 S AMHP=$P(AMHSTR,P,3)
 S AMHIVB=(9999999-AMHB)+.0001
 ...
 .. N AMHDE,AMHPRG,AMHST,AMHPRB,AMHPRV,AMHRD,AMHPIEN,AMHPAT,AMHDOB,AMHSEX,AMHCHT,AMHDCI,AMHDC
 .. N AMHPRVM,AMHSPT,AMHMSG,AMHDOBI
 .. S AMHPRVM=0
 .. Q:'$$ALLOWTP^AMHLETP(DUZ,AMHIEN)  ;screen on user and treatment plan
 ...
 .. S AMHRD=$$GET1^DIQ(9002011.56,AMHIEN,.09,"I")
 .. S AMHRCNT=+$P($G(^AMHPTXP(AMHIEN,41,0)),U,4)
 .. S AMHDCI=$P($G(^AMHPTXP(AMHIEN,0)),U,12)
 .. I $G(AMHDCI)]"" S AMHDC=$$LVDT^AMHGU(AMHDCI)
 .. S AMHSPT=$$SPT^AMHGDA(AMHPIEN)
 .. I $G(AMHSPT) D
 ... S AMHDOB="**SENSITIVE**"
 ... S AMHMSG=$G(AMHDGMSG)
 .. S AMHI=AMHI+1
 .. S @RETVAL@(AMHI)=AMHIEN_U_AMHDE_U_$$LVDT^AMHGU(AMHDE)_U_AMHPIEN_R_AMHPAT_U_AMHCHT_U_AMHSEX_U_AMHDOB_U_AMHPRG_U_AMHST_U_AMHPRB_U_AMHPRV_U_$$LVDT^AMHGU(AMHRD)_U_AMHRCNT_U_+$G(AMHSPT)_U_$G(AMHMSG)_U_$G(AMHDCI)_U_$G(AMHDC)_U_$G(AMHDOBI)_$C(30)
 S @RETVAL@(AMHI+1)=$C(31)
 Q
 ;
...
SFLAP(RETVAL,AMHSTR) ;-- get suicide forms for record selector screen all patients
 S X="MERR^AMHGU",@^%ZOSF("TRAP") ; m error trap
 N AMHI,AMHP,AMHE,AMHB,P,AMHIVB,AMHIVE,AMHDA,AMHIEN,AMHDOBI
 S P="|",R="~"
 S RETVAL="^AMHTMP("_$J_")"
 S AMHI=0
 K ^AMHTMP($J)
 S @RETVAL@(AMHI)="T00010BMXIEN^T00030SortDate^T00030Date^T00040Patient^T00010Chart^T00001Sex^T00030DOB^T00030LocalCaseNumber^T00030Provider^T00080SuicidalBehavior^T00001Spt^T02500Message^T00001Incomplete^T00030DOBI"_$C(30)
 S AMHB=$P(AMHSTR,P)
 S AMHE=$P(AMHSTR,P,2)
 S AMHP=$P(AMHSTR,P,3)
 S AMHIVB=(9999999-AMHB)+.0001
 ...
 ... S AMHMSG=$G(AMHDGMSG)
 .. S AMHI=AMHI+1
 .. S @RETVAL@(AMHI)=AMHIEN_U_AMHD_U_$$LVDT^AMHGU(AMHD)_U_AMHPIEN_R_AMHPAT_U_AMHCHT_U_AMHSEX_U_AMHDOB_U_AMHLCN_U_AMHPRV_U_AMHSB_U_+$G(AMHSPT)_U_$G(AMHMSG)_U_AMHINC_U_AMHDOBI_$C(30)
 S @RETVAL@(AMHI+1)=$C(31)
 Q
 ;
