AMHRSS5P ; IHS/CMI/LAB - list refusals ;
...
PRINT1 ;
 ...
 I '$D(^XTMP("AMHRSS5",AMHRJ,AMHRH)) D HEADER W !!,"No data to report.",! G DONE
 D HEADER
 S AMHRTOT=AMHRCNT
 S AMHRPTOT=$$PTOT
...
RES ;
 ...
 K AMHRRES S X=0 F  S X=$O(^XTMP("AMHRSS5",AMHRJ,AMHRH,"VSTS",X)) Q:X'=+X  D
 .S AMHRY=^XTMP("AMHRSS5",AMHRJ,AMHRH,"VSTS",X)
...
MH ;
 Q:'$D(AMHRTALL(8))
 K AMHRRES S X=0 F  S X=$O(^XTMP("AMHRSS5",AMHRJ,AMHRH,"PTS",X)) Q:X'=+X  D
 .S D=$O(^XTMP("AMHRSS5",AMHRJ,AMHRH,"PTS",X,""),-1) S AMHRY=^XTMP("AMHRSS5",AMHRJ,AMHRH,"PTS",X,D)
...
DATE ;
 ...
 K ^TMP($J) S X=0 F  S X=$O(^XTMP("AMHRSS5",AMHRJ,AMHRH,"VSTS",X)) Q:X'=+X  D
 .S AMHRY=^XTMP("AMHRSS5",AMHRJ,AMHRH,"VSTS",X)
...
PTOT() ;
 NEW C,X
 S C=0
 S X=0 F  S X=$O(^XTMP("AMHRSS5",AMHRJ,AMHRH,"PTS",X)) Q:X'=+X  S C=C+1
 Q C
TOT() ;
 NEW C,X
 S C=0
 S X=0 F  S X=$O(^XTMP("AMHRSS5",AMHRJ,AMHRH,"VSTS",X)) Q:X'=+X  S C=C+1
 Q C
HEADER ;EP
 G:'AMHRPG HEADER1
...
HEADER1 ;
 ...
 W !,$$CTR("***  SUICIDE SCREENING VISIT TALLY"_$S(AMHRLIST:" AND VISIT LISTING",1:"")_"  ***",80),!
...
LIST ;EP - called from xbdbque
 ...
 K ^TMP($J)
 ;resort by sort item
 S AMHRX=0 F  S AMHRX=$O(^XTMP("AMHRSS5",AMHRJ,AMHRH,"VSTS",AMHRX)) Q:AMHRX'=+AMHRX  S AMHRY=^XTMP("AMHRSS5",AMHRJ,AMHRH,"VSTS",AMHRX) D
 .S DFN=$P(AMHRY,U,9)
 .D @AMHRSORT
 ...
 .Q:$D(^XTMP("AMHRSS5",AMHRJ,AMHRH,"PTS",DFN))
 .Q:$$END
