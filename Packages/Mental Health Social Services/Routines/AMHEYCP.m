AMHEYCP ; IHS/CMI/LAB - PRNT RECORD REVIEW ;
 ;;3.0;IHS BEHAVIORAL HEALTH;**4,7**;JAN 27, 2003
 ;
START ;
 S AMH80E="==============================================================================="
 S AMH80D="-------------------------------------------------------------------------------"
 S Y=AMHBD D DD^%DT S AMHBDD=Y S Y=AMHED D DD^%DT S AMHEDD=Y
 S AMHPG=0 D HEAD I '$D(^XTMP("AMHEYC",AMHJOB,AMHBT)) W !,"No errors to report",! G DONE
 S AMHR=0 K AMHQUIT
 F  S AMHR=$O(^XTMP("AMHEYC",AMHJOB,AMHBT,"ERRORS",AMHR)) Q:AMHR=""!($D(AMHQUIT))  S AMHREC=^AMHREC(AMHR,0) D PROC
 G:$D(AMHQUIT) DONE
 I $Y>(IOSL-6) D HEAD G:$D(AMHQUIT) DONE
DONE ;
 D DONE^AMHLEIN
 K ^XTMP("AMHEYC",AMHJOB,AMHBT)
 Q
PROC ;
 ...
 K ^UTILITY("DIQ1",$J)
 W !!,AMHDATE,?22,AMHNAME,?43,AMHHRCN,?52,AMHPROG,?56,AMHCAT,?74,AMHACT,!,^XTMP("AMHEYC",AMHJOB,AMHBT,"ERRORS",AMHR)
 Q
CHKDISC ;
 Q:'$D(^VA(200,AMHAP))
