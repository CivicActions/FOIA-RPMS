AMHRAS31 ; IHS/CMI/LAB - list ALCOHOL screenings ; 24 Apr 2023  3:00 PM
...
PROC ;
 ...
 D XTMP^AMHUTIL("AMHRAS3","ALCOHOL SCREENING REPORT")
 ...
 .S AMHREFS="" I $D(AMHRREST(3)) S AMHREFS="" I AMHREXPC S AMHREFS=$$REFUSAL(DFN,9999999.15,$O(^AUTTEXAM("C",35,0)),AMHRBD,AMHRED)
 .I $P(AMHREFS,U,1)>$P(AMHALSC,U,1) S AMHALSC=AMHREFS,AMHPFI="PCC"
 .I AMHALSC="" Q  ;no screenings
 .S ^XTMP("AMHRAS3",AMHRJ,AMHRH,"PTS",DFN)=AMHALSC,$P(^XTMP("AMHRAS3",AMHRJ,AMHRH,"PTS",DFN),U,20)=AMHPFI
 .S AMHRCNT=AMHRCNT+1
 .Q
 Q
BHALCS(P,BDATE,EDATE) ;
 ...
 ....I M="AUDT",E="",'$D(AMHRREST(6)) Q
 ....I M="AUDT",E<8,'$D(AMHRREST(1)) Q
 ....I M="AUDT",E>7,'$D(AMHRREST(2)) Q
 ....I M="CRFT",E="",'$D(AMHRREST(6)) Q
 ....I M="CRFT",E<2,'$D(AMHRREST(1)) Q
 ....I M="CRFT",E>1,'$D(AMHRREST(2)) Q
 ....I M="AUDC",E="",'$D(AMHRREST(6)) Q
 ....I M="AUDC",E<4,$$SEX^AUPNPAT(P)="M",'$D(AMHRREST(1)) Q
 ....I M="AUDC",E<3,$$SEX^AUPNPAT(P)="F",'$D(AMHRREST(1)) Q
 ....I M="AUDC",E>3,$$SEX^AUPNPAT(P)="M",'$D(AMHRREST(2)) Q
 ....I M="AUDC",E>2,$$SEX^AUPNPAT(P)="F",'$D(AMHRREST(2)) Q
 ...
 ..I AMHRRES["REFERRAL",'$D(AMHRREST(5)) G BHHF
...
BHHF ..;
 ...
 ...I $P(^AUTTHF($P(^AUTTHF(M,0),U,3),0),U,2)="C001"!($P(^AUTTHF($P(^AUTTHF(M,0),U,3),0),U,2)="C036")!($P(^AUTTHF($P(^AUTTHF(M,0),U,3),0),U,2)="C029") D
 ...
 ....I AMHRSPUN,E'="" Q  ;want only unknown and this one has a SCR provider
 ....;check result
 ....I $$VAL^XBDIQ1(9002011.08,X,.01)="CAGE 0/4",'$D(AMHRREST(1)) Q
 ....I $$VAL^XBDIQ1(9002011.08,X,.01)'="CAGE 0/4",'$D(AMHRREST(2)) Q
 ....I $$VAL^XBDIQ1(9002011.08,X,.01)="CAGE-AID 0/4",'$D(AMHRREST(1)) Q
 ....I $$VAL^XBDIQ1(9002011.08,X,.01)'="CAGE-AID 0/4",'$D(AMHRREST(2)) Q
 ....I $$VAL^XBDIQ1(9002011.08,X,.01)="ALCOHOL-MINIMAL RISK",'$D(AMHRREST(1)) Q
 ....I $$VAL^XBDIQ1(9002011.08,X,.01)'="ALCOHOL-MINIMAL RISK",'$D(AMHRREST(2)) Q
 ...
 ..I R]"" Q
 ..I $D(AMHRREST(1)) S X=0 F  S X=$O(^AMHRPROC("AD",V,X)) Q:X'=+X!(R]"")  D
 ...S M=$$VALI^XBDIQ1(9002011.04,X,.01)
 ...Q:'$$ICD^ATXCHK(M,$O(^ATXAX("B","BGP ALCOHOL SCREENING CPTS",0)),1)
 ...Q:$$ICD^ATXCHK(M,$O(^ATXAX("B","BGP ALCOHOL POSITIVE SCRN CPTS",0)),1)
 ...S R=$$BHRT(V,"CPT: "_$$VAL^XBDIQ1(9002011.04,X,.01),"",P,"")
 ..I R]"" Q
 ..I $D(AMHRREST(2)) S X=0 F  S X=$O(^AMHRPROC("AD",V,X)) Q:X'=+X!(R]"")  D
 ...S M=$$VALI^XBDIQ1(9002011.04,X,.01)
 ...Q:'$$ICD^ATXCHK(M,$O(^ATXAX("B","BGP ALCOHOL SCREENING CPTS",0)),1)
 ...Q:'$$ICD^ATXCHK(M,$O(^ATXAX("B","BGP ALCOHOL POSITIVE SCRN CPTS",0)),1)
 ...S R=$$BHRT(V,"CPT: "_$$VAL^XBDIQ1(9002011.04,X,.01),"",P,"")
...
PCCSCR(V) ;is there a screening?  return in R
 ...
 ..I AMHRSPUN,E'="" Q  ;want only unknown and this one has a SCR provider
 ..;check result
 ..I $$VAL^XBDIQ1(9000010.23,X,.01)="CAGE 0/1",'$D(AMHRREST(1)) Q
 ..I $$VAL^XBDIQ1(9000010.23,X,.01)'="CAGE 0/1",'$D(AMHRREST(2)) Q
 ...
 I R]"" Q R
 ;get CPTs
 I $D(AMHRREST(6)) S X=0 F  S X=$O(^AUPNVCPT("AD",V,X)) Q:X'=+X  D
 .S M=$$VALI^XBDIQ1(9000010.18,X,.01)
 .Q:'$$ICD^ATXCHK(M,$O(^ATXAX("B","BGP ALCOHOL SCREENING CPTS",0)),1)
 .S T=D_U_$$VAL^XBDIQ1(9000010.18,X,.01)_U_U_V_U_9000010.18_U_X
 .S R=$$PCCV^AMHRAS1(T,P)
 I R]"" Q R
 Q R
 ;
