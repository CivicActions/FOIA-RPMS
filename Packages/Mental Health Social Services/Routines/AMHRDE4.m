AMHRDE4 ; IHS/CMI/LAB - list refusals ;
...
ZIS ;
 ...
 S XBRP="PRINT^AMHRDE4P",XBRC="PROC^AMHRDE4",XBRX="XIT^AMHRDE4",XBNS="AMH"
 D ^XBDBQUE
 D XIT
 Q
BROWSE ;
 S XBRP="VIEWR^XBLM(""^AMHRDE4P"")"
 S XBNS="AMH",XBRC="PROC^AMHRDE4",XBRX="XIT^AMHRDE4",XBIOP=0 D ^XBDBQUE
 Q
XIT ;
 D EN^XBVK("AMHR")
 D ^XBFMK
...
PROC ;
 S AMHRCNT=0
 S AMHRH=$H,AMHRJ=$J
 K ^XTMP("AMHRDE4",AMHRJ,AMHRH)
 D XTMP^AMHUTIL("AMHRDE4","DEPRESSION SCREENING REPORT")
 ;now go through BH
 ...
 .S AMHALSC=$$BHALCS^AMHRDE1(DFN,AMHRBD,AMHRED),AMHPFI="BH"  ;include refusals
 .S AMHPCALS="" I AMHREXPC S AMHPCALS=$$PCCALCS^AMHRDE1(DFN,AMHRBD,AMHRED)  ;include refusals
 .I $P(AMHPCALS,U,1)>$P(AMHALSC,U,1) S AMHALSC=AMHPCALS,AMHPFI="PCC"
 .S AMHREFS="" I AMHREXPC S AMHREFS=$$REFUSAL^AMHRDE1(DFN,9999999.15,$O(^AUTTEXAM("C",36,0)),AMHRBD,AMHRED)
 .I $P(AMHREFS,U,1)>$P(AMHALSC,U,1) S AMHALSC=AMHREFS,AMHPFI="PCC"
 .I AMHALSC="" Q  ;no screenings
 .S ^XTMP("AMHRDE4",AMHRJ,AMHRH,"PTS",DFN)=AMHALSC,$P(^XTMP("AMHRDE4",AMHRJ,AMHRH,"PTS",DFN),U,20)=AMHPFI
 Q
 ;
