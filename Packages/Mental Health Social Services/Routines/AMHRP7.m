AMHRP7 ; IHS/CMI/LAB - VISIT COUNTS BY PROVIDER ; [ 02/21/2007  3:57 PM ]
 ;;3.0;IHS BEHAVIORAL HEALTH;**1,2,7,8,10**;JAN 27, 2003
...
PROG ;select program to run report for
 S AMHPROG=""
 S DIR(0)="S^M:MENTAL HEALTH;S:SOCIAL SERVICES;C:CHEMICAL DEPENDENCY or ALCOHOL/SUBSTANCE ABUSE;O:OTHER;T:OTHER (2)",DIR("A")="Which DESIGNATED PROVIDER",DIR("B")="M" D ^DIR K DIR S:$D(DUOUT) DIRUT=1
 I $D(DIRUT) D END Q
...
FT ;
 S AMHPRV=""
 S DIR(0)="F^1:30",DIR("A")="Enter the OTHER (2) Designated Provider's Name" KILL DA D ^DIR KILL DIR
 I $D(DIRUT) Q
 S AMHPRV=Y
 Q
BROWSE ;
