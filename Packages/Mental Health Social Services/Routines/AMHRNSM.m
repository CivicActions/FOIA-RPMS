AMHRNSM ; IHS/CMI/LAB - report of a patient's no show visits ;
 ;;4.0;IHS BEHAVIORAL HEALTH;**11**;JUN 02, 2010;Build 27
 ;
 ;
START ;EP - called from option
 D XIT
 W:$D(IOF) @IOF
 S X="BEHAVIORAL HEALTH NO SHOW REPORT FOR MULTIPLE PATIENTS" W !,$$CTR^AMHLEIN(X,80)
 W !!,"This option will print a list of visits with POVs related to No Shows"
 W !,"and Cancellations for multiple patients.  Date range, program and"
 W !,"provider can be specified by the user.",!!
 D DBHUSR^AMHUTIL
GETDATES ;
 W !!,"Please enter the range of dates for the No Show visits"
BD ;get beginning date
 S DIR(0)="D^:DT:EP",DIR("A")="Enter Beginning Date" D ^DIR K DIR S:$D(DUOUT) DIRUT=1
 I $D(DIRUT) G XIT
 S AMHBD=Y
ED ;get ending date
 W ! S DIR(0)="D^"_AMHBD_":DT:EP",DIR("A")="Enter Ending Date" S Y=AMHBD D DD^%DT D ^DIR K DIR S:$D(DUOUT) DIRUT=1
 I $D(DIRUT) G BD
 S AMHED=Y
 S X1=AMHBD,X2=-1 D C^%DTC S AMHSD=X S Y=AMHBD D DD^%DT S AMHBDD=Y S Y=AMHED D DD^%DT S AMHEDD=Y
PROG ;
 S AMHPROG=""
 S DIR(0)="S^O:ONE Program;A:ALL Programs",DIR("A")="Run the Report for which PROGRAM",DIR("B")="A" KILL DA D ^DIR KILL DIR
 G:$D(DIRUT) GETDATES
 I Y="A" G PROV
 S DIR(0)="9002011,.02",DIR("A")="Which PROGRAM" KILL DA D ^DIR KILL DIR
 G:$D(DIRUT) PROG
 I X="" G PROG
 S AMHPROG=Y
PROV ;
 S AMHPROV=""
 S DIR(0)="S^A:All Providers;O:One Provider",DIR("A")="Include visits for",DIR("B")="A" K DA D ^DIR K DIR
 G:$D(DIRUT) PROG
 I Y="A" G SORT
 S DIC="^VA(200,",DIC(0)="AEMQ",DIC("A")="Which PROVIDER: " D ^DIC
 K DIC,DA
 I Y=-1 G PROV
 S AMHPROV=+Y
SORT ;
 S AMHSORT=""
 S DIR(0)="S^P:Patient Name;D:Date of Visit",DIR("A")="How would you like the report sorted",DIR("B")="P" KILL DA D ^DIR KILL DIR
 G:$D(DIRUT) PROV
 S AMHSORT=Y
DEMO ;
 D DEMOCHK^AMHUTIL1(.AMHDEMO)
 I AMHDEMO=-1 G SORT
ZIS ;
 S DIR(0)="S^P:PRINT Output;B:BROWSE Output on Screen",DIR("A")="Do you wish to ",DIR("B")="P" K DA D ^DIR K DIR
 I $D(DIRUT) G XIT
 I $G(Y)="B" D BROWSE,XIT Q
 W !! S XBRP="PRINT^AMHRNSM",XBRC="PROC^AMHRNSM",XBNS="AMH*",XBRX="XIT^AMHRNSM"
 D ^XBDBQUE
 D XIT
 Q
BROWSE ;
 S XBRP="VIEWR^XBLM(""PRINT^AMHRNSM"")"
 S XBNS="AMH",XBRC="PROC^AMHRNSM",XBRX="XIT^AMHRNSM",XBIOP=0 D ^XBDBQUE
 Q
 ;
PROC ;EP - called from xbdbque
 ;loop through all visits in date range and look for problem 8-8.9 or visit type of N
 S AMHJOB=$J,AMHTOT=0,AMHBT=$H,AMHPTOT=0
 D XTMP^AMHUTIL("AMHRNSM","BH - NO SHOW VISITS")
 S X1=AMHBD,X2=-1 D C^%DTC S AMHSD=X
 S AMHODAT=AMHSD_".9999" F  S AMHODAT=$O(^AMHREC("B",AMHODAT)) Q:AMHODAT=""!((AMHODAT\1)>AMHED)  D
 .S AMHR=0 F  S AMHR=$O(^AMHREC("B",AMHODAT,AMHR)) Q:AMHR'=+AMHR  D
 ..I AMHPROG]"",$P(^AMHREC(AMHR,0),U,2)'=AMHPROG Q
 ..I AMHPROV,$$PPINT^AMHUTIL(AMHR)'=AMHPROV Q
 ..Q:'$$ALLOWVI^AMHUTIL(DUZ,AMHR)
 ..S DFN=$P(^AMHREC(AMHR,0),U,8) Q:'DFN
 ..Q:'$$ALLOWP^AMHUTIL(DUZ,DFN)
 ..Q:$$DEMO^AMHUTIL1(DFN,$G(AMHDEMO))
 ..;if any dx is 8 then do set and q
 ..S (AMHX,AMHDNKA)=0 F  S AMHX=$O(^AMHRPRO("AD",AMHR,AMHX)) Q:AMHX'=+AMHX!(AMHDNKA)  D
 ...I $P(^AMHPROB($P(^AMHRPRO(AMHX,0),U),0),U)=8 S AMHDNKA=1 D SET Q  ;do not pass dnka
 ...I $P(^AMHPROB($P(^AMHRPRO(AMHX,0),U),0),U)=8.1 S AMHDNKA=1 D SET Q  ;do not pass dnka
 ...I $P(^AMHPROB($P(^AMHRPRO(AMHX,0),U),0),U)=8.11 S AMHDNKA=1 D SET Q  ;do not pass dnka
 ...I $P(^AMHPROB($P(^AMHRPRO(AMHX,0),U),0),U)=8.2 S AMHDNKA=1 D SET Q  ;do not pass dnka
 ...I $P(^AMHPROB($P(^AMHRPRO(AMHX,0),U),0),U)=8.21 S AMHDNKA=1 D SET Q
 ...I $P(^AMHPROB($P(^AMHRPRO(AMHX,0),U),0),U)=8.3 S AMHDNKA=1 D SET Q  ;do not pass dnka
 ...I $P(^AMHREC(AMHR,0),U,33)="N" D SET Q
 ...Q
 ..Q
 .Q
 Q
SET ;
 S ^XTMP("AMHRNSM",AMHJOB,AMHBT,"NO SHOWS",$S(AMHSORT="P":$P(^DPT(DFN,0),U),1:(9999999-$P(AMHODAT,"."))),AMHR)="",AMHTOT=AMHTOT+1
 I $D(^XTMP("AMHRNSM",AMHJOB,AMHBT,"PATIENTS",DFN)) Q
 S AMHPTOT=AMHPTOT+1
 S ^XTMP("AMHRNSM",AMHJOB,AMHBT,"PATIENTS",DFN)=""
 Q
PRINT ;EP - called from xbdbque
 D PRINT1
 K ^XTMP("AMHRNSM",AMHJOB,AMHBT)
 Q
PRINT1 ;
 K AMHQ S AMHPG=0
 I '$D(^XTMP("AMHRNSM",AMHJOB,AMHBT,"NO SHOWS")) D HEADER W !!,"There were no NO SHOW visits during the time period.",! Q
 D HEADER
 S AMHSORT="" F  S AMHSORT=$O(^XTMP("AMHRNSM",AMHJOB,AMHBT,"NO SHOWS",AMHSORT)) Q:AMHSORT=""!($D(AMHQ))  D
 .S AMHR=0 F  S AMHR=$O(^XTMP("AMHRNSM",AMHJOB,AMHBT,"NO SHOWS",AMHSORT,AMHR)) Q:AMHR'=+AMHR!($D(AMHQ))  D
 ..I $Y>(IOSL-4) D HEADER Q:$D(AMHQ)
 ..S DFN=$P(^AMHREC(AMHR,0),U,8)
 ..W !,$E($P(^DPT(DFN,0),U),1,18),?19,$$HRN^AUPNPAT(DFN,DUZ(2)),?26,$$FMTE^XLFDT($P(^AMHREC(AMHR,0),U)),?45,$E($$PPNAME^AMHUTIL(AMHR),1,10)
 ..S X=$$GETPREF^AUPNSOGI(DFN,"I",1) I X]"" W !,"Preferred Name: ",X
 ..W ?56,$E($$VAL^XBDIQ1(9002011,AMHR,.02),1,1),?58,$$PRIMPOV^AMHUTIL1(AMHR,"C")_"-"
 ..S X=$$PRIMPOV^AMHUTIL1(AMHR,"I") I X W $E($P(^AMHPROB(X,0),U,2),1,10)
 ..S X=$$VALI^XBDIQ1(9002011,AMHR,.34) W ?76,$S(X:"GRP",1:"IND")
 .Q
 Q:$D(AMHQ)
 I $Y>(IOSL-4) D HEADER Q:$D(AMHQ)
 W !!,"Total # of Patients: ",AMHPTOT,"     Total # of No Show Visits: ",AMHTOT,!
 Q
XIT ;
 D EN^XBVK("AMH")
 D KILL^AUPNPAT
 Q
CTR(X,Y) ;EP - Center X in a field Y wide.
 Q $J("",$S($D(Y):Y,1:IOM)-$L(X)\2)_X
 ;----------
HEADER ;EP
 I 'AMHPG G HEADER1
 I $E(IOST)="C",IO=IO(0) W ! S DIR(0)="EO" D ^DIR K DIR I Y=0!(Y="^")!($D(DTOUT)) S AMHQ="" Q
HEADER1 ;
 W:$D(IOF) @IOF S AMHPG=AMHPG+1
 W !?13,"********** CONFIDENTIAL PATIENT INFORMATION **********"
 W !?(80-$L($P(^DIC(4,DUZ(2),0),U))/2),$P(^DIC(4,DUZ(2),0),U),?($S(80=132:120,1:72)),"Page ",AMHPG,!
 S AMHTEXT="BEHAVIORAL HEALTH NO SHOW APPOINTMENT LISTING"
 W !?(80-$L(AMHTEXT)/2),AMHTEXT,!
 S AMHTEXT="Appointment Dates:  "_AMHBDD_" and "_AMHEDD
 W ?(80-$L(AMHTEXT)/2),AMHTEXT,!
 I AMHPROG]"" S X="Program: "_$$EXTSET^XBFUNC(9002011,.02,AMHPROG) W $$CTR^AMHLEIN(X,80),!
 I AMHPROV S X="Provider: "_$P(^VA(200,AMHPROV,0),U) W $$CTR^AMHLEIN(X,80),!
 W !,"PATIENT NAME",?19,"HRN",?26,"DATE/TIME",?45,"PROVIDER",?55,"PG",?58,"POV",?76,"I/G"
 W !,$TR($J(" ",80)," ","-")
 Q
