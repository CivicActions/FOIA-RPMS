AMHRDE4P ; IHS/CMI/LAB - list refusals ;
...
PRINT1 ;
 ...
 I '$D(^XTMP("AMHRDE4",AMHRJ,AMHRH)) D HEADER W !!,"No data to report.",! G DONE
 D HEADER
 S AMHRPTOT=$$PTOT
...
RES ;
 ...
 K AMHRRES S X=0 F  S X=$O(^XTMP("AMHRDE4",AMHRJ,AMHRH,"PTS",X)) Q:X'=+X  D
 .S AMHRY=^XTMP("AMHRDE4",AMHRJ,AMHRH,"PTS",X)  ;S D=$O(^XTMP("AMHRDE4",AMHRJ,AMHRH,"PTS",X,""),-1) S AMHRY=^XTMP("AMHRDE4",AMHRJ,AMHRH,"PTS",X,D)
...
DATE ;
 ...
 K ^TMP($J) S X=0 F  S X=$O(^XTMP("AMHRDE4",AMHRJ,AMHRH,"PTS",X)) Q:X'=+X  D
 .S AMHRY=^XTMP("AMHRDE4",AMHRJ,AMHRH,"PTS",X)  ;S D=$O(^XTMP("AMHRDE4",AMHRJ,AMHRH,"PTS",X,""),-1) S AMHRY=^XTMP("AMHRDE4",AMHRJ,AMHRH,"PTS",X,D)
...
TOT() ;
 NEW C,X
 S C=0
 S X=0 F  S X=$O(^XTMP("AMHRDE4",AMHRJ,AMHRH,"PTS",X)) Q:X'=+X  S C=C+1
 Q C
PTOT() ;
 NEW C,X
 S C=0
...
LIST ;EP - called from xbdbque
 ...
 K ^TMP($J)
 ;resort by sort item
 S DFN=0 F  S DFN=$O(^XTMP("AMHRDE4",AMHRJ,AMHRH,"PTS",DFN)) Q:DFN'=+DFN  S AMHRY=^XTMP("AMHRDE4",AMHRJ,AMHRH,"PTS",DFN) D
 .D @AMHRSORT
 ...
 .Q:$D(^XTMP("AMHRDE4",AMHRJ,AMHRH,"PTS",DFN))
 .Q:$$END
