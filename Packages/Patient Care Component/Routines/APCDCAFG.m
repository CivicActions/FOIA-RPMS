...
SUB ;
 ...
 S APCDVSTD=Y
ZIS ;call xbdbque
 S XBRC="DRIVER^APCDCAFG",XBRP="PRINT^APCDCAFG",XBRX="XIT^APCDCAFG",XBNS="APCD"
 D ^XBDBQUE
 D XIT
 Q
DRIVER ;EP entry point for taskman
 S APCDBT=$H,APCDJOB=$J
 K ^XTMP("APCDCAFG",APCDJOB,APCDBT)
 S ^XTMP("APCDCAFG",0)=$$FMADD^XLFDT(DT,14)_"^"_DT_"^"_"CODING QUEUE TALLY"
...
V ;
 ...
 ..S ^TMP($J,"VISITS",APCDVIEN)=""
 ;NOW GO THROUGH AND GET LAST AUPNVCA FOR EACH OF THESE VISITS AND TALLY
 S APCDVIEN=0 F  S APCDVIEN=$O(^TMP($J,"VISITS",APCDVIEN)) Q:APCDVIEN=""  D
 .S APCDVCAI=$$LASTVCA(APCDVIEN)
 .Q:'APCDVCAI
...
LASTVCA(V) ;
 NEW X,Y
 S X=0 F  S X=$O(^AUPNVCA("AD",APCDVIEN,X)) Q:X'=+X  S Y=X
 Q Y
SET(R,D,P) ;
 S $P(^XTMP("APCDCAFG",APCDJOB,APCDBT,R,D),U,P)=$P($G(^XTMP("APCDCAFG",APCDJOB,APCDBT,R,D)),U,P)+1
 S $P(^XTMP("APCDCAFG",APCDJOB,APCDBT,R),U,P)=$P($G(^XTMP("APCDCAFG",APCDJOB,APCDBT,R)),U,P)+1
 Q
PRINT ;EP
 S APCDQUIT="",APCDPG=0,APCDTOTV=0,APCDTOTR=0 D HDR
 I '$D(^XTMP("APCDCAFG",APCDJOB,APCDBT)) D HDR W !!,"NO DATA TO REPORT",! G DONE
 S APCDU="" F  S APCDU=$O(^XTMP("APCDCAFG",APCDJOB,APCDBT,APCDU)) Q:APCDU=""!(APCDQUIT)  D
 . I $Y>(IOSL-3) D HDR Q:APCDQUIT
 . W !!,$P(^VA(200,APCDU,0),U),?40,$$C($P(^XTMP("APCDCAFG",APCDJOB,APCDBT,APCDU),U),0),?55,$$C($P(^XTMP("APCDCAFG",APCDJOB,APCDBT,APCDU),U,2),0)
 . S APCDTOTV=APCDTOTV+$P(^XTMP("APCDCAFG",APCDJOB,APCDBT,APCDU),U,1),APCDTOTR=APCDTOTR+$P(^XTMP("APCDCAFG",APCDJOB,APCDBT,APCDU),U,2)
 . Q:'APCDVSTD
 . S APCDD=0 F  S APCDD=$O(^XTMP("APCDCAFG",APCDJOB,APCDBT,APCDU,APCDD)) Q:APCDD'=+APCDD!(APCDQUIT)  D
 .. I $Y>(IOSL-3) D HDR Q:APCDQUIT
 .. W !?10,$$FMTE^XLFDT(APCDD),?40,$$C($P(^XTMP("APCDCAFG",APCDJOB,APCDBT,APCDU,APCDD),U),0),?55,$$C($P(^XTMP("APCDCAFG",APCDJOB,APCDBT,APCDU,APCDD),U,2),0)
 .Q
 I APCDQUIT G DONE
 I $Y>(IOSL-3) D HDR G:APCDQUIT DONE
 W !!,"Total Number of Visits:  ",?40,$$C(APCDTOTV,0),?55,$$C(APCDTOTR,0),!
DONE ;
 K ^XTMP("APCDCAFG",APCDJOB,APCDBT),APCDJ,APCDBT
...
HDR1 ;
 ...
 W !,$$CTR("*             COUNTS LAST REVIEW ONLY                *",80)
 W !,$$CTR("******************************************************",80)
...
INFORM ;inform user what this report is all about
 ...
 W !,"were reviewed.",!!
 W !,"NOTE:  Only the LAST user who reviewed the visit by marking it either"
 W !,"complete or incomplete will be counted in this report."
 W !
 Q
OLOC ;one location
