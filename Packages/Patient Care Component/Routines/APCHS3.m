APCHS3 ; IHS/TUCSON/LAB - PART 3 OF APCHS -- SUMMARY PRODUCTION COMPONENTS ;  [ 06/24/97  2:42 PM ]
...
LBLD K APCHSLRT,APCHSLDT,APCHSLD2
 ...
 Q
LDATE S APCHSLT="" F APCHSQ=0:0 S APCHSLT=$O(^AUPNVLAB("AE",APCHSPAT,APCHSIVD,APCHSLT)) Q:'APCHSLT  S APCHSDFN=$O(^(APCHSLT,"")) D LSET
 Q
LSET ;
 ...
 S ^TMP($J,"APCHSLRT",APCHSLTO,APCHSIVD)=APCHSLR S APCHSLTX=$P(^LAB(60,APCHSLT,0),U,1) S:$L(APCHSLTX)>APCHSMXL APCHSMXL=$L(APCHSLTX)
 S:'$D(^TMP($J,"APCHSLDT",APCHSIVD)) APCHSNDT=APCHSNDT+1 S ^TMP($J,"APCHSLDT",APCHSIVD)=""
 Q
 ; <PRINT>
LPRT S APCHSLW=APCHSMXL+1,APCHSLL=10,APCHSNMX=(80-1-APCHSLW)\APCHSLL
