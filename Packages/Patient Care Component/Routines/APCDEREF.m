APCDEREF ; IHS/CMI/LAB - prompt for refusal value ; 23 May 2013  10:50 AM
 ;;2.0;IHS PCC SUITE;**11,17,23,29**;MAY 14, 2009;Build 20
 ;
IN ;EP
 ...
 S APCDT09=$G(APCDVSIT)
 I 'APCDT02 W !!,"Patient missing..." Q
 D EN^XBNEW("IN1^APCDEREF","APCDTREF;APCDT02;APCDT09")
 Q
...
CENTRY ;
 ...
 S X=APCDT01,DIC="^AUPNPREF(",DIC(0)="L",DIC("DR")=".02////"_APCDT02_";.09////"_APCDT09 K DD,DO,D0 D FILE^DICN K DIC
 I Y=-1 W !,"ERROR CREATING ENTRY....TRY AGAIN..." Q
 S APCDTDA=+Y
CENTRYE ;now edit it
 ;FIRST WIPE OUT FIELDS
 S APCDTDD=$$VD^APCLV(APCDT09)
...
MOD(APCDP) ;EP
 ...
 I %="UAS" S APCDTRTD="U"
 I %="PRD" S APCDTRTD="P"
 D EN^XBNEW("MOD1^APCDEREF","APCDP;APCDTIEN;APCDTRTD;APCDT09")
...
MOD1 ;EP called from xbnew
 ...
 F  S APCDX=$O(^AUPNPREF("AC",APCDP,APCDX)) Q:APCDX'=+APCDX  I $$VALI^XBDIQ1(9000022,APCDX,.07)=APCDTRTD S APCDC=APCDC+1,APCDD(APCDC)=APCDX D
 .W !?2,APCDC,")",?6,$E($$VAL^XBDIQ1(9000022,APCDX,.01),1,15),?22,$$VAL^XBDIQ1(9000022,APCDX,.04),?68,$$VAL^XBDIQ1(9000022,APCDX,.03),!?6,"Refusal Type: ",$$VAL^XBDIQ1(9000022,APCDX,.07)
 .I $$VAL^XBDIQ1(9000022,APCDX,.09)]"" W ?50,"VISIT: ",$$VAL^XBDIQ1(9000022,APCDX,.09)
...
MOD2 ;EP called from input template
 D EN^XBNEW("VALUE^APCDEREF","APCDTMOD;APCDTDA;APCDT01;APCDT09;APCDTREF;APCDTOLD")
 Q
