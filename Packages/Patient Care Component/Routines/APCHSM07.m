APCHSM07 ; IHS/CMI/LAB -- CONTINUATION OF ROUTINES ;
 ;;2.0;IHS PCC SUITE;**2,16,23**;MAY 14, 2009;Build 17
 ;
...
SIGMOID ;EP
 ...
 I $$IPLSNO^APCHSMU1(APCHSPAT,"PXRM COLORECTAL CANCER") X APCHSURX Q  ;cancel if pt has snomed on pl
 ;PATCH 23 ADDED NOT MEDICALLY INDICATED REFUSAL FOR COLONOSCOPY CPT OR PROC
 I $$NMICOLO(APCHSPAT,$$DOB^AUPNPAT(DFN),DT) X APCHSURX Q  ;NMI COLONOSCOPY AT ANY TIME
 ...
 S APCHLCTI=$$LASTCTCO^APCLAPI5(APCHSPAT,,,"A")
 S APCHLCT=$P(APCHLCTI,U,1)
 ...
 I APCHLCT>APCHLAST S APCHLAST=APCHLCT,APCHICAR=APCHLCTI
 ...
 I APCHLCT]"",$$FMDIFF^XLFDT(DT,APCHLCT)>(5*365) S APCHLCT="",APCHLCTI=""
 ...
 I APCHLCT]"" S APCHNUMD=APCHNUMD+1
...
SIGMOIDR ;
 ...
 .Q
 I APCHNUMD=1 D  Q
 .S APCHTEST=$S(APCHLSIG]"":"FLEXIBLE SIG",APCHLCOL]"":"COLONOSCOPY",1:"CT COLONOGRAPHY")
 .S APCHSINT=$S(APCHLSIG]""!(APCHLCT]""):(5*365),1:(10*365))
...
SIGREF ;
 ...
 S APCHT=$O(^ATXAX("B","BGP COLO PROCS",0))
 I APCHT S V=$$PRCREFT^APCHSMU2(APCHSPAT,APCHLAST,DT,APCHT) I V]"" D SETR
 ...
 Q
NMICOLO(P,BDATE,EDATE) ;
 NEW APCHT,V
 S APCHT=$O(^ATXAX("B","BGP COLO PROCS",0))
 I APCHT S V=$$PRCREFT^APCHSMU2(P,BDATE,EDATE,APCHT,"N") I V]"" Q 1
 S APCHT=$O(^ATXAX("B","BGP COLO CPTS",0))
 I APCHT S V=$$CPTREFT^APCHSMU2(P,BDATE,EDATE,APCHT,"N") I V]"" Q 1
 Q ""
REF ;
...
WRITALL ;
 ...
 .S $P(APCHSGHR(1),"|",2)=X
 .S X="CT COLONOSCOPY"_U_APCHLCT_U_$$DATE^APCHSMU(APCHLCT)_U_"CLINICAL DISCRETION"_U_U_$P(APCHLCTI,U,4,99)
 .S $P(APCHSGHR(1),"|",3)=X
 ...
 X APCHSCKP Q:$D(APCHSQIT)
 I APCHSNPG W ?26,"LAST",?38,"NEXT" S APCHSCT=0
 W !?5,"CT COLONOSCOPY",?26,$$DATE^APCHSMU(APCHLCT)
