APCDCAF1 ; IHS/CMI/LAB - MENTAL HLTH ROUTINE 16-AUG-1994 ; 15 Oct 2020  8:09 AM
 ;;2.0;IHS PCC SUITE;**2,5,7,8,11,25,26**;MAY 14, 2009;Build 48
 ;; ;
 ;
DISP ;EP
 S APCDALAB="Which Visit"
 D GREC^APCDCAF4
 I 'APCDVSIT W !,"No visit selected." D EOP G DISPX
 ;RELINKER?
 D ^XBFMK
...
MODIFY ;EP
 D FULL^VALM1
 S APCDALAB="Modify which Visit"
 D GREC^APCDCAF4
 I 'APCDVSIT W !,"No visit selected." D EOP G MODIFYX
 ;RELINKER?
...
APPEND ;EP
 D FULL^VALM1
 S APCDALAB="Append to which Visit"
 D GREC^APCDCAF4
 I 'APCDVSIT W !,"No visit selected." D EOP G APPENDX
 ;RELINKER
 S (Y,APCDPAT)=$P(^AUPNVSIT(APCDVSIT,0),U,5)
 D ^AUPNPAT
 S APCDVLK=APCDVSIT
...
UPDATE ;EP
 D FULL^VALM1
 S APCDALAB="Update Chart Status for which visit"
 D GREC^APCDCAF4
 I 'APCDVSIT W !,"No visit selected." D EOP G UPDATEX
 S AUPNVSIT=APCDVSIT
 D MOD^AUPNVSIT
UPD0 ;EP
 ;CHECK POV
...
MERGE ;EP
 ...
 S APCDALAB="Which Visit"
 D GREC^APCDCAF4
 I 'APCDVSIT W !,"No visit selected." D EOP G MERGEX
...
DEL ;EP
 ...
 S APCDALAB="Which Visit"
 D GREC^APCDCAF4
 I 'APCDVSIT W !,"No visit selected." D EOP G DELX
 D ^XBFMK
 ;DSPLY ;
...
MERGEDD ;EP
 ...
 S APCDALAB="Which Visit"
 D GREC^APCDCAF4
 I 'APCDVSIT W !,"No visit selected." D EOP G MERGEDDX
...
MOVEVD ;EP
 ...
 S APCDALAB="Which Visit"
 D GREC^APCDCAF4
 I 'APCDVSIT W !,"No visit selected." D EOP G MOVEVDX
 S APCDVLDT=$P($P(^AUPNVSIT(APCDVSIT,0),U),"."),APCDPAT=$P(^AUPNVSIT(APCDVSIT,0),U,5)
 S (X,APCDIDT)=(9999999-APCDVLDT),C=0 F  S X=$O(^AUPNVSIT("AA",APCDPAT,X)) Q:X=""!($P(X,".")>APCDIDT)  D
 .S V=0 F  S V=$O(^AUPNVSIT("AA",APCDPAT,X,V)) Q:V'=+V  S C=C+1
...
CVD ;EP
 S APCDALAB="Which Visit"
 D GREC^APCDCAF4
 I 'APCDVSIT W !,"No visit selected." D EOP G CVDX
 D ^XBFMK
 ;DSPLY ;
 W !
...
RESEQ ;EP
 D FULL^VALM1
 S APCDALAB="Resequence POVs for which visit"
 D GREC^APCDCAF4
 I 'APCDVSIT W !,"No visit selected." D EOP G DISPX
