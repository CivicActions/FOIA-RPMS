...
DRIVER ;EP
 ...
 .I $D(APCDPRVS),'$D(APCDPRVS(APCDPROV)) Q
 ...
 ..Q:APCDVLOC=""
 ..I $D(APCDLOCS),'$D(APCDLOCS(APCDVLOC)) Q
 ..S X=$P(APCDV0,U,7)
 ..Q:X=""  ;no sc
 ..I $D(APCDSCS),'$D(APCDSCS(X)) Q
 ..S APCDVCLN=$P(APCDV0,U,8)
 ..I APCDVCLN="",$D(APCDCLNS) Q
 ..I $D(APCDCLNS),'$D(APCDCLNS(APCDVCLN)) Q
...
SUMPAGE ;
 ...
 D HEAD
 I '$D(^XTMP("APCDCAFK",APCDJOB,APCDBT,"PROV")) W !!,"There are no pending deficiencies that meet the report criteria." Q
 S APCDS=0 F  S APCDS=$O(^XTMP("APCDCAFK",APCDJOB,APCDBT,"PROV","SUMMARY",APCDS)) Q:APCDS'=+APCDS!($D(APCDQUIT))  D
 ...
 .;deficiencies
 .S APCDDEF=""
 .S APCDI=0 F  S APCDI=$O(^XTMP("APCDCAFK",APCDJOB,APCDBT,"PROV","CDR",APCDS,APCDI)) Q:APCDI'=+APCDI!($D(APCDQUIT))  D
 ...
 ..S APCDTOT(APCDI)=$G(APCDTOT(APCDI))+^XTMP("APCDCAFK",APCDJOB,APCDBT,"PROV","CDR",APCDS,APCDI)
 .K ^UTILITY($J,"W") S X=APCDDEF,DIWL=0,DIWR=40 D ^DIWP
 .W ?40,$G(^UTILITY($J,"W",0,1,0))
 .I $O(^UTILITY($J,"W",0,1)) D
 ..S X=1 F  S X=$O(^UTILITY($J,"W",0,X)) Q:X'=+X  W !?40,^UTILITY($J,"W",0,X,0)
 .W !
 .K ^UTILITY($J,"W")
 I $Y>(IOSL-3) D HEAD Q:$D(APCDQUIT)
 W !,"TOTAL:",?27,+$P(APCDTOT,U,1),?33,+$P(APCDTOT,U,2) D
 .S APCDX="" F  S APCDX=$O(APCDTOT(APCDX)) Q:APCDX=""!($D(APCDQUIT))  D
 ..I $Y>(IOSL-2) D HEAD Q:$D(APCDQUIT)
 ..W ?40,$P(^AUTTCDR(APCDX,0),U,1)_" ("_APCDTOT(APCDX)_")",!
 Q
