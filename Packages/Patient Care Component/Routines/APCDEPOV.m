APCDEPOV ; IHS/CMI/LAB - POV LOOKUP ; 01 Apr 2015  3:18 PM
 ;;2.0;IHS PCC SUITE;**11,13,16,20,23,25**;MAY 14, 2009;Build 37
 ;
EDIT01 ;EP
...
LEX ;EP - called from input template
 ...
 S APCDVAL=""
 I APCDUINP=".9999" S APCDVAL=+$$ICDDX^ICDEX($S(APCDIMP=1:".9999",1:"ZZZ.999"),,APCDIMP,"E") G LEXN
 ...
 .S APCDVAL="" I $P(Y,U)'=-1 S APCDVAL=$$ICDDX^ICDEX(+Y,$$VD^APCLV(APCDVSIT))
 ...
 S APCDVAL=$$ICDDX^ICDEX(Y,$P(APCDD,"."),,"E")
 I $P(APCDVAL,U,1)="-1" W !!,"lexicon isn't passing back an ICD code." S APCDTERR=1,APCDTNPV="" G XITL
LEXN ;
 S Q=""
 ...
 ..W !
 ;
 S APCDTNPV="`"_+APCDVAL,APCDTNQP=APCDUINP
 W !
...
DELETE ;
 ...
 .I $D(DIRUT) Q
 .S APCDTDEL=1
 .NEW APCD1415,APCD14PT,APCD14VS
 .S APCD1415=$$VALI^XBDIQ1(9000010.07,APCDTDA,.16),APCD14PT=$P($G(^AUPNVPOV(APCDTDA,0)),U,2),APCD14VS=$P($G(^AUPNVPOV(APCDTDA,0)),U,3)
 .S DA=APCDTDA,DIK="^AUPNVPOV(" D ^DIK K DA,DIK
 .;GO REMOVE 1401 AND 1501 IN MULTIPLE OF PROBLEM IF THIS VISIT IS THERE.
 .D DELPL
 K DIR
...
SET1415 ;EP
 ...
 Q
DELPLCPV ;EP
 D EN^XBNEW("DELPL^APCDEPOV","APCD1415;APCD14VS;APCD14PT")
 Q
DELPL ;
 Q:'APCD1415
 NEW APCDY
 S APCDY=0 S APCDY=$O(^AUPNPROB(APCD1415,14,"B",APCD14VS,APCDY)) Q:APCDY'=+APCDY  D
 .;call die to change to Delete
 .K DIE,DA,DR
 .S DA(1)=APCD1415,DA=APCDY,DIE="^AUPNPROB("_APCD1415_",14,",DR=".01///@"
 .D ^DIE
 .K DIE,DA,DR
 ;USED AS INPATIENT
 S APCDY=0 S APCDY=$O(^AUPNPROB(APCD1415,15,"B",APCD14VS,APCDY)) Q:APCDY'=+APCDY  D
 .;call die to change to APCDVMT
 .K DIE,DA,DR
 .S DA(1)=APCD1415,DA=APCDY,DIE="^AUPNPROB("_APCD1415_",15,",DR=".01///@"
 .D ^DIE
 .K DIE,DA,DR
 .Q
 Q
