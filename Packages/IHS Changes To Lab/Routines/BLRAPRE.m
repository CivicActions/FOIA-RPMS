BLRAPRE ; IHS/HQW/TPF - ENVIRONMENT CHECK FOR ELECTRONIC SIGNATURE ; [ 05/16/02  7:35 AM ]
...
VERSION ;
 ...
 S %=$D(^XPD("9.7","B","LR*5.2*1012"))
 I '% D  Q
 . W !,$$C^XBFUNC("Patch 1013 of version 5.2 of the RPMS Laboratory Package Cannot Be Installed Unless")
 . W !,$$C^XBFUNC("Patch 1012 of version 5.2 Has not Been Previously Installed.")
 . D SORRY
 . I $$DIR^XBDIR("E","Press RETURN...")
 ;
 ;GET INSTALL STATUS
 S %=$O(^XPD("9.7","B","LR*5.2*1012",""))
 S LRSTATUS=$P($G(^XPD(9.7,%,0)),U,9)
 I LRSTATUS'=3 D  Q   ;IF INSTALL STATUS NOT COMPLETE QUIT
 .W !,$$C^XBFUNC("Install of Patch 1012 not complete!")
 .D SORRY
 .I $$DIR^XBDIR("E","Press RETURN...")
 ;
 W !!!,$$C^XBFUNC("Patch 1012 of version LR 5.2 Has Been Previously Installed...... OK to continue")
 ;
 ;
