BLRWU4 ;VA/DALOI/RWF - READ ACCESSION ; 22-Oct-2013 09:22 ; MKK
 ...
 ;
 ;;VA Patches: 128,153,201,271,402     ;Sep 27, 1994;Build 1
 ;
...
AA ;
 ...
 ;
 ; see if entry is UID
 I $G(LRVBY)<1,$D(^BLRARC68("C",LRX)) D UNIV Q
 ;
 ...
 ;
 S LRX=$G(^BLRARC68(LRAA,0)),LRIDIV=$S($L($P(LRX,U,19)):$P(LRX,U,19),1:"CP")
 W !,$P(LRX,U)
 ;
 ...
 ;
 I $D(LRACC),'$D(^BLRARC68(LRAA,1,LRAD,1,LRAN,0)) D  Q
 . W !,"ACCESSION: ",$P(^BLRARC68(LRAA,0),U,11)," ",$$FMTE^XLFDT(LRAD,"5D")," ",LRAN," DOES NOT EXIST!"
 ;
 S LRQUIT=1
 Q
 ;
...
UNIV ; see if entry is UID
 ...
 S LRQUIT=1
 W "  (",$P($G(^BLRARC68(LRAA,1,LRAD,1,LRAN,.2)),"^"),")"
 Q
 ;
 ;
QUES ;
...
UID(LRX,LRY) ; Lookup accession by UID
 ...
 S DIR(0)="F^10:10^K:'$D(^BLRARC68(""C"",X)) X"
...
CHECKUID(LRX) ; Check if UID is valid, accession exists.
 ...
 ;
 S LRY=0
 S Y=$Q(^BLRARC68("C",LRX))
 I $QS(Y,2)=LRX,+$QS(Y,3),+$QS(Y,4),+$QS(Y,5) D
 . I '$D(^BLRARC68(+$QS(Y,3),1,+$QS(Y,4),1,+$QS(Y,5),0)) Q
 . S LRY=1_"^"_$QS(Y,3)_"^"_$QS(Y,4)_"^"_+$QS(Y,5)
 Q LRY
