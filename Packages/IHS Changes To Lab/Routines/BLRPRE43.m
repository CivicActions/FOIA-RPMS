BLRPRE43 ; IHS/MSC/MKK - RPMS Lab Patch LR*5.2*1043 Pre/Install/Post Routine ; 15-Nov-2016 08:22 ; MKK
 ;;5.2;IHS LABORATORY;**1043**;NOV 01, 1997;Build 15
 ;
...
PRE ; EP - Ask for confirmation of Backup
 ...
 ;
 S $P(^VBEC(6009,65.5,0),"^",3)=""     ; Ensure "previously installed" flag is not YES.
 ;
 Q:$$BACKUP()="Q"
 ;
...
DEBUG ; EP - Debugging Line Label for environment checker
 ...
 Q
 ;
 ; NOTE:  This POST INSTALL code must be called AFTER the
 ;        LR*5.2*408 post-install.  
POST ; EP - Post-Install
 ...
 S WOTCNT=$$WOTCNT(BLRVERN)
 ;
 D ADDOPTS    ; Add BB menu to BLRMENU
 ;
 D BLANK
 ...
 Q
 ;
DELBBMNU ; EP - DELete Blood Bank MeNU option from main LRMENU
 NEW GONE
 ;
 S GONE=$$DELETE^XPDMENU("LRMENU","LRBL")   ; Remove Blood Bank option from main LRMENU
 Q
 ;
DELOOBBO ; EP - DELete all Out-Of-Order Options from the various Blood Bank Menus
 NEW BBIEN,BBOPT,CHEKIT,GONE,OPT,OPTIEN,SUBO,SUBIEN,SUBMENU
 ;
 S BBOPT="LRBL"
 F  S BBOPT=$O(^DIC(19,"B",BBOPT))  Q:BBOPT=""!($E(BBOPT,1,4)'="LRBL")  D
 . S BBIEN=0
 . F  S BBIEN=$O(^DIC(19,"B",BBOPT,BBIEN))  Q:BBIEN<1  D
 .. Q:$O(^DIC(19,BBIEN,10,0))<1     ; Skip if not a menu
 .. ;
 .. S SUBO=0
 .. F  S SUBO=$O(^DIC(19,BBIEN,10,SUBO))  Q:SUBO<1  D
 ... S SUBIEN=$$GET1^DIQ(19.01,SUBO_","_BBIEN,"ITEM","I")
 ... Q:$L($$GET1^DIQ(19,SUBIEN,"OUT OF ORDER MESSAGE"))<1   ; Skip if not Out-of-Order
 ... ;
 ... S SUBMENU=$$GET1^DIQ(19,SUBIEN,"NAME")
 ... S CHEKIT=$$DELETE^XPDMENU(BBOPT,SUBMENU)      ; Remove Out-of-Order option
 ;
 ; Now remove empty sub-menus (i.e., BB menus with No options) from menus
 ; or options/menus that should be removed.
 ;
 ; Main BB menu removal
 S CHEKIT=$$DELETE^XPDMENU("LRBL","LRBLI")      ; Remove empty "Inventory" sub-menu
 ;
 ; Supervisor's Menu removals
 S CHEKIT=$$DELETE^XPDMENU("LRBLS","LRCENDEL")  ; Remove "Delete"
 S CHEKIT=$$DELETE^XPDMENU("LRBLS","LRBLEF")    ; Remove "Edit BB files"
 S CHEKIT=$$DELETE^XPDMENU("LRBLS","LRBLSI")    ; Remove empty "BB inventory edit options" sub-menu
 S CHEKIT=$$DELETE^XPDMENU("LRBLS","LRBLSD")    ; Remove empty "Blood donor edit options" sub-menu
 ;
 ; Blood bank patient removals
 S CHEKIT=$$DELETE^XPDMENU("LRBLP","LRDELOG")      ; Remove "Remove an accession"
 S CHEKIT=$$DELETE^XPDMENU("LRBLP","LRADD TO ACC") ; Remove "Add tests to accession"
 S CHEKIT=$$DELETE^XPDMENU("LRBLP","LRTSTOUT")     ; Remove "Delete test from accession"
 S CHEKIT=$$DELETE^XPDMENU("LRBLP","LRBLPC")       ; Remove empty "Request/select/xmatch blood components" sub-menu
 ;
 Q
 ;
ADDOPTS ; EP - Add Blood Bank option to BLRMENU
 D ADDOPTS^BLRKIDS2("BLRMENU","LRBL","BB")
 Q
 ;
