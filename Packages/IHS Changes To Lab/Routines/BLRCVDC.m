BLRCVDC ;IHS/CMI/MAW - BLR Create COVID-19 Test for Abbott ID NOW
 ;;5.2;IHS LABORATORY;**1045**;APRIL 07,2020;Build 21
 Q
 ;
...
TESTS ;-- test subscript
 ;LAB^ORDCD^RESC^DATATYPE^ORDERNAME^RESULTNAME^COLSAMP^SPEC^PRINTNAME^LOINC^CPT^QUALRES^SYNONYM^REFLOW^REFHIGH
 S COVID19P="ABBOTT^COVID19P^^^COVID-19 (Abbott ID NOW)^^SWAB-COVID19*NASOPHARYNGEAL MUCUS*SWAB^^ABBOTT ID NOW^^87635*U0002^^CORONAVIRUS*ABBOTT ID NOW*COVID19"
 S COVID19A="ABBOTT^COVID19P^COVID19A^S*P:COVID-19 POSITIVE;N:COVID-19 Negative;^COVID-19 (Abbott ID NOW)^_COVID-19(Abbott ID NOW)^SWAB-COVID19*NASOPHARYNGEAL MUCUS*SWAB^NASOPHARYNGEAL MUCUS^_COVID RESULT^94534-5^^P*p*POSITIVE*positive^^Negative"
 S COVID19A("INT")="A Negative Result does not rule out co-infections with other pathogens."
 S PROCCONT="ABBOTT^COVID19P^PROCCONT^S*V:Valid;^COVID-19 (Abbott ID NOW)^_PROCEDURAL CONTROL^SWAB-COVID19*NASOPHARYNGEAL MUCUS*SWAB^NASOPHARYNGEAL MUCUS^Procedural QC^^^"
 F I="COVID19P","COVID19A","PROCCONT" D
 . D SETTESTS(@I)
 Q
 ;
...
LOOP(RL) ;-- loop through the stored data and create tests
 ...
 .. ;DATA=ORDC^RESC^DATATYPE^ORDER TEST NAME^RESULT TEST NAME^COLL SAMP^SITE SPECIMEN^PRINT NAME^LOINC^CPT^QUAL RES^SYNONYM^DATANAME^TESTIEN^REFLOW^REFHIGH
...
DATANAME(R,OR,RE,DAT) ;-- create the dataname, always free text
 ...
 I DTP="S" D  Q
 . S CVAL=$P(DAT,"*",2)
 . S (X,LRNAME)=NAM,DIC="^DD(63.04,",DIC(0)="XM" D ^DIC I Y>0 S $P(^BLRLTMP(R,OR,RE),U,15)=+Y_"|"_NAM Q
 ...
 . S $P(^BLRLTMP(R,OR,RE),U,15)=DA_"|"_NAM
 I DTP="F" D  Q
 . S MIN=1
 . S MAX=30
 . S (X,LRNAME)=NAM,DIC="^DD(63.04,",DIC(0)="XM" D ^DIC I Y>0 S $P(^BLRLTMP(R,OR,RE),U,15)=+Y_"|"_NAM Q
...
TEST(PAN,R,OR,RE,DAT) ;-- build the child test in file 60
 ...
 Q:'$G(FIENS(1))
 S TESTIEN=$G(FIENS(1))
 I PAN'="PAN" S $P(^BLRLTMP(R,OR,RE),U,16)=TESTIEN
 I PAN="PAN" S $P(^BLRLTMP(R,OR),U,16)=TESTIEN
 ...
 ;D ADDACC(TESTIEN)
 D SN(TESTIEN,"LAB TEST CREATED IN LAB PATCH 1045, APRIL 2020")
 Q
 ;
...
CPT(OTST,TST,CPT) ;-- build the IHS LAB CPT CODE entries
 Q:TST=""  ;no test code in piece 10
 ...
 . S FDA(9009021,"?+1,",.02)="COVID"
 . S FDA(9009021,"?+1,",.03)=NOW
 . S FDA(9009021,"?+1,",.04)=NOW
 . S FDA(9009021,"?+1,",101)=TST
 . S FDA(9009021.01101,"?+2,?+1,",.01)=CPTE
 . ;S FDA(9009021.1101101,"?+3,?+2,?+1,",.01)=90  ;ref lab always has 90 modifier per Karen
 . D UPDATE^DIE("","FDA","FIENS","FERR(1)")
 Q
 ;
