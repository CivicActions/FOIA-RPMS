ACGSRQ5 ;IHS-OIRM-DSD/THL - CHECK CIS RECORD INTEGRITY CONT'D; [ 10/27/2004   4:19 PM ]
 ;;2.0t1;CONTRACT INFORMATION SYSTEM;**13**;FEB 16, 2000
 ;;CHECK CIS RECORD INTEGRITY CONT'D
 ;;MODIFIED FOR CACHE' COMPLIANCY & TO CLARIFY CODE; ACR*2.1*13.02 IM13574
EN ;EP;TO CHECK INTEGRITY OF CIS ENTRY
 ;Begin old code  ; ACR*2.1*13.02 IM13574
 ...
 ;Q
 ;Begin new code ; ACR*2.1*13.02 IM13574
 N TMP
45 I ACG26>100000,ACG45="" D
 .D T
 .S TMP="26++"_ACG26
 .S TMP=TMP_";45++"_ACG45
 .S TMP=TMP_"^W !?5,""Synopsis is required if Item 26 greater than $100,000."""
 .S ^TMP("ACG",$J,ACG2,45)=TMP
 I ACG45=2,ACG18'=77 D
 .D T
 .S TMP="45++"_ACG45
 .S TMP=TMP_";18++"_ACG18
 .S TMP=TMP_"^W !?5,""I Synopsis is 2, Item 18 must equal 77."""
 .S ^TMP("ACG",$J,ACG2,45)=TMP
46 ;
47 ;
48 ;
...
51 I "ABCDGILMOQRSTU"[ACG1 D
 ...
 ..D T
 ..S TMP="51++"_ACG51
 ..S TMP=TMP_"^W !?5,""Item 51 is required and must be '1' or '2'"""
 ..S ^TMP("ACG",$J,ACG2,51)=TMP
 .I ACG51=1,ACG12'=""&(ACG13'="")&(ACG15'="")&(ACG17'="")&(ACG18'="")&(ACG20'="")&(ACG21'="")&(ACG30'="")&(ACG31'="")&(ACG33'="")&(ACG45'="")&(ACG52'="")&(ACG62'="") D
 ..D T
 ..S TMP="51++"_ACG51
 ..S TMP=TMP_"^W !?5,""If Item 51=1, Items 12, 13, 15, 17, 18, 20, 21, 30, 31, 33, 45, 52 and 62 must be null"""
 ..S ^TMP("ACG",$J,ACG2,51)=TMP
52 ;
53 I "ABCDGILMOQRSTU"[ACG1 D                  ;ACR*2.1*13.02 IM13574
 .I ACG53=""!(ACG53'?4N) D
 ..D T
 ..S TMP="1++"_ACG1
 ..S TMP=TMP_";53++"_ACG53
 ..S TMP=TMP_"^W !?5,""If Item 1 equals A, D, I, L, S, O, Q or G, Item 53 is required"""
 ..S ^TMP("ACG",$J,ACG2,1,53)=TMP
 .I "^15^16^17^"[(U_$E(ACG53,1,2)_U),"A1A2A3B1B2"[ACG13,ACG63'=1 D
 ..D T
 ..S TMP="53++"_ACG53
 ..S TMP=TMP_";63++"_ACG63
 ..S TMP=TMP_"^W !?5,""If Item 53=15, 16 or 17, & Item 13=A1, A2, A3, B1, or B2, Item 63 must be coded '1'"""
 ..S ^TMP("ACG",$J,ACG2,53,63)=TMP
54 ;
55 ;
56 I ACG56=1,"A1A2A3"[ACG13 D
 .D T
 .S TMP="56++"_ACG56
 .S TMP=TMP_";13++"_ACG13
 .S TMP=TMP_"^W !?5,""If Item 56 equals 1, Item 13 must not equal A1, A2, or A3"""
 .S ^TMP("ACG",$J,ACG2,56,13)=TMP
 I ACG56=1,ACG62="B" D
 .D T
 .S TMP="56++"_ACG56
 .S TMP=TMP_";62++"_ACG62
 .S TMP=TMP_"^W !?5,""If Item 56 equals 1, Item 62 must not equal B"""
 .S ^TMP("ACG",$J,ACG2,56,62)=TMP
57 ;
58 ;
59 ;
60 ;
...
62 I "ADILSMCRTQ"[ACG1 D
 ...
 ..S TMP="1++"_ACG1
 ..S TMP=TMP_";62++"_ACG62
 ..S TMP=TMP_"^W !,""Item 62 is required"""
 ..S ^TMP("ACG",$J,ACG2,1,62)=TMP
 .I ACG18="",ACG62="A",ACG19<50!(ACG19>59) D
 ..D T
 ..S TMP="18++"_ACG18
 ..S TMP=TMP_";62++"_ACG62
 ..S TMP=TMP_";19++"_ACG19                             ;ACR*2.1*13.02 IM13574
 ..S TMP=TMP_"^W !?5,""If Item 18 is null and Item 62 equals A, Item 19 must equal 50-59"""
 ..S ^TMP("ACG",$J,ACG2,18,62,19)=TMP
 .I ACG62="D",ACG52'=1&(ACG52'=2) D
 ..D T
 ..S TMP="62++"_ACG62
 ..S TMP=TMP_";52++"_ACG52
 ..S TMP=TMP_"^W !?5,""If Item 62 equals D, Item 52 must equal 1 or 2"""
 ..S ^TMP("ACG",$J,ACG2,62,52)=TMP
 .I ACG62="C",ACG18'=81 D
 ..D T
 ..S TMP="62++"_ACG62
 ..S TMP=TMP_";18++"_ACG18
 ..S TMP=TMP_"^W !?5,""If Item 62 equals C, Item 18 must equal 81"""
 ..S ^TMP("ACG",$J,ACG2,62,18)=TMP
 .I ACG62="B" D
 ..I "MCRTU"'[ACG1 D
 ...D T
 ...S TMP="62++"_ACG62
 ...S TMP=TMP_";1++"_ACG1
 ...S TMP=TMP_"^W !?5,""If Item 62 equals B, Item 1 must equal M, C, R, T or U"""
 ...S ^TMP("ACG",$J,ACG2,62,1)=TMP
 ..I ACG56=1 D
 ...D T
 ...S TMP="62++"_ACG62
 ...S TMP=TMP_";56++"_ACG56
 ...S TMP=TMP_"^W !?5,""If Item 62 equals B, Item 56 must not equal 1"""
 ...S ^TMP("ACG",$J,ACG2,62,56)=TMP
 ..I ACG15'=10 D
 ...D T
 ...S TMP="62++"_ACG62
 ...S TMP=TMP_";15++"_ACG15
 ...S TMP=TMP_"^W !?5,""If Item 62 equals B, Item 15 must equal 10"""
 ...S ^TMP("ACG",$J,ACG2,62,15)=TMP
 ..I ACG52'="" D
 ...D T
 ...S TMP="62++"_ACG62
 ...S TMP=TMP_";52++"_ACG52
 ...S TMP=TMP_"^W !?5,""If Item 62 equals B, Item 52 must be null"""
 ...S ^TMP("ACG",$J,ACG2,62,52)=TMP
 D ^ACGSRQ6
 Q
T I '$D(^TMP("ACG",$J,ACG2)) D
 .S ^TMP("ACG",$J,"T")=^TMP("ACG",$J,"T")+1
 .W:'$D(ZTQUEUED)&($E(IOST,1,2)="C-") "."
 Q
 ;End new code ; ACR*2.1*13.02 IM13574
