ACGSREST ;IHS/OIRM/DSD/THL,AEF - RESTORES DATA GLOBALS FROM AREA SAVES TO CREATE HQ DATABASE; [ 10/27/2004   4:19 PM ]
 ;;2.0t1;CONTRACT INFORMATION SYSTEM;**13**;FEB 16, 2000
 ;;RESTORES DATA GLOBALS FROM AREA SAVES TO CREATE HQ DATABASE
 ;;IHS/ITSC/SD/MRS:ACR*2.1*9
EN D ^XBKVAR
 D EN1
EXIT K %DEV,%TAP,%IN,%DFLT,%SEL,%ANS,%FN,%TIME,%CMT
 Q
EN1 ;F ACG=102,161,235,239,241:1:249,285 S %FN="/usr3/cis/acg"_ACG_".asc" D RESTORE  ;ACR*2.1*13.06 IM14144
 N ARMSDIR                       ;ACR*2.1*13.06 IM14144
 S ARMSDIR=$$ARMSDIR^ACRFSYS(1)  ;ACR*2.1*13.06 IM14144
 Q:ARMSDIR']""                   ;ACR*2.1*13.06 IM14144
 F ACG=102,161,235,239,241:1:249,285 S %FN=ARMSDIR_ACG_".asc" D RESTORE  ;ACR*2.1*13.06 IM14144
 D REINDEX
...
RESTORE F ACGI=1:1:4 S (IOP,ION)="HOST FILE SERVER #"_ACGI,%IS("IOPAR")="("""_%FN_""":""R"")" D ^%ZIS Q:'POP
 ...
 U IO R %TIME:999,%CMT:999       ;READ UNIX FILE
 I %TIME="" D DONE Q
START ;D START^ACGSGR                      ;ACR*2.1*13.01 IM13574
 N ACGOS                              ;ACR*2.1*13.01 IM13574
 S ACGOS=$$VERSION^%ZOSV(1)           ;ACR*2.1*13.01 IM13574
 I ACGOS["MSM" D START^ACGSGR         ;ACR*2.1*13.01 IM13574
 I ACGOS["Cache" D ^%GI               ;ACR*2.1*13.01 IM13574
DONE D ^%ZISC
 Q
