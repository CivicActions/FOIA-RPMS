BADEENV6 ;IHS/OIT/GAB - BADE ENVIRONMENT CHECK ROUTINE PATCH #6;20-JUNE-2019 16:21;GAB
 ;;1.0;DENTAL/EDR INTERFACE;**6**;FEB 22, 2010;Build 26
 ;
ENV ;EP
 N IN,PATCH,INSTDA,STAT
 W !,"Checking environment ....",!
 ;Check for the installation of the EDR/BADE v1.0 Patch 5
 S IN="BADE*1.0*5",INSTDA=""
 I '$D(^XPD(9.7,"B",IN)) D  Q
 .D MES("You must first install the DENTAL/EDR INTERFACE BADE Patch 5 before this patch",2)
 S INSTDA=$O(^XPD(9.7,"B",IN,INSTDA),-1)
 S STAT=+$P($G(^XPD(9.7,INSTDA,0)),U,9)
 I STAT'=3 D  Q
 .D MES("DENTAL/EDR INTERFACE BADE Patch 5 must be completely installed before installing this patch",2)
 ; now check the DUZ ..
 I '$G(IOM) D HOME^%ZIS
