AGRHI1 ; IHS/ASDS/EFG - RESTRICTED HEALTH REPORT ;   [ 05/08/2003  8:29 AM ]
...
PROCESS ; PROCESS LOOP FOR REPORT RECORDS
 ...
 ... S DFN=$P(RECORD,U,1)
 ... S MR=$P(^AUPNPAT(DFN,41,DUZ(2),0),U,2)
 ... S RHI=$P(RECORD,U,2)
 ... S STREC=$S(RHISTAT="A":2,RHISTAT="P":1,RHISTAT="N":3,RHISTAT="R":4,RHISTAT="E":5)
 ... S USER=$S(STREC=1:$P(^AUPNRHI(RHIREC,STREC),U,2),STREC=5:$P(^AUPNRHI(RHIREC,STREC),U,1),1:$P(^AUPNRHI(RHIREC,STREC),U,3))
 ... S ^XTMP($J,"AGRHI",ENTDAT,USER)=MR_U_RHISTAT_U_RHI
...
PRINT ; DO HEADER AND DETAIL PRINTING HERE
 ...
 F  S ENTDAT=$O(^XTMP($J,"AGRHI",ENTDAT)) Q:'ENTDAT  D  Q:$G(AG("F1"))
 . F  S USER=$O(^XTMP($J,"AGRHI",ENTDAT,USER)) Q:'USER  D  Q:$G(AG("F1"))
 .. S RHIREC=$G(^XTMP($J,"AGRHI",ENTDAT,USER))
 .. S USERNAM=$P(^VA(200,USER,0),U,1)
