...
MCDY ;if medicaid coverage
 S AGSAME=0
 ; MediCaid name.
 S (AGMNM,AG1(1))=$P($G(^AUPNMCD(AG("IEN"),21)),U,1)
 ; MediCaid DOB.
...
FILE(AG) ;EP - File Medicaid
 ...
 .Q
 ;
 ; S DA(1)=AG("IEN"),DIK="^AUPNMCD(DA(1),11,",DA=0
 ; F  S DA=$O(^AUPNMCD(DA(1),11,DA)) Q:'DA  D ^DIK
 ;
...
ADD(X,AG2,AG3) ;
 NEW DA,DIC,DR
 ;S DA(1)=AG("IEN"),DIC="^AUPNMCD(DA(1),11,",DIC(0)="F",DIC("P")=$P(^DD(9000004,1101,0),U,2)  ; IHS/SD/EFG  AG*7*2  #22
 S DA(1)=AG("IEN"),DIC="^AUPNMCD("_DA(1)_",11,",DIC(0)="F",DIC("P")=$P(^DD(9000004,1101,0),U,2)  ; IHS/SD/EFG  AG*7*2  #22
 KILL DD,DO
...
EDIT(AGDATES)  ;
 ...
 ;S DA(1)=AG("IEN"),DIE="^AUPNMCD(DA(1),11,",DR=".02///"_$P(AGDATES,U,2)  ; IHS/SD/EFG  AG*7*2  #22
 S DA(1)=AG("IEN"),DIE="^AUPNMCD("_DA(1)_",11,",DR=".02///"_$P(AGDATES,U,2)  ; IHS/SD/EFG  AG*7*2  #22
 D ^DIE
 S AGUPDATE=1
 Q
 ;
