AGFACE ; IHS/ASDS/EFG - FACE SHEET ; MAR 19, 2010    
 ;;7.1;PATIENT REGISTRATION;**1,2,4,5,7,9,11,15,16,17**;AUG 25, 2005;Build 9
...
ALIAS ;OTHER NAMES
 ...
 E  W !
 ;IHS/OIT/NKD AG*7.1*16 SSN REMOVAL - START OLD CODE
 ...
 ;IHS/OIT/NKD AG*7.1*16 SSN REMOVAL - END OLD CODE
 W "CLASS: " S DR=1111,DIC=9000001 D ^AGDICLK W:$D(AG("LKPRINT")) AG("LKPRINT")  ;IHS/OIT/NKD AG*7.1*16 SSN REMOVAL
 ;W ?62,"SEX: " S DR=.02,DIC=2 D ^AGDICLK W:$D(AG("LKPRINT")) AG("LKPRINT")  ;IHS/OIT/NKD AG*7.1*15 BIRTH SEX
 W ?56,"BIRTH SEX: " S DR=.02,DIC=2 D ^AGDICLK W:$D(AG("LKPRINT")) AG("LKPRINT")
 ...
 W:AG]"" !?5,AG
 W !,"PHONE NUMBERS ---"
 W !,"HOME: " S DR=.131 D ^AGDICLK W:$D(AG("LKPRINT")) AG("LKPRINT") W ?27,"WORK: " S DR=.132 D ^AGDICLK W:$D(AG("LKPRINT")) AG("LKPRINT")
 W !,"CELL: " S DR=.134 D ^AGDICLK W:$D(AG("LKPRINT")) AG("LKPRINT") W ?27,"OTHER: " S DR=1801 W $$GET1^DIQ(9000001,DFN,DR) ;;IHS/OIT/JS AG*7.1*17 
 W !
 ...
 .;IHS/OIT/NKD AG*7.1*15 MU3 - CHANGED DISPLAY TO USE MULTIPLE FIELD
 .;S ETHNIC=$O(^DPT(DFN,.06,0))
 .;I ETHNIC S ETHNIC=$$GET1^DIQ(10.2,ETHNIC,.01)
 .;W ?46,"ETHNICITY: ",$G(ETHNIC)
 .N AGETHN S AGETHN=$$ETHN^AGUTL(DFN)
 .W ?46,"ETHNICITY: "_$S(+AGETHN>1:"MULTIPLE ETHNICITIES",1:$E($P(AGETHN,"^",2),1,23))
 ;END NEW CODE
...
HDR ;EP - FACE SHEET HEADER
 ...
 ;I '$D(RHIFLAG) W "PATIENT: " S DIC=2,DA=DFN,DR=.01 D ^AGDICLK W:$D(AG("LKPRINT")) AG("LKPRINT") I $D(^DPT(DFN,"VET")),^("VET")="Y" W " (VETERAN)"  ;IHS/OIT/NKD AG*7.1*15 PREFERRED NAME
 S DA=DFN
 I '$D(RHIFLAG) W "PATIENT: ",$$GETPREF^AUPNSOGI(DFN,"E",1) I $D(^DPT(DFN,"VET")),^("VET")="Y" W " (VETERAN)"
 I $D(RHIFLAG)  D
 . ;I RHIFLAG="A" W "PATIENT: " S DIC=2,DA=DFN,DR=.01 D ^AGDICLK W:$D(AG("LKPRINT")) $$S^AGVDF("RVN"),AG("LKPRINT"),$$S^AGVDF("BLN")," (RHI)",$$S^AGVDF("BLF"),$$S^AGVDF("RVF")  ;IHS/OIT/NKD AG*7.1*15 PREFERRED NAME
 . ;E  W "PATIENT: " S DIC=2,DA=DFN,DR=.01 D ^AGDICLK W:$D(AG("LKPRINT")) AG("LKPRINT") I $D(^DPT(DFN,"VET")),^("VET")="Y" W " (VETERAN)"  ;IHS/OIT/NKD AG*7.1*15 PREFERRED NAME
 . I RHIFLAG="A" W "PATIENT: ",$$S^AGVDF("RVN"),$$GETPREF^AUPNSOGI(DFN,"E",1),$$S^AGVDF("BLN")," (RHI)",$$S^AGVDF("BLF"),$$S^AGVDF("RVF")
 . E  W "PATIENT: ",$$GETPREF^AUPNSOGI(DFN,"E",1) I $D(^DPT(DFN,"VET")),^("VET")="Y" W " (VETERAN)"
