AGED8 ; IHS/ASDS/EFG - EDIT DEATH INFO/OTHER NAMES ;  
 ;;7.1;PATIENT REGISTRATION;**2,15**;AUG 25, 2005;Build 1
 ...
 ;
 ;S AG("N")=5  ;IHS/OIT/NKD AG*7.1*15
 ;VAR D DRAW  ;IHS/OIT/NKD AG*7.1*15
VAR N CLLST
 D DRAW
 Q:$D(AGSEENLY)
 ;W !?10,"Enter ""4"" to edit OTHER NAMES or ""5"" to edit LEGAL NAMES." ;IHS/OIT/NKD AG*7.1*15 DISPLAY CLEANUP
 W !,AGLINE("EQ")
 I '$D(AGSEENLY) D
 . K DIR
 ...
 . ;S DIR("A")="CHANGE which item? (1-"_AG("N")_") NONE// "  ;IHS/OIT/NKD AG*7.1*15
 . S DIR("A")="CHANGE which item? (1-"_AG("E")_") NONE// "
 ...
 ;I $D(DQOUT)!(+Y<1)!(+Y>AG("N")) W !!,"You must enter a number from 1 to ",AG("N") H 2 G VAR  ;IHS/OIT/NKD AG*7.1*15
 I $D(DQOUT)!(+Y<1)!(+Y>AG("E")) W !!,"You must enter a number from 1 to ",AG("E") H 2 G VAR
 W !!
 ;CC S AG("C")="DATE^AGED8,STATE^AGED8,CERT^AGED8,ALIAS^AG3A,NAMCHG^AGNAMCHG"  ;IHS/OIT/NKD AG*7.1*15
C ;EP - Edit multiple fields on a Reg edit page.
 ;S AGY=Y F AGI=1:1 S AG("SEL")=+$P(AGY,",",AGI) Q:AG("SEL")<1!(AG("SEL")>AG("N"))  D @($P(AG("C"),",",AG("SEL")))  ;IHS/OIT/NKD AG*7.1*15
 S AGY=Y F AGI=1:1 S AG("SEL")=+$P(AGY,",",AGI) Q:AG("SEL")<1!(AG("SEL")>AG("E"))  D @(CLLST(AG("SEL")))
...
LEGNAM(NUM) ;
 N DIC,DIR,DA,X,Y
 K DTOUT,DUOUT
 ;IHS/OIT/NKD AG*7.1*15 START NEW CODE
 W !,"CHANGED TO",?32,"BY",?38,"PROOF",?54,"DOC. #",?70,"DATE"
 W !,$G(NUM),". " I '$D(^AUPNNAMC("C",DFN)) W "<NO LEGAL NAME CHANGES ON FILE>" Q
 ;IHS/OIT/NKD AG*7.1*15 END NEW CODE
 I $D(^AUPNNAMC("C",DFN))  D
 . S (PTR,REC,PRFPTR)=0
 . S (DTCHG,CHGTO,PROOF)=""
 . ;IHS/OIT/NKD AG*7.1*15 START OLD CODE
 . ;W !,"CHANGED TO"
 . ;W ?32,"BY"
 . ;W ?38,"PROOF"
 . ;W ?54,"DOC. #"
 ...
 .. W ?54,$E(DOCNUM,1,15)
 .. ;W ?70,$E(DTCHG,4,5)_"/"_$E(DTCHG,6,7)_"/"_($E(DTCHG,1,3)+1700),!  ;IHS/OIT/NKD AG*7.1*15
 .. W ?70,$E(DTCHG,4,5)_"/"_$E(DTCHG,6,7)_"/"_($E(DTCHG,1,3)+1700) I $O(^AUPNNAMC("C",DFN,PTR)) W !
 Q
DRAW ;EP
 S AG("PG")=7
 ;IHS/OIT/NKD AG*7.1*15 - START NEW CODE
 N CALLS
 S CALLS="DATE^AGED8,STATE^AGED8,CERT^AGED8,ALIAS^AG3A,PREF^AGED14,NAMCHG^AGNAMCHG"
 S AG("N")=6
 S AG("E")=6
 S CLLST=0
 ;IHS/OIT/NKD AG*7.1*15 - END NEW CODE
 ...
 D ^AGED
 K ^UTILITY("DIQ1",$J)
 ;F AG=1:1 D  Q:$G(AGSCRN)[("*END*")  ;IHS/OIT/NKD AG*7.1*15
 F AG=1:1:AG("N") D
 ...
 . S CLLST=CLLST+1,CLLST(CLLST)=$P(CALLS,",",AG)  ;IHS/OIT/NKD AG*7.1*15
 . I AG=4 W !,$E(AGLINE("-"),1,33) W ?33," Other Names ",$E(AGLINE("-"),1,34)  ;IHS/OIT/NKD AG*7.1*15
 . I AG=6 W !,$E(AGLINE("-"),1,33) W ?33," Legal Names ",$E(AGLINE("-"),1,34) D LEGNAM(CLLST) Q  ;IHS/OIT/NKD AG*7.1*15
 . ;W @NEWLINE,AG,".",@CAPDENT,$S($G(CAPTION)'="":CAPTION,1:$P($G(^DD(DIC,DR,0)),U))_" : "  ;IHS/OIT/NKD AG*7.1*15
 . W @NEWLINE,CLLST,".",@CAPDENT,$S($G(CAPTION)'="":CAPTION,1:$P($G(^DD(DIC,DR,0)),U))_" : "  ;IHS/OIT/NKD AG*7.1*15
 . ;IHS/OIT/NKD AG*7.1*15 START NEW CODE
 . I AG=4 D  Q
 . . N CNT S CNT=0 F  S CNT=$O(^DPT(DFN,.01,CNT)) Q:'CNT  W ?27,$P($G(^DPT(DFN,.01,CNT,0)),U) I $O(^DPT(DFN,.01,CNT)) W !
 . ;IHS/OIT/NKD AG*7.1*15 END NEW CODE
 ...
 ;D LEGNAM
 W !,AGLINE("EQ")  ;IHS/OIT/NKD AG*7.1*15 LAST UPDATED
...
1 ;
 ...
 ;;^?10^9000001^1115^!^2
 ;;^?3^9000001^1116^!^3
 ;;OTHER NAMES^?13^2^1^!^4
 ;;PREFERRED NAME^?10^9000001^9001^!^5
 ;;^?10^9000033^^!^6
 ;;*END*
