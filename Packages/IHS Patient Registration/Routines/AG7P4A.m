AG7P4A ;IHS/SD/EFG - Patient Registration 7.0 Patch 4 CONT. ;   [ 06/17/2003  10:49 AM ]
 ;;7.0;IHS PATIENT REGISTRATION;**1,2,3,4**;MAR 28, 2003
 ;
 ;
PRE ;EP - From KIDS.
 Q
...
POST ;EP - From KIDS.
 ;
 D BMES^XPDUTL("Beginning post-install routine (POST^AG7P4A)."),TS
 ;
 D ^AGSETPRT
 ;
 D AGFAC
 ...
 D MCD
 ;
 I $$INSTALLD^AG7P2("AG*7.0*4") D
 . D TS,BMES^XPDUTL("Delivering AG*7.0*4 install message to select users ...")
 . D MAIL
 . D BMES^XPDUTL("Post-install routine is complete."),TS
 ;
 Q:$$INSTALLD^AG7P4("AG*7.0*4")
 ;
 D TS,OPTRES("AGMENU")
 ;
 D TS,BMES^XPDUTL("Delivering AG*7.0*4 install message to select users...")
...
MAIL ; Send install mail message.
 ...
 KILL ^TMP("AG7P4MS",$J)
 S ^TMP("AG7P4MS",$J,1)=" --- AG v 7.0, Patch 4 has been installed into this uci ---"
 S %=0
 F  S %=$O(^XTMP("XPDI",XPDA,"BLD",XPDBLD,1,%)) Q:'%   S ^TMP("AG7P4MS",$J,(%+1))=" "_^(%,0)
 S XMSUB=$P($P($T(+1),";",2)," ",3,99),XMDUZ=$S($G(DUZ):DUZ,1:.5),XMTEXT="^TMP(""AG7P4MS"",$J,",XMY(1)="",XMY(DUZ)=""
...
OPTRES(AGM) ;
 ...
 NEW AG,AGI
 I '$D(^XTMP("AG7P4",7.2,"OPTSAV",AGM)) D BMES^XPDUTL("FAILED.  Option '"_AGM_"' was not previously saved.") Q
 S AG=0
 F  S AG=$O(^XTMP("AG7P4",7.2,"OPTSAV",AGM,AG)) Q:'AG  S AGI=^(AG) I '$$ADD^XPDMENU(AGM,$P(AGI,U,1),$P(AGI,U,2),$P(AGI,U,3)) D BMES^XPDUTL("....FAILED to re-atch "_$P(AGI,U,1)_" to "_AGM_".")
