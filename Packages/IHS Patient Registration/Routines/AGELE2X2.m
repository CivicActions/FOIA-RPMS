AGELE2X2 ; IHS/ASDS/EFG - PAGE 2 - INSURER PART 2 ;   [ 05/07/2003  3:10 PM ]
 ...
 ;     CREATE NEW VARIABLES CONTAINING PATIENT'S ADDRESS AND PHONE
 ;         NUMBER DATA TO POPULATE POLICY HOLDER'S ADDRESS AND PHONE
 ;         NUMBER FIELDS IF THE POLICY HOLDER IS A REGISTERED PATIENT.
 ;
 I +AGV("X2")="" G XIT
...
REG I $P(^AUPN3PPH(+AGV("X2"),0),U,2)="" G PHINFO
 ...
 .S AG("PH9")=$P(^(.11),U)  ; IHS/SD/EFG  AG*7*1  02/27/2003
 .S $P(AGV("X2"),U,4)=$P(^(.11),U,4)_", "
 .S AG("PH11")=$P(^(.11),U,4)  ; IHS/SD/EFG  AG*7*1  02/27/2003
 E  G REMPL
 I $P(^DPT(AGX("HDFN"),.11),U,5)]"",$D(^DIC(5,$P(^(.11),U,5),0)) D
 .S $P(AGV("X2"),U,4)=$P(AGV("X2"),U,4)_$P(^(0),U,2)_"  "_$P(^DPT(AGX("HDFN"),.11),U,6)
 .S AG("PH12")=$P(^(.11),U,5)  ; IHS/SD/EFG  AG*7*1  02/27/2003
 .S AG("PH13")=$P(^(.11),U,6)  ; IHS/SD/EFG  AG*7*1  02/27/2003
 .S:$D(^(.13)) $P(AGV("X2"),U,5)=$P(^(.13),U)
 .S:$D(^(.13)) AG("PH14")=$P(^(.13),U)  ; IHS/SD/EFG  AG*7*1  02/27/2003
