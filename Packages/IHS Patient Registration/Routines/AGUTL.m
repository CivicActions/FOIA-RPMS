AGUTL ;IHS/OIT/NKD - PAT REG UTILITY ROUTINE ; FEBRUARY 10, 2014
 ;;7.1;PATIENT REGISTRATION;**11,12,13,14,15**;AUG 25, 2005;Build 1
 ;
 ...
 ;IHS/OIT/NKD AG*7.1*15 ETHNICITY API
 ;IHS/OIT/NKD AG*7.1*15 RACE/ETHNICITY AGGREGATE API
 ;
 Q
RACE(AGPATDFN) ;EP - RETURN LIST OF PATIENT'S RACES (EXTERNAL FORMAT)
 ...
 Q AGLIST
 ;IHS/OIT/NKD AG*7.1*15
ETHN(AGPATDFN) ;EP - RETURN LIST OF PATIENT'S ETHNICITIES (EXTERNAL FORMAT)
 ; # OF ETHNICITIES^ETHNICITY 1^ETHNICITY 2^ETC
 ; EX: PT W/0 ETHNICITIES => "0"
 ; EX: PT W/2 ETHNICITIES => "2^HISPANIC OR LATINO^NOT HISPANIC OR LATINO"
 N AGETHN,AGLIST,AGCNT
 D LIST^DIC(2.06,","_AGPATDFN_",","@;.01E","P",,,,,,,"AGETHN")
 S AGLIST=$P(AGETHN("DILIST",0),"^",1)
 F AGCNT=1:1:$P(AGETHN("DILIST",0),"^",1) S AGLIST=AGLIST_"^"_$P(AGETHN("DILIST",AGCNT,0),"^",2)
 Q AGLIST
 ;IHS/OIT/NKD AG*7.1*15
AGRACE(AGPATDFN) ;EP - RETURN LIST OF PATIENT'S AGGREGATED RACES (HL7 FORMAT)
 ; # OF AGGREGATED RACES^AGG RACE 1^AGG RACE 2^ETC
 ; EX: PT W/0 AGGREGATED RACES => "0"
 ; EX: PT W/2 AGGREGATED RACES => "2^1002-5^2028-9"
 N AGRACE,AGLIST,AGCNT,AGT1,AGT2 S AGLIST=""
 S AGCNT=0 F  S AGCNT=$O(^DPT(AGPATDFN,.02,AGCNT)) Q:'AGCNT  S AGT1=$$GET1^DIQ(10,$P($G(^DPT(AGPATDFN,.02,AGCNT,0)),"^"),3) S:AGT1]"" AGRACE(AGT1)=""
 S AGCNT="" F  S AGCNT=$O(AGRACE(AGCNT)) Q:'$L(AGCNT)  S AGLIST=$G(AGLIST)_$S($L($G(AGLIST)):"^",1:"")_AGCNT
 Q $S('$L($G(AGLIST)):0,1:$L(AGLIST,"^")_"^"_AGLIST)
 ;IHS/OIT/NKD AG*7.1*15
AGETHN(AGPATDFN) ;EP - RETURN LIST OF PATIENT'S AGGREGATED ETHNICITIES (HL7 FORMAT)
 ; # OF AGGREGATED ETHNICITIES^AGG ETHNICITY 1^AGG ETHNICITY 2^ETC
 ; EX: PT W/0 AGGREGATED ETHNICITIES => "0"
 ; EX: PT W/2 AGGREGATED ETHNICITIES => "2^2135-2^2186-5"
 N AGETHN,AGLIST,AGCNT,AGT1,AGT2 S AGLIST=""
 S AGCNT=0 F  S AGCNT=$O(^DPT(AGPATDFN,.06,AGCNT)) Q:'AGCNT  S AGT1=$$GET1^DIQ(10.2,$P($G(^DPT(AGPATDFN,.06,AGCNT,0)),"^"),3) S:AGT1]"" AGETHN(AGT1)=""
 S AGCNT="" F  S AGCNT=$O(AGETHN(AGCNT)) Q:'$L(AGCNT)  S AGLIST=$G(AGLIST)_$S($L($G(AGLIST)):"^",1:"")_AGCNT
 Q $S('$L($G(AGLIST)):0,1:$L(AGLIST,"^")_"^"_AGLIST)
