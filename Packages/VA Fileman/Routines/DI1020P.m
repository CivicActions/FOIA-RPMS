DI1020P ; IHS/OIT/FBD - FILEMAN V22.0 PATCH 1020 ENVIRONMENT CHECKS AND POST-INIT PROCESS;   
 ;;22.0;VA FILEMAN;**1020**;JUN 03,2015;Build 4
 ;
 ;
 ...
 ;
 ;
PRE ;PATCH 1020 PRE-INIT CHECKS AND PROCESSES
 D MES^XPDUTL($$CJ^XLFSTR("Beginning DI v22.0 patch 1020 pre-init process.",80))
 ;
 ...
 ;
 D MES^XPDUTL($$CJ^XLFSTR("DI v22.0 patch 1020 pre-init process complete.",80))
 Q
 ;
 ;
POST ;PATCH 1020 POST-INIT PROCESSES
 D MES^XPDUTL($$CJ^XLFSTR("Beginning DI v22.0 patch 1020 post-init process.",80))
 ;
 D BMES^XPDUTL("Delivering DI v22.0 patch 1020 install message to select users...")
 D MAIL
 ;
 D MES^XPDUTL($$CJ^XLFSTR("DI v22.0 patch 1020 post-init process complete.",80))
 Q
 ;
 ;
MAIL ;----- SEND INSTALL MAIL MESSAGE 
 ...
 ;
 K ^TMP("DI1020MS",$J)
 S ^TMP("DI1020MS",$J,1)=" --- DI v22.0 patch 1020 has been installed into this namespace ---"
 S ^TMP("DI1020MS",$J,2)="  "
 I $G(XPDA) D
 . S %=0
 . F  S %=$O(^XTMP("XPDI",XPDA,"BLD",XPDBLD,1,%)) Q:'%   D
 . . S ^TMP("DI1020MS",$J,(%+2))=" "_^(%,0)
 ;
 S XMSUB=$P($P($T(+1),";",2)," ",3,99)
 S XMDUZ=$S($G(DUZ):DUZ,1:.5)
 S XMTEXT="^TMP(""DI1020MS"",$J,"
 S XMY(1)=""
 S XMY(DUZ)=""
 ;
