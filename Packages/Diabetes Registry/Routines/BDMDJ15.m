BDMDJ15 ; IHS/CMI/LAB -IHS -CUMULATIVE REPORT 03 Feb 2014 5:38 PM ;
...
GENDER ;
 ...
 S V=$G(^XTMP("BDMDM22",BDMJOB,BDMBTH,"AUDIT",BDMPD,20))
...
TYPE ;
 ...
 S X=$$TYPE^BDMDJ16(BDMPD,BDMDMRG,BDMADAT,$G(BDMMULTR),.BDMREGLT)
...
DURDMC ;
 ...
 ;
 S V=$$DURDM^BDMDJ16(BDMPD,BDMDMRG,BDMADAT,$G(BDMMULTR),.BDMREGLT)
...
BMI ;
 ...
 ;S V=$G(^XTMP("BDMDM22",BDMJOB,BDMBTH,"AUDIT",BDMPD,112))
 S H=$$LASTHT^BDMDJ13(BDMPD,BDMRED,"I") S:H]"" H=$$STRIP^XLFSTR($J(H,5,2)," ")
 S W=+$$LASTWT^BDMDJ13(BDMPD,BDMBDAT,BDMRED),W=$S(W=0:"",1:W) I W]"" S W=W\1
 S V=$$BMIEPI^BDMDJ19(H,W)
 ...
 .I $$OW^BDMDJ14(BDMPD,V,BDMADAT) S $P(BDMCUML(40),U,3)=$P(BDMCUML(40),U,3)+1 Q
 .I $$OB^BDMDJ14(BDMPD,V,BDMADAT) S $P(BDMCUML(40),U,4)=$P(BDMCUML(40),U,4)+1
 .I $$SOB^BDMDJ14(BDMPD,V,BDMADAT) S $P(BDMCUML(40),U,7)=$P(BDMCUML(40),U,7)+1,BDMSEVOB=1 Q
HGB ;
...
BPC ;blood pressure control
 ...
 .I S<160&(D<100) S $P(BDMCUML(60),U,6)=$P(BDMCUML(60),U,6)+1 Q  ;Patch 13 2022 change 95 to 100
...
HTN ;410
 ...
 S H=$E($G(^XTMP("BDMDM22",BDMJOB,BDMBTH,"AUDIT",BDMPD,34)))
 ...
 S A=$E($G(^XTMP("BDMDM22",BDMJOB,BDMBTH,"AUDIT",BDMPD,60)))  ;ACE/ARB USAGE
...
TBSTAT ;
 ...
 S V=$G(^XTMP("BDMDM22",BDMJOB,BDMBTH,"AUDIT",BDMPD,71))   ;TB DX
 I $E(V)=1 S BDMTBDX=1,$P(BDMCUML(70),U,8)=$P(BDMCUML(70),U,8)+1
 S V=$G(^XTMP("BDMDM22",BDMJOB,BDMBTH,"AUDIT",BDMPD,70))  ;TB TEST DONE
 S BDMTBTD=0 I 'BDMTBDX,($E(V)=1!($E(V)=2)) S $P(BDMCUML(70),U,4)=$P(BDMCUML(70),U,4)+1,BDMTBTD=1  ;TEST DONE, NO DX
 S BDMTBRE="" I 'BDMTBDX,($E(V)=1!($E(V)=2)) S BDMTBRE=$E($P($$PPD^BDMDJ18(BDMPD,BDMADAT),"||",2))
 I BDMTBDX!(BDMTBRE=1) S $P(BDMCUML(70),U,3)=$P(BDMCUML(70),U,3)+1
 S V=$$TBINH^BDMDJ18(BDMPD,BDMADAT) I $E(V)=1 S $P(BDMCUML(70),U,5)=$P(BDMCUML(70),U,5)+1
 I BDMTBTD=1,$E($P($$PPD^BDMDJ18(BDMPD,BDMADAT),"||",2))=2 S $P(BDMCUML(70),U,6)=$P(BDMCUML(70),U,6)+1
 ;GET DOD
 S X=$$LASTNP^BDMDJ18(BDMPD,BDMADAT)
 I X D
 .I $$DODX^BDMDJ16(BDMPD,BDMDMRG,"I",$G(BDMMULTR),.BDMREGLT)="" Q
 .S D=$$DODX^BDMDJ16(BDMPD,BDMDMRG,"I",$G(BDMMULTR),.BDMREGLT) I D>X Q
 .S $P(BDMCUML(70),U,7)=$P(BDMCUML(70),U,7)+1
TOBACCO ;
 S $P(BDMCUML(80),U,2)=$P(BDMCUML(80),U,2)+1
 S V=$G(^XTMP("BDMDM22",BDMJOB,BDMBTH,"AUDIT",BDMPD,27))
 S V1=$G(^XTMP("BDMDM22",BDMJOB,BDMBTH,"AUDIT",BDMPD,28))
...
ENDS ;
 ...
 S T=$G(^XTMP("BDMDM22",BDMJOB,BDMBTH,"AUDIT",BDMPD,27))  ;TOBACCO USER
...
DMTX ;diabetes treatment
 ...
 S V=$$THERAPY^BDMDJ16(BDMPD,BDM6MBD,BDMRED)  ;^ pieced with each item  or =1 for diet alone or =14 for refused
...
ASPIRIN ;
 ...
 I $E($G(^XTMP("BDMDM22",BDMJOB,BDMBTH,"AUDIT",BDMPD,116)))'=1 G LIPIDAG  ;not cvd
 S $P(BDMCUML(100),U,2)=$P(BDMCUML(100),U,2)+1  ;total with cvd
 S V=$G(^XTMP("BDMDM22",BDMJOB,BDMBTH,"AUDIT",BDMPD,62))
...
LIPIDAG ;lipid agents
 ...
 S V=$E($$STATIN^BDMDJ16(BDMPD,BDM6M,BDMADAT))
 ...
 S C=0
 I $E($G(^XTMP("BDMDM22",BDMJOB,BDMBTH,"AUDIT",BDMPD,116)))=1 D
 ...
 ;
LIPID ;115
 S V=$G(^XTMP("BDMDM22",BDMJOB,BDMBTH,"AUDIT",BDMPD,61))
 S L=$$LDL^BDMDJ18(BDMPD,BDMBDAT,BDMADAT,"I"),L=$P(L,U)
 S T=$$CHOL^BDMDJ18(BDMPD,BDMBDAT,BDMADAT,"I"),T=$P(T,U)
 S T=$$STV^BDMDJ18(T,5,1)
 S L=$$STV^BDMDJ18(L,5,1)
DEP ;
...
EXAMS ;
 ...
 S V=$G(^XTMP("BDMDM22",BDMJOB,BDMBTH,"AUDIT",BDMPD,108))
...
LEAMP ;
 ...
 S V=$G(^XTMP("BDMDM22",BDMJOB,BDMBTH,"AUDIT",BDMPD,230))
...
SYSMEAN(P,BDATE,EDATE) ;EP
 NEW X,Z,C,Y
 S X=$$BPS^BDMDJ13(P,BDATE,EDATE,"I")
 I X="" Q ""
