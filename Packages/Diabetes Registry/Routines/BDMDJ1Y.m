BDMDJ1Y ; IHS/CMI/LAB - IHS Diabetes Audit 2022 ;
...
BEGIN ;EP - called from option
 ...
 W:$D(IOF) @IOF
REGASK ;
 W !!!,$$CTR("ASSESSMENT OF DIABETES CARE, 2022")
...
TYPE ;
 S BDMSTP=0
 K ^XTMP("BDMDM22",BDMJOB,BDMBTH),^TMP($J,"PATS")
 S BDMTYPE=""
...
IF ;PEP - called from BDM indivdual or epi
 S BDMSTP=0
 S BDMPREP=6
 D QUALCHK^BDMDJ1J G:BDMSTP TYPE
ZIS ;
DEMO ;
 S BDMDEMO="I"
 I BDMTYPE'="P" D DEMOCHK^BDMUTL(.BDMDEMO) I BDMDEMO=-1 G TYPE
 I BDMTYPE="P" S BDMDEMO="I"
 I BDMPREP=2 S XBRP="",XBRC="^BDMDJ10",XBRX="XIT^BDMDJ1",XBNS="BDM" D ^XBDBQUE,XIT Q
 W ! S DIR(0)="S^P:PRINT Output;B:BROWSE Output on Screen",DIR("A")="Do you wish to",DIR("B")="P" K DA D ^DIR K DIR
 I $D(DIRUT) D XIT Q
 S BDMOPT=Y
 I Y="B" D BROWSE,XIT Q
 S XBRP="^BDMDJ1P",XBRC="^BDMDJ10",XBRX="XIT^BDMDJ1",XBNS="BDM"
 D ^XBDBQUE
 D XIT
 Q
BROWSE ;
 S XBRP="VIEWR^XBLM(""^BDMDJ1P"")"
 S XBRC="^BDMDJ10",XBRX="XIT^BDMDJ1",XBIOP=0 D ^XBDBQUE
 Q
IAEP ;EP
 S BDMTYPE="P"
 S BDMPREP=1
 ...
 G IF
P ;
 S BDMSTP=0 K ^XTMP("BDMDM22",BDMJOB,BDMBTH),^TMP($J,"PATS")
 S BDMBEN=3
P1 ;
 K DIC S DIC="^AUPNPAT(",DIC(0)="AEMQ" D ^DIC K DIC
 I Y=-1,'$D(^XTMP("BDMDM22",BDMJOB,BDMBTH,"PATS")) W !,"No patients selected" S BDMSTP=1 Q
 I Y=-1 Q
 S ^XTMP("BDMDM22",BDMJOB,BDMBTH,"PATS",+Y)=""
 G P1
E ;
 D ^BDMDJ2
 Q
 ;
S ; 
 K ^XTMP("BDMDM22",BDMJOB,BDMBTH),^TMP($J,"PATS") S BDMSTP=0
...
PREG ;
 ...
 .Q:$P(^DPT(P,0),U,2)'="F"
 .;W ".",P
 .I $$PREG^BDMDJ1B(P,BDMBDAT,BDMADAT,1,1,BDMBDAT,BDMADAT) K ^TMP($J,"PATS",X)
...
RAND ;random sample or not
 ...
 G:$D(DIRUT) C
 I Y="A" S C=0 F  S C=$O(^TMP($J,"PATS",C)) Q:C'=+C  S X=$O(^TMP($J,"PATS",C,0)),^XTMP("BDMDM22",BDMJOB,BDMBTH,"PATS",X)=""
 ...
 I $D(DIRUT) S BDMSTP=1 Q
 S C=0 F N=1:1:BDMCNT Q:C=Y  S I=$R(BDMCNT) I I,$D(^TMP($J,"PATS",I)) S X=$O(^TMP($J,"PATS",I,0)),^XTMP("BDMDM22",BDMJOB,BDMBTH,"PATS",X)="",C=C+1 K ^TMP($J,"PATS",I,X)
 K ^TMP($J,"PATS")
 Q
...
XIT1 ;
 ...
 K ^XTMP("BDMDM22",BDMJOB,BDMBTH),BDMJOB,BDMBTH
XIT ;
...
WRITEF ;EP write flat file
 ...
 S I=$O(^BDMRECD("B","DM AUDIT 2022 EXPORT RECORD",0))
 ...
 S XBMED="F",XBFN=BDMFILE,XBTLE="SAVE OF DM AUDIT 2022 EXPORT RECORDS GENERATED BY -"_$P(^VA(200,DUZ,0),U)
...
BDMG(BDMJOB,BDMBTH,BDMDMRG,BDMADAT,BDMTYPE,BDMSTMP,BDMPDP,BDMCOM,BDMRAND,BDMRCNT,BDMCMS,BDMSTAT,BDMPREP,BDMFILE,BDMDSP,BDMGIEN,BDMSDPI,BDMSDPG,BDMPPN,BDMDUZ2,BDMDEMO,BDMBEN,BDMDQ,BDMACTI,BDMPTAX) ;PEP - gui call
 ...
 N BDMOPT  ;maw
 S BDMOPT="2022 Data Quality Report"
 S BDMPREP=6
 D NOW^%DTC
 S BDMNOW=$G(%)
 K DD,D0,DIC
 ...
 D ^XBFMK
 K ZTSAVE S ZTSAVE("*")=""
 S ZTIO="",ZTDTH=$$NOW^XLFDT,ZTRTN="GUIEP^BDMDJ1",ZTDESC="GUI DM AUDIT" D ^%ZTLOAD
 D XIT
 Q
GUIEP ;EP - called from taskman
 ...
 D GUIR^XBLM("^BDMDJ1P","^TMP($J,""BDMDM22"",")
 Q:$G(BDMDSP)  ;quit if to screen
 S X=0,C=0 F  S X=$O(^TMP($J,"BDMDM22",X)) Q:X'=+X  D
 .S BDMDATA=^TMP($J,"BDMDM22",X)
