BDMDL17 ; IHS/CMI/LAB - 2026 DIABETES AUDIT 03 Nov 2015 9:29 AM ; 
...
DIETEDUC(P,BDATE,EDATE)  ;EP
 ...
 ..S X=0 F  S X=$O(^AUPNVCPT("AD",V,X)) Q:X'=+X  S Z=$$VAL^XBDIQ1(9000010.18,X,.01) I Z=97802!(Z=97803)!(Z=97804)!(Z="G0270")!(Z="G0271") S RD=1,BDMV=BDMV_"RD: CPT "_Z_" "_$$DATE^BDMS9B1($$VD^APCLV(V))_" "
...
EXEDUC(P,BDATE,EDATE) ;EP
 ...
 I G Q "1  Yes "_T_"  "_$$DATE^BDMS9B1($$VD^APCLV($P(^AUPNVPED(I,0),U,3)))
 K BDM
...
OTHEDUC(P,BDATE,EDATE) ;EP
 ...
 .I $P(T,"-",1)]"",$$SNOMED^BDMUTL(2026,"PXRM DIABETES",$P(T,"-",1)) S G="1  Yes "_T_"  "_$$DATE^BDMS9B1($$VD^APCLV($P(^AUPNVPED(I,0),U,3),"I"))
 I G Q G
 ;now check for a cpt code
 S G=$$LASTCPTT^BDMAPIU(P,BD,ED,"DM AUDIT OTHER EDUC CPTS","E")
 I G Q "1  Yes  "_$$DATE^BDMS9B1($P(G,U,1))_" "_$P(G,U,2)
 ;now check for clinic visit of A1, 98
 K ^TMP($J,"A")
 ...
 .I B="A1"!(B=98) S G="1  Yes  "_$$DATE^BDMS9B1($$VD^APCLV(V))_" Clinic "_B
 I G Q G
...
DFE(P,BDATE,EDATE,F,R) ;EP - FOOT EXAM
 ...
 I F="E" Q "2  No"  ;don't check if in audit  ;patch 16
 I F="D" Q $P(LASTI,U,1)
...
ADA(V) ;any ada other than 9991
 I '$G(V) Q ""
 NEW X,Y,Z,G
 S G="",X=0 F  S X=$O(^AUPNVDEN("AD",V,X)) Q:X'=+X!(G)  S Y=$P($G(^AUPNVDEN(X,0)),U) I Y,$D(^AUTTADA(Y,0)),$P(^AUTTADA(Y,0),U)'=9991,$P(^AUTTADA(Y,0),U)'="D9991" S G=1
 Q G
...
EYE(P,BDATE,EDATE,F,R) ;EP
 ...
 ;
 ;
 S X=$$LASTCPTT^BDMAPIU(P,BDATE,EDATE,"BGP DM RETINAL EXAM CPTS","E")
 I $P(X,U)>$P(LASTI,U) S LASTI=$P(X,U)_U_"1  Yes  "_$$DATE^BDMS9B1($P(X,U))_" "_$P(X,U,2) ;I F="H"!(F="D") Q LASTI
 K BDMV,BDMY
 I F="E",LASTI]"" Q $P(LASTI,U,2)
 S X=$$LASTCPTT^BDMAPIU(P,BDATE,EDATE,"BGP DM EYE EXAM CPTS","E")
 I $P(X,U)>$P(LASTI,U) S LASTI=$P(X,U)_U_"1  Yes  "_$$DATE^BDMS9B1($P(X,U))_" "_$P(X,U,2) ;I F="H"!(F="D") Q LASTI
 K BDMV,BDMY
 ;
 I F="E",LASTI]"" Q $P(LASTI,U,2)  ; if in audit and has exam display it
 I F="E" Q "2  No"  ;don't check if in audit  ;patch 18
 I F="D" Q $P(LASTI,U)
 ...
 Q "2  No"_$S(G:" - Not Medically Indicated",1:"")
 ;
DENTAL(P,BDATE,EDATE,F) ;EP - P18 GPRA DEV DOCUMENTED DENTAL VISIT
 NEW BDMG,BDMC,X,%,G,V,PROV,T
 K BDMG
 I $G(F)="" S F="E"
 S %=P_"^LAST EXAM 30;DURING "_$$FMTE^XLFDT(BDATE)_"-"_$$FMTE^XLFDT(EDATE),E=$$START1^APCLDF(%,"BDMG(")
 I $D(BDMG(1)) Q "1  Yes  "_$$DATE^BDMS9B1($P(BDMG(1),U))_" Dental Exam"
 K BDMG
 S BDMC="",%=P_"^LAST ADA 0000;DURING "_$$FMTE^XLFDT(BDATE)_"-"_$$FMTE^XLFDT(EDATE),E=$$START1^APCLDF(%,"BDMG(")
 I $D(BDMG(1)) Q "1  Yes  "_$$DATE^BDMS9B1($P(BDMG(1),U))_" RPMS DENTAL 0000"
 K BDMG
 S %=P_"^LAST ADA 0190;DURING "_$$FMTE^XLFDT(BDATE)_"-"_$$FMTE^XLFDT(EDATE),E=$$START1^APCLDF(%,"BDMG(")
 I $D(BDMG(1)) Q "1  Yes  "_$$DATE^BDMS9B1($P(BDMG(1),U))_" RPMS DENTAL 0190"
 K BDMG
 S BDMG(1)=$$LASTCPTT^BDMAPIU(P,BDATE,EDATE,"BGP DENTAL VISIT CPT CODES","E")
 I BDMG(1)]""  Q "1  Yes  "_$$DATE^BDMS9B1($P(BDMG(1),U))_" "_$P(BDMG(1),U,2)
 K BDMG
 S BDMG=$$LASTDX^BDMAPIU(P,"BGP DENTAL VISIT DXS",BDATE,EDATE)
 I BDMG Q "1  Yes  "_$$DATE^BDMS9B1($P(BDMG,U,3))_" DX: "_$P(BDMG,U,2)
 ;
 ...
 I F="E" Q "2  No"
 ;
 ;look for dental clinic visit/provider for health summary only
 K T
 S T="T"
 D ALLV^BDMAPIU(P,BDATE,EDATE,.T)
 ;reorder by date of visit/reverse order
