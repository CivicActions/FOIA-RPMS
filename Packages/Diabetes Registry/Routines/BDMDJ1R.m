BDMDJ1R ; IHS/CMI/LAB -IHS -CUMULATIVE REPORT 03 Feb 2014 5:38 PM ;
...
SDPI16 ;EP
 ...
 ;
 ;PNEUMO
 S BDMCUML(120)="Immunizations: Pneumovax/PPSV23"
 S BDMCUML(120,1)="Pneumovax/PPSV23 vaccine - ever"
 ;TETANUS
...
PROCESS ;
 S BDMNOGO=0
 S BDMPD=0 F  S BDMPD=$O(^XTMP("BDMDM22",BDMJOB,BDMBTH,"AUDIT",BDMPD)) Q:BDMPD'=+BDMPD  D CUML1
 Q
 ;
CUML1 ;
ASPIRIN ;
 I $$DODX^BDMDJ16(BDMPD,BDMDMRG,"I",$G(BDMMULTR),.BDMREGLT)>BDMADAT S BDMNOGO=BDMNOGO+1 Q
 S V=$G(^XTMP("BDMDM22",BDMJOB,BDMBTH,"AUDIT",BDMPD,116))  ;CVD
 I $E(V)=1 S $P(BDMCUML(10,1),U,3)=$P(BDMCUML(10,1),U,3)+1  ;CVD DENOM
 ;CVD AND ASPIRIN
 S A=$G(^XTMP("BDMDM22",BDMJOB,BDMBTH,"AUDIT",BDMPD,62))
...
BPC ;blood pressure control
 ...
 S S=$$SYSMEAN^BDMDJ15(BDMPD,BDMRBD,BDMRED)
 S D=$$DIAMEAN^BDMDJ15(BDMPD,BDMRBD,BDMRED)
 D
 .I S=""!(D="") Q
 .I S<140&(D<90) S $P(BDMCUML(20,1),U,2)=$P(BDMCUML(20,1),U,2)+1 Q
CKD ;
 I $$AGE^AUPNPAT(BDMPD,BDMADAT)<18 G DENT
 S V=$G(^XTMP("BDMDM22",BDMJOB,BDMBTH,"AUDIT",BDMPD,79)),V=$$STV^BDMDJ18($P(V,U,2),5)
 S T=$P($G(^XTMP("BDMDM22",BDMJOB,BDMBTH,"AUDIT",BDMPD,92)),U,5)  ;type of test
 S Q=$G(^XTMP("BDMDM22",BDMJOB,BDMBTH,"AUDIT",BDMPD,92))
 S R=$S($P(Q,U,6)]"":$P(Q,U,6),1:$P(Q,U,2))  ;value/result
 ;CONVERT RESULT LIKE IN WEB EXPORT
 S S=$$AS^BDMDJ18(R),S=$$STV^BDMDJ18(S,8)
 ;I S="" S V=0  ;X=$E(X,1,5)
...
HEPC ;
 ...
 .S V=$G(^XTMP("BDMDM22",BDMJOB,BDMBTH,"AUDIT",BDMPD,222))  ;HEPC DX
 .S S=$G(^XTMP("BDMDM22",BDMJOB,BDMBTH,"AUDIT",BDMPD,223))
 .I $E(V)="1"!($E(S)=1) S $P(BDMCUML(95,1),U,2)=$P(BDMCUML(95,1),U,2)+1  ;WITH SCREEN OR DX
IMM ;
HEPB ;
 S V=$G(^XTMP("BDMDM22",BDMJOB,BDMBTH,"AUDIT",BDMPD,115))
...
NUTR ;
 ...
 S G=0,V=$G(^XTMP("BDMDM22",BDMJOB,BDMBTH,"AUDIT",BDMPD,44))
...
TOB ;
 ...
 S V=$G(^XTMP("BDMDM22",BDMJOB,BDMBTH,"AUDIT",BDMPD,215))
 I +V=1!(+V=2) S $P(BDMCUML(170,1),U,2)=$P(BDMCUML(170,1),U,2)+1
TB ;
 S T=$G(^XTMP("BDMDM22",BDMJOB,BDMBTH,"AUDIT",BDMPD,71))
 S $P(BDMCUML(180,1),U,3)=$P(BDMCUML(180,1),U,3)+1
 S V=$G(^XTMP("BDMDM22",BDMJOB,BDMBTH,"AUDIT",BDMPD,70))
