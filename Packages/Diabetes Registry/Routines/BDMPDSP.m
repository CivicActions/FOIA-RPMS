BDMPDSP ; IHS/CMI/LAB - display dm ;
 ;;2.0;DIABETES MANAGEMENT SYSTEM;**2,3,4,17**;JUN 14, 2007;Build 138
 ;
EN ;
 W:$D(IOF) @IOF
 W !!,$$CTR("*** Print Prediabetes Patient Care Supplement ***"),!!
...
ZIS ;
 ...
 S XBRP="PRINT^BDMPDSP",XBRC="",XBRX="EXIT^BDMPDSP",XBNS="BDM;DFN"
 D ^XBDBQUE
 D EXIT
 Q
BROWSE ;
 S XBRP="VIEWR^XBLM(""PRINT^BDMPDSP"")"
 S XBRC="",XBRX="EXIT^BDMPDSP",XBIOP=0 D ^XBDBQUE
 Q
EXIT ;
...
OUTPUT S BDMSCVD="S:Y]"""" Y=+Y,Y=$E(Y,4,5)_""/""_$S($E(Y,6,7):$E(Y,6,7)_""/"",1:"""")_$E(Y,2,3)"
 K ^TMP("APCHS",$J)
 S BDMSPAT=DFN
 D EP2^BDMS9D1(DFN) ;gather up data
W ;write out array
 ;W:$D(IOF) @IOF
 K BDMQUIT
