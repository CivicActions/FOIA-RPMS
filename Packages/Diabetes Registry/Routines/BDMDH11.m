BDMDH11 ; IHS/CMI/LAB -IHS -CUMULATIVE REPORT 10 Oct 2014 9:50 AM ; 
...
IMM ;
 ...
 ;TDAP EVER
 S V=$G(^XTMP("BDMDM20",BDMJOB,BDMBTH,"AUDIT",BDMPD,216))
...
QUAN ;
 ...
 S V=$E($G(^XTMP("BDMDM20",BDMJOB,BDMBTH,"AUDIT",BDMPD,92)))  ;test done?
 ...
 S G=$P($G(^XTMP("BDMDM20",BDMJOB,BDMBTH,"AUDIT",BDMPD,79)),U,2)  ;egfr value
 S H=0
 I $$AGE^AUPNPAT(BDMPD,BDMADAT)>17 D
 .S G=$$AS^BDMDH18(G)
 .S G=$$STV^BDMDH18(G,5,1)
 .I G="" Q
...
GFR ;
 ...
 S V=$G(^XTMP("BDMDM20",BDMJOB,BDMBTH,"AUDIT",BDMPD,79))
...
TCHOL ;
 ...
 S V=$$CHOL^BDMDH18(BDMPD,BDMBDAT,BDMADAT,"I")
...
LDL ;
 ...
 S V=$$LDL^BDMDH18(BDMPD,BDMBDAT,BDMADAT,"I")
 I V="" S $P(BDMCUML(190),U,7)=$P(BDMCUML(190),U,7)+1 G HDL
 S V=$P(V,U)
 ;S V=$$STV^BDMDH18(V,4)
 S V=$$STV^BDMDH18(V,5,1) I $E(V)'=+$E(V)!(+V=0) S $P(BDMCUML(190),U,8)=$P(BDMCUML(190),U,8)+1 G HDL
...
HDL ;
 ...
 S V=$$HDL^BDMDH18(BDMPD,BDMBDAT,BDMADAT,"I")
 S S=$P(^DPT(BDMPD,0),U,2)  ;GENDER
 I S="F" D
 .S $P(BDMCUML(195),U,2)=$P(BDMCUML(195),U,2)+1
 .I V="" S $P(BDMCUML(195),U,5)=$P(BDMCUML(195),U,5)+1 Q
 .S V=$P(V,U)
 .S V=$$STV^BDMDH18(V,5,1) I $E(V)'=+$E(V)!(+V=0) S $P(BDMCUML(195),U,5)=$P(BDMCUML(195),U,5)+1 Q
 .S V=$P(V,"."),V=$$STV^BDMDH18(V,5,1)
 ...
 .S V=$P(V,U)
 .S V=$$STV^BDMDH18(V,5,1) I $E(V)'=+$E(V)!(+V=0) S $P(BDMCUML(195),U,9)=$P(BDMCUML(195),U,9)+1 Q  ;unable to determine result, not a number
 .S V=$P(V,"."),V=$$STV^BDMDH18(V,5,1)
...
TRIG ;
 ...
 S V=$$TRIG^BDMDH18(BDMPD,BDMBDAT,BDMADAT,"I")
 I V="" S $P(BDMCUML(200),U,7)=$P(BDMCUML(200),U,7)+1 G ACE
 S V=$P(V,U)
 S V=$$STV^BDMDH18(V,5,1) I $E(V)'=+$E(V) S $P(BDMCUML(200),U,5)=$P(BDMCUML(200),U,5)+1 G ACE ;unable to determine result, not a number
...
ACE ;110 title^total pts^total pts with protein^# of those on ace^# with htn^# of those on ace"
 ...
 S P=$G(^XTMP("BDMDM20",BDMJOB,BDMBTH,"AUDIT",BDMPD,92))  ;URINE PROTEIN VALUE
 S H=$E($G(^XTMP("BDMDM20",BDMJOB,BDMBTH,"AUDIT",BDMPD,34)))
 S A=$E($G(^XTMP("BDMDM20",BDMJOB,BDMBTH,"AUDIT",BDMPD,60)))
...
CVD ;USE 250
 ...
 S V=$G(^XTMP("BDMDM20",BDMJOB,BDMBTH,"AUDIT",BDMPD,116))
...
COMBINED ;USE 260
 ...
 I $E($G(^XTMP("BDMDM20",BDMJOB,BDMBTH,"AUDIT",BDMPD,300)))=3 G E  ;statin allergy
 S V=$G(^XTMP("BDMDM20",BDMJOB,BDMBTH,"AUDIT",BDMPD,118))
