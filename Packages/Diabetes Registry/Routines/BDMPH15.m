BDMPH15 ; IHS/CMI/LAB -IHS -CUMULATIVE REPORT ;
...
CUML ;EP
 ...
 ;
PROCESS ;
 S BDMPG=0 F  S BDMPG=$O(^XTMP("BDMPH1",BDMJOB,BDMBTH,"AUDIT",BDMPG)) Q:BDMPG'=+BDMPG  D CUML1
 Q
 ;
CUML1 ;
GENDER ;
 ...
 S V=$G(^XTMP("BDMPH1",BDMJOB,BDMBTH,"AUDIT",BDMPG,20))
 S P=$S($E(V)="F":3,$E(V)="M":4,1:5)
 S $P(BDMCUML(10),U,P)=$P(BDMCUML(10),U,P)+1
AGE ;
 S V=$$AGE^AUPNPAT(BDMPG,BDMADAT)
...
BMI ;
 ...
 S V=$G(^XTMP("BDMPH1",BDMJOB,BDMBTH,"AUDIT",BDMPG,112))
 S $P(BDMCUML(40),U,2)=$P(BDMCUML(40),U,2)+1
 D
 .I V="" S $P(BDMCUML(40),U,5)=$P(BDMCUML(40),U,5)+1 Q
 .I $$OW^BDMDH14(BDMPG,V,BDMADAT) S $P(BDMCUML(40),U,3)=$P(BDMCUML(40),U,3)+1
 .I $$OB^BDMDH14(BDMPG,V,BDMADAT) S $P(BDMCUML(40),U,4)=$P(BDMCUML(40),U,4)+1
BPC ;blood pressure control
 ;take last 3 bp's and get mean systolic and mean diastolic
 S $P(BDMCUML(60),U,2)=$P(BDMCUML(60),U,2)+1
 S S=$$SYSMEAN(BDMPG,BDMRBD,BDMRED)
 S D=$$DIAMEAN(BDMPG,BDMRBD,BDMRED)
 D
...
TOBACCO ;
 ...
 S V=$G(^XTMP("BDMPH1",BDMJOB,BDMBTH,"AUDIT",BDMPG,27))
 S V1=$G(^XTMP("BDMPH1",BDMJOB,BDMBTH,"AUDIT",BDMPG,28))
...
DMTX ;diabetes treatment
 ...
 S V=$$THERAPY^BDMPH16(BDMPG,BDM6MBD,BDMRED)
...
ASPIRIN ;
 ...
 S V=$G(^XTMP("BDMPH1",BDMJOB,BDMBTH,"AUDIT",BDMPG,62))
...
ACE ;110 title^total pts^total pts with protein^# of those on ace^# with htn^# of those on ace"
 ...
 ;S P=$G(^XTMP("BDMPH1",BDMJOB,BDMBTH,"AUDIT",BDMPG,94))
 S H=$G(^XTMP("BDMPH1",BDMJOB,BDMBTH,"AUDIT",BDMPG,34))
 S A=$G(^XTMP("BDMPH1",BDMJOB,BDMBTH,"AUDIT",BDMPG,60))
...
LIPID ;115
 S V=$G(^XTMP("BDMPH1",BDMJOB,BDMBTH,"AUDIT",BDMPG,61))
 S L=$$LDL^BDMDH18(BDMPG,BDMBDAT,BDMADAT,"I"),L=$P(L,U)
 S T=$$CHOL^BDMDH18(BDMPG,BDMBDAT,BDMADAT,"I"),T=$P(T,U)
...
SYSMEAN(P,BDATE,EDATE) ;EP
 NEW X S X=$$BPS^BDMDH13(P,BDATE,EDATE,"I")
 I X="" Q ""
