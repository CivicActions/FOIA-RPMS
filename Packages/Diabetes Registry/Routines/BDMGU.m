BDMGU ; cmi/anch/maw - BDM DMS GUI Utilities ;
 ;;2.0;BDM DIABETES MANAGEMENT SYSTEM;**1,4,7,8,13,17**;JUN 14, 2007;Build 138
 ;
 ;
 ;
...
PATADO(PIEN) ;-- ado return
 ...
 . Q:'$D(^AUPNPAT(BDMPI,41,DUZ(2)))  ;20231019 maw p17 don't display if no local chart
...
DMG(BDMRET,BDMSTR) ;-- get patient demographics
 S X="MERR^BDMGU",@^%ZOSF("TRAP") ; m error trap
 ;D EN^XBNEW("DMGLOG^BDMGU","BDMPAT")
...
ERR(ERR) ; BMX ADO SCHEMA ERROR PROCESSOR
 ...
 Q
 ;
BUSA(RETVAL,BDMSTR) ;call BUSA
 N BDMCL,BDMDS,BDMPAR,P,BDMI,BDMTP,BDMR
 S BDMI=0
 S P="|"
 K ^BDMTMP($J)
 S RETVAL="^BDMTMP("_$J_")"
 S ^BDMTMP($J,BDMI)="T00080BUSA"_$C(30)
 S BDMCL=$P(BDMSTR,P)
 S BDMDS=$P(BDMSTR,P,2)
 S BDMPR(1)=$P(BDMSTR,P,3)_U_U
 S BDMTP=$P(BDMSTR,P,4)
 S BDMR=$P(BDMSTR,P,5)
 I $G(BDMR)="P" D  Q
 . D LOG^BUSAAPI("R",BDMR,BDMTP,BDMCL,BDMDS,"BDMPR")
 . S ^BDMTMP($J,BDMI+1)=$C(31)
 D LOG^BUSAAPI("R",BDMR,BDMTP,BDMCL,BDMDS)
 S ^BDMTMP($J,BDMI+1)=$C(31)
 Q
 ;
