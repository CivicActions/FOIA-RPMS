BDMDK1E ; IHS/CMI/LAB - IHS Diabetes Audit 2023 ;
...
REGASK ;
 ...
 W !!,"This option is used to run the 2023 Electronic Diabetes Audit for a"
...
IF ;PEP - called from BDM indivdual or epi
 S BDMSTP=0
 K DIR
 S DIR(0)="S^1:Print Individual Reports;2:Create AUDIT EXPORT file;3:Cumulative Audit Only;4:Both Individual and Cumulative Audits;5:SDPI Key Measures Report (2015 Version);6:SDPI Key Measures Report (2017 Version)"
...
ZIS ;
 ...
 I BDMPREP'=2 S XBRP="^BDMDK1P",XBRC="EAUDIT^BDMDK10",XBRX="XIT^BDMDK1",XBNS="BDM"
 I BDMPREP=2 S XBRP="",XBRC="EAUDIT^BDMDK10",XBRX="XIT^BDMDK1",XBNS="BDM"
 D ^XBDBQUE
 D XIT
 Q
P ;
...
WRITEF ;EP write flat file
 D WRITEF^BDMDK1
 Q
BDMG(BDMJOB,BDMBTH,BDMDMRG,BDMADAT,BDMTYPE,BDMSTMP,BDMPDP,BDMCOM,BDMRAND,BDMRCNT,BDMCMS,BDMSTAT,BDMPREP,BDMFILE,BDMDSP,BDMGIEN,BDMSDPI,BDMSDPG,BDMDUZ2,BDMDEMO,BDMDQ,BDMPTAX) ;PEP - gui call
 F X="BDMJOB","BDMBTH","BDMDMRG","BDMADAT","BDMTYPE","BDMSTMP","BDMPDP","BDMCOM","BDMRAND","BDMRCNT","BDMCMS","BDMSTAT","BDMPREP","BDMFILE","BDMDSP","BDMDUZ2","BDMDEMO" S @X=$G(@X)
 I $G(BDMJOB)="" S BDMIEN=-1 Q
 I $G(BDMBTH)="" S BDMIEN=-1 Q
 I $G(BDMADAT)="" S BDMIEN=-1 Q
 ;I $G(BDMTYPE)="" S BDMIEN=-1 Q
 I $G(BDMPREP)="" S BDMIEN=-1 Q
 I BDMTYPE="E" S BDMTAXI=BDMCOM
 ...
 N BDMOPT  ;maw
 S BDMOPT="Run the 2023 Audit w/predefined set of Pts"  ;maw
 D NOW^%DTC
 S BDMNOW=$G(%)
 K DD,D0,DIC
 S X=DUZ_"."_BDMBTH
 ...
 S ZTIO="",ZTDTH=$$NOW^XLFDT,ZTRTN="GUIEP^BDMDK1E",ZTDESC="GUI DM AUDIT" D ^%ZTLOAD
 D XIT
 Q
...
TEST ;
 ...
 F X=1:1:10 S ^XTMP("BDMDM23",BDMJOB,BDMBTH,"PATS",X)=""
 D BDMG^BDMDK1(BDMJOB,BDMBTH,1,DT,"P","","","","","","","",4,"TESTEPI",.BDMIEN)
 Q
