BDMDO1P ; IHS/CMI/LAB - 2025 DIABETES AUDIT PRINT
 ;;2.0;DIABETES MANAGEMENT SYSTEM;**18**;JUN 14, 2007;Build 147
 ;
 ;
 D LOG^BUSAAPI("O","P","P",$S($G(XQY0)]"":$P(XQY0,U),1:"BDMDN1P"),"DM 2025 AUDIT REPORT "_$S(BDMPREP=6:"QUALITY CHECK RPT",1:""))
 S BDMQUIT=0,BDMPG=0,BDMIOSL=$S($G(BDMGUI):57,1:IOSL)
 I BDMPREP=3 G CUML
 I BDMPREP=5 G CUML
 I BDMPREP=6 G CUML
 ;print ind audits first
 S BDMPD=0,BDMGUIC=0 F  S BDMPD=$O(^XTMP("BDMDM25",BDMJOB,BDMBTH,"AUDIT",BDMPD)) Q:BDMPD'=+BDMPD!(BDMQUIT)  D
 .I $G(BDMGUI),BDMGUIC W !,"ZZZZZZZ",!  ;maw
 .I BDMGUIC W:$D(IOF) @IOF
 .S BDMGUIC=1
 .S BDMPG=BDMPG+1 W:$G(BDMGUI) !! W "IHS Diabetes Care and Outcomes Audit, 2025    DATE RUN: "_$$DATE^BDMS9B1(DT)_"   Page: "_BDMPG
 ...
 .W !!,"Vital Statistics"
 .S X=$$I(30)
 .W !!?2,"Height (last ever): ",X  ;$J($P(X," "),2,0)_" "_$P(X," ",2,99)  ;ROUND HT KS 1208 2 DECIMALS THROUGHOUT 2025 PER K SHEFF
 .I $Y>(BDMIOSL-4) D PAGE Q:BDMQUIT
 ...
 .I $Y>(BDMIOSL-1) D PAGE Q:BDMQUIT
 .W !?2,"Pneumococcal [PCV15, PCV20, PCV21, or PPSV23] (ever): ",?50,$$I(66)
 .I $Y>(BDMIOSL-1) D PAGE Q:BDMQUIT
 ...
 .I $$AGE^AUPNPAT(BDMPD,BDMADAT)<40!($E($G(^XTMP("BDMDM25",BDMJOB,BDMBTH,"AUDIT",BDMPD,300)))=3) W !?5,"This is only calculated for patients 40 years of age and older without",!?5,"a statin allergy or intolerance." I 1
...
N1 .W !!,"Local Questions ",$$LOCN^BDMDO10(BDMPD,BDMDMRG)
 .W !!?2,"Select one:"
 .W !?2,"Text: ",$$LOCT^BDMDO10(BDMPD,BDMDMRG)
 .I $E(IOST,1,2)'="P-" W !! S DIR(0)="E" D ^DIR K DIR
CUML ;
 I BDMPREP=4!(BDMPREP=3) D CUML^BDMDO14
 I BDMPREP=5 D SDPI16^BDMDO1U
 I BDMPREP=6 D QUALCHK^BDMDO1V
DONE ;
 K ^TMP($J)
 K ^XTMP("BDMTAX",BDMJOB,BDMBTH)
 K ^XTMP("BDMDM25",BDMJOB,BDMBTH)
 K ^XTMP("BDMDM25 ERRORS",BDMJOB,BDMBTH),BDMJOB,BDMBTH
 Q
I(I) ;
 Q $G(^XTMP("BDMDM25",BDMJOB,BDMBTH,"AUDIT",BDMPD,I))
 ;
PAGE ;
 Q:$G(BDMDSP)
 ...
 I $G(BDMGUI) W !!
 S BDMPG=BDMPG+1
 W "IHS Diabetes Care and Outcomes Audit, 2025    DATE RUN: "_$$DATE^BDMS9B1(DT)_"   Page: "_BDMPG
