BDMPH1P ; IHS/CMI/LAB - 2003 DIABETES AUDIT PRINT ;
 ...
 S BDMPG=0,BDMGUIC=0 F  S BDMPG=$O(^XTMP("BDMPH1",BDMJOB,BDMBTH,"AUDIT",BDMPG)) Q:BDMPG'=+BDMPG!(BDMQUIT)  D
 .I $G(BDMGUI),BDMGUIC W !,"ZZZZZZZ"
 .S BDMGUIC=1
 .W:$D(IOF) @IOF
 .W !,$$CTR("ASSESSMENT PREDIABETES/METABOLIC SYNDROME CARE, FY 2020",80)
 ...
 .W !?1,"1 IFG - ",$$I(200)
 .S X=200 F  S X=$O(^XTMP("BDMPH1",BDMJOB,BDMBTH,"AUDIT",BDMPG,X)) Q:X>200.99!(X="")  D
 ..W !?3,^XTMP("BDMPH1",BDMJOB,BDMBTH,"AUDIT",BDMPG,X)
 .W !?1,"2 IGT - ",$$I(210)
 .S X=210 F  S X=$O(^XTMP("BDMPH1",BDMJOB,BDMBTH,"AUDIT",BDMPG,X)) Q:X>210.99!(X="")  D
 ..W !?3,^XTMP("BDMPH1",BDMJOB,BDMBTH,"AUDIT",BDMPG,X)
 .W !?1,"3 METABOLIC SYNDROME - ",$$I(220)
 .S X=220 F  S X=$O(^XTMP("BDMPH1",BDMJOB,BDMBTH,"AUDIT",BDMPG,X)) Q:X>220.99!(X="")  D
 ..W !?3,^XTMP("BDMPH1",BDMJOB,BDMBTH,"AUDIT",BDMPG,X)
 .W !?1,"OTHER ABNORMAL GLUCOSE - ",$$I(230)
 .S X=230 F  S X=$O(^XTMP("BDMPH1",BDMJOB,BDMBTH,"AUDIT",BDMPG,X)) Q:X>230.99!(X="")  D
 ..W !?3,^XTMP("BDMPH1",BDMJOB,BDMBTH,"AUDIT",BDMPG,X)
 .W !?1,"CMS Register DX: ",$$I(22)_" "_$$I(24)
 .I $D(^XTMP("BDMPH1",BDMJOB,BDMBTH,"AUDIT",BDMPG,25)) W !?1,^(25)
 .I $D(^XTMP("BDMPH1",BDMJOB,BDMBTH,"AUDIT",BDMPG,26)) W !?1,^(26)
...
CUML ;
 I BDMPREP=2!(BDMPREP=3) D CUML^BDMPH14
DONE ;
 K ^TMP($J)
 K ^XTMP("BDMPH1",BDMJOB,BDMBTH),BDMJOB,BDMBTH
 Q
I(I) ;
 Q ^XTMP("BDMPH1",BDMJOB,BDMBTH,"AUDIT",BDMPG,I)
 ;
PAGE ;
