BDMPI1 ; IHS/CMI/LAB - IHS PRE Diabetes Audit 2015 ;
 ;;2.0;DIABETES MANAGEMENT SYSTEM;**14**;JUN 14, 2007;Build 80
 ;
BEGIN ;EP - called from option
 D TAXCHK^BDMPI19  ;
 W:$D(IOF) @IOF
 W !!!,$$CTR("ASSESSMENT OF PREDIABETES/METABOLIC SYNDROME CARE, 2021")
...
TYPE ;
 S BDMSTP=0
 K ^XTMP("BDMPI1",BDMJOB,BDMBTH),^TMP($J,"PATS")
 S BDMTYPE=""
...
DEMO ;
 ...
 S XBRP="^BDMPI1P",XBRC="^BDMPI10",XBRX="XIT^BDMPI1",XBNS="BDM"
 D ^XBDBQUE
 D XIT
 Q
BROWSE ;
 S XBRP="VIEWR^XBLM(""^BDMPI1P"")"
 S XBRC="^BDMPI10",XBRX="XIT^BDMPI1",XBIOP=0 D ^XBDBQUE
 Q
P ;
 S BDMSTP=0 K ^XTMP("BDMPI1",BDMJOB,BDMBTH),^TMP($J,"PATS")
P1 ;
 K DIC S DIC="^AUPNPAT(",DIC(0)="AEMQ" D ^DIC K DIC
 I Y=-1,'$D(^XTMP("BDMPI1",BDMJOB,BDMBTH,"PATS")) W !,"No patients selected" S BDMSTP=1 Q
 I Y=-1 Q
 S ^XTMP("BDMPI1",BDMJOB,BDMBTH,"PATS",+Y)=""
 G P1
 Q
S ; Get patient name or cohort
 K ^XTMP("BDMPI1",BDMJOB,BDMBTH),^TMP($J,"PATS") S BDMSTP=0
...
PREG ;
 ...
 .Q:$P(^DPT(P,0),U,2)'="F"
 .;W ".",P
 .I $$PREG^BDMDI1B(P,BDMBDAT,BDMADAT,1,1) K ^TMP($J,"PATS",X)
...
SEL1 .S DIC="^VA(200,",DIC(0)="AEMQ",D="AK.PROVIDER",DIC("A")="Enter PROVIDER NAME: " D MIX^DIC1 K DIC,D
 ...
 I Y=-1 G PCP
 S BDMPIP=+Y
 S X=0 F  S X=$O(^TMP($J,"PATS",X)) Q:X'=+X  S P=$O(^TMP($J,"PATS",X,0)) I $P(^AUPNPAT(P,0),U,14)'=BDMPIP K ^TMP($J,"PATS",X,P)
 S (X,C)=0 F  S X=$O(^TMP($J,"PATS",X)) Q:X'=+X  S C=C+1
 W !!,C," patients will be used in the audit.",!
 Q
...
RAND ;random sample or not
 ...
 G:$D(DIRUT) C
 I Y="A" S C=0 F  S C=$O(^TMP($J,"PATS",C)) Q:C'=+C  S X=$O(^TMP($J,"PATS",C,0)),^XTMP("BDMPI1",BDMJOB,BDMBTH,"PATS",X)=""
 ...
 I $D(DIRUT) S BDMSTP=1 Q
 S C=0 F N=1:1:BDMCNT Q:C=Y  S I=$R(BDMCNT) I I,$D(^TMP($J,"PATS",I)) S X=$O(^TMP($J,"PATS",I,0)),^XTMP("BDMPI1",BDMJOB,BDMBTH,"PATS",X)="",C=C+1 K ^TMP($J,"PATS",I,X)
 K ^TMP($J,"PATS")
 Q
...
XIT1 ;
 ...
 K ^XTMP("BDMPI1",BDMJOB,BDMBTH),BDMJOB,BDMBTH
XIT ;
...
CTR(X,Y) ;EP - Center X in a field Y wide.
 ...
 ;
BDMG(BDMJOB,BDMBTH,BDMDMRG,BDMADAT,BDMTYPE,BDMSTMP,BDMPIP,BDMCOM,BDMRAND,BDMRCNT,BDMCMS,BDMSTAT,BDMPREP,BDMFILE,BDMDSP,BDMGIEN,BDMSDPI,BDMSDPG,BDMPPN,BDMDUZ2,BDMDEMO,BDMBEN,BDMDQ,BDMPTAX) ;PEP - gui call
 F X="BDMJOB","BDMBTH","BDMDMRG","BDMADAT","BDMTYPE","BDMSTMP","BDMPIP","BDMCOM","BDMRAND","BDMRCNT","BDMCMS","BDMSTAT","BDMPREP","BDMFILE","BDMDSP","BDMPPN","BDMDUZ2" S @X=$G(@X)
 ...
 S BDMQSRT=$G(BDMDQ)
 I $G(BDMPIP)]"" D
 . F I=1:1 D  Q:$P(BDMPIP,"~",I)=""
 .. Q:$P(BDMPIP,"~",I)=""
 .. S BDMPIEN=$O(^VA(200,"B",$P(BDMPIP,"~",I),0))
 .. Q:'BDMPIEN
 .. S BDMPRV(BDMPIEN)=""
 I $G(BDMPTAX) D
 . N X
 ...
 S BDMNOW=$G(%)
 N BDMOPT  ;maw
 S BDMOPT="DM 2021 Pre-Diabetes Audit"  ;maw
 S X=0 F  S X=$O(^TMP($J,"PATS",X)) Q:X'=+X  S P=$O(^TMP($J,"PATS",X,0)) D
 .I BDMBEN=1,$$BEN^AUPNPAT(P,"C")'="01" K ^TMP($J,"PATS",X)
 .I BDMBEN=2,$$BEN^AUPNPAT(P,"C")="01" K ^TMP($J,"PATS",X)
 K DD,D0,DIC
 ...
 S ZTIO="",ZTDTH=$$NOW^XLFDT,ZTRTN="GUIEP^BDMPI1",ZTDESC="GUI DM AUDIT" D ^%ZTLOAD
 D XIT
 Q
GUIEP ;EP - called from taskman
 ...
 D GUIR^XBLM("^BDMPI1P","^TMP($J,""BDMPI1"",")
 Q:$G(BDMDSP)  ;quit if to screen
 S X=0,C=0 F  S X=$O(^TMP($J,"BDMPI1",X)) Q:X'=+X  D
 .S BDMDATA=^TMP($J,"BDMPI1",X)
...
TEST ;
 ...
 F X=1:1:10 S ^XTMP("BDMPI1",BDMJOB,BDMBTH,"PATS",X)=""
 D BDMG^BDMPI1(BDMJOB,BDMBTH,1,DT,"P","","","","","","","",4,"TESTEPI",.BDMIEN)
 Q
