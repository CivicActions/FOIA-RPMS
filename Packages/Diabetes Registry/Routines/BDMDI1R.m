BDMDI1R ; IHS/CMI/LAB -IHS -CUMULATIVE REPORT 03 Feb 2014 5:38 PM ;
...
SDPI16 ;EP
 ...
 S BDMCUML(95,1)="Screened for HCV ever"
 ;HEP B
 ...
 S BDMCUML(140,2)="with diagnosed CVD, statin prescribed"
 ;
 ...
 S BDMCUML(180,1)="If not diagnosed with TB ever, TB test "
 S BDMCUML(180,2)="done ever (skin test or blood test)"
 ;
PROCESS ;
 S BDMNOGO=0
 S BDMPD=0 F  S BDMPD=$O(^XTMP("BDMDM21",BDMJOB,BDMBTH,"AUDIT",BDMPD)) Q:BDMPD'=+BDMPD  D CUML1
 Q
 ;
CUML1 ;
ASPIRIN ;
 I $$DODX^BDMDI16(BDMPD,BDMDMRG,"I",$G(BDMMULTR),.BDMREGLT)>BDMADAT S BDMNOGO=BDMNOGO+1 Q
 S V=$G(^XTMP("BDMDM21",BDMJOB,BDMBTH,"AUDIT",BDMPD,116))  ;CVD
 I $E(V)=1 S $P(BDMCUML(10,1),U,3)=$P(BDMCUML(10,1),U,3)+1  ;CVD DENOM
 ;CVD AND ASPIRIN
 S A=$G(^XTMP("BDMDM21",BDMJOB,BDMBTH,"AUDIT",BDMPD,62))
...
BPC ;blood pressure control
 ...
 S S=$$SYSMEAN^BDMDI15(BDMPD,BDMRBD,BDMRED)
 S D=$$DIAMEAN^BDMDI15(BDMPD,BDMRBD,BDMRED)
 D
 .I S=""!(D="") Q
 .I S<140&(D<90) S $P(BDMCUML(20,1),U,2)=$P(BDMCUML(20,1),U,2)+1 Q
CKD ;
 I $$AGE^AUPNPAT(BDMPD,BDMADAT)<18 G DENT
 S V=$G(^XTMP("BDMDM21",BDMJOB,BDMBTH,"AUDIT",BDMPD,79)),V=$$STV^BDMDI18($P(V,U,2),5)
 S T=$P($G(^XTMP("BDMDM21",BDMJOB,BDMBTH,"AUDIT",BDMPD,92)),U,5)  ;type of test
 S Q=$G(^XTMP("BDMDM21",BDMJOB,BDMBTH,"AUDIT",BDMPD,92))
 S R=$S($P(Q,U,6)]"":$P(Q,U,6),1:$P(Q,U,2))  ;value/result
 ;CONVERT RESULT LIKE IN WEB EXPORT
 S S=$$AS^BDMDI18(R),S=$$STV^BDMDI18(S,8)
 ;I S="" S V=0  ;X=$E(X,1,5)
...
HEPC ;
 S V=$G(^XTMP("BDMDM21",BDMJOB,BDMBTH,"AUDIT",BDMPD,222))  ;HEPC DX
 I $E(V)'=1 D
 .S $P(BDMCUML(95,1),U,3)=$P(BDMCUML(95,1),U,3)+1  ;DENOM FOR HEPC, THOSE WITH NO DX
 .S V=$G(^XTMP("BDMDM21",BDMJOB,BDMBTH,"AUDIT",BDMPD,223))
 .I $E(V)="1" S $P(BDMCUML(95,1),U,2)=$P(BDMCUML(95,1),U,2)+1  ;WITH SCREEN
IMM ;
HEPB ;
 S V=$G(^XTMP("BDMDM21",BDMJOB,BDMBTH,"AUDIT",BDMPD,115))
...
LIPID ;
 S V=$E($G(^XTMP("BDMDM21",BDMJOB,BDMBTH,"AUDIT",BDMPD,300)))  ;STATIN
 S C=0
 S A=$$AGE^AUPNPAT(BDMPD,BDMADAT)
 I $E($G(^XTMP("BDMDM21",BDMJOB,BDMBTH,"AUDIT",BDMPD,116)))=1 S C=1  ;CVD
 I C=1!(A>39&(A<76)) D
...
NUTR ;
 ...
 S G=0,V=$G(^XTMP("BDMDM21",BDMJOB,BDMBTH,"AUDIT",BDMPD,44))
...
TOB ;
 ...
 S V=$G(^XTMP("BDMDM21",BDMJOB,BDMBTH,"AUDIT",BDMPD,215))
 I +V=1!(+V=2) S $P(BDMCUML(170,1),U,2)=$P(BDMCUML(170,1),U,2)+1
TB ;
 S T=$G(^XTMP("BDMDM21",BDMJOB,BDMBTH,"AUDIT",BDMPD,71))
 I $E(T)=1 Q
 S $P(BDMCUML(180,1),U,3)=$P(BDMCUML(180,1),U,3)+1
 S V=$G(^XTMP("BDMDM21",BDMJOB,BDMBTH,"AUDIT",BDMPD,70))
