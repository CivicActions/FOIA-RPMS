BDMGEA ; cmi/anch/maw - BDM DMS GUI Filing Routine ; 28 Oct 2014  4:31 PM
 ;;2.0;BDM DIABETES MANAGEMENT SYSTEM;**1,8,10,16**;JUN 14, 2007;Build 122
 ;
 ;
...
IMM(BDMRET,BDMSTR) ;-- update dm Immunizations
 S X="MERR^BDMGU",@^%ZOSF("TRAP") ; m error trap
 N P,BDMDMIEN,BDMPIEN,BDMFLU,BDMPNU,BDMTD,BDMFLUD,BDMTDD,BDMPNUD,BDMHEPBD,BDMHEPB,BDMTDAPD,BDMTDAP,BDMSHD,BDMSH
 S P="|"
 I $G(BDMSTR)="" D CATSTR^BDMGU(.BDMSTR,.BDMSTR)
 I $G(BDMSTR)="" S BDMRET="Error Updating BDMEDMUP Entry, Invalid String" Q
 S BDMDMIEN=$P(BDMSTR,P)
 ...
 S BDMHEPBD=$P(BDMSTR,P,9)
 S BDMHEPB=$P(BDMSTR,P,10)
 S BDMHEPB=$S($G(BDMHEPB)]"":$O(^AUTTIMM("D",BDMHEPB,0)),1:"")
 S BDMTDAPD=$P(BDMSTR,P,11)
 S BDMTDAP=$P(BDMSTR,P,12)
 S BDMTDAP=$S($G(BDMTDAP)]"":$O(^AUTTIMM("D",BDMTDAP,0)),1:"")
 S BDMSHD=$P(BDMSTR,P,13)
 S BDMSH=$P(BDMSTR,P,14)
 S BDMSH=$S($G(BDMSH)]"":$O(^AUTTIMM("D",BDMSH,0)),1:"")
 N BDMERR,BDMIENS,BDMFDA
 S BDMIENS=""
 ...
 S BDMFDA(9003203.2,BDMDMIEN_",",1126)=BDMTDAPD
 S BDMFDA(9003203.2,BDMDMIEN_",",1127)=BDMTDAP
 S BDMFDA(9003203.2,BDMDMIEN_",",1124)=BDMSHD
 S BDMFDA(9003203.2,BDMDMIEN_",",1125)=BDMSH
 D FILE^DIE("K","BDMFDA","BDMERR(1)")
 I $G(BDMERR(1)) S BDMRET="" Q
 S BDMRET=$G(BDMIENS(1))
 Q
