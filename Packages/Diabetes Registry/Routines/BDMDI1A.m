BDMDI1A ; IHS/CMI/LAB -IHS -CUMULATIVE REPORT 14 Dec 2016 11:59 AM ; 
...
IMM ;
 ...
 W !?3,"Pneumovax/PPSV23 vaccine - ever",?49,$$C($P(V,U,4)),?61,$$C($P(V,U,2)),?73,$$P($P(V,U,2),$P(V,U,4))
 ...
 ;p14 add shringrix
 W !?3,"In patients age >=50 years ",?49,$$C($P(V,U,14)),?61,$$C($P(V,U,15)),?73,$$P($P(V,U,15),$P(V,U,14)),!?3,"Shingrix complete series - ever"
DEP ;
 I $Y>(BDMIOSL-6) D HEADER Q:BDMQUIT
 S V=$G(BDMCUML(300)) W !!,"Depression",!?3,"Active problem/diagnosis"
 W !?6,"Yes",?49,$$C($P(V,U,3)),?61,$$C($P(V,U,2)),?73,$$P($P(V,U,2),$P(V,U,3))
 S Z=$P(V,U,2)-$P(V,U,3)
 W !?6,"No",?49,$$C(Z),?61,$$C($P(V,U,2)),?73,$$P($P(V,U,2),Z)
 ;screening
 I $Y>(BDMIOSL-8) D HEADER Q:BDMQUIT
 S V=$G(BDMCUML(301))
