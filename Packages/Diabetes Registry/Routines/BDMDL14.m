BDMDL14 ; IHS/CMI/LAB -IHS -CUMULATIVE REPORT 14 Oct 2014 9:54 AM ; 
...
BPC ;
 ...
 W !?3,"<130/<80",?49,$$C($P(V,U,3)),?61,$$C($P(V,U,2)),?73,$$P($P(V,U,2),$P(V,U,3))
 W !?3,"130/80 - <140/<90",?49,$$C($P(V,U,4)),?61,$$C($P(V,U,2)),?73,$$P($P(V,U,2),$P(V,U,4))
 W !?3,"140/90 - <160/<100",?49,$$C($P(V,U,5)),?61,$$C($P(V,U,2)),?73,$$P($P(V,U,2),$P(V,U,5))
 W !?3,"160/100 or higher",?49,$$C($P(V,U,6)),?61,$$C($P(V,U,2)),?73,$$P($P(V,U,2),$P(V,U,6))
 W !?3,"BP category undetermined",?49,$$C($P(V,U,7)),?61,$$C($P(V,U,2)),?73,$$P($P(V,U,2),$P(V,U,7))
 W !?3,"----------------------------------"
 W !?3,"<140/<90",?49,$$C($P(V,U,8)),?61,$$C($P(V,U,2)),?73,$$P($P(V,U,2),$P(V,U,8))
 ;
HTN ;
 I $Y>(BDMIOSL-7) D HEADER Q:BDMQUIT
 S V=$G(BDMCUML(410))
 ...
 W !?6,"Diagnosed hypertension and mean BP <130/<80",?49,$$C($P(V,U,6)),?61,$$C($P(V,U,3)),?73,$$P($P(V,U,3),$P(V,U,6))
 ...
 W !?9,"or ARB currently prescribed."
TOBSCR ;
 I $Y>(BDMIOSL-9) D HEADER Q:BDMQUIT
...
ENDS ;
 I $Y>(BDMIOSL-7) D HEADER Q:BDMQUIT
 S V=BDMCUML(69)
 W !!,"Electronic nicotine delivery system (ENDS) use"
 W !?3,"Screened",?49,$$C($P(V,U,3)),?61,$$C($P(V,U,2)),?73,$$P($P(V,U,2),$P(V,U,3))
 W !?6,"If screened, user",?49,$$C($P(V,U,5)),?61,$$C($P(V,U,3)),?73,$$P($P(V,U,3),$P(V,U,5))
 W !,"----------------------------------"
 W !,"User of both tobacco and ENDS*",?49,$$C($P(V,U,8)),?61,$$C($P(V,U,10)),?73,$$P($P(V,U,10),$P(V,U,8))
 W !,"User of tobacco and/or ENDS*",?49,$$C($P(V,U,9)),?61,$$C($P(V,U,10)),?73,$$P($P(V,U,10),$P(V,U,9))
 W !,"*Excludes patients not screened for both tobacco and ENDS use"
TX ;
...
NUMMEDS ;
 ...
 W !!?6,"Sulfonylurea [glipizide, glyburide, ",?49,$$C($P(V,U,6)),?61,$$C($P(V,U,2)),?73,$$P($P(V,U,2),$P(V,U,6)),!?6,"others]"
 W !!?6,"DPP-4 inhibitor [alogliptin (Nesina), ",?49,$$C($P(V,U,7)),?61,$$C($P(V,U,2)),?73,$$P($P(V,U,2),$P(V,U,7)),!?6,"linagliptin (Tradjenta), saxagliptin (Onglyza), ",!?6,"sitagliptin (Januvia)]"
 W !!?6,"GLP-1 receptor agonist [dulaglutide  ",!?6,"(Trulicity), exenatide (Byetta, Bydureon), ",?49,$$C($P(V,U,8)),?61,$$C($P(V,U,2)),?73,$$P($P(V,U,2),$P(V,U,8)),!?6,"liraglutide (Victoza, Saxenda),",!?6,"lixisenatide (Adlyxin),"
 W !?6,"semaglutide (Ozempic, Rybelsus, Wegovy)]"
 W !!?6,"SGLT-2 inhibitor ",?49,$$C($P(V,U,9)),?61,$$C($P(V,U,2)),?73,$$P($P(V,U,2),$P(V,U,9)),!?6,"[bexagliflozin (Brenzavvy),",!?6,"canagliflozin,(Invokana), ",!?6,"dapagliflozin (Farxiga),",!?6,"empagliflozin (Jardiance), "
 W !?6,"ertugliflozin (Steglatro), ",!?6,"sotagliflozin (Inpefa)]"
 W !!?6,"Pioglitazone [Actos] or rosiglitazone",?49,$$C($P(V,U,10)),?61,$$C($P(V,U,2)),?73,$$P($P(V,U,2),$P(V,U,10)),!?6,"[Avandia]"
 W !!?6,"Tirzepatide [Mounjaro, Zepbound]",?49,$$C($P(V,U,13)),?61,$$C($P(V,U,2)),?73,$$P($P(V,U,2),$P(V,U,13))
 ...
 W !!?6,"Pramlintide [Symlin]",?49,$$C($P(V,U,14)),?61,$$C($P(V,U,2)),?73,$$P($P(V,U,2),$P(V,U,14))
...
CVD ;
 ...
 W !?6,"CVD and mean BP <130/<80",?49,$$C($P(V,U,9)),?61,$$C($P(V,U,8)),?73,$$P($P(V,U,8),$P(V,U,9))
 W !?6,"CVD and mean BP <140/<90",?49,$$C($P(V,U,33)),?61,$$C($P(V,U,8)),?73,$$P($P(V,U,8),$P(V,U,33))
 W !?6,"CVD and not tobacco user*",?49,$$C($P(V,U,10)),?61,$$C($P(V,U,51)),?73,$$P($P(V,U,51),$P(V,U,10))
 W !?6,"*Excludes patients not screened for tobacco use"
 W !?6,"CVD and aspirin or other",?49,$$C($P(V,U,12)),?61,$$C($P(V,U,8)),?73,$$P($P(V,U,8),$P(V,U,12)),!?9,"antiplatelet/anticoagulant therapy",!?9,"currently prescribed"
 W !?6,"CVD and GLP-1 receptor agonist",?49,$$C($P(V,U,42)),?61,$$C($P(V,U,8)),?73,$$P($P(V,U,8),$P(V,U,42)),!?9,"currently prescribed"
 W !?6,"CVD and SGLT-2 inhibitor",?49,$$C($P(V,U,41)),?61,$$C($P(V,U,8)),?73,$$P($P(V,U,8),$P(V,U,41)),!?9,"currently prescribed"
 W !?6,"CVD and GLP-1 receptor agonist and/or ",?49,$$C($P(V,U,46)),?61,$$C($P(V,U,8)),?73,$$P($P(V,U,8),$P(V,U,46)),!?9,"SGLT-2 inhibitor currently prescribed"
 W !?6,"CVD and statin currently prescribed*",?49,$$C($P(V,U,11)),?61,$$C($P(V,U,34)),?73,$$P($P(V,U,34),$P(V,U,11))
 W !?6,"*Denominator excludes patients with an allergy, intolerance,",!?6,"or contraindication."
RETIN ;
...
HEADER1 ;
 ...
 N BDMDNDR
 S BDMDNDR="Audit Report for 2026 (Audit Period "_$$DATE^BDMS9B1(BDMBDAT)_" to "_$$DATE^BDMS9B1(BDMADAT)_")"
 W $$CTR(BDMDNDR,80),!
 ;W $$CTR("AUDIT REPORT FOR 2026 (Audit Period "_$$DATE^BDMS9B1(BDMBDAT)_" to "_$$DATE^BDMS9B1(BDMADAT)_")"),!
