BDMDO13 ; IHS/CMI/LAB - 2025 DIABETES AUDIT ;
...
PNEU1(P,BDATE,EDATE) ;EP
 NEW BDMG,T1,BDMOPNU,I,X,CVX,T,D,BDMZ,G,B,T2,T3
 K BDMG
 ;
 S BDMOPNU=""
 ...
 ;CPTS
 S %=$$CPT^BDMDODU(P,BDATE,EDATE,$O(^ATXAX("B","BGP PPSV23 CPT CODES",0)),5)
 I $P(BDMOPNU,U,1)<$P(%,U,1) S BDMOPNU=$P(%,U,1)
 S %=$$TRAN^BDMDODU(P,BDATE,EDATE,$O(^ATXAX("B","BGP PPSV23 CPT CODES",0)),5)
 I $P(BDMOPNU,U,1)<$P(%,U,1) S BDMOPNU=$P(%,U,1)
 S %=$$CPT^BDMDODU(P,BDATE,EDATE,$O(^ATXAX("B","BGP PCV15 CPT CODES",0)),5)
 I $P(BDMOPNU,U,1)<$P(%,U,1) S BDMOPNU=$P(%,U,1)
 S %=$$TRAN^BDMDODU(P,BDATE,EDATE,$O(^ATXAX("B","BGP PCV15 CPT CODES",0)),5)
 I $P(BDMOPNU,U,1)<$P(%,U,1) S BDMOPNU=$P(%,U,1)
 S %=$$CPT^BDMDODU(P,BDATE,EDATE,$O(^ATXAX("B","BGP PCV20 CPT CODES",0)),5)
 I $P(BDMOPNU,U,1)<$P(%,U,1) S BDMOPNU=$P(%,U,1)
 S %=$$TRAN^BDMDODU(P,BDATE,EDATE,$O(^ATXAX("B","BGP PCV20 CPT CODES",0)),5)
 I $P(BDMOPNU,U,1)<$P(%,U,1) S BDMOPNU=$P(%,U,1)
 S %="",E=+$$CODEN^ICPTCOD(90684),%=$$CPTI^BDMDODU(P,BDATE,EDATE,E) ;patch 18
 I $P(BDMOPNU,U,1)<$P(%,U,2) S BDMOPNU=$P(%,U,2)  ;patch 18
 I BDMOPNU]"" Q BDMOPNU
 Q ""
BI() ;
...
HTNDX(P,EDATE) ;EP - is HTN on problem list
 ...
 .I $P($G(^AUPNPROB(X,800)),U,1)]"",$$SNOMED^BDMUTL(2025,"PXRM ESSENTIAL HYPERTENSION",$P(^AUPNPROB(X,800),U,1)) S G=1
 .Q
 I G Q "1  Yes"
 K BDM
 ...
 .Q:'$$SNOMED^BDMUTL(2025,"PXRM ESSENTIAL HYPERTENSION",S)
...
PLDMDOO(P,F) ;EP
 ...
 ..Q
 .I $P($G(^AUPNPROB(X,800)),U,1)]"",$$SNOMED^BDMUTL(2025,"PXRM DIABETES",$P(^AUPNPROB(X,800),U,1)) D
 ..I $P(^AUPNPROB(X,0),U,13)]"" S D($P(^AUPNPROB(X,0),U,13))=""
 .Q
...
PLDMDXS(P) ;EP - get all DM dxs from problem list
 ...
 .I $P($G(^AUPNPROB(X,800)),U,1)]"",$$SNOMED^BDMUTL(2025,"PXRM DIABETES",$P(^AUPNPROB(X,800),U,1)) S:D]"" D=D_";" S D=D_"SNOMED: "_$P(^AUPNPROB(X,800),U,1)
 .Q
 Q D
 ;
...
INCHES ;
 NEW F,FI,Z
 S (X,Z)=$$LASTHT^BDMDO13(BDMPD,BDMRED,"I")
 Q:X=""
 S X=X/12  ;get feet
 S F=$P(X,".")
...
HEP(P,EDATE,NR,F) ;EP
 ...
 ...S Y=$P(^AUPNVCPT(X,0),U) S Z=$P($$CPT^ICPTCOD(Y),U,2) I Z'=90743,Z'=90739,$$ICD^BDMUTL(Y,$P(^ATXAX(T,0),U),1) S BDMHEP(9999999-$P(ED,"."))=""  ;cmi/maw 05/15/2014 p8
 ..S X=0 F  S X=$O(^AUPNVTC("AD",V,X)) Q:X'=+X  D
 ...S Y=$P(^AUPNVTC(X,0),U,7) Q:'Y  S Z=$P($$CPT^ICPTCOD(Y),U,2) I Z'=90743,Z'=90739,$$ICD^BDMUTL(Y,$P(^ATXAX(T,0),U),1) S BDMHEP(9999999-$P(ED,"."))=""  ;cmi/maw 05/15/2014 p8
 ;
...
I ;;
 ...
 S %="",E=+$$CODEN^ICPTCOD(90739),%=$$CPTI^BDMDODU(P,$$DOB^AUPNPAT(P),EDATE,E) I % S BDMHEP($P(%,U,2))="" ;patch 18
 S %="",E=+$$CODEN^ICPTCOD(90743),%=$$CPTI^BDMDODU(P,$$DOB^AUPNPAT(P),EDATE,E) I % S BDMHEP($P(%,U,2))="" ;patch 18
 ...
 I $$PLTAX^BDMDO12(P,"BGP HEP EVIDENCE") Q "3  Immune by DX"
