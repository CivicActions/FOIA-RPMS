BDMDH1Y ; IHS/CMI/LAB - IHS Diabetes Audit 2020 ;
...
BEGIN ;EP - called from option
 ...
 W:$D(IOF) @IOF
REGASK ;
 W !!!,$$CTR("ASSESSMENT OF DIABETES CARE, 2020")
...
IF ;PEP - called from BDM indivdual or epi
 S BDMSTP=0
 S BDMPREP=6
 D QUALCHK^BDMDH1J G:BDMSTP TYPE
ZIS ;
DEMO ;
 S BDMDEMO="I"
 I BDMTYPE'="P" D DEMOCHK^BDMUTL(.BDMDEMO) I BDMDEMO=-1 G TYPE
 I BDMTYPE="P" S BDMDEMO="I"
 I BDMPREP=2 S XBRP="",XBRC="^BDMDH10",XBRX="XIT^BDMDH1",XBNS="BDM" D ^XBDBQUE,XIT Q
 W ! S DIR(0)="S^P:PRINT Output;B:BROWSE Output on Screen",DIR("A")="Do you wish to",DIR("B")="P" K DA D ^DIR K DIR
 I $D(DIRUT) D XIT Q
 S BDMOPT=Y
 I Y="B" D BROWSE,XIT Q
 S XBRP="^BDMDH1P",XBRC="^BDMDH10",XBRX="XIT^BDMDH1",XBNS="BDM"
 D ^XBDBQUE
 D XIT
 Q
BROWSE ;
 S XBRP="VIEWR^XBLM(""^BDMDH1P"")"
 S XBRC="^BDMDH10",XBRX="XIT^BDMDH1",XBIOP=0 D ^XBDBQUE
 Q
IAEP ;EP
 S BDMTYPE="P"
 S BDMPREP=1
...
PREG ;
 ...
 .Q:$P(^DPT(P,0),U,2)'="F"
 .;W ".",P
 .I $$PREG^BDMDH1B(P,BDMBDAT,BDMADAT,1,1,BDMBDAT,BDMADAT) K ^TMP($J,"PATS",X)
...
WRITEF ;EP write flat file
 ...
 S I=$O(^BDMRECD("B","DM AUDIT 2020 EXPORT RECORD",0))
 ...
 S XBMED="F",XBFN=BDMFILE,XBTLE="SAVE OF DM AUDIT 2020 EXPORT RECORDS GENERATED BY -"_$P(^VA(200,DUZ,0),U)
...
BDMG(BDMJOB,BDMBTH,BDMDMRG,BDMADAT,BDMTYPE,BDMSTMP,BDMPDP,BDMCOM,BDMRAND,BDMRCNT,BDMCMS,BDMSTAT,BDMPREP,BDMFILE,BDMDSP,BDMGIEN,BDMSDPI,BDMSDPG,BDMPPN,BDMDUZ2,BDMDEMO,BDMBEN,BDMDQ,BDMACTI,BDMPTAX) ;PEP - gui call
 ...
 N BDMOPT  ;maw
 S BDMOPT="2020 Data Quality Report"
 S BDMPREP=6
 D NOW^%DTC
 S BDMNOW=$G(%)
 K DD,D0,DIC
 ...
 D ^XBFMK
 K ZTSAVE S ZTSAVE("*")=""
 S ZTIO="",ZTDTH=$$NOW^XLFDT,ZTRTN="GUIEP^BDMDH1",ZTDESC="GUI DM AUDIT" D ^%ZTLOAD
 D XIT
 Q
GUIEP ;EP - called from taskman
 ...
 D GUIR^XBLM("^BDMDH1P","^TMP($J,""BDMDM20"",")
 Q:$G(BDMDSP)  ;quit if to screen
 S X=0,C=0 F  S X=$O(^TMP($J,"BDMDM20",X)) Q:X'=+X  D
 .S BDMDATA=^TMP($J,"BDMDM20",X)
