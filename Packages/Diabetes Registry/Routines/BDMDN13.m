BDMDN13 ; IHS/CMI/LAB - 2024 DIABETES AUDIT ;
...
PNEU1(P,BDATE,EDATE) ;EP
 ...
 ;CPTS
 S %=$$CPT^BDMDNDU(P,BDATE,EDATE,$O(^ATXAX("B","BGP PPSV23 CPT CODES",0)),5)
 I $P(BDMOPNU,U,1)<$P(%,U,1) S BDMOPNU=$P(%,U,1)
 S %=$$TRAN^BDMDNDU(P,BDATE,EDATE,$O(^ATXAX("B","BGP PPSV23 CPT CODES",0)),5)
 I $P(BDMOPNU,U,1)<$P(%,U,1) S BDMOPNU=$P(%,U,1)
 S %=$$CPT^BDMDNDU(P,BDATE,EDATE,$O(^ATXAX("B","BGP PCV15 CPT CODES",0)),5)
 I $P(BDMOPNU,U,1)<$P(%,U,1) S BDMOPNU=$P(%,U,1)
 S %=$$TRAN^BDMDNDU(P,BDATE,EDATE,$O(^ATXAX("B","BGP PCV15 CPT CODES",0)),5)
 I $P(BDMOPNU,U,1)<$P(%,U,1) S BDMOPNU=$P(%,U,1)
 S %=$$CPT^BDMDNDU(P,BDATE,EDATE,$O(^ATXAX("B","BGP PCV20 CPT CODES",0)),5)
 I $P(BDMOPNU,U,1)<$P(%,U,1) S BDMOPNU=$P(%,U,1)
 S %=$$TRAN^BDMDNDU(P,BDATE,EDATE,$O(^ATXAX("B","BGP PCV20 CPT CODES",0)),5)
 I $P(BDMOPNU,U,1)<$P(%,U,1) S BDMOPNU=$P(%,U,1)
 I BDMOPNU]"" Q BDMOPNU
 Q ""
BI() ;
...
HTNDX(P,EDATE) ;EP - is HTN on problem list
 ...
 .I $P($G(^AUPNPROB(X,800)),U,1)]"",$$SNOMED^BDMUTL(2024,"PXRM ESSENTIAL HYPERTENSION",$P(^AUPNPROB(X,800),U,1)) S G=1
 .Q
 I G Q "1  Yes"
 K BDM
 ...
 .Q:'$$SNOMED^BDMUTL(2024,"PXRM ESSENTIAL HYPERTENSION",S)
...
PLDMDOO(P,F) ;EP
 ...
 ..Q
 .I $P($G(^AUPNPROB(X,800)),U,1)]"",$$SNOMED^BDMUTL(2024,"PXRM DIABETES",$P(^AUPNPROB(X,800),U,1)) D
 ..I $P(^AUPNPROB(X,0),U,13)]"" S D($P(^AUPNPROB(X,0),U,13))=""
 .Q
...
PLDMDXS(P) ;EP - get all DM dxs from problem list
 ...
 .I $P($G(^AUPNPROB(X,800)),U,1)]"",$$SNOMED^BDMUTL(2024,"PXRM DIABETES",$P(^AUPNPROB(X,800),U,1)) S:D]"" D=D_";" S D=D_"SNOMED: "_$P(^AUPNPROB(X,800),U,1)
 .Q
 Q D
 ;
...
INCHES ;
 NEW F,FI,Z
 S (X,Z)=$$LASTHT^BDMDN13(BDMPD,BDMRED,"I")
 Q:X=""
 S X=X/12  ;get feet
 S F=$P(X,".")
...
I ;;
 ...
 I $$PLTAX^BDMDN12(P,"BGP HEP EVIDENCE") Q "3  Immune by DX"
