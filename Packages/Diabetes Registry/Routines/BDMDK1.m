BDMDK1 ; IHS/CMI/LAB - IHS Diabetes Audit 2023 ;
...
BEGIN ;EP - called from option
 ...
 W:$D(IOF) @IOF
REGASK ;
 W !!!,$$CTR("ASSESSMENT OF DIABETES CARE, 2023")
...
TYPE ;
 S BDMSTP=0
 K ^XTMP("BDMDM23",BDMJOB,BDMBTH),^TMP($J,"PATS")
 S BDMTYPE=""
...
DEMO ;
 ...
 I BDMPREP=2 S XBRP="",XBRC="^BDMDK10",XBRX="XIT^BDMDK1",XBNS="BDM" D ^XBDBQUE,XIT Q
 W ! S DIR(0)="S^P:PRINT Output;B:BROWSE Output on Screen",DIR("A")="Do you wish to",DIR("B")="P" K DA D ^DIR K DIR
 I $D(DIRUT) D XIT Q
 S BDMOPT=Y
 I Y="B" D BROWSE,XIT Q
 S XBRP="^BDMDK1P",XBRC="^BDMDK10",XBRX="XIT^BDMDK1",XBNS="BDM"
 D ^XBDBQUE
 D XIT
 Q
BROWSE ;
 S XBRP="VIEWR^XBLM(""^BDMDK1P"")"
 S XBRC="^BDMDK10",XBRX="XIT^BDMDK1",XBIOP=0 D ^XBDBQUE
 Q
IAEP ;EP
 S BDMTYPE="P"
 S BDMPREP=1
 ...
 G IF2
P ;
 S BDMSTP=0 K ^XTMP("BDMDM23",BDMJOB,BDMBTH),^TMP($J,"PATS")
 S BDMBEN=3
P1 ;
 K DIC S DIC="^AUPNPAT(",DIC(0)="AEMQ" D ^DIC K DIC
 I Y=-1,'$D(^XTMP("BDMDM23",BDMJOB,BDMBTH,"PATS")) W !,"No patients selected" S BDMSTP=1 Q
 I Y=-1 Q
 S ^XTMP("BDMDM23",BDMJOB,BDMBTH,"PATS",+Y)=""
 G P1
 ;
S ; Get patient name or cohort
 K ^XTMP("BDMDM23",BDMJOB,BDMBTH),^TMP($J,"PATS") S BDMSTP=0
...
PREG ;
 ...
 .Q:$P(^DPT(P,0),U,2)'="F"
 .;W ".",P
 .I $$PREG^BDMDK1B(P,BDMBDAT,BDMADAT,1,1,BDMBDAT,BDMADAT) K ^TMP($J,"PATS",X)
...
RAND ;random sample or not
 ...
 G:$D(DIRUT) C
 I Y="A" S C=0 F  S C=$O(^TMP($J,"PATS",C)) Q:C'=+C  S X=$O(^TMP($J,"PATS",C,0)),^XTMP("BDMDM23",BDMJOB,BDMBTH,"PATS",X)=""
 ...
 I $D(DIRUT) S BDMSTP=1 Q
 S C=0 F N=1:1:BDMCNT Q:C=Y  S I=$R(BDMCNT) I I,$D(^TMP($J,"PATS",I)) S X=$O(^TMP($J,"PATS",I,0)),^XTMP("BDMDM23",BDMJOB,BDMBTH,"PATS",X)="",C=C+1 K ^TMP($J,"PATS",I,X)
 K ^TMP($J,"PATS")
 Q
...
XIT1 ;
 ...
 K ^XTMP("BDMDM23",BDMJOB,BDMBTH),BDMJOB,BDMBTH
XIT ;
...
WRITEF ;EP write flat file
 ...
 S I=$O(^BDMRECD("B","DM AUDIT 2023 EXPORT RECORD",0))
 ...
 S XBMED="F",XBFN=BDMFILE,XBTLE="SAVE OF DM AUDIT 2023 EXPORT RECORDS GENERATED BY -"_$P(^VA(200,DUZ,0),U)
...
BDMG(BDMJOB,BDMBTH,BDMDMRG,BDMADAT,BDMTYPE,BDMSTMP,BDMPDP,BDMCOM,BDMRAND,BDMRCNT,BDMCMS,BDMSTAT,BDMPREP,BDMFILE,BDMDSP,BDMGIEN,BDMSDPI,BDMSDPG,BDMPPN,BDMDUZ2,BDMDEMO,BDMBEN,BDMDQ,BDMACTI,BDMPTAX) ;PEP - gui call
 ...
 N BDMOPT  ;maw
 S BDMOPT="2023 Diabetes Program Audit"
 ...
 D ^XBFMK
 K ZTSAVE S ZTSAVE("*")=""
 S ZTIO="",ZTDTH=$$NOW^XLFDT,ZTRTN="GUIEP^BDMDK1",ZTDESC="GUI DM AUDIT" D ^%ZTLOAD
 D XIT
 Q
GUIEP ;EP - called from taskman
 ...
 D GUIR^XBLM("^BDMDK1P","^TMP($J,""BDMDM23"",")
 Q:$G(BDMDSP)  ;quit if to screen
 S X=0,C=0 F  S X=$O(^TMP($J,"BDMDM23",X)) Q:X'=+X  D
 .S BDMDATA=^TMP($J,"BDMDM23",X)
