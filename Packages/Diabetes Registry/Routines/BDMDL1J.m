BDMDL1J ; IHS/CMI/LAB -IHS -CUMULATIVE REPORT 10 Oct 2014 9:50 AM ; 
...
COMORBID ;USE 400
 ...
 S V=$G(^XTMP("BDMDM26",BDMJOB,BDMBTH,"AUDIT",BDMPD,200))
 I $E(V)="1" S $P(BDMCUML(400),U,3)=$P(BDMCUML(400),U,3)+1,BDMCOMOR=BDMCOMOR+1
 ;tobacco use piece 4
 S V=$G(^XTMP("BDMDM26",BDMJOB,BDMBTH,"AUDIT",BDMPD,27))
 ...
 ;cvd piece 8
 S V=$G(^XTMP("BDMDM26",BDMJOB,BDMBTH,"AUDIT",BDMPD,116))
 I $E(V)=1 S $P(BDMCUML(400),U,8)=$P(BDMCUML(400),U,8)+1,BDMCOMOR=BDMCOMOR+1
 ;
 ;CVD & BP <130/<80 PIECE 9
 I $E(V)=1,BDMBP130 S $P(BDMCUML(400),U,9)=$P(BDMCUML(400),U,9)+1
 ;CVD & BP <140/<90 piece 33
 I $E(V)=1,BDMBP140 S $P(BDMCUML(400),U,33)=$P(BDMCUML(400),U,33)+1
 ;CVD AND NOT A TOBACCO USER PIECE 10
 I BDMSNTNE,$E(V)=1 S $P(BDMCUML(400),U,10)=$P(BDMCUML(400),U,10)+1
 ;denominator for tobacco, exclude in denominator if not tobacco screened PIECE 51
 I BDMSTOSC,$E(V)=1 S $P(BDMCUML(400),U,51)=$P(BDMCUML(400),U,51)+1
 ;CVD AND STATIN PRESCRIBED PIECE 11
 S T=+$G(^XTMP("BDMDM26",BDMJOB,BDMBTH,"AUDIT",BDMPD,300))
 ...
 S T=$G(^XTMP("BDMDM26",BDMJOB,BDMBTH,"AUDIT",BDMPD,62))
 I $E(V)=1,$E(T)=1 S $P(BDMCUML(400),U,12)=$P(BDMCUML(400),U,12)+1
 ;CVD AND GLP1  PIECE 42
 I $E(V)=1,BDMTXGLP S $P(BDMCUML(400),U,42)=$P(BDMCUML(400),U,42)+1
 ;CVD AND SLGT PIECE 41
 I $E(V)=1,BDMTXSGL S $P(BDMCUML(400),U,41)=$P(BDMCUML(400),U,41)+1
 ;PATCH 18 PIECE 46 CVD AND SGLT OR GLP1
 I $E(V)=1,(BDMTXGLP!(BDMTXSGL)) S $P(BDMCUML(400),U,46)=$P(BDMCUML(400),U,46)+1
 ;
 ;CKD PIECE 13 & 14
 ...
 ;CKD AND BP <130/<80 PIECE 24
 I BDMCKD,BDMBP130 S $P(BDMCUML(400),U,24)=$P(BDMCUML(400),U,24)+1
 ;CKD AND ACE PIECE 16
 S A=$E($G(^XTMP("BDMDM26",BDMJOB,BDMBTH,"AUDIT",BDMPD,60)))
 I A=1,BDMCKD S $P(BDMCUML(400),U,16)=$P(BDMCUML(400),U,16)+1
 ;ckd and glp1  43
 I BDMCKD,BDMTXGLP S $P(BDMCUML(400),U,43)=$P(BDMCUML(400),U,43)+1
 ;CVD AND SLGT PIECE 44
 I BDMCKD,BDMTXSGL S $P(BDMCUML(400),U,44)=$P(BDMCUML(400),U,44)+1
 I BDMCKD,(BDMTXGLP!(BDMTXSGL)) S $P(BDMCUML(400),U,47)=$P(BDMCUML(400),U,47)+1
 ...
 ;STAGE 3 EGFR 30-59
 I BDMEGFRV]"",BDMEGFRV>29,BDMEGFRV<60 S $P(BDMCUML(400),U,20)=$P(BDMCUML(400),U,20)+1 G N
 ;STAGE 4 EGFR 15-20 PIECE 21
 I BDMEGFRV]"",BDMEGFRV>14,BDMEGFRV<30 S $P(BDMCUML(400),U,21)=$P(BDMCUML(400),U,21)+1 G N
 ;STAGE 5 EGFR <15
...
N ;p12 HEP C AND RETINOPATHY
 S V=$G(^XTMP("BDMDM26",BDMJOB,BDMBTH,"AUDIT",BDMPD,222))
 I $E(V)="1" S $P(BDMCUML(400),U,35)=$P(BDMCUML(400),U,35)+1,BDMCOMOR=BDMCOMOR+1  ;DX
 S V1=$G(^XTMP("BDMDM26",BDMJOB,BDMBTH,"AUDIT",BDMPD,223))  ;SCREENED
 I $$AGE^AUPNPAT(BDMPD,BDMADAT)<18 G R
 S $P(BDMCUML(400),U,50)=$P(BDMCUML(400),U,50)+1  ;total >= 18
 I $E(V)'=1 S $P(BDMCUML(400),U,40)=$P(BDMCUML(400),U,40)+1  ;total with no dx who are 18+
 ;
 I $E(V)'=1,$E(V1)="1" S $P(BDMCUML(400),U,37)=$P(BDMCUML(400),U,37)+1  ;TOTAL NO DX BUT SCREENED
 I $E(V)=1!($E(V1)=1) S $P(BDMCUML(400),U,45)=$P(BDMCUML(400),U,45)+1  ;DX OR SCREENED
...
R ;RETINOPATHY
 S V=$G(^XTMP("BDMDM26",BDMJOB,BDMBTH,"AUDIT",BDMPD,224))
...
EPICHK ;create epi info record in ^BDMDATA("BDMEPI",$J,n)
 ...
 I $G(BDMQSRT)="" S BDMQSRT="P"
 S X=$$DODX^BDMDL16(BDMPD,BDMDMRG,"I",$G(BDMMULTR),.BDMREGLT)
 I X>BDMADAT Q
 NEW BDMECNT,BDMEPIR,BDMTHER
 S BDMEPIR="",BDMTHER="",BDMECNT=0
 S BDMEPIR=$$REC(BDMPD,"DM AUDIT 2026 EXPORT RECORD",BDMRBD,BDMRED,BDMADAT,BDMDMRG,BDMBDAT)
 ;CHECK ERRORS ON BDMEPIR
 S BDMRTYP("IEN")=$O(^BDMRECD("B","DM AUDIT 2026 EXPORT RECORD",0))
 S Z=0 F  S Z=$O(^BDMRECD(BDMRTYP("IEN"),21,"O",Z)) Q:Z'=+Z  D
 .S K=$O(^BDMRECD(BDMRTYP("IEN"),21,"O",Z,0))
 .X:$D(^BDMRECD(BDMRTYP("IEN"),21,K,1)) ^BDMRECD(BDMRTYP("IEN"),21,K,1)
 .Q:'$T
 .S S=""
 .I BDMQSRT="P" S S=$P(^DPT(BDMPD,0),U,1)
 .I BDMQSRT="E" S S=$P(^BDMRECD(BDMRTYP("IEN"),21,K,0),U,1)
 .S BDMECNT=BDMECNT+1
 .S $P(^XTMP("BDMDM26 ERRORS",BDMJOB,BDMBTH,"PATS",S,BDMPD,BDMECNT),U,1)=$P(^BDMRECD(BDMRTYP("IEN"),21,K,0),U,1)
 .S J=$P(^BDMRECD(BDMRTYP("IEN"),21,K,0),U,2)
 .S $P(^XTMP("BDMDM26 ERRORS",BDMJOB,BDMBTH,"PATS",S,BDMPD,BDMECNT),U,2)=$S(J="P":"POTENTIAL",J="D":"DEFINITE",1:"")
 .S $P(^XTMP("BDMDM26 ERRORS",BDMJOB,BDMBTH,"PATS",S,BDMPD,BDMECNT),U,3)=$P($G(^BDMRECD(BDMRTYP("IEN"),21,K,11)),U,1)
 .S P=$P(^BDMRECD(BDMRTYP("IEN"),21,K,0),U,4)
 .S A=""
 .I P="" D
 ..I $P(^BDMRECD(BDMRTYP("IEN"),21,K,0),U,1)["BMI" S A=$$BMI^BDMDL18(BDMPD,BDMRBD,BDMRED)
 ..I $P(^BDMRECD(BDMRTYP("IEN"),21,K,0),U,1)["HEIGHT TOTAL" S A="" I $P(BDMEPIR,U,18)]"" S A=$P(BDMEPIR,U,18)*12,A=A+$P(BDMEPIR,U,19)
 ..I $P(^BDMRECD(BDMRTYP("IEN"),21,K,0),U,1)["DURATION" S A=$$DURDM^BDMDL16(BDMPD,BDMDMRG,BDMADAT)
 .I P S A=$P(BDMEPIR,U,P)
 .S $P(^XTMP("BDMDM26 ERRORS",BDMJOB,BDMBTH,"PATS",S,BDMPD,BDMECNT),U,4)=A
 Q
QUALCHK ;EP - addl questions for data quality report
SUMONLY ;
 S BDMRTYPE=""
 S DIR(0)="S^S:SUMMARY OF ERRORS ONLY;P:PATIENT LIST ONLY;B:BOTH A SUMMARY AND PATIENT LIST",DIR("A")="Type of Report" KILL DA D ^DIR KILL DIR
 I $D(DIRUT) S BDMSTP=1 Q
 S BDMRTYPE=Y
 ;get sort value
 S BDMQSRT=""
 I BDMRTYPE="S" S BDMQSRT="P" Q
 S DIR(0)="S^P:PATIENT NAME;E:ERROR FIELD NAME",DIR("A")="How should the PATIENT list be sorted",DIR("B")="P" KILL DA D ^DIR KILL DIR
 I $D(DIRUT) S BDMSTP=1 Q
 S BDMQSRT=Y
 Q
