BDMPL1 ; IHS/CMI/LAB - IHS Diabetes Audit 2026 ;
...
BEGIN ;EP - called from option
 ...
 W:$D(IOF) @IOF
REGASK ;
 W !!!,$$CTR("ASSESSMENT OF PREDIABETES DIABETES CARE, 2026")
 W !!
 K BDMQUIT
 D REG^BDMFUTIL
 I $G(BDMRDA)="" W !,"NO Register Selected!!!  The CMS register will not be used in retrieving",!,"any data."
 S (BDMDMRG,BDMCMS)=$G(BDMRDA)
 S BDMJOB=$J,BDMBTH=$H
GETDATES ;
 S BDMSTP=0 D TIME I BDMSTP D XIT1,XIT Q
TYPE ;
 S BDMSTP=0
 K ^XTMP("BDMPL1",BDMJOB,BDMBTH),^TMP($J,"PATS")
 S BDMTYPE=""
...
IF ;PEP - called from BDM indivdual or epi
 S BDMSTP=0
 K DIR
 S DIR(0)="S^1:Individual Assessment of Care;2:Health Status Report;3:Both Individual Assessment of Care and Health Status Report"
...
IF2 ;
 ...
 .K DIR S DIR(0)="Y",DIR("A")="Do you wish to print the Patient's Name on the assessment sheet",DIR("B")="N" KILL DA D ^DIR KILL DIR
...
DEMO ;
 ...
 W ! S DIR(0)="S^P:PRINT Output;B:BROWSE Output on Screen",DIR("A")="Do you wish to",DIR("B")="B" K DA D ^DIR K DIR
 I $D(DIRUT) D XIT Q
 S BDMOPT=Y
 I Y="B" D BROWSE,XIT Q
 S XBRP="^BDMPL1P",XBRC="^BDMPL10",XBRX="XIT^BDMPL1",XBNS="BDM"
 D ^XBDBQUE
 D XIT
 Q
BROWSE ;
 S XBRP="VIEWR^XBLM(""^BDMPL1P"")"
 S XBRC="^BDMPL10",XBRX="XIT^BDMPL1",XBIOP=0 D ^XBDBQUE
 Q
IAEP ;EP
 S BDMTYPE="P"
 S BDMPREP=1
 ...
 G IF2
P ;
 S BDMSTP=0 K ^XTMP("BDMPL1",BDMJOB,BDMBTH),^TMP($J,"PATS")
 S BDMBEN=3
P1 ;
 K DIC S DIC="^AUPNPAT(",DIC(0)="AEMQ" D ^DIC K DIC
 I Y=-1,'$D(^XTMP("BDMPL1",BDMJOB,BDMBTH,"PATS")) W !,"No patients selected" S BDMSTP=1 Q
 I Y=-1 Q
 S ^XTMP("BDMPL1",BDMJOB,BDMBTH,"PATS",+Y)=""
 G P1
 ;
S ; Get patient name or cohort
 K ^XTMP("BDMPL1",BDMJOB,BDMBTH),^TMP($J,"PATS") S BDMSTP=0
...
PCP ;
 ...
 S DIR(0)="Y",DIR("A")="Limit the audit to one or more primary care providers",DIR("B")="N" KILL DA D ^DIR KILL DIR
 ...
 I $D(DIRUT) G PCP
 S BDMPLTY=Y
 I BDMPLTY="IP" D  I $G(BDMSTP) G PCP
 .K BDMPRV
 .;W !!,"Enter the name of the provider.",!
 .S DIC="^VA(200,",DIC(0)="AEMQ",D="AK.PROVIDER",DIC("A")="Enter PROVIDER NAME: " D MIX^DIC1 K DIC,D
 .I Y<0 S BDMSTP=1 Q
 .S BDMPRV(+Y)=""
 I BDMPLTY="SEL" D  I $G(BDMSTP) G PCP
 .K BDMPRV
 .W !!,"Enter the name of the provider.",!
SEL1 .S DIC="^VA(200,",DIC(0)="AEMQ",D="AK.PROVIDER",DIC("A")="Enter PROVIDER NAME: " D MIX^DIC1 K DIC,D
 .I Y<0,'$D(BDMPRV) S BDMSTP=1 Q
 .I Y<0 Q
 .S BDMPRV(+Y)=""
 .G SEL1
 I BDMPLTY="TAX" D  I $G(BDMSTP) G PCP
 .W !!,"Enter the name of the provider taxonomy"
 .S DIC="^ATXAX(",DIC("S")="I $P(^(0),U,15)=200",DIC(0)="AEMQ",DIC("A")="Enter TAXONOMY NAME: " D ^DIC K DIC
 .I Y<0 S BDMSTP=1 Q
 .S BDMPRTX=+Y
 .W !,"The following providers are members of this taxonomy: "
 .S X=0 F  S X=$O(^ATXAX(+Y,21,"B",X)) Q:X'=+X  S BDMPRV(X)="" W !?5,"- ",$P(^VA(200,X,0),U,1)
 ;S BDMPDP=+Y
 S X=0 F  S X=$O(^TMP($J,"PATS",X)) Q:X'=+X  S P=$O(^TMP($J,"PATS",X,0)) D
 .S Y=$P(^AUPNPAT(P,0),U,14)
 .I 'Y K ^TMP($J,"PATS",X,P) Q
 .I Y,'$D(BDMPRV(Y)) K ^TMP($J,"PATS",X,P)
...
C ;get register, status, random or not
 ...
 S BDMCMS="",BDMSTP=0
 D REG^BDMFUTIL
 I $G(BDMRDA)="" W !,"No register selected." S BDMSTP=1 Q
 S BDMCMS=BDMRDA
C0 ;get status
 S BDMSTAT=""
...
PREG ;
 ...
 .Q:$P(^DPT(P,0),U,2)'="F"
 .;W ".",P
 .I $$PREG^BDMDL1B(P,BDMBDAT,BDMADAT,1,1,BDMBDAT,BDMADAT) K ^TMP($J,"PATS",X)
...
RAND ;random sample or not
 ...
 G:$D(DIRUT) C
 I Y="A" S C=0 F  S C=$O(^TMP($J,"PATS",C)) Q:C'=+C  S X=$O(^TMP($J,"PATS",C,0)),^XTMP("BDMPL1",BDMJOB,BDMBTH,"PATS",X)=""
 ...
 I $D(DIRUT) S BDMSTP=1 Q
 S C=0 F N=1:1:BDMCNT Q:C=Y  S I=$R(BDMCNT) I I,$D(^TMP($J,"PATS",I)) S X=$O(^TMP($J,"PATS",I,0)),^XTMP("BDMPL1",BDMJOB,BDMBTH,"PATS",X)="",C=C+1 K ^TMP($J,"PATS",I,X)
 K ^TMP($J,"PATS")
 Q
TIME ;PEP - called from BDM Get fiscal year or time frame
 S BDMSTP=0
 S (BDMRBD,BDMRED,BDMADAT)=""
 W !!,"Enter the date of the assessment.  This date will be considered the ending",!,"date of the audit period.  For most data items all data for the period one",!,"year prior to this date will be reviewed.",!
...
XIT1 ;
 ...
 K ^XTMP("BDMPL1",BDMJOB,BDMBTH),BDMJOB,BDMBTH
XIT ;
...
BDMG(BDMJOB,BDMBTH,BDMDMRG,BDMADAT,BDMTYPE,BDMSTMP,BDMPOP,BDMCOM,BDMRAND,BDMRCNT,BDMCMS,BDMSTAT,BDMPREP,BDMFILE,BDMDSP,BDMGIEN,BDMSDPI,BDMSDPG,BDMPPN,BDMDUZ2,BDMDEMO,BDMBEN,BDMDQ,BDMPTAX) ;PEP - gui call
 F X="BDMJOB","BDMBTH","BDMDMRG","BDMADAT","BDMTYPE","BDMSTMP","BDMPOP","BDMCOM","BDMRAND","BDMRCNT","BDMCMS","BDMSTAT","BDMPREP","BDMFILE","BDMDSP","BDMPPN","BDMDUZ2" S @X=$G(@X)
 ...
 S BDMQSRT=$G(BDMDQ)
 I $G(BDMPOP)]"" D
 . F I=1:1 D  Q:$P(BDMPOP,"~",I)=""
 .. Q:$P(BDMPOP,"~",I)=""
 .. S BDMPNEN=$O(^VA(200,"B",$P(BDMPOP,"~",I),0))
 .. Q:'BDMPNEN
 .. S BDMPRV(BDMPNEN)=""
 I $G(BDMPTAX) D
 . N X
 . S X=0 F  S X=$O(^ATXAX(BDMPTAX,21,"B",X)) Q:X'=+X  S BDMPRV(X)=""
 I $G(BDMDSP) D GUIEP Q
 ;create entry in fileman file to hold output
 D NOW^%DTC
 S BDMNOW=$G(%)
 N BDMOPT  ;maw
 S BDMOPT="DM 2026 Pre-Diabetes Audit"  ;maw
 S X=0 F  S X=$O(^TMP($J,"PATS",X)) Q:X'=+X  S P=$O(^TMP($J,"PATS",X,0)) D
 .I BDMBEN=1,$$BEN^AUPNPAT(P,"C")'="01" K ^TMP($J,"PATS",X)
 .I BDMBEN=2,$$BEN^AUPNPAT(P,"C")="01" K ^TMP($J,"PATS",X)
 K DD,D0,DIC
 ...
 S ZTIO="",ZTDTH=$$NOW^XLFDT,ZTRTN="GUIEP^BDMPL1",ZTDESC="GUI Prediabetes AUDIT" D ^%ZTLOAD
 D XIT
 Q
GUIEP ;EP - called from taskman
 ...
 D GUIR^XBLM("^BDMPL1P","^TMP($J,""BDMPL1"",")
 Q:$G(BDMDSP)  ;quit if to screen
 S X=0,C=0 F  S X=$O(^TMP($J,"BDMPL1",X)) Q:X'=+X  D
 .S BDMDATA=^TMP($J,"BDMPL1",X)
 .;I BDMDATA="ZZZZZZZ" S BDMDITA=$C(12)
