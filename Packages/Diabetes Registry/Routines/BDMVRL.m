BDMVRL ; IHS/CMI/LAB - VIEW PT RECORD LT ; 07 May 2025  9:31 AM
...
CS1 ;EP;
 ...
 D DQ^BDMPPDT
 ;LOG individual summary print in BUSA
 NEW BDMDETAL
 S BDMDETAL(1)=DFN_U_U
 D LOG^BUSAAPI("O","P","P",$S($G(XQY0)]"":$P(XQY0,U),1:"CS1~BDMVRL"),"DM CASE SUMMARY/HEALTH SUMMARY","BDMDETAL")
 Q
AS ;EP;TO REVIEW AUDIT STATUS
 ...
 S BDMBTH=$H
 S BDMDMRG=BDMRDA
 S ^XTMP("BDMDM26",BDMJOB,BDMBTH,"PATS",DFN)=""
 S ^TMP("BDMDM26",BDMJOB,BDMBTH,"PATS",DFN)=""
 D TIME^BDMDL1
 Q:$G(BDMSTP)
 D EN^XBNEW("IAEP^BDMDL1","BDMJOB;BDMBTH;BDMDMRG;BDMADAT;BDMRED;BDMBDAT;BDMRBD")
 Q
 ;
...
HS ;EP;TO VIEW A HEALTH SUMMARY
 ...
 NEW BDMDETAL
 S BDMDETAL(1)=DFN_U_U
 D LOG^BUSAAPI("O","P","P",$S($G(XQY0)]"":$P(XQY0,U),1:"HS~BDMVRL"),"VIEW HEALTH SUMMARY","BDMDETAL")
 Q
...
RS ;EP;TO EDIT REGISTER STATUS
 ...
 W !!
 NEW BDMOLD
 S BDMOLD=$$VALI^XBDIQ1(9002241,BDMRPDA,1)  ;old value of status for busa
 D DIE^BDMFDIC
 I BDMOLD'=$$VALI^XBDIQ1(9002241,BDMRPDA,1) D
 .;LOG BUSA CHANGE OF STATUS
 .NEW BDMDETAL
 .S BDMDETAL(1)=DFN_U_U_"Changed Register Status"_U_BDMOLD_U_$$VALI^XBDIQ1(9002241,BDMRPDA,1)
 .D LOG^BUSAAPI("O","P","E",$S($G(XQY0)]"":$P(XQY0,U),1:"RS~BDMVRL"),"Reg Status Change "_$$VAL^XBDIQ1(9002241.1,BDMRDA,.01),"BDMDETAL")
 D DXUPD^BDMVRL5
 S DA=BDMRPDA
 S DIE="^ACM(41,"
 S DR="11////"_DT
...
Z(X) ;SET TMP NODE
 ...
 Q
PDMAC ;EP;TO PREDM ASSESSMENT OF CARE
 D REG^BDMFUTIL
 Q:$D(BDMQUIT)
 D CLEAR^VALM1
 N X,BDMY,BDMZ,BDMQUIT,BDMVRLY
 ...
 S BDMBTH=$H
 S BDMDMRG=BDMRDA
 S ^XTMP("BDMPL1",BDMJOB,BDMBTH,"PATS",DFN)=""
 S ^TMP("BDMPL1",BDMJOB,BDMBTH,"PATS",DFN)=""
 D TIME^BDMPL1
 Q:$G(BDMSTP)
 D EN^XBNEW("IAEP^BDMPL1","BDMJOB;BDMBTH;BDMDMRG;BDMADAT;BDMRED;BDMBDAT;BDMRBD")
 Q
PPSC ;EP;DPSC
 D REG^BDMFUTIL
 Q:$D(BDMQUIT)
 ...
 D CLEAR^VALM1
 W @IOF
 W !?5,"Display Prediabetes Patient Care Summary for: ",$P(BDMPAT0,U),!!
 D VIEWR^XBLM("PRINT^BDMPDSP")
 Q
