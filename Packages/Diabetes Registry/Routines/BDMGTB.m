BDMGTB ; cmi/anch/maw - BDM DMS GUI Table Lookup ;
 ;;2.0;BDM DIABETES MANAGEMENT SYSTEM;**1,8,12,13**;JUN 14, 2007;Build 66
 ;
 ;
...
REGADOAU(BDMRET) ;-- return register to work with
 ...
 K ^BDMTMP($J)
 S BDMRET="^BDMTMP("_$J_")"
 S ^BDMTMP($J,BDMC)="T00007BMXIEN^T00030Register Name^T00010Active Members^T00010Members^T00030Last Patient Update"_$C(30)
 S BDMDA=0 F  S BDMDA=$O(^ACM(41.1,"B",BDMDA)) Q:BDMDA=""  D
 . Q:BDMDA'["DIAB"
 . S BDMRIEN=$O(^ACM(41.1,"B",BDMDA,0))
 . Q:'$D(^ACM(41.1,BDMRIEN,"AU","B",DUZ))
 . S BDMC=BDMC+1
 . S ^BDMTMP($J,BDMC)=BDMRIEN_U_BDMDA_U_$TR($$ACTM^BDMFUTIL(BDMRIEN)," ","")_U_$TR($$ALLM^BDMFUTIL(BDMRIEN)," ","")_U_$$DATE^BDMS9B1($$LU^BDMFUTIL(BDMRIEN))_$C(30)
 S ^BDMTMP($J,BDMC+1)=$C(31)_$G(BDMERR)
 Q
 ;
