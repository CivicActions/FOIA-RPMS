BDM20P16 ; IHS/CMI/LAB - BDM V2.0 patch 15 environment check  ; 14 Nov 2022  10:45 AM
 ...
 I '$$INSTALLD("BDM*2.0*15") D SORRY(2)
 I '$$INSTALLD("ATX*5.1*45") D SORRY(2)
 I +$$VERSION^XPDUTL("BGP")<22.1 D MES^XPDUTL($$CJ^XLFSTR("Version 22.1 of the IHS CLINICAL REPORTING is required.  Not installed.",80)) D SORRY(2) I 1
 E  D MES^XPDUTL($$CJ^XLFSTR("Version 22.1 of IHS CLINICAL REPORTING is installed.",80))
 ;
 Q
 ;
PRE ;
 ...
 S BDMX=0 F  S BDMX=$O(^BDMSNME(BDMX)) Q:BDMX'=+BDMX  S DA=BDMX,DIK="^BDMSNME(" D ^DIK
 K DIE,DA,DR
 S DA=$O(^DIC(19,"B","BDMDMSP",0))
 I DA S DIE="^DIC(19,",DR="2///@" D ^DIE K DA,DIE,DR
 ;fix taxonomy entry on supplement
 S DA=$O(^APCHSUP("B","PRE-DIABETES CARE SUMMARY",0))
 I DA D
 .S %=0 F  S %=$O(^APCHSUP(DA,13,%)) Q:%'=+%  I ^APCHSUP(DA,13,%,0)["]" D
 ..S Y=^APCHSUP(DA,13,%,0) S N=$TR(Y,"]","")
 ..K ^APCHSUP(DA,13,"B",Y,%) S ^APCHSUP(DA,13,"B",N,%)=""
 ..S ^APCHSUP(DA,13,%,0)=N
 Q
 Q
POST ;
 ;REMOVE Patient SSN from general retrieval per Renee Chase
 S DA=$O(^ACM(58.1,"B","Patient SSN",0))
 I DA S DIK="^ACM(58.1," D ^DIK K DIK,DA
 ;IF IT DOESN'T EXIST, ADD PREDIABETES TO CMS DIAGNOSIS LIST ENTRY
 S X=$O(^ACM(44.1,"B","PREDIABETES",0))
 I 'X D
 .S X="PREDIABETES",DIC="^ACM(44.1,",DIC(0)="L" D ^DIC K DA,DIC
 D ^BDMDKZ
 D BMXPO
 D MEDTAX
 ;move glp-1 from 2 taxonomies
 S X=$O(^ATXAX("B","DM AUDIT GLP-1 ANALOG DRUGS",0))
 S N=$O(^ATXAX("B","DM AUDIT GLP-1 RECEPT AGONISTS",0))
 ;get last ien of N taxonomy
 S L=0,Y=0 F  S Y=$O(^ATXAX(N,21,Y)) Q:Y'=+Y  S L=Y
 I X D
 .S Y=0 F  S Y=$O(^ATXAX(X,21,"B",Y)) Q:Y'=+Y  D
 ..S L=L+1
 ..Q:$D(^ATXAX(N,21,"B",Y))  ;ALREADY HAVE IT
 ..S Z=$O(^ATXAX(X,21,"B",Y,0))
 ..S ^ATXAX(N,21,L,0)=Y_U_Y,^ATXAX(N,21,"B",Y,L)=""
 S X=$O(^ATXAX("B","DM AUDIT INCRETIN MIMETIC",0))
 S N=$O(^ATXAX("B","DM AUDIT GLP-1 RECEPT AGONISTS",0))
 ;get last ien of N taxonomy
 S L=0,Y=0 F  S Y=$O(^ATXAX(N,21,Y)) Q:Y'=+Y  S L=Y
 I X D
...
MEDTAX ;EP set up drug taxonomies
 S ATXFLG=1
 S BDMX="DM AUDIT TB MEDS",BDMTAX="",BDMNDCT="DM AUDIT TB DRUGS NDC",BDMVAPI="" D DRUG1
 S BDMX="DM AUDIT GLP-1 RECEPT AGONISTS",BDMTAX="",BDMNDCT="",BDMVAPI="" D DRUG1
 S BDMX="DM AUDIT TIRZEPATIDE DRUGS",BDMTAX="",BDMNDCT="",BDMVAPI="" D DRUG1
 Q
 ;
DRUG1 ;
