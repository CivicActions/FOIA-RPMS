BDMS9D2 ; IHS/CMI/LAB - DIABETIC CARE SUMMARY SUPPLEMENT ; 11 Nov 2022  8:16 AM
 ;;2.0;DIABETES MANAGEMENT SYSTEM;**3,4,5,16**;JUN 14, 2007;Build 122
 ;
 ;
MORE ;EP
 ...
 S X="                      A1C:" S Y=$$HBA1C^BDMS9B2(BDMSDFN),$E(X,28)=$P(Y,"|||"),$E(X,42)=$$DATE^BDMS9B1($P(Y,"|||",2)),$E(X,56)=$P(Y,"|||",3) D S(X)
 I $P(Y,"|||",4)]"" S X="   Note: "_$P(Y,"|||",4) D S(X)
 S X="     Next most recent A1C:" S Y=$$NLHGB^BDMS9B2(BDMSDFN),$E(X,28)=$P(Y,"|||"),$E(X,42)=$$DATE^BDMS9B1($P(Y,"|||",2)),$E(X,56)=$P(Y,"|||",3) D S(X)
 ...
 ;.S BDMY=0 F  S BDMY=$O(BDMX(BDMY)) Q:BDMY'=+BDMY  S X="",$E(X,28)=$P(BDMX(BDMY),U),$E(X,42)=$$DATE^BDMS9B1($P(BDMX(BDMY),U,2)) D S(X)
