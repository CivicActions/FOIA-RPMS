BDMPO1P ; IHS/CMI/LAB - 2025 DIABETES AUDIT PRINT 15 Dec 2016 3:05 PM ; 24 Oct 2019  3:42 PM
 ;;2.0;DIABETES MANAGEMENT SYSTEM;**18**;JUN 14, 2007;Build 147
 ;
 ;
 D LOG^BUSAAPI("O","P","P",$S($G(XQY0)]"":$P(XQY0,U),1:"BDMDN1P"),"DM 2025 PREDIABETES REPORT")
 S BDMQUIT=0,BDMPG=0,BDMIOSL=$S($G(BDMGUI):57,1:IOSL)
 I BDMPREP=2 G CUML
 ;print ind audits first
 S BDMPD=0,BDMGUIC=0 F  S BDMPD=$O(^XTMP("BDMPO1",BDMJOB,BDMBTH,"AUDIT",BDMPD)) Q:BDMPD'=+BDMPD!(BDMQUIT)  D
 .I $G(BDMGUI),BDMGUIC W !,"ZZZZZZZ",!  ;maw
 .I BDMGUIC W:$D(IOF) @IOF
 .S BDMGUIC=1
 .S BDMPG=BDMPG+1 W:$G(BDMGUI) !! W "IHS Prediabetes Assessment of Care, 2025    DATE RUN: "_$$DATE^BDMS9B1(DT)_"   Page: "_BDMPG
 ...
 .W !?3,$$I(58),?6,"5 Tirzepatide [Mounjaro, Zepbound]"
...
CUML ;
 I BDMPREP=2!(BDMPREP=3) D CUML^BDMPO14
DONE ;
 K ^TMP($J)
 ...
 Q
I(I) ;
 Q $G(^XTMP("BDMPO1",BDMJOB,BDMBTH,"AUDIT",BDMPD,I))
 ;
PAGE ;
 Q:$G(BDMDSP)
 ...
 I $G(BDMGUI) W !!
 S BDMPG=BDMPG+1
 W "IHS Assessment of Prediabetes Care, 2025    DATE RUN: "_$$DATE^BDMS9B1(DT)_"   Page: "_BDMPG
