...
MORE ;EP
 ...
 S X="   Influenza vaccine (since August 1st): ",$E(X,42)=$$FLU^BDMS9B3(BDMSDFN) D S(X)
 S X="   Pneumococcal [PCV15, PCV20, PCV21, or PPSV23] (ever):",$E(X,59)=$$PNEU^BDMS9B4(BDMSDFN) D S(X)
 S X="   Td/Tdap/DTAP/DT (in past 10 yrs):",$E(X,42)=$$TD^BDMS9B3(BDMSDFN,(DT-100000),DT) D S(X)
 S X="   Tdap (ever):",$E(X,42)=$P($$TDAP^BDMDL1B(BDMSDFN,DT,"H",0),"  ",2,99) D S(X)
 S X="   Hepatitis B complete series (ever): ",$E(X,42)=$P($$HEP^BDMDL13(BDMSDFN,DT),"  ",2,99) D S(X)
 S X="   Shingrix/RZV complete series (ever): ",$E(X,42)=$P($P($$SHINGRIX^BDMDL19(BDMSDFN,$$DOB^AUPNPAT(BDMSDFN),DT,0),U,1),"  ",2) D S(X)
 S X="   Respiratory syncytial virus (RSV) vaccine (ever): ",$E(X,54)=$P($$RSV^BDMDL19(BDMSDFN,$$DOB^AUPNPAT(BDMSDFN),DT,1),"  ",2,3) D S(X)
 S X="Tuberculosis (TB):" D S(X,1)
 S X="   TB diagnosis (latent or active) documented ever: "_$P($$TBDX^BDMDL18(BDMSDFN,DT),U,2) D S(X)
 S BDMDX=$$TBDX^BDMDL18(BDMSDFN,DT)
 S Y=$$PPD^BDMDL18(BDMSDFN,DT) D S("   Last Documented TB Test: "_$P($P(Y,"||",1)," - ",2))
 S X="",$E(X,3)="   TB Test Result: "_$P($P(Y,"||",2)," - ",2) D S(X)
 I $P(BDMDX,U,1)=1!($E($P($$PPD^BDMDL18(BDMSDFN,DT),"||",2))=1) D
 .S X="   TB Treatment initiated (isoniazid, rifampin, rifapentine, others): " D S(X) S X="",$E(X,6)=$P($$TBTX^BDMDL12(BDMSDFN,DT)," - ",2) D S(X)
HEPC ;
 D S("Hepatitis C (HCV)",1)
 S X=$$HEPCDX^BDMDL1D(BDMSDFN,DT) D S("   Diagnosed with HCV ever: "_$P(X,"  ",2))
 S B=$$DOB^AUPNPAT(BDMSDFN)
 I $E(X)'=1 D S("   Screened for HCV ever: "_$P($$HEPSCR^BDMDL1D(BDMSDFN,DT),"  ",2))
R ;
 S X=$$DMRETDX^BDMDL1D(BDMSDFN,DT) D S("Retinopathy Diagnosed (ever): "_$S($E(X)=1:"Yes",1:"No"),1)
 ;
LEAMP ;
 S R=$$LEAMP^BDMDL1D(BDMSDFN,DT,2)
 ;
 D S("Amputation",1)
...
L ;
 ...
 S Y=$$ACRATIO(BDMSDFN)
 S X="   Quantitative UACR:",$E(X,25)=$P(Y,"|||"),$E(X,44)=$$DATE^BDMS9B1($P(Y,"|||",2)),$E(X,55)=$P(Y,"|||",3) D S(X)
...
EDUC ;EP
 ...
 I '$D(BDMY) S BDMX(1)="   <No Education Topics recorded in past year>" K BDMY G EDCPT
...
EDCPT ;
 F C="G0270","G0271",97802,97803,97804,99605,99606,98961 D
 .S G=$$CPTI^BDMDLDU(BDMSDFN,BDMSBEG,DT,+$$CODEN^ICPTCOD(C)) I G S D=$P(G,U,2) S BDMX(C)=C_"-"_$E($P($$CPT^ICPTCOD(C,DT),U,3),1,25),$E(BDMX(C),33)=$$DATE^BDMS9B1(D)
 K BDMY
 Q
EPRV(I) ;
 NEW P,D,%
...
EDUCREF ;EP
 ...
 .Q:$D(BDMP($$UP^XLFSTR($P(^AUTTEDT(BDMY,0),U))))
...
GFR(P) ;
 ...
 ...Q:'$D(^AUPNVLAB(Y,0))
 ...I $O(^LAB(60,$P(^AUPNVLAB(Y,0),U,1),2,0)) Q
