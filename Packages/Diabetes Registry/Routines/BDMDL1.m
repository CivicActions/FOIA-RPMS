BDMDL1 ; IHS/CMI/LAB - IHS Diabetes Audit 2026 ;
...
BEGIN ;EP - called from option
 ...
 W:$D(IOF) @IOF
REGASK ;
 W !!!,$$CTR("ASSESSMENT OF DIABETES CARE, 2026")
...
TYPE ;
 S BDMSTP=0
 K ^XTMP("BDMDM26",BDMJOB,BDMBTH),^TMP($J,"PATS")
 S BDMTYPE=""
...
DEMO ;
 ...
 I BDMPREP=2 S XBRP="",XBRC="^BDMDL10",XBRX="XIT^BDMDL1",XBNS="BDM" D ^XBDBQUE,XIT Q
 W ! S DIR(0)="S^P:PRINT Output;B:BROWSE Output on Screen",DIR("A")="Do you wish to",DIR("B")="B" K DA D ^DIR K DIR
 I $D(DIRUT) D XIT Q
 S BDMOPT=Y
 I Y="B" D BROWSE,XIT Q
 S XBRP="^BDMDL1P",XBRC="^BDMDL10",XBRX="XIT^BDMDL1",XBNS="BDM"
 D ^XBDBQUE
 D XIT
 Q
BROWSE ;
 S XBRP="VIEWR^XBLM(""^BDMDL1P"")"
 S XBRC="^BDMDL10",XBRX="XIT^BDMDL1",XBIOP=0 D ^XBDBQUE
 Q
IAEP ;EP
 S BDMTYPE="P"
 S BDMPREP=1
 ...
 G IF2
P ;
 S BDMSTP=0 K ^XTMP("BDMDM26",BDMJOB,BDMBTH),^TMP($J,"PATS")
 S BDMBEN=3
P1 ;
 K DIC S DIC="^AUPNPAT(",DIC(0)="AEMQ" D ^DIC K DIC
 I Y=-1,'$D(^XTMP("BDMDM26",BDMJOB,BDMBTH,"PATS")) W !,"No patients selected" S BDMSTP=1 Q
 I Y=-1 Q
 S ^XTMP("BDMDM26",BDMJOB,BDMBTH,"PATS",+Y)=""
 G P1
 ;
S ; Get patient name or cohort
 K ^XTMP("BDMDM26",BDMJOB,BDMBTH),^TMP($J,"PATS") S BDMSTP=0
...
E ;
 D ^BDMDL2
 I $G(BDMACTI)="" S BDMSTP=1
 I $G(BDMPREG)="" S BDMSTP=1
 I $G(BDMTAXI)="" S BDMSTP=1
 I BDMSTP W !!,"Missing Parameters...if you want to run E audit, choose E audit again ","and respond to all prompts." H 2
 Q
BENC(B) ;
...
RAND ;random sample or not
 ...
 G:$D(DIRUT) C
 I Y="A" S C=0 F  S C=$O(^TMP($J,"PATS",C)) Q:C'=+C  S X=$O(^TMP($J,"PATS",C,0)),^XTMP("BDMDM26",BDMJOB,BDMBTH,"PATS",X)=""
 ...
 I $D(DIRUT) S BDMSTP=1 Q
 S C=0 F N=1:1:BDMCNT Q:C=Y  S I=$R(BDMCNT) I I,$D(^TMP($J,"PATS",I)) S X=$O(^TMP($J,"PATS",I,0)),^XTMP("BDMDM26",BDMJOB,BDMBTH,"PATS",X)="",C=C+1 K ^TMP($J,"PATS",I,X)
 K ^TMP($J,"PATS")
 Q
...
XIT1 ;
 ...
 K ^XTMP("BDMDM26",BDMJOB,BDMBTH),BDMJOB,BDMBTH
XIT ;
...
WRITEF ;EP write flat file
 ...
 S I=$O(^BDMRECD("B","DM AUDIT 2026 EXPORT RECORD",0))
 ...
 S XBMED="F",XBFN=BDMFILE,XBTLE="SAVE OF DM AUDIT 2026 EXPORT RECORDS GENERATED BY -"_$P(^VA(200,DUZ,0),U)
 ...
 K ^XTMP("BDMDM26",BDMJOB,BDMBTH),BDMJOB,BDMBTH
ALERT ;
 ;send alert to user that file completed
 S BDMEDIR=$$GETDIR^BDMDL10()
 K XQA,XQATEXT,XQAMSG
 S XQA(DUZ)=""
 S XQAOPT=""
 S XQAROU=""
 ;S XQAFLG="D"
 S XQAMSG="The Diabetes Audit file has completed."
 S XQATEXT(1)=" "
 S XQATEXT(2)="The file can be found here: "_BDMEDIR_"  Filename: "_BDMFILE
 D SETUP^XQALERT
 K XQA,XQAMSG,XQATEXT
 Q
BDMG(BDMJOB,BDMBTH,BDMDMRG,BDMADAT,BDMTYPE,BDMSTMP,BDMPDP,BDMCOM,BDMRAND,BDMRCNT,BDMCMS,BDMSTAT,BDMPREP,BDMFILE,BDMDSP,BDMGIEN,BDMSDPI,BDMSDPG,BDMPPN,BDMDUZ2,BDMDEMO,BDMBEN,BDMDQ,BDMACTI,BDMPTAX) ;PEP - gui call
 ...
 S BDMQSRT=$G(BDMDQ)
 I $G(BDMPDP)]"" D
 . F I=1:1 D  Q:$P(BDMPDP,"~",I)=""
 .. Q:$P(BDMPDP,"~",I)=""
 .. S BDMPIEN=$O(^VA(200,"B",$P(BDMPDP,"~",I),0))
 .. Q:'BDMPIEN
 .. S BDMPRV(BDMPIEN)=""
 I $G(BDMPTAX) D
 . N X
 ...
 N BDMOPT  ;maw
 S BDMOPT="2026 Diabetes Program Audit"
 ...
 D ^XBFMK
 K ZTSAVE S ZTSAVE("*")=""
 S ZTIO="",ZTDTH=$$NOW^XLFDT,ZTRTN="GUIEP^BDMDL1",ZTDESC="GUI DM AUDIT" D ^%ZTLOAD
 D XIT
 Q
GUIEP ;EP - called from taskman
 ...
 D GUIR^XBLM("^BDMDL1P","^TMP($J,""BDMDM26"",")
 Q:$G(BDMDSP)  ;quit if to screen
 S X=0,C=0 F  S X=$O(^TMP($J,"BDMDM26",X)) Q:X'=+X  D
 .S BDMDATA=^TMP($J,"BDMDM26",X)
