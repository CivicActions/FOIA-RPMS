BDMDI1Y ; IHS/CMI/LAB - IHS Diabetes Audit 2021 ;
...
TYPE ;
 S BDMSTP=0
 K ^XTMP("BDMDM21",BDMJOB,BDMBTH),^TMP($J,"PATS")
 S BDMTYPE=""
...
IF ;PEP - called from BDM indivdual or epi
 S BDMSTP=0
 S BDMPREP=6
 D QUALCHK^BDMDI1J G:BDMSTP TYPE
ZIS ;
DEMO ;
 S BDMDEMO="I"
 I BDMTYPE'="P" D DEMOCHK^BDMUTL(.BDMDEMO) I BDMDEMO=-1 G TYPE
 I BDMTYPE="P" S BDMDEMO="I"
 I BDMPREP=2 S XBRP="",XBRC="^BDMDI10",XBRX="XIT^BDMDI1",XBNS="BDM" D ^XBDBQUE,XIT Q
 W ! S DIR(0)="S^P:PRINT Output;B:BROWSE Output on Screen",DIR("A")="Do you wish to",DIR("B")="P" K DA D ^DIR K DIR
 I $D(DIRUT) D XIT Q
 S BDMOPT=Y
 I Y="B" D BROWSE,XIT Q
 S XBRP="^BDMDI1P",XBRC="^BDMDI10",XBRX="XIT^BDMDI1",XBNS="BDM"
 D ^XBDBQUE
 D XIT
 Q
BROWSE ;
 S XBRP="VIEWR^XBLM(""^BDMDI1P"")"
 S XBRC="^BDMDI10",XBRX="XIT^BDMDI1",XBIOP=0 D ^XBDBQUE
 Q
IAEP ;EP
 S BDMTYPE="P"
 S BDMPREP=1
 ...
 G IF
P ;
 S BDMSTP=0 K ^XTMP("BDMDM21",BDMJOB,BDMBTH),^TMP($J,"PATS")
 S BDMBEN=3
P1 ;
 K DIC S DIC="^AUPNPAT(",DIC(0)="AEMQ" D ^DIC K DIC
 I Y=-1,'$D(^XTMP("BDMDM21",BDMJOB,BDMBTH,"PATS")) W !,"No patients selected" S BDMSTP=1 Q
 I Y=-1 Q
 S ^XTMP("BDMDM21",BDMJOB,BDMBTH,"PATS",+Y)=""
 G P1
E ;
 D ^BDMDI2
 Q
 ;
S ; 
 K ^XTMP("BDMDM21",BDMJOB,BDMBTH),^TMP($J,"PATS") S BDMSTP=0
...
PREG ;
 ...
 .Q:$P(^DPT(P,0),U,2)'="F"
 .;W ".",P
 .I $$PREG^BDMDI1B(P,BDMBDAT,BDMADAT,1,1,BDMBDAT,BDMADAT) K ^TMP($J,"PATS",X)
...
WRITEF ;EP write flat file
 ...
 S I=$O(^BDMRECD("B","DM AUDIT 2021 EXPORT RECORD",0))
 ...
 S XBMED="F",XBFN=BDMFILE,XBTLE="SAVE OF DM AUDIT 2021 EXPORT RECORDS GENERATED BY -"_$P(^VA(200,DUZ,0),U)
...
BDMG(BDMJOB,BDMBTH,BDMDMRG,BDMADAT,BDMTYPE,BDMSTMP,BDMPDP,BDMCOM,BDMRAND,BDMRCNT,BDMCMS,BDMSTAT,BDMPREP,BDMFILE,BDMDSP,BDMGIEN,BDMSDPI,BDMSDPG,BDMPPN,BDMDUZ2,BDMDEMO,BDMBEN,BDMDQ,BDMACTI,BDMPTAX) ;PEP - gui call
 ...
 N BDMOPT  ;maw
 S BDMOPT="2021 Data Quality Report"
 S BDMPREP=6
 D NOW^%DTC
 S BDMNOW=$G(%)
 K DD,D0,DIC
 ...
 D ^XBFMK
 K ZTSAVE S ZTSAVE("*")=""
 S ZTIO="",ZTDTH=$$NOW^XLFDT,ZTRTN="GUIEP^BDMDI1",ZTDESC="GUI DM AUDIT" D ^%ZTLOAD
 D XIT
 Q
GUIEP ;EP - called from taskman
 ...
 D GUIR^XBLM("^BDMDI1P","^TMP($J,""BDMDM21"",")
 Q:$G(BDMDSP)  ;quit if to screen
 S X=0,C=0 F  S X=$O(^TMP($J,"BDMDM21",X)) Q:X'=+X  D
 .S BDMDATA=^TMP($J,"BDMDM21",X)
