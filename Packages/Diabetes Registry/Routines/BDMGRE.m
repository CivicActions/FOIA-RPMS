...
DPCS(RETVAL,BDMSTR) ;-- return the DPCS report
 ...
 Q
 ;
PDPCS(RETVAL,BDMSTR) ;-- return the PDPCS report
 N BDMPAT,BDMDATA,BDMI,BDMERR,P
 S P="|"
 S BDMPAT=$P(BDMSTR,P)
 S BDMI=0
 ...
 S @RETVAL@(BDMI)="T00250DATA"_$C(30)
 S DFN=BDMPAT
 D GUIR^XBLM("PRINT^BDMPDSP","^XTMP(""BDMPDSP"",$J)")
 I '$D(^XTMP("BDMPDSP",$J)) D  Q
 . S BDMI=BDMI+1
 . S ^BDMTMP($J,BDMI)="NO DATA"_$C(30)
 . S ^BDMTMP($J,BDMI+1)=$C(31)
 S BDMDA=.5 F  S BDMDA=$O(^XTMP("BDMPDSP",$J,BDMDA)) Q:'BDMDA  D
 . N BDMDATA
 . S BDMI=BDMI+1
 . S BDMDATA=$G(^XTMP("BDMPDSP",$J,BDMDA))
 . S ^BDMTMP($J,BDMI)=BDMDATA_$C(30)
 S ^BDMTMP($J,BDMI+1)=$C(31)
 K ^XTMP("BDMPDSP",$J)
 Q
