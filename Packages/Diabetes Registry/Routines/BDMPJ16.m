BDMPJ16 ; IHS/CMI/LAB - 2003 DIABETES AUDIT ;
...
ASPIRIN(P,BDATE,EDATE) ;EP
 ...
 I 'T Q "None"
 S X=0 F  S X=$O(^ATXAX(T,21,X)) Q:X'=+X!(G)  S G=$$REFUSAL^BDMPJ17(P,50,$P(^ATXAX(T,21,X,0),U),BDATE,EDATE)
 I G,$P(G,U,2)'="N" Q "Refused"
 I G Q "None - Not Medically Indicated"
 NEW T S T=$O(^ATXAX("B","DM AUDIT ANTI-PLATELET DRUGS",0))
 I 'T Q "None"
 S (X,G)=0 F  S X=$O(^ATXAX(T,21,X)) Q:X'=+X!(G)  S G=$$REFUSAL^BDMPJ17(P,50,$P(^ATXAX(T,21,X,0),U),BDATE,EDATE)
 I G,$P(G,U,2)'="N" Q "Refused"
 I G Q "None - Not Medically Indicated"
 ;
 Q "None"
...
THERAPY(P,BD,EDATE) ;EP - therapy code for epi
 ...
 S STR="",TNAME=""
 S X=$$SULF^BDMDJ12(P,BD,EDATE)
 I X]"" S STR=STR_5
 S X=$$MET^BDMDJ12(P,BD,EDATE)
 I X]"" S STR=STR_2
 S X=$$ACAR^BDMDJ12(P,BD,EDATE)
 I X]"" S STR=STR_3
 S X=$$TROG^BDMDJ12(P,BD,EDATE)
 I X]"" S STR=STR_"4"
 I STR]"" Q STR
 Q 1
 ;
