BDMDL1MR ; IHS/CMI/LAB - IHS Diabetes Audit 2026 ;
...
BEN ;GET BENEFICIARY
 ...
 K ^XTMP("BDMDM26",BDMJOB,BDMBTH),^TMP($J,"PATS"),^TMP($J,"PATSNODUPE")
 ;LOOP EACH REGISTER
...
IF ;PEP - called from BDM indivdual or epi
 ...
 S BDMPREP=Y
 I BDMPREP=2 D FLAT^BDMDL1 Q:BDMSTP
 ;
IF2 ;
...
DEMO ;
 ...
 S BDMTYPE="C"
 I BDMPREP=2 S XBRP="",XBRC="^BDMDL10",XBRX="XIT^BDMDL1MR",XBNS="BDM" D ^XBDBQUE,XIT Q
 W ! S DIR(0)="S^P:PRINT Output;B:BROWSE Output on Screen",DIR("A")="Do you wish to",DIR("B")="B" K DA D ^DIR K DIR
 I $D(DIRUT) D XIT Q
 S BDMOPT=Y
 I Y="B" D BROWSE,XIT Q
 S XBRP="^BDMDL1P",XBRC="^BDMDL10",XBRX="XIT^BDMDL1MR",XBNS="BDM"
 D ^XBDBQUE
 D XIT
 Q
BROWSE ;
 S XBRP="VIEWR^XBLM(""^BDMDL1P"")"
 S XBRC="^BDMDL10",XBRX="XIT^BDMDL1MR",XBIOP=0 D ^XBDBQUE
 Q
BENC(B) ;
...
PREG ;
 ...
 .Q:$P(^DPT(P,0),U,2)'="F"
 .;W ".",P
 .I $$PREG^BDMDL1B(P,BDMBDAT,BDMADAT,1,1,BDMBDAT,BDMADAT) K ^TMP($J,"PATS",X,P)
