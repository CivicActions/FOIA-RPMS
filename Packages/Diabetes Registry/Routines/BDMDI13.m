BDMDI13 ; IHS/CMI/LAB - 2021 DIABETES AUDIT ;
...
PPSV23(P,BDATE,EDATE) ;EP
 ...
 S %=$$CPT^BDMDIDU(P,BDATE,EDATE,$O(^ATXAX("B","BGP PPSV23 CPT CODES",0)),5)
 I $P(BDMOPNU,U,1)<$P(%,U,1) S BDMOPNU=$P(%,U,1)
 S %=$$TRAN^BDMDIDU(P,BDATE,EDATE,$O(^ATXAX("B","BGP PPSV23 CPT CODES",0)),5)
 I $P(BDMOPNU,U,1)<$P(%,U,1) S BDMOPNU=$P(%,U,1)
 I BDMOPNU]"" Q BDMOPNU
 Q ""
BI() ;
...
HTNDX(P,EDATE) ;EP - is HTN on problem list
 ...
 .I $P($G(^AUPNPROB(X,800)),U,1)]"",$$SNOMED^BDMUTL(2021,"PXRM ESSENTIAL HYPERTENSION",$P(^AUPNPROB(X,800),U,1)) S G=1
 .Q
 I G Q "1  Yes"
 K BDM
...
PLDMDOO(P,F) ;EP
 ...
 ..Q
 .I $P($G(^AUPNPROB(X,800)),U,1)]"",$$SNOMED^BDMUTL(2021,"PXRM DIABETES",$P(^AUPNPROB(X,800),U,1)) D
 ..I $P(^AUPNPROB(X,0),U,13)]"" S D($P(^AUPNPROB(X,0),U,13))=""
 .Q
...
PLDMDXS(P) ;EP - get all DM dxs from problem list
 ...
 .I $P($G(^AUPNPROB(X,800)),U,1)]"",$$SNOMED^BDMUTL(2021,"PXRM DIABETES",$P(^AUPNPROB(X,800),U,1)) S:D]"" D=D_";" S D=D_"SNOMED: "_$P(^AUPNPROB(X,800),U,1)
 .Q
 Q D
 ;
...
INCHES ;
 NEW F,FI,Z
 S (X,Z)=$$LASTHT^BDMDI13(BDMPD,BDMRED,"I")
 Q:X=""
 S X=X/12  ;get feet
 S F=$P(X,".")
...
HEP(P,EDATE,NR,F) ;EP
 ...
 I BDMHEP>2 Q "1  Yes"
I ;;
 ;NEW FOR 2021 AUDIT CHECK FOR 2 OF CVX 189
 S C="189"
 D GETIMMS^BDMUTL(P,EDATE,C,.BDMX)
 ;go through and set into array if 10 days apart
 S X=0 F  S X=$O(BDMX(X)) Q:X'=+X  S BDMHEP(X)=""
 ...
 I $$PLTAX^BDMDI12(P,"BGP HEP EVIDENCE") Q "3  Immune by DX"
