BDMDL1D ; IHS/CMI/LAB - BDM Continuation of BDMDL1C 1/11/2014 1:54:57 PM ;
...
MICRO ;EP
 ...
 ...D SETV
 ...Q
 I '$D(BDM(1)) S %1="" D ACRATIOM^BDMDL1C Q %1
 D SETN
...
FGLUCOSE ;EP
 ...
 Q $P(^AUPNVLAB(+$P(BDM(N),U,4),0),U,4)_$S($P(^AUPNVLAB(+$P(BDM(N),U,4),0),U,4)]"":" mg/dl ",1:"")_U_$$FMTE^XLFDT($P(BDM(N),U),5)_U_$$VAL^XBDIQ1(9000010.09,+$P(BDM(N),U,4),.01)
G75 ;EP
 I $G(F)="" S F="E"
 NEW BDM,X,%,E,R,V,BDMLT,BDMOT,B,D,L,J,BDMC,BDMV,V
 K BDM
...
ASPIRIN ;EP
 ...
 S BDM=$$PRESD^BDMDL18(P,$O(^ATXAX("B","DM AUDIT ASPIRIN DRUGS",0)),ED,186)
 I BDM]"" Q BDM
 S BDM=$$PRESD^BDMDL18(P,$O(^ATXAX("B","DM AUDIT ANTIPLT/ANTICOAG RX",0)),ED,186)
 I BDM]"" Q BDM
 S BDM=$$PRESD^BDMDL18(P,$O(^ATXAX("B","DM AUDIT ANTI-PLATELET DRUGS",0)),ED,186)
 I BDM]"" Q BDM
 Q "2  No"
 ;
 ;
HEPCDX(P,EDATE) ;EP - dx of HEP C ever?
 ...
 .I $$ICD^ATXAPI(Y,T,9) S G="1  Yes - "_"Problem List "_$$VAL^XBDIQ1(9000011,X,.01)_" "_$$DATE^BDMS9B1($P(^AUPNPROB(X,0),U,8)) Q
 .I $P($G(^AUPNPROB(X,800)),U,1)]"",$$SNOMED^BDMUTL(2026,"PXRM HEPATITIS C",$P(^AUPNPROB(X,800),U,1)) S G=1
 .Q
 I G Q G
 Q "2  No"
HEPSCR(P,EDATE) ;
 ...
 NEW X,G,T,%,BDMC,BDMLT,L,D,J,R,Y
 ;now get all loinc/taxonomy tests
 S BDMC=""
 S T=$O(^ATXAX("B","BGP HEP C TEST LOINC CODES",0))
 S BDMLT=$O(^ATXLAB("B","BGP HEP C TESTS TAX",0))
 S E=9999999-EDATE S D=E-1 F  S D=$O(^AUPNVLAB("AE",P,D)) Q:D'=+D!(BDMC)  D
 ...
 ....S BDMC="1  Yes "_$$DATE^BDMS9B1((9999999-D))_" "_$$VAL^XBDIQ1(9000010.09,X,.01) I $$VAL^XBDIQ1(9000010.09,X,.04)]"" S BDMC=BDMC_" ("_$$VAL^XBDIQ1(9000010.09,X,.04)_")" Q
 ...Q:'T
 ...S J=$P($G(^AUPNVLAB(X,11)),U,13) Q:J=""
 ...Q:'$$LOINC(J,T)
 ...S BDMC="1  Yes - "_$$DATE^BDMS9B1((9999999-D))_" "_$$VAL^XBDIQ1(9000010.09,X,.01) I $$VAL^XBDIQ1(9000010.09,X,.04)]"" S BDMC=BDMC_" ("_$$VAL^XBDIQ1(9000010.09,X,.04)_")"
 ...Q
 I BDMC Q BDMC
 S %="",E=+$$CODEN^ICPTCOD(86803),%=$$CPTI^BDMDLDU(P,$$DOB^AUPNPAT(P),EDATE,E)
...
DMRETDX(P,EDATE) ;EP - is DM RETINOPATHY on problem list
 ...
 ..I $$SNOMED^BDMUTL(2026,"PXRM BGP DM RETINOPATHY",$P(^AUPNPROB(X,800),U,1)) S G=X
 .Q
 ...
 .S I=0
 .I $$SNOMED^BDMUTL(2026,"PXRM BGP DM RETINOPATHY",S) S I=1
 .Q:'I
...
LEAMP(P,EDATE,F) ;EP - dx of AMP ever?
 ...
 S S="" F  S S=$O(^AUPNVPOV("ASNC",P,S)) Q:S=""!(G)  D
 .S I=0
 .I $$SNOMED^BDMUTL(2026,"PXRM BGP DM BTK AMP",S) S I=1
 .I $$SNOMED^BDMUTL(2026,"PXRM BGP DM ATK AMP",S) S I=1
 .Q:'I
 .S D=0 F  S D=$O(^AUPNVPOV("ASNC",P,S,D)) Q:D=""!(G)  D
 ..S Y=9999999-D
 ..Q:Y>EDATE
 ...
 .Q:$P(^AUPNPROB(X,0),U,8)>EDATE  ;if added to pl after end of time period, no go
 .Q:$P(^AUPNPROB(X,0),U,13)>EDATE  ;if doo after end of time period, no go
 .S Y=$P(^AUPNPROB(X,0),U)
 .Q:$P(^AUPNPROB(X,0),U,12)="D"
 .;Q:$P(^AUPNPROB(X,0),U,12)="I"
 .I T,$$ICD^ATXCHK(Y,T,9) S G=$S(F=1:"1  Yes - "_"Problem List "_$$VAL^XBDIQ1(9000011,X,.01)_" "_$$DATE^BDMS9B1($P(^AUPNPROB(X,0),U,8)),1:"Yes") Q
 .I T1,$$ICD^ATXCHK(Y,T1,9) S G=$S(F=1:"1  Yes - "_"Problem List "_$$VAL^XBDIQ1(9000011,X,.01)_" "_$$DATE^BDMS9B1($P(^AUPNPROB(X,0),U,8)),1:"Yes") Q
 .I $P($G(^AUPNPROB(X,800)),U,1)]"",$$SNOMED^BDMUTL(2026,"PXRM BGP DM BTK AMP",$P(^AUPNPROB(X,800),U,1)) S G=$S(F=1:"1  Yes - "_"Problem List "_$$VAL^XBDIQ1(9000011,X,80001)_" "_$$DATE^BDMS9B1($P(^AUPNPROB(X,0),U,8)),1:"Yes") Q
 .I $P($G(^AUPNPROB(X,800)),U,1)]"",$$SNOMED^BDMUTL(2026,"PXRM BGP DM ATK AMP",$P(^AUPNPROB(X,800),U,1)) S G=$S(F=1:"1  Yes - "_"Problem List "_$$VAL^XBDIQ1(9000011,X,80001)_" "_$$DATE^BDMS9B1($P(^AUPNPROB(X,0),U,8)),1:"Yes") Q
 .Q
 I G]"" Q G
 S X=""
