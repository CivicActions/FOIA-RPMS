BDMGTA ; cmi/anch/maw - BDM DMS GUI Table Lookup ;
 ;;2.0;BDM DIABETES MANAGEMENT SYSTEM;**1,4,8,16,17**;JUN 14, 2007;Build 138
 ;
 ;
...
MSR(BDMRET) ;-- measurement type
 ...
 . Q:$P($G(^AUTTMST(BDMMST,0)),U,4)  ;maw p17 102736
 . S BDMI=BDMI+1
...
TD(BDMRET) ;-- td immunization table
 ...
 S BDMIMM=0 F  S BDMIMM=$O(^AUTTIMM("D",BDMIMM)) Q:BDMIMM=""  D
 . S BDMIEN=0 F  S BDMIEN=$O(^AUTTIMM("D",BDMIMM,BDMIEN)) Q:'BDMIEN  D
 .. Q:'$$TD^BDMEDMU3(BDMIEN)
 .. S ^BDMTMP($J,BDMI)=BDMIMM_$C(30)
 .. S BDMI=BDMI+1
 S ^BDMTMP($J,BDMI)=$C(31)_$G(BDMERR)
 Q
 ;
TDAP(BDMRET) ;-- tdap immunization table
 S X="MERR^BDMGU",@^%ZOSF("TRAP") ; m error trap
 N BDMIMM,BDMI,BDMERR,BDMIEN
 K ^BDMTMP($J)
 S BDMRET="^BDMTMP("_$J_")"
 ...
 S BDMIMM=0 F  S BDMIMM=$O(^AUTTIMM("D",BDMIMM)) Q:BDMIMM=""  D
 . S BDMIEN=0 F  S BDMIEN=$O(^AUTTIMM("D",BDMIMM,BDMIEN)) Q:'BDMIEN  D
 .. Q:'$$TDAP^BDMEDMU3(BDMIEN)
 .. S ^BDMTMP($J,BDMI)=BDMIMM_$C(30)
 .. S BDMI=BDMI+1
 S ^BDMTMP($J,BDMI)=$C(31)_$G(BDMERR)
 Q
 ;
SHINGRIX(BDMRET) ;-- shingrix immunization table
 S X="MERR^BDMGU",@^%ZOSF("TRAP") ; m error trap
 N BDMIMM,BDMI,BDMERR,BDMIEN
 K ^BDMTMP($J)
 S BDMRET="^BDMTMP("_$J_")"
...
SKT(BDMRET) ;-- skin test table
 ...
 .. Q:$P($G(^AUTTSK(BDMIEN,0)),U,3)  ;maw p17 102736
 .. S BDMI=BDMI+1
...
XAM(BDMRET) ;-- exam table
 ...
 .. Q:$P($G(^AUTTEXAM(BDMIEN,0)),U,4)  ;maw p17 102736
 .. S BDMI=BDMI+1
...
RFL(BDMRET) ;-- refusal types
 ...
 S BDMRFL=0 F  S BDMRFL=$O(^BDMDMURT("B",BDMRFL)) Q:BDMRFL=""  D
 . S BDMIEN=0 F  S BDMIEN=$O(^BDMDMURT("B",BDMRFL,BDMIEN)) Q:'BDMIEN  D
 .. S BDMI=BDMI+1
