BDMDN1D ; IHS/CMI/LAB - BDM Continuation of BDMDN1C 1/11/2014 1:54:57 PM ;
...
MICRO ;EP
 ...
 ...D SETV
 ...Q
 I '$D(BDM(1)) S %1="" D ACRATIOM^BDMDN1C Q %1
 D SETN
...
ASPIRIN ;EP
 ...
 S BDM=$$PRESD^BDMDN18(P,$O(^ATXAX("B","DM AUDIT ASPIRIN DRUGS",0)),ED,186)
 I BDM]"" Q BDM
 S BDM=$$PRESD^BDMDN18(P,$O(^ATXAX("B","DM AUDIT ANTIPLT/ANTICOAG RX",0)),ED,186)
 I BDM]"" Q BDM
 S BDM=$$PRESD^BDMDN18(P,$O(^ATXAX("B","DM AUDIT ANTI-PLATELET DRUGS",0)),ED,186)
 I BDM]"" Q BDM
 Q "2  No"
 ;
 ;
HEPCDX(P,EDATE) ;EP - dx of HEP C ever?
 ...
 .I $P($G(^AUPNPROB(X,800)),U,1)]"",$$SNOMED^BDMUTL(2024,"PXRM HEPATITIS C",$P(^AUPNPROB(X,800),U,1)) S G=1
 .Q
 I G Q G
 Q "2  No"
HEPSCR(P,EDATE) ;
 ...
 ...Q
 I BDMC Q BDMC
 S %="",E=+$$CODEN^ICPTCOD(86803),%=$$CPTI^BDMDNDU(P,$$DOB^AUPNPAT(P),EDATE,E)
...
DMRETDX(P,EDATE) ;EP - is DM RETINOPATHY on problem list
 ...
 ..I $$SNOMED^BDMUTL(2024,"PXRM BGP DM RETINOPATHY",$P(^AUPNPROB(X,800),U,1)) S G=X
 .Q
 ...
 .S I=0
 .I $$SNOMED^BDMUTL(2024,"PXRM BGP DM RETINOPATHY",S) S I=1
 .Q:'I
...
LEAMP(P,EDATE,F) ;EP - dx of AMP ever?
 ...
 S S="" F  S S=$O(^AUPNVPOV("ASNC",P,S)) Q:S=""!(G)  D
 .S I=0
 .I $$SNOMED^BDMUTL(2024,"PXRM BGP DM BTK AMP",S) S I=1
 .I $$SNOMED^BDMUTL(2024,"PXRM BGP DM ATK AMP",S) S I=1
 .Q:'I
 .S D=0 F  S D=$O(^AUPNVPOV("ASNC",P,S,D)) Q:D=""!(G)  D
 ..S Y=9999999-D
 ..Q:Y>EDATE
 ...
 .I $P($G(^AUPNPROB(X,800)),U,1)]"",$$SNOMED^BDMUTL(2024,"PXRM BGP DM BTK AMP",$P(^AUPNPROB(X,800),U,1)) S G=$S(F=1:"1  Yes - "_"Problem List "_$$VAL^XBDIQ1(9000011,X,80001)_" "_$$DATE^BDMS9B1($P(^AUPNPROB(X,0),U,8)),1:"Yes") Q
 .I $P($G(^AUPNPROB(X,800)),U,1)]"",$$SNOMED^BDMUTL(2024,"PXRM BGP DM ATK AMP",$P(^AUPNPROB(X,800),U,1)) S G=$S(F=1:"1  Yes - "_"Problem List "_$$VAL^XBDIQ1(9000011,X,80001)_" "_$$DATE^BDMS9B1($P(^AUPNPROB(X,0),U,8)),1:"Yes") Q
 .Q
 I G]"" Q G
 S X=""
