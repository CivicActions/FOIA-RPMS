BDMDL15 ; IHS/CMI/LAB -IHS -CUMULATIVE REPORT 03 Feb 2014 5:38 PM ;
...
GENDER ;
 ...
 S V=$G(^XTMP("BDMDM26",BDMJOB,BDMBTH,"AUDIT",BDMPD,20))
...
TYPE ;
 ...
 S X=$$TYPE^BDMDL16(BDMPD,BDMDMRG,BDMADAT,$G(BDMMULTR),.BDMREGLT)
...
DURDMC ;
 ...
 ;
 S V=$$DURDM^BDMDL16(BDMPD,BDMDMRG,BDMADAT,$G(BDMMULTR),.BDMREGLT)
...
BMI ;
 ...
 ;S V=$G(^XTMP("BDMDM26",BDMJOB,BDMBTH,"AUDIT",BDMPD,112))
 S H=$$LASTHT^BDMDL13(BDMPD,BDMRED,"I") S:H]"" H=$$STRIP^XLFSTR($J(H,5,2)," ")
 S W=+$$LASTWT^BDMDL13(BDMPD,BDMBDAT,BDMRED),W=$S(W=0:"",1:W) I W]"" S W=W\1
 S V=$$BMIEPI^BDMDL19(H,W)
 ...
 .I $$OW^BDMDL14(BDMPD,V,BDMADAT) S $P(BDMCUML(40),U,3)=$P(BDMCUML(40),U,3)+1 Q
 .I $$OB^BDMDL14(BDMPD,V,BDMADAT) S $P(BDMCUML(40),U,4)=$P(BDMCUML(40),U,4)+1
 .I $$SOB^BDMDL14(BDMPD,V,BDMADAT) S $P(BDMCUML(40),U,7)=$P(BDMCUML(40),U,7)+1,BDMSEVOB=1 Q
HGB ;
...
HTN ;410
 ...
 S H=$E($G(^XTMP("BDMDM26",BDMJOB,BDMBTH,"AUDIT",BDMPD,34)))
 ...
 S A=$E($G(^XTMP("BDMDM26",BDMJOB,BDMBTH,"AUDIT",BDMPD,60)))  ;ACE/ARB USAGE
...
TBSTAT ;
 ...
 S V=$G(^XTMP("BDMDM26",BDMJOB,BDMBTH,"AUDIT",BDMPD,71))   ;TB DX
 I $E(V)=1 S BDMTBDX=1,$P(BDMCUML(70),U,8)=$P(BDMCUML(70),U,8)+1
 S V=$G(^XTMP("BDMDM26",BDMJOB,BDMBTH,"AUDIT",BDMPD,70))  ;TB TEST DONE
 ...
 S BDMTBRE="" I 'BDMTBDX,($E(V)=1!($E(V)=2)) S BDMTBRE=$E($P($$PPD^BDMDL18(BDMPD,BDMADAT),"||",2))
 I BDMTBDX!(BDMTBRE=1) S $P(BDMCUML(70),U,3)=$P(BDMCUML(70),U,3)+1
 S V=$$TBINH^BDMDL18(BDMPD,BDMADAT) I $E(V)=1 S $P(BDMCUML(70),U,5)=$P(BDMCUML(70),U,5)+1
 I BDMTBTD=1,$E($P($$PPD^BDMDL18(BDMPD,BDMADAT),"||",2))=2 S $P(BDMCUML(70),U,6)=$P(BDMCUML(70),U,6)+1
 I BDMTBDOT S $P(BDMCUML(70),U,10)=$P(BDMCUML(70),U,10)+1  ;either test or dx patch 17
 ;GET DOD
 S X=$$LASTNP^BDMDL18(BDMPD,BDMADAT)
 I X D
 .I $$DODX^BDMDL16(BDMPD,BDMDMRG,"I",$G(BDMMULTR),.BDMREGLT)="" Q
 .S D=$$DODX^BDMDL16(BDMPD,BDMDMRG,"I",$G(BDMMULTR),.BDMREGLT) I D>X Q
...
TOBSCR ;
 ...
 S V=$G(^XTMP("BDMDM26",BDMJOB,BDMBTH,"AUDIT",BDMPD,215))  ;S^U
 S V1=$G(^XTMP("BDMDM26",BDMJOB,BDMBTH,"AUDIT",BDMPD,28))  ;COUNSEL
...
ENDS ;
 ...
 S T=$P($G(^XTMP("BDMDM26",BDMJOB,BDMBTH,"AUDIT",BDMPD,215)),U,2)  ;TOBACCO USER
...
DMTX ;diabetes treatment
 ...
 S V=$$THERAPY^BDMDL16(BDMPD,BDM6MBD,BDMRED)  ;^ pieced with each item  or =1 for diet alone or =14 for refused
...
ASPIRIN ;
 ...
 I $E($G(^XTMP("BDMDM26",BDMJOB,BDMBTH,"AUDIT",BDMPD,116)))'=1 G LIPIDAG  ;not cvd
 S $P(BDMCUML(100),U,2)=$P(BDMCUML(100),U,2)+1  ;total with cvd
 S V=$G(^XTMP("BDMDM26",BDMJOB,BDMBTH,"AUDIT",BDMPD,62))
...
LIPIDAG ;lipid agents
 ...
 S V=$E($$STATIN^BDMDL16(BDMPD,BDM6M,BDMADAT))
 ...
 S C=0
 I $E($G(^XTMP("BDMDM26",BDMJOB,BDMBTH,"AUDIT",BDMPD,116)))=1 D
 ...
 ;
LIPID ;115
 S V=$G(^XTMP("BDMDM26",BDMJOB,BDMBTH,"AUDIT",BDMPD,61))
 S L=$$LDL^BDMDL18(BDMPD,BDMBDAT,BDMADAT,"I"),L=$P(L,U)
 S T=$$CHOL^BDMDL18(BDMPD,BDMBDAT,BDMADAT,"I"),T=$P(T,U)
 S T=$$STV^BDMDL18(T,,0)
 S L=$$STV^BDMDL18(L,,0)
 ;
DEP ;
 S $P(BDMCUML(300),U,2)=$P(BDMCUML(300),U,2)+1
 S S=$G(^XTMP("BDMDM26",BDMJOB,BDMBTH,"AUDIT",BDMPD,210))  ;SCREENING P16
 I $E(S)="1" S $P(BDMCUML(300),U,3)=$P(BDMCUML(300),U,3)+1
 S D=$G(^XTMP("BDMDM26",BDMJOB,BDMBTH,"AUDIT",BDMPD,200))  ;DX P16
...
EXAMS ;
 ...
 S V=$G(^XTMP("BDMDM26",BDMJOB,BDMBTH,"AUDIT",BDMPD,108))
...
LEAMP ;
 ...
 S V=$G(^XTMP("BDMDM26",BDMJOB,BDMBTH,"AUDIT",BDMPD,230))
...
SYSMEAN(P,BDATE,EDATE) ;EP
 NEW X,Z,C,Y
 S X=$$BPS^BDMDL13(P,BDATE,EDATE,"I")
 I X="" Q ""
