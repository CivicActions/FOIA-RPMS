BDMDN1P ; IHS/CMI/LAB - 2024 DIABETES AUDIT PRINT 15 Dec 2016 3:05 PM ; 24 Oct 2019  3:42 PM
 ;;2.0;DIABETES MANAGEMENT SYSTEM;**17**;JUN 14, 2007;Build 138
 ;
 ;
 D LOG^BUSAAPI("O","P","P",$S($G(XQY0)]"":$P(XQY0,U),1:"BDMDN1P"),"DM 2024 AUDIT REPORT "_$S(BDMPREP=6:"QUALITY CHECK RPT",1:""))
 S BDMQUIT=0,BDMPG=0,BDMIOSL=$S($G(BDMGUI):57,1:IOSL)
 I BDMPREP=3 G CUML
 I BDMPREP=5 G CUML
 I BDMPREP=6 G CUML
 ;print ind audits first
 S BDMPD=0,BDMGUIC=0 F  S BDMPD=$O(^XTMP("BDMDM24",BDMJOB,BDMBTH,"AUDIT",BDMPD)) Q:BDMPD'=+BDMPD!(BDMQUIT)  D
 .I $G(BDMGUI),BDMGUIC W !,"ZZZZZZZ",!  ;maw
 .I BDMGUIC W:$D(IOF) @IOF
 .S BDMGUIC=1
 .S BDMPG=BDMPG+1 W:$G(BDMGUI) !! W "IHS Diabetes Care and Outcomes Audit, 2024    DATE RUN: "_$$DATE^BDMS9B1(DT)_"   Page: "_BDMPG
 ...
 .I $$AGE^AUPNPAT(BDMPD,BDMADAT)<40!($E($G(^XTMP("BDMDM24",BDMJOB,BDMBTH,"AUDIT",BDMPD,300)))=3) W !?5,"This is only calculated for patients 40 years of age and older without",!?5,"a statin allergy or intolerance." I 1
...
N1 .W !!,"Local Questions ",$$LOCN^BDMDN10(BDMPD,BDMDMRG)
 .W !!?2,"Select one:"
 .W !?2,"Text: ",$$LOCT^BDMDN10(BDMPD,BDMDMRG)
 .I $E(IOST,1,2)'="P-" W !! S DIR(0)="E" D ^DIR K DIR
CUML ;
 I BDMPREP=4!(BDMPREP=3) D CUML^BDMDN14
 I BDMPREP=5 D SDPI16^BDMDN1U
 I BDMPREP=6 D QUALCHK^BDMDN1V
DONE ;
 K ^TMP($J)
 ...
 Q
I(I) ;
 Q $G(^XTMP("BDMDM24",BDMJOB,BDMBTH,"AUDIT",BDMPD,I))
 ;
PAGE ;
 Q:$G(BDMDSP)
 ...
 I $G(BDMGUI) W !!
 S BDMPG=BDMPG+1
 W "IHS Diabetes Care and Outcomes Audit, 2024    DATE RUN: "_$$DATE^BDMS9B1(DT)_"   Page: "_BDMPG
