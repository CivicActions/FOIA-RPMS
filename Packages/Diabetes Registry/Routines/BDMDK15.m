BDMDK15 ; IHS/CMI/LAB -IHS -CUMULATIVE REPORT 03 Feb 2014 5:38 PM ;
...
GENDER ;
 ...
 S V=$G(^XTMP("BDMDM23",BDMJOB,BDMBTH,"AUDIT",BDMPD,20))
...
TYPE ;
 ...
 S X=$$TYPE^BDMDK16(BDMPD,BDMDMRG,BDMADAT,$G(BDMMULTR),.BDMREGLT)
...
DURDMC ;
 ...
 ;
 S V=$$DURDM^BDMDK16(BDMPD,BDMDMRG,BDMADAT,$G(BDMMULTR),.BDMREGLT)
...
BMI ;
 ...
 ;S V=$G(^XTMP("BDMDM23",BDMJOB,BDMBTH,"AUDIT",BDMPD,112))
 S H=$$LASTHT^BDMDK13(BDMPD,BDMRED,"I") S:H]"" H=$$STRIP^XLFSTR($J(H,5,2)," ")
 S W=+$$LASTWT^BDMDK13(BDMPD,BDMBDAT,BDMRED),W=$S(W=0:"",1:W) I W]"" S W=W\1
 S V=$$BMIEPI^BDMDK19(H,W)
 ...
 .I $$OW^BDMDK14(BDMPD,V,BDMADAT) S $P(BDMCUML(40),U,3)=$P(BDMCUML(40),U,3)+1 Q
 .I $$OB^BDMDK14(BDMPD,V,BDMADAT) S $P(BDMCUML(40),U,4)=$P(BDMCUML(40),U,4)+1
 .I $$SOB^BDMDK14(BDMPD,V,BDMADAT) S $P(BDMCUML(40),U,7)=$P(BDMCUML(40),U,7)+1,BDMSEVOB=1 Q
HGB ;
...
BPC ;blood pressure control
 ...
 .I S<160&(D<100) S $P(BDMCUML(60),U,6)=$P(BDMCUML(60),U,6)+1 Q  ;Patch 13 2023 change 95 to 100
 .S $P(BDMCUML(60),U,7)=$P(BDMCUML(60),U,7)+1
 ;I $$AGE^AUPNPAT(BDMPD,BDMADAT)>59 S $P(BDMCUML(60),U,9)=$P(BDMCUML(60),U,9)+1 I S]"",D]"",S<150,D<90 S $P(BDMCUML(60),U,10)=$P(BDMCUML(60),U,10)+1  ;PATCH 16
HTN ;410
 S $P(BDMCUML(410),U,2)=$P(BDMCUML(410),U,2)+1  ;total pts htn denom
 S H=$E($G(^XTMP("BDMDM23",BDMJOB,BDMBTH,"AUDIT",BDMPD,34)))
 ...
 S A=$E($G(^XTMP("BDMDM23",BDMJOB,BDMBTH,"AUDIT",BDMPD,60)))  ;ACE/ARB USAGE
 I H=1,$E(A)=1 S $P(BDMCUML(410),U,5)=$P(BDMCUML(410),U,5)+1  ;TOTAL HTN AND ACE/ARB USAGE
TBSTAT ;
 S BDMTBDX=0,BDMTBTD=0,BDMTBRE=""
 S $P(BDMCUML(70),U,2)=$P(BDMCUML(70),U,2)+1  ;TOTAL PATIENTS
 S V=$G(^XTMP("BDMDM23",BDMJOB,BDMBTH,"AUDIT",BDMPD,71))   ;TB DX
 I $E(V)=1 S BDMTBDX=1,$P(BDMCUML(70),U,8)=$P(BDMCUML(70),U,8)+1
 S V=$G(^XTMP("BDMDM23",BDMJOB,BDMBTH,"AUDIT",BDMPD,70))  ;TB TEST DONE
 S BDMTBTD=0 I 'BDMTBDX,($E(V)=1!($E(V)=2)) S $P(BDMCUML(70),U,4)=$P(BDMCUML(70),U,4)+1,BDMTBTD=1  ;TEST DONE, NO DX
 S BDMTBRE="" I 'BDMTBDX,($E(V)=1!($E(V)=2)) S BDMTBRE=$E($P($$PPD^BDMDK18(BDMPD,BDMADAT),"||",2))
 I BDMTBDX!(BDMTBRE=1) S $P(BDMCUML(70),U,3)=$P(BDMCUML(70),U,3)+1
 S V=$$TBINH^BDMDK18(BDMPD,BDMADAT) I $E(V)=1 S $P(BDMCUML(70),U,5)=$P(BDMCUML(70),U,5)+1
 I BDMTBTD=1,$E($P($$PPD^BDMDK18(BDMPD,BDMADAT),"||",2))=2 S $P(BDMCUML(70),U,6)=$P(BDMCUML(70),U,6)+1
 ;GET DOD
 S X=$$LASTNP^BDMDK18(BDMPD,BDMADAT)
 I X D
 .I $$DODX^BDMDK16(BDMPD,BDMDMRG,"I",$G(BDMMULTR),.BDMREGLT)="" Q
 .S D=$$DODX^BDMDK16(BDMPD,BDMDMRG,"I",$G(BDMMULTR),.BDMREGLT) I D>X Q
 .S $P(BDMCUML(70),U,7)=$P(BDMCUML(70),U,7)+1
TOBACCO ;
 S $P(BDMCUML(80),U,2)=$P(BDMCUML(80),U,2)+1
 S V=$G(^XTMP("BDMDM23",BDMJOB,BDMBTH,"AUDIT",BDMPD,27))
 S V1=$G(^XTMP("BDMDM23",BDMJOB,BDMBTH,"AUDIT",BDMPD,28))
...
ENDS ;
 ...
 S V=$G(^XTMP("BDMDM23",BDMJOB,BDMBTH,"AUDIT",BDMPD,31))
 I +V=1 S $P(BDMCUML(69),U,3)=$P(BDMCUML(69),U,3)+1  ;screened
 I +V=2 S $P(BDMCUML(69),U,4)=$P(BDMCUML(69),U,4)+1  ;not screened
 S V=$G(^XTMP("BDMDM23",BDMJOB,BDMBTH,"AUDIT",BDMPD,33))
 I +V=1 S $P(BDMCUML(69),U,5)=$P(BDMCUML(69),U,5)+1  ;USER
 I +V=2 S $P(BDMCUML(69),U,6)=$P(BDMCUML(69),U,6)+1  ;not USER
 I +V=3 S $P(BDMCUML(69),U,7)=$P(BDMCUML(69),U,7)+1  ;not DOC
 S T=$G(^XTMP("BDMDM23",BDMJOB,BDMBTH,"AUDIT",BDMPD,27))  ;TOBACCO USER
 I +V=1,+T=1 S $P(BDMCUML(69),U,8)=$P(BDMCUML(69),U,8)+1
 I +V=1!(+T=1) S $P(BDMCUML(69),U,9)=$P(BDMCUML(69),U,9)+1  ;tobacco or ENDS
DMTX ;diabetes treatment
 ...
 S V=$$THERAPY^BDMDK16(BDMPD,BDM6MBD,BDMRED)  ;^ pieced with each item  or =1 for diet alone or =14 for refused
...
ASPIRIN ;
 ...
 I $E($G(^XTMP("BDMDM23",BDMJOB,BDMBTH,"AUDIT",BDMPD,116)))'=1 G LIPIDAG  ;not cvd
 S $P(BDMCUML(100),U,2)=$P(BDMCUML(100),U,2)+1  ;total with cvd
 S V=$G(^XTMP("BDMDM23",BDMJOB,BDMBTH,"AUDIT",BDMPD,62))
...
LIPIDAG ;lipid agents
 ...
 S V=$E($$STATIN^BDMDK16(BDMPD,BDM6M,BDMADAT))
 ...
 S C=0
 I $E($G(^XTMP("BDMDM23",BDMJOB,BDMBTH,"AUDIT",BDMPD,116)))=1 D
 ...
 ;
LIPID ;115
 S V=$G(^XTMP("BDMDM23",BDMJOB,BDMBTH,"AUDIT",BDMPD,61))
 S L=$$LDL^BDMDK18(BDMPD,BDMBDAT,BDMADAT,"I"),L=$P(L,U)
 S T=$$CHOL^BDMDK18(BDMPD,BDMBDAT,BDMADAT,"I"),T=$P(T,U)
 S T=$$STV^BDMDK18(T,,0)
 S L=$$STV^BDMDK18(L,,0)
DEP ;
 S $P(BDMCUML(300),U,2)=$P(BDMCUML(300),U,2)+1
 S S=$G(^XTMP("BDMDM23",BDMJOB,BDMBTH,"AUDIT",BDMPD,210))  ;SCREENING P16
 I $E(S)="1" S $P(BDMCUML(300),U,3)=$P(BDMCUML(300),U,3)+1
 S D=$G(^XTMP("BDMDM23",BDMJOB,BDMBTH,"AUDIT",BDMPD,200))  ;DX P16
...
EXAMS ;
 ...
 S V=$G(^XTMP("BDMDM23",BDMJOB,BDMBTH,"AUDIT",BDMPD,108))
...
LEAMP ;
 ...
 S V=$G(^XTMP("BDMDM23",BDMJOB,BDMBTH,"AUDIT",BDMPD,230))
...
SYSMEAN(P,BDATE,EDATE) ;EP
 NEW X,Z,C,Y
 S X=$$BPS^BDMDK13(P,BDATE,EDATE,"I")
 I X="" Q ""
