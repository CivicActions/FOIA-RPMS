BDMDK16 ; IHS/CMI/LAB - 2023 DIABETES AUDIT 02 Feb 2014 2:41 PM ; 
...
TBCODE(P,EDATE,R) ;EP
 ...
 S BDMJ=""
 S BDMR=$$PPD^BDMDK18(P,EDATE)
 I $E($P(BDMR,"||",2))=1 D  Q BDMJ
 .I $$TBTX^BDMDK12(P)["TX COMPLETE" S BDMJ=1 Q
 .S BDMJ=2
 .Q
 I $E($P(BDMR,"||",2))=2 S BDMJ=4 D  Q BDMJ
 .I $$DODX(P,R,"I")="" S BDMJ=6 Q
 .S D=$$DODX(P,R,"I"),E=$$PPD^BDMDK18(P,EDATE,"I") S BDMJ=$S(D>E:4,1:3)
 .Q
 I $E(BDMR)=4 S BDMJ=5
 I $E(BDMR)=3 S BDMJ=5
...
SYSMEAN(P,BDATE,EDATE) ;EP
 NEW X S X=$$BPS^BDMDK13(P,BDATE,EDATE,"I")
 I X="" Q ""
...
DIAMEAN(P,BDATE,EDATE) ;EP
 ...
 Q C\Z
PPDDATE(P,EDATE) ;EP
 NEW X S X=$$LASTNP^BDMDK18(P,EDATE)
 Q X
STATIN(P,BDATE,EDATE) ;EP
 ...
 S BDM=$$PRESD^BDMDK18(P,$O(^ATXAX("B","DM AUDIT STATIN DRUGS",0)),ED,186)
 I BDM]"" Q BDM
 K %DT S %DT="P",X=EDATE D ^%DT S ED=Y
 S BDM=$$PRESD^BDMDK18(P,$O(^ATXAX("B","BGP PQA STATIN MEDS",0)),ED,186)
 I BDM]"" Q BDM
...
STATALG(P,BDATE,EDATE,RPB) ;EP
 ...
 ...S J=$P($G(^AUPNVLAB(X,11)),U,13) Q:J=""
 ...I '$$LOINC^BDMDK1C(J,T),'$$LOINC^BDMDK1C(J,T2)
...
STATCON(P,BDATE,EDATE) ;EP does patient have an STATIN Contra
 ...
 ;
 ;pregnant
 S X=$$PREG^BDMDK1B(P,BDATE,EDATE,1,1) I X Q 1_U_"Contraindication pregnant"
 ;nmi
 S BDMG=""
 ...
 .I $P(T,"-",1)]"",$$SNOMED^BDMUTL(2023,"BREASTFEEDING PATIENT ED",$P(T,"-",1)) S %=T_U_$P(BDMG(X),U) Q
...
ACE(P,BDATE,EDATE) ;EP
 ...
 K %DT S %DT="P",X=EDATE D ^%DT S ED=Y
 S BDM=$$PRESD^BDMDK18(P,$O(^ATXAX("B","DM AUDIT ACE INHIBITORS",0)),ED,186)
 I BDM]"" Q BDM
 ;refusals
 NEW T S T=$O(^ATXAX("B","DM AUDIT ACE INHIBITORS",0))
 I 'T Q "2  No"
 S (G,X)=0 F  S X=$O(^ATXAX(T,21,X)) Q:X'=+X!(G)  S G=$$REFUSAL^BDMDK17(P,50,$P(^ATXAX(T,21,X,0),U),BDATE,EDATE)
 I G,$P(G,U,2)'="N" S G=""
...
DIETONLY(P,BD,EDATE) ;EP - diet and exercise for audit export
 ...
 ;
 ;
TYPE(P,R,EDATE,M,Z) ;EP return type 1 or 2 for epi file
 I '$G(P) Q ""
 S M=$G(M)
 S R=$G(R)
 NEW TYPE S TYPE=""
 S TYPE=$$CMSFDX^BDMDK13(P,R,"DX",M,.Z)
 I TYPE="NIDDM" Q 2
 I TYPE["TYPE II" Q 2
 I TYPE="IDDM" Q 1
 I TYPE="TYPE I" Q 1
 I TYPE["2" Q 2
 I TYPE["1" Q 1
 S TYPE="" NEW X,I,C S X=$$PLDMDXS^BDMDK13(P)
 ...
 I TYPE]"" Q TYPE
 S X=$$LASTDMDX^BDMDK13(P,EDATE)
 I X[2 Q 2
 I X[1 Q 1
 Q ""
 ;
DURDM(P,R,EDATE,M,Z) ;EP
 I '$G(P) Q ""
 S M=$G(M)
 S R=$G(R)
 NEW Y S Y=$$DODX(P,R,"I",M,.Z)
 I Y="" Q ""
 I 'Y Q ""
 I Y>EDATE Q ""
 Q $P(($$FMDIFF^XLFDT(EDATE,Y,1)\365.25),".")
 ;
DODX(P,R,F,M,Z) ;EP - date of dx for export file
 I $G(F)="" S F="E"
 I $G(M)="" S M=""
 S R=$G(R)
 NEW DATE,EARLY
 S DATE="",EARLY=9999999
 S DATE=$$CMSFDX^BDMDK13(P,R,"ID",M,.Z)
 I DATE S EARLY=DATE
 S DATE=$$PLDMDOO^BDMDK13(P,"I")
...
D(D) ;
 I D="" Q ""
 ;IF YEAR ONLY USE 0701
 I $E(D,4,7)="0000" Q "07/01/"_(1700+$E(D,1,3))
 Q $S($E(D,4,5)="00":"07",1:$E(D,4,5))_"/"_$S($E(D,6,7)="00":"15",1:$E(D,6,7))_"/"_(1700+$E(D,1,3))
 ;
DI(D) ;EP
 I D="" Q ""
 ;IF YEAR ONLY USE 0701, IF NO DAY USE 15
 I $E(D,4,7)="0000" Q $E(D,1,3)_"0701"
 Q $E(D,1,3)_$S($E(D,4,5)="00":"07",1:$E(D,4,5))_$S($E(D,6,7)="00":"15",1:$E(D,6,7))
