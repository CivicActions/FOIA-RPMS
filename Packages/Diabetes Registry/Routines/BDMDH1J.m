BDMDH1J ; IHS/CMI/LAB -IHS -CUMULATIVE REPORT 10 Oct 2014 9:50 AM ; 
...
COMORBID ;USE 400
 ...
 S T=$G(^XTMP("BDMDM20",BDMJOB,BDMBTH,"AUDIT",BDMPD,27))
 I $E(T)=2,$E(V)=1 S $P(BDMCUML(400),U,10)=$P(BDMCUML(400),U,10)+1
 ;CVD AND STATIN PRESCRIBED PIECE 11
 S T=+$G(^XTMP("BDMDM20",BDMJOB,BDMBTH,"AUDIT",BDMPD,300))
 ...
 S T=$G(^XTMP("BDMDM20",BDMJOB,BDMBTH,"AUDIT",BDMPD,62))
...
N ;p12 HEP C AND RETINOPATHY
 S V=$G(^XTMP("BDMDM20",BDMJOB,BDMBTH,"AUDIT",BDMPD,222))
...
R ;RETINOPATHY
 S V=$G(^XTMP("BDMDM20",BDMJOB,BDMBTH,"AUDIT",BDMPD,224))
...
EPICHK ;create epi info record in ^BDMDATA("BDMEPI",$J,n)
 ...
 S BDMEPIR=$$REC(BDMPD,"DM AUDIT 2020 EXPORT RECORD",BDMRBD,BDMRED,BDMADAT,BDMDMRG,BDMBDAT)
 ;CHECK ERRORS ON BDMEPIR
 S BDMRTYP("IEN")=$O(^BDMRECD("B","DM AUDIT 2020 EXPORT RECORD",0))
 ...
 .S BDMECNT=BDMECNT+1
 .S $P(^XTMP("BDMDM20 ERRORS",BDMJOB,BDMBTH,"PATS",S,BDMPD,BDMECNT),U,1)=$P(^BDMRECD(BDMRTYP("IEN"),21,Z,0),U,1)
 .S J=$P(^BDMRECD(BDMRTYP("IEN"),21,Z,0),U,2)
 .S $P(^XTMP("BDMDM20 ERRORS",BDMJOB,BDMBTH,"PATS",S,BDMPD,BDMECNT),U,2)=$S(J="P":"POTENTIAL",J="D":"DEFINITE",1:"")
 .S $P(^XTMP("BDMDM20 ERRORS",BDMJOB,BDMBTH,"PATS",S,BDMPD,BDMECNT),U,3)=$P($G(^BDMRECD(BDMRTYP("IEN"),21,Z,11)),U,1)
 .S P=$P(^BDMRECD(BDMRTYP("IEN"),21,Z,0),U,4)
 .S A=""
 .I P="" D
 ..I $P(^BDMRECD(BDMRTYP("IEN"),21,Z,0),U,1)["BMI" S A=$$BMI^BDMDH18(BDMPD,BDMRBD,BDMRED)
 ..I $P(^BDMRECD(BDMRTYP("IEN"),21,Z,0),U,1)["HEIGHT TOTAL" S A="" I $P(BDMEPIR,U,18)]"" S A=$P(BDMEPIR,U,18)*12,A=A+$P(BDMEPIR,U,19)
 ..I $P(^BDMRECD(BDMRTYP("IEN"),21,Z,0),U,1)["DURATION" S A=$$DURDM^BDMDH16(BDMPD,BDMDMRG,BDMADAT)
 .I P S A=$P(BDMEPIR,U,P)
 .S $P(^XTMP("BDMDM20 ERRORS",BDMJOB,BDMBTH,"PATS",S,BDMPD,BDMECNT),U,4)=A
 Q
