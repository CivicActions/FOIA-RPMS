BDMDH16 ; IHS/CMI/LAB - 2020 DIABETES AUDIT 02 Feb 2014 2:41 PM ; 
...
TBCODE(P,EDATE,R) ;EP
 ...
 S BDMJ=""
 S BDMR=$$PPD^BDMDH18(P,EDATE)
 I $E($P(BDMR,"||",2))=1 D  Q BDMJ
 .I $$TBTX^BDMDH12(P)["TX COMPLETE" S BDMJ=1 Q
 .S BDMJ=2
 .Q
 I $E($P(BDMR,"||",2))=2 S BDMJ=4 D  Q BDMJ
 .I $$DODX(P,R,"I")="" S BDMJ=6 Q
 .S D=$$DODX(P,R,"I"),E=$$PPD^BDMDH18(P,EDATE,"I") S BDMJ=$S(D>E:4,1:3)
 .Q
 I $E(BDMR)=4 S BDMJ=5
 I $E(BDMR)=3 S BDMJ=5
...
DIAMEAN(P,BDATE,EDATE) ;EP
 ...
 Q C\Z
PPDDATE(P,EDATE) ;EP
 NEW X S X=$$LASTNP^BDMDH18(P,EDATE)
 Q X
STATIN(P,BDATE,EDATE) ;EP
 ...
 S BDM=$$PRESD^BDMDH18(P,$O(^ATXAX("B","DM AUDIT STATIN DRUGS",0)),ED,186)
 I BDM]"" Q BDM
 K %DT S %DT="P",X=EDATE D ^%DT S ED=Y
 S BDM=$$PRESD^BDMDH18(P,$O(^ATXAX("B","BGP PQA STATIN MEDS",0)),ED,186)
 I BDM]"" Q BDM
...
STATALG(P,BDATE,EDATE,RPB) ;EP
 ...
 ...S J=$P($G(^AUPNVLAB(X,11)),U,13) Q:J=""
 ...I '$$LOINC^BDMDH1C(J,T),'$$LOINC^BDMDH1C(J,T2)
...
STATCON(P,BDATE,EDATE) ;EP does patient have an STATIN Contra
 ...
 ;
 ;pregnant
 S X=$$PREG^BDMDH1B(P,BDATE,EDATE,1,1) I X Q 1_U_"Contraindication pregnant"
 ;nmi
 S BDMG=""
 ...
 .I $P(T,"-",1)]"",$$SNOMED^BDMUTL(2020,"BREASTFEEDING PATIENT ED",$P(T,"-",1)) S %=T_U_$P(BDMG(X),U) Q
...
ACE(P,BDATE,EDATE) ;EP
 ...
 K %DT S %DT="P",X=EDATE D ^%DT S ED=Y
 S BDM=$$PRESD^BDMDH18(P,$O(^ATXAX("B","DM AUDIT ACE INHIBITORS",0)),ED,186)
 I BDM]"" Q BDM
 ;refusals
 NEW T S T=$O(^ATXAX("B","DM AUDIT ACE INHIBITORS",0))
 I 'T Q "2  No"
 S (G,X)=0 F  S X=$O(^ATXAX(T,21,X)) Q:X'=+X!(G)  S G=$$REFUSAL^BDMDH17(P,50,$P(^ATXAX(T,21,X,0),U),BDATE,EDATE)
 I G,$P(G,U,2)'="N" S G=""
...
TYPE(P,R,EDATE) ;EP return type 1 or 2 for epi file
 I '$G(P) Q ""
 NEW TYPE S TYPE=""
 I $G(R) S TYPE=$$CMSFDX^BDMDH13(P,R,"DX")
 I TYPE="NIDDM" Q 2
 I TYPE["TYPE II" Q 2
 I TYPE="IDDM" Q 1
 I TYPE="TYPE I" Q 1
 I TYPE["2" Q 2
 I TYPE["1" Q 1
 S TYPE="" NEW X,I,C S X=$$PLDMDXS^BDMDH13(P)
...
DODX(P,R,F) ;EP - date of dx for export file
 ...
 I $G(R) S DATE=$$CMSFDX^BDMDH13(P,R,"ID")
 I DATE S EARLY=DATE
 S DATE=$$PLDMDOO^BDMDH13(P,"I")
