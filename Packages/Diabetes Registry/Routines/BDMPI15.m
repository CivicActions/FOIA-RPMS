BDMPI15 ; IHS/CMI/LAB -IHS -CUMULATIVE REPORT ;
...
CUML ;EP
 ...
 ;
PROCESS ;
 S BDMPG=0 F  S BDMPG=$O(^XTMP("BDMPI1",BDMJOB,BDMBTH,"AUDIT",BDMPG)) Q:BDMPG'=+BDMPG  D CUML1
 Q
 ;
CUML1 ;
GENDER ;
 ...
 S V=$G(^XTMP("BDMPI1",BDMJOB,BDMBTH,"AUDIT",BDMPG,20))
...
BMI ;
 ...
 S V=$G(^XTMP("BDMPI1",BDMJOB,BDMBTH,"AUDIT",BDMPG,112))
 S $P(BDMCUML(40),U,2)=$P(BDMCUML(40),U,2)+1
 D
 .I V="" S $P(BDMCUML(40),U,5)=$P(BDMCUML(40),U,5)+1 Q
 .I $$OW^BDMDI14(BDMPG,V,BDMADAT) S $P(BDMCUML(40),U,3)=$P(BDMCUML(40),U,3)+1
 .I $$OB^BDMDI14(BDMPG,V,BDMADAT) S $P(BDMCUML(40),U,4)=$P(BDMCUML(40),U,4)+1
...
TOBACCO ;
 ...
 S V=$G(^XTMP("BDMPI1",BDMJOB,BDMBTH,"AUDIT",BDMPG,27))
 S V1=$G(^XTMP("BDMPI1",BDMJOB,BDMBTH,"AUDIT",BDMPG,28))
...
DMTX ;diabetes treatment
 ...
 S V=$$THERAPY^BDMPI16(BDMPG,BDM6MBD,BDMRED)
...
ASPIRIN ;
 ...
 S V=$G(^XTMP("BDMPI1",BDMJOB,BDMBTH,"AUDIT",BDMPG,62))
...
ACE ;110 title^total pts^total pts with protein^# of those on ace^# with htn^# of those on ace"
 ...
 ;S P=$G(^XTMP("BDMPI1",BDMJOB,BDMBTH,"AUDIT",BDMPG,94))
 S H=$G(^XTMP("BDMPI1",BDMJOB,BDMBTH,"AUDIT",BDMPG,34))
 S A=$G(^XTMP("BDMPI1",BDMJOB,BDMBTH,"AUDIT",BDMPG,60))
...
LIPID ;115
 S V=$G(^XTMP("BDMPI1",BDMJOB,BDMBTH,"AUDIT",BDMPG,61))
 S L=$$LDL^BDMDI18(BDMPG,BDMBDAT,BDMADAT,"I"),L=$P(L,U)
 S T=$$CHOL^BDMDI18(BDMPG,BDMBDAT,BDMADAT,"I"),T=$P(T,U)
