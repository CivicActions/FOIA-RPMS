BDMPL1P ; IHS/CMI/LAB - 2026 DIABETES AUDIT PRINT 15 Dec 2016 3:05 PM ; 24 Oct 2019  3:42 PM
 ;;2.0;DIABETES MANAGEMENT SYSTEM;**19**;JUN 14, 2007;Build 159
 ;
 ;
 D LOG^BUSAAPI("O","P","P",$S($G(XQY0)]"":$P(XQY0,U),1:"BDMDN1P"),"DM 2026 PREDIABETES REPORT")
 S BDMQUIT=0,BDMPG=0,BDMIOSL=$S($G(BDMGUI):57,1:IOSL)
 I BDMPREP=2 G CUML
 ;print ind audits first
 S BDMPD=0,BDMGUIC=0 F  S BDMPD=$O(^XTMP("BDMPL1",BDMJOB,BDMBTH,"AUDIT",BDMPD)) Q:BDMPD'=+BDMPD!(BDMQUIT)  D
 .I $G(BDMGUI),BDMGUIC W !,"ZZZZZZZ",!  ;maw
 .I BDMGUIC W:$D(IOF) @IOF
 .S BDMGUIC=1
 .S BDMPG=BDMPG+1 W:$G(BDMGUI) !! W "IHS Prediabetes Assessment of Care, 2026    DATE RUN: "_$$DATE^BDMS9B1(DT)_"   Page: "_BDMPG
 ...
 .W !!,"Local Questions ",$$LOCN^BDMDL10(BDMPD,BDMDMRG)
 .W !!?2,"Select one:"
 .W !?2,"Text: ",$$LOCT^BDMDL10(BDMPD,BDMDMRG)
 .I $E(IOST,1,2)'="P-" W !! S DIR(0)="E" D ^DIR K DIR
CUML ;
 I BDMPREP=2!(BDMPREP=3) D CUML^BDMPL14
DONE ;
 K ^TMP($J)
 ...
 Q
I(I) ;
 Q $G(^XTMP("BDMPL1",BDMJOB,BDMBTH,"AUDIT",BDMPD,I))
 ;
PAGE ;
 Q:$G(BDMDSP)
 ...
 I $G(BDMGUI) W !!
 S BDMPG=BDMPG+1
 W "IHS Assessment of Prediabetes Care, 2026    DATE RUN: "_$$DATE^BDMS9B1(DT)_"   Page: "_BDMPG
