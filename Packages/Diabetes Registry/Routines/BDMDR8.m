BDMDR8 ; IHS/CMI/LAB - Possible inactive patients now active ;
...
STAT ;get status
 ...
 S BDMSTAT=""
 K DIR
 W !?5,"1)  INACTIVE",!?5,"2)  TRANSIENT",!?5,"3)  LOST TO FOLLOW-UP"
 W !?5,"You can enter a list (e.g. 1, OR 1,2) or a range (e.g. 1-3)"
 S DIR(0)="L^1:3",DIR("B")="",DIR("A")="Which status(s) should be included" KILL DA D ^DIR KILL DIR
 I $D(DIRUT) S BDMRDA="",BDMREG="" G R
 S BDMSTAT=Y
 S A=Y,C="" F I=1:1 S C=$P(A,",",I) Q:C=""  S BDMSTAT($$S(C))=""
CLINIC ;
...
DEMO ;
 ...
 S XBRP="PRINT^BDMDR8",XBRC="PROC^BDMDR8",XBRX="EXIT^BDMDR8",XBNS="BDM"
 D ^XBDBQUE
 D EXIT
 Q
S(S) ;
 ...
 Q "I"
BROWSE ;
 S XBRP="VIEWR^XBLM(""PRINT^BDMDR8"")"
 S XBRC="PROC^BDMDR8",XBRX="EXIT^BDMDR8",XBIOP=0 D ^XBDBQUE
 Q
EXIT ;clean up and exit
 I '$D(BDMGUI) D EN^XBVK("BDM"),EN^XBVK("AMQQ")
 D ^XBFMK
 D KILL^AUPNPAT
 Q
INFORM ;
 ...
 W !,$$CTR($$LOC)
 W !,$$CTR($$USR)
 W !!,"This report will list patients who are on the diabetes register with a status",!
 W "other than active who have had a visit to a set of primary care clinics in a date"
 W !,"range defined by the user.",!
 W "The report provides a way to identify patients who could possibly be re-activated",!
 W "(status changed to Active) in the register.",!!
 Q
PROC ;EP - called from XBDBQUE
 S BDMJOB=$J,BDMBTH=$H
 K ^XTMP("BDMDR8",BDMJOB,BDMBTH)
 D XTMP^BDMOSUT("BDMDR8","DM ON REGISTER INACTIVE/HAD VISIT")
 S BDMP=0 F  S BDMP=$O(^ACM(41,"B",BDMREG,BDMP)) Q:BDMP'=+BDMP  D
 .Q:'$D(^ACM(41,BDMP))
 .S BDMPAT=$P(^ACM(41,BDMP,0),U,2)
 .Q:$$DEMO^BDMUTL(BDMPAT,$G(BDMDEMO))
 .S S=$P($G(^ACM(41,BDMP,"DT")),U,1)
 .I S="" Q  ;no status
 .I '$D(BDMSTAT(S)) Q  ;not a status we want
 ...
 ..S BDMG=1
 .I BDMG S ^XTMP("BDMDR8",BDMJOB,BDMBTH,"PATIENTS",$$GET1^DIQ(2,BDMPAT,.01),BDMPAT)=""
 Q
PRINT ;EP - called from xbdbque
 D LOG^BUSAAPI("O","P","P",$S($G(XQY0)]"":$P(XQY0,U),1:"BDMDR8"),"DM REG PTS WITH VISIT")
 S BDMIOSL=$S($G(BDMGUI):55,1:IOSL)
 S BDM80D="-------------------------------------------------------------------------------"
 S BDMPG=0 D HEAD
 I '$D(^XTMP("BDMDR8",BDMJOB,BDMBTH)) W !!,"NO PATIENTS TO REPORT" G DONE
 S BDMNAME="" K BDMQ
 F  S BDMNAME=$O(^XTMP("BDMDR8",BDMJOB,BDMBTH,"PATIENTS",BDMNAME)) Q:BDMNAME=""!($D(BDMQ))  D
 .S DFN="" F  S DFN=$O(^XTMP("BDMDR8",BDMJOB,BDMBTH,"PATIENTS",BDMNAME,DFN)) Q:DFN=""!($D(BDMQ))  S BDMX=^XTMP("BDMDR8",BDMJOB,BDMBTH,"PATIENTS",BDMNAME,DFN) D
 ...
 ..S V=$$LASTV(DFN)
 ..W ?38,$$DATE^BDMS9B1($P(V,U,1))
 ..W ?51,$$LBLK^BDMUTL($$NUMDXS(DFN),5)
 ..W ?59,$$DATE^BDMS9B1($$LASTDMDX(DFN))
 ..W ?73,$$CLINIC^APCLV($P(V,U,3),"C")
DONE ;
 I $E(IOST)="C",IO=IO(0) S DIR(0)="EO",DIR("A")="End of report.  HIT RETURN" D ^DIR K DIR S:$D(DUOUT) DIRUT=1
 W:$D(IOF) @IOF
 K ^XTMP("BDMDR8",BDMJOB,BDMBTH),BDMJOB,BDMBTH
 Q
...
HEAD1 ;
 ...
 W $$CTR("with a visit between "_$$FMTE^XLFDT(BDMBD)_" and "_$$FMTE^XLFDT(BDMED),80),!
 W $$CTR("who do not currently have a status of Active.",80),!
PIH W !,"PATIENT NAME",?22,"HRN",?29,"STATUS",?38,"LAST VISIT",?52,"# DM",?59,"LAST DM DX",?72,"CLINIC",!,?52,"DXS",?72,"CODE",!,BDM80D
 Q
NUMDXS(P) ;
 I '$G(P) Q ""
 NEW X,E,BDM,Y
...
LASTV(P) ;
 ...
 ..S V=X
 I V="" Q ""
 Q $$GET1^DIQ(9000010,V,.01,"I")_U_$$GET1^DIQ(9000010,V,.08,"I")_U_V
LASTDMDX(P) ;
 I '$G(P) Q ""
 NEW X,E,BDM,Y
 S Y="BDM("
...
BDMG(BDMTR,BDMREG,BDMSTAT,BDMND,BDMLDAT) ;EP - GUI DMS Entry Point
 ...
 S ZTIO="",ZTDTH=$$NOW^XLFDT,ZTRTN="GUIEP^BDMDR8",ZTDESC="GUI DM PTS NO DX PL" D ^%ZTLOAD
 D EXIT
 Q
GUIEP ;EP
 D PROC
 K ^TMP($J,"BDMDR8")
 S IOM=80
 D GUIR^XBLM("PRINT^BDMDR8","^TMP($J,""BDMDR8"",")
  S X=0,C=0 F  S X=$O(^TMP($J,"BDMDR8",X)) Q:X'=+X  D
 .S BDMDATA=^TMP($J,"BDMDR8",X)
