BDMDK1C ; IHS/CMI/LAB - 2023 DIABETES AUDIT 09 Nov 2015 11:34 AM ; 31 Oct 2022  11:59 AM
...
GATHER ;
 ...
 ...S R=$P(^AUPNVLAB(X,0),U,4),R=$$STV^BDMDK18(R,8,1) I R="" Q  ;no numeric result 
...
GFR(P,BDATE,EDATE,NOCALC) ;EP - Estimated GFR
 ...
 I '$D(BDM(1)) Q ""
 D SETN
 Q 1_U_$P(^AUPNVLAB(+$P(BDM(N),U,4),0),U,4)_U_$$DATE^BDMS9B1($P(BDM(N),U))_U_$E($P(BDM(N),U,3),1,25)_U_$P(BDM(N),U)_U_$$VAL^XBDIQ1(9000010.09,+$P(BDM(N),U,4),1101)
CREAT ;EP
 I $G(F)="" S F="E"
...
CHOL ;EP
 ...
 D SETN
 I F="I" Q $P(^AUPNVLAB(+$P(BDM(N),U,4),0),U,4)_"^"_$P(BDM(N),U)
 Q $P(^AUPNVLAB(+$P(BDM(N),U,4),0),U,4)_$S($P(^AUPNVLAB(+$P(BDM(N),U,4),0),U,4)]"":" "_$$VAL^XBDIQ1(9000010.09,+$P(BDM(N),U,4),1101),1:"")_U_$$DATE^BDMS9B1($P(BDM(N),U))_U_$E($P(BDM(N),U,3),1,25)
HDL ;EP
 I $G(F)="" S F="E"
 NEW BDM,X,%,E,R,V,BDMLT,BDMOT,B,D,L,J,BDMC,BDMV,V
 K BDM
...
LDL ;EP
 ...
 Q $P(^AUPNVLAB(+$P(BDM(G),U,4),0),U,4)_$S($P(^AUPNVLAB(+$P(BDM(G),U,4),0),U,4)]"":" "_$$VAL^XBDIQ1(9000010.09,+$P(BDM(G),U,4),1101),1:"")_U_$$DATE^BDMS9B1($P(BDM(G),U))_U_$E($P(BDM(G),U,3),1,25)
TRIG ;EP
 I $G(F)="" S F="E"
 NEW BDM,X,%,E,R,V,BDMLT,BDMOT,B,D,L,J,BDMC,BDMV,V
 K BDM
...
UACR(P,BDATE,EDATE) ;EP - albumin/creatinine ration UACR
 ...
 I '$D(BDM(1)) Q ""
 D SETN
 I $D(BDM(N)) Q "X^"_$P(^AUPNVLAB(+$P(BDM(N),U,4),0),U,4)_$S($P(^AUPNVLAB(+$P(BDM(N),U,4),0),U,4)]"":" "_$$VAL^XBDIQ1(9000010.09,+$P(BDM(N),U,4),1101),1:"")_U_$$DATE^BDMS9B1($P(BDM(N),U,1))_U_$E($P(BDM(N),U,3),1,30)
 Q ""
 ;
LPROF ;
 K BDM,BDMX S BDMX=""
...
SEMI(P,BDATE,EDATE) ;EP - albumin/creatinine ration UACR
 ...
 I '$D(BDM(1)) Q ""
 D SETN
 I $D(BDM(1)) Q "X^"_$P(^AUPNVLAB(+$P(BDM(N),U,4),0),U,4)_$S($P(^AUPNVLAB(+$P(BDM(N),U,4),0),U,4)]"":" "_$$VAL^XBDIQ1(9000010.09,+$P(BDM(N),U,4),1101),1:"")_U_$$DATE^BDMS9B1($P(BDM(N),U,1))_U_$E($P(BDM(N),U,3),1,30)
 Q ""
 ;
...
COMBINED(P,BDMBDAT,BDMADAT,BDM6MBD) ;EP
 NEW A1C,LDL,BP,ALL,G,V,S,D,STATIN,BDMRBD,BDMRED  ;ADDED FOR ICARE 12/06/17
 S BDMRBD=$$FMTE^XLFDT(BDMBDAT)  ;ADDED FOR ICARE 12/06/17
 S BDMRED=$$FMTE^XLFDT(BDMADAT)  ;ADDED FOR ICARE 12/06/17
 S G=0,ALL=""
 ;S A1C=$P(^XTMP("BDMDM23",BDMJOB,BDMBTH,"AUDIT",P,78),U,2)
 S A1C=$P($$HGBA1C^BDMDK18(BDMPD,BDMBDAT,BDMADAT),U,2)
 S V=A1C
 ...
 S G=G+S
CSTAT ;
 ;S STATIN=+^XTMP("BDMDM23",BDMJOB,BDMBTH,"AUDIT",P,300)
 S STATIN=+$$STATIN^BDMDK16(BDMPD,BDM6MBD,BDMADAT)
 S ALL=ALL_"; statin prescribed: "_$S(STATIN=1:"Yes",1:"No")
 I STATIN=1 S G=G+1
BP ;
 S S=$$SYSMEAN^BDMDK15(P,BDMRBD,BDMRED)
 S D=$$DIAMEAN^BDMDK15(P,BDMRBD,BDMRED)
 D
 .I S=""!(D="") Q
...
EGFRUACR(P,BD,ED,F) ;EP - 
 ...
 S F=$G(F)
 NEW G,R
 S G=$$GFR^BDMDK1C(BDMPD,BD,ED)
 S R=$$URIN^BDMDK18(BDMPD,BD,ED)
