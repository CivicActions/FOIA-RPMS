BDMDN11 ; IHS/CMI/LAB -IHS -CUMULATIVE REPORT 10 Oct 2014 9:50 AM ; 
...
IMM ;
 ...
 ;TDAP EVER
 S V=$G(^XTMP("BDMDM24",BDMJOB,BDMBTH,"AUDIT",BDMPD,216))
 ...
 .S V=$G(^XTMP("BDMDM24",BDMJOB,BDMBTH,"AUDIT",BDMPD,73))
...
QUAN ;
 ...
 S V=$E($G(^XTMP("BDMDM24",BDMJOB,BDMBTH,"AUDIT",BDMPD,92)))  ;test done?
 ...
 S G=$P($G(^XTMP("BDMDM24",BDMJOB,BDMBTH,"AUDIT",BDMPD,79)),U,2)  ;egfr value
 S H=0
...
GFR ;
 ...
 S V=$G(^XTMP("BDMDM24",BDMJOB,BDMBTH,"AUDIT",BDMPD,79))
...
TCHOL ;
 ...
 S V=$$CHOL^BDMDN18(BDMPD,BDMBDAT,BDMADAT,"I")
...
TRIG ;
 ...
 S V=$P(V,U)
 S V=$$STV^BDMDN18(V,,0) I $E(V)'=+$E(V) S $P(BDMCUML(200),U,5)=$P(BDMCUML(200),U,5)+1 G ACE ;unable to determine result, not a number
...
ACE ;110 title^total pts^total pts with protein^# of those on ace^# with htn^# of those on ace"
 ...
 S P=$G(^XTMP("BDMDM24",BDMJOB,BDMBTH,"AUDIT",BDMPD,92))  ;URINE PROTEIN VALUE
...
COMBINED ;USE 260
 ...
 I $E($G(^XTMP("BDMDM24",BDMJOB,BDMBTH,"AUDIT",BDMPD,300)))=3 G E  ;statin allergy
 S V=$G(^XTMP("BDMDM24",BDMJOB,BDMBTH,"AUDIT",BDMPD,118))
