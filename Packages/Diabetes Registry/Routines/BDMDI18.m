BDMDI18 ; IHS/CMI/LAB - 2021 DIABETES AUDIT ;
...
TBINH(P,EDATE) ;EP
 I $P($$TBDX(P,EDATE),U,1)=1 Q $E($$TBTX^BDMDI12(P,EDATE))
 I $E($P($$PPD(P,EDATE),"||",2))=1 Q $E($$TBTX^BDMDI12(P,EDATE))
 Q ""
...
PPDSPL ;CHECK PL
 ...
 .;now check SNOMED
 .I 'G S S=$$VAL^XBDIQ1(9000011,X,80001) I S]"",$$SNOMED^BDMUTL(2021,"PXRM BQI TUBERCULOSIS",S) S G=1
 .I 'G Q
 ...
 .I '$$SNOMED^BDMUTL(2021,"PXRM BQI TUBERCULOSIS",S) Q
...
PPD(P,EDATE,F) ;EP
 ...
 .I $P(BDM(D,C),U,1)="LAB",$P(BDM(D,C),U,4)]"" S G=BDM(D,C)
...
AS(X) ;EP
 ...
 Q X
 ;
GFRDES(P,BDATE,EDATE,EEGFR) ;EP
 NEW A,B,C
 S A=$$CALCGFR(P,BDATE,EDATE)
 I A="" Q 0
 S A=$P(A,U,2)
 I $E(A)=">" S C=0 D  Q C
 .S A=60
 .I EEGFR>60 S C=0 Q
 .I EEGFR<50 S C=1 Q
 S A=$$STV^BDMDI18(A,5,1)
 S B=EEGFR-A
 I B>10 Q 1
 I B<-10 Q 1
 Q 0
CALCGFRZ(P,BDATE,EDATE) ;EP
 NEW R,C,X,Y,Z,D
 S R=$$CREAT^BDMDI18(P,BDATE,EDATE,"I"),D=$P(R,U,2),R=$P(R,U,1)
 I R="" Q ""
 ;CALCULATE IT
 S BDMSEX=$P(^DPT(P,0),U,2)
 S BDMAGE=$$AGE^AUPNPAT(P,D)
 S X=$$GFRSE1CZ(P,R,BDMAGE,BDMSEX)
 Q D_U_X_"^ESTIMATED GFR (CALC)"
GFRSE1CZ(DFN,CRET,BDMAGE,BDMSEX) ; EP
