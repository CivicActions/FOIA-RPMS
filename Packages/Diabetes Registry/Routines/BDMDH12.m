BDMDH12 ; IHS/CMI/LAB - 2020 DIABETES AUDIT ;
...
TBTX(P) ;EP
 ...
 I G]"" Q $S(G["TX COMPLETE":"1 Yes",G["TX INCOMPLETE"!(G["TX UNTREATED")!(G["TX IN PROGRESS"):"2 No",1:"3 Unknown")_U_G
 Q ""
...
INSULIN(P,BDATE,EDATE) ;EP
 ...
 S BDM=$$PRES^BDMDH18(P,$O(^ATXAX("B","DM AUDIT INSULIN DRUGS",0)),ED)
 Q BDM
 ;
SULF(P,BDATE,EDATE) ;EP
 ...
 I $D(BDM(1)) Q "X"
 K %DT S %DT="P",X=EDATE D ^%DT S ED=Y
 S BDM=$$PRES^BDMDH18(P,$O(^ATXAX("B","DM AUDIT SULFONYLUREA DRUGS",0)),ED)
 Q BDM
 ;
SULFLIKE(P,BDATE,EDATE) ;EP
 ...
 I $D(BDM(1)) Q "X"
 K %DT S %DT="P",X=EDATE D ^%DT S ED=Y
 S BDM=$$PRES^BDMDH18(P,$O(^ATXAX("B","DM AUDIT SULFONYLUREA-LIKE",0)),ED)
 Q BDM
MET(P,BDATE,EDATE) ;EP
 ...
 I $D(BDM(1)) Q "X"
 K %DT S %DT="P",X=EDATE D ^%DT S ED=Y
 S BDM=$$PRES^BDMDH18(P,$O(^ATXAX("B","DM AUDIT METFORMIN DRUGS",0)),ED)
 Q BDM
 ;
ACAR(P,BDATE,EDATE) ;EP
 ...
 I $D(BDM(1)) Q "X"
 K %DT S %DT="P",X=EDATE D ^%DT S ED=Y
 S BDM=$$PRES^BDMDH18(P,$O(^ATXAX("B","DM AUDIT ACARBOSE DRUGS",0)),ED)
 Q BDM
 ;
 ;
TROG(P,BDATE,EDATE) ;EP
 ...
 I $D(BDM(1)) Q "X"
 K %DT S %DT="P",X=EDATE D ^%DT S ED=Y
 S BDM=$$PRES^BDMDH18(P,$O(^ATXAX("B","DM AUDIT GLITAZONE DRUGS",0)),ED)
 Q BDM
 ;
SGLT2(P,BDATE,EDATE) ;EP
 ...
 I $D(BDM(1)) Q "X"
 K %DT S %DT="P",X=EDATE D ^%DT S ED=Y
 S BDM=$$PRES^BDMDH18(P,$O(^ATXAX("B","DM AUDIT SGLT-2 INHIBITOR DRUG",0)),ED)
 Q BDM
 ;
DPP4(P,BDATE,EDATE) ;EP  
 ...
 I $D(BDM(1)) Q "X"
 K %DT S %DT="P",X=EDATE D ^%DT S ED=Y
 S BDM=$$PRES^BDMDH18(P,$O(^ATXAX("B","DM AUDIT DPP4 INHIBITOR DRUGS",0)),ED)
 Q BDM
AMYLIN(P,BDATE,EDATE) ;EP
 ...
 I $D(BDM(1)) Q "X"
 K %DT S %DT="P",X=EDATE D ^%DT S ED=Y
 S BDM=$$PRES^BDMDH18(P,$O(^ATXAX("B","DM AUDIT AMYLIN ANALOGUES",0)),ED)
 Q BDM
 ;
GLP1(P,BDATE,EDATE) ;EP
 ...
 I $D(BDM(1)) Q "X"
 K %DT S %DT="P",X=EDATE D ^%DT S ED=Y
 S BDM=$$PRES^BDMDH18(P,$O(^ATXAX("B","DM AUDIT GLP-1 ANALOG DRUGS",0)),ED)
 I BDM="X" Q BDM
 K %DT S %DT="P",X=EDATE D ^%DT S ED=Y
 S BDM=$$PRES^BDMDH18(P,$O(^ATXAX("B","DM AUDIT INCRETIN MIMETIC",0)),ED)
 Q BDM
 ;
BROM(P,BDATE,EDATE) ;EP
 ...
 I $D(BDM(1)) Q "X"
 K %DT S %DT="P",X=EDATE D ^%DT S ED=Y
 S BDM=$$PRES^BDMDH18(P,$O(^ATXAX("B","DM AUDIT BROMOCRIPTINE DRUGS",0)),ED)
 Q BDM
 ;
COLE(P,BDATE,EDATE) ;EP
 ...
 I $D(BDM(1)) Q "X"
 K %DT S %DT="P",X=EDATE D ^%DT S ED=Y
 S BDM=$$PRES^BDMDH18(P,$O(^ATXAX("B","DM AUDIT COLESEVELAM DRUGS",0)),ED)
 Q BDM
DEPDX(P,BDATE,EDATE) ;EP
 ...
 .I $P($G(^AUPNPROB(X,800)),U,1)]"",$$SNOMED^BDMUTL(2020,"DEPRESSION DIAGNOSES",$P(^AUPNPROB(X,800),U,1)) S G="1  Yes  Problem List (SNOMED: "_$P(^AUPNPROB(X,800),U,1)_") "
 .Q
 I G]"" Q G
 S (G,X,I)=""
...
LASTDEPS(BDMLPDFN,BDMLBD,BDMLED,BDMLFORM) ;PEP - return last depression screen
 ...
 D E
 S BDMLVAL=$$CPT^BDMDHDU(BDMLPDFN,$S($P(BDMLLAST,U)]"":$P(BDMLLAST,U),1:BDMLBD),BDMLED,$O(^ATXAX("B","BGP DEPRESSION SCREEN CPTS",0)),5)
 D E
