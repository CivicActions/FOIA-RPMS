BDMDKTL ; IHS/CMI/LAB - DISPLAY IND LISTS ;
...
HDR ; -- header code
 ...
 S VALMHDR(2)="ITEM NAME",$E(VALMHDR(2),64)="Internal Entry #"
 Q
 ;
INIT ; -- init variables and list array
 ...
 .S BDMLAB(C,0)=C_")  "_$$VAL^XBDIQ1($P(^ATXAX(BDMTAXI,0),U,15),BDMLABI,.01),$E(BDMLAB(C,0),65)=BDMLABI
...
LAB ;
 ...
 .S BDMLAB(C,0)=C_")  "_$P($G(^LAB(60,BDMLABI,0)),U),$E(BDMLAB(C,0),65)=BDMLABI
 .S BDMLAB("IDX",C,C)=BDMLABI
 .Q
 S (VALMCNT,BDMHIGH)=C
 Q
...
REM ;
 ...
 I $D(DIRUT) G REMX
 D ^XBFMK
 I BDMTAXT="L" S DA(1)=BDMTAXI,DA=$O(^ATXLAB(BDMTAXI,21,"B",BDMLABI,0)),DIE="^ATXLAB("_BDMTAXI_",21,",DR=".01///@" D ^DIE D
 .NEW BDMDETAL
 .S BDMDETAL(1)="^^REMOVE LAB TEST IEN "_BDMLABI_" FROM TAXONOMY "_$P(^ATXLAB(BDMTAXI,0),U,1)_"^^"_BDMLABI
 .D LOG^BUSAAPI("O","O","D",$S($G(XQY0)]"":$P(XQY0,U),1:"REM~BDMHUTL"),"DM AUDIT LAB TAXONOMY UPDATE: "_$P(^ATXLAB(BDMTAXI,0),U,1),"BDMDETAL")
 I BDMTAXT="T" S DA(1)=BDMTAXI,DA=$O(^ATXAX(BDMTAXI,21,"B",BDMLABI,0)),DIE="^ATXAX("_BDMTAXI_",21,",DR=".01///@" D ^DIE D
 .NEW F,BDMDETAL
 .S F=$P(^ATXAX(BDMTAXI,0),U,15)
 .S BDMDETAL(1)="^^REMOVED "_$P(^DIC(F,0),U,1)_" IEN "_BDMLABI_" FROM TAXONOMY^^"_BDMLABI
 .D LOG^BUSAAPI("O","O","D",$S($G(XQY0)]"":$P(XQY0,U),1:"REM~BDMHUTL"),"DM AUDIT TAXONOMY UPDATE: "_$P(^ATXAX(BDMTAXI,0),U,1),"BDMDETAL")
REMX ;
 D ^XBFMK
 D BACK
 Q
ADD ;EP - add an item to the selected list - called from a protocol
 ...
 ;IHS/CMI/LAB - added busa call to log adding entry to lab taxonomy
 NEW BDMDETAL
 S BDMDETAL(1)="^^ADD "_$P(^DIC(BDMFILE,0),U,1)_"IEN "_BDMTXLI_"^"_BDMTXLI
 D LOG^BUSAAPI("O","O","A",$S($G(XQY0)]"":$P(XQY0,U),1:"ADD~BDMHUTL"),"DM AUDIT TAXONOMY UPDATE: "_$P(^ATXAX(DA,0),U,1),"BDMDETAL")
 ;
 G ADDX
LABADD ;
 K DIC
 ...
 ;IHS/CMI/LAB - added busa call to log adding entry to lab taxonomy
 NEW BDMDETAL
 S BDMDETAL(1)="^^ADD LAB TEST IEN: "_BDMTXLI_" TO "_$P(^ATXLAB(BDMTAXI,0),U,1)_"^"_BDMTXLI
 D LOG^BUSAAPI("O","O","A",$S($G(XQY0)]"":$P(XQY0,U),1:"ADD~BDMHUTL"),"DM AUDIT LAB TAXONOMY UPDATE","BDMDETAL")
 ;
ADDX ;
