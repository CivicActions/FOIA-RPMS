BDMDH13 ; IHS/CMI/LAB - 2020 DIABETES AUDIT ;
...
FLU(P,BDATE,EDATE,R,F) ;EP
 ...
 .S G=$$REFUSAL^BDMDH17(P,9999999.14,$O(^AUTTIMM("C",A,0)),BDATE,EDATE)
...
PNEU(P,EDATE,R,F) ;EP
 ...
 .S G=$$REFUSAL^BDMDH17(P,9999999.14,$O(^AUTTIMM("C",R,0)),$$DOB^AUPNPAT(P),DT,"R")
...
HTNDX(P,EDATE) ;EP - is HTN on problem list
 ...
 .I $P($G(^AUPNPROB(X,800)),U,1)]"",$$SNOMED^BDMUTL(2020,"PXRM ESSENTIAL HYPERTENSION",$P(^AUPNPROB(X,800),U,1)) S G=1
 .Q
 I G Q "1  Yes"
 K BDM
...
PLDMDOO(P,F) ;EP
 ...
 ..Q
 .I $P($G(^AUPNPROB(X,800)),U,1)]"",$$SNOMED^BDMUTL(2020,"PXRM DIABETES",$P(^AUPNPROB(X,800),U,1)) D
 ..I $P(^AUPNPROB(X,0),U,13)]"" S D($P(^AUPNPROB(X,0),U,13))=""
 .Q
...
PLDMDXS(P) ;EP - get all DM dxs from problem list
 ...
 .I $P($G(^AUPNPROB(X,800)),U,1)]"",$$SNOMED^BDMUTL(2020,"PXRM DIABETES",$P(^AUPNPROB(X,800),U,1)) S:D]"" D=D_";" S D=D_"SNOMED: "_$P(^AUPNPROB(X,800),U,1)
 .Q
 Q D
 ;
...
INCHES ;
 NEW F,FI,Z
 S (X,Z)=$$LASTHT^BDMDH13(BDMPD,BDMRED,"I")
 Q:X=""
 S X=X/12  ;get feet
 S F=$P(X,".")
...
HEP(P,EDATE,S,F) ;EP
 ...
 I BDMHEP>2 Q "1  Yes"
I ;;
 ;NEW FOR 2020 AUDIT CHECK FOR 2 OF CVX 189
 S C="189"
 D GETIMMS^BDMUTL(P,EDATE,C,.BDMX)
 ;go through and set into array if 10 days apart
 S X=0 F  S X=$O(BDMX(X)) Q:X'=+X  S BDMHEP(X)=""
 ...
 I $$PLTAX^BDMDH12(P,"BGP HEP EVIDENCE") Q "4  Immune by DX"
 ;now go to Refusals
 ...
 S R=$$CPTREFT^BDMDHDU(P,$$DOB^AUPNPAT(P),EDATE,$O(^ATXAX("B","BGP HEPATITIS CPTS",0)))
