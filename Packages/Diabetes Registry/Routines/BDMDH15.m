BDMDH15 ; IHS/CMI/LAB -IHS -CUMULATIVE REPORT 03 Feb 2014 5:38 PM ;
...
GENDER ;
 ...
 S V=$G(^XTMP("BDMDM20",BDMJOB,BDMBTH,"AUDIT",BDMPD,20))
...
TYPE ;
 ...
 S X=$$TYPE^BDMDH16(BDMPD,BDMDMRG,BDMADAT)
...
DURDMC ;
 ...
 ;
 S V=$$DURDM^BDMDH16(BDMPD,BDMDMRG,BDMADAT)
...
BMI ;
 ...
 ;S V=$G(^XTMP("BDMDM20",BDMJOB,BDMBTH,"AUDIT",BDMPD,112))
 S H=$$LASTHT^BDMDH13(BDMPD,BDMRED,"I") S:H]"" H=$$STRIP^XLFSTR($J(H,5,2)," ")
 S W=+$$LASTWT^BDMDH13(BDMPD,BDMBDAT,BDMRED),W=$S(W=0:"",1:W) I W]"" S W=W\1
 S V=$$BMIEPI^BDMDH19(H,W)
 ...
 .I $$OW^BDMDH14(BDMPD,V,BDMADAT) S $P(BDMCUML(40),U,3)=$P(BDMCUML(40),U,3)+1 Q
 .I $$OB^BDMDH14(BDMPD,V,BDMADAT) S $P(BDMCUML(40),U,4)=$P(BDMCUML(40),U,4)+1
 .I $$SOB^BDMDH14(BDMPD,V,BDMADAT) S $P(BDMCUML(40),U,7)=$P(BDMCUML(40),U,7)+1,BDMSEVOB=1 Q
HGB ;
...
BPC ;blood pressure control
 ...
 .I S<160&(D<100) S $P(BDMCUML(60),U,6)=$P(BDMCUML(60),U,6)+1 Q  ;Patch 13 2020 change 95 to 100
...
HTN ;410
 ...
 S H=$E($G(^XTMP("BDMDM20",BDMJOB,BDMBTH,"AUDIT",BDMPD,34)))
 ...
 S A=$E($G(^XTMP("BDMDM20",BDMJOB,BDMBTH,"AUDIT",BDMPD,60)))  ;ACE/ARB USAGE
...
TBSTAT ;
 ...
 I $E($P(V,"||",2))=1,$E($G(^XTMP("BDMDM20",BDMJOB,BDMBTH,"AUDIT",BDMPD,72)))=1 S $P(BDMCUML(70),U,7)=$P(BDMCUML(70),U,7)+1
 I $E($P(V,"||",2))=2 D
 .S $P(BDMCUML(70),U,9)=$P(BDMCUML(70),U,9)+1
 .S X=$G(^XTMP("BDMDM20",BDMJOB,BDMBTH,"AUDIT",BDMPD,114))
 .I $$DODX^BDMDH16(BDMPD,BDMDMRG,"I")="" Q
 .S D=$$DODX^BDMDH16(BDMPD,BDMDMRG,"I"),E=X I D>E Q
 .S $P(BDMCUML(70),U,8)=$P(BDMCUML(70),U,8)+1
TOBACCO ;
 S $P(BDMCUML(80),U,2)=$P(BDMCUML(80),U,2)+1
 S V=$G(^XTMP("BDMDM20",BDMJOB,BDMBTH,"AUDIT",BDMPD,27))
 S V1=$G(^XTMP("BDMDM20",BDMJOB,BDMBTH,"AUDIT",BDMPD,28))
...
ENDS ;
 ...
 S T=$G(^XTMP("BDMDM20",BDMJOB,BDMBTH,"AUDIT",BDMPD,27))  ;TOBACCO USER
...
DMTX ;diabetes treatment
 ...
 S V=$$THERAPY^BDMDH16(BDMPD,BDM6MBD,BDMRED)  ;^ pieced with each item  or =1 for diet alone or =14 for refused
...
ASPIRIN ;
 ...
 I $E($G(^XTMP("BDMDM20",BDMJOB,BDMBTH,"AUDIT",BDMPD,116)))'=1 G LIPIDAG  ;not cvd
 S $P(BDMCUML(100),U,2)=$P(BDMCUML(100),U,2)+1  ;total with cvd
 S V=$G(^XTMP("BDMDM20",BDMJOB,BDMBTH,"AUDIT",BDMPD,62))
...
LIPIDAG ;lipid agents
 ...
 S V=$E($$STATIN^BDMDH16(BDMPD,BDM6M,BDMADAT))
 ...
 S C=0
 I $E($G(^XTMP("BDMDM20",BDMJOB,BDMBTH,"AUDIT",BDMPD,116)))=1 D
 ...
 ;
LIPID ;115
 S V=$G(^XTMP("BDMDM20",BDMJOB,BDMBTH,"AUDIT",BDMPD,61))
 S L=$$LDL^BDMDH18(BDMPD,BDMBDAT,BDMADAT,"I"),L=$P(L,U)
 S T=$$CHOL^BDMDH18(BDMPD,BDMBDAT,BDMADAT,"I"),T=$P(T,U)
 S T=$$STV^BDMDH18(T,5,1)
 S L=$$STV^BDMDH18(L,5,1)
DEP ;
...
EXAMS ;
 ...
 S V=$G(^XTMP("BDMDM20",BDMJOB,BDMBTH,"AUDIT",BDMPD,108))
...
LEAMP ;
 ...
 S V=$G(^XTMP("BDMDM20",BDMJOB,BDMBTH,"AUDIT",BDMPD,230))
...
SYSMEAN(P,BDATE,EDATE) ;EP
 NEW X,Z,C,Y
 S X=$$BPS^BDMDH13(P,BDATE,EDATE,"I")
 I X="" Q ""
