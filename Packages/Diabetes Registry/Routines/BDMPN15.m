BDMPN15 ; IHS/CMI/LAB -IHS -CUMULATIVE REPORT ;
 ;;2.0;DIABETES MANAGEMENT SYSTEM;**17**;JUN 14, 2007;Build 138
 ;
 ;
CUML ;EP
 K BDMCUML
 S BDMCUML(10)="Birth Gender"
 S BDMCUML(20)="Age"
 ...
 ;
PROCESS ;
 S BDMPD=0 F  S BDMPD=$O(^XTMP("BDMPN1",BDMJOB,BDMBTH,"AUDIT",BDMPD)) Q:BDMPD'=+BDMPD  D CUML1
 Q
 ;
CUML1 ;
GENDER ;
 ...
 S V=$G(^XTMP("BDMPN1",BDMJOB,BDMBTH,"AUDIT",BDMPD,20))
...
CLASS ;25
 ...
 S X=$G(^XTMP("BDMPN1",BDMJOB,BDMBTH,"AUDIT",BDMPD,220))  ;PREDM
 I $E(X)!($P(X,U,3)) S $P(BDMCUML(25),U,3)=$P(BDMCUML(25),U,3)+1  ;add 1 to predm
 S X=$G(^XTMP("BDMPN1",BDMJOB,BDMBTH,"AUDIT",BDMPD,200))  ;ifg
 I $E(X)!($P(X,U,3)) S $P(BDMCUML(25),U,4)=$P(BDMCUML(25),U,4)+1  ;add 1 to ifg
 S X=$G(^XTMP("BDMPN1",BDMJOB,BDMBTH,"AUDIT",BDMPD,210))  ;igt
...
HGB ;
 ...
 S V=$P($G(^XTMP("BDMPN1",BDMJOB,BDMBTH,"AUDIT",BDMPD,78)),U,2)
 S P=""
 I V=""!(V="?") S P=6 G HGBS
 I V["<" S P=3
 I V[">" S P=5
...
HTN ;
 ...
 S H=$E($G(^XTMP("BDMPN1",BDMJOB,BDMBTH,"AUDIT",BDMPD,34)))
...
TOBSCR ;
 ...
 S V=$G(^XTMP("BDMPN1",BDMJOB,BDMBTH,"AUDIT",BDMPD,215))  ;S^U
 S V1=$G(^XTMP("BDMPN1",BDMJOB,BDMBTH,"AUDIT",BDMPD,28))  ;COUNSEL
...
DMTX ;diabetes treatment
 ...
 S V=$G(^XTMP("BDMPN1",BDMJOB,BDMBTH,"AUDIT",BDMPD,54)) I V="X" S $P(BDMCUML(90),U,3)=$P(BDMCUML(90),U,3)+1
 S V=$G(^XTMP("BDMPN1",BDMJOB,BDMBTH,"AUDIT",BDMPD,100)) I V="X" S $P(BDMCUML(90),U,4)=$P(BDMCUML(90),U,4)+1
 S V=$G(^XTMP("BDMPN1",BDMJOB,BDMBTH,"AUDIT",BDMPD,103)) I V="X" S $P(BDMCUML(90),U,5)=$P(BDMCUML(90),U,5)+1
 S V=$G(^XTMP("BDMPN1",BDMJOB,BDMBTH,"AUDIT",BDMPD,56)) I V="X" S $P(BDMCUML(90),U,6)=$P(BDMCUML(90),U,6)+1
 S V=$G(^XTMP("BDMPN1",BDMJOB,BDMBTH,"AUDIT",BDMPD,58)) I V="X" S $P(BDMCUML(90),U,7)=$P(BDMCUML(90),U,7)+1
 S V=$G(^XTMP("BDMPN1",BDMJOB,BDMBTH,"AUDIT",BDMPD,55)) I V="X" S $P(BDMCUML(90),U,8)=$P(BDMCUML(90),U,8)+1
 ;
STATIN ;
 S $P(BDMCUML(95),U,2)=$P(BDMCUML(95),U,2)+1  ;total pats
 S V=$G(^XTMP("BDMPN1",BDMJOB,BDMBTH,"AUDIT",BDMPD,300))
...
QUAN ;
 ...
 S Q=$G(^XTMP("BDMPN1",BDMJOB,BDMBTH,"AUDIT",BDMPD,92))
 S V=$E($G(^XTMP("BDMPN1",BDMJOB,BDMBTH,"AUDIT",BDMPD,92)))  ;test done?
 ;S T=$P($G(^XTMP("BDMPN1",BDMJOB,BDMBTH,"AUDIT",BDMPD,92)),U,5)  ;type of test
