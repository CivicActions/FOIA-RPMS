BDMPL15 ; IHS/CMI/LAB -IHS -CUMULATIVE REPORT ; 16 Oct 2025  12:48 PM
...
CUML ;EP
 ...
 ;
PROCESS ;
 S BDMPD=0 F  S BDMPD=$O(^XTMP("BDMPL1",BDMJOB,BDMBTH,"AUDIT",BDMPD)) Q:BDMPD'=+BDMPD  D CUML1
 Q
 ;
CUML1 ;
GENDER ;
 ;gender BDMCUML(10)="Sex^total^females^males"
 S $P(BDMCUML(10),U,2)=$P(BDMCUML(10),U,2)+1  ;total patients
 S V=$G(^XTMP("BDMPL1",BDMJOB,BDMBTH,"AUDIT",BDMPD,20))
...
CLASS ;25
 ...
 S X=$G(^XTMP("BDMPL1",BDMJOB,BDMBTH,"AUDIT",BDMPD,220))  ;PREDM
 I $E(X)!($P(X,U,3)) S $P(BDMCUML(25),U,3)=$P(BDMCUML(25),U,3)+1  ;add 1 to predm
 S X=$G(^XTMP("BDMPL1",BDMJOB,BDMBTH,"AUDIT",BDMPD,200))  ;ifg
 I $E(X)!($P(X,U,3)) S $P(BDMCUML(25),U,4)=$P(BDMCUML(25),U,4)+1  ;add 1 to ifg
 S X=$G(^XTMP("BDMPL1",BDMJOB,BDMBTH,"AUDIT",BDMPD,210))  ;igt
...
BMI ;
 ...
 .I $$SOB^BDMDL14(BDMPD,V,BDMADAT) S $P(BDMCUML(40),U,7)=$P(BDMCUML(40),U,7)+1 Q
 ;
HGB ;
 ;use last hgba1c value only
 ;BDMCUML(50)=
 S $P(BDMCUML(50),U,2)=$P(BDMCUML(50),U,2)+1
 S V=$P($G(^XTMP("BDMPL1",BDMJOB,BDMBTH,"AUDIT",BDMPD,78)),U,2)
 S P=""
 I V=""!(V="?") S P=6 G HGBS
 I V["<" S P=3
 I V[">" S P=5
...
HTN ;
 ...
 S H=$E($G(^XTMP("BDMPL1",BDMJOB,BDMBTH,"AUDIT",BDMPD,34)))
...
TOBSCR ;
 ...
 S V=$G(^XTMP("BDMPL1",BDMJOB,BDMBTH,"AUDIT",BDMPD,215))  ;S^U
 S V1=$G(^XTMP("BDMPL1",BDMJOB,BDMBTH,"AUDIT",BDMPD,28))  ;COUNSEL
...
DMTX ;diabetes treatment
 ...
 S V=$G(^XTMP("BDMPL1",BDMJOB,BDMBTH,"AUDIT",BDMPD,54)) I V="X" S $P(BDMCUML(90),U,3)=$P(BDMCUML(90),U,3)+1
 S V=$G(^XTMP("BDMPL1",BDMJOB,BDMBTH,"AUDIT",BDMPD,100)) I V="X" S $P(BDMCUML(90),U,4)=$P(BDMCUML(90),U,4)+1
 S V=$G(^XTMP("BDMPL1",BDMJOB,BDMBTH,"AUDIT",BDMPD,103)) I V="X" S $P(BDMCUML(90),U,5)=$P(BDMCUML(90),U,5)+1
 S V=$G(^XTMP("BDMPL1",BDMJOB,BDMBTH,"AUDIT",BDMPD,56)) I V="X" S $P(BDMCUML(90),U,6)=$P(BDMCUML(90),U,6)+1
 S V=$G(^XTMP("BDMPL1",BDMJOB,BDMBTH,"AUDIT",BDMPD,58)) I V="X" S $P(BDMCUML(90),U,7)=$P(BDMCUML(90),U,7)+1
 S V=$G(^XTMP("BDMPL1",BDMJOB,BDMBTH,"AUDIT",BDMPD,55)) I V="X" S $P(BDMCUML(90),U,8)=$P(BDMCUML(90),U,8)+1
 ;
STATIN ;
 S $P(BDMCUML(95),U,2)=$P(BDMCUML(95),U,2)+1  ;total pats
 S V=$G(^XTMP("BDMPL1",BDMJOB,BDMBTH,"AUDIT",BDMPD,300))
...
LDL ;
 ...
 S V=$$LDL^BDMDL18(BDMPD,BDMBDAT,BDMADAT,"I")
 I V="" S $P(BDMCUML(190),U,7)=$P(BDMCUML(190),U,7)+1 G HDL
 S V=$P(V,U)
 ;S V=$$STV^BDMDL18(V,4)
 S V=$$STV^BDMDL18(V,,0) I $E(V)'=+$E(V)!(+V=0) S $P(BDMCUML(190),U,8)=$P(BDMCUML(190),U,8)+1 G HDL
...
HDL ;
 ...
 S V=$$HDL^BDMDL18(BDMPD,BDMBDAT,BDMADAT,"I")
 S S=$P(^DPT(BDMPD,0),U,2)  ;
 I S="F" D
 .S $P(BDMCUML(195),U,2)=$P(BDMCUML(195),U,2)+1
 .I V="" S $P(BDMCUML(195),U,5)=$P(BDMCUML(195),U,5)+1 Q
 .S V=$P(V,U)
 .S V=$$STV^BDMDL18(V,,0) I $E(V)'=+$E(V)!(+V=0) S $P(BDMCUML(195),U,5)=$P(BDMCUML(195),U,5)+1 Q
 .S V=$P(V,"."),V=$$STV^BDMDL18(V,,0)
 ...
 .S V=$P(V,U)
 .S V=$$STV^BDMDL18(V,,0) I $E(V)'=+$E(V)!(+V=0) S $P(BDMCUML(195),U,9)=$P(BDMCUML(195),U,9)+1 Q  ;unable to determine result, not a number
 .S V=$P(V,"."),V=$$STV^BDMDL18(V,,0)
...
TRIG ;
 ...
 S V=$$TRIG^BDMDL18(BDMPD,BDMBDAT,BDMADAT,"I")
 I V="" S $P(BDMCUML(200),U,7)=$P(BDMCUML(200),U,7)+1 G QUAN
 S V=$P(V,U)
 S V=$$STV^BDMDL18(V,,0) I $E(V)'=+$E(V) S $P(BDMCUML(200),U,5)=$P(BDMCUML(200),U,5)+1 G QUAN ;unable to determine result, not a number
...
QUAN ;
 ...
 S Q=$G(^XTMP("BDMPL1",BDMJOB,BDMBTH,"AUDIT",BDMPD,92))
 S V=$E($G(^XTMP("BDMPL1",BDMJOB,BDMBTH,"AUDIT",BDMPD,92)))  ;test done?
 ;S T=$P($G(^XTMP("BDMPL1",BDMJOB,BDMBTH,"AUDIT",BDMPD,92)),U,5)  ;type of test
