BDMDN1B ; IHS/CMI/LAB - get dm audit values ;
...
MA ;now check for abortion or miscarriage
 ...
 S %=$$CPT^BDMDNDU(P,BDMD,EDATE,T,3)
 I %]"" Q 0
 S T=$O(^ATXAX("B","BGP CPT MISCARRIAGE",0))
 S %=$$CPT^BDMDNDU(P,BDMD,EDATE,T,3)
 I %]"" Q 0
 S T=$O(^ATXAX("B","BGP CPT ABORTION",0))
 S %=$$TRAN^BDMDNDU(P,BDMD,EDATE,T,3)
 I %]"" Q 0
 S T=$O(^ATXAX("B","BGP CPT MISCARRIAGE",0))
 S %=$$TRAN^BDMDNDU(P,BDMD,EDATE,T,3)
 I %]"" Q 0
...
CESS ;EP - find any cessation hf in 12 months before
 ...
 I '$G(P) Q ""
 S X=$$TOBACCO^BDMDN1T(P,BDATE,EDATE)
 I +$P(X,U,2)'=1 Q ""
...
PED(P,BDATE,EDATE) ;EP
 ...
 ..I $P(T,"-")]"",$$SNOMED^BDMUTL(2024,"PXRM BGP TOBACCO SCREENED",$P(T,"-")),$P(BDMOPED,U)<$P(BDMALLED(X),U) S %=$P(BDMALLED(X),U)_U_T Q
