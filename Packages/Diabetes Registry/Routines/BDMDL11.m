BDMDL11 ; IHS/CMI/LAB -IHS -CUMULATIVE REPORT 10 Oct 2014 9:50 AM ; 
...
IMM ;
 ...
 I $E(V)="3"!($E(V)=1) S $P(BDMCUML(140),U,10)=$P(BDMCUML(140),U,10)+1  ;complete series or immune
 I $E(V)="3" S $P(BDMCUML(140),U,11)=$P(BDMCUML(140),U,11)+1
 ;TDAP EVER
 S V=$G(^XTMP("BDMDM26",BDMJOB,BDMBTH,"AUDIT",BDMPD,216))
 I $E(V)="1" S $P(BDMCUML(140),U,12)=$P(BDMCUML(140),U,12)+1
 I $E(V)="3" S $P(BDMCUML(140),U,13)=$P(BDMCUML(140),U,13)+1
 ;SHINGRIX
 I $$AGE^AUPNPAT(BDMPD,BDMADAT)>49 D
 .S V=$G(^XTMP("BDMDM26",BDMJOB,BDMBTH,"AUDIT",BDMPD,73))
 .I $E(V)="1" S $P(BDMCUML(140),U,14)=$P(BDMCUML(140),U,14)+1
 .S $P(BDMCUML(140),U,15)=$P(BDMCUML(140),U,15)+1
 ;RSV
 I $$AGE^AUPNPAT(BDMPD,BDMADAT)>49 D
 .S V=$G(^XTMP("BDMDM26",BDMJOB,BDMBTH,"AUDIT",BDMPD,74))
 .I $E(V)="1" S $P(BDMCUML(140),U,16)=$P(BDMCUML(140),U,16)+1
 ;
QUAN ;
 ;145
 ...
 S V=$E($G(^XTMP("BDMDM26",BDMJOB,BDMBTH,"AUDIT",BDMPD,92)))  ;test done?
 ...
 .;S R=+R
 .I S<30 S $P(BDMCUML(145),U,12)=$P(BDMCUML(145),U,12)+1,BDMUACRV="<30" Q  ;NORMAL UACR
 .I S'>300 S $P(BDMCUML(145),U,13)=$P(BDMCUML(145),U,13)+1,BDMCKD=1,BDMUACRV=">30" Q  ;INREASCED 30-300
 .S $P(BDMCUML(145),U,14)=$P(BDMCUML(145),U,14)+1,BDMCKD=1,BDMUACRV=">30"  ;INCREASED >300
 S G=$P($G(^XTMP("BDMDM26",BDMJOB,BDMBTH,"AUDIT",BDMPD,79)),U,2)  ;egfr value
 S H=0
...
GFR ;
 ...
 S V=$G(^XTMP("BDMDM26",BDMJOB,BDMBTH,"AUDIT",BDMPD,79))
 ...
 S BDMEGFRV=+V
 I BDMEGFRV<60 S BDMCKD=1
 I BDMEGFRV>60!(BDMEGFRV=60) S $P(BDMCUML(175),U,6)=$P(BDMCUML(175),U,6)+1 G TCHOL
 I BDMEGFRV<15 S $P(BDMCUML(175),U,9)=$P(BDMCUML(175),U,9)+1 G TCHOL
 I BDMEGFRV>29,BDMEGFRV<60 S $P(BDMCUML(175),U,7)=$P(BDMCUML(175),U,7)+1 G TCHOL
 I BDMEGFRV>14,BDMEGFRV<30 S $P(BDMCUML(175),U,8)=$P(BDMCUML(175),U,8)+1 G TCHOL
 ;
TCHOL ;
 S:'$D(BDMCUML(180)) BDMCUML(180)="Total Cholesterol obtained in past 12 months"
 S $P(BDMCUML(180),U,2)=$P(BDMCUML(180),U,2)+1
 S V=$$CHOL^BDMDL18(BDMPD,BDMBDAT,BDMADAT,"I")
...
TRIG ;
 ...
 S V=$P(V,U)
 S V=$$STV^BDMDL18(V,,0) I $E(V)'=+$E(V) S $P(BDMCUML(200),U,5)=$P(BDMCUML(200),U,5)+1 G ACE ;unable to determine result, not a number
...
ACE ;110 title^total pts^total pts with protein^# of those on ace^# with htn^# of those on ace"
 ...
 S P=$G(^XTMP("BDMDM26",BDMJOB,BDMBTH,"AUDIT",BDMPD,92))  ;URINE PROTEIN VALUE
...
COMBINED ;USE 260
 ...
 I $E($G(^XTMP("BDMDM26",BDMJOB,BDMBTH,"AUDIT",BDMPD,300)))=3 G E  ;statin allergy
 S V=$G(^XTMP("BDMDM26",BDMJOB,BDMBTH,"AUDIT",BDMPD,118))
