BDMDL19 ; IHS/CMI/LAB - 2026 DIABETES AUDIT ;
 ;;2.0;DIABETES MANAGEMENT SYSTEM;**19**;JUN 14, 2007;Build 159
 ;
 ;
 W:$D(IOF) @IOF
 W !!,"Checking for Taxonomies to support the 2026 Audit. ",!,"Please enter the device for printing.",!
ZIS ;
 S XBRC="",XBRP="TAXCHK^BDMDL19",XBNS="",XBRX="XIT^BDMDL19"
 D ^XBDBQUE
 D XIT
 Q
TAXCHK ;EP - called by gui
 ;D HOME^%ZIS
 K BDMQUIT
GUICHK ;EP
 NEW A,BDMX,I,Y,Z,J,BDMY,BDMT
 K A
 S BDMYR=$O(^BDMTAXS("B",2026,0))
 S BDMT=0 F  S BDMT=$O(^BDMTAXS(BDMYR,11,BDMT)) Q:BDMT'=+BDMT  D
 .S BDMY=$G(^BDMTAXS(BDMYR,11,BDMT,0))
 .I '$G(BDMIPRE) Q:'$P(^BDMTAXS(BDMYR,11,BDMT,0),U,6)
 .I $G(BDMIPRE) Q:'$P(^BDMTAXS(BDMYR,11,BDMT,0),U,7)
 .S BDMTN=$P(BDMY,U,1)
 .S BDMFILE=$P(BDMY,U,2)
 .Q:BDMFILE=""
 .Q:'$D(^DIC(BDMFILE,0))
 .S BDMTYPE=$P(^DIC($P(BDMY,U,2),0),U)
 .S Y=BDMTYPE_" taxonomy "
 .I BDMFILE'=60 D
 ..I '$D(^ATXAX("B",BDMTN)) S A(BDMTN)=Y_"^is Missing" Q
 ..S I=$O(^ATXAX("B",BDMTN,0))
 ..I '$D(^ATXAX(I,21,"B")) S A(BDMTN)=Y_"^has no entries "
 .I BDMFILE=60 D
 ..I '$D(^ATXLAB("B",BDMTN)) S A(BDMTN)=Y_"^is Missing " Q
 ..S I=$O(^ATXLAB("B",BDMTN,0))
 ..I '$D(^ATXLAB(I,21,"B")) S A(BDMTN)=Y_"^has no entries "
 ..;check for panels and warn
 ..I '$P(^ATXLAB(I,0),U,11) D
 ...S BDMY=0 F  S BDMY=$O(^ATXLAB(I,21,"B",BDMY)) Q:BDMY'=+BDMY  D
 ....I $O(^LAB(60,BDMY,2,0)) S A(BDMTN)=Y_"^contains a panel test: "_$P(^LAB(60,BDMY,0),U)_" and should not."
 I '$D(A) W !,"All taxonomies are present.",! K A,BDMX,BDMT,BDMY,Y,I,Z D DONE Q
 W !!,"In order for the 2026 DM AUDIT Report to find all necessary data, several",!,"taxonomies must be established.  The following taxonomies are missing or have",!,"no entries:"
 S BDMX="" F  S BDMX=$O(A(BDMX)) Q:BDMX=""!($D(BDMQUIT))  D
 .;I $Y>(IOSL-2) D PAGE Q:$D(BDMQUIT)
 .W !,$P(A(BDMX),U)," [",BDMX,"] ",$P(A(BDMX),U,2)
 .Q
DONE ;
 I $E(IOST)="C",IO=IO(0) S DIR(0)="EO",DIR("A")="End of taxonomy check.  HIT RETURN" D ^DIR K DIR S:$D(DUOUT) DIRUT=1
 Q
XIT ;EP
 K BDM,BDMX,BDMQUIT,BDMLINE,BDMJ,BDMX,BDMTEXT,BDM
 K X,Y,J
 Q
LASTHF(P,C,BDATE,EDATE,F) ;EP - get last factor in category C for patient P
 I '$G(P) Q ""
 I $G(C)="" Q ""
 I $G(F)="" S F=""
 S C=$O(^AUTTHF("B",C,0)) ;ien of category passed
 I '$G(C) Q ""
 NEW H,D,O S H=0 K O
 F  S H=$O(^AUTTHF("AC",C,H))  Q:'+H  D
 .  Q:'$D(^AUPNVHF("AA",P,H))
 .  S D="" F  S D=$O(^AUPNVHF("AA",P,H,D)) Q:D'=+D  D
 .. Q:(9999999-D)>EDATE  ;after time frame
 .. Q:(9999999-D)<BDATE  ;before time frame
 .. S O(D)=$O(^AUPNVHF("AA",P,H,D,""))
 .  Q
 S D=$O(O(0))
 I D="" Q D
 I F="F" Q $P(^AUTTHF($P(^AUPNVHF(O(D),0),U),0),U)
 ;
 Q 1
 ;;
BANNER ;EP - banner for 2014 audit menu
 S BDMTEXT="TEXTD",BDM("VERSION")="2.0 (Patch 18)"
 F BDMJ=1:1 S BDMX=$T(@BDMTEXT+BDMJ),BDMX=$P(BDMX,";;",2) Q:BDMX="QUIT"!(BDMX="")  S BDMLINE=BDMJ
PRINT D ^XBCLS W:$D(IOF) @IOF
 F BDMJ=1:1:BDMLINE S BDMX=$T(@BDMTEXT+BDMJ),BDMX=$P(BDMX,";;",2) W !?80-$L(BDMX)\2,BDMX K BDMX
 W !?80-(8+$L(BDM("VERSION")))/2,"Version ",BDM("VERSION")
  G XIT:'$D(DUZ(2)) G:'DUZ(2) XIT S BDM("SITE")=$P(^DIC(4,$S($G(BDMDUZ2):BDMDUZ2,1:DUZ(2)),0),"^") W !!?80-$L(BDM("SITE"))\2,BDM("SITE")
 D XIT
 Q
TEXTD ;EP
 ;;****************************************
 ;;**     DIABETES MANAGEMENT SYSTEM     **
 ;;**   2026 Diabetes Audit Report Menu  **
 ;;****************************************
 ;;QUIT
PAGE ;
 I $E(IOST)="C",IO=IO(0) W ! S DIR(0)="EO" D ^DIR K DIR I Y=0!(Y="^")!($D(DTOUT)) S BDMQUIT="" Q
 Q
BMIEPI(H,W) ;EP ;
 NEW %
 I H="" Q ""
 I W="" Q ""
 I 'H Q ""
 ;S W=W*.45359,H=(H*.0254),H=(H*H),%=(W/H),%=$J(%,4,1)
 S H=H*H,%=W/H,%=%*703,%=$$STRIP^XLFSTR($J(%,5,1)," ")
 Q %
SHINGRIX(P,BDATE,EDATE,NR) ;EP
 NEW BDMOPNU,BDMG,X,E,R,BD,ED,G,%,BDMX,BDMSHIN,RED,RBD
 S BDMOPNU=""
 S BD=BDATE
 S ED=EDATE
 S EDATE=$$FMTE^XLFDT(EDATE)
 S BDATE=$$FMTE^XLFDT(BDATE)
 ;get all immunizations
 S C="187"
 D GETIMMS^BDMUTL(P,ED,C,.BDMX)
 ;go through and set into array if 60 days apart
 S X=0 F  S X=$O(BDMX(X)) Q:X'=+X  S BDMSHIN(X)=""
 ;now get cpts
 S RED=9999999-ED,RBD=9999999-$$DOB^AUPNPAT(P),G=0
 F  S RED=$O(^AUPNVSIT("AA",P,RED)) Q:RED=""!($P(RED,".")>RBD)  D
 .S V=0 F  S V=$O(^AUPNVSIT("AA",P,RED,V)) Q:V'=+V  D
 ..Q:'$D(^AUPNVSIT(V,0))
 ..S X=0 F  S X=$O(^AUPNVCPT("AD",V,X)) Q:X'=+X  D
 ...S Y=$P(^AUPNVCPT(X,0),U) I $$ICD^ATXAPI(Y,$O(^ATXAX("B","BGP ZOSTER SHINGRIX CPTS",0)),1) S BDMSHIN(9999999-$P(RED,"."))=""
 ..S X=0 F  S X=$O(^AUPNVTC("AD",V,X)) Q:X'=+X  D
 ...S Y=$P(^AUPNVTC(X,0),U,7) I $$ICD^ATXAPI(Y,$O(^ATXAX("B","BGP ZOSTER SHINGRIX CPTS",0)),1) S BDMSHIN(9999999-$P(RED,"."))=""
 ;now check to see if they are all spaced days apart, if not, kill off the odd ones
 S X="",Y="",C=0 F  S X=$O(BDMSHIN(X)) Q:X'=+X  S C=C+1 D
 .I C=1 S Y=X Q
 .I $$FMDIFF^XLFDT(X,Y)<56 K BDMSHIN(X) Q
 .S Y=X
 ;now count them and see if there are 2 of them
 S BDMSHIN=0,X=0 F  S X=$O(BDMSHIN(X)) Q:X'=+X  S BDMSHIN=BDMSHIN+1
 I BDMSHIN>1 Q "1  Yes"_U_1
 I $G(NR) Q "2  No"
 ;CONTRA/REFUSAL
 ;LOOK FOR CONTRAINDICATION FIRST
 K BDMREF
 S S(187)=""
 S G="",Z="" F  S Z=$O(S(Z)) Q:Z=""  S X=0,Y=$O(^AUTTIMM("C",Z,0)) I Y F  S X=$O(^BIPC("AC",P,Y,X)) Q:X'=+X!(G)  D
 .S S=$P(^BIPC(X,0),U,3)
 .Q:S=""
 .Q:'$D(^BICONT(S,0))
 .Q:$P(^BICONT(S,0),U,1)["Refusal"
 .S T=$P(^BIPC(X,0),U,4)
 .Q:T=""
 .S BDMREF(9999999-T)=1_U_$$DATE^BDMS9B1(T)_U_$$VAL^XBDIQ1(9002084.11,X,.03)
 ;NMI, PROV DISCONTINUED, CONSIDERED AND NOT DONE 
 S BDMCVX="",G="" F  S BDMCVX=$O(S(BDMCVX)) Q:BDMCVX=""  D
 .S G=$$REFUSAL^BDMDL17(P,9999999.14,$O(^AUTTIMM("C",BDMCVX,0)),$$DOB^AUPNPAT(P),DT,"N")
 .I G I '$D(BDMREF(9999999-$P(G,U,7))) S BDMREF((9999999-$P(G,U,7)))=1_U_$P(G,U,3)_U_$P(G,U,5)
 .S G=$$REFUSAL^BDMDL17(P,9999999.14,$O(^AUTTIMM("C",BDMCVX,0)),$$DOB^AUPNPAT(P),DT,"P")
 .I G I '$D(BDMREF(9999999-$P(G,U,7))) S BDMREF((9999999-$P(G,U,7)))=1_U_$P(G,U,3)_U_$P(G,U,5)
 .S G=$$REFUSAL^BDMDL17(P,9999999.14,$O(^AUTTIMM("C",BDMCVX,0)),$$DOB^AUPNPAT(P),DT,"U")
 .I G I '$D(BDMREF(9999999-$P(G,U,7))) S BDMREF((9999999-$P(G,U,7)))=1_U_$P(G,U,3)_U_$P(G,U,5)
 S X=$O(BDMREF(0)) I X Q "  Contraindication: "_$P(BDMREF(X),U,3)_" "_$P(BDMREF(X),U,2)
 ;NOW CHECK REFUSAL IN PAST YEAR ONLY
 S BD=$$FMADD^XLFDT(DT,-365)
 S ED=DT
 S G="",Z="" F  S Z=$O(S(Z)) Q:Z=""  S X=0,Y=$O(^AUTTIMM("C",Z,0)) I Y F  S X=$O(^BIPC("AC",P,Y,X)) Q:X'=+X!(G)  D
 .S S=$P(^BIPC(X,0),U,3)
 .Q:S=""
 .Q:'$D(^BICONT(S,0))
 .Q:$P(^BICONT(S,0),U,1)'["Refusal"
 .S T=$P(^BIPC(X,0),U,4)
 .Q:T<BD
 .Q:T>ED
 .S BDMREF(9999999-T)=1_U_$$DATE^BDMS9B1(T)_U_$$VAL^XBDIQ1(9002084.11,X,.03)
 ;NMI, PROV DISCONTINUED, CONSIDERED AND NOT DONE 
 S BDMCVX="",G="" F  S BDMCVX=$O(S(BDMCVX)) Q:BDMCVX=""  D
 .S G=$$REFUSAL^BDMDL17(P,9999999.14,$O(^AUTTIMM("C",BDMCVX,0)),BD,DT,"R")
 .I G I '$D(BDMREF(9999999-$P(G,U,7))) S BDMREF((9999999-$P(G,U,7)))=1_U_$P(G,U,3)_U_$P(G,U,5)
 S X=$O(BDMREF(0)) I X Q "  "_$P(BDMREF(X),U,3)_"-"_$P(BDMREF(X),U,2)
 Q "2  No"
ANIMCONT(P,C,ED) ;EP - ANApHYLAXIS/IMMUNE CONTRAINDICATION
 NEW X,Y,R,D,G
 S X=0,G="",Y=$O(^AUTTIMM("C",C,0)) I Y F  S X=$O(^BIPC("AC",P,Y,X)) Q:X'=+X!(G)  D
 .S R=$P(^BIPC(X,0),U,3)
 .Q:R=""
 .Q:'$D(^BICONT(R,0))
 .S D=$P(^BIPC(X,0),U,4)
 .Q:D=""
 .;Q:$P(^BIPC(X,0),U,4)<BD
 .Q:$P(^BIPC(X,0),U,4)>ED
 .I $P(^BICONT(R,0),U,1)="Anaphylaxis" S G=D_U_"Contra Anaphylaxis"
 .I $P(^BICONT(R,0),U,1)="Immune Deficiency" S G=D_U_"Contra: Immune Deficiency"
 .;I $P(^BICONT(R,0),U,1)["Immune Deficient" S G=D_U_"Contra: Immune Deficient"
 Q G
LASTDENT(P) ;EP FOR FOLLOW UP REPORT
 NEW BDMG,BDMC,X,%,G,V,PROV,T,LASTD,BDATE,EDATE
 S BDATE=$$DOB^AUPNPAT(P),EDATE=DT
 S LASTD=""
 K BDMG
 S %=P_"^LAST EXAM 30;DURING "_$$FMTE^XLFDT(BDATE)_"-"_$$FMTE^XLFDT(EDATE),E=$$START1^APCLDF(%,"BDMG(")
 I $D(BDMG(1)) S LASTD=$P(BDMG(1),U)
 S BDMC="",%=P_"^LAST ADA 0000;DURING "_$$FMTE^XLFDT(BDATE)_"-"_$$FMTE^XLFDT(EDATE),E=$$START1^APCLDF(%,"BDMG(")
 I $D(BDMG(1)),$P(BDMG(1),U,1)>LASTD S LASTD=$P(BDMG(1),U)
 K BDMG
 S %=P_"^LAST ADA 0190;DURING "_$$FMTE^XLFDT(BDATE)_"-"_$$FMTE^XLFDT(EDATE),E=$$START1^APCLDF(%,"BDMG(")
 I $D(BDMG(1)),$P(BDMG(1),U,1)>LASTD S LASTD=$P(BDMG(1),U)
 K BDMG
 S BDMG(1)=$$LASTCPTT^BDMAPIU(P,BDATE,EDATE,"BGP DENTAL VISIT CPT CODES","E")
 I BDMG(1)]"",$P(BDMG(1),U,1)>LASTD S LASTD=$P(BDMG(1),U)
 K BDMG
 S BDMG=$$LASTDX^BDMAPIU(P,"BGP DENTAL VISIT DXS",BDATE,EDATE)
 I BDMG,$P(BDMG,U,3)>LASTD S LASTD=$P(BDMG,U,3)
 ;
 K BDMG S G="" S %=P_"^ALL ADA;DURING "_$$FMTE^XLFDT(BDATE)_"-"_$$FMTE^XLFDT(EDATE),E=$$START1^APCLDF(%,"BDMG(")
 I $D(BDMG) D  I G]"" Q G
 .S X=0 F  S X=$O(BDMG(X)) Q:X'=+X  S V=$P(BDMG(X),U,5) D
 ..I $P($G(^AUPNVSIT(V,0)),U,7)="C" Q
 ..I $P($G(^AUPNVSIT(V,0)),U,7)="T" Q
 ..S Y=+$P(BDMG(X),U,4)
 ..Q:'$$DVADA^BDMDL17($$VAL^XBDIQ1(9000010.05,Y,.01))
 ..S D=$$VD^APCLV(V)
 ..I D>LASTD S LASTD=D
 ..;S G="1  Yes  "_$$DATE^BDMS9B1($P(BDMG(X),U))_" ADA: "_$$VAL^XBDIQ1(9000010.05,Y,.01) Q
 Q LASTD
 ;
LASTRSV(BDMPDFN,BDMBD,BDMED,BDMFORM) ;PEP - date of last FLU
 ;
 I $G(BDMPDFN)="" Q ""
 I $G(BDMBD)="" S BDMBD=$$DOB^AUPNPAT(BDMPDFN)
 I $G(BDMED)="" S BDMED=DT
 I $G(BDMFORM)="" S BDMFORM="D"
 NEW BDMLAST,BDMVAL,BDMX,R,X,Y,V,E,T,G,BDMY,BDMF,Z,D,S
 S BDMLAST="",S=""
 S T=$O(^ATXAX("B","DM AUDIT RSV IZ CVX CODES",0))
 I T S X=0 F  S X=$O(^ATXAX(T,21,"B",X)) Q:X=""  S S(X)=""
 S R="" F  S R=$O(S(R)) Q:R=""  D
 .S BDMVAL=$$LASTITEM^APCLAPIU(BDMPDFN,R,"IMMUNIZATION",$S($P(BDMLAST,U)]"":$P(BDMLAST,U),1:BDMBD),BDMED,"A")
 .D E
 S BDMVAL=$$LASTCPTT^APCLAPIU(BDMPDFN,$S($P(BDMLAST,U)]"":$P(BDMLAST,U),1:BDMBD),BDMED,"DM AUDIT RSV CPT CODES","A")
 D E
 I BDMFORM="D" Q $P(BDMLAST,U)
 Q BDMLAST
E ;
 I $P(BDMVAL,U,1)>$P(BDMLAST,U,1) S BDMLAST=BDMVAL
 Q
RSV(P,BDATE,EDATE,R,F) ;EP
 NEW BDM,X,E,%,%DT,BD,B,D,C,Y,LFLU,TFLU,Z,G,T,S,A,J,BDMREF
 I $G(F)="" S F="E"
 S R=$G(R)
 S LFLU=$$LASTRSV(P,BDATE,EDATE,"D")
 I LFLU]"" Q $S(F="E":"1  Yes  "_$$DATE^BDMS9B1(LFLU),1:LFLU)
 I 'R Q $S(F="E":"2  No",1:"")  ;NO REFUSALS
 ;check refusals for DPCS
  ;CONTRA/REFUSAL
 ;LOOK FOR CONTRAINDICATION FIRST
 K BDMREF
 S T=$O(^ATXAX("B","DM AUDIT RSV IZ CVX CODES",0))
 S A=0 F  S A=$O(^ATXAX(T,21,"B",A)) Q:A=""  S S(A)=""
 S G="",Z="" F  S Z=$O(S(Z)) Q:Z=""  S X=0,Y=$O(^AUTTIMM("C",Z,0)) I Y F  S X=$O(^BIPC("AC",P,Y,X)) Q:X'=+X!(G)  D
 .S S=$P(^BIPC(X,0),U,3)
 .Q:S=""
 .Q:'$D(^BICONT(S,0))
 .Q:$P(^BICONT(S,0),U,1)["Refusal"
 .S T=$P(^BIPC(X,0),U,4)
 .Q:T=""
 .S BDMREF(9999999-T)=1_U_$$DATE^BDMS9B1(T)_U_$$VAL^XBDIQ1(9002084.11,X,.03)
 ;NMI, PROV DISCONTINUED, CONSIDERED AND NOT DONE 
 S BDMCVX="",G="" F  S BDMCVX=$O(S(BDMCVX)) Q:BDMCVX=""  D
 .S G=$$REFUSAL^BDMDL17(P,9999999.14,$O(^AUTTIMM("C",BDMCVX,0)),$$DOB^AUPNPAT(P),DT,"N")
 .I G I '$D(BDMREF(9999999-$P(G,U,7))) S BDMREF((9999999-$P(G,U,7)))=1_U_$P(G,U,3)_U_$P(G,U,5)
 .S G=$$REFUSAL^BDMDL17(P,9999999.14,$O(^AUTTIMM("C",BDMCVX,0)),$$DOB^AUPNPAT(P),DT,"P")
 .I G I '$D(BDMREF(9999999-$P(G,U,7))) S BDMREF((9999999-$P(G,U,7)))=1_U_$P(G,U,3)_U_$P(G,U,5)
 .S G=$$REFUSAL^BDMDL17(P,9999999.14,$O(^AUTTIMM("C",BDMCVX,0)),$$DOB^AUPNPAT(P),DT,"U")
 .I G I '$D(BDMREF(9999999-$P(G,U,7))) S BDMREF((9999999-$P(G,U,7)))=1_U_$P(G,U,3)_U_$P(G,U,5)
 S X=$O(BDMREF(0)) I X Q "  Contraindication: "_$P(BDMREF(X),U,3)_" "_$P(BDMREF(X),U,2)
 ;NOW CHECK REFUSAL IN PAST YEAR ONLY
 S BD=$$FMADD^XLFDT(DT,-365)
 S ED=DT
 S G="",Z="" F  S Z=$O(S(Z)) Q:Z=""  S X=0,Y=$O(^AUTTIMM("C",Z,0)) I Y F  S X=$O(^BIPC("AC",P,Y,X)) Q:X'=+X!(G)  D
 .S S=$P(^BIPC(X,0),U,3)
 .Q:S=""
 .Q:'$D(^BICONT(S,0))
 .Q:$P(^BICONT(S,0),U,1)'["Refusal"
 .S T=$P(^BIPC(X,0),U,4)
 .Q:T<BD
 .Q:T>ED
 .S BDMREF(9999999-T)=1_U_$$DATE^BDMS9B1(T)_U_$$VAL^XBDIQ1(9002084.11,X,.03)
 ;NMI, PROV DISCONTINUED, CONSIDERED AND NOT DONE 
 S BDMCVX="",G="" F  S BDMCVX=$O(S(BDMCVX)) Q:BDMCVX=""  D
 .S G=$$REFUSAL^BDMDL17(P,9999999.14,$O(^AUTTIMM("C",BDMCVX,0)),BD,DT,"R")
 .I G I '$D(BDMREF(9999999-$P(G,U,7))) S BDMREF((9999999-$P(G,U,7)))=1_U_$P(G,U,3)_U_$P(G,U,5)
 S X=$O(BDMREF(0)) I X Q "  "_$P(BDMREF(X),U,3)_"-"_$P(BDMREF(X),U,2)
 Q "2  No"
