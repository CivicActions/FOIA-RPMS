AZZZEOB6 ; IHS/ADC/GTH - PROCESS EOBRS (7/7) SUMMARY/ERROR REPORT ; [ 09/17/97   9:12 AM ]
 ;;3.0;CONTRACT HEALTH MGMT SYSTEM;**10**;SEP 17, 1997
 ;
START ;EP - From TaskMan.
 S ACHSSUM=0,ACHS("R")=$O(^ACHSEOBR(0)),ACHSPG=0
 G END:ACHS("R")=""
 D SITENAME
 ...
 . F ACHSCTR(1)=0:0 Q:$D(DUOUT)!$D(DTOUT)  S ACHSCTR(1)=$O(^ACHSEOBR(ACHSFAC,ACHSCTR(1))) Q:+ACHSCTR(1)'=ACHSCTR(1)  S ACHSOLD=$E(^ACHSEOBR(ACHSFAC,ACHSCTR(1)),1,18) D PRNT
 .Q
 G:$D(DUOUT)!$D(DTOUT) K^AZZZEOB5
...
PRNT ;
 ...
 I ACHSRPT=2,$D(^ACHSEOBR("ER",ACHSFAC,ACHSCTR(1))) D ^AZZZEOB2 KILL ^TMP("ACHSEOB",$J) Q
 I $D(^ACHSEOBR(ACHSFAC,ACHSCTR(1))) D W1 W ?34,ACHSEOBR("B",8) D COST W !
P0 ;
 S ACHS="",ACHSER1=0
 F  S ACHS=$O(^ACHSEOBR("ER",ACHSFAC,ACHSCTR(1),ACHS)) Q:ACHS=""  D
 . S ACHSER=$P($T(@ACHS^AZZZEOBG),";",4),ACHSDESC=$P($T(@ACHS^AZZZEOBG),";",3)
...
PRNT2A ;
 ...
 D PRNT,RTRN^ACHS
 I $D(DUOUT)!$D(DTOUT) G K^AZZZEOB5
 G PRNT2A
 ;
HDR ;
 U IO
...
DCR ; Print Begin/End values of DCRs
 NEW %,B,E,N,R,X,Y
 S N=^ACHS(9,DUZ(2),"RN"),Y=0
 F  S Y=$O(^ACHSEOBR("DCR",Y)) Q:'Y  D  Q:$G(ACHSQUIT)
 . I ^ACHSEOBR("DCR",Y,"B")=^ACHSEOBR("DCR",Y,"E") Q
 . S B=^ACHSEOBR("DCR",Y,"B")
 . S E=^ACHSEOBR("DCR",Y,"E")
