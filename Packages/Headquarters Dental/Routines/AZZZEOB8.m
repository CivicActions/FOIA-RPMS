AZZZEOB8 ; IHS/ADC/GTH - AREA WRITE EOBR FILES FOR FACILITIES (1/2) ; [ 09/17/97   9:12 AM ]
...
A ;
 ...
 ;F ACHSMDIA="C","D","F","T","X" Q:ACHSFLG  I $D(^ACHSAOP(DUZ(2),16,"E",ACHSMDIA))
 S ACHSPFAC="",ACHSMDIA="F"
 F  S ACHSPFAC=$O(^ACHSAOP(DUZ(2),16,"B",ACHSPFAC)) Q:ACHSPFAC=""!(ACHSFLG)  D SEQ,CDATA,START:ACHSFAC
K ;
 KILL ACHSFAC,ACHSFLG,ACHSMDIA,ACHSMSG,ACHSMSM,ACHSPFAC,ACHSEOBX,ACHSFACN,ACHSMCNT,ACHSR,ACHSRR,ACHSZRCT
 S ACHSIO=IO
 D RTRN^ACHS
 Q
 ;
...
HFS ;
 ...
 U IO(0)
 W !,"Please Standby - Copying Data to Host File ",$$EX^ACHS,ACHSZFN,!
 D SAVE
...
END ;
 KILL %MT,ACHS,ACHSFAC,R,ACHSRCTR,X1,X2
 Q
 ;
SAVE ;EP.
 D WAIT^DICD
