BQIDCAH6 ;GDIT/HS/ALA-Ad Hoc Logic ; 26 Apr 2013  11:00 AM
 ;;2.9;ICARE MANAGEMENT SYSTEM;**1,2,5,7**;Mar 01, 2021;Build 14
 ;
...
NKNN ;EP - No known allergy
 ...
 Q
 ;
REM(FGLOB,TGLOB,REMCODE,RMFROM,RMTHRU,OVD,FUT,RMOP,MPARMS) ;EP
 I $G(TGLOB)="" Q
 I $G(REMCODE)'="" D RMD
 ...
 ;
 I RMOP="&" D
 . S RM="",CT=0 F  S RM=$O(MPARMS("REMCODE",RM)) Q:RM=""  S CT=CT+1
 . S IEN="",LCT=0 F  S IEN=$O(@CRIT@("REM",IEN)) Q:IEN=""  D
 .. S CD="",LCT=0 F  S CD=$O(@CRIT@("REM",IEN,CD)) Q:CD=""  S LCT=LCT+1
 .. I LCT'=CT K @CRIT@("REM",IEN),@TGLOB@(IEN) Q
 K REMCODE,MPARMS("REMCODE"),RMPT,LCT,CT,RM
 Q
 ;
RMD ;EP
 NEW IEN
...
RMF(DFN) ;EP - Find reminder
 NEW RIEN,RDATA,RSTAT
 S RIEN=$O(^BQIPAT(DFN,40,"B",REMCODE,"")),RDATA=""
 I RIEN="",$P(REMCODE,"_",1)'="CMET" Q
 I RIEN'="" S RDATA=$G(^BQIPAT(DFN,40,RIEN,0)) I RDATA="",$P(REMCODE,"_",1)'="CMET" Q
 S RSTAT=$P(RDATA,"^",3),RSTAT=$$UP^XLFSTR(RSTAT)
 ;
 I FUT'=0 D  Q
 . ;I RSTAT="DUE NOW" Q
 . I RSTAT="RESOLVED" Q
 ...
 .. NEW RMIEN,RMDT
 .. S RMIEN="" F  S RMIEN=$O(^BTPWP("AE",DFN,"F",RMIEN)) Q:RMIEN=""  D
 ... I $P(^BTPWP(RMIEN,0),U,1)'=$P(REMCODE,"_",2) Q
 ... S RMDT=$P(^BTPWP(RMIEN,0),"^",13)
 ... I RMDT'<RMFROM,RMDT'>RMTHRU D  Q
 .... S @TGLOB@(DFN)="",@CRIT@("REM",DFN,REMCODE)="" Q
 .... ;I RMOP="&" S RMPT(DFN,REMCODE)="",@CRIT@("REM",DFN,REMCODE)=""
 . I $P(RDATA,U,4)'="" D
 .. I $G(RMFROM)'="" D
 ... I $P(RDATA,U,4)<RMFROM!($P(RDATA,U,4)>RMTHRU) Q
 ... S @TGLOB@(DFN)="",@CRIT@("REM",DFN,REMCODE)="" Q
 ... ;I RMOP="&" S RMPT(DFN,REMCODE)="",@CRIT@("REM",DFN,REMCODE)=""
 .. I $G(RMFROM)="",$P(RDATA,U,4)>DT D
 ... S @TGLOB@(DFN)="",@CRIT@("REM",DFN,REMCODE)="" Q
 ... ;I RMOP="&" S RMPT(DFN,REMCODE)="",@CRIT@("REM",DFN,REMCODE)=""
 ;
 I OVD'=0 D  Q
 ...
 ... S @TGLOB@(DFN)="",@CRIT@("REM",DFN,REMCODE)="" Q
 ... ;I RMOP="&" S RMPT(DFN,REMCODE)="",@CRIT@("REM",DFN,REMCODE)=""
 . I $P(RDATA,"^",2)=""&($P(RDATA,"^",3)="")&($P(RDATA,"^",4)="") Q
 . I $P(RDATA,"^",4)'=""&($P(RDATA,"^",4)>DT) Q
 . S @TGLOB@(DFN)="",@CRIT@("REM",DFN,REMCODE)="" Q
 . ;I RMOP="&" S RMPT(DFN,REMCODE)="",@CRIT@("REM",DFN,REMCODE)=""
 Q
 ;
POVS(FGLOB,TGLOB,POVS,POVSB,FDT,TDT,PVOP,MPARMS) ;EP - POV SNOMED search
...
PVS ;
 ...
 . I DFN'="",PVOP="&" S PVPT(DFN,POVS)=IEN,@CRIT@("POV",DFN,IEN)=""
 ;
 Q
 ;
PVSDT ; EP
 ...
 . I PVOP="&" S PVPT(IEN,POVS)=LIEN,@CRIT@("POV",IEN,LIEN)=""
 Q
 ;
POV(FGLOB,TGLOB,POV,POVTX,FDT,TDT,PVOP,MPARMS) ;EP - POV search
...
PV ;EP
 ...
 . S VSDTM=$P(^AUPNVSIT(VIS,0),U,1)\1
 . I FDT'="",VSDTM<FDT!(VSDTM>TDT) Q
 . I DFN'="",PVOP="!" S @TGLOB@(DFN)="",@CRIT@("POV",DFN,IEN)="" Q
 . ;I DFN'="",PVOP="!" S @NGLOB@(DFN)="" Q
...
PVDT ;EP
 ...
 . I $G(^AUPNVSIT(VIS,0))="" Q
 . Q:"DXCT"[$P(^AUPNVSIT(VIS,0),U,7)
 . I PVOP="!" S @TGLOB@(IEN)=LIEN,@CRIT@("POV",IEN,LIEN)="" Q
 . ;I PVOP="!",CNOT S @NGLOB@(IEN)="" Q
 . I PVOP="&" S PVPT(IEN,POV)=LIEN,@CRIT@("POV",IEN,LIEN)=""
 Q
