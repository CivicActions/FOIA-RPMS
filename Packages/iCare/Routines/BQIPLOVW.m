BQIPLOVW ;GDIT/HS/ALA-Orders Tab View ; 04 May 2021  9:25 AM
 ;;2.9;ICARE MANAGEMENT SYSTEM;**3**;Mar 01, 2021;Build 32
 ;
 ;
LST(DATA,OWNR,PLIEN) ; EP - BQI GET ORDERS VIEW
 ;Description
 ...
 S DATA=$NA(^TMP("BQIPLOVW",UID))
 K @DATA
 S II=0
 NEW $ESTACK,$ETRAP S $ETRAP="D ERR^BQIPLOVW D UNWIND^%ZTER" ; SAC 2006 2.2.3.3.2
 ;
...
ERR ;
 ...
 Q
 ;
UPD(DATA,OWNR,PLIEN,TEMPL,SOR,SDIR,DOR) ; EP -- BQI SET ORDERS VIEW
 ;
 ;Description
...
FIL(OWNR,PLIEN,TEMPL,SOR,SDIR,DOR) ; EP - File data
 ...
 . F  S DA=$O(^BQICARE(OWNR,1,PLIEN,28,DA)) Q:'DA  D
 .. S IENS=$$IENS^DILF(.DA)
 .. S BQIDEL(90505.128,IENS,.01)="@"
 . I $D(BQIDEL) D FILE^DIE("","BQIDEL","ERROR")
 . S DA=0
 . F  S DA=$O(^BQICARE(OWNR,1,PLIEN,4,DA)) Q:'DA  D
 .. S IENS=$$IENS^DILF(.DA)
 .. I $$GET1^DIQ(90505.14,IENS,.02,"I")'="ORP" Q
 ...
 .. S DA(2)=OWNR,DA(1)=PLIEN
 .. S DIC="^BQICARE("_DA(2)_",1,"_DA(1)_",28,",DIE=DIC
 .. S DLAYGO=90505.128,DIC(0)="FL",DIC("P")=DLAYGO
 .. S X=GIEN
 .. I '$D(^BQICARE(DA(2),1,DA(1),28,0)) S ^BQICARE(DA(2),1,DA(1),28,0)="^90505.128^^"
 .. K DO,DD D FILE^DICN
 .. S DA=+Y I DA<1 S ERROR=1 Q
 .. S IENS=$$IENS^DILF(.DA)
 .. S BQIUPD(90505.128,IENS,.02)=DI
 .. I $D(BQIUPD) D FILE^DIE("","BQIUPD","ERROR")
 . ;
 . F SI=1:1:$L(SOR,$C(29)) S SIEN=$P(SOR,$C(29),SI) Q:SIEN=""  D
 .. NEW DA,X,IENS,BQIUPD,SN
 .. S SN=$O(^BQICARE(OWNR,1,PLIEN,28,"B",SIEN,""))
 .. S DA(2)=OWNR,DA(1)=PLIEN,DA=SN,IENS=$$IENS^DILF(.DA)
 .. S BQIUPD(90505.128,IENS,.03)=SI
 .. S BQIUPD(90505.128,IENS,.04)=$P(SDIR,$C(29),SI)
 .. I $D(BQIUPD) D FILE^DIE("","BQIUPD","ERROR")
 ;
 ; If the user is sharing someone else's panel.
 NEW DA,IENS
 S DA(3)=OWNR,DA(2)=PLIEN,DA(1)=DUZ,DA=0
 F  S DA=$O(^BQICARE(OWNR,1,PLIEN,30,DUZ,28,DA)) Q:'DA  D
 . S IENS=$$IENS^DILF(.DA)
 . S BQIDEL(90505.328,IENS,.01)="@"
 I $D(BQIDEL) D FILE^DIE("","BQIDEL","ERROR")
 S DA=0
 F  S DA=$O(^BQICARE(OWNR,1,PLIEN,30,DUZ,4,DA)) Q:'DA  D
 . S IENS=$$IENS^DILF(.DA)
 . I $$GET1^DIQ(90505.34,IENS,.02,"I")'="ORP" Q
 ...
 . NEW DA,X,DINUM,DIC,DIE,DLAYGO,IENS
 . S DA(3)=OWNR,DA(2)=PLIEN,DA(1)=DUZ
 . S DIC="^BQICARE("_DA(3)_",1,"_DA(2)_",30,"_DA(1)_",28,",DIE=DIC
 . S DLAYGO=90505.328,DIC(0)="FL",DIC("P")=DLAYGO
 . S X=GIEN
 . I '$D(^BQICARE(DA(3),1,DA(2),30,DA(1),28,0)) S ^BQICARE(DA(3),1,DA(2),30,DA(1),28,0)="^90505.328^^"
 . K DO,DD D FILE^DICN
 . S DA=+Y I DA<1 S ERROR=1
 ;
 F SI=1:1:$L(SOR,$C(29)) S SIEN=$P(SOR,$C(29),SI) Q:SIEN=""  D
 . NEW DA,X,IENS,SN
 . S SN=$O(^BQICARE(OWNR,1,PLIEN,30,DUZ,28,"B",SIEN,""))
 . S DA(3)=OWNR,DA(2)=PLIEN,DA(1)=DUZ,DA=SN,IENS=$$IENS^DILF(.DA)
 . S BQIUPD(90505.328,IENS,.02)=SI
 . S BQIUPD(90505.328,IENS,.03)=$P(SDIR,$C(29),SI)
 I $D(BQIUPD) D FILE^DIE("","BQIUPD","ERROR")
 K BQIUPD
 Q
 ;
...
CVW() ;EP - Get Customized view
 ...
 .. S SIEN=$P(^BQICARE(OWNR,1,PLIEN,28,IEN,0),"^",3)
 .. S RIEN=$P(^BQICARE(OWNR,1,PLIEN,28,IEN,0),"^",4)
 ...
 .. S GIEN=$O(^BQI(90506.1,"B",CODE,""))
 .. S SIEN=$P(^BQICARE(OWNR,1,PLIEN,30,DUZ,28,IEN,0),"^",3)
 .. S RIEN=$P(^BQICARE(OWNR,1,PLIEN,30,DUZ,28,IEN,0),"^",4)
 .. S DISPLAY=DISPLAY_CODE_$C(29)
 .. I SIEN'="" D
 ... S $P(SORT,$C(29),SIEN)=CODE
 ... S $P(SDIR,$C(29),SIEN)=RIEN
