...
NGHT ;EP - Nightly Update of panels
 ...
 K BQIUPD
 ;
 NEW USR,PNL,LGLOB,LOCK,PLIDEN,LFLG,CSTA,DNN
 ...
 . S LOGIN=$P($G(^BQICARE(USR,0)),U,6),GUI=$P(^(0),U,17)
 . I LOGIN="" D FIX Q
 . I ($E(DT,1,3)-$E(LOGIN,1,3)>0),GUI'=$P(^BQI(90508,1,0),U,8) D FIX Q
 . ; Check if panel has reminders defined
 . S PNL="" F  S PNL=$O(^BQICARE("AC","N",USR,PNL)) Q:PNL=""  D
 .. S DNN="" F  S DNN=$O(^BQICARE(USR,1,PNL,15,"B",DNN)) Q:DNN=""  D
 ... I DNN="REMCODE" S $P(^BQICARE(USR,1,PNL,3),"^",12)=1
 ;
 D ORD^BQIPLPU
 NEW ORD,LKSUCC
 S ORD=""
 ...
 ... ; repopulate
 ... S BQINIGHT=1 D POP^BQIPLPP("",USR,PNL,"",USR) I $G(BMXSEC)'="" K BMXSEC
 ... ; Reset description
 ... NEW DA,IENS
...
CMA ;EP - Do Community Alerts
 ...
 D ^BQICALRT
 K BDATE,BQQN,DA,DIC,DTO,EDATE,LIEN,NMID,PDT,PROB,SCID,SDATE,SYS,TEMP,X,Y
 D ^BQICASUI
 K BDATE,BQIOK,BQISUB,BQQN,DIC,NMID,PIEN,SCID,SNIEN,STAT,TB
 D ^BQICAOPO
 K ALERT,BGDT,BQDFN,BQIOK,BQIOPO,BQQN,CONF,DA,DFN,DIAG,DIC,DXN,ENDT,FTYP,GRP,IEN,IENS,NMID,PIEN
 K POS,PRDT,SCID,SNIEN,SNO,SUBS,TIEN,TMFRAME,TREF,VDATE,VFILE,VSDTM,X,Y
 NEW DA
 S DA=$O(^BQI(90508,0)) I 'DA Q
 ...
 D FILE^DIE("","BQIUPD","ERROR")
 K BQIUPD
 F BQTSK="BQIALRTTMP","BQIPRCR","BQITAX","BQIALERT","BQISNOM" K ^TMP(BQTSK,UID)
 ;
 ; Do Export
 I $P(^BQI(90508,1,0),"^",6)="" D ^BQICAEXP
 I $P(^BQI(90508,1,0),"^",6)=2 D EN^BQICACCD
 I $P(^BQI(90508,1,0),"^",6)=1 D ^BQICAEXP,DUO^BQICACCD
 ;
 ...
 ;
 K BGDT,BQDFN,BQDXS,BQIOK,BQQN,CONF,ENDT,HLSTATE,I,NMID,PIEN,POS,PRDT,SCID,SNIEN,SNO,SUBS,TIEN,TMFRAME,TREF,VSDTM
 Q
 ;
ARM ;EP - Check and set up the 'ALL REMINDERS' Patient Health Summary Definition if needed
 ...
 . S X="ALL REMINDERS",DIC(0)="FLZ",DLAYGO=9001015,DIC="^APCHSCTL("
 . D FILE^DICN
 . ;
 . ; Build Sort Order Sub-File
 . N DIC,DA,DIE,DR,X,BQIUPD
 . S DLAYGO=9001015.01
 . S (DA(1),REMNDX)=+Y,DA=10,DIC(0)="FLZ",DIC="^APCHSCTL("_DA(1)_",1,"
 . D FILE^DICN
 . ;
 ...
 . S DA(1)=REMNDX,DLAYGO=9001015.06,DIC(0)="FLZ"
...
IMM ;EP - Set up Immunizations
 ...
 .. S DA(1)=8,X=CD,DIC="^BQI(90506.5,"_DA(1)_",10,",DIC(0)="FL",DLAYGO=90506.51
...
PRF ;EP - Communication Preference
 ...
 .. S DIC="^BQI(90506.3,"_DA(2)_",10,"_DA(1)_",5,",DIC(0)="FL"
 .. K DO,DD D FILE^DICN
 .. S DA=+Y
 .. NEW IENS
 .. S IENS=$$IENS^DILF(.DA)
