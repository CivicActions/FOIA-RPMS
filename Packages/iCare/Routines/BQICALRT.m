BQICALRT ;GDIT/HS/ALA-Expanded Community Alerts ; 11 Oct 2018  8:03 AM
...
FND ;EP - Find alerts
 ...
 ... S VCAT=$P(^AUPNVSIT(VISIT,0),U,7) I VCAT="" Q
 ... I "DINSRX"[VCAT Q
 ...
 ... S TREF=$NA(^XTMP("BQITAX"))
 ... K @TREF S @TREF@(0)=$$FMADD^XLFDT(DT,2)_U_$$DT^XLFDT()_U_"Taxonomy"
 ...
 ... I $G(TAX)="" D
 .... S TREF=$NA(^XTMP("BQITAX")) K @TREF S @TREF@(0)=$$FMADD^XLFDT(DT,2)_U_$$DT^XLFDT()_U_"HAS Taxonomy"
 .... S N=0
 ...
 ... S DTE=""
 ... F  S DTE=$O(@DATA@(CM,TY,PT,"SN",DTE)) Q:DTE=""  D
 .... I $D(@DATA@(CM,TY,PT,"DX",DTE)) Q
 .... S DXN=""
 .... F  S DXN=$O(@DATA@(CM,TY,PT,"SN",DTE,DXN)) Q:DXN=""  D
 ..... S DXCC=DXN
 ..... S VISIT=$P(@DATA@(CM,TY,PT,"SN",DTE,DXN),U,1)
 ..... I $P(HAS,U,1)=1 Q
 ..... D NFILE(CM,DTY,DXCC,DTE,VISIT,PT,ATY,@DATA@(CM,TY,PT,"SN",DTE,DXN))
 ... S DTE=""
...
NFILE(COMM,DCAT,DXC,DATE,VISIT,PT,ATYP,SDATA) ;
 ...
 ;   SDATA - Data
 ;
 NEW DIC,DA,D,NFLG,DFN
 ;  Set the community
 S ZNOW=$$NOW^XLFDT()
 S DIC="^BQI(90507.6,",X="`"_COMM,DIC(0)="FLMZ"
 D ^DIC
 S CIEN=+Y
 I CIEN=-1 S (X,DINUM)=COMM K DO,DD D FILE^DICN S CIEN=+Y
 ;  Set the Alert Type
 K DA
 S DA(1)=CIEN,X=ATYP,DIC="^BQI(90507.6,"_DA(1)_",1,",DIC(0)="FLX"
 I $G(^BQI(90507.6,DA(1),1,0))="" S ^BQI(90507.6,DA(1),1,0)="^90507.61A^^"
 D ^DIC
 S AIEN=+Y
 ;  Set the DX Category
 K DA
 S DA(2)=CIEN,DA(1)=AIEN,X=DCAT,DIC(0)="FLX"
 ...
 ;  Set the Dx Code
 K X,DA
 S DA(3)=CIEN,DA(2)=AIEN,DA(1)=DIEN,DIC(0)="FLNX",X=DXC
 ...
 S CXRF=$D(BQI(90507.6,DA(3),1,DA(2),1,DA(1),1,"C",DXC,DATE,$P(SDATA,U,2)))
 S DXRF=$D(BQI(90507.6,DA(3),1,DA(2),1,DA(1),1,"D",PT,DATE,$P(SDATA,U,2)))
 ;I '$D(BQI(90507.6,DA(3),1,DA(2),1,DA(1),1,"C",DXC,DATE,$P(SDATA,U,2))),'$D(BQI(90507.6,DA(3),1,DA(2),1,DA(1),1,"D",PT,DATE,$P(SDATA,U,2))) D
 I CXRF=0&(DXRF=0) D
 . K DD,DO D FILE^DICN
 ...
 . S BQIUPD(90507.6111,IENS,.06)=$P(SDATA,U,1)
 . D FILE^DIE("","BQIUPD","ERROR1")
 ;
 K CXRF,DXRF
 NEW DIK
 S DIK=DIC,DIK(1)=.04
...
SUP ; File Supporting Data
 ...
 S DA(4)=CIEN,DA(3)=AIEN,DA(2)=DIEN,DA(1)=RIEN,DIC(0)="FLN"
 ...
 I $D(BQIUPD) D FILE^DIE("","BQIUPD","ERROR2")
 Q
 ;
NLAB(COMM,DCAT,VISIT,PT,ATYP,LIEN,SDATA) ;
 ...
 S DFN=PT
 ;  Set the community
 S DIC="^BQI(90507.6,",X="`"_COMM,DIC(0)="FLX"
 D ^DIC
 S CIEN=+Y
 ;I CIEN=-1,COMM="Not identified" S COMM=$O(^AUTTCOM("B","UNKNOWN",""))
 I CIEN=-1 S (X,DINUM)=COMM K DO,DD D FILE^DICN S CIEN=+Y
 ;  Set the Alert Type
 K DA
 S DA(1)=CIEN,X=ATYP,DIC="^BQI(90507.6,"_DA(1)_",1,",DIC(0)="FLMN"
 I $G(^BQI(90507.6,DA(1),1,0))="" S ^BQI(90507.6,DA(1),1,0)="^90507.61A^^"
 D ^DIC
 S AIEN=+Y
 ;  Set the DX Category
 ...
 ;  Set the Lab
 K DA
 S DA(3)=CIEN,DA(2)=AIEN,DA(1)=DIEN,X=LIEN,DIC(0)="FLXN"
 ...
 D FILE^DIE("","BQIUPD","ERROR3")
 Q
 ;
SRN(TIEN,DFN) ;EP - Search through all records
 S VISIT=""
 ...
 .. I $P(^AUPNVPOV(IEN,0),U,16)'="" S PROB=1
...
SNS(BQCID,DFN) ;EP - Look by concept ID
 ...
 .. I $P(^AUPNVPOV(IEN,0),U,16)'="" S PROB=1
 .. S @DATA@(COMM,ALRT,DFN,"SN",VSDTM,BQCID)=VISIT_U_IEN_U_"9000010.07"
 Q
 ;
COMM() ;EP - Get UNKNOWN community
 ...
 Q COMM
 ;
SNO(BQCID,DFN,TMFRAME) ;EP
 S SDATE=$$DATE^BQIUL1(TMFRAME)
 S IEN=""
 F  S IEN=$O(^AUPNPROB("APCT",DFN,BQCID,IEN)) Q:IEN=""  D
 . S PDT=$$PROB^BQIUL1(IEN)
 . I PDT<SDATE,PDT'>DT Q
 . S @DATA@(COMM,ALRT,DFN,"SN",PDT,BQCID)=U_IEN_U_9000011
 Q
 ;
IPRB(TIEN,DFN) ; Search through problems
 NEW IEN,PDATA,PDT,SDATE
 S SDATE=$$DATE^BQIUL1(TMFRAME)
 S IEN=""
 F  S IEN=$O(^AUPNPROB("AC",DFN,IEN)) Q:IEN=""  D
 . S PDATA=$G(^AUPNPROB(IEN,0)) I PDATA="" Q
 . I $P(PDATA,U,1)'=TIEN Q
 . I $P(PDATA,U,12)'="E",$P(PDATA,U,12)'="A" Q
 . S PDT=$$PROB^BQIUL1(IEN)
 . I PDT<SDATE,PDT'>DT Q
 . S @DATA@(COMM,ALRT,DFN,"DX",PDT,TIEN)=U_IEN_U_9000011
 Q
