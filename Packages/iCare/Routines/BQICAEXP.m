...
EN ; Entry Point
 ...
 .... S DFN=$P(DATA,U,4),VISIT=$P(DATA,U,6),DATE=$P(DATA,U,2),LAB=$P(DATA,U,1)
 .... S VFILE=$P(DATA,U,5),RIEN=$P(DATA,U,3)
 .... I VISIT="" Q
 .... I $D(^BQI(90507.7,"AC",DFN,CAT,VISIT,DATE)) Q
 .... S GRN=$O(^BQI(90507.8,"B",$E(CAT,1,30),""))
 .... S GRP="O" I GRN'="" S GRP=$P(^BQI(90507.8,GRN,0),U,3)
 ...
 ;
OUT ;EP - Output data
 I $P(^BQI(90508,1,0),"^",5)=1 Q
 K ^BQIDATA($J)
 ; Get export format type 'D' is delimited and 'H' or blank is HL7
 S FRM=$P($G(^BQI(90508,1,0)),U,2)
 S IN=$S(FRM="D":1,1:0) I $G(EXDT)="" S EXDT=DT
 I FRM="D" S HDR=$$JDATE(EXDT)_DELIM_CT_DELIM_ASUNM,^BQIDATA($J,IN)=HDR
 S IEN=0
 ...
 . S DXN=$P(DATA,U,6),VDATE=$P(DATA,U,8),VFILE=$P(DATA,U,9),RIEN=$P(DATA,U,10)
 ...
 . S $P(RECORD,DELIM,35)=$P($G(^DPT(DFN,"MPI")),"^",1)
 ...
 . I VFILE=9000011 S $P(RECORD,DELIM,29)="P"
 . I $P(RECORD,DELIM,29)="V" S $P(RECORD,DELIM,30)=VFILE_":"_RIEN
 . ; Dxn ICD9 code
 . I DXN["." S $P(RECORD,DELIM,18)=DXN
 . I DXN'["." S $P(RECORD,DELIM,34)=DXN
 ...
 .. I LAB="" S LAB=$P($G(^AUPNVLAB(RIEN,0)),"^",1) I LAB="" Q
 ...
 .. I SITE="" D  Q
 ... S LCP=$P($G(^LAB(60,LAB,9999999)),"^",2)
 ... I LCP'="" S LOINC=LCP_"-"_$P(^LAB(95.3,LCP,0),U,15)
 ... I LCP'="" S $P(RECORD,DELIM,25)=LOINC_"^"_$P(^LAB(60,LAB,0),U,1)_"^LN="_RESULT_"^^"_$P($G(^LAB(95.3,LCP,80)),U,1)
 ... I LCP="" S $P(RECORD,DELIM,25)=LAB_"^"_$P(^LAB(60,LAB,0),U,1)_"^99"_$P(^AUTTLOC(ASUN,0),U,7)_"="_RESULT
 ... S $P(RECORD,DELIM,31)=UNITS,$P(RECORD,DELIM,32)=RLOW_"^"_RHIGH,$P(RECORD,DELIM,33)=ABN
 .. I SITE'="" D
 ... S LCP=$P($G(^LAB(60,LAB,1,SITE,95.3)),"^",1)
 ... I LCP="" S LCP=$P($G(^LAB(60,LAB,9999999)),"^",2)
 ... I LCP="" S $P(RECORD,DELIM,25)=LAB_"^"_$P(^LAB(60,LAB,0),U,1)_"^99"_$P(^AUTTLOC(ASUN,0),U,7)_"="_RESULT
 ... I LCP'="" S LOINC=LCP_"-"_$P(^LAB(95.3,LCP,0),U,15),$P(RECORD,DELIM,25)=LOINC_"^"_$P(^LAB(60,LAB,0),U,1)_"^LN="_RESULT_"^^"_$P($G(^LAB(95.3,LCP,80)),U,1)
 ... S $P(RECORD,DELIM,31)=UNITS,$P(RECORD,DELIM,32)=RLOW_"^"_RHIGH,$P(RECORD,DELIM,33)=ABN
 ...
 D LAB^BQICAVAL
 D PROC^BQICAVAL
 ; Add Opioid surveillance
 D OPO^BQICAEP3($$DATE^BQIUL1("T-30"),EXDT)
 ;
 ; If HL7
 I FRM'="D" D ^BQICAHLO
 D WRITE
...
WRITE ; use XBGSAVE to save the temp global (BQIDATA) to a file that is exported
 ...
 S XBFN=FLNM_"_"_ASUFAC_"_"_$$DATE(EXDT)_".txt"
 ;S XBFN=$S(FRM="D":"CANES_"_ASUFAC_"_"_$$DATE(EXDT)_".txt",1:"CANES_"_ASUFAC_"HL7"_"_"_$$DATE(EXDT)_".txt")
