...
SEG ; For each record, create the segments
 ...
 D SET^HLOAPI(.SEG,$P(RECORD,DELIM,29),51)
 S X=$$ADDSEG^HLOAPI(.HLMSTATE,.SEG)
 ;DG1
 D SET^HLOAPI(.SEG,"DG1",0)
 D SET^HLOAPI(.SEG,1,1)
 NEW CODE,SYS,CSY,SNO
 S CODE=$P(RECORD,DELIM,18),SNO=$P(RECORD,DELIM,34)
 I CODE'="",SNO="" D
 ...
 I SNO'="" S CSY="SNM" D SET^HLOAPI(.SEG,$P(RECORD,DELIM,34),3,1)
 I $G(CSY)'="" D SET^HLOAPI(.SEG,CSY,3,3)
 D SET^HLOAPI(.SEG,$P(RECORD,DELIM,19),3,5)
 D SET^HLOAPI(.SEG,$P(RECORD,DELIM,20),3,6)
 D SET^HLOAPI(.SEG,$P(RECORD,DELIM,21),3,4)
 I $P(RECORD,DELIM,29)="V" D
 . NEW VALUE,FILE,RIEN,PRI
 . S VALUE=$P(RECORD,DELIM,30),PRI=0
 . S FILE=$P(VALUE,":",1),RIEN=$P(VALUE,":",2)
 . I FILE'=9000010.07 Q
 . ;
 . S RVIS=$P($G(^AUPNVPOV(RIEN,0)),"^",3) I RVIS="" Q
 . S PVN="" F  S PVN=$O(^AUPNVPOV("AD",RVIS,PVN)) Q:PVN=""  D
 .. S PRI=PRI+1
 .. I PVN'=RIEN Q
 .. I $P(^AUPNVPOV(PVN,0),"^",12)="P" D SET^HLOAPI(.SEG,1,15) Q
 .. E  D SET^HLOAPI(.SEG,PRI,15)
 S X=$$ADDSEG^HLOAPI(.HLMSTATE,.SEG)
 I $P(RECORD,DELIM,29)="V" S INC=1 D
 . NEW VALUE,FILE,RIEN,RVIS,PVN,PRI
 . S VALUE=$P(RECORD,DELIM,30)
 . S FILE=$P(VALUE,":",1),RIEN=$P(VALUE,":",2)
 . I FILE=9002011.65 Q
 . I FILE=""!(RIEN="") Q
 . I FILE=9000010.09 S RVIS=$P($G(^AUPNVLAB(RIEN,0)),"^",3) I RVIS="" Q
 . I FILE=9000010.25 S RVIS=$P($G(^AUPNVMIC(RIEN,0)),"^",3) I RVIS="" Q
 . I FILE=9000010.07 S RVIS=$P($G(^AUPNVPOV(RIEN,0)),"^",3) I RVIS="" Q
 . I FILE=9000010 S RVIS=RIEN I RVIS="" Q
 . S PVN="",PRI=0 F  S PVN=$O(^AUPNVPOV("AD",RVIS,PVN)) Q:PVN=""  D
 .. S PRI=PRI+1
 .. I PVN=RIEN Q
 .. S INC=INC+1 I INC>10 Q
 .. S CODE=$$GET1^DIQ(9000010.07,PVN_",",.01,"E"),CSY="I10"
 .. D SET^HLOAPI(.SEG,"DG1",0)
 .. D SET^HLOAPI(.SEG,INC,1)
 .. D SET^HLOAPI(.SEG,CODE,3,1)
 .. D SET^HLOAPI(.SEG,CSY,3,3)
 .. I $P(^AUPNVPOV(PVN,0),"^",12)="P" D SET^HLOAPI(.SEG,1,15)
 .. E  D SET^HLOAPI(.SEG,PRI,15)
 ...
 . D SET^HLOAPI(.SEG,RESULT,5)
 . NEW FILE,RIEN,VALUE,FLAG
 . S VALUE=$P(RECORD,DELIM,30)
 . S FILE=$P(VALUE,":",1),RIEN=$P(VALUE,":",2)
 . I FILE=9000010.09 D
 .. S FLAG=$P($G(^AUPNVLAB(RIEN,11)),"^",9)
 .. D SET^HLOAPI(.SEG,FLAG,11)
 . I FILE=9000010.25 D
 .. S FLAG=$P($G(^AUPNVMIC(RIEN,11)),"^",9)
 .. D SET^HLOAPI(.SEG,FLAG,11)
 . I FILE'=9000010.09!(FILE'=9000010.25) D SET^HLOAPI(.SEG,"R",11)
 ...
 ... S X=$$ADDSEG^HLOAPI(.HLMSTATE,.SEG,.ERR)
 . I FILE=9000010.25 D
 .. F I=1:1:3 S COMM=$P($G(^AUPNVMIC(RIEN,13)),"^",I) I COMM'="" D
 ... D SET^HLOAPI(.SEG,"NTE",0)
 ... D SET^HLOAPI(.SEG,I,1)
 ... D SET^HLOAPI(.SEG,COMM,3)
 ... S X=$$ADDSEG^HLOAPI(.HLMSTATE,.SEG,.ERR)
 ...
 D SET^HLOAPI(.SEG,$P(RECORD,DELIM,35),2)
 S X=$$ADDSEG^HLOAPI(.HLMSTATE,.SEG)
 ;ZLI
