...
CON(DATA,FAKE) ;EP -- BQI GET CONSULT TEXT
 ...
 G DONE
 ;
ORD(DATA,FAKE) ;EP -- BQI GET ORDER TEXT
 NEW UID,II,HDR,VALUE,RANGE
 S UID=$S($G(ZTSK):"Z"_ZTSK,1:$J)
 S DATA=$NA(^TMP("BQISYORP",UID))
 K @DATA
 ;
 S II=0
 NEW $ESTACK,$ETRAP S $ETRAP="D ERR^BQISYPNL D UNWIND^%ZTER" ; SAC 2006 2.2.3.3.2
 ;
 S HDR="T00060ORD_TEXT"
 S @DATA@(II)=HDR_$C(30)
 S VALUE=$P($G(^BQI(90508,1,16)),"^",6) I VALUE="" S VALUE="T-1M"
 S RANGE=$$FMTE^XLFDT($$DATE^BQIUL1(VALUE))_" - "_$$FMTE^XLFDT(DT,1)
 S II=II+1,@DATA@(II)="Orders for the dates of "_RANGE_$C(30)
 G DONE
