BQIRGPL ;GDIT/HS/ALA-Registers and Panels ; 01 Nov 2007  1:00 PM
 ;;2.9;ICARE MANAGEMENT SYSTEM;**3,5**;Mar 01, 2021;Build 20
 ;
 Q
 ;
EN(DATA,OWNR,PLIEN,CARE,PLIST) ;EP -- BQI GET REG DATA BY PANEL
 ...
 ;
 NEW UID,II,X,BQIRM,VAL,HIEN,E,J,K,L,MAX,MIN,NAFLG,BN,LIST,BQI,DFN,QFL,TQFL
 ...
 ;
 ;I $G(CARE)="STI/STD" D EN^BQISTPNL(.DATA,OWNR,PLIEN,$G(PLIST)) Q
 ;
 ...
 I $O(^BQICARE(OWNR,1,PLIEN,40,DFN))="" D PAT(.DATA,OWNR,PLIEN,CARE,"") G DONE
DONE ;
 I II=0,'$D(@DATA) D PAT(.DATA,OWNR,PLIEN,CARE,"")
 S II=II+1,@DATA@(II)=$C(31)
 Q
 ;
PAT(DATA,OWNR,PLIEN,CARE,DFN) ;EP - Build record by patient
 ; Get standard display
 NEW PCIEN,HDR,VALUE,HEADR,SENS,HDOB,Y,STVW,TEXT,ORD,GMET,GHDR,RGIEN,CRIEN,CTYP
 NEW GIEN,CIEN,ATAGN,ATAGNM,ATAGST,CRN,TYP,DIS,BTREF,CODE,CT,GREF,IEN,IFL,ITIEN
 NEW RES,RESULT,STOP,STVCD,TIEN,TREF,VISIT,VSDTM
 S VALUE="",RGIEN=""
 ;B:DFN=""
 ...
 S HEADR=BHEADR_"I00010HIDE_ASSOC_TAG_IEN^T00030HIDE_ASSOC_TAG_NAME^T00001HIDE_ASSOC_TAG_STATUS^"
 ;
 I CARE'?.N S CRN=$O(^BQI(90506.5,"B",CARE,"")) I CRN="" Q
 I CRN'="" S TYP=$P(^BQI(90506.5,CRN,0),U,2),CTYP=TYP
 ...
 S VALUE=BVALUE_ATAGN_U_ATAGNM_U_ATAGST_U
 ;I DFN="" S HEADR=$$TKO^BQIUL1(HEADR,"^"),@DATA@(II)=HEADR_$C(30) Q
 ; Find IEN
 ;S PCIEN=""
 ;F  S PCIEN=$O(^BQI(90506.1,"AC",CTYP,PCIEN)) Q:PCIEN=""  D
 ;. I $P(^BQI(90506.1,PCIEN,0),U,6)'=".001" Q
 ;. S HEADR=HEADR_$P(^BQI(90506.1,PCIEN,0),U,8)_U
 ;. I DFN="" Q
 ;. S STVW=PCIEN
...
DEF ; Default list of fields
 NEW CRIEN,TYP,ORD,PCIEN,STVW,KEY,CODE
 ...
 . S PCIEN=""
 . F  S PCIEN=$O(^BQI(90506.1,"AF",TYP,ORD,PCIEN)) Q:PCIEN=""  D
 .. I $$GET1^DIQ(90506.1,PCIEN_",",.1,"I")=1 Q
 .. S KEY=$$GET1^DIQ(90506.1,PCIEN_",",3.1,"E")
 .. I KEY'="",'$$KEYCHK^BQIULSC(KEY,DUZ) Q
 .. I $$GET1^DIQ(90506.1,PCIEN_",",3.04,"I")'="O" D
 ... S CODE=$P(^BQI(90506.1,PCIEN,0),U,1) Q:CODE=""
 ... I $P($G(^BQI(90506.1,PCIEN,0)),U,10)=1 Q
 ... S STVW=PCIEN
 ... D CVAL
 ... S VALUE=VALUE_VAL_"^"
 ... S HEADR=HEADR_HDR_"^"
 ...
 . S PCIEN=""
 . F  S PCIEN=$O(^BQI(90506.1,"AD",TYP,ORD,PCIEN)) Q:PCIEN=""  D
 .. I $$GET1^DIQ(90506.1,PCIEN_",",.1,"I")=1 Q
 .. S KEY=$$GET1^DIQ(90506.1,PCIEN_",",3.1,"E")
 .. I KEY'="",'$$KEYCHK^BQIULSC(KEY,DUZ) Q
 .. I $$GET1^DIQ(90506.1,PCIEN_",",3.04,"I")'="O" D
...
CKO ; Check other
 ...
 . ;Get Value
 . S VAL="",RES=""
 . I DFN'="" S RES=$$FND^BQICMUTL(CRN,GIEN,DFN)
 ...
 . I DIS="B" D
 .. S HDR="T00025"_STVCD
 .. S VAL=$S(BQDTFRM="S":$$FMTE^BQIUL1($P(RES,U,2)),1:$$FMTMDY^BQIUL1($P(RES,U,2)))_$S($P(RES,U,3)'="":" ("_$P(RES,U,3)_")",1:"")
 ...
 . ;Get Value
 . S VAL="",RES=""
 . I DFN'="" S RES=$$FTAG^BQICMUTL(ATAGN,GIEN,DFN)
