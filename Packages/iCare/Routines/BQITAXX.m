...
SNOM ;EP - Get SNOMED subsets
 ...
 Q
 ;
RXN ;EP - Get RXNORM subsets
 S BQILIST=$NA(^TMP("BQIRXLST",$J)) K @BQILIST
 D SUBSET^BSTSAPIA(BQILIST,"1552^1")
 S BSN=0
 F  S BSN=$O(@BQILIST@(BSN)) Q:BSN=""  D
 . S II=II+1
 . S @DATA@(II)=@BQILIST@(BSN)_"^"_@BQILIST@(BSN)_";BSTS^RXNORM Subset^NO^RXNORM Subset^YES^^NO"_$C(30)
 K @BQILIST
 Q
 ;
