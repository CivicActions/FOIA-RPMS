BQIRMZDR ;PRXM/HC/ALA-Find all reminder values ; 15 Mar 2007  2:30 PM
 ;;2.9;ICARE MANAGEMENT SYSTEM;**1,7**;Mar 01, 2021;Build 14
 ;
PAT(BQRDFN,REF,BQIIJB) ;EP
 ...
 NEW DA,DIK
 S DA(1)=BQRDFN,DA=0,DIK="^BQIPAT("_DA(1)_",40,"
 F  S DA=$O(^BQIPAT(BQRDFN,40,DA)) Q:'DA  D ^DIK
 ;
 S REF=$G(REF,0)
 ;
 S APCHSPAT=BQRDFN
 ...
 I $G(^BQIPAT(BQRDFN,0))="" D
 . NEW DIC,X,DINUM,DLAYGO
 . S (X,DINUM)=BQRDFN,DLAYGO=90507.5,DIC="^BQIPAT(",DIC(0)="FL"
 . K DO,DD D FILE^DICN
 ;
 S RIEN=""
 ...
 . S HIEN=$P(CODE,"_",2)
 . ;
 . I $P(CODE,"_",1)="REG" D REG^BQIRMDR1(BQRDFN,CODE) Q
 . ;
 . I $P(CODE,"_",1)="EHR" D EMR^BQIRMDR1(BQRDFN,CODE) Q
 . ;
 . I $P(CODE,"_",1)="CMET" D CMT^BQIRMDR1(BQRDFN,CODE) Q
 . ;
 ...
 . I CODE["AUTTIMM" Q
 . I $O(^BQIPAT(BQRDFN,40,"B",CODE,""))'="" Q
 . I $E(CODE,1,3)="REG" Q
 . I $E(CODE,1,4)="CMET" Q
 . S (REMDUE,REMLAST,REMNEXT)=""
 . D FIL
 ;
IMRM ;EP - Immunization reminders
 S SUB=$S(BQIIJB="N":"BQIRMFORN",1:"BQIRMFORW")
 I '$D(^XTMP(SUB,BQRDFN)) D IRPT^BQIRMIZ(BQRDFN,BQIIJB)
 D IZ^BQIRMDR2(BQRDFN,BQIIJB)
 Q
 ;
RMR(BQRDFN,HIEN) ;EP
 ...
 .. I IMOK D
 ... S VALUE=$$IMM^BQIREM(BQRDFN,IMNIEN)
 ...
 .. I 'IMOK D
 ... S X=REMNME_U_U_$$IMM^BQIREM(BQRDFN,IMNIEN)
 ... D PRS(X,HIEN)
...
FIL ;EP - File the reminder
 ...
 S X=CODE,DLAYGO=90507.54,DA(1)=BQRDFN,DIC("P")=DLAYGO
 I $G(^BQIPAT(DA(1),40,0))="" S ^BQIPAT(DA(1),40,0)="^90507.54P^^"
 S DIC="^BQIPAT("_DA(1)_",40,",DIC(0)="FLX"
 S DA=$O(^BQIPAT(DA(1),40,"B",CODE,""))
 I DA="" K DO,DD D FILE^DICN S DA=+Y I DA=-1 Q
 ...
 S BQIUPD(90507.5,BQRDFN_",",.08)=$$NOW^XLFDT()
...
FILE ;File record
 NEW DA,X,DIC,DLAYGO
 S DIC="^BQI(90506.1,",DIC(0)="FL",X=CODE
 K DO,DD D FILE^DICN
 ...
 . S BQIUPD(90506.1,DA_",",1)="S VAL=$$REM^BQIULPT(BQRDFN,STVW)"
 . I CODE["AUTTIMM" D
 .. I 'IMOK S BQIUPD(90506.1,DA_",",3.07)=1 Q
 .. I IMOK S BQIUPD(90506.1,DA_",",3.07)="@"
 I SOURCE="Performance" S BQIUPD(90506.1,DA_",",1)="S VAL=$$PER^BQIULPT(BQRDFN,STVW)"
 D FILE^DIE("","BQIUPD","ERROR")
 ;
