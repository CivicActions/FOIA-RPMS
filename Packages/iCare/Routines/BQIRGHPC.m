...
DSP(ASDFN,CODE) ; EP - Display Care Mgmt field
 ...
 ;
FIB4(BQDFN) ;EP
 NEW TEMP,COMP,AGE,VAL
 S TEMP="BQLABS" K @TEMP
 D AST(BQDFN)
 D PLT(BQDFN)
 D ALT(BQDFN)
 D FFND
 ;B
 I 'COMP Q ""
 S AGE=$$AGE^BQIAGE(BQDFN,FDAT)
 S VAL=$$FIB4C(AGE,ASTV,PLTV,ALTV)
 K ASTV,PLTV,ALTV
 Q VAL
AST(BQDFN) ;
 NEW VALUE,ITM,TREF,START,END,TYP,TAX
 S VALUE="",ITM="",TREF=""
 S START=$$DATE^BQIUL1("T-24M"),END=DT,TYP="AST"
 S TAX="DM AUDIT AST TAX",FREF=9000010.09,RREF=60
 D LABS^BQIRGUT1(BQDFN,TAX,TREF,START,END,TYP,.TEMP)
 Q
PLT(BQDFN) ;
 NEW VALUE,ITM,TREF,START,END,TYP,TAX
 S VALUE="",ITM="",TREF=""
 S START=$$DATE^BQIUL1("T-24M"),END=DT,TYP="PLT"
 S TAX="BQI PLATELET TAX",FREF=9000010.09,RREF=60
 D LABS^BQIRGUT1(BQDFN,TAX,TREF,START,END,TYP,.TEMP)
 Q
ALT(BQDFN) ;
 NEW VALUE,ITM,TREF,START,END,TYP,TAX
 S VALUE="",ITM="",TREF=""
 S START=$$DATE^BQIUL1("T-24M"),END=DT,TYP="ALT"
 S TAX="DM AUDIT ALT TAX",FREF=9000010.09,RREF=60
 D LABS^BQIRGUT1(BQDFN,TAX,TREF,START,END,TYP,.TEMP)
 Q
 ;
FIB4C(AGE,AST,PLT,ALT) ;EP - Calculate FIB-4 score
 ; AGE - Age of patient
 ; AST - AST level
 ; PLT - Platelet count
 ; ALT - ALT level
 ;
 ; Top - Age x AST divided by Bottom - PLT x Squareroot of ALT
 NEW TOP,BOT,SQR
 S TOP=AGE*AST,BOT=PLT*($$SQR(ALT))
 S FIB=TOP/BOT
 Q $J(FIB,1,2)
 ;
SQR(X) ;EP - squareroot
 ...
 Q $G(Y)
 ;
DOO(DFN) ;EP - First Hep C DX
 NEW DATE,IEN,VAL,TYP,HDATA,HDA1,HDA2
 S DATE="",DATE=$O(^XTMP("BQIHEPC",DFN,"D",""))
 I DATE="" Q ""
 S IEN=$O(^XTMP("BQIHEPC",DFN,"D",DATE,"")),TYP=^XTMP("BQIHEPC",DFN,"D",DATE,IEN)
 S HDA1=$S(TYP="PROB":$P(^AUPNPROB(IEN,0),"^",1),TYP="POV":$P(^AUPNVPOV(IEN,0),"^",1),1:"")
 S HDA2=$S(TYP="PROBS":$P($G(^AUPNPROB(IEN,800)),"^",1),TYP="POVS":$P($G(^AUPNVPOV(IEN,11)),"^",1),1:"")
 I HDA1'="" S HDATA=$$ICD9^BQIUL3(HDA1,,4)
 I HDA2'="" S HDATA=$P($$CONC^BSTSAPI(HDA2_"^36"),"^",4)
 Q $$FMTMDY^BQIUL1(DATE)_" ("_HDATA_")^"_DATE
 ;
EPRG(DFN) ;EP - Pregnant after Hep C DX
 NEW DOO,PGD
 I $P($G(^DPT(DFN,0)),"^",2)'="F" Q ""
 S DOO=$O(^XTMP("BQIHEPC",DFN,"D","")) I DOO="" Q ""
 S PGD=$O(^XTMP("BQIHEPC",DFN,"P",DOO))
 I PGD="" Q "NO^"_PGD
 I PGD'<DOO Q "YES ("_$$FMTMDY^BQIUL1(PGD)_")^"_PGD
 I PGD<DOO Q "NO^"_PGD
 Q ""
 ;
FFND ; EP
 NEW ALCT,ASCT,PLCT,LIEN
 S FDAT="",ALCT=0,ASCT=0,PLCT=0,COMP=0
 F  S FDAT=$O(@TEMP@(FDAT),-1) Q:FDAT=""  D  Q:COMP
 . I $D(@TEMP@(FDAT,"ALT")) D
 .. S ALCT=1,LIEN=$O(@TEMP@(FDAT,"ALT","")),ALTV=@TEMP@(FDAT,"ALT",LIEN),ALDAT=FDAT
 .. I $E(ALTV,1,1)="<"!($E(ALTV,1,1)=">") S ALTV=$E(ALTV,2,$L(ALTV))
 . I $D(@TEMP@(FDAT,"AST")) D
 .. S ASCT=1,LIEN=$O(@TEMP@(FDAT,"AST","")),ASTV=@TEMP@(FDAT,"AST",LIEN),ASDAT=FDAT
 . I $D(@TEMP@(FDAT,"PLT")) D
 .. S PLCT=1,LIEN=$O(@TEMP@(FDAT,"PLT","")),PLTV=@TEMP@(FDAT,"PLT",LIEN),PLDAT=FDAT
 . I ALCT,ASCT,PLCT S COMP=1
 ;
 ; If not complete and only PLT is missing
 I 'COMP D
 . NEW PTMP,START,END,VALUE,ITM,TREF,TYP,TAX,FREF,RREF,FDAT,LIEN
 . S PTMP="BQPLT" K @PTMP
 . I $G(PLTV)="",$G(ALDAT)'="",$G(ASDAT)'="" D
 .. I ALDAT=ASDAT S FDAT=ALDAT
 .. I ALDAT<ASDAT S FDAT=ALDAT
 .. I ALDAT>ASDAT S FDAT=ALDAT
 .. S START=$$FMADD^XLFDT(FDAT,-90)
 .. S END=$$FMADD^XLFDT(FDAT,90)
 .. S VALUE="",ITM="",TREF="",TYP="PLT"
 .. S TAX="BQI PLATELET TAX",FREF=9000010.09,RREF=60
 .. D LABS^BQIRGUT1(BQDFN,TAX,TREF,START,END,TYP,.PTMP)
 .. S FDAT="" F  S FDAT=$O(@PTMP@(FDAT)) Q:FDAT=""  D
 ... I $D(@PTMP@(FDAT,"PLT")) D
 .... S PLCT=1,LIEN=$O(@PTMP@(FDAT,"PLT","")),PLTV=@PTMP@(FDAT,"PLT",LIEN),PLDAT=FDAT
 . I ALCT,ASCT,PLCT S COMP=1
 ;
 ; If not complete, only AST is missing
 I 'COMP D
 . NEW PTMP,START,END,VALUE,ITM,TREF,TYP,TAX,FREF,RREF,FDAT,LIEN
 . S PTMP="BQAST" K @PTMP
 . I $G(PLTV)'="",$G(ALDAT)'="",$G(ASDAT)="" D
 .. I ALDAT=PLDAT S FDAT=ALDAT
 .. I ALDAT<PLDAT S FDAT=ALDAT
 .. I ALDAT>PLDAT S FDAT=ALDAT
 .. S START=$$FMADD^XLFDT(FDAT,-90)
 .. S END=$$FMADD^XLFDT(FDAT,90)
 .. S VALUE="",ITM="",TREF="",TYP="AST"
 .. S TAX="DM AUDIT AST TAX",FREF=9000010.09,RREF=60
 .. D LABS^BQIRGUT1(BQDFN,TAX,TREF,START,END,TYP,.PTMP)
 .. S FDAT="" F  S FDAT=$O(@PTMP@(FDAT)) Q:FDAT=""  D
 ... I $D(@PTMP@(FDAT,"AST")) D
 .... S ASCT=1,LIEN=$O(@PTMP@(FDAT,"AST","")),ASTV=@PTMP@(FDAT,"AST",LIEN),ASDAT=FDAT
 . I ALCT,ASCT,PLCT S COMP=1
 ;
 ; If not complete, only ALT is missing
 I 'COMP D
 . NEW PTMP,START,END,VALUE,ITM,TREF,TYP,TAX,FREF,RREF,FDAT,LIEN
 . S PTMP="BQALT" K @PTMP
 . I $G(PLTV)'="",$G(ALDAT)="",$G(ASDAT)'="" D
 .. I PLDAT=ASDAT S FDAT=ASDAT
 .. I PLDAT<ASDAT S FDAT=ASDAT
 .. I PLDAT>ASDAT S FDAT=ASDAT
 .. S START=$$FMADD^XLFDT(FDAT,-90)
 .. S END=$$FMADD^XLFDT(FDAT,90)
 .. S VALUE="",ITM="",TREF="",TYP="ALT"
 .. S TAX="DM AUDIT ALT TAX",FREF=9000010.09,RREF=60
 .. D LABS^BQIRGUT1(BQDFN,TAX,TREF,START,END,TYP,.PTMP)
 .. S FDAT="" F  S FDAT=$O(@PTMP@(FDAT)) Q:FDAT=""  D
 ... I $D(@PTMP@(FDAT,"ALT")) D
 .... S ALCT=1,LIEN=$O(@PTMP@(FDAT,"ALT","")),ALTV=@PTMP@(FDAT,"ALT",LIEN),ALDAT=FDAT
 .... I $E(ALTV,1,1)="<"!($E(ALTV,1,1)=">") S ALTV=$E(ALTV,2,$L(ALTV))
 . I ALCT,ASCT,PLCT S COMP=1
 Q
