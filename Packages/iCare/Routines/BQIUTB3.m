...
EN(DATA,TAXTY) ;EP -- BQI GET TAXONOMIES
 ...
 I TAXTY'="" S FILE=$S(TAXTY="DX":80,TAXTY="M":50,TAXTY="CP":81,TAXTY="PR":80.1,TAXTY="LB":60,TAXTY="ED":9999999.09,TAXTY="HF":9999999.64,TAXTY="IZ":9999999.14,1:"")
 I TAXTY'="LB" D
 . S TN=0
 . F  S TN=$O(^ATXAX("AD",FILE,TN)) Q:'TN  D
...
LAB(DATA) ;EP - Lab Tests
 ...
 K @DATA
 S II=0,LN=0
 S @DATA@(II)="T00010IEN^T00060ITEM^T00030CAT1^I00010SORTORD"_$C(30)
 F  S LN=$O(^AUPNVLAB("B",LN)) Q:LN=""  D
 . I $G(^LAB(60,LN,0))="" Q
 . S NAM=$P(^LAB(60,LN,0),"^",1)
 . S LVAL(1,NAM)=LN
 ;
 ...
 .. I TY="[" S LNAME="*"_LNAME
 .. S ORD=ORD+1,LNAME=LNAME_" - L"
 .. S II=II+1,@DATA@(II)="LAB_"_LN_U_LNAME_U_"NON-MICROBIOLOGY^"_ORD_$C(30)
 K LVAL
 ;
 S LN=0 F  S LN=$O(^AUPNVMIC("B",LN)) Q:LN=""  D
 . I $G(^LAB(60,LN,0))="" Q
 . S NAM=$P(^LAB(60,LN,0),"^",1)
 . S LVAL(1,NAM)=LN
 ;
 S TY="" F  S TY=$O(LVAL(TY)) Q:TY=""  D
 . S LNM="" F  S LNM=$O(LVAL(TY,LNM)) Q:LNM=""  D
 .. S LN=LVAL(TY,LNM),LNAME=LNM
 .. I TY="[" S LNAME="*"_LNAME
 .. S ORD=ORD+1,LNAME=LNAME_" - M"
 .. S II=II+1,@DATA@(II)="LABM_"_LN_U_LNAME_U_"MICROBIOLOGY^"_ORD_$C(30)
 S II=II+1,@DATA@(II)=$C(31)
 K LVAL,SOR,TY,BI,II,LNAME,LNM,INAC,ORD,NAM
 Q
 ;
LABR(DATA) ;EP - Lab Tests and results
 ...
 K @DATA
 S II=0,LN=0
 S @DATA@(II)="T00010IEN^T00060ITEM^T00030CAT1^T00010TYPE^T00030VALUE^I00010SORTORD"_$C(30)
 F  S LN=$O(^AUPNVLAB("B",LN)) Q:LN=""  D
 . I $G(^LAB(60,LN,0))="" Q
 . S NAM=$P(^LAB(60,LN,0),"^",1)
 . S LVAL(1,NAM)=LN
 ...
 .. I $G(TYPE)="" S TYPE="FREE TEXT",LTYP="F"
 .. S LNAME=LNAME_$S(TYPE="":"",1:" ("_LTYP_") - L")
 .. S II=II+1,@DATA@(II)="LAB_"_LN_U_LNAME_U_"NON-MICROBIOLOGY"_U_TYPE_U_VALUE_U_ORD_$C(30)
 K LVAL
 ;
 S LN=0 F  S LN=$O(^AUPNVMIC("B",LN)) Q:LN=""  D
 . I $G(^LAB(60,LN,0))="" Q
 ...
 .. I $O(^LAB(60,LN,2,0))'="" S LTYP="P",TYPE="PANEL"
 .. I $G(TYPE)="" S TYPE="FREE TEXT",LTYP="F"
 .. S LNAME=LNAME_$S(TYPE="":"",1:" ("_LTYP_") - M")
 .. S II=II+1,@DATA@(II)="LABM_"_LN_U_LNAME_U_"MICROBIOLOGY"_U_TYPE_U_VALUE_U_ORD_$C(30)
 S II=II+1,@DATA@(II)=$C(31)
 K LVAL,SOR,TY,BI,II,LNAME,LNM,INAC,ORD,NAM
 Q
 ;
...
CPT(DATA) ; EP - CPT
 ...
 ;
EXM(DATA) ;EP - Exams
 NEW MN,COD,RLIST,N,TY,INAC,TYP,VALUE,ORD
 K @DATA,LVAL
 S II=0,MN=""
 S @DATA@(II)="I00010IEN^T00030^T00010TYPE^T00030VALUE^I00010SORTORD"_$C(30)
 F  S MN=$O(^AUPNVXAM("B",MN)) Q:MN=""  D
 . I $G(^AUTTEXAM(MN,0))="" Q
 . S COD=+$P(^AUTTEXAM(MN,0),"^",2)
 . K RLIST
 . D VXAMR^AUPNCIX(MN,"RLIST")
 . S N="",VALUE="" F  S N=$O(RLIST(N)) Q:N=""  S VALUE=VALUE_$TR(RLIST(N),"^",":")_";"
 . S TY=$S(VALUE'="":"SET",1:"FREE TEXT"),INAC=$S($P(^AUTTEXAM(MN,0),"^",4)=1:"*",1:"")
 . S NAM=$P(^AUTTEXAM(MN,0),"^",1)
 . I INAC="*" S LVAL("[",NAM)=MN_"^"_TY_"^"_VALUE Q
 . S LVAL(1,NAM)=MN_"^"_TY_"^"_VALUE
 ;
 S TYP="",ORD=0 F  S TYP=$O(LVAL(TYP)) Q:TYP=""  D
 . S LNM="" F  S LNM=$O(LVAL(TYP,LNM)) Q:LNM=""  D
 .. S LNAME=LNM
 .. I TYP="[" S LNAME="*"_LNAME
 .. S ORD=ORD+1
 .. S MN=$P(LVAL(TYP,LNM),"^",1),TY=$P(LVAL(TYP,LNM),"^",2),VALUE=$P(LVAL(TYP,LNM),"^",3)
 .. S II=II+1,@DATA@(II)=MN_U_LNAME_" ("_$E(TY,1)_")"_"^"_TY_"^"_VALUE_"^"_ORD_$C(30)
 S II=II+1,@DATA@(II)=$C(31)
 K LVAL
 Q
 ;
...
POVS(DATA) ;EP - Snomed IDs for POV
 ...
 . S SN=$O(^BSTS(9002318.4,"C",36,DN,"")) I SN="" Q
...
PROB(DATA) ; EP - Problems
 ...
 S DN=0 F  S DN=$O(^XTMP("BQIPBICD",DN)) Q:DN=""  D
 . I $D(^AUPNPROB("B",DN)) Q
 . I $G(^ICD9(DN,0))="" Q
 . S II=II+1,@DATA@(II)=DN_U_$$VST^ICDCODE(DN,"",80)_" ["_$$CODEC^ICDCODE(DN,80)_"]"_$C(30) Q
 S II=II+1,@DATA@(II)=$C(31)
...
DIV(DATA) ;EP - Divisions
 ...
 . I $G(^DIC(4,IEN,0))="" Q
 . I $G(^XTMP("BQIDIV","I",IEN)) Q
 . S NAME=NM_" ("_$G(^XTMP("BQIDIV","A",IEN))_")"_"-"_$P(^AUTTLOC(IEN,0),"^",7)
 . S II=II+1,@DATA@(II)=IEN_U_NAME_$C(30)
 S NM=0 F  S NM=$O(^XTMP("BQISYDIV",NM)) Q:NM=""  D
 . S IEN=^XTMP("BQISYDIV",NM)
 . I $G(^DIC(4,IEN,0))="" Q
 . I $G(^XTMP("BQIDIV","A",IEN)) Q
 . S NAME="*"_NM_" ("_$G(^XTMP("BQIDIV","I",IEN))_")"_"-"_$P(^AUTTLOC(IEN,0),"^",7)
 . S II=II+1,@DATA@(II)=IEN_U_NAME_$C(30)
 S II=II+1,@DATA@(II)=$C(31)
 Q
 ;
