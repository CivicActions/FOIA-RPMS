BQIMTCRD ;GDIT/HS/ALA-Get Definition Detail Data ; 04 Mar 2013  9:44 AM
 ;;2.9;ICARE MANAGEMENT SYSTEM;**1,3,4,5,7**;Mar 01, 2021;Build 14
 ;
 Q
 ;
...
DONE ;
 ...
 . D THDR^BQIHEADR(OWNR,PLIEN,DFN,.BHEADR,.BVALUE)
...
PAT(DATA,OWNR,PLIEN,CARE,DFN) ;EP - Build record by patient
 ...
 . I MTYP="LAB" D  Q
 .. F LTYP="LAB","LABM" D
 ... I $O(^BQICARE(OWNR,1,PLIEN,40,DFN,5,"B",LTYP,""))'="" D
 .... S MC=$O(^BQICARE(OWNR,1,PLIEN,40,DFN,5,"B",LTYP,"")) I MC="" Q
 .... S RIEN="",NFLG=0
 .... F  S RIEN=$O(^BQICARE(OWNR,1,PLIEN,40,DFN,5,MC,1,"B",RIEN)) Q:RIEN=""  D STND
 . I MTYP="HRCD"&($O(^BQICARE(OWNR,1,PLIEN,40,DFN,5,"B",MTYP,""))="") S MTYP="HRIMNO"
...
STND ; Get standard display
 ...
 NEW GIEN,CIEN,ATAGN,ATAGNM,ATAGST,TYP,DIS,REMDESC,LINK
 S VALUE="",RGIEN=""
 ...
 ;I CTYP="CD" S HEADR=HEADR_"T00010HIDE_RNOTE^"
 ;
...
CKO ; Check other
 ...
 Q
 ;
SYM(RIEN,RESULT) ;EP - Signs and symptoms
 NEW GN
 S RESULT=""
 S GN=0
...
LAC(DFN) ;last time no active problems for patient
 ...
 Q
 ;
CUS ; Customized view for owner/user
 S IEN=0
 ...
 Q
 ;
CSS ; Customized view for owner/shared user
 S IEN=0
 ...
 Q
 ;
TMS ;Look for default template for shared user
 S DA="",TQFL=0 F  S DA=$O(^BQICARE(OWNR,1,PLIEN,30,DUZ,4,"C",CTYP,DA)) Q:DA=""  D  Q:TQFL
 . S DA(3)=OWNR,DA(2)=PLIEN,DA(1)=DUZ,IENS=$$IENS^DILF(.DA)
 . S TEMPL=$$GET1^DIQ(90505.34,IENS,.01,"E")
 . S LYIEN=$$TPN^BQILYUTL(DUZ,TEMPL) I $P(^BQICARE(DUZ,15,LYIEN,0),"^",3) S TQFL=1
 Q
 ;
TMO ;Look for default template for owner user
 S DA="",TQFL=0 F  S DA=$O(^BQICARE(OWNR,1,PLIEN,4,"C",CTYP,DA)) Q:DA=""  D  Q:TQFL
 . S DA(2)=OWNR,DA(1)=PLIEN,IENS=$$IENS^DILF(.DA)
 . S TEMPL=$$GET1^DIQ(90505.14,IENS,.01,"E")
 . S LYIEN=$$TPN^BQILYUTL(DUZ,TEMPL) I $P(^BQICARE(DUZ,15,LYIEN,0),"^",3) S TQFL=1
 Q
 ;
TSD ; Template display
 S DOR=""
