PSBRPC ;BIRMINGHAM/EFC - BCMA RPC BROKER CALLS ;01-Mar-2023 14:14
 ;;3.0;BAR CODE MED ADMIN;**6,3,4,1005,1006,13,32,28,1010,42,1015,1033**;Mar 2004;Build 34
 ...
 ;                        - 12/29/22 - Ref to Preferred Name
 ;                          01/03/23 - Ref to DISCHARGE
 ;            IHS/MSC/MIR - 03/01/23 - Lines NWLIST+15:+27
...
SCANPT(RESULTS,PSBDATA) ; Lookup Pt by Full SSN
 ...
 ;2019.6.3 JTC ? COMMENTED OUT TO REMOVE CHECK FOR DISCHARGE
 ...
 ;IHS/MSC/PLS - 12/29/2022 - set patient name to results of AUPNSOGI call
 S RESULTS(2)=$$GETPREF^AUPNSOGI(DFN,"E",1)
...
NWLIST(RESULTS,DUMMY) ; ward list - NURS LOCATION, file 211.4
 ...
 K ^TMP("PSB",$J)
 ; - IHS/OCA/JTC - 2022.05.10 - ADD ENTRIES FROM CLINIC DEFINITION file
 ;Q:'$G(^JTC("BCMA"))=1
 S BZOI=0
 S BZOOFF=9000000                            ; OFFSET TO DIFFER FROM WARDS
 F  S BZOI=$O(^PS(53.46,BZOI)) Q:'BZOI  D
 .Q:$P($G(^PS(53.46,BZOI,0)),U,4)'=1         ; SEND TO BCMA IS NOT YES SO QUIT
 .S BZOCLINIC=$P($G(^PS(53.46,BZOI,0)),U,1)  ; IEN OF HOSPITAL LOCATION
 .Q:'BZOCLINIC
 .S BZONAME=$P($G(^SC(BZOCLINIC,0)),U,1)     ; NAME IN HOSPITAL LOCATION FILE
 .S RESULTS(0)=RESULTS(0)+1
 .S RESULTS(RESULTS(0))=(BZOCLINIC+BZOOFF)_U_BZONAME_U_1
 K BZOI,BZOOFF,BZOCLINIC,BZONAME
 ; - END -
 Q
 ;
