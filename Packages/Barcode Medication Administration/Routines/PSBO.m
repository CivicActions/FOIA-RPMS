PSBO ;BIRMINGHAM/EFC - BCMA OUTPUTS ;26-Apr-2023 14:16;DU
 ;;3.0;BAR CODE MED ADMIN;**13,32,2,25,28,51,50,42,1033**;Mar 2004;Build 34
 ...
 ; ^ORD(101.24/3429
 ; ^PSDRUG(/221
 ; Modified - IHS/MSC/MIR - 03/01/23 - SETUP+14:+42
 ;
...
SETUP() ; Setup parameters for the report in PSBRPT
 ...
 ; - IHS/OCA/JTC - 2022.05.10 - HOSPITAL LOCATION
 I $P(PSBRPT(.1),U,1)="W" I $P(PSBRPT(.1),U,3)>9000000 D  S PSBWRD=$P(PSBRPT(.1),U,3) I PSBWRD Q 1
 .S PSBWRD=$P(PSBRPT(.1),U,3)-9000000 ; PSBWRD = IEN in HOSPITAL LOCATION file
 .S BZONAME=$P($G(^SC(PSBWRD,0)),U)   ; CLINIC LOCATION NAME
 .;S PSBWRD=+$P(PSBRPT(.1),U,3) ; TRY SC
 .;S BZOSTR=$P(PSBRPT(.1),U,6)_$P(PSBRPT(.1),U,7)
 .S BZOSTR=$$FMADD^XLFDT($P(PSBRPT(.1),U,6),-1) ; START DAY BEFORE FOR HOSPITAL LOCATION TO COVER ED LATE CHECK-INS
 .S BZOEND=$P(PSBRPT(.1),U,8)_$P(PSBRPT(.1),U,9)
 .S BZOD=BZOSTR
 .F  S BZOD=$O(^SC(PSBWRD,"S",BZOD)) Q:('BZOD)!(BZOD>BZOEND)  D
 ..S BZOJ=0
 ..F  S BZOJ=$O(^SC(PSBWRD,"S",BZOD,1,BZOJ)) Q:'BZOJ  D
 ...S BZOX=^SC(PSBWRD,"S",BZOD,1,BZOJ,0)
 ...Q:$P(BZOX,U,9)="C"
 ...S BZODFN=+BZOX
 ...S BZOX=$G(^DPT(BZODFN,"S",BZOD,0))
 ...Q:+BZOX'=PSBWRD  ; appt cancelled/resched
 ...I $P(BZOX,U,2)'="NT",($P(BZOX,U,2)["C")!($P(BZOX,U,2)["N") Q  ; quit if appt cancelled or no show
 ...S DFN=BZODFN D DEM^VADPT
 ...Q:(VADM(1)="")!(VA("PID")="")
 ...S ^TMP("PSBO",$J,BZODFN,0)=VADM(1)_U_VA("PID")
 ...; Determine Sort or default to Pt Name...
 ...S:$P(PSBRPT(.1),U,5)="P" PSBINDX=VADM(1)
 ...I $P(PSBRPT(.1),U,5)="B" S PSBINDX=$P($G(^DPT(DFN,.101)),U) S:PSBINDX="" PSBINDX="** NO ROOM BED **"
 ...S:$P(PSBRPT(.1),U,5)="" PSBINDX=VADM(1)
 ...S:$G(PSBINDX)="" PSBINDX=VADM(1)
 ...S ^TMP("PSBO",$J,"B",PSBINDX,BZODFN)=""
 K BZOSTR,BZOEND,BZOD,BZOJ,BZOX,BZOJ,BZODFN
 ; - END -
