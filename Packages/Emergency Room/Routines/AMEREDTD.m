AMEREDTD ; IHS/OIT/SCR - Sub-routine for ER VISIT edit of discharge data
 ;;3.0;ER VISIT SYSTEM;**2,6,11,12,13**;MAR 03, 2009;Build 36
 ;
 ;DISCHARGE
 ;PROCEDURES
 ;EXIT ASSESSMENT
...
EDDISCHG(AMERDA,AMERAIEN) ; EP from AMEREDIT for discharge information
 ...
 NEW DPDATE,DPIEN,DNIEN
 S DPDATE=$P($G(^AMERVSIT(AMERDA,6)),U,2)
 S (DPIEN,DNIEN)=""
 ;
 ...
 N AMERNO,Y,AMEROLD,AMERNEW,AMEREDTS,AMERSTRG,DR,DIC,DIR,AMERDR,AMERDR
 S (AMEROLD,AMERNEW,AMEREDTS,AMERSTRG,AMERDR)=""
 S DIR("A")="*(PRIMARY)Provider who signed PCC form: "
 S DIR("?")="^D NHELP^AMERMPV1(""P"")"
 S (DPIEN,AMEROLD,AMERNO)=$P($G(^AMERVSIT(AMERDA,6)),U,3)
 S DIR("B")="" S:AMERNO DIR("B")=$P($G(^VA(200,AMERNO,0)),U)
 ...
 ..NEW AUPD,ERR
 ..;
 ..;Delete existing entry and sync
 .. I DPIEN,DPDATE D
 ... NEW PROV,NURSE
 ... D DRESET^AMER3(AMERPAT,"P",AMERPCC)
 ... D SYNC^AMERMPRV(AMERPAT,AMERPCC,.PROV,.NURSE,"")
 ..;
 ..;Now add new entry and sync
 .. I AMERNEW,DPDATE D
 ... NEW PROV,NURSE,NAMERP
 ... S NAMERP("DISCHARGE PROVIDER",DPDATE,+AMERNEW)=""
 ... D SYNC^AMERMPRV(AMERPAT,AMERPCC,.PROV,.NURSE,.NAMERP)
 ... S DPIEN=+AMERNEW
 ..;
 ..S AUPD(9009080,AMERDA_",",6.3)=$S(AMERNEW="":"@",1:AMERNEW)
 ..D FILE^DIE("","AUPD","ERR")
 ...
 S DIR("A")="Discharge nurse: "
 S DIR("?")="^D NHELP^AMERMPV1(""N"")"
 S (DNIEN,AMEROLD,AMERNO)=$P($G(^AMERVSIT(AMERDA,6)),U,4)
 S DIR("B")="" S:AMERNO DIR("B")=$P($G(^VA(200,AMERNO,0)),U)
 S DIR("S")="I $D(^VA(200,""AK.PROVIDER"",$P($G(^VA(200,+Y,0)),U),+Y))"
 S DIR="^VA(200,",DIR(0)="PO^200:AEQM"
 D ^DIR
 ...
 .. I DNIEN,DPDATE D
 ... NEW PROV,NURSE
 ... D DRESET^AMER3(AMERPAT,"N",AMERPCC)
 ... D SYNC^AMERMPRV(AMERPAT,AMERPCC,.PROV,.NURSE,"")
 ..;
 ..;Now add new entry and sync
 .. I AMERNEW,DPDATE D
 ... NEW PROV,NURSE,NAMERP
 ... S NAMERP("DISCHARGE NURSE",DPDATE,+AMERNEW)=""
 ... D SYNC^AMERMPRV(AMERPAT,AMERPCC,.PROV,.NURSE,.NAMERP)
 ... S DNIEN=+AMERNEW
 ..;
 ..S AUPD(9009080,AMERDA_",",6.4)=$S(AMERNEW="":"@",1:AMERNEW)
 ..D FILE^DIE("","AUPD","ERR")
 ...
 ..NEW AUPD,ERR
 ..;
 ..;Delete existing DP and DN entries and sync
 .. I (DPIEN!DNIEN),DPDATE D
 ... NEW PROV,NURSE
 ... D DRESET^AMER3(AMERPAT,"NP",AMERPCC)
 ... D SYNC^AMERMPRV(AMERPAT,AMERPCC,.PROV,.NURSE,"")
 ..;
 ..;Now add new entry and sync
 .. I (DPIEN!DNIEN),AMERNEW D
 ... NEW PROV,NURSE,NAMERP
 ... I DPIEN S NAMERP("DISCHARGE PROVIDER",AMERNEW,DPIEN)=""
 ... I DNIEN S NAMERP("DISCHARGE NURSE",AMERNEW,DNIEN)=""
 ... D SYNC^AMERMPRV(AMERPAT,AMERPCC,.PROV,.NURSE,.NAMERP)
 ..;
 ..S AUPD(9009080,AMERDA_",",6.2)=$S(AMERNEW="":"@",1:AMERNEW)
 ..D FILE^DIE("","AUPD","ERR")
...
EDEXTAS(AMERDA,AMERAIEN)   ;EP from AMEREDIT - ER EXIT ASSESSMENT 
 ...
 D:AMEREDTS'="" MULTAUDT^AMEREDAU(AMEREDTS,AMERAIEN)
 ;
 ;GDIT/HS/BEE;11102021;FEATURE#79981;Update PATIENT STATUS CODE (NUBC)
 D:AMEREDTS'="" UPNUBC^AMERSAV1($G(AMERDA))
 ;
