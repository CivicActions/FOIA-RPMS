AMERD ; IHS/ANMC/GIS - PRIMARY ROUTINE FOR ER DISCHARGE ; 
 ;;3.0;ER VISIT SYSTEM;**4,5,6,8,11,13,15,16**;MAR 03, 2009;Build 14
 ;
CHECK ;
...
VAR ; ENTRY POINT FOR BATCH MODE
 S %="",$P(%,"~",80)="",AMERLINE=%,AMERSTRT=1,AMERFIN=53,AMERQSEQ="" K %
 S IOP=0 D ^%ZIS
...
RUN F AMERRUN=AMERSTRT:1 Q:$D(AMERBFLG)  Q:AMERRUN>AMERFIN  Q:$D(AMERQUIT)  D  Q:$D(AMERQUIT)
 ...
 .. D LOG^AMERBUSA("P","E","AMER","AMER: Viewed/Updated Patient ER visit information - "_AFIELD_" ("_AUDDFN_")",AUDDFN)
 .. KILL AUDDFN
 . ;
...
ELOOP I $D(AMEREFLG) G FIX
 ...
 . ;Update provider
 . NEW AUPNVSIT
 . S AUPNVSIT=$$GET1^DIQ(9009081,DFN_",",1.1,"I")
 . I AUPNVSIT D SYNC^AMERPRV($G(AUPNVSIT))
 . ;
...
SET K AMERMAND I $D(AMERQUIT) Q
 ...
 .I AMERPCC>0 D
 ..I $T(REG^BEDDUTW1)]"" D REG^BEDDUTW1(AMERPCC) W !,"BEDD visit marked as deleted"  ;Mark BEDD visit as deleted
 ..S APCDVDLT=AMERPCC
 ..D EN^APCDVDLT
 ..K APCDVDLT
 ..W !,"PCC Visit deleted..."
...
TRGSET ; SET TRIAGE INFO IN ER ADM FILE
 N AMERPCC,FIELD  ;AMER*3.0*13;ADDED FIELD
 D ^AMER0
 S AMERTRGS=20
 S DA=AMERDFN,DIE="^AMERADM("
 F  S AMERTRGS=$O(^TMP("AMER",$J,2,AMERTRGS)) Q:AMERTRGS'?1N.N  D
 .;GDIT/HS/BEE 10/05/22;CR#5100;AMER*3.0*13;Save new triage provider fields
 .;S DR=AMERTRGS-3_"////"_$P(^TMP("AMER",$J,2,AMERTRGS),U)
 .S FIELD=AMERTRGS-3
 .I AMERTRGS=26 S FIELD=1.01  ;Triage provider
 .I AMERTRGS=27 S FIELD=1.02  ;Triage date/time
 .S DR=FIELD_"////"_$P(^TMP("AMER",$J,2,AMERTRGS),U)
 .;
 .D ^DIE
 .Q
 S AMERPCC=$$EXISTING^AMERPCC(AMERDFN)
...
TRG ; SET TRIAGE INFO IN TMP FILE
 ...
 .;GDIT/HS/BEE;FEATURE#110347;AMER*3.0*16;TRAUMA REGISTRY - Omit 37,38,39
 .I (AMERTRGS=37)!(AMERTRGS=38)!(AMERTRGS=39) Q
...
DXCK(AUPNPAT) ; Entry point to check for valid Dx entry for visit
 ...
 Q AMERDCNT
 ;
IVAR NEW %Y,AMERBCH,AMERBFLG,AMERDA,AMERDFN,AMERFIN,AMERLINE,AMERNUM,AMERQNO,AMERQSEQ,AMERRUN,AMERSTRT,AMERTFLG,AMERTRG,AMERTRGS,AMRBCH
 NEW DA,DFN,DIE,DIROUT,DR,IOP
 Q
