AMERPOV ;GDIT/HS/BEE - SYNCHRONIZE AMER WITH PCC ; 07 Oct 2013  11:33 AM
 ;;3.0;ER VISIT SYSTEM;**6,10,15**;MAR 03, 2009;Build 5
 ;
 Q
 ;
SYNC ;PEP - Sync AMER with PCC
 ...
 ;
 ;GDIT/HS/BEE;AMER*3.0*15;FEATURE#97912;04/12/2024;Synch admission date/time changes with VISIT
 D SYNCVDTM(AUPNVSIT,AMERVSIT)
 ;
 ;Get DFN
...
DX(X,D0,CODE,VDATE) ;Return the ICD Description
 ...
 ;
 ;GDIT/HS/BEE;AMER*3.0*15;FEATURE#97912;04/12/2024;Synch admission date/time changes with VISIT
SYNCVDTM(AUPNVSIT,AMERVSIT) ;Sync the PCC visit date/time with AMER if needed
 ;
 NEW AMERADTM,AUPNADTM,UPD,ERROR
 ;
 ;Grab the VISIT adtm
 S AUPNADTM=$$GET1^DIQ(9000010,AUPNVSIT_",",.01,"I") Q:AUPNADTM=""
 ;
 ;Grab the ER VISIT adtm
 S AMERADTM=$$GET1^DIQ(9009080,AMERVSIT_",",.01,"I")
 ;
 ;Quit if they match
 I AUPNADTM=AMERADTM Q
 ;
 ;Update the ER VISIT entry
 S UPD(9009080,AMERVSIT_",",".01")=AUPNADTM
 D FILE^DIE("","UPD","ERROR")
 ;
 ;Update BEDD if in use
 I $T(VDTM^BEDDSYNC)]"" D VDTM^BEDDSYNC(AUPNVSIT,AMERADTM,AUPNADTM)
 ;
 Q
