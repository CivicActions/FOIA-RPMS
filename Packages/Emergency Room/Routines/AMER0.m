AMER0 ; IHS/ANMC/GIS - AMER SUBROUTINE ;  
 ;;3.0;ER VISIT SYSTEM;**1,2,5,8,13,16**;MAR 03, 2009;Build 14
 ;
...
SAVE N AMERPCMP S %=$$STG^AMER0(.AMERPCMP)
 ...
 I $D(^AMERADM(AMERDFN)) D SETADM(AMERSTG,AMERPCMP) D TRF(AMERDFN),TRM(AMERDFN) Q
...
STG(AMERPCMP) ; EP FROM SAVE^AMER0
 ...
 . I (%<6!(%>9)),'((%>30)&(%<40)) S Y=^AMER(2.3,$O(^AMER(2.3,"B",X,0)),0) D
 ...
 .. I Z=1.01 S I=26  ;Triage provider
 .. I Z=1.02 S I=27  ;Triage provider time
...
UTL(E) ; ENTRY POINT FROM AMER
 ...
 ;
 ;GDIT/HS/BEE;FEATURE#110347;AMER*3.0*16;TRAUMA REGISTRY
TRM(DA) ; DR STRINGS RELATED TO TRAUMA
 NEW AMUPD,AMERR,AMERPCC
 S AMUPD(9009081,DA_",",2.5)=$S($G(^TMP("AMER",$J,1,37))]"":^TMP("AMER",$J,1,37),1:"@")
 S AMUPD(9009081,DA_",",2.6)=$S(+$P($G(^TMP("AMER",$J,1,38)),U):$P($G(^TMP("AMER",$J,1,38)),U),1:"@")
 S AMUPD(9009081,DA_",",2.7)=$S(+$G(^TMP("AMER",$J,1,39)):^TMP("AMER",$J,1,39),1:"@")
 D FILE^DIE("","AMUPD","AMERR")
 ;
 ;Get visit IEN
 S AMERPCC=$$GET1^DIQ(9009081,DA_",",1.1,"I") Q:AMERPCC'>0
 ;
 ;Save trauma provider to V PROVIDER
 I +$P($G(^TMP("AMER",$J,1,38)),U) D TRAUMA^AMERPRV(AMERPCC,DA,+$P($G(^TMP("AMER",$J,1,38)),U),+$G(^TMP("AMER",$J,1,39)))
 Q
 ;
TRF(DA) ; DR STRINGS RELATED TO TRANSFER
 S A=""
 F I=1:1:7 S %=+$G(^TMP("AMER",$J,1,(I+5))) S $P(A,U,I)=%
 S @AMERADM@(DA,2)=A
 Q
 ;
...
SETADM(AMERSTG,AMERPCMP)  ;
 ...
 Q
 ;
VAR NEW AMERPAT,AMERQUIT,BY,DHD,DIK,DINUM,DIR,FLDS,FR,TO,V
 Q
