...
TRAUMA(AMERDA)  ; EP from AMEREDTA
 ;
 N AMERNEW,AMEROLD,AMERSTRG,AMERDR,DIR
TRAUMA1 S (AMERNEW,AMEROLD,AMERSTRG,AMERDR)=""
 ;QA37
 S (AMEROLD,DIR("B"))="NO" I $P($G(^AMERVSIT(AMERDA,20)),U)=1 S (AMEROLD,DIR("B"))="YES"
 S DIR(0)="YO",DIR("A")="*Was the Trauma Team Activated for this visit"
 D ^DIR K DIR
 I $D(DUOUT)!$D(DTOUT) Q
 Q:Y=-1!(Y="")
 S AMERNEW=Y
 S AMERNEW=$$EDDISPL^AMEREDAU(AMERNEW,"B") ; "BOOLEAN" translates from 0 to NO
 I AMEROLD'=AMERNEW D
 .S AMERSTRG=$$EDAUDIT^AMEREDAU("20.01",AMEROLD,AMERNEW,"TRAUMA TEAM ACTIVATION")
 .S AMERDR=$S(AMERDR'="":AMERDR_";",1:""),AMERDR=AMERDR_"20.01////"_Y
 .D DIEREC^AMEREDAU(AMERAIEN,AMERSTRG)
 .;
 .;If yes to no - clear other fields
 .I AMERNEW="NO" S AMERDR=AMERDR_";20.02////@;20.03////@"
 .D DIE^AMEREDIT(AMERDA,AMERDR)
 .S (AMERDR,Y,AMEROLD)=""
 .Q
 ;QA38
 ...
 I AMERNEW="NO" Q 1
TRAUMA2 S (AMERNEW,AMEROLD,AMERSTRG,AMERDR)=""
 S DIR("A")="*Trauma Activated Provider"
 K DIR("?")
 S AMEROLD=$P($G(^AMERVSIT(AMERDA,20)),U,2)
 S DIR("B")="" S:AMEROLD DIR("B")=$P($G(^VA(200,AMEROLD,0)),U)
 K DIR("S")
 ...
 .S AMERNEW=+Y
 .I AMERNEW'=AMEROLD D
 ..S AMERSTRG=$$EDAUDIT^AMEREDAU("20.02",AMEROLD,AMERNEW,"TRAUMA TEAM ACTIVATED PROVIDER")
 ..S AMERDR="20.02////"_AMERNEW
 ..D DIE^AMEREDIT(AMERDA,AMERDR)
 ..D DIEREC^AMEREDAU(AMERAIEN,AMERSTRG)
 ;
 ;QA39
 S (AMERNEW,AMEROLD,AMERSTRG,AMERDR)=""
 S AMEROLD=$P($G(^AMERVSIT(AMERDA,20)),U,3),DIR("B")=""
 I AMEROLD'="" S Y=AMEROLD X ^DD("DD") S DIR("B")=Y
 S DIR(0)="DO^::ER",DIR("A")="*Trauma Team Activation Date/Time"
 ...
 S AMERNEW=+Y
 I AMERNEW'=AMEROLD D
 .S AMERSTRG=$$EDAUDIT^AMEREDAU("20.03",AMEROLD,AMERNEW,"TRAUMA TEAM ACTIVATION DTM")
 .S AMERDR="20.03////"_AMERNEW
 .D DIE^AMEREDIT(AMERDA,AMERDR)
 .D DIEREC^AMEREDAU(AMERAIEN,AMERSTRG)
 ;
 Q 1
