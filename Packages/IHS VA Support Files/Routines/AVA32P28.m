AVA32P28 ;IHS/OIT/NKD - AVA 93.2 PATCH 28 ENVIRONMENT CHECK ; 09/16/2024
 ;;93.2;VA SUPPORT FILES;**28**;JUL 01, 1993;Build 1
 ;
 ;
ENV ;----- ENVIRONMENT CHECK
 ...
 I '$$CHKPAT("AVA*93.2*27") S XPDQUIT=1
 D DUPE
 D OK
 ;
...
PRE ;EP -- PRE-INSTALL FROM KIDS.
 ...
 D ^XBKVAR
 ;
 D BMES^XPDUTL("Beginning pre-install routine (PRE^AVA32P28)...")
 ;
...
POST ;EP -- POST-INSTALL FROM KIDS.
 ...
 D ^XBKVAR
 ;
 D BMES^XPDUTL("Beginning post-install routine (POST^AVA32P28)...")
 ;
 D BMES^XPDUTL("Delivering AVA*93.2*28 install message to select users...")
 D MAIL
 ;
...
MAIL ;----- SEND INSTALL MAIL MESSAGE 
 ...
 K ^TMP("AVAP28MS",$J)
 ;
 S ^TMP("AVAP28MS",$J,1)=" --- AVA v 93.2, Patch 28, has been installed into this namespace ---"
 ;
 I $G(XPDA) D
 . S %=0
 . F  S %=$O(^XTMP("XPDI",XPDA,"BLD",XPDBLD,1,%)) Q:'%   D
 . . S ^TMP("AVAP28MS",$J,(%+1))=" "_^(%,0)
 ;
 S XMSUB=$P($P($T(+1),";",2)," ",3,99)
 S XMDUZ=$S($G(DUZ):DUZ,1:.5)
 S XMTEXT="^TMP(""AVAP28MS"",$J,"
 S XMY(1)=""
 S XMY(DUZ)=""
 ;
