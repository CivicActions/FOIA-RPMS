AVA32P25 ;IHS/OIT/FBD&NKD - AVA 93.2 PATCH 25 ENVIRONMENT CHECK ; 02/20/2020
 ;;93.2;VA SUPPORT FILES;**25**;JUL 01, 1993;Build 1
 ;
 ;
ENV ;----- ENVIRONMENT CHECK
 ...
 I '$$CHKPAT("AVA*93.2*24") S XPDQUIT=1
 D DUPE
 D OK
 ;
...
PRE ;EP -- PRE-INSTALL FROM KIDS.
 ...
 D ^XBKVAR
 ;
 D BMES^XPDUTL("Beginning pre-install routine (PRE^AVA32P25)...")
 ;
...
POST ;EP -- POST-INSTALL FROM KIDS.
 ...
 D ^XBKVAR
 ;
 D BMES^XPDUTL("Beginning post-install routine (POST^AVA32P25)...")
 ;
 N DIK,IND
 D BMES^XPDUTL("Re-indexing Race file entries...")
 F IND="ACDC","AHL7","B","C" K ^DIC(10,IND)
 S DIK="^DIC(10," D IXALL^DIK
 D BMES^XPDUTL("Re-indexing Ethnicity file entries...")
 F IND="ACDC","AHL7","B","C" K ^DIC(10.2,IND)
 S DIK="^DIC(10.2," D IXALL^DIK
 ;
 D BMES^XPDUTL("Delivering AVA*93.2*25 install message to select users...")
 D MAIL
 ;
...
MAIL ;----- SEND INSTALL MAIL MESSAGE 
 ...
 K ^TMP("AVAP25MS",$J)
 ;
 S ^TMP("AVAP25MS",$J,1)=" --- AVA v 93.2, Patch 25, has been installed into this namespace ---"
 ;
 I $G(XPDA) D
 . S %=0
 . F  S %=$O(^XTMP("XPDI",XPDA,"BLD",XPDBLD,1,%)) Q:'%   D
 . . S ^TMP("AVAP25MS",$J,(%+1))=" "_^(%,0)
 ;
 S XMSUB=$P($P($T(+1),";",2)," ",3,99)
 S XMDUZ=$S($G(DUZ):DUZ,1:.5)
 S XMTEXT="^TMP(""AVAP25MS"",$J,"
 S XMY(1)=""
 S XMY(DUZ)=""
 ;
