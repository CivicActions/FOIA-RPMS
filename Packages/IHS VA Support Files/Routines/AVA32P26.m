AVA32P26 ;IHS/OIT/FBD&NKD - AVA 93.2 PATCH 26 ENVIRONMENT CHECK ; 02/20/2020
 ;;93.2;VA SUPPORT FILES;**26**;JUL 01, 1993;Build 1
 ;
 ;
ENV ;----- ENVIRONMENT CHECK
 ...
 I '$$CHKPAT("AVA*93.2*25") S XPDQUIT=1
 D DUPE
 D OK
 ;
...
PRE ;EP -- PRE-INSTALL FROM KIDS.
 ...
 D ^XBKVAR
 ;
 D BMES^XPDUTL("Beginning pre-install routine (PRE^AVA32P26)...")
 ;
 ;ANY PRE-INSTALL SUBROUTINE CALLS GO HERE
 D BMES^XPDUTL("Removing ETHNICITY AONLYONE index...")
 D DELIXN^DDMOD(2.06,"AONLYONE")
 ;
...
POST ;EP -- POST-INSTALL FROM KIDS.
 ...
 D ^XBKVAR
 ;
 D BMES^XPDUTL("Beginning post-install routine (POST^AVA32P26)...")
 ;
 D BMES^XPDUTL("Delivering AVA*93.2*26 install message to select users...")
 D MAIL
 ;
...
MAIL ;----- SEND INSTALL MAIL MESSAGE 
 ...
 K ^TMP("AVAP26MS",$J)
 ;
 S ^TMP("AVAP26MS",$J,1)=" --- AVA v 93.2, Patch 26, has been installed into this namespace ---"
 ;
 I $G(XPDA) D
 . S %=0
 . F  S %=$O(^XTMP("XPDI",XPDA,"BLD",XPDBLD,1,%)) Q:'%   D
 . . S ^TMP("AVAP26MS",$J,(%+1))=" "_^(%,0)
 ;
 S XMSUB=$P($P($T(+1),";",2)," ",3,99)
 S XMDUZ=$S($G(DUZ):DUZ,1:.5)
 S XMTEXT="^TMP(""AVAP26MS"",$J,"
 S XMY(1)=""
 S XMY(DUZ)=""
 ;
