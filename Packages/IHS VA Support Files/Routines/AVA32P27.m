AVA32P27 ;IHS/OIT/FBD - AVA 93.2 PATCH 27 ENVIRONMENT CHECK ; 08/02/2022
...
ENV ;----- ENVIRONMENT CHECK
 ...
 I '$$CHKPAT("AVA*93.2*26") S XPDQUIT=1
 D DUPE
 D OK
 ;
...
PRE ;EP -- PRE-INSTALL FROM KIDS.
 ...
 D ^XBKVAR
 ;
 D BMES^XPDUTL("Beginning pre-install routine (PRE^AVA32P27)...")
 ;
...
POST ;EP -- POST-INSTALL FROM KIDS.
 ...
 D ^XBKVAR
 ;
 D BMES^XPDUTL("Beginning post-install routine (POST^AVA32P27)...")
 ;
 D BMES^XPDUTL("- ^AVA200K deletion of files 3/6/16 and supporting cross-references...")
 D ^AVA200K
 ;
 ;
 D BMES^XPDUTL("- Delivering AVA*93.2*27 install message to select users...")
 D MAIL
 ;
...
MAIL ;----- SEND INSTALL MAIL MESSAGE 
 ...
 K ^TMP("AVAP27MS",$J)
 ;
 S ^TMP("AVAP27MS",$J,1)=" --- AVA v 93.2, Patch 27, has been installed into this namespace ---"
 ;
 I $G(XPDA) D
 . S %=0
 . F  S %=$O(^XTMP("XPDI",XPDA,"BLD",XPDBLD,1,%)) Q:'%   D
 . . S ^TMP("AVAP27MS",$J,(%+1))=" "_^(%,0)
 ;
 S XMSUB=$P($P($T(+1),";",2)," ",3,99)
 S XMDUZ=$S($G(DUZ):DUZ,1:.5)
 S XMTEXT="^TMP(""AVAP27MS"",$J,"
 S XMY(1)=""
 S XMY(DUZ)=""
 ;
