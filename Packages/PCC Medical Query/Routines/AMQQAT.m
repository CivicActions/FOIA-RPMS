AMQQAT ; IHS/OHPRD/JCM - GETS ATTRIBUTE ; [ 01/31/2008   5:08 PM ]
 ;;2.0;PCC QUERY UTILITY;**4,18,20,21**;FEB 07, 2007
VAR D EXIT S AMQQQ="" K AMQQMULT
 ;I '$D(AMQQNOET) S X="ATERR^AMQQAT",@^%ZOSF("TRAP")
 I '$D(AMQQNOET),$$VERSION^%ZOSV(1)["Cache" S AMQQETRP=$ZT,X="*ATERR^AMQQAT",@^%ZOSF("TRAP")      ;TASSC/MFD added * to not unwind the stack ;IHS/CIM/THL PATCH 18
 I '$D(AMQQNOET),$$VERSION^%ZOSV(1)'["Cache" S AMQQETRP=$ZT,X="ATERR^AMQQAT",@^%ZOSF("TRAP")      ;TASSC/MFD added * to not unwind the stack ;IHS/CIM/THL PATCH 18
RUN D GET
EXIT ; ENTRY POINT FROM MULTIPLE ROUTINES
 I $G(AMQQETRP)]"" S $ZT=AMQQETRP K AMQQETRP ;IHS/CIM/THL PATCH 18
...
CTXS ; ENTRY POINT FROM AMQQATG
 ...
 ;D ^AMQQSQ
 D:$G(AMQQLINK)'=763 ^AMQQSQ ;IHS/CIM/THL PATCH 20
...
ATERR ;I '$D(AMQQNOET) X "I $P($ZE,"">"")=""<INRPT""!($ZE[""-CTRAP"")" I  W !!,"Session terminated...",!! H 2 S AMQQQUIT="" G EXIT ;IHS/OHPRD/JCM 4/20/94 TVA SPECIFIC
 I '$D(AMQQNOET) X "I $P($ZE,"">"")=""<INTERRUPT""!($ZE[""-CTRAP"")" I  W !!,"Attribute Session terminated...",!! H 2 S AMQQQUIT="" G EXIT       ;TASSC/MFD added line for Cache, added attribute to see level you're at
