AMQQEMAN ; OHPRD/GIS,DWG - Q-MAN TO DOS EXPORT UTILITY.  BUILDS FLAT ASCII FILES ACCORDING TO SPECS. ;  [ 01/31/2008   5:08 PM ]
 ;;2.0;PCC QUERY UTILITY;**18,20,21**;FEB 07, 2007
 ;;IHS/CIM/THL PATCH 18 TO MAKE EXPORT CACHE COMPATIBLE
RUN S U="^" F AMQQERUN=1:1:11 D @$P("EXIT^INIT^STUFF^LEN^TYPE^DEL^MLEN^FIX^HLEN^DATA^PATH",U,AMQQERUN) I $D(AMQQQUIT) Q
...
INIT I '$D(DUZ(2)) W !!,"Kernel variables not present...Session cancelled",*7,!! S AMQQQUIT="" Q
 I '$D(IOSL) D HOME^%ZIS
 S %=$G(AMQQCCLS) I %'="P",%'="V" W !!,"The subject of your search must be a PATIENT or a VISIT to create an ASCI file.",!,*7,"Sorry...",!!! H 3 S AMQQQUIT="" Q
 S AMQQEMS=""
HEADER W @IOF,!!,?20,"*****  E-MAN DATA EXPORT UTILITY  *****"
 ...
 ;
STUFF ; 3
 D MARK W "EXPORT TO WHAT APPLICATION",!
 S DIR(0)="P^9009073:EQM",DIR("A")="Select export format",DIR("B")="CUSTOM CONFIG",DIR("??")="AMQQEMANCONFIG" D ^DIR K DIR S:$D(DUOUT) DIRUT=1
 I X="" K AMQQEM("STUFF") Q
 I X?1."^" S AMQQQUIT="" K DIRUT,DUOUT,DIROUT,DTOUT Q
 I Y["CUSTOM CONFIG" K AMQQEM("STUFF") D FWD Q
 S AMQQEM("STUFF")=^AMQQ(3,+Y,0) D FWD
...
BACKUP S AMQQERUN=$P(AMQQEMS,U)-1,AMQQEMS=$P(AMQQEMS,U,2,99)
 ...
 ;
LEN ; RECORD LENGTH ; 4
 S %=$P($G(AMQQEM("STUFF")),U,2) I % S AMQQEM("LEN")=% Q
 D MARK W "MAXIMUM RECORD LENGTH",!
 S DIR("B")=$S($D(AMQQEM("LEN")):AMQQEM("LEN"),1:256)
 S DIR(0)="N^1:256:0",DIR("A")="Enter the max. number of characters in ea. record",DIR("?")="Most analytic/graphics software can import 256 character records" D ^DIR K DIR S:$D(DUOUT) DIRUT=1
 I X=U D BACKUP Q
 D CK I $D(AMQQQUIT) Q
 I 'Y W "??",*7 G LEN
 S AMQQEM("LEN")=Y D FWD
 Q
 ;
TYPE ; RECORD TYPE ; 5
 ...
 D MARK W "RECORD FORMAT",!
 S DIR("B")=$S($D(AMQQEM("TYPE")):AMQQEM("TYPE"),1:1),DIR("??")="AMQQEMANFIXDEL"
 S DIR(0)="S^1:DELIMITED;2:FIXED LENGTH",DIR("A")="     Your choice" D ^DIR K DIR S:$D(DUOUT) DIRUT=1
 I X=U D BACKUP Q
 D CK I $D(AMQQQUIT) Q
 I Y=2 K AMQQEM("DEL"),AMQQEM("MLEN") D FWD S AMQQERUN=7,AMQQEM("TYPE")=2 Q
 K AMQQEM("FIX") D FWD S AMQQEM("TYPE")=1
 Q
 ;
DEL ; DELIMITER CHARACTER ; 6
 S %=$P($G(AMQQEM("STUFF")),U,4) I %'="" S AMQQEM("DEL")=% Q
 D MARK W "DELIMITER CHARACTER",!
 I $D(AMQQEM("DEL")) S DIR("B")=AMQQEM("DEL")
 S DIR(0)="F^:^K:((X'=""UP ARROW"")&(X'=""TAB"")&((X'?1P)!(X=$C(34)))) X",DIR("A")="Enter the character to be used as a delimiter"
 ...
 D FWD S AMQQEM("DEL")=Y
 Q
 ;
MLEN ; DEL FIELD LENGTH ; 7
 S %=$P($G(AMQQEM("STUFF")),U,5) I % S AMQQEM("MLEN")=%,AMQQERUN=8 Q
 D MLEN^AMQQEM11
 Q
 ;
FIX ; FIXED FIELD LENGTH ; 8
 S %=$P($G(AMQQEM("STUFF")),U,7) I % S AMQQEM("FIX")=% Q
 D FIX^AMQQEM11
 Q
 ;
HLEN ; DEL FIELD LENGTH ; 9
 S %=$P($G(AMQQEM("STUFF")),U,6) I % S AMQQEM("HLEN")=% Q
 I '$D(AMQQEM("HLEN")),$D(AMQQEM("FIX")) S AMQQEM("HLEN")=AMQQEM("FIX")
 D HLEN^AMQQEM11
 Q
 ;
DATA ; ASSIGN DATA TYPE ; 10
 S %=$P($G(AMQQEM("STUFF")),U,8) I %'="" S AMQQEM("DATA")=% Q
 D DATA^AMQQEM11
 Q
 ;
ACCN ; ACCESSION NUMBER ; 11
 D MARK W "MAKE THE FIRST FIELD A SEQUENTIAL NUMBER",!
 S DIR("B")=$S($D(AMQQEM("ACCN")):AMQQEM("ACCN"),1:"NO")
 S DIR(0)="Y",DIR("A")="Want to make the 1st field a sequential (serial) number"
 S DIR("?")="In some cases you may want to enter an serial number (starting with 1 and incrementing by 1 for each entry) as the first field of each record" D ^DIR K DIR S:$D(DUOUT) DIRUT=1
 I X=U D BACKUP Q
 D CK I $D(AMQQQUIT) Q
 D FWD S AMQQEM("ACCN")=$S(Y:"YES",1:"NO")
 Q
 ;
PATH ; PATH AND FILE...MSM ON ; 11 ;IHS/CIM/THL PATCH 18
 I ^DD("OS")=18 D  Q
