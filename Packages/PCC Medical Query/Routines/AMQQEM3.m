AMQQEM3 ; OHPRD/GIS,DWG - FINE TUNES THE HEADER LINE ;  [ 01/31/2008   5:08 PM ]
 ;;2.0;PCC QUERY UTILITY;**18,20,21**;FEB 07, 2007
 I 'Y Q
 S AMQQEMN=Y,AMQQEM3=0 N Y
...
LIST ; - EP -
 ...
 .S X=$E(X,1,Y),Z="",$P(Z," ",((Y+1)-$L(X)))="",T=T+Y
 .I (Y+N)>78 S:AMQQEMFN>1 J=J+1 S N=0,H(J)=""
 .S I=I+1,N=N+Y,H(J)=H(J)_X_Z_";"_Y_";"_$C(64+I)_U
 .Q
 I AMQQEMFN<99 D DISP
 Q
 ;
DISP N A,D,I,X,Y,Z,%,J,N,K,T S (T,K)=0
 S D=$G(AMQQEM("DEL")) S:D="TAB" D="t" S:D="UP ARROW" D=U
 W @IOF
 ...
 .I $D(H(J+1)) Q
 .S A=78-N S:N>78 A=78-(N#78) S:(AMQQEM("LEN")-T)<A A=AMQQEM("LEN")-T S %="",$P(%,".",A+1)="" W % W:(AMQQEM("LEN")-T)>A ">" Q
 .Q
 S %="",$P(%,"-",78)=""
 W !,%
 Q
...
TUNE I $D(AMQQEMKL) D T21 W !!,"This field cancelled..." H 2 K AMQQEMKL Q
 W @IOF D LIST W !
 S %=$P(@G@(AMQQEMN,0),U,6) I %="" S %=$P(^(0),U,3)
 ...
 S DIR("A")="     Your choice",DIR("B")="NO ADDITIONAL CHANGES" D ^DIR K DIR S:$D(DUOUT) DIRUT=1
 I Y?1."^" S AMQQQUIT="" Q
 I 'Y Q
 I Y=2 D T2 Q
 I Y=8 S Y=9
 ...
 D MARK^AMQQEMAN,@("T"_Y) I $D(AMQQQUIT) Q
 D LIST G TUNE
 Q
 ;
DATE I $P(@G@(AMQQEMN,0),U,4)="D",'$D(AMQQEM("DATE TRANS")) K DIR D  Q
 .D ^AMQQEM21 I $D(AMQQQUIT) Q
...
TYPE I $G(AMQQEM("DATA"))'=2!($D(AMQQEMKL)) Q
 W "DATA TYPE",!
TYPE1 S %=$P(@G@(AMQQEMN,0),U,4),DIR("B")=$S(%="D":"D",%="N":"N",1:"F")
 S DIR(0)="S^N:NUMBER;D:DATE;F:FREE TEXT;M:MONEY($dollars.cents)",DIR("A")="Data type" D ^DIR K DIR S:$D(DUOUT) DIRUT=1
 I X?1."^" S AMQQQUIT="" Q
...
T1 W "VIEW/EDIT DATA TYPE",!
 ...
 ;
T2 W "DELETE",!
 S DIR(0)="YO",DIR("A")="Are you sure that you want to delete this segment" D ^DIR K DIR S:$D(DUOUT) DIRUT=1
 I X?1."^" S AMQQQUIT="" Q
 I 'Y Q
...
T3 W "MOVE",!
 ...
 S X=0 F %=1:1 S Y=$P(AMQQEMFS,U,%) Q:Y=""  I Y S X=X+1 I Y=AMQQEMN S AMQQEMP=%
 S %(1)=$S(AMQQEMP=1:"B",1:"A"),%(2)=$S(AMQQEMP=X:$C(64+X-1),1:$C(64+X))
 S AMQQEMAX=X I 'X W "  ??",*7 Q
 I X=2 S AMQQEMFS=$P(AMQQEMFS,U,2)_U_$P(AMQQEMFS,U)_U Q
 S DIR("B")="("_%(1)_"-"_%(2)_")"
 S DIR(0)="FO^:",DIR("A")="Move to which segment" D ^DIR K DIR S:$D(DUOUT) DIRUT=1
 I "^"[X Q
 I X?2."^" S AMQQQUIT="" Q
 I X'?1U W "  ??",*7 D MARK^AMQQEMAN G T3
 I ($A(X)-64)>AMQQEMAX W "  ??",*7 G T3
 I $P(AMQQEMFS,U,$A(X)-64)=AMQQEMN W "  ??",*7 G T3
 S Z=$P(AMQQEMFS,U,AMQQEMP),$P(AMQQEMFS,U,AMQQEMP)=""
...
T4 W "RENAME FIELD",!
 ...
 S DIR(0)="FO^1:"_AMQQEM("HLEN"),DIR("A")="Enter new name" D ^DIR K DIR S:$D(DUOUT) DIRUT=1
 I "^"[X Q
 I "^^"=X S AMQQQUIT="" Q
