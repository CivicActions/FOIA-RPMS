AMQQEM21 ; OHPRD/GIS,DWG - PARSES DATE FORMAT AND GENERATES OUTPUT CODE ;  [ 01/31/2008   5:08 PM ]
 ;;2.0;PCC QUERY UTILITY;**18,20,21**;FEB 07, 2007
 ; CHECK FOR JUNE 3, 1992
...
VAR S J=0,X="",T="~" K AMQQEMNO
RUN D ASK I $D(AMQQQUIT)!($D(AMQQEMNO)) G EXIT
 D PARSE
 I $D(AMQQEMNO) D EXIT G VAR
 D CONFIRM I $D(AMQQQUIT) G EXIT
 I $D(AMQQEMNO) D EXIT G VAR
...
PARSE F I=1:1 S Z=$E(%,I) Q:Z=""  D
 ...
 .S P(J)=P(J)_Z
 .Q
EVAL S A="",Z="JUNE^JUN^06^6^03^3^1992^92"
...
ASK ; GET DATE FORMAT
 ...
 S DIR(0)="F^:",DIR("B")="6/3/92",DIR("A")="Enter JUNE 3, 1992 in the desired format",DIR("?")="Enter the date in the proper format for your application; e.g., 6.3.92 or 3JUN92 or JUNE 3, 1992 etc" D ^DIR K DIR S:$D(DUOUT) DIRUT=1
 I Y=U S AMQQEMNO="" Q
...
CONFIRM ;
 ...
 S X=2851023 X C W X,!
 S DIR(0)="Y",DIR("A")="Is this OK",DIR("B")="YES" D ^DIR K DIR S:$D(DUOUT) DIRUT=1
 I X=U S AMQQEMNO=""
 I X="^^" S AMQQQUIT=""
 I 'Y S AMQQEMNO=""
...
PATIENT ; ENTRY POINT FROM AMQQEM2
 ...
 S DIR("A")="Enter the number of your choice",DIR("?")="Choice #4 will create a new field to hold the FIRST/MIDDLENAME)"
 S DIR("B")="1" D ^DIR K DIR S:$D(DUOUT) DIRUT=1
 I X=U S AMQQEMNO=""
 I X?2."^" S AMQQQUIT=""
...
TWO S @G@(1,2)="S X=$P(X,"","")",^(0)="^^LAST NAME^F^^"_$E("LAST NAME",1,AMQQEM("HLEN"))_U_($G(AMQQEM("FIX"))+$G(AMQQEM("MLEN")))
 ...
 S C=C+1,@G@(C,2)="S X=$P(X,"","",2),X=$E(X,"" "")",^(1)="S X=$P(^DPT(AMQP(0),0),U)",^(0)="^^FIRST NAME^F^^"_$E("FIRST NAME",1,AMQQEM("HLEN"))_U_($G(AMQQEM("FIX"))+$G(AMQQEM("MLEN"))),AMQQEMFS=AMQQEMFS_1_U_C_U
 Q
 ;
