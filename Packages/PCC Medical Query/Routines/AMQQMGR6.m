AMQQMGR6 ; GIS/ILC - AMQQMGR CONTINUED ;  [ 01/31/2008   5:08 PM ]
 ...
 D YN^DICN
 I %=1 D EN^AMQQMGR3
 S TMP="^TMP(""AMQQ LAB IEN"","_$J_")",AMQQCNT=0
 K @TMP
 W !!
 S AMQQLIEN=0
...
DETECT(%) ; DETECT LAB TEST TYPES
 N X,Y,N,Z,J,K
 S Z=$P(%,".",2),J=+%,K=(%\1)_"."_$P(J,".",2)
 I $D(@TMP@(K)) Q
 S X=$P($G(^LAB(60,+(%\1),0)),U) I X="" Q
 S Y=$P($G(^LAB(61,+Z,0)),U)
 S N=X I $L(Y) S N=N_", "_Y I Y="UNKNOWN" S N=N_" SOURCE"
 S @TMP@(K)=N,@TMP@("B",N,K)="",@TMP@("C",K)=Z
