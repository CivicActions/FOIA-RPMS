AMQQREG ; IHS/CMI/THL - QUERY CMS REGISTER INTERFACE ; [ 01/31/2008   5:08 PM ]
 ;;2.0;PCC QUERY UTILITY;**14,15,17,18,20,21**;FEB 07, 2007
...
EN1 D REG
 ...
 D ACTIVE
 Q:$D(AMQQQUIT)
 D DX:AMQQCNAM["DIABET" ;IHS/CIM/THL PATCH 17
 Q:$D(AMQQQUIT)
 D COHORT
 Q
REG ;EP;TO SELECT A REGISTER
 ...
 D DECEASED^ACMGTP(AMQQRDA) ;IHS/CIM/THL PATCH 17
 Q
ACTIVE ;EP;TO SELECT PATIENT STATUS
 ;PATCH 17 ALLOW FOR MULTIPLE STATUSES
 ;PATCH 18 ADD 'Lost to Follow-up' status
 K DIR
 W !!,"Select the Patient Status for this report"
 ;S DIR(0)="SO^A:Active;I:Inactive;T:Transient;U:Unreviewed;D:Deceased;Z:All Register Patients" ;IHS/CIM/THL PATCH 17
 ;S DIR("A")="Which patients"
 ;S DIR("B")="Active" ;IHS/CIM/THL PATCH 17
 W !!?10,"1     Active" ;IHS/CIM/THL PATCH 17
 W !?10,"2     Inactive"
 W !?10,"3     Transient"
 W !?10,"4     Unreviewed"
 W !?10,"5     Deceased"
 ...
 S DIR("B")="1" ;IHS/CIM/THL PATCH 17
 W !
 D ^DIR
 K DIR
 I 'Y S AMQQQUIT="" Q
 I Y[8 S AMQQ("CMS STATUS","Z")="" Q  ;PATCH 17
 I Y D
 .N X,Z
 .F J=1:1 S X=$P(Y,",",J) Q:X=""  D
 ..S Z=$S(X=1:"A",X=2:"I",X=3:"T",X=4:"U",X=5:"D",X=6:"N",X=7:"L",X=8:"Z",1:"") ;IHS/CIM/THL PATCH 18
 ..Q:Z=""
 ..S AMQQ("CMS STATUS",Z)=""
...
COHORT ;CREATE SEARCH TEMPLATE COHORT WITH REGISTER PATIENTS
 ...
 .Q:'Y
 .I $G(AMQQ("DM DIAGNOSIS"))]"" D PATDX I $D(AMQQQUIT) K AMQQQUIT Q  ;PATCH 17
 .I $D(AMQQ("CMS STATUS","Z"))!$D(AMQQ("CMS STATUS",Z)) D  ;PATCH 17
 ..S ^DIBT(AMQQDA,1,Y)=""
 ..W "."
 W !
 Q
C1 ;CREATE SEARCH TEMPLATE
 ;S X=AMQQ("CMS STATUS") ;PATCH 17
 ;S X=$S(X="A":"Active",X="I":"Inactive",X="T":"Transient",X="U":"Unreviewed",X="D":"Deceased",1:"All Patients") ;PATCH 17
...
DIC ;FM DIC INTERFACE
 ...
 K DIC,DA,DD,DR,DINUM,D,DLAYGO
 Q
DX ;EP;TO SELECT DIABETES DIAGNOSIS  ;IHS/CIM/THL PATCH 17
 ...
 K DIR
 I 'Y S AMQQQUIT="" Q
 ;S AMQQ("DM DIAGNOSIS")=$S(Y=1:"TYPE 1",Y=2:"TYPE 2",Y=3:"GESTATIONAL DM",Y=4:"IMPAIRED GLUCOSE TOLERANCE",1:"")
 S AMQQ("DM DIAGNOSIS")=$S(Y=1:"TYPE 1",Y=2:"TYPE 2",Y=3:"TYPE 1 & TYPE 2",Y=4:"GESTATIONAL DM",Y=5:"IMPAIRED GLUCOSE TOLERANCE",1:"")
 Q
PATDX ;INCLUDE PATIENTS WITH SPECIFIC DIAGNOSIS ;IHS/CIM/THL PATCH 17
 S AMQQQUIT=""
 Q:'$D(^ACM(44,"D",X))
 N Y,Z
 S Y=0
