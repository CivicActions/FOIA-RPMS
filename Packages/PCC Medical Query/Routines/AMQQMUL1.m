AMQQMUL1 ; IHS/OHPRD/JCM - COLLECTS MULTIPLE VALUES FOR POVS, PROCEDURES, RXS, ADAS ETC. ; [ 01/31/2008   5:08 PM ]
 ;;2.0;PCC QUERY UTILITY;**7,16,18,19,20,21**;FEB 07, 2007
...
RUN I '$D(AMQQSQVN),AMQQGR="AUPNVHF" D HINC G SQ ;IHS/OHPRD/GS 3/20/95
 ;I $G(AMQP(0)),$G(AMQQONE)]""!($G(AMQV(1))["DIBT("),$G(AMQP(.1))="","^AUPNVRAD^AUPNVCPT^AUPNVDXP^"[(U_AMQQGR_U),$D(^UTILITY("AMQQ TAX",$J)) D AMQP G SQ ;IHS/CIM/THL PATCH 18
 I $G(AMQP(0)),$G(AMQQONE)]""!($G(AMQV(1))["DIBT("),$G(AMQP(.1))="","^AUPNVRAD^AUPNVCPT^AUPNVDXP^AUPNVMSR^"[(U_AMQQGR_U),$D(^UTILITY("AMQQ TAX",$J)) D AMQP G SQ ;IHS/CIM/THL PATCH 18
...
TRUE ;I AMQQSPEC="EXISTS" K ^UTILITY("AMQQ",$J,AMQQAG,AMQQUATN) S ^(AMQQUATN,1)="+",AMQP(AMQQFVAR)="+"
 I AMQQSPEC="EXISTS",AMQQ'["AUPNVIF" K ^UTILITY("AMQQ",$J,AMQQAG,AMQQUATN) S ^(AMQQUATN,1)="+",AMQP(AMQQFVAR)="+" ;IHS/CIM/THL PATCH 20 FOR INFANT FEEDING
 S AMQT(AMQQT)=1
...
INCDATE I "^AUPNVRAD^AUPNVCPT^AUPNVDXP^AUPNVMSR^"'[(U_AMQQGR_U) S AMQQVDAT=$O(@AMQQ@(AMQQVDAT)) ;IHS/CIM/THL PATCH 18
 E  S AMQQVDAT=$O(@AMQQ@($S('$G(AMQP(99.1)):AMQP(.1),1:AMQP(99.1)),AMQQVDAT))
 I AMQQVDAT'=+AMQQVDAT Q
 I (9999999-AMQQVDAT)'>AMQQST Q
 S AMQQVNO=0
INCITEM S AMQQVNO=$S(AMQQGR'["AUPNVRAD"&(AMQQGR'["AUPNVCPT")&(AMQQGR'["VDXP")&(AMQQGR'["VMSR"):$O(@AMQQ@(AMQQVDAT,AMQQVNO)),1:$O(@AMQQ@($S('$G(AMQP(99.1)):AMQP(.1),1:AMQP(99.1)),AMQQVDAT,AMQQVNO))) ;IHS/CMI/THL PATCH 16 ;IHS/CIM/THL PATCH 18
 I 'AMQQVNO G INCDATE
