AMQQMGR7 ; GIS/ILC - LAB STARTUP OVERFLOW ;  [ 01/31/2008   5:08 PM ]
 ;;2.0;PCC QUERY UTILITY;**17,18,20,21**;FEB 07, 2007
 ;
 ;IHS/CMI/THL PATCH 17 MULTIPLE CHANGES TO QMAN LAB UTILITIES
 ;
SAVE(AMQQLDFN) ;EP;
 ...
 S X=AMQQLDFN,AMQQLSS="",AMQQLSSX="",AMQQLSSN=0
 ...
 . S AMQQLSSN=AMQQLSSN+1,AMQQLSSX=AMQQLSSX_Y_U
 . Q
 I 'AMQQLSSN G LEXIT
 S AMQQLDS=0
 S %=$P(^LAB(60,AMQQLDFN,0),U,12)
...
LN1 S I=0,AMQQLSTG=""
 ...
 Q
 ;
LN2 S AMQQLSTG="",I=0 F Y=1:1 S Z=$P(AMQQLAKA,U,Y) Q:Z=""  D
 .S I=I+1
...
RESCO ; RESTORE COMPANION LAB TESTS
 ...
 S DIC(0)="L",DIC("P")=$P($G(^DD(9009075,40,0)),U,2)
 S AMQQLAB=0 F  S AMQQLAB=$O(^UTILITY("AMQQ LC",$J,AMQQLAB)) Q:'AMQQLAB  S AMQQCO=0 F  S AMQQCO=$O(^UTILITY("AMQQ LC",$J,AMQQLAB,AMQQCO)) Q:'AMQQCO  D ADD W "."
 K ^UTILITY("AMQQ LC",$J)
 Q
 ; 
...
PASK S DIC="^AMQQ(5,",DIC(0)="AEQM",DIC("S")="I Y>999"
 D ^DIC K DIC I Y=-1 Q
 S AMQQLAB=+Y
 D CLOOK
 S DIC("A")="Enter a companion test name: "
CASK S DIC="^LAB(60,",DIC(0)="AEQM"
 D ^DIC K DIC I Y=-1 G LOOP
 S AMQQCO=+Y
 D CADD
 S DIC("A")="Enter another companion test: " G CASK
LOOP W ! S DIC("A")="Enter another primary test name: " G PASK
 ;
CADD N %,%Y,DIC,X,Y,DA
...
ADD ;
 ...
 S DA(1)=AMQQLAB,DIC="^AMQQ(5,"_DA(1)_",4.1,"
 S X="`"_AMQQCO
 D ^DIC
 Q
 ;
CLOOK ;
 ...
 S X=0 F  S X=$O(^AMQQ(5,"LC",AMQQLAB,X)) Q:'X  S Y=$P($G(^LAB(60,+X,0)),U) I $L(Y) W !?2,Y
 W !!
 Q
