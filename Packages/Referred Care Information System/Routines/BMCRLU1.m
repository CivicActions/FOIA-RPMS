BMCRLU1 ; IHS/PHXAO/TMJ - GEN RETR UTILITIES ; 03 May 2018  4:47 PM
 ;;4.0;REFERRED CARE INFO SYSTEM;**3,12,13,14**;JAN 09, 2006;Build 168
 ;BMC 4.0*3 11/30/06 IHS/OIT/FCJ PRNT POL #
 ;BMC*4.0*13 4.1.18 IHS.OIT.FCJ ADDED NEW MBI FOR MEDICARE AND RRR
 ;BMC*4.0*14 4.21.22 IHS.OIT.FCJ TEST FOR NULL ENTRIES IN PI TO BY PASS ENTRIES
 ;
...
PI ; EP
 ...
 .;BMC*4.0*14 IHS/OIT/FCJ REWROTE NXT SECTION BECUASE OF ERRORS WITH INS
 .Q:$P($G(^AUPNPRVT(P,11,BMCMIFN,0)),U)=""
 .Q:$P($G(^AUTNINS($P(^AUPNPRVT(P,11,BMCMIFN,0),U),0)),U)=""
 .S BMCPCNT=BMCPCNT+1
 .;S BMCPRNM(BMCPCNT)=$P(^AUTNINS($P(^AUPNPRVT(P,11,BMCMIFN,0),U),0),U)_" - "_$P($G(^AUPN3PPH($P(^AUPNPRVT(P,11,BMCMIFN,0),U,8),0)),U,4)
 .S BMCPRNM(BMCPCNT)=$P(^AUTNINS($P(^AUPNPRVT(P,11,BMCMIFN,0),U),0),U)_" - "
 .I $P($G(^AUPNPRVT(P,11,BMCMIFN,0)),U,8)'="" S BMCPRNM(BMCPCNT)=BMCPRNM(BMCPCNT)_$P($G(^AUPN3PPH($P(^AUPNPRVT(P,11,BMCMIFN,0),U,8),0)),U,4)
