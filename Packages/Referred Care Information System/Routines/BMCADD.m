BMCADD ; IHS/PHXAO/TMJ - ADD A NEW REFERRAL ;         [ 07/12/2006  3:48 PM ]
 ;;4.0;REFERRED CARE INFO SYSTEM;**2,8,9,12,14,15,16**;JAN 09, 2006;Build 168
 ...
 ;4.0*14 4.26.2022 IHS.OIT.FCJ ADDED TEST FOR FAC CODE WHEN DISPLAYING LAST 5 REF
 ;4.0*15 4.20.2023 IHS.OIT.FCJ ADD PREFERRED NAME FOR SCREEN DISPLAYS AND CHG BO/CHS COM TO REFERRAL NOTES
 ;4.0*16 1.8.2024 IHS.OIT.FCJ ADD CALL FOR DUPLICATE CHECK
 ;
...
MAIN ;
 S BMCQ=0
 S BMCMODE="A",(BMCLOOK,BMCVDFN,BMCVRIE,BMCSTRM,BMCSNO,BMCSCOD,BMCMPRI,BMCMPRCD)="" ;BMC*4.0*8 ADDED BMCVDFN AND BMCSTRM
 S APCDOVRR=""
...
NEXT ;2ND $O
 ...
 . Q:BMCDT=""
 . Q:BMCRIEN=""
 . Q:$P($G(^BMCREF(BMCRIEN,1)),U)'=""  ;4.0 TST FOR SEC REF
 . Q:CT=0   ;BMC*3.1*9
 . Q:DUZ(2)'=$P($G(^BMCREF(BMCRIEN,0)),U,5)  ;BMC*4.0*14 TEST FOR FACILITY CODE
 . D START^BMCLKID1
 . S CT=CT-1  ;BMC*3.1*9
 Q
 ;
...
ADD ; ADD NEW REFERRAL RECORD
 ...
 .W !!,"REFERRAL number : ",BMCRNUMB W:$G(BMCCDT)'="" "C1" W !
 .S BMCRIEN=+Y
 .S BMCQ=0
 .Q
 ;
...
CHK ;CHECK FOR DUPLICATE RECORD;BMC*4.0*16
 D CHKDUP^BMCADD1              ;BMC*4.0*16
 I BMCDUP=0 D DELETE S BMCQ=1 Q      ;BMC*4.0*16
 ;
...
BOCOM ;EP;BUSINESS OFFCIE COMMENTS
 W !
 ;S DIR("A")="Do you want to enter Business Office/CHS Comments"  ;BMC*4.0*15
 S DIR("A")="Do you want to enter Referral Notes"       ;BMC*4.0*15
 S BMCCTYP="B"
 D COMMENTS
 Q
COMMENTS ;EP
