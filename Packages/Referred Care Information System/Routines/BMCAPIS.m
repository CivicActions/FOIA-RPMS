BMCAPIS ; IHS/CAS/FS - REFERRED CARE INFO SYSTEM API FOR MU2 ;     
 ;;4.0;REFERRED CARE INFO SYSTEM;**8,15**;JAN 09, 2006;Build 168
 ;4.0*8 IHS/ITSC/FS NEW ROUTINE
 ;4.0*15 5.4.2023 OIT/IHS/FCJ ADDED DATE TOC TRANSMITTED
...
GETREFFV(RSLT,VISITS) ; search referral w.r.t Visit
 ...
 N INDX,INDEX,VREFIEN,RREFIEN,VPATIEN,RRPURPOS,RRSTATUS,VNDLCIEN,VNDFAX,VNDPARYN,VNDEMAIL,RRVNDRNM
 I $D(VISITS)  D
 . N INDX,U
 . S INDX=$O(VISITS(""))
 ...
 . . . I $G(RREFIEN)'=""  D
 . . . . S RRPURPOS=$$GET1^DIQ(90001,RREFIEN_",",1201)
 . . . . S RRSTATUS=$$GET1^DIQ(90001,RREFIEN_",",.15)
 . . . . S VNDLCIEN=$$GET1^DIQ(90001,RREFIEN_",",.07,"I")
 ...
 . . . . I $G(VNDLCIEN)'=""  D
 . . . . . S RRVNDRNM=$$GET1^DIQ(9999999.11,VNDLCIEN_",",.01)
 . . . . . S VNDFAX=$$GET1^DIQ(9999999.11,VNDLCIEN_",",1114)
 . . . . . S VNDPARYN=$$GET1^DIQ(9999999.11,VNDLCIEN_",",2104)
 . . . . . S VNDEMAIL=$$GET1^DIQ(9999999.11,VNDLCIEN_",",2105)
 . . . . I $G(VNDLCIEN)=""  D
 . . . . . S VNDLCIEN=$$GET1^DIQ(90001,RREFIEN_",",.08,"I")
 . . . . . I $G(VNDLCIEN)'=""  D
 . . . . . . S RRVNDRNM=$$GET1^DIQ(9999999.06,VNDLCIEN_",",.01)
 . . . . . . S VNDEMAIL=$$GET1^DIQ(9999999.06,VNDLCIEN_",",2105)
 . . . S ^TMP("BMCDATA",$J,INDX,INDEX)=$G(VISIT)_"^"_$G(RREFIEN)_"^"_$G(VPATIEN)_"^"_$G(RRPURPOS)_"^"_$G(RRSTATUS)_"^"_$G(RRNUMBER)_"^"_$G(VNDFAX)_"^"_$G(VNDPARYN)_"^"_$G(VNDEMAIL)_"^"_$G(RRVNDRNM)
 . . . S VREFIEN="",RREFIEN="",VPATIEN="",RRPURPOS="",RRSTATUS="",VNDLCIEN="",VNDFAX="",VNDPARYN="",VNDEMAIL="",RRVNDRNM=""
 . . . S INDEX=$O(BMCOUT("DILIST","ID",INDEX))
 . . S INDX=$O(VISITS(INDX))
 S RSLT=$NA(^TMP("BMCDATA",$J))
 Q
 ;
CRENTOCD(RSLT,RRIEN,PRTXDATE,PRTXBY,DOCTYPE,CCDADOCID,TXDT) ;    ;BMC*4.0*15 ADDED TXDT
 ; Create entry in 600 (TRANSITION OF CARE DOCUMENT) multiple of RCIS REFERRAL (90001) file
 ...
 S FDA(42,90001.6,"+1,"_RRIEN_",",.06)=$G(TXDT)     ;BMC*4.0*15
