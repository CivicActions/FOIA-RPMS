BMCFPRN ; IHS/OIT/FCJ - PRINT REFERRAL FORMS ;   [ 09/27/2006  1:33 PM ]
 ;;4.0;REFERRED CARE INFO SYSTEM;**1,2,3,4,12,14,15,16**;JAN 09, 2006;Build 168
 ...
 ;BMC*4.0*14 4.15.2022 IHS.OIT.FCJ REMOVE SSN
 ;BMC*4.0*15 3.07.23 IHS.OIT.FCJ Add Cell # and Referral label
 ;4.0*16 12.5.2023 IHS.OIT.FCJ ADD "C1" SUFFIX FOR CALLIN;END DATE CHG;CHG CHS TO PRC;ADD ICD DX
 ;
 ;
PRINT ;EP print referral form - Roll and Scroll Front End
 ...
 Q
 ;
GPRINT(BMCREF,BMCPCSE) ;EP print referral form - GUI ;BMC*4.0*16 ADDED OPTION FOR CASE NOTES
 ;
...
PRT ;print referral form
 NEW REFTO,BMCCHSAS,BMCCMT,BMCDA,BMCDFN,BMCFILE,BMCFTYP,BMCNODE,BMCPG,BMCQUIT,BMCR0,BMCV,BMCVIEN,BMCX,BMCY
 NEW DTOUT,I,X,BMCQ
 ;
 S BMCR0=^BMCREF(BMCREF,0),BMCPG=0,BMCDFN=$P(BMCR0,U,3)
 D @("HEAD"_(2-($E(IOST,1,2)="C-")))
 S BMCQUIT=0,BMCEDOS=""  ;BMC*4.0*16
...
DEMO ;Demographic Data
 ...
 ;I $P($G(^BMCPARM(DUZ(2),4100)),U,2)="" S X="SSN:  "_"",C=0,T=9,N=1 D W Q:BMCQUIT   ;BMC*4.0*14 IHS.OIT.FCJ REMOVE SSN
 ;I $P($G(^BMCPARM(DUZ(2),4100)),U,2)="Y" S X="SSN:  "_$$VAL^XBDIQ1(2,$P(BMCR0,U,3),.09),C=0,T=9,N=1 D W Q:BMCQUIT  ;BMC*4.0*14 IHS.OIT.FCJ REMOVE SSN
 ;BMC*4.0*14 IHS.OIT.FCJ CHANGED N=1 IN NXT LINE TO PERFORM FORM FEED THAT WAS BEING DONE IN SSN LINE ABOVE
 S X="Sex:  "_$$VAL^XBDIQ1(2,$P(BMCR0,U,3),.02),N=1,T=56,C=0 D W Q:BMCQUIT
 ...
 S X="Cell Phone: "_$$VAL^XBDIQ1(2,$P(BMCR0,U,3),.134) S C=0,T=49,N=1 D W Q:BMCQUIT     ;BMC*4.0*15 IHS.OIT.FCJ CELL#
 ;
REFTO ;
 D L
 ...
 ;S X="Referral: "_$$VAL^XBDIQ1(90001,BMCREF,.02),N=0,C=0,T=40 D W Q:BMCQUIT      ;BMC*4.0*15 MV TAB
 S X="Referral: "_$$VAL^XBDIQ1(90001,BMCREF,.02)_$$CALLIN^BMCRLU(BMCREF) S N=1,C=0,T=40 D W Q:BMCQUIT      ;BMC*4.0*16
DATE ;
 ...
 ;S X="Expected Ending Date: "_$$VAL^XBDIQ1(90001,BMCREF,1107),C=0,T=40,N=0 D W Q:BMCQUIT  ;BMC*4.0*16
 S BMCEDOS=$$AVEOS^BMCRLU(BMCREF,"E") I BMCEDOS'="" S X="Ending Date: "_BMCEDOS,C=0,T=40,N=0 D W Q:BMCQUIT  ;BMC*4.0*16
...
DX ;BMC*4.0*16 PRINT DX AND PRV NAR
 ...
 .I CT=1 S X="ICD Diagnosis: "_$P($$ICDDX^ICDEX(BMCD,BMCDOS,,"I"),U,2)_" - "_$E($P($$ICDDX^ICDEX(BMCD,BMCDOS,,"I"),U,4),1,50),C=0,T=0,N=1 D W Q
 .E  S X=$P($$ICDDX^ICDEX(BMCD,BMCDOS,,"I"),U,2)_"-"_$E($P($$ICDDX^ICDEX(BMCD,BMCDOS,,"I"),U,4),1,50),C=0,T=15,N=1 D W Q
PERTMED ;
 S BMCCMT=0
...
LINE ;CHS Supervisor Signature (if Type=CHS)
 ;G:$P(BMCR0,U,4)'="C" ROUT     ;BMC*4.0*16
 G:$P(BMCR0,U,4)'="C" CASENT    ;BMC*4.0*16
 W !!!!!
 G:BMCFTYP="CI" ROUT
 S X="____________________",C=0,T=50,N=1 D W Q:BMCQUIT
 W !
 ;S X="Contract Health Service Office",C=0,T=50,N=1 D W Q:BMCQUIT  ;BMC*4.0*16
 S X="Purchase Referred Care",C=0,T=50,N=1 D W Q:BMCQUIT    ;BMC*4.0*16
CASENT ;PRINT CASE NOTES
 D:$G(BMCPCSE) PRT^BMCFPRNN ;BMC*4.0*16 PRINT CASE NOTES
ROUT ;Print Routing slip
 I $G(BMCPROUT)=1 W # D PRINT^BMCFDRS  ;BMC*4.0*16 IHS.OIT.FCJ CHANGED TO $G
...
HEAD2 ;
 I 'BMCPG S BMCPG=BMCPG+1 Q
 ;S BMCPG=BMCPG+1 I '$G(PRTGUI) W:$D(IOF) @IOF W !,?(IOM-20),"Page ",BMCPG  ;BMC*4.0*16-GUI will print Page#
 S BMCPG=BMCPG+1 W:$D(IOF) @IOF W !,?(IOM-20),"Page ",BMCPG    ;BMC*4.0*16
