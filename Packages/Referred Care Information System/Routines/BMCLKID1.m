BMCLKID1 ; IHS/PHXAO/TMJ - IDENTIFIERS FOR REFERRAL LOOKUP 2 ;    [ 09/27/2006  1:37 PM ]
 ;;4.0;REFERRED CARE INFO SYSTEM;**1,15,16**;JAN 09, 2006;Build 168
 ...
 ;
 ;4.0*1 4.9.06 IHS/OIT/FCJ ADDED NUMBER COUNT DISPLAY FOR CALLED BY BMCLKID2 AND FIXED THE UNKNOWN SERVICE DATE PRINT
...
START ; EXTERNAL ENTRY POINT -
 ...
 ;W ?13,$P(^BMCREF(BMCRIEN,0),U,2),$P($G(^BMCREF(BMCRIEN,1)),U)," "    ;BMC*4.0*16
 W ?13,$P(^BMCREF(BMCRIEN,0),U,2),$P($G(^BMCREF(BMCRIEN,1)),U) W:$P($G(^BMCREF(BMCRIEN,1)),U)="" $$CALLIN^BMCRLU(BMCRIEN) W " "   ;BMC*4.0*16
 W " "    ;BMC*4.0*16
 ;S BMCPAT=$P(^DPT(BMCDFN,0),U) W:'$G(BMCFLG) $E(BMCPAT,1,15)," "    ;BMC*4.0*15
 S BMCRFAC=$$FACREF^BMCRLU(BMCRIEN) W ?30,$E($S(BMCRFAC'="":BMCRFAC,1:"UNKNOWN"),1,30)  ;BMC*4.0*15 CHG TAB TO 30
