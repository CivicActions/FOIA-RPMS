BMCMOD ; IHS/PHXAO/TMJ - MODIFY A REFERRAL 1 OF 2 ;           [ 08/22/2006  11:07 AM ]
 ;;4.0;REFERRED CARE INFO SYSTEM;**2,3,6,8,9,12,15,16**;JAN 09, 2006;Build 168
 ;
 ...
 ;4.0*3 10.1.2007 IHS/ITSC/FCJ ADDED OPT 16 ALERT TO PHYS
 ;4.0*12 IHS/OIT/FCJ CHANGED TEMPLATES FOR CALL IN TYPE
 ;4.0*15 IHS/OIT/FCJ CHANGED LOOKUP VALUES FOR PNN AND CHG BO/CHS COMM TO REFERRAL NOTES
 ;4.0*16 IHS/OIT/FCJ CHG BO/CHS COMM TO REFERRAL NOTES
 ;
...
MAIN ;
 ...
 Q:BMCQ
 I $G(BMCDTYPE)="S" D @BMCDTYPE Q    ;BMC*4.0*15
...
REFERRAL ; GET REFERRAL TO MODIFY
 I BMCCLOSE=1 D SCREEN Q   ;CLOSED REF
...
GETREF ;Screens out closed Referrals
 ...
 S BMCQ=1
 W !
 ;S DIC="^BMCREF(",DIC("S")="I $$FILTER^BMCFLTR(0,BMCCURFY,BMCRTY)",DIC(0)="AEMQ",DIC("A")="Select RCIS REFERRAL by Patient or by Referral Date or #: "  ;BMC*4.0*15
 S DIC="^BMCREF(",DIC("S")="I $$FILTER^BMCFLTR(0,BMCCURFY,BMCRTY)",DIC(0)="AEMQS",DIC("A")="Select RCIS REFERRAL by Patient or by Referral Date or #: "  ;BMC*4.0*15
 D DIC^BMCFMC
 Q:Y<1
 S BMCRIEN=+Y
 S BMCQ=0
...
SCREEN ;Display ONLY CLOSED Referrals
 S BMCQ=1
 W !
 ;S DIC="^BMCREF(",DIC("S")="I $$FILTER^BMCFLTR(1,BMCCURFY,0)",DIC(0)="AEMQ",DIC("A")="Select RCIS REFERRAL by Patient or by Referral Date or #: "  ;BMC*4.0*15
 S DIC="^BMCREF(",DIC("S")="I $$FILTER^BMCFLTR(1,BMCCURFY,0)",DIC(0)="AEMQS",DIC("A")="Select RCIS REFERRAL by Patient or by Referral Date or #: "  ;BMC*4.0*15
 D DIC^BMCFMC
 Q:Y<1
 S BMCRIEN=+Y
 S BMCQ=0
...
TYPE ; EP - Get type of data to edit
 S BMCQ=1,BMCDTYPE=""
 ;BMC*4.0*9 CHNG ICD9 TO ICD IN NXT LINE;BMC*4.0*15 CHNG BUSINESS OFFICE TO REFERRAL NOTES IN NXT LINE
 S DIR(0)="SO^0:QUIT;1:MINI MOD;2:ALL DATA;3:DATE/COUNTS;4:COSTS;5:ICD DX;6:CPT PROCEDURES;7:CASE REVIEW;8:PURPOSE/MED HX/OTHER DX INFO;9:REFERRAL NOTES;10:DISCHARGE NOTES;11:ADD DOCUMENTATION;12:CHS ELIG FACTORS;13:OTHER REFERRALS"
...
9 ; LABEL FOR @DO
 ...
 ;S BMCV="COM",BMCTERM="Business Office Comment",BMCATEMP="[BMC COMMENTS ADD]",BMCG="^BMCCOM(",BMCETEMP="[BMC COMMENTS EDIT]"  ;BMC*4.0*15
 S BMCV="COM",BMCTERM="Referral Notes",BMCATEMP="[BMC COMMENTS ADD]",BMCG="^BMCCOM(",BMCETEMP="[BMC COMMENTS EDIT]"   ;BMC*4.0*15
 S BMCCTYP="B"
 D ASK
 Q
 ;
...
12 ; LABEL FOR @DO
 ...
 D DDS^BMCFMC
 ;S BMCV="COM",BMCTERM="Business Office Comments",BMCATEMP="[BMC COMMENTS ADD]",BMCG="^BMCCOM(",BMCETEMP="[BMC COMMENTS EDIT]"  ;BMC*4.0*16
 S BMCV="COM",BMCTERM="Referral Notes",BMCATEMP="[BMC COMMENTS ADD]",BMCG="^BMCCOM(",BMCETEMP="[BMC COMMENTS EDIT]"     ;BMC*4.0*16
 S BMCCTYP="B"
 D ASK
 Q
...
SNO ;EP FROM BMCMODS;BMC*4.0*9 NEW MODULE TO ADD SNOMED FOR SITES W/O PCC
 ...
 K DIE,DR,X,DA
 Q
SCHED ; SCHEDULING DATA ;BMC4.0*15 NEW MODULE FOR FOR PNN LOOKUP
S ; LABEL FOR @DO
 S DIE="^BMCREF(",DA=BMCRIEN,DR="[BMC MOD SCHEDULING]"
 D DIE^BMCFMC
 Q
 ;
EOJ ; END OF JOB
