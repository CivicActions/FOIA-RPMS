BMCADD1 ; IHS/PHXAO/TMJ - add a referral part 2 ;
 ;;4.0;REFERRED CARE INFO SYSTEM;**3,9,10,15,16**;JAN 09, 2006;Build 168
 ;
 ...
 ;4.0*16 12.15.2023 IHS.OIT.FCJ ADD "C1" SUFFIX FOR CALLIN AND CHECK FOR DUP
 ;
...
MEDHX ; GET PROVISIONAL DIAGNOSES
 W:$D(IOF) @IOF
 ;W !?5,"Referral #: ",$$GETR^BMC    ;BMC*4.0*16
 W !?5,"Referral #: ",$$GETR^BMC W:$G(BMCCDT)'="" "C1"    ;BMC*4.0*16
 W !?5,"Referral Date: " S Y=$P(^BMCREF(BMCRIEN,0),U) D DD^%DT W Y
 ;W ?40,"Patient Name: ",$P(^DPT(BMCDFN,0),U)    ;BMC*4.0*15
 W ?38,"Patient: ",BMCREC("PAT NAME")            ;BMC*4.0*15 PNN  
 W !!
...
STATIC ;EP - STORE STATIC DATA
 ...
 ;W !,"Entry of Referral ",$P(^BMCREF(BMCRIEN,0),U,2)," is complete.",!   ;BMC*4.0*16
 W !,"Entry of Referral ",$P(^BMCREF(BMCRIEN,0),U,2) W:$G(BMCCDT)'="" "C1" W " is complete.",!    ;BMC*4.0*16
...
REFSNO ;EP FR BMCMODS AND BMCADDS;BMC*4.0*9 ADD SNOMED CODE FOR SITE W/O PCC
 ...
 K DIC,DA
 Q
CHKDGUI(BMCRIEN) ;GPRINT(BMCREF,BMCPCSE) ;EP print referral form - GUI
 S BMCGUI=1
CHKDUP ;EP FR BMCADD;CHECKDUP PATIENT ;BMC*4.0*16
 ...
 S BMCTPRV=$P(^BMCREF(BMCRIEN,0),U,6),BMCTVEN=$P(^BMCREF(BMCRIEN,0),U,7),BMCTPRI=$P(^BMCREF(BMCRIEN,0),U,32)
 ...
 ..Q:BMCTPRI'=$P(^BMCREF(BMCRIEN,0),U,32)
 ..S BMCDUP(BMCTIEN)="",BMCDUP=BMCDUP+1
 Q:$G(BMCGUI)
 I BMCDUP>0 D
 .W @IOF,!,"The following Referral(s) match on Referral Type, Vendor, Referring Provider,",!,"Visit Type and Priority:"
 .S BMCTIEN=0 F  S BMCTIEN=$O(BMCDUP(BMCTIEN)) Q:BMCTIEN'?1N.N  D
 ..W !?5,$P(^BMCREF(BMCTIEN,0),U,2)
 .W !,"Referral: ",BMCRNUMB," maybe a possible duplicate Referral."
 .S DIR("A")="Do you wish to continue with adding this referral",DIR(0)="Y",DIR("B")="NO"
 .D ^DIR
 K BMCTDT,BMCTIEN,BMCTPRV,BMCTVEN,BMCTPRI,BMCDUP,DIR
 S BMCDUP=Y
 Q
