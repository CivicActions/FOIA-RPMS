...
BOMED ;EP ;TEST FOR SCREEN ON BUSINESS OFFICE COMMENTS AND PRINTING MED HX 
 ...
 K Y,B,Z Q
 ;
SNO ;EP-PRINT SNOMED;BMC*4.01*16
 N VREF,SNO,BMCSTRM
 S VREF=$P(^BMCREF(BMCREF,13),U,3) I 'VREF S BMCPRNT="-" Q
 S SNO=$P($G(^AUPNVREF(VREF,0)),U)
 S BMCSTRM=$P($$CONC^BSTSAPI(SNO_"^^^1"),U,2)
 S BMCPRNT=SNO_" "_$S($G(BMCCSV):""""_BMCSTRM_"""",1:BMCSTRM)
 Q
