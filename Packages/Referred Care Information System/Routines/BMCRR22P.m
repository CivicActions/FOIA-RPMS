BMCRR22P ; IHS/PHXAO/TMJ - DETAILED OLOG REPORT ;      
 ;;4.0;REFERRED CARE INFO SYSTEM;**9,14,15,16**;JAN 09, 2006;Build 168
 ;IHS/ITSC/FCJ PRINTS COMMENTS FROM RCIS COMMENT FILE
 ;IHS/ITSC/FCJ MODIFIED PRINT FORMAT AND ADDED SR SUF
 ;BMC*4.0*9 11.11.2012 IHS.OIT.FCJ ADDED ICD-10 CHANGE
...
BUILD ; build array
 ...
 S BMCSTR="",BMCCTR=0
 ;W "Name: "_$E($P(^DPT($P(BMCRREC,U,3),0),U),1,25),?32,$$FMTE^XLFDT(AUPNDOB,"5D")_" "_$$VAL^XBDIQ1(9000001,DFN,1102.98)_" "_$$SSN^AUPNPAT(DFN)  ;BMC*4.0*14 REMOVE SSN
 W "Name: "_$E($P(^DPT($P(BMCRREC,U,3),0),U),1,25),?32,$$FMTE^XLFDT(AUPNDOB,"5D")_" "_$$VAL^XBDIQ1(9000001,DFN,1102.98)  ;BMC*4.0*14 REMOVE SSN
 ;W ?60,"Ref:",$$VAL^XBDIQ1(90001,BMCREF,.02),$P($G(^BMCREF(BMCREF,1)),U)        ;BMC*4.0*16
 W ?58,"Ref:",$$VAL^XBDIQ1(90001,BMCREF,.02),$P($G(^BMCREF(BMCREF,1)),U),$$CALLIN^BMCRLU(BMCREF)         ;BMC*4.0*16
...
DRG ;
 ...
 D VFILES
2 ;REFERRAL NOTES  ;BMC*4.0*15 chg business office comments
 I '$D(^BMCCOM("AD",BMCREF)) Q
 ;S BMCCTYP="B",BMCSTR="Business Office Notes: " D COMMENTS Q:$D(BMCQUIT)   ;BMC*4.0*15
 S BMCCTYP="B",BMCSTR="Referral Notes: " D COMMENTS Q:$D(BMCQUIT)   ;BMC*4.0*15
3 ;
...
VF3 ;
 ...
 ;4.0*9 11.11.2012 IHS.OIT.FCJ ADDED ICD-10 2 NEW LINES
