BMCMODS ;IHS/ITSC/FCJ - EDIT SECONDARY REFERRAL ;   
 ;;4.0;REFERRED CARE INFO SYSTEM;**3,6,8,9,15**;JAN 09, 2006;Build 168
 ;
 ; This option allows the user to EDIT Secondary Referrals
 ;   BMCRIEN=Primary referral
 ;   BMCSRIEN=Secondary referral ien
 ;   BMCVCT=Vist count
 ;BMC*4.0 ADDED THE BMCMODE VARIABLE AND TYPE OF REF TO EDIT
 ;BMC*4.0*3 8.13.07 IHS.OIT.FCJ ADD BO/CHS COMMENTS
 ;BMC*4.0*3 12.10.2007 IHS.OIT.FCJ ADDED EP LABEL TO LINES EDIT AND MEDHX
 ;BMC*4.0*6 4.22.2010 IHS.OIT.FCJ MODIFIED TO ALLOW EDIT OF CLOSED REF
 ;BMC*4.0*15 4.20.2023 IHS.OIT.FCJ ADD PREFERRED NAME FOR SCREEN DISPLAYS AND CHG BO/CHS COM TO REFERRAL NOTES
 ;
START ;
...
GETREF ;
 ...
 S DIC="^BMCREF(",DIC(0)="AEMQS",DIC("A")="Select Secondary Referral by Patient Name or by Referral Date or #: "    ;BMC*4.0*15 ADDED "S" TO DIC(0)
...
MEDHX ;EP;DISPLAY MED HX COMMENTS IF ANY AND ADD NEW COMMENTS TO SEC REF
 ...
 ;W !,$$CTR^BMC("REFERRAL: "_$P(BMCREC,U,2)_"   PATIENT: "_$$VAL^XBDIQ1(90001,BMCSRIEN,.03),80),!     ;BMC*4.0*15
 W !,$$CTR^BMC("REFERRAL: "_$P(BMCREC,U,2)_"   PATIENT: "_$E($$GETPREF^AUPNSOGI($P(BMCREC,U,3),"E",1),1,37),80),!   ;BMC*4.0*15
 F I=1:1:80 W "-"
...
SBCOM ;ADD BO/CHS COMMENTS ;BMC*4.0*3 8.13.07 IHS.OIT.FCJ ADD BO/CHS COMMENTS
 ;S BMCV="COM",BMCTERM="Business Office/CHS Comments",BMCATEMP="[BMC COMMENTS ADD]",BMCG="^BMCCOM(",BMCETEMP="[BMC COMMENTS EDIT]"  ;BMC*4.0*15
