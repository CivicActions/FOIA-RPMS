BMCSPD ; IHS/PHXAO/TMJ - display site parameters ;    [ 09/27/2006  2:16 PM ]
 ;;4.0;REFERRED CARE INFO SYSTEM;**1,2,3,14,15,16**;JAN 09, 2006;Build 168
 ...
 ;BMC*4.0*14 4.14.2022 IHS.OIT.FCJ REMOVE SSN
 ;BMC*4.0*15 5.8.23 IHS.OIT.FCJ DISPLAY NEW PARAMETER-CLOSING AN INP DENIAL REFERRAL
 ;BMC*4.0*16 1.3.24 IHS.OIT.FCJ DISPLAY NEW PARAMETER-MINOR AGE
 ;
 ;
EN1 ;
 K BMCQUIT
...
PRINT ;
 ...
 S BMCI=4101 I $D(^DD(90001.31,BMCI)) S BMCX(BMCI)=$P(^DD(90001.31,BMCI,0),U)_U_$$VAL^XBDIQ1(90001.31,BMCPARM,BMCI)  ;4.0*3;4.0*14 IHS/OIT/FCJ
 S BMCI=4114 I $D(^DD(90001.31,BMCI)) S BMCX(4102.99)=$P(^DD(90001.31,BMCI,0),U)_U_$$VAL^XBDIQ1(90001.31,BMCPARM,BMCI)  ;4.0*3;4.0*15 IHS/OIT/FCJ
 F BMCI=4103:.01:4110 I $D(^DD(90001.31,BMCI)) S BMCX(BMCI)=$P(^DD(90001.31,BMCI,0),U)_U_$$VAL^XBDIQ1(90001.31,BMCPARM,BMCI)  ;4.0*3;4.0*14 IHS/OIT/FCJ
 S BMCI=4115 I $D(^DD(90001.31,BMCI)) S BMCX(BMCI)=$P(^DD(90001.31,BMCI,0),U)_U_$$VAL^XBDIQ1(90001.31,BMCPARM,BMCI)  ;4.0*3;4.0*16 IHS/OIT/FCJ
 F BMCI=1101:.01:1199 I $D(^DD(90001.31,BMCI)) S BMCX(BMCI)=$P(^DD(90001.31,BMCI,0),U)_U_$$VAL^XBDIQ1(90001.31,BMCPARM,BMCI)  ;4.0*14 IHS/OIT/FCJ RMD SSN
 ...
 S X=0 F  S X=$O(BMCX(X)) Q:X'=+X!($D(BMCQUIT))  D  Q:$D(BMCQUIT)
 ...
 S BMCI=0 F  S BMCI=$O(BMCX(1,BMCI)) Q:BMCI'=+BMCI  D  Q:$D(BMCQUIT)
 .I $Y>(IOSL-4) D FF Q:$D(BMCQUIT)
 .W !,BMCX(1,BMCI)
 .Q
HCDB ;
 ...
 S BMCI=0 F  S BMCI=$O(BMCX(BMCI)) Q:BMCI'=+BMCI  D  Q:$D(BMCQUIT)
 .I $Y>(IOSL-4) D FF Q:$D(BMCQUIT)
...
SSTXT ; SITE-SPECIFIC TEXT FOR REFERRAL LETTERS
 Q:$D(BMCQUIT)
 N BMCI1 F BMCI1=31:1:34 D  Q:$D(BMCQUIT)
 . K BMCX
 . D ENP^XBDIQ1(90001.31,BMCPARM,BMCI1_"01","BMCX(","E")
 . W:$D(^BMCPARM(BMCPARM,BMCI1)) !!,$P(^DD(90001.31,BMCI1_"01",0),U),":"
 . S BMCI=0 F  S BMCI=$O(BMCX(BMCI1_"01",BMCI)) Q:BMCI'=+BMCI  D  Q:$D(BMCQUIT)
 .. I $Y>(IOSL-4) D FF Q:$D(BMCQUIT)
 .. W !,BMCX(BMCI1_"01",BMCI)
 .. Q
 . Q
 K BMCI1
APTTXT ; SITE-SPECIFIC TEXT FOR REFERRAL LETTERS  ;BMC*4.0*16
 Q:$D(BMCQUIT)
 N BMCI1 F BMCI1=36,35 D  Q:$D(BMCQUIT)     ;
 .K BMCX
 . D ENP^XBDIQ1(90001.31,BMCPARM,BMCI1_"01","BMCX(","E")
 . W !!,$S(BMCI1=35:"OUTPATIENT Appointment Letter Text:",BMCI1=36:"INPATIENT Appointment Letter Text:",1:"")
 . S BMCI=0 F  S BMCI=$O(BMCX(BMCI1_"01",BMCI)) Q:BMCI'=+BMCI  D  Q:$D(BMCQUIT)
 .. I $Y>(IOSL-4) D FF Q:$D(BMCQUIT)
 .. W !,BMCX(BMCI1_"01",BMCI)
 .. Q
