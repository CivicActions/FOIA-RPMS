BMC ; IHS/PHXAO/TMJ - REFERRED CARE INFO SYSTEM ;     
 ;;4.0;REFERRED CARE INFO SYSTEM;**5,6,15,16**;JAN 09, 2006;Build 168
 ;IHS/ITSC/FCJ CHG TST FOR IOT FOR VIR TRM
 ;BMC*4.0*6 2.16.2010 IHS.OIT.FCJ ADDED PATCH NUMBER PRINT IN LOGO SUB
 ;BMC*4.0*15 4.4.23 IHS.OIT.FCJ ADDED UPDATE TO REF# FOR FY
 ;BMC*4.0*16 12.8.23 IHS.OIT.FCJ ADDED UPDATE TO REF# FOR FY JAN START DATE;MINOR PARAMETER
 ;
 ;
...
PARMCHK ;EP - Check RCIS SITE PARAMETER file
 ...
 ;I BMCRY'=BMCFY W !!,*7,"RCIS SITE PARAMETER file REFERRAL YEAR does not match current FISCAL YEAR",!,"IGNORE If Operating on Calendar Year Basis..",! H 2 ;BMC*4.0*15
...
PARMSET ;EP - SET SYSTEM WIDE VARIABLES FROM SITE PARAMETER FILE
 ...
 S BMCMNOR=$P(^BMCPARM(DUZ(2),4100),U,15) S:BMCMNOR="" BMCMNOR=18   ;BMC*4.0*16; NEW MINOR PARAMETER
 ; set taxonomy ien's
...
BMCFYRY ; calculate current fiscal year and referral year
 ;ST OF CHG;BMC*3.1*16
 ;S BMCGFY=$P($$FISCAL^XBDT(DT,10),U)
 ;S BMCFY=$E(BMCGFY,3,4)
 ;SET THE MONTH... IF MONTH=1 SUBSTRACT 1 FROM THE FY.
 S MTH=$P($G(^APCCCTRL(DUZ(2),0)),U,8)
 S BMCGFY=$P($$FISCAL^XBDT(DT,MTH),U),BMCFY=$E(BMCGFY,3,4)
 I MTH=1 S BMCFY=BMCFY-1
 ;END OF CHG
 S BMCRY=$P(BMCPARM,U,2)
 ;START OF CHANGES ;BMC*4.0*15
 Q:BMCRY=BMCFY
 LOCK +^BMCPARM(DUZ(2)):20 E  W:'$D(ZTQUEUED) *7,!!,"  Unable to lock the RCIS SITE PARAMETER entry for ",$$LOC,".",!! D EOP Q 0
 S BMCPARM=$G(^BMCPARM(DUZ(2),0))
 S X=BMCFY
 S DIE="^BMCPARM(",DA=DUZ(2),DR=".02////"_X D DIE^BMCFMC
 LOCK -^BMCPARM(DUZ(2)):20
 ;END OF CHANGES ;BMC*4.0*15
 Q
 ;----------
...
REFN() ;EP - Return the next referral number and update control file
 N FY,REFN ;BMC*4.0*15
 LOCK +^BMCPARM(DUZ(2)):20 E  W:'$D(ZTQUEUED) *7,!!,"  Unable to lock the RCIS SITE PARAMETER entry for ",$$LOC,".",!! D EOP Q 0
 S BMCPARM=$G(^BMCPARM(DUZ(2),0))
 S X=$$ASF
 S X=X_$P(BMCPARM,U,2)
 S (Y,REFN)=$P(BMCPARM,U,7)+1
 S X=X_$$LZERO(Y,5)
 S BMCX=X
 S DIE="^BMCPARM(",DA=DUZ(2),DR=".07////"_Y D DIE^BMCFMC
 ;SET FY AND REF NUMBER;ADDED NXT 4 LINES ;BMC*4.0*15
 S FY=$P(BMCPARM,U,2)
 I '$D(^BMCPARM(DUZ(2),3,FY)) D
 .S DIC("P")=$P(^DD(90001.31,3,0),U,2),(DIC,DIE)="^BMCPARM("_DUZ(2)_",3,",DA(1)=DUZ(2),(DA,X)=FY,DR=".01////"_FY D DIE^BMCFMC
 S DIE="^BMCPARM("_DUZ(2)_",3,",DA(1)=DUZ(2),DA=FY,DR=".02////"_REFN D DIE^BMCFMC
 ;END OF CHANGES BMC*4.0*15
 LOCK -^BMCPARM(DUZ(2)):20
 Q BMCX
 ;----------
REFNFY() ;EP - Get Referral Number for Desired Fiscal Year
 ;ADDED NXT 4 LINES ;BMC*4.0*15
 LOCK +^BMCPARM(DUZ(2)):20 E  W:'$D(ZTQUEUED) *7,!!,"  Unable to lock the RCIS SITE PARAMETER entry for ",$$LOC,".",!! D EOP Q 0
 I '$D(^BMCPARM(DUZ(2),3,+BMCFY)) D
 .D REFNFND
 .S DIC(0)="L",DLAYGO=90001.31,DIC("P")=$P(^DD(90001.31,3,0),U,2),(DIC,DIE)="^BMCPARM("_DUZ(2)_",3,",DA(1)=DUZ(2),(DA,X)=+BMCFY
 .S DIC("DR")=".02////"_BMCRNUM D DIC^BMCFMC
 S BMCRNUM=$P(^BMCPARM(DUZ(2),3,+BMCFY,0),U,2)+1
 S DIE="^BMCPARM("_DUZ(2)_",3,",DA(1)=DUZ(2),DA=+BMCFY,DR=".02////"_BMCRNUM D DIE^BMCFMC
 LOCK -^BMCPARM(DUZ(2)):20
 ;END OF CHANGES BMC*4.0*15
 ;
 S X=$$ASF
 ...
 Q BMCX
 ;
REFNFND ;FIND LAST REF NUMBER FOR A GIVEN FY ;BMC*4.0*15 NEW SUB
 S (X,BMCRNUM)=BMCSTART_"00000"
 F  S X=$O(^BMCREF("C",X)) Q:X'?1N.N  D
 .S BMCRNUM=X
 S BMCRNUMB=+$E(BMCRNUM,9,13)
 Q
 ;
