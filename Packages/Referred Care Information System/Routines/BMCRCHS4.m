BMCRCHS4 ; IHS/ITSC/FCJ - STATUS REPORT FOR CHS REFERRALS;      [ 09/27/2006  2:21 PM ]
 ;;4.0;REFERRED CARE INFO SYSTEM;**2,9,16**;JAN 09, 2006;Build 168
 ;4.0 IHS/ITSC/FCJ ADDED SUFFIX AND REF
 ;Menu option - CHSR   CHS Status Report for Referrals
...
INIT ; INITIALIZAION
 ...
 S BMCJOB=$J
 S BMCBT=$H           ;BMC*4.0*16
 ;F  D  Q:BMCBT]""    ;BMC*4.0*16
 ;. S BMCBT=$H
 ;. LOCK +^XTMP("BMCRCHS4",BMCJOB,BMCBT):1
 ;. E  S BMCBT=""
 ;K ^XTMP("BMCRCHS4",BMCJOB,BMCBT)    ;BMC*4.0*16 
 K ^XTMP("BMCRCHS4",BMCJOB)           ;BMC*4.0*16 
 Q
 ;
DBQUE ;call to XBDBQUE
 ...
 S DIR(0)="S^P:PRINT Output;B:BROWSE Output on Screen;C:CSV Output",DIR("A")="Do you wish to",DIR("B")="P"  ;BMC*4.0*16
 W ! D ^DIR K DIR  ;BMC*4.0*16
 I $D(DIRUT) S BMCQUIT=1 Q
 S BMCOPT=Y
 I $G(BMCOPT)="B" D BROWSE,EOJ Q
 S XBRP="REFPRT^BMCRCHS4",XBRC="REFCHK^BMCRCHS4",XBRX="EOJ^BMCRCHS4",XBNS="BMC"
 I $G(BMCOPT)="C" D XPRTCSV,EOJ Q   ;BMC*4.0*16
 D ^XBDBQUE
 Q
 ;
BROWSE ;
...
PRINT ;print one referral
 ...
 W @$S(BMCCSV:"BMCSPRTR",1:"?11"),$S(BMCCSV:""""_BMCREC("PAT NAME")_"""",1:$E(BMCREC("PAT NAME"),1,18))
 W @$S(BMCCSV:"BMCSPRTR",1:"?30"),$P(BMCRREC,U,2)
 W:$D(^BMCREF(BMCRIEN,1)) $P(^BMCREF(BMCRIEN,1),U)
 W:$P($G(^BMCREF(BMCRIEN,1)),U)="" $$CALLIN^BMCRLU(BMCRIEN)         ;BMC*4.0*16
 W @$S(BMCCSV:"BMCSPRTR",1:"?46"),$P(^BMCREF(BMCRIEN,11),U,12)_"/"_$P(BMCRREC,U,15)
 W @$S(BMCCSV:"BMCSPRTR",1:"?51"),$S($P(BMCRREC,U,6):$$PROVINI^XBFUNC1($P(BMCRREC,U,6)),1:"--")
 W @$S(BMCCSV:"BMCSPRTR",1:"?55"),$S(BMCCSV:""""_$$TOFAC^BMC(BMCRIEN)_"""",1:$E($$TOFAC^BMC(BMCRIEN),1,25))
 W !
 ...
 .I BMCCSV W BMCSPRTR,""""_^UTILITY($J,"W",DIWL,Z,0)_"""",!
 .E  W ?10,^UTILITY($J,"W",DIWL,Z,0),!
 K DIWL,DIWR,DIWF,Z
 K ^UTILITY($J,"W")
 W !
 Q
...
HEAD2 ; WRITE HEADER
 ...
 W $$CTR^BMC("CHS REFERRALS: "_$S(BMCCSV:$TR(BMCRTYP,","," "),1:BMCRTYP),80),!
 W $$CTR^BMC("REFERRALS STATUS: "_BMCRTYPS,80),!
 ;BMC*4.0*16 CHG NXT LINE TO IF FOR CSV FORMAT
 ;W !,"REF DATE",?11,"PATIENT NAME",?32," REF #",?44,"STATUS",?51,"PRV",?55,"FACILITY REF TO"
 I BMCCSV W !,"REF DATE,PATIENT NAME",BMCSPRTR_"REF #",BMCSPRTR,"STATUS",BMCSPRTR,"PRV",BMCSPRTR,"FACILITY REF TO"
 E  W !,"REF DATE",?11,"PATIENT NAME",?32," REF #",?44,"STATUS",?51,"PRV",?55,"FACILITY REF TO"
 W !,BMC80D
 W !
 Q
 ;
EOJ ; END OF JOB
 ;LOCK -^XTMP("BMCRCHS4",BMCJOB,BMCBT) ;BMC*4.0*16
 K ^XTMP("BMCRCHS4",$J)
 D ^BMCKILL
