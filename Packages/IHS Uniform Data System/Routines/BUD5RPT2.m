BUD5RPT2 ; IHS/CMI/LAB - UDS REPORT DRIVER ;
...
EN1 ;
 S BUDSITE=""
 W ! S DIC="^BUDVSITE(",DIC(0)="AEMQ",DIC("A")="Enter your site: " D ^DIC
 I Y=-1 D EOJ Q
 S BUDSITE=+Y
 I '$O(^BUDVSITE(BUDSITE,11,0)) W !!,"Warning:  There are no locations defined in the site parameter file for this",!,"site.  Report will not be accurate!" G EN
 D TAXCHK^BUDTXCH
 D YEAR
 ...
 I $D(DIRUT) D EOJ Q
 I Y D EOJ Q
 ;S XBRP="PRINT^BUD5RPTP",XBRC="PROC^BUD5RPTC",XBRX="EOJ^BUD5RPT2",XBNS="BUD"
 ;D ^XBDBQUE
 ;D EOJ
ZIS ;call to XBDBQUE
 K IOP,%ZIS
...
TSKMN ;EP ENTRY POINT FROM TASKMAN
 ...
 S ZTCPU=$G(IOCPU),ZTRTN="DRIVER^BUD5RPT2",ZTDTH="",ZTDESC="UDS 05 REPORT" D ^%ZTLOAD D EOJ Q
 Q
M ;EP - called from option
 ...
 W !?5,"6   Center/Grantee Profile:  User/Patient by Zip Code"
 W !?5,"7   ALL Tables"
 S DIR(0)="L^1:7",DIR("A")="Include which Lists",DIR("B")=6 KILL DA D ^DIR KILL DIR
 I $D(DIRUT) D EOJ Q
...
YEAR ;
 S BUDYEAR=""
 W !
 W !,"Enter the Calendar Year.  Use a 4 digit year, e.g. 2003, 2005"
 S DIR(0)="D^::EP"
