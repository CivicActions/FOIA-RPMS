BUD7RPL1 ; IHS/CMI/LAB - UDS print lists ;
 ;;2.0;IHS/RPMS UNIFORM DATA SYSTEM;**4**;DEC 29, 2004
 ;
 ;
TOL ;EP
 I '$D(^XTMP("BUD7RPT1",BUDJ,BUDH,"ORPHANS")) D T6H W !!,"No Incomplete visits to report.",!! Q
 S BUDP=0
 S BUD7L="" F   S BUD7L=$O(^XTMP("BUD7RPT1",BUDJ,BUDH,"ORPHANS",BUD7L)) Q:BUD7L=""!(BUDQUIT)  D
 .D T6H Q:BUDQUIT
 .W !!,"Line ",BUD7L,"   ",$P($T(@BUD7L),";;",2)
 .S BUDCOM="" F  S BUDCOM=$O(^XTMP("BUD7RPT1",BUDJ,BUDH,"ORPHANS",BUD7L,BUDCOM)) Q:BUDCOM=""!(BUDQUIT)  D
 ..S BUDAGE="" F  S BUDAGE=$O(^XTMP("BUD7RPT1",BUDJ,BUDH,"ORPHANS",BUD7L,BUDCOM,BUDAGE)) Q:BUDAGE=""!(BUDQUIT)  D
 ...S BUDSEX="" F  S BUDSEX=$O(^XTMP("BUD7RPT1",BUDJ,BUDH,"ORPHANS",BUD7L,BUDCOM,BUDAGE,BUDSEX)) Q:BUDSEX=""!(BUDQUIT)  D
 ....S DFN=0 F  S DFN=$O(^XTMP("BUD7RPT1",BUDJ,BUDH,"ORPHANS",BUD7L,BUDCOM,BUDAGE,BUDSEX,DFN)) Q:DFN'=+DFN!(BUDQUIT)  D T6W
 ....Q
 ...Q
 ..Q
 .Q
...
T6W ;
 ...
 W ?55,$$AGE^AUPNPAT(DFN,BUDCAD),?60,$P($$RACE^BUD7RPTC(DFN),U,2)
 S BUDV=0 F  S BUDV=$O(^XTMP("BUD7RPT1",BUDJ,BUDH,"ORPHANS",BUD7L,BUDCOM,BUDAGE,BUDSEX,DFN,BUDV)) Q:BUDV'=+BUDV!(BUDQUIT)  D
 .I $Y>(IOSL-3) D T6H Q:BUDQUIT  W !!,"Line ",BUD7L,"   ",$P($T(@BUD7L),";;",2)
 .W !?5,$$FMTE^XLFDT($P($P(^AUPNVSIT(BUDV,0),U),".")),?25,^XTMP("BUD7RPT1",BUDJ,BUDH,"ORPHANS",BUD7L,BUDCOM,BUDAGE,BUDSEX,DFN,BUDV),?40,$P(^AUPNVSIT(BUDV,0),U,7),?45,$E($$CLINIC^APCLV(BUDV,"E"),1,14),?60,$E($$LOCENC^APCLV(BUDV,"E"),1,14)
 .Q
 Q
T6H ;
 G:'BUDGPG T6H1
