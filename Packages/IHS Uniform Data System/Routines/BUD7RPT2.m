BUD7RPT2 ; IHS/CMI/LAB - UDS REPORT DRIVER 02 Dec 2007 11:27 AM ; 
...
EN1 ;
 S BUDSITE=""
 W ! S DIC="^BUDESITE(",DIC(0)="AEMQ",DIC("A")="Enter your site: " D ^DIC
 I Y=-1 D EOJ Q
 S BUDSITE=+Y
 ...
 I $D(DIRUT) D EOJ Q
 I Y D EOJ Q
 ;S XBRP="PRINT^BUD7RPTP",XBRC="PROC^BUD7RPTC",XBRX="EOJ^BUD7RPT2",XBNS="BUD"
 ;D ^XBDBQUE
 ;D EOJ
ZIS ;call to XBDBQUE
 K IOP,%ZIS
...
TSKMN ;EP ENTRY POINT FROM TASKMAN
 ...
 S ZTCPU=$G(IOCPU),ZTRTN="DRIVER^BUD7RPT2",ZTDTH="",ZTDESC="UDS 07 REPORT" D ^%ZTLOAD D EOJ Q
 Q
M ;EP - called from option
 ...
 W !!?5,"1   Center/Grantee Profile:  Patients by Zip Code"
 W !?5,"2   Patients by Age, Gender, Race (Tables 3A & 3B)"
 W !?5,"3   All patients by Service Category (Table 5 col B&C)"
 W !?5,"4   Provider/Staff List (Table 5 col A)"
 W !?5,"5   All visits with Uncategorized Providers (Table 5)"
 W !?5,"6   All patients w/Multiple Visits in Svc Cats (Table 5 col B&C)"
 W !?5,"7   All Patients by Selected Primary Diagnosis (Table 6)"
 W !?5,"8   ALL Tables"
 S DIR(0)="L^1:8",DIR("A")="Include which Lists",DIR("B")=1 KILL DA D ^DIR KILL DIR
 I $D(DIRUT) D EOJ Q
 I Y[8 S (BUDT3A,BUDT3B,BUDT4,BUDT5,BUDT6,BUDT3AL,BUDT5L,BUDT5L1,BUDT5L2,BUDT6L,BUDTZ,BUDTZL,BUDT5L3)=1
 I Y[2 S (BUDT3A,BUDT3AL)=1
 I Y[3 S (BUDT5,BUDT5L)=1
 I Y[4 S (BUDT5,BUDT5L1)=1
 I Y[5 S (BUDT5,BUDT5L2)=1
 I Y[7 S (BUDT6,BUDT6L)=1
 I Y[1 S (BUDTZ,BUDTZL)=1
 I Y[6 S (BUDT5,BUDT5L3)=1
 G EN1
 Q
 ;
...
T3AI ;intro for table 3A
 W !!,"ALL PATIENTS BY AGE, GENDER & RACE (Tables 3A and 3B)"
...
TZAI ;intro for table Z
 W !!,"Center/Grantee Profile:  Patient by Zip Code"
...
T6I ;intro for table 6
 W !,"ALL PATIENTS BY PRIMARY DIAGNOSIS (Table 6)"
 W !,"This report lists all patients and related visits as defined above, categorized"
