BUD4RPC1 ; IHS/CMI/LAB - UDS TABLE 6 05 Dec 2006 5:39 AM ; [ 05 Dec 2006 5:39 AM ]
 ;;2.0;IHS/RPMS UNIFORM DATA SYSTEM;**4**;DEC 29, 2004
 ;
 ;
PT ;
 I $G(BUDT6L) S ^XTMP("BUD4RPT1",BUDJ,BUDH,"T6",C,BUDCOM,BUDAGE,BUDSEX,DFN,BUDV)=$E(BUDVAL,1,20)
PT1 S $P(BUDT6("V"),U,C)=$P(BUDT6("V"),U,C)+1
 ...
 Q
OT ;
 I $D(^XTMP("BUD4RPT1",BUDJ,BUDH,"T6",C,BUDCOM,BUDAGE,BUDSEX,DFN)) Q  ;patient already a hit on this line
 S ^XTMP("BUD4RPT1",BUDJ,BUDH,"ORPHANS",C,BUDCOM,BUDAGE,BUDSEX,DFN,BUDV)=$E(BUDVAL,1,20)
 Q
T6 ;EP
...
SERV ;
 ...
 ..S C=22 D PT1 S ^XTMP("BUD4RPT1",BUDJ,BUDH,"T6",C,BUDCOM,BUDAGE,BUDSEX,DFN,"WH","MAM",BUDW)=^TMP($J,"MAMMS",BUDW)
 ...
 ..S C=23 D PT1 S ^XTMP("BUD4RPT1",BUDJ,BUDH,"T6",C,BUDCOM,BUDAGE,BUDSEX,DFN,"WH","PAP",BUDW)=^TMP($J,"PAPS",BUDW)
...
PAP(BUDV) ;
 ...
 I G]"" Q "V LAB: "_G
 S T=$O(^ATXAX("B","BUD CPT PAP 04",0))
 S G="" I T S X=0 F  S X=$O(^AUPNVCPT("AD",BUDV,X)) Q:X'=+X!(G]"")  D
 .S Z=$P(^AUPNVCPT(X,0),U)
 .I $$ICD^ATXCHK(Z,T,1) S G=$P($$CPT^ICPTCOD(Z),U,2)
 .Q
...
IMM(BUDV) ;
 ...
 .S BUDTIEN=$O(^BUDFCNTL("B","IMMUNIZATIONS LINE 24",0))
 .I $D(^BUDFCNTL(BUDTIEN,11,"B",Z)) S G=Z_" imm"
 .Q
 Q G
HIV(BUDV) ;
...
MAMM(BUDV) ;
 ...
 .I Z=76091!(Z=76090)!(Z=76092) S G="V CPT: "_Z
 .Q
 Q G
