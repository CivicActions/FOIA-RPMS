BUD6RPT2 ; IHS/CMI/LAB - UDS REPORT DRIVER 02 Dec 2006 11:27 AM ; [ 02 Dec 2006 11:27 AM ]
...
EN1 ;
 S BUDSITE=""
 W ! S DIC="^BUDSSITE(",DIC(0)="AEMQ",DIC("A")="Enter your site: " D ^DIC
 I Y=-1 D EOJ Q
 S BUDSITE=+Y
 I '$O(^BUDSSITE(BUDSITE,11,0)) W !!,"Warning:  There are no locations defined in the site parameter file for this",!,"site.  Report will not be accurate!" G EN
 D TAXCHK^BUDTXCH
 D YEAR
 ...
 I $D(DIRUT) D EOJ Q
 I Y D EOJ Q
 ;S XBRP="PRINT^BUD6RPTP",XBRC="PROC^BUD6RPTC",XBRX="EOJ^BUD6RPT2",XBNS="BUD"
 ;D ^XBDBQUE
 ;D EOJ
ZIS ;call to XBDBQUE
 K IOP,%ZIS
...
TSKMN ;EP ENTRY POINT FROM TASKMAN
 ...
 S ZTCPU=$G(IOCPU),ZTRTN="DRIVER^BUD6RPT2",ZTDTH="",ZTDESC="UDS 06 REPORT" D ^%ZTLOAD D EOJ Q
 Q
M ;EP - called from option
 ...
 W !!?5,"1   Center/Grantee Profile:  User/Patient by Zip Code"
 W !?5,"2   Users by Age, Gender, Race (Tables 3A & 3B)"
 W !?5,"3   All users by Service Category (Table 5 col B&C)"
 W !?5,"4   Provider/Staff List (Table 5 col A)"
 W !?5,"5   All visits with Uncategorized Providers (Table 5)"
 W !?5,"6   All Users by Selected Primary Diagnosis (Table 6)"
 W !?5,"7   ALL Tables"
 S DIR(0)="L^1:7",DIR("A")="Include which Lists",DIR("B")=1 KILL DA D ^DIR KILL DIR
 I $D(DIRUT) D EOJ Q
 I Y[7 S (BUDT3A,BUDT3B,BUDT4,BUDT5,BUDT6,BUDT3AL,BUDT5L,BUDT5L1,BUDT5L2,BUDT6L,BUDTZ,BUDTZL)=1
 I Y[2 S (BUDT3A,BUDT3AL)=1
 I Y[3 S (BUDT5,BUDT5L)=1
 I Y[4 S (BUDT5,BUDT5L1)=1
 I Y[5 S (BUDT5,BUDT5L2)=1
 I Y[6 S (BUDT6,BUDT6L)=1
...
TZAI ;intro for table Z
 W !!,"Center/Grantee Profile:  User/Patient by Zip Code"
 W !,"The User/Patient by Zip Code table reports the number of users by"
...
YEAR ;
 S BUDYEAR=""
 W !
 W !,"Enter the Calendar Year.  Use a 4 digit year, e.g. 2003, 2006"
 S DIR(0)="D^::EP"
