...
EN1 ;
 S BUDSITE=""
 S DIC="^BUDVSITE(",DIC(0)="AEMQ",DIC("A")="Enter your site: " D ^DIC
 I Y=-1 D EOJ Q
 S BUDSITE=+Y
 ...
 ;S XBRP="PRINT^BUD5RPTP",XBRC="PROC^BUD5RPTC",XBRX="EOJ^BUD5RPT1",XBNS="BUD"
 ;D ^XBDBQUE
ZIS ;call to XBDBQUE
 K IOP,%ZIS
 W !! S %ZIS="PQM" D ^%ZIS
...
TSKMN ;EP ENTRY POINT FROM TASKMAN
 ...
 S ZTCPU=$G(IOCPU),ZTRTN="DRIVER^BUD5RPT1",ZTDTH="",ZTDESC="UDS 05 REPORT" D ^%ZTLOAD D EOJ Q
 Q
M ;EP - called from option
 ...
 W !?5,"7   ALL Tables"
 S DIR(0)="L^1:7",DIR("A")="Include which Tables",DIR("B")=6 KILL DA D ^DIR KILL DIR
 I $D(DIRUT) D EOJ Q
 I Y[7 S (BUDT3A,BUDT3B,BUDT4,BUDT5,BUDT6,BUDTZ)=1
 I Y[1 S BUDT3A=1
 I Y[2 S BUDT3B=1
 I Y[3 S BUDT5=1
 I Y[4 S BUDT6=1
