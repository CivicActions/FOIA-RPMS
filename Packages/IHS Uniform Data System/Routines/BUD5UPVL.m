BUD5UPVL ; IHS/CMI/LAB - update visit locations UDS ;
 ;;2.0;IHS/RPMS UNIFORM DATA SYSTEM;**1**;DEC 29, 2004
 ;
 ;
START ;update visit locations
 K BUDVL,BUDX,BUDY
 W:$D(IOF) @IOF W !!,"***  Update/Review UDS 2005 Site Parameters ***",!!
 ...
 S DIC(0)="AEMLQ",DIC="^BUDVSITE(" D ^DIC
 I Y=-1 W !!,"No site selected" D EOJ Q
 S BUDSITE=+Y
 S DIE="^BUDVSITE(",DR=".02",DA=BUDSITE D ^DIE
 D ^XBFMK
 D EN
 S BUDCNT=2
 G START
EN ; -- main entry point for BUD UPDATE VISIT LOCATIONS
 D EN^VALM("BUD 05 UPDATE VISIT LOCATIONS")
...
GATHER ;
 ...
 K BUDLIST
 S X=0 F  S X=$O(^BUDVSITE(BUDSITE,11,X)) Q:X'=+X  S BUDLIST($P(^DIC(4,$P(^BUDVSITE(BUDSITE,11,X,0),U),0),U),X)=X
 ...
 .S C=C+1,BUDVL(C,0)=I_") "_$P(^DIC(4,$P(^BUDVSITE(BUDSITE,11,BUDSEL(I),0),U),0),U) S BUDDISP(I)="",BUDVL("IDX",C,C)=BUDSEL(I)
 .; J=I+BUDCUT I $D(BUDSEL(J)),'$D(BUDDISP(J)) S $E(BUDVL(C,0),40)=J_") "_$P(^DIC(4,$P(^BUDVSITE(BUDSITE,11,BUDSEL(J),0),U),0),U) S BUDDISP(J)=""
 K BUDDISP
 S VALMCNT=C
 Q
ADD ;EP called from protocol to open a new case
 ...
 I Y=-1 Q
 S BUDLOC=+Y
 I $D(^BUDVSITE(BUDSITE,11,BUDLOC)) W !!,$P(^DIC(4,BUDLOC,0),U)," is already on the list." D RETURN,EXIT Q
 W !,"Adding UDS Visit Location..."
 D ^XBFMK
 S X="`"_BUDLOC,DIC="^BUDVSITE("_BUDSITE_",11,",DIC(0)="L",DIC("P")=$P(^DD(90345.1,1101,0),U,2),DA(1)=BUDSITE D ^DIC
...
ADDALL ;EP
 ...
 .I $D(^BUDVSITE(BUDSITE,11,BUDX)) W !,$P(^DIC(4,BUDX,0),U),"     --already on list" Q
 .D ^XBFMK
 .S X="`"_BUDX,DIC="^BUDVSITE("_BUDSITE_",11,",DIC(0)="L",DIC("P")=$P(^DD(90345.1,1101,0),U,2),DA(1)=BUDSITE D ^DIC K DIC,DA,DR,DIADD,DLAYGO,DD,D0,DO,X
 .W !,$P(^DIC(4,BUDX,0),U)
...
EDIT ;
 ...
 D FULL^VALM1
 S BUDANS=Y,BUDC="" F BUDI=1:1 S BUDC=$P(BUDANS,",",BUDI) Q:BUDC=""  S X=BUDVL("IDX",BUDC,BUDC) K ^BUDVSITE(BUDSITE,11,X,0),^BUDVSITE(BUDSITE,11,"B",X,X) W !,$P(^DIC(4,X,0),U)," removed from list"
 S DA=BUDSITE,DIK="^BUDVSITE(" D EN^DIK
 D ^XBFMK
 D PAUSE
 D EXIT
 Q
