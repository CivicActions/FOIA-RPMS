BUD7UPVL ; IHS/CMI/LAB - update visit locations UDS ;
 ;;2.0;IHS/RPMS UNIFORM DATA SYSTEM;**4**;DEC 29, 2004
 ;
 ;
START ;update visit locations
 K BUDVL,BUDX,BUDY
 W:$D(IOF) @IOF W !!,"***  Update/Review UDS 2007 Site Parameters ***",!!
...
EN ; -- main entry point for BUD UPDATE VISIT LOCATIONS
 D EN^VALM("BUD 07 UPDATE VISIT LOCATIONS")
...
GATHER ;
 ...
 .; J=I+BUDCUT I $D(BUDSEL(J)),'$D(BUDDISP(J)) S $E(BUDVL(C,0),40)=J_") "_$P(^DIC(4,$P(^BUDESITE(BUDSITE,11,BUDSEL(J),0),U),0),U) S BUDDISP(J)=""
 K BUDDISP
 S VALMCNT=C
 Q
ADD ;EP called from protocol to open a new case
 ...
 D ^XBFMK
 S X="`"_BUDLOC,DIC="^BUDESITE("_BUDSITE_",11,",DIC(0)="L",DIC("P")=$P(^DD(90345.1,1101,0),U,2),DA(1)=BUDSITE D ^DIC
...
ADDALL ;EP
 ...
 .D ^XBFMK
 .S X="`"_BUDX,DIC="^BUDESITE("_BUDSITE_",11,",DIC(0)="L",DIC("P")=$P(^DD(90345.1,1101,0),U,2),DA(1)=BUDSITE D ^DIC K DIC,DA,DR,DIADD,DLAYGO,DD,D0,DO,X
 .W !,$P(^DIC(4,BUDX,0),U)
...
EDIT ;
 ...
 D FULL^VALM1
 S BUDANS=Y,BUDC="" F BUDI=1:1 S BUDC=$P(BUDANS,",",BUDI) Q:BUDC=""  S X=BUDVL("IDX",BUDC,BUDC) K ^BUDESITE(BUDSITE,11,X,0),^BUDESITE(BUDSITE,11,"B",X,X) W !,$P(^DIC(4,X,0),U)," removed from list"
