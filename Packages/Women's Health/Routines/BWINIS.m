BWINIS ;IHS/ANMC/MWR -  ;15-Feb-2003 21:55;PLS
...
PAC(PKG,VER) ; called from package init (DIFROM7 created this routine)
 ...
 ;
 N %,%I,%H,DATE,DIFROM,NOW,PACKAGE,RUN,SERVER,SITE,START,X,XMDUZ,XMSUB,XMTEXT,XMY,Y K ^TMP("BWINIS",$J)
 ;
 ...
 ;
 ; Message for server
 S ^TMP("BWINIS",$J,1,0)="PACKAGE INSTALL"
 S ^TMP("BWINIS",$J,2,0)="SITE: "_SITE
 S ^TMP("BWINIS",$J,3,0)="PACKAGE: "_PACKAGE
 S ^TMP("BWINIS",$J,4,0)="VERSION: "_VER
 S ^TMP("BWINIS",$J,5,0)="Start time: "_START
 S ^TMP("BWINIS",$J,6,0)="Completion time: "_NOW
 S ^TMP("BWINIS",$J,7,0)="Run time: "_RUN
 S ^TMP("BWINIS",$J,8,0)="DATE: "_DATE
 ;
 ; Data is sent to server on FORUM - S.A5CSTS
 S XMY(SERVER)="",XMDUZ=.5,XMTEXT="^TMP(""BWINIS"",$J,",XMSUB=PACKAGE_" VERSION "_VER_" INSTALLATION"
 D ^XMD
 K ^TMP("BWINIS",$J)
 Q
