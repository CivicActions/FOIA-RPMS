VENPCCMD ; IHS/ITSC/GIS - USER PREFERENCE MANAGER FOR DIAGNOSES AND ICD CODES ; [ 06/19/06  10:56 AM ]
 ;;2.5;PCC+;**5**;JUN 15, 2006
 ;
 ;
 ; 
...
EDIT S STATUS=$$STATUS(+EF,LIST) W !!,$P(STATUS,U,3)
 ...
 Q
 ; 
IGRP(IGRP) ; RETURN THE ICD GROUP
 N DIC,X,Y,%
...
PRV(PRV) ;
 ...
 Q Y
 ; 
PGRP(DFLT) ; RETURN THE PATIENT GROUP
 N DIC,X,Y,%
...
LIST(EF,PRV,PGRP) ; 
 ...
 Q REC
 ; 
NLIST(IGRP,PRV,PGRP) ; RAW PREFERENCE LIST FOR A PROVIDER, ICD GROUP, AND PT GRP
 ...
 . I $L(REC) S REC=REC_U
 . S REC=REC_NAME_";"_CODE
 . I '$O(^ICD9("AB",CODE,0)) W !,CODE," IS AN INVALID ICD CODE!!!"
 . Q
 Q REC
 ; 
...
DEL3 S %=$P(STAT,U,2)-1 S $P(STAT,U,2)=%
 ...
 G DEL1
 ; 
WIPE ; CLEAN OUT ALL 19707.1 ENTRIES FOR A PROVIDER AND PT GROUP
 N DA,DIK,IX
 ...
 Q
 ;
NWIPE ; CLEAN OUT ALL 19707.34 FOR ENTRIES FOR A PROVIDER, ICD GROUP, AND PT GROUP
...
SAVE(LIST,EF,PRV,PGRP) ; EP-DELETE THE OLD LIST AND SAVE THE NEW ONE
 ...
 Q
 ; 
NSAVE ; SAVE TO NEW ICD FILE 19707.34
