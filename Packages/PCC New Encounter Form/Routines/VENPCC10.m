VENPCC10 ; IHS/ITSC/GIS - CREATE THE PCC+ ENCOUNTER FORM AND HEALTH SUMMARY ; [ 03/28/2006  4:29 PM ]
 ;;2.5;PCC+;**4**;MAR 28, 2006
 ;
...
PRINT(VISIT,VCN,DEPTIEN,PRV,DEFEF,DEFHS,APPT,VARS,EXT) ; EP-PRINT ENCOUNTER FORMS
 ...
 I $G(JOB),'$G(MOJOFLAG) S ^TMP("VEN MON",JOB)="Monitoring the process...." H 3
 I $D(^VA(200,+$G(PRV),0)),$D(^VEN(7.95,+$G(DEPTIEN),0))
 E  Q
NEW N %,%1,%DT,%HELP,%Q,%T,%Y,CFIGIEN,DATE,DFN,ERR,PATH2,PIEN,POP
 N STOP,TMP,UID,X,Y,MSG,FFIEN,FCIEN,PEIEN,VER25,HDR25
...
CFG S CFIGIEN=$$CFG^VENPCCU I 'CFIGIEN D ERR8^VENPCC1 Q
 I $G(HSONLY),'$G(OGFLAG) X ("I 0") G HS ; JUST PRINT HS - NOTHING MORE
...
HEAD ; EP - GET HEADER FILE
 NEW F,I,X,Y,%
 S PATH2=$G(^VEN(7.5,CFIGIEN,2)) ; TRADITIONAL WAY TO BUILD THE HEADER ARRAY
