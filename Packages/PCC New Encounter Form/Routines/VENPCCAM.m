VENPCCAM ; IHS/ITSC/GIS - NEW CHECK IN MODULE ;  [ 06/19/06  10:58 AM ]
 ;;2.5;PCC+;**4,5**;JUN 15, 2006
 ;
...
MFILE ; FILE RESULTS
 ...
 I VAL'="S" Q
FILE ; PROCESS THE CHECKIN DATA ; PATCHED BY GIS/OIT 2/1/06 ; PCC + VERSION 2.5, PATCH 4
 N DIC,DIE,DA,DR,X,Y
 I $D(MSR(0)) D
 . S EXT=$G(EXT)
 . I $L(EXT)  S EXT=EXT_U
 . S EXT=EXT_"CC;"_MSR(0) ; PASSES CHIEF COMPLAINT TO PCC+
 . Q
...
NFILE(VIEN,TXT) ; EP - POPULTES THE V NARRATIVE TEXT FILE WITH A CC
 ; PATCHED BY GIS/OIT 7/21/06 ; PCC + VERSION 2.5, PATCH 5
 N X,Y,Z,%,ARR,CNT,DA,DR,DIE,DIC,DLAYGO,TTIEN,CREF,LINE,A,B,NIEN
 S TTIEN=$O(^AUTTNTYP("B","CHIEF COMPLAINT",0)) I 'TTIEN Q
 I '$L(TXT) Q
 S DA=0,NIEN=0
 F  S DA=$O(^AUPNVNT("AD",+$G(VIEN),DA)) Q:'DA  I +$G(^AUPNVNT(DA,0))=TTIEN S NIEN=DA
