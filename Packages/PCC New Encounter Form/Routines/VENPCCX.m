VENPCCX ; IHS/ITSC/GIS - EXTERNAL CALL TO PCC+ DATA EXTRACTOR ;  [ 11/23/2005  4:57 PM ]
 ...
 ; 
VP N STG S STG="VISIT;70918^NURSE;381^DEPT;1^PROV;5^HS;0^EF;1^WT;139^HT;46^TEMP;100.6^BP;128/69^PULSE;136^ORDER;708;1076;^ORDER;307;1076;^CC;1.) Patient complains of Fever, Ear Ache, Cough. ^RF-STARTDT;12/12/1998^RF-LMP;11/11/00" W $$IMP(STG) Q
 ; 
IMP(STG,NOQ) ; EP-VISIT PLANNER ENTRY POINT
 N DSTG,FLD,VAL,INIT,EXT,NIEN,TIME,PROV,EF,HS,DEPT,VISIT,VCN,VARS,%,X,Y,I,PCODE,DISCPL,NIEN16,NURSE,APPT,SPGRP,DEFEF,DEFHS,DEPTIEN,PRV
 S DSTG="^VISIT^NURSE^PROV^EF^HS^DEPT^APPT^SPGRP^",INIT="",EXT=""
...
MORE S VCN=$P($G(^AUPNVSIT(VISIT,11)),U,3) I '$L(VCN) S ERR="MISSING VCN" D ERR^VENPCC1(ERR) Q -1
 ...
 S PCODE=$P($G(^DIC(6,NIEN16,9999999)),U,2)
 S %=+$P($G(^DIC(6,NIEN16,0)),U,4) S DISCPL=$P($G(^DIC(7,%,9999999)),U)
...
EN(VIEN,DIEN,PIEN) ; EP FROM BILLING PACKAGE CHECK IN MODULE
 I $D(^AUPNVSIT(+$G(VIEN),0)),$D(^ABSBDEPT(+$G(DIEN),0)),$D(^DIC(6,+$G(PIEN),0))
...
CLINIC S %=$P($G(^VEN(7.5,CFIGIEN,0)),U,6) I '% G CL1
 ...
 W !,"Invalid clinic stop!  Request cancelled...",! Q
CL1 S DIC("A")="Clinic: "
...
HS ; DEFAULT HEALTH SUMMARY ; PATCHED IHS/ITSC/GIS 11/04/2004
 K DIC
