APCLDV4P ;IHS/CMI/LAB - list refusals [ 01/10/2005   3:42 PM ]
...
PRINT1 ;
 ...
 K APCLLSTP S APCLTALP=0
 I '$D(^XTMP("APCLDV4",APCLJ,APCLH)) D HEADER W !!,"No data to report.",! G DONE
 D HEADER
 S APCLPTOT=$$PTOT
 W !," Total Number of Patients in TEMPLATE:",?40,$J($$COM(APCLPTOT,0),8)
 S APCLTOT=$$TOT
 W !!,"     Total Number of Patients screened",?40,$J($$COM(APCLTOT,0),8),?55,$$PER(APCLTOT,APCLPTOT)
 S APCLTALP=1
 W !!?46,"#",?53,"% of patients",$S(APCLTALP:" screened",1:"")
 D RES
 Q:$$END
 D GENDER
 Q:$$END
...
RES ;
 ...
 K APCLRES S X=0 F  S X=$O(^XTMP("APCLDV4",APCLJ,APCLH,"PTS",X)) Q:X'=+X  D
 .S D=$O(^XTMP("APCLDV4",APCLJ,APCLH,"PTS",X,""),-1) S APCLY=^XTMP("APCLDV4",APCLJ,APCLH,"PTS",X,D)
...
DATE ;
 ...
 K ^TMP($J) S X=0 F  S X=$O(^XTMP("APCLDV4",APCLJ,APCLH,"PTS",X)) Q:X'=+X  D
 .S D=$O(^XTMP("APCLDV4",APCLJ,APCLH,"PTS",X,""),-1) S APCLY=^XTMP("APCLDV4",APCLJ,APCLH,"PTS",X,D)
...
TOT() ;
 NEW C,X
 S C=0
 S X=0 F  S X=$O(^XTMP("APCLDV4",APCLJ,APCLH,"PTS",X)) Q:X'=+X  S C=C+1
 Q C
PTOT() ;
 NEW C,X
 S C=0
 S X=0 F  S X=$O(^DIBT(APCLSEAT,1,X)) Q:X'=+X  S C=C+1
 Q C
HEADER ;EP
 G:'APCLPG HEADER1
...
HEADER1 ;
 ...
 S X="SEARCH TEMPLATE OF PATIENTS: "_$P(^DIBT(APCLSEAT,0),U) W $$CTR(X,80),!
 I $G(APCLLSTP),APCLSCRD S X="Listing of those patients screened" W $$CTR(X,80),!
 I $G(APCLLSTP),'APCLSCRD S X="Listing of those NOT Screened" W $$CTR(X,80),!
 I $G(APCLLSTP) W !?35,"DATE",!,"PATIENT NAME",?22,"HRN",?29,"AGE",?35,"SCREENED",?44,"RESULT",?61,"CLINIC"
 W !,$TR($J("",80)," ","-")
 I '$G(APCLLSTP) W !?46,"#",?53,"% of patients",$S(APCLTALP:" screened",1:"")
 Q
DONE ;
 K ^TMP($J)
 K ^XTMP("APCLDV4",APCLJ,APCLH)
 D EOP
 Q
...
LIST ;EP - called from xbdbque
 S APCLPG=0 K APCLQUIT
 S APCLLSTP=1,APCLSCRD=1
 D HEADER
 K ^TMP($J)
 ;resort by sort item
 S DFN=0 F  S DFN=$O(^XTMP("APCLDV4",APCLJ,APCLH,"PTS",DFN)) Q:DFN'=+DFN  S D=$O(^XTMP("APCLDV4",APCLJ,APCLH,"PTS",DFN,""),-1) S APCLY=^XTMP("APCLDV4",APCLJ,APCLH,"PTS",DFN,D) D
 .D @APCLSORT
 ...
 S APCLSCRD=0
 D HEADER
 S DFN=0 F  S DFN=$O(^DIBT(APCLSEAT,1,DFN)) Q:DFN'=+DFN!($D(APCLQUIT))  D
 .Q:$D(^XTMP("APCLDV4",APCLJ,APCLH,"PTS",DFN))
 .Q:$$END
