APCLD712 ;IHS/CMI/LAB - 2007 DIABETES AUDIT;     
...
EKG(P,EDATE,F) ;EP
 ...
 K APCL I T S APCL(1)=$$CPT^APCLD712(P,$$DOB^AUPNPAT(P),ED,T,3) D
 .I APCL(1)="" K APCL Q
 .Q:LEKG>$P(APCL(1),U)
 .S LEKG=$P(APCL(1),U)
 K APCL I T S APCL(1)=$$RAD^APCLD712(P,$$DOB^AUPNPAT(P),ED,T,3) D
 .I APCL(1)="" K APCL Q
 .Q:LEKG>$P(APCL(1),U)
 .S LEKG=$P(APCL(1),U)
...
DEPSCR(P,BDATE,EDATE) ;EP
 ...
 I $D(APCL(1)) Q "Yes V79.0"_" "_$$DATE^APCLD710($P(APCL(1),U))
 ;check patient education
 S Y="APCL("
 S X=P_"^LAST EXAM 36;DURING "_BDATE_"-"_EDATE S E=$$START1^APCLDF(X,Y)
 I $D(APCL(1)) Q "Yes Exam 36-Dep Screen "_$$DATE^APCLD710($P(APCL(1),U))
 S Y="APCL("
 ...
 .I $P(T,"-",1)="DEP"!($P(T,"-",1)="BH")!($P(T,"-",1)="GAD")!($P(T,"-",1)="SB")!($P(T,"-",1)="PDEP") S D="Yes pt ed "_T_" "_$$DATE^APCLD710($P(APCL(X),U))
 K APCL
 I $P(D,U)]"" Q D
BHSCR ;
 ...
 .I $P($G(^AMHREC(V,14)),U,5)]"",$P(^AMHREC(V,14),U,5)'="UAS",$P(^AMHREC(V,14),U,5)'="REF" S APCLC="Yes BH Exam 36 "_$$DATE^APCLD710(9999999-D) Q
 .S X=0 F  S X=$O(^AMHRPRO("AD",V,X)) Q:X'=+X!(APCLC]"")  S APCLP=$P($G(^AMHRPRO(X,0)),U) D
 ..Q:'APCLP
 ..S APCLP=$P($G(^AMHPROB(APCLP,0)),U)
 ..I APCLP=14.1 S APCLC="Yes BH 14.1 "_$$DATE^APCLD710(9999999-D) Q
 ..I '$D(^AMHREDU("AD",V)) Q
 ...
 ...Q:'$D(^AUTTEDT(T,0))
 ...S T=$P(^AUTTEDT(T,0),U,2)
 ...I $P(T,"-",1)="DEP"!($P(T,"-",1)="BH")!($P(T,"-",1)="GAD")!($P(T,"-",1)="SB")!($P(T,"-",1)="PDEP") S APCLC="Yes BH pt ed "_T_" "_$$DATE^APCLD710(9999999-D)
 ...Q
 I APCLC]"" Q APCLC
 ;refusal
 NEW G S G=$$REFUSAL^APCLD717(P,9999999.15,$O(^AUTTEXAM("B","DEPRESSION SCREENING",0)),BDATE,EDATE)
 I G Q "Refused"
 S D=0,APCLC="",E=9999999-BDATE,D=9999999-EDATE-1_".99" F  S D=$O(^AMHREC("AE",P,D)) Q:D'=+D!($P(D,".")>E)!(APCLC]"")  S V=0 F  S V=$O(^AMHREC("AE",P,D,V)) Q:V'=+V!(APCLC]"")  D
 .I $P($G(^AMHREC(V,14)),U,5)]"",($P(^AMHREC(V,14),U,5)="UAS"!($P(^AMHREC(V,14),U,5)'="REF")) S APCLC="Refused BH Exam 36 "_$$DATE^APCLD710(9999999-D) Q
 I APCLC]"" Q APCLC
 Q "No"
LOINC(A,B) ;
 NEW %
