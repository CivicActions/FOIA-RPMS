APCLD994 ; IHS/CMI/LAB -IHS -CUMULATIVE REPORT  [ 09/26/00  9:03 AM ]
 ;;3.0;IHS PCC REPORTS;**7,17**;FEB 05, 1997
 ;
 ;
DURDM(P,R,EDATE,F) ;EP
 ...
 NEW DATE
 S DATE=""
 I $G(R) S DATE=$$CMSFDX^APCLD997(P,R,"ID")
 I DATE]"" Q $S(F="I":DATE,1:$$D(DATE))
 S DATE=$$PLDMDOO^APCLD997(P,"I")
 I DATE]"" Q ($$FMDIFF^XLFDT(EDATE,DATE,1)\365)
 ;S DATE=$$FRSTDMDX^APCLD997(P,"I")
 ;I DATE]"" Q $S(F="I":DATE,1:$$D(DATE))
 Q ""
D(D) ;
 I $G(D)="" Q ""
...
EXAMS ;
 I $Y>(IOSL-5) D HEADER Q:APCLQUIT
 S V=$G(APCLCUML(120)) W !!,$P(V,U),?68,"(% refused)"
