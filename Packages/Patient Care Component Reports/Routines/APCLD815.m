APCLD815 ; IHS/CMI/LAB -IHS -CUMULATIVE REPORT    
...
CUML ;EP
 ...
 S APCLCUML(40)="Weight Control (BMI)"
 ...
 S APCLCUML(100)="ANTIPLATELET THERAPY"
 S APCLCUML(110)="ACE INHIBITOR (OR ARB) USE"
 S APCLCUML(115)="LIPID LOWERING AGENT USE"
 S APCLCUML(300)="Depression on Problem List or as POVs"
 S APCLCUML(301)="Depression Screening?"
 ;
PROCESS ;
 S APCLPD=0 F  S APCLPD=$O(^XTMP("APCLDM81",APCLJOB,APCLBTH,"AUDIT",APCLPD)) Q:APCLPD'=+APCLPD  D CUML1
 Q
 ;
CUML1 ;
GENDER ;
 ...
 S V=$G(^XTMP("APCLDM81",APCLJOB,APCLBTH,"AUDIT",APCLPD,20))
...
TYPE ;
 ...
 S X=$$TYPE^APCLD816(APCLPD,APCLDMRG,APCLADAT)
...
DURDMC ;
 ...
 S V=$$DURDM^APCLD814(APCLPD,APCLDMRG,APCLADAT)
...
BMI ;
 ...
 S V=$G(^XTMP("APCLDM81",APCLJOB,APCLBTH,"AUDIT",APCLPD,112))
 S $P(APCLCUML(40),U,2)=$P(APCLCUML(40),U,2)+1
 D
 .I V="" S $P(APCLCUML(40),U,5)=$P(APCLCUML(40),U,5)+1 Q
 .I $$OW^APCLD814(APCLPD,V,APCLADAT) S $P(APCLCUML(40),U,3)=$P(APCLCUML(40),U,3)+1 Q
 .I $$OB^APCLD814(APCLPD,V,APCLADAT) S $P(APCLCUML(40),U,4)=$P(APCLCUML(40),U,4)+1 Q
 .S $P(APCLCUML(40),U,6)=$P(APCLCUML(40),U,6)+1 Q
HGB ;
 ;use last hgba1c value only
 ;APCLCUML(50)=
 S $P(APCLCUML(50),U,2)=$P(APCLCUML(50),U,2)+1
 S V=$P($G(^XTMP("APCLDM81",APCLJOB,APCLBTH,"AUDIT",APCLPD,78)),U)
 ;S V=$$STV^APCLD818(V,4)
...
BPC ;blood pressure control
 ...
 .I S<130&(D<80) S $P(APCLCUML(60),U,4)=$P(APCLCUML(60),U,4)+1 Q
...
TBSTAT ;
 ...
 S V=$$TBCODE^APCLD816(APCLPD,APCLRED,APCLDMRG)
 S $P(APCLCUML(70),U,(V+2))=$P(APCLCUML(70),U,(V+2))+1
TOBACCO ;
 S $P(APCLCUML(80),U,2)=$P(APCLCUML(80),U,2)+1
 S V=$G(^XTMP("APCLDM81",APCLJOB,APCLBTH,"AUDIT",APCLPD,27))
 S V1=$G(^XTMP("APCLDM81",APCLJOB,APCLBTH,"AUDIT",APCLPD,28))
...
DMTX ;diabetes treatment
 ...
 S V=$$THERAPY^APCLD816(APCLPD,APCL6MBD,APCLRED)
 I V=1 S $P(APCLCUML(90),U,3)=$P(APCLCUML(90),U,3)+1
 I V=9 S $P(APCLCUML(90),U,11)=$P(APCLCUML(90),U,11)+1
 ;I $L(V)=1 S P=$S(V=2:4,V=3:5,V=4:6,V=5:7,V=6:8,1:"") S $P(APCLCUML(90),U,P)=$P(APCLCUML(90),U,P)+1  ;cmi/maw 12/26/2007 orig line
 ;I $L(V)=1 S P=$S(V=2:4,V=3:5,V=4:6,V=5:7,V=6:8,V=7:12,V=8:13,1:"") S $P(APCLCUML(90),U,P)=$P(APCLCUML(90),U,P)+1  ;cmi/maw 12/26/2007 DM2008
 N I
 F I=1:1:$L(V) D
 . N Q
 . S Q=$E(V,I,I)
 . S P=$S(Q=2:4,Q=3:5,Q=4:6,Q=5:7,Q=6:8,Q=7:13,Q=8:12,1:"") S $P(APCLCUML(90),U,P)=$P(APCLCUML(90),U,P)+1  ;cmi/maw 4/2/2008 modified for incretin/dpp4 per gary
...
ASPIRIN ;
 ...
 G ACE:$$AGE^AUPNPAT(APCLPD,APCLADAT)<30
 S $P(APCLCUML(100),U,2)=$P(APCLCUML(100),U,2)+1
 S V=$G(^XTMP("APCLDM81",APCLJOB,APCLBTH,"AUDIT",APCLPD,62))
...
ACE ;110 title^total pts^total pts with protein^# of those on ace^# with htn^# of those on ace"
 ...
 S P=$G(^XTMP("APCLDM81",APCLJOB,APCLBTH,"AUDIT",APCLPD,91))  ;01/09/08
 ;S P=$G(^XTMP("APCLDM81",APCLJOB,APCLBTH,"AUDIT",APCLPD,94))
 S H=$G(^XTMP("APCLDM81",APCLJOB,APCLBTH,"AUDIT",APCLPD,34))
 S A=$G(^XTMP("APCLDM81",APCLJOB,APCLBTH,"AUDIT",APCLPD,60))
 I $P(P,U,2)[">" S $P(P,U,2)=300  ;cmi/maw 3/26/2008 convert to 300 if it contains a > signaling overt
 I $E(P)]"",$P(P,U,2)>299.9 S $P(APCLCUML(110),U,3)=$P(APCLCUML(110),U,3)+1 I $E(A)="Y",+$P(P,U,2)>299.9 S $P(APCLCUML(110),U,4)=$P(APCLCUML(110),U,4)+1  ;01/09/08
...
LIPID ;115
 S V=$G(^XTMP("APCLDM81",APCLJOB,APCLBTH,"AUDIT",APCLPD,61))
 S L=$$LDL^APCLD818(APCLPD,APCLBDAT,APCLADAT,"I"),L=$P(L,U)
 S T=$$CHOL^APCLD818(APCLPD,APCLBDAT,APCLADAT,"I"),T=$P(T,U)
 ...
 I $E(V)="R" S $P(APCLCUML(115),U,11)=$P(APCLCUML(115),U,11)+1
DEP ;
 S $P(APCLCUML(300),U,2)=$P(APCLCUML(300),U,2)+1
 S V=$G(^XTMP("APCLDM81",APCLJOB,APCLBTH,"AUDIT",APCLPD,200))
 I $E(V)="Y" S $P(APCLCUML(300),U,3)=$P(APCLCUML(300),U,3)+1
 I $E(V)'="Y" S $P(APCLCUML(301),U,2)=$P(APCLCUML(301),U,2)+1 D
 .S V=$G(^XTMP("APCLDM81",APCLJOB,APCLBTH,"AUDIT",APCLPD,210))
 .I $E(V)="Y" S $P(APCLCUML(301),U,3)=$P(APCLCUML(301),U,3)+1
 .I $E(V)="N" S $P(APCLCUML(301),U,4)=$P(APCLCUML(301),U,4)+1
 .I $E(V)="R" S $P(APCLCUML(301),U,5)=$P(APCLCUML(301),U,5)+1
EXAMS ;
 S:'$D(APCLCUML(120)) APCLCUML(120)="EXAMS - Yearly"
 S $P(APCLCUML(120),U,2)=$P(APCLCUML(120),U,2)+1
 S V=$G(^XTMP("APCLDM81",APCLJOB,APCLBTH,"AUDIT",APCLPD,38))
 I $E(V)="Y" S $P(APCLCUML(120),U,3)=$P(APCLCUML(120),U,3)+1
 I $E(V)="R" S $P(APCLCUML(120),U,6)=$P(APCLCUML(120),U,6)+1
 S V=$G(^XTMP("APCLDM81",APCLJOB,APCLBTH,"AUDIT",APCLPD,40))
 I $E(V)="Y" S $P(APCLCUML(120),U,4)=$P(APCLCUML(120),U,4)+1
 I $E(V)="R" S $P(APCLCUML(120),U,7)=$P(APCLCUML(120),U,7)+1
 S V=$G(^XTMP("APCLDM81",APCLJOB,APCLBTH,"AUDIT",APCLPD,42))
 ...
 S V=$G(^XTMP("APCLDM81",APCLJOB,APCLBTH,"AUDIT",APCLPD,108))
...
EDUC ;
 ...
 S G=0,V=$G(^XTMP("APCLDM81",APCLJOB,APCLBTH,"AUDIT",APCLPD,44))
 I $E(V)="Y" S $P(APCLCUML(130),U,3)=$P(APCLCUML(130),U,3)+1 S G=1
 I $E(V)="R" S $P(APCLCUML(130),U,7)=$P(APCLCUML(130),U,7)+1
 I $G(V)="Yes (RD)" S $P(APCLCUML(130),U,10)=$P(APCLCUML(130),U,10)+1 S G=1
 I $G(V)="Yes (RD & Non RD - Other)" S $P(APCLCUML(130),U,10)=$P(APCLCUML(130),U,10)+1 S G=1  ;cmi/maw 1/15/08
 S V=$G(^XTMP("APCLDM81",APCLJOB,APCLBTH,"AUDIT",APCLPD,46))
 I $E(V)="Y" S $P(APCLCUML(130),U,4)=$P(APCLCUML(130),U,4)+1 S G=1
 I $E(V)="R" S $P(APCLCUML(130),U,8)=$P(APCLCUML(130),U,8)+1
 S V=$G(^XTMP("APCLDM81",APCLJOB,APCLBTH,"AUDIT",APCLPD,48))
