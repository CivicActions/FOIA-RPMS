APCLD21 ; IHS/CMI/LAB - IHS Diabetes Audit 2001 ;    [ 03/02/2007  1:35 PM ]
 ;;3.0;IHS PCC REPORTS;**10,13,19**;FEB 05, 1997
 ;
BEGIN ;EP - called from option
 D TAXCHK^APCLD219
 W:$D(IOF) @IOF
 W !!!,$$CTR("ASSESSMENT OF DIABETES CARE, 2001")
...
TYPE ;
 S APCLSTP=0
 K ^XTMP("APCLDM21",APCLJOB,APCLBTH),^TMP($J,"PATS")
 S APCLTYPE=""
...
ZIS ;
 I APCLPREP'=2 S XBRP="^APCLD21P",XBRC="^APCLD210",XBRX="XIT^APCLD21",XBNS="APCL"
 I APCLPREP=2 S XBRP="",XBRC="^APCLD210",XBRX="XIT^APCLD21",XBNS="APCL"
 D ^XBDBQUE
 D XIT
 Q
P ;
 S APCLSTP=0 K ^XTMP("APCLDM21",APCLJOB,APCLBTH),^TMP($J,"PATS")
P1 ;
 K DIC S DIC="^AUPNPAT(",DIC(0)="AEMQ" D ^DIC K DIC
 I Y=-1,'$D(^XTMP("APCLDM21",APCLJOB,APCLBTH,"PATS")) W !,"No patients selected" S APCLSTP=1 Q
 I Y=-1 Q
 S ^XTMP("APCLDM21",APCLJOB,APCLBTH,"PATS",+Y)=""
 G P1
 Q
S ; Get patient name or cohort
 K ^XTMP("APCLDM21",APCLJOB,APCLBTH),^TMP($J,"PATS") S APCLSTP=0
...
RAND ;random sample or not
 ...
 G:$D(DIRUT) C
 I Y="A" S C=0 F  S C=$O(^TMP($J,"PATS",C)) Q:C'=+C  S X=$O(^TMP($J,"PATS",C,0)),^XTMP("APCLDM21",APCLJOB,APCLBTH,"PATS",X)=""
 ...
 I $D(DIRUT) S APCLSTP=1 Q
 S C=0 F N=1:1:APCLCNT Q:C=Y  S I=$R(APCLCNT) I I,$D(^TMP($J,"PATS",I)) S X=$O(^TMP($J,"PATS",I,0)),^XTMP("APCLDM21",APCLJOB,APCLBTH,"PATS",X)="",C=C+1 K ^TMP($J,"PATS",I,X)
 K ^TMP($J,"PATS")
 Q
...
WRITEF ;EP write flat file
 ...
 S (X,N)=0 F  S X=$O(^APCLRECD(16,13,X)) Q:X'=+X  S N=N+1,^APCLDATA($J,N)=^APCLRECD(16,13,X,0)
