...
TYPE ;
 S APCLSTP=0
 K ^XTMP("APCLDM20",APCLJOB,APCLBTH),^TMP($J,"PATS")
 S APCLTYPE=""
...
ZIS ;
 ...
 Q
P ;
 S APCLSTP=0 K ^XTMP("APCLDM20",APCLJOB,APCLBTH),^TMP($J,"PATS")
P1 ;
 K DIC S DIC="^AUPNPAT(",DIC(0)="AEMQ" D ^DIC K DIC
 I Y=-1,'$D(^XTMP("APCLDM20",APCLJOB,APCLBTH,"PATS")) W !,"No patients selected" S APCLSTP=1 Q
 I Y=-1 Q
 S ^XTMP("APCLDM20",APCLJOB,APCLBTH,"PATS",+Y)=""
 G P1
 Q
S ; Get patient name or cohort
 K ^XTMP("APCLDM20",APCLJOB,APCLBTH),^TMP($J,"PATS") S APCLSTP=0
...
RAND ;random sample or not
 ...
 G:$D(DIRUT) C
 I Y="A" S C=0 F  S C=$O(^TMP($J,"PATS",C)) Q:C'=+C  S X=$O(^TMP($J,"PATS",C,0)),^XTMP("APCLDM20",APCLJOB,APCLBTH,"PATS",X)=""
...
XIT1 ;
 ...
 K ^XTMP("APCLDM20",APCLJOB,APCLBTH),APCLJOB,APCLBTH
XIT ;
...
WRITEF ;EP write flat file
 ...
 S (X,N)=0 F  S X=$O(^APCLRECD(13,13,X)) Q:X'=+X  S N=N+1,^APCLDATA($J,N)=^APCLRECD(13,13,X,0)
