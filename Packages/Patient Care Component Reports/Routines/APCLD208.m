APCLD208 ;IHS/CMI/LAB - 2000 DIABETES AUDIT;   [ 04/15/03  5:33 PM ]
 ;;3.0;IHS PCC REPORTS;**8,9,10,11,20**;FEB 05, 1997
 ;
...
LDL(P,BDATE,EDATE,F) ;EP
 ...
 I '$D(APCL(1)) Q ""
 S D=$P(APCL(1),U),D=$$FMTE^XLFDT(D) K APCL S %=P_"^ALL LAB [DM AUDIT LDL CHOLESTEROL TAX;DURING "_D_"-"_D,E=$$START1^APCLDF(%,"APCL(")
 NEW N D SETN
 I F="I" Q $P(^AUPNVLAB(+$P(APCL(N),U,4),0),U,4)_"^"_$P(APCL(N),U)
 Q $P(^AUPNVLAB(+$P(APCL(N),U,4),0),U,4)_$S($P(^AUPNVLAB(+$P(APCL(N),U,4),0),U,4)]"":" mg/dl ",1:"")_$$FMTE^XLFDT($P(APCL(N),U),5)
...
PPD(P,EDATE,F) ;EP
 ...
 .S X=EDATE,%DT="P" D ^%DT S BDATE=$$FMADD^XLFDT(Y,-365),BDATE=$$FMTE^XLFDT(BDATE),G=0 I $$REFUSAL^APCLD207(P,9999999.28,$O(^AUTTSK("B","PPD",0)),BDATE,EDATE) S G=1
