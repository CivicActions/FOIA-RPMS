APCLHOPA ; IHS/CMI/LAB - list SDOH SCREENING ;
...
INFORM ;
 ...
 W !,"This report will tally the patients by age groups, sex, screening exam,"
...
TALLY ;which items to tally
 ...
 I $D(DIRUT) G DATES
 I Y="" G DATES
 I Y[0 K APCLRALL G EXCL
 S APCLRALL=Y
 S A=Y,C="" F I=1:1 S C=$P(A,",",I) Q:C=""  S APCLRALL(C)=""
AGEG ;
 I '$D(APCLRALL(2)) G EXCL
...
BIN ;
 ...
 I $D(DIRUT) S APCLQUIT="" G TALLY
 I Y=0 G EXCL
RUN ;
 K APCLQUIT S APCLRY="",APCLRA=-1 W ! F  D AGE Q:APCLRX=""  I $D(APCLQUIT) G BIN
...
AGE ;
 S APCLRX=""
 S DIR(0)="NO^0:150:0",DIR("A")="Enter the starting age of the "_$S(APCLRY="":"first",1:"next")_" age group" D ^DIR K DIR S:$D(DUOUT) DIRUT=1
 I $D(DUOUT)!($D(DTOUT)) S APCLQUIT="" Q
 S APCLRX=Y
 I Y="" Q
 I APCLRX?1.3N,APCLRX>APCLRA D SET Q
 W $C(7) W !,"Make sure the age is higher the beginning age of the previous group.",! G RUN
 ;
SET S APCLRA=APCLRX
 I APCLRY="" S APCLRY=APCLRX Q
 S APCLRY=APCLRY_"-"_(APCLRX-1)_";"_APCLRX
 Q
 ;
CLOSE I APCLRY="" Q
GC ;
 ...
 I $D(DUOUT)!($D(DTOUT)) S APCLQUIT="" Q
 S APCLRX=Y I Y="" S APCLRX=199
 I APCLRX?1.3N,APCLRX'<APCLRA S APCLRY=APCLRY_"-"_APCLRX,APCLRBIN=APCLRY Q
 W "  ??",$C(7) G CLOSE
 Q
 ;
LIST ;
 ...
 ;
SETBIN ;
 S APCLRBIN="0-12;13-17;18-64;65-125"
 Q
EXCL ;
 K APCLBINA
 I '$D(APCLRBIN) D SETBIN
 F I=1:1 S J=$P(APCLRBIN,";",I) Q:J=""  S APCLBINA(J)=$P(J,"-",1)_U_$P(J,"-",2)
PCC ;
 S APCLEXBH=""
 W !!,"Would you like to include SDOH Screenings documented in the Behavioral"
 ;W !,""
 S DIR(0)="Y",DIR("A")="Health database",DIR("B")="Y" KILL DA D ^DIR KILL DIR
 I $D(DIRUT) G DATES
 S APCLEXBH=Y
 ;
DEMO ;
...
ZIS ;CALL TO XBDBQUE
 ...
 S XBRP="PRINT^APCLHOPP",XBRC="PROC^APCLHOPA",XBRX="XIT^APCLHOPA",XBNS="APCL"
 D ^XBDBQUE
 D XIT
 Q
BROWSE ;
 S XBRP="VIEWR^XBLM(""^APCLHOPP"")"
 S XBNS="APCL",XBRC="PROC^APCLHOPA",XBRX="XIT^APCLHOPA",XBIOP=0 D ^XBDBQUE
 Q
XIT ;
 D EN^XBVK("APCLR")
 D ^XBFMK
...
PROC ;
 ...
 D XTMP^APCLOSUT("APCLHOPA","SDOH SCREENING REPORT")
 ;now go through each patient
 S DFN=0 F  S DFN=$O(^AUPNPAT(DFN)) Q:DFN'=+DFN  D
 .Q:$$DEMO^APCLUTL(DFN,$G(APCLDEMO))
 .;GET EACH OF THE 5 SCREENS AND PUT IN ^XTMP
 .D GATHER("FOOD",DFN,APCLBD,APCLED,2203,46)  ;2203,46
 .D GATHER("HOUSING",DFN,APCLBD,APCLED,2205,47)  ;2205,47
 .D GATHER("TRANSPORTATION",DFN,APCLBD,APCLED,2207,48)
 .D GATHER("UTILITIES",DFN,APCLBD,APCLED,2209,49)
 .D GATHER("INTERPERSONAL SAFETY",DFN,APCLBD,APCLED,2211,50)
 Q
GATHER(SUB,PAT,BDATE,EDATE,BHF,CODE) ;
 ...
 I 'X G PCCREF
 S A=$$VALI^XBDIQ1(9000010,$P(X,U,4),.06) I $D(APCLLOCT) Q:A=""  Q:'$D(APCLLOCT(A))
 S A=$$VALI^XBDIQ1(9000010,$P(X,U,4),.08) I $D(APCLCLNT) Q:A=""  Q:'$D(APCLCLNT(A))
 I $P(X,U)>$P(RESULT,U,1) S RESULT=$P(X,U)_U_$P(X,U,3)_U_$$CLINIC($P(X,U,4))
PCCREF ;now check REF and UAS in refusal file
 S F=0,G="" S F=9999999.15
 ...
 ..Q:'$D(^AMHREC(V,0))
 ..;Q:'$$ALLOWVI^APCLUTIL(DUZ,V)
 ..S A=$$VALI^XBDIQ1(9002011,V,.04) I $D(APCLLOCT) Q:A=""  Q:'$D(APCLLOCT(A))  ;I A,$D(APCLLOCT),'$D(APCLLOCT(A)) Q
 ..S A=$$VALI^XBDIQ1(9002011,V,.25) I $D(APCLCLNT) Q:A=""  Q:'$D(APCLCLNT(A))  ;not correct clinic
 ..;get exams
...
S ;
 Q:RESULT=""
 S $P(^XTMP("APCLHOPA",APCLJ,APCLH,SUB),U,1)=$P($G(^XTMP("APCLHOPA",APCLJ,APCLH,SUB)),U,1)+1 ;W !,PAT," ",SUB  ;number screened
 S R=$P(RESULT,U,2)
 S P=$S(R="POSITIVE":2,R["NEGATIVE":3,R="PATIENT REFUSED SCREENING":4,R="UNABLE TO SCREEN":5,R="REFERRAL NEEDED":6,1:"")
 I 'P Q  ;HUH
 S $P(^XTMP("APCLHOPA",APCLJ,APCLH,SUB),U,P)=$P($G(^XTMP("APCLHOPA",APCLJ,APCLH,SUB)),U,P)+1  ;result
 ;patient list
 S V=$S(SUB="FOOD":1,SUB="HOUSING":2,SUB="TRANSPORTATION":3,SUB="UTILITIES":4,SUB="INTERPERSONAL SAFETY":5,1:"")
 I V="" Q  ;HUH
 S $P(^XTMP("APCLHOPA",APCLJ,APCLH,"PATS",PAT),U,V)=$$FMTE^XLFDT($P(RESULT,U))_" Result: "_$P(RESULT,U,2)
TALLIES ;
 ;now all tallies
 S G=$$VAL^XBDIQ1(2,PAT,.02)
 I G="" S G="UNKNOWN"
 S ^XTMP("APCLHOPA",APCLJ,APCLH,SUB,"SEX",G)=$G(^XTMP("APCLHOPA",APCLJ,APCLH,SUB,"SEX",G))+1
 ;age
 S A=$$AGE^AUPNPAT(PAT,$P(RESULT,U))
 S APCLRAGE=""
 F I=1:1 S APCLNN=$P(APCLRBIN,";",I) Q:APCLNN=""  S APCLX=$P(APCLNN,"-"),APCLY=$P(APCLNN,"-",2) I A'<APCLX,A'>APCLY  S APCLRAGE=I
 S ^XTMP("APCLHOPA",APCLJ,APCLH,SUB,"AGE",APCLRAGE)=$G(^XTMP("APCLHOPA",APCLJ,APCLH,SUB,"AGE",APCLRAGE))+1
 S A=$P(RESULT,U,3)
 S ^XTMP("APCLHOPA",APCLJ,APCLH,SUB,"CLINIC",A)=$G(^XTMP("APCLHOPA",APCLJ,APCLH,SUB,"CLINIC",A))+1
 S A=$$ETHN(PAT)
 S ^XTMP("APCLHOPA",APCLJ,APCLH,SUB,"ETHNICITY",A)=$G(^XTMP("APCLHOPA",APCLJ,APCLH,SUB,"ETHNICITY",A))+1
 S A=$$RACE(PAT)
 S ^XTMP("APCLHOPA",APCLJ,APCLH,SUB,"RACE",A)=$G(^XTMP("APCLHOPA",APCLJ,APCLH,SUB,"RACE",A))+1
 S A=$$COMMRES^AUPNPAT(PAT,"E")
 S ^XTMP("APCLHOPA",APCLJ,APCLH,SUB,"COMMUNITY",A)=$G(^XTMP("APCLHOPA",APCLJ,APCLH,SUB,"COMMUNITY",A))+1
 Q
 ;
GETAGE(A) ;
 NEW R,L,H,I
 S R=""
 S I="" F  S I=$O(APCLBINA(I)) Q:I=""!(R]"")  D
 .S L=$P(APCLBINA(I),U,1)
 .S H=$P(APCLBINA(I),U,2)
 .I A'<L,A'>H S R=I
 Q R
 ;
CLINIC(V) ;
...
LOC() ;EP - Return location name from file 4 based on DUZ(2).
 ...
 ;
RACE(P) ;EP
 ;RETURN EXTERNAL RACE VALUE FOR SCREENING REPORTS
 NEW X,Y,Z,C,A,V,G,B
 S X=$$RACE^AGUTL(P)
 S G=""
 I 'X Q "NOT ENTERED"
 F A=2:1:99 S B=$P(X,U,A) Q:B=""  S:G]"" G=G_"/" S G=G_B
 Q G
 ;
ETHN(P) ;EP
 NEW Z,E,G,X
