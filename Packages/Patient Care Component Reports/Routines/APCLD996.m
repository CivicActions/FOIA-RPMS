APCLD996 ;IHS/CMI/LAB - 1999 DIABETES AUDIT;   [ 08/30/00  12:52 PM ]
 ;;3.0;IHS PCC REPORTS;**7,8,20**;FEB 05, 1997
 ;
...
TBCODE(P,EDATE,R) ;EP
 ...
 I X]"" Q 2
 I $$PPD^APCLD998(P,EDATE)["POS" D  Q APCLJ
 .I $$TBTX^APCLD998(P)["TX COMPLETE" S APCLJ=1 Q
 .S APCLJ=2
 .Q
 I $$PPD^APCLD998(P,EDATE)["NEG" S APCLJ=4 D  Q APCLJ
 .I $$DODX(P,R,"I")="" S APCLJ=5 Q
 .S D=$$DODX(P,R,"I"),E=$$PPD^APCLD998(P,EDATE,"I") S APCLJ=$S(D>E:4,1:3)
 .Q
 S APCLJ=6
 Q APCLJ
 ;;
...
DIAMEAN(P,BDATE,EDATE) ;EP
 ...
 Q C\3
PPDDATE(P,EDATE) ;EP
 NEW X S X=$$LASTNP^APCLD998(P,EDATE)
 Q X
FLU(P,BDATE,EDATE) ;EP
 ...
 I $$REFUSAL^APCLD997(P,9999999.14,$O(^AUTTIMM("C",$S($$BI:88,1:12),0)),BD,EDATE) Q "Refused"
 Q "No"
PNEU(P,EDATE) ;EP
 NEW APCL,X,E
 S X=P_"^LAST IMM "_$S($$BI:33,1:19)_";DURING "_$$FMTE^XLFDT($$DOB^AUPNPAT(P))_"-"_EDATE S E=$$START1^APCLDF(X,"APCL(")
 I $D(APCL(1)) Q "Yes - "_$$FMTE^XLFDT($P(APCL(1),U))
 I $$REFUSAL^APCLD997(P,9999999.14,$O(^AUTTIMM("C",$S($$BI:33,1:19),0)),$$DOB^AUPNPAT(P,"E"),EDATE) Q "Refused"
 Q "No"
TD(P,EDATE) ;EP
 ...
 I $$REFUSAL^APCLD997(P,9999999.14,$O(^AUTTIMM("C",$S($$BI:9,1:"02"),0)),B,EDATE) Q "Refused"
 I $$REFUSAL^APCLD997(P,9999999.14,$O(^AUTTIMM("C",$S($$BI:1,1:"03"),0)),B,EDATE) Q "Refused"
 I $$REFUSAL^APCLD997(P,9999999.14,$O(^AUTTIMM("C",$S($$BI:28,1:34),0)),B,EDATE) Q "Refused"
 I $$REFUSAL^APCLD997(P,9999999.14,$O(^AUTTIMM("C",$S($$BI:20,1:42),0)),B,EDATE) Q "Refused"
 Q "No"
 ;
...
TOBACCO1 ;check problem file for tobacco use
 ...
 . ;I $P(^ICD9($P(APCL(1),U,2),0),U,1)=305.13 S APCLTOB="3  Past use"_" - "_$E($P(^AUTNPOV($P(^AUPNPROB(+$P(APCL(1),U,4),0),U,5),0),U),1,30) Q  ;cmi/anch/maw 9/10/2007 orig line
 . I $P($$ICDDX^ICDCODE($P(APCL(1),U,2)),U,2)=305.13 S APCLTOB="3  Past use"_" - "_$E($P(^AUTNPOV($P(^AUPNPROB(+$P(APCL(1),U,4),0),U,5),0),U),1,30) Q  ;cmi/anch/maw 9/10/2007 csv
...
TOBACCO2 ;check pov file for TOBACCO USE DOC
 ...
 . I $P(APCL(1),U,2)=305.13 S APCLTOB="3  Past use"_" - "_$E($P(^AUTNPOV($P(^AUPNVPOV(+$P(APCL(1),U,4),0),U,4),0),U),1,30) Q
...
THERAPY(P,BD,EDATE) ;EP - therapy code for epi
 ...
 S STR="",TNAME=""
 S X=$$INSULIN^APCLD998(P,BD,EDATE)
 I X]"" S STR=STR_"2"
 S X=$$SULF^APCLD998(P,BD,EDATE)
 I X]"" S STR=STR_3
 S X=$$MET^APCLD998(P,BD,EDATE)
 I X]"" S STR=STR_4
 S X=$$ACAR^APCLD998(P,BD,EDATE)
 I X]"" S STR=STR_5
 S X=$$TROG^APCLD998(P,BD,EDATE)
 I X]"" S STR=STR_"6"
 I STR]"" Q STR
 Q 1
 ;
