APCLD81 ; IHS/CMI/LAB - IHS Diabetes Audit 2008 ;     
...
TYPE ;
 S APCLSTP=0
 K ^XTMP("APCLDM81",APCLJOB,APCLBTH),^TMP($J,"PATS")
 S APCLTYPE=""
...
IF ;PEP - called from BDM indivdual or epi
 ...
 I APCLPREP=1!(APCLPREP=4) S APCLPPN="" D  G:APCLSTP IF
 .K DIR S DIR(0)="Y",DIR("A")="Do you wish to print the Patient's Name on the audit sheet",DIR("B")="N" KILL DA D ^DIR KILL DIR
 .I $D(DIRUT) S APCLSTP=1
 .S APCLPPN=Y
ZIS ;
 I APCLPREP'=2 S XBRP="^APCLD81P",XBRC="^APCLD810",XBRX="XIT^APCLD81",XBNS="APCL"
 I APCLPREP=2 S XBRP="",XBRC="^APCLD810",XBRX="XIT^APCLD81",XBNS="APCL"
 D ^XBDBQUE
 D XIT
 Q
P ;
 S APCLSTP=0 K ^XTMP("APCLDM81",APCLJOB,APCLBTH),^TMP($J,"PATS")
P1 ;
 K DIC S DIC="^AUPNPAT(",DIC(0)="AEMQ" D ^DIC K DIC
 I Y=-1,'$D(^XTMP("APCLDM81",APCLJOB,APCLBTH,"PATS")) W !,"No patients selected" S APCLSTP=1 Q
 I Y=-1 Q
 S ^XTMP("APCLDM81",APCLJOB,APCLBTH,"PATS",+Y)=""
 G P1
 Q
S ; Get patient name or cohort
 K ^XTMP("APCLDM81",APCLJOB,APCLBTH),^TMP($J,"PATS") S APCLSTP=0
...
RAND ;random sample or not
 ...
 G:$D(DIRUT) C
 I Y="A" S C=0 F  S C=$O(^TMP($J,"PATS",C)) Q:C'=+C  S X=$O(^TMP($J,"PATS",C,0)),^XTMP("APCLDM81",APCLJOB,APCLBTH,"PATS",X)=""
 ...
 I $D(DIRUT) S APCLSTP=1 Q
 S C=0 F N=1:1:APCLCNT Q:C=Y  S I=$R(APCLCNT) I I,$D(^TMP($J,"PATS",I)) S X=$O(^TMP($J,"PATS",I,0)),^XTMP("APCLDM81",APCLJOB,APCLBTH,"PATS",X)="",C=C+1 K ^TMP($J,"PATS",I,X)
 K ^TMP($J,"PATS")
 Q
...
FLAT ;
 ...
 W !!,"I am going to create a file called ",APCLFILE," which will reside in ",!,"the "
 W $S($P($G(^APCCCTRL(DUZ(2),0)),U,11)]"":$P(^APCCCTRL(DUZ(2),0),U,11),$P(^AUTTSITE(1,0),U,21)=1:"/usr/spool/uucppublic",$P($G(^AUTTSITE(1,1)),U,2)]"":$P(^AUTTSITE(1,1),U,2),1:"C:\EXPORT")
...
WRITEF ;EP write flat file
 ...
 S I=$O(^APCLRECD("B","DM AUDIT 2008 EPI REC 1",0))
 ...
 S XBMED="F",XBFN=APCLFILE,XBTLE="SAVE OF DM AUDIT 2008 EPI INFO RECORDS GENERATED BY -"_$P(^VA(200,DUZ,0),U)
 ...
 K ^XTMP("APCLDM81",APCLJOB,APCLBTH),APCLJOB,APCLBTH
 Q
BDMG(APCLJOB,APCLBTH,APCLDMRG,APCLADAT,APCLTYPE,APCLSTMP,APCLPCP,APCLCOM,APCLRAND,APCLRCNT,APCLCMS,APCLSTAT,APCLPREP,APCLFILE,APCLDSP,BDMGIEN,APCLSDPI,APCLSDPG,APCLPPN) ;PEP - gui call
 ...
 N APCLOPT  ;maw
 S APCLOPT="2008 Diabetes Program Audit"
 D NOW^%DTC
 S APCLNOW=$G(%)
 K DD,D0,DIC
 ...
 S ZTIO="",ZTDTH=$$NOW^XLFDT,ZTRTN="GUIEP^APCLD81",ZTDESC="GUI DM AUDIT" D ^%ZTLOAD
 D XIT
 Q
GUIEP ;EP - called from taskman
 ...
 D GUIR^XBLM("^APCLD81P","^TMP($J,""APCLDM81"",")
 Q:$G(APCLDSP)  ;quit if to screen
 S X=0,C=0 F  S X=$O(^TMP($J,"APCLDM81",X)) Q:X'=+X  D
 .S APCLDATA=^TMP($J,"APCLDM81",X)
...
TEST ;
 ...
 F X=1:1:10 S ^XTMP("APCLDM81",APCLJOB,APCLBTH,"PATS",X)=""
 D BDMG^APCLD81(APCLJOB,APCLBTH,1,DT,"P","","","","","","","",4,"TESTEPI",.APCLIEN)
 Q
