APCLD616 ;IHS/CMI/LAB - 2006 DIABETES AUDIT;     
 ;;3.0;IHS PCC REPORTS;**18,19,20**;FEB 05, 1997
 ;
...
TBCODE(P,EDATE,R) ;EP
 ...
 I X]"" Q 2
 S APCLR=$$PPD^APCLD618(P,EDATE)
 I APCLR["POS" D  Q APCLJ
 .I $$TBTX^APCLD612(P)["TX COMPLETE" S APCLJ=1 Q
 .S APCLJ=2
 .Q
 I APCLR["NEG" S APCLJ=4 D  Q APCLJ
 .I $$DODX(P,R,"I")="" S APCLJ=6 Q
 .S D=$$DODX(P,R,"I"),E=$$PPD^APCLD618(P,EDATE,"I") S APCLJ=$S(D>E:4,1:3)
 .Q
...
SYSMEAN(P,BDATE,EDATE) ;EP
 NEW X S X=$$BPS^APCLD613(P,BDATE,EDATE,"I")
 I X="" Q ""
...
DIAMEAN(P,BDATE,EDATE) ;EP
 ...
 Q C\3
PPDDATE(P,EDATE) ;EP
 NEW X S X=$$LASTNP^APCLD618(P,EDATE)
 Q X
LIPID(P,BDATE,EDATE) ;EP
 ...
 I 'T Q "None"
 S X=0 F  S X=$O(^ATXAX(T,21,X)) Q:X'=+X!(G)  S G=$$REFUSAL^APCLD617(P,50,$P(^ATXAX(T,21,X,0),U),BDATE,EDATE)
 I G Q "Refused or Adverse Rxn"
 NEW T S T=$O(^ATXAX("B","DM AUDIT STATIN DRUGS",0))
 I 'T G LIPIDA
 S (X,G)=0 F  S X=$O(^ATXAX(T,21,X)) Q:X'=+X!(G)  S G=$$REFUSAL^APCLD617(P,50,$P(^ATXAX(T,21,X,0),U),BDATE,EDATE)
 I G Q "Refused or Adverse Rxn"
LIPIDA ;
...
ACE(P,BDATE,EDATE) ;EP
 ...
 I 'T Q "No"
 S (G,X)=0 F  S X=$O(^ATXAX(T,21,X)) Q:X'=+X!(G)  S G=$$REFUSAL^APCLD617(P,50,$P(^ATXAX(T,21,X,0),U),BDATE,EDATE)
...
SELF(P,BDATE,EDATE) ;EP
 ...
 S E=$$LASTHF^APCLD619(P,"DIABETES SELF MONITORING",BD,ED,"F")
...
SDM(P,BDATE,EDATE) ;EP
 ...
 S X=EDATE,%DT="P" D ^%DT S ED=Y
 S X=BDATE,%DT="P" D ^%DT S BD=Y
 S E=$$LASTHF^APCLD619(P,"STAGED DIABETES MANAGEMENT",BD,ED)
 I E Q "Yes"
...
THERAPY(P,BD,EDATE) ;EP - therapy code for epi
 ...
 S STR="",TNAME=""
 S X=$$INSULIN^APCLD612(P,BD,EDATE)
 I X]"" S STR=STR_"2"
 S X=$$SULF^APCLD612(P,BD,EDATE)
 I X]"" S STR=STR_3
 S X=$$MET^APCLD612(P,BD,EDATE)
 I X]"" S STR=STR_4
 S X=$$ACAR^APCLD612(P,BD,EDATE)
 I X]"" S STR=STR_5
 S X=$$TROG^APCLD612(P,BD,EDATE)
 I X]"" S STR=STR_"6"
 I STR]"" Q STR
 S X=$$REFMED^APCLD612(P,BD,EDATE)
 I X Q 9
 Q 1
 ;
