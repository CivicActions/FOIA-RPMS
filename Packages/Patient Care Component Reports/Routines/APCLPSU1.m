...
COMM ;
 ...
 G:$D(DIRUT) GETDATES
 I Y="A" W !!,"All communities will be included in the report.",! G BIN
 I Y="O" D  G:'$D(APCLCOMM) COMM G:$D(APCLCOMM) BIN I 1
...
V ; Run by visit date
 ...
 Q
GETAGE ;
 S APCLDOB=$P(^DPT($P(^AMHPSUIC(APCLVDFN,0),U,4),0),U,3) Q:APCLDOB=""
ATT ;
...
V1 ;
 ...
 .S APCLAGE=""
 .F I=1:1 S APCLNN=$P(APCLBIN,";",I) Q:APCLNN=""  S APCLX=$P(APCLNN,"-"),APCLY=$P(APCLNN,"-",2) I A'<APCLX,A'>APCLY  S APCLAGE=I
 .;S APCLAGE=$S(A<0:" 0-0",A>0&(A<5):"1-4",A>4&(A<15):"5-14",A>14&(A<20):"15-19",A>19&(A<25):"20-24",A>24&(A<45):"25-44",A>44&(A<65):"45-64",A>64&(A<199):"65-125",1:"OTHER")
 .S APCLTOT(APCLAGE)=$G(APCLTOT(APCLAGE))+1
 ...
 ..S:Y="" Y="ZZZZZ" S:X="" X="DATA NOT ENTERED" S ^(X)=$S($D(^XTMP("APCLPSU1",APCLJ,APCLH,"TALLY","AGE",APCLAGE,APCLC,Y,X)):^(X)+1,1:1),^(X)=$S($D(^XTMP("APCLPSU1",APCLJ,APCLH,"TALLY","TOTAL",APCLC,Y,X)):^(X)+1,1:1)
 .;method 10
...
PRINT ;EP called from xbdbque
 ...
 S APCLAGE="" F  S APCLAGE=$O(^XTMP("APCLPSU1",APCLJ,APCLH,"TALLY","AGE",APCLAGE)) Q:APCLAGE=""!($D(APCLQUIT))  D
 .D HEAD Q:$D(APCLQUIT)
 .W !,"Age Range: ",$P(APCLBIN,";",APCLAGE)," years",?30,"Total # of Suicide Forms: ",APCLTOT(APCLAGE),!?63,"REPORT TOTALS"
 .S APCLV="" F  S APCLV=$O(^XTMP("APCLPSU1",APCLJ,APCLH,"TALLY","AGE",APCLAGE,APCLV)) Q:APCLV=""!($D(APCLQUIT))  D
 ..I $Y>(IOSL-6) D HEAD Q:$D(APCLQUIT)
 ..S APCLL=$P($T(@APCLV),";;",2) W !?1,$$LBLK(APCLL,28)
 ..S APCLY="" F  S APCLY=$O(^XTMP("APCLPSU1",APCLJ,APCLH,"TALLY","AGE",APCLAGE,APCLV,APCLY)) Q:APCLY=""!($D(APCLQUIT))  D
 ...S APCLX="" S APCLX=$O(^XTMP("APCLPSU1",APCLJ,APCLH,"TALLY","AGE",APCLAGE,APCLV,APCLY,APCLX)) Q:APCLX=""!($D(APCLQUIT))  D
 ....S X=^XTMP("APCLPSU1",APCLJ,APCLH,"TALLY","AGE",APCLAGE,APCLV,APCLY,APCLX)
 ....W ?31,$E(APCLX,1,30),?63,$J(X,4) S T=APCLTOT(APCLAGE) W ?72,$J(((X/T)*100),3,0)_"%",!
 ..Q
 .Q
 I $D(APCLQUIT) G DONE
 D HEAD Q:$D(APCLQUIT)
