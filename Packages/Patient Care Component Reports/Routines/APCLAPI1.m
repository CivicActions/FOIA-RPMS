APCLAPI1 ; IHS/CMI/LAB - visit data ; 02 Sep 2010  7:04 AM
 ;;2.0;IHS PCC SUITE;**2,5,7,10,11,16,23**;MAY 14, 2009;Build 17
...
LASTPAP(APCLPDFN,APCLBD,APCLED,APCLFORM) ;PEP - date of last PAP SMEAR
 ...
 NEW APCLLAST,APCLVAL,APCLX,R,X,Y,V,E
 S APCLLAST=""
 S APCLVAL=$$LASTLAB^APCLAPIU(APCLPDFN,$S($P(APCLLAST,U)]"":$P(APCLLAST,U),1:APCLBD),APCLED,,$O(^ATXLAB("B","BGP PAP SMEAR TAX",0)),,$O(^ATXAX("B","BGP PAP LOINC CODES",0)),"A")
 D E
 ...
 ;D E
 ;P23 ADDED 4 DX CODES PER DONNA REQS CR 9498
 S APCLVAL=$$LASTITEM^APCLAPIU(APCLPDFN,"V76.2","DX",$S($P(APCLLAST,U)]"":$P(APCLLAST,U),1:APCLBD),APCLED,"A")  ;P23
 D E
 S APCLVAL=$$LASTITEM^APCLAPIU(APCLPDFN,"V72.32","DX",$S($P(APCLLAST,U)]"":$P(APCLLAST,U),1:APCLBD),APCLED,"A")  ;P23
 D E
 S APCLVAL=$$LASTITEM^APCLAPIU(APCLPDFN,"Z12.4","DX",$S($P(APCLLAST,U)]"":$P(APCLLAST,U),1:APCLBD),APCLED,"A")  ;P23
 D E
 S APCLVAL=$$LASTITEM^APCLAPIU(APCLPDFN,"Z01.42","DX",$S($P(APCLLAST,U)]"":$P(APCLLAST,U),1:APCLBD),APCLED,"A")  ;P23
 D E
 ;S APCLVAL=$$LASTITEM^APCLAPIU(APCLPDFN,"[BJPC PAP SMEAR DXS","DX",$S($P(APCLLAST,U)]"":$P(APCLLAST,U),1:APCLBD),APCLED,"A")  ;P23
 ;D E
...
LASTTOBS(APCLPDFN,APCLBD,APCLED,APCLFORM) ;PEP - date of last TOBACCO USE SCREENING
 ...
 ;   - V Health Factor in Category TOBACCO (SMOKING), TOBACCO (SMOKELESS - CHEWING/DIP), 
 ...
 ;D E
 ;IHS/CMI/LAB - patch 23 added ENDS screening check cr 08759  - TOOK IT OUT PER CALL WITH PCC TEAM 1/9/19
 ;S APCLVAL=$$LASTHF^APCLAPIU(APCLPDFN,"ELECTRONIC NICOTINE DELIV SYSTEM (ENDS)",$S($P(APCLLAST,U)]"":$P(APCLLAST,U),1:APCLBD),APCLED,"A")
 ;D E
