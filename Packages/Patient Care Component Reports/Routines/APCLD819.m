APCLD819 ;IHS/CMI/LAB - 2008 DIABETES AUDIT;     
...
ZIS ;
 S XBRC="",XBRP="TAXCHK^APCLD819",XBNS="",XBRX="XIT^APCLD819"
 D ^XBDBQUE
 D XIT
 Q
TAXCHK ;EP
 ...
 .I '$D(^ATXLAB(I,21,"B")) S A(APCLX)=Y_"^has no entries " Q
 .I '$P(^ATXLAB(I,0),U,11) D
 ..;check for panels and warn
 ..S APCLY=0 F  S APCLY=$O(^ATXLAB(I,21,"B",APCLY)) Q:APCLY'=+APCLY  D
 ...I $O(^LAB(60,APCLY,2,0)) S C=C+1,B(APCLX,C)=Y_"^contains a panel test: "_$P(^LAB(60,APCLY,0),U)_" and should not."
 I $Y>(IOSL-2) D PAGE
 I '$D(A),'$D(B) W !,"All taxonomies are present.",! K A,APCLX,Y,I,Z Q
 ...
 .Q
 G:$D(APCLQUIT) DONE
 S APCLX="" F  S APCLX=$O(B(APCLX)) Q:APCLX=""!($D(APCLQUIT))  D
 .S APCLY=0 F  S APCLY=$O(B(APCLX,APCLY)) Q:APCLY'=+APCLY!($D(APCLQUIT))  D
 ..W !,$P(B(APCLX,APCLY),U)," [",APCLX,"] ",$P(B(APCLX,APCLY),U,2)
DONE ;
