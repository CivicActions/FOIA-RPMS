APCLDV2P ;IHS/CMI/LAB - list refusals [ 01/10/2005   3:42 PM ]
...
PRINT1 ;
 ...
 K APCLLSTP
 I '$D(^XTMP("APCLDV2",APCLJ,APCLH)) D HEADER W !!,"No data to report.",! G DONE
 D HEADER
 S APCLTOT=APCLCNT
 S APCLPTOT=$$PTOT
...
RES ;
 ...
 K APCLRES S X=0 F  S X=$O(^XTMP("APCLDV2",APCLJ,APCLH,"VSTS",X)) Q:X'=+X  D
 .S APCLY=^XTMP("APCLDV2",APCLJ,APCLH,"VSTS",X)
...
DATE ;
 ...
 K ^TMP($J) S X=0 F  S X=$O(^XTMP("APCLDV2",APCLJ,APCLH,"VSTS",X)) Q:X'=+X  D
 .S APCLY=^XTMP("APCLDV2",APCLJ,APCLH,"VSTS",X)
...
PTOT() ;
 NEW C,X
 S C=0
 S X=0 F  S X=$O(^XTMP("APCLDV2",APCLJ,APCLH,"PTS",X)) Q:X'=+X  S C=C+1
 Q C
TOT() ;
 NEW C,X
 S C=0
 S X=0 F  S X=$O(^XTMP("APCLDV2",APCLJ,APCLH,"VSTS",X)) Q:X'=+X  S C=C+1
 Q C
HEADER ;EP
 G:'APCLPG HEADER1
...
LIST ;EP - called from xbdbque
 ...
 K ^TMP($J)
 ;resort by sort item
 S APCLX=0 F  S APCLX=$O(^XTMP("APCLDV2",APCLJ,APCLH,"VSTS",APCLX)) Q:APCLX'=+APCLX  S APCLY=^XTMP("APCLDV2",APCLJ,APCLH,"VSTS",APCLX) D
 .S DFN=$P(APCLY,U,9)
 .D @APCLSORT
