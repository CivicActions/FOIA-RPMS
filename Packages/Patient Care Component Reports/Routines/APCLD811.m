APCLD811 ; IHS/CMI/LAB -IHS -CUMULATIVE REPORT    
...
IMM ;
 ...
 S V=$G(^XTMP("APCLDM81",APCLJOB,APCLBTH,"AUDIT",APCLPD,64))
 I $E(V)="Y" S $P(APCLCUML(140),U,3)=$P(APCLCUML(140),U,3)+1
 I $E(V)="R" S $P(APCLCUML(140),U,6)=$P(APCLCUML(140),U,6)+1
 S V=$G(^XTMP("APCLDM81",APCLJOB,APCLBTH,"AUDIT",APCLPD,66))
 I $E(V)="Y" S $P(APCLCUML(140),U,4)=$P(APCLCUML(140),U,4)+1
 I $E(V)="R" S $P(APCLCUML(140),U,7)=$P(APCLCUML(140),U,7)+1
 S V=$G(^XTMP("APCLDM81",APCLJOB,APCLBTH,"AUDIT",APCLPD,68))
 I $E(V)="Y" S $P(APCLCUML(140),U,5)=$P(APCLCUML(140),U,5)+1
 I $E(V)="R" S $P(APCLCUML(140),U,8)=$P(APCLCUML(140),U,8)+1
 ;
QUAN ;
 ;145
 ;title^total^uacr^upcr^other quant^albumin normal^micro present^overt present
 K APCLOFLG
 S APCLOFLG=0
 ;I '$D(^MAW($J,"UACR")) S ^MAW($J,"UACR")=0
 ;I '$D(^MAW($J,"UPCR")) S ^MAW($J,"UPCR")=0
 ;I '$D(^MAW($J,"OTH")) S ^MAW($J,"OTH")=0
 S:'$D(APCLCUML(145)) APCLCUML(145)="URINALYSIS"
 ;S:'$D(APCLCUML(146)) APCLCUML(146)="Total Urine Prot Not Known"
 S V=$G(^XTMP("APCLDM81",APCLJOB,APCLBTH,"AUDIT",APCLPD,91))
 ;S V=$$UACR^APCLD81C(APCLPD,APCLBDAT,APCLADAT) ;cmi/maw 12/17/2007 DM2008
 ;S A=$$UPCR^APCLD81C(APCLPD,APCLBDAT,APCLADAT) ;cmi/maw 12/17/2007 DM2008
 ;S O=$$QUAN^APCLD81C(APCLPD,APCLBDAT,APCLADAT) ;cmi/maw 12/17/2007 DM2008
 S A=$G(^XTMP("APCLDM81",APCLJOB,APCLBTH,"AUDIT",APCLPD,93))
 S O=$G(^XTMP("APCLDM81",APCLJOB,APCLBTH,"AUDIT",APCLPD,95))
 I $E(V)="X" D
 .;S $P(APCLCUML(146),U,2)=$P(APCLCUML(146),U,2)+1  ;1/9/08
 .S $P(APCLCUML(145),U,2)=$P(APCLCUML(145),U,2)+1  ;added for total 1/9/08
 .S $P(APCLCUML(145),U,3)=$P(APCLCUML(145),U,3)+1
 .;S ^MAW($J,"UACR")=^MAW($J,"UACR")+1
 .I $P(V,U,2)]"",$P(V,U,2)["<" S $P(APCLCUML(145),U,6)=$P(APCLCUML(145),U,6)+1 Q  ;albumin normal
 .I $P(V,U,2)]"",$P(V,U,2)["-" S $P(APCLCUML(145),U,7)=$P(APCLCUML(145),U,7)+1 Q  ;micro
 .I $P(V,U,2)]"",$P(V,U,2)[">" S $P(APCLCUML(145),U,8)=$P(APCLCUML(145),U,8)+1 Q  ;overt
 .I $P(V,U,2)]"" S $P(V,U,2)=$$STV^APCLD818($P(V,U,2),9) ;if stripping chars evals to null quit
 .Q:$P(V,U,2)=""  ;don't count if all chars were alpha
 .I $P(V,U,2)]"",$P(V,U,2)<30 S $P(APCLCUML(145),U,6)=$P(APCLCUML(145),U,6)+1  ;albumin normal
 .I $P(V,U,2)]"",$P(V,U,2)>30,$P(V,U,2)<299.9 S $P(APCLCUML(145),U,7)=$P(APCLCUML(145),U,7)+1  ;micro present
 .I $P(V,U,2)]"",$P(V,U,2)>299.9 S $P(APCLCUML(145),U,8)=$P(APCLCUML(145),U,8)+1  ;overt
 I $E(A)="X" D
 . ;S APCLOFLG=1,$P(APCLCUML(146),U,2)=$P(APCLCUML(146),U,2)+1  ;1/9/08
 . S $P(APCLCUML(145),U,2)=$P(APCLCUML(145),U,2)+1  ;added for total 1/9/08
 . Q:$E(V)="X"
 . ;S ^MAW($J,"UPCR")=^MAW($J,"UPCR")+1
 . S $P(APCLCUML(145),U,4)=$P(APCLCUML(145),U,4)+1
 I $E(O)="X" D
 . ;S APCLOFLG=1,$P(APCLCUML(146),U,2)=$P(APCLCUML(146),U,2)+1  ;1/9/08
 . S $P(APCLCUML(145),U,2)=$P(APCLCUML(145),U,2)+1  ;added for total 1/9/08
 . Q:$E(V)="X"
 . Q:$E(A)="X"
 . ;S ^MAW($J,"OTH")=^MAW($J,"OTH")+1
 . S $P(APCLCUML(145),U,5)=$P(APCLCUML(145),U,5)+1
 ;S OV=$G(^XTMP("APCLDM81",APCLJOB,APCLBTH,"AUDIT",APCLPD,94))
 ;I $E(OV)="Y" S $P(APCLCUML(145),U,8)=$P(APCLCUML(145),U,8)+1
 ;S Q=$G(^XTMP("APCLDM81",APCLJOB,APCLBTH,"AUDIT",APCLPD,96))
 ;I $E(Q,1,3)="Pos" S $P(APCLCUML(145),U,7)=$P(APCLCUML(145),U,7)+1
 K APCLOFLG  ;1/9/08
URIN ;cmi/maw 12/26/2007 no longer needed DM2008
 ...
 ;145
 ;'$D(APCLCUML(146)) APCLCUML(146)="URINALYSIS"
 ;S $P(APCLCUML(146),U,2)=$P(APCLCUML(146),U,2)+1
 ;S V=$G(^XTMP("APCLDM81",APCLJOB,APCLBTH,"AUDIT",APCLPD,92))
 ;S A=$G(^XTMP("APCLDM81",APCLJOB,APCLBTH,"AUDIT",APCLPD,94))
 ...
 ;.S V=$G(^XTMP("APCLDM81",APCLJOB,APCLBTH,"AUDIT",APCLPD,96))
...
EKG ;need date of last ekg
 ...
 S V=$$EKG^APCLD812(APCLPD,APCLRED,"I")
 I V]"" D
...
CREAT ;
 ...
 S V=$$CREAT^APCLD818(APCLPD,APCLBDAT,APCLADAT,"I")
 ...
 ;
GFR ;
 S:'$D(APCLCUML(175)) APCLCUML(175)="Estimated GFR documented in the past 12 months"
 S $P(APCLCUML(175),U,2)=$P(APCLCUML(175),U,2)+1
 ;S V=$$GFR^APCLD81C(APCLPD,APCLBDAT,APCLADAT)
 S V=$G(^XTMP("APCLDM81",APCLJOB,APCLBTH,"AUDIT",APCLPD,79))  ;maw 1/18/08
 I V="Yes" S $P(APCLCUML(175),U,5)=$P(APCLCUML(175),U,5)+1
 ;
TCHOL ;
 S:'$D(APCLCUML(180)) APCLCUML(180)="Total Cholesterol obtained in past 12 months"
 S $P(APCLCUML(180),U,2)=$P(APCLCUML(180),U,2)+1
 S V=$$CHOL^APCLD818(APCLPD,APCLBDAT,APCLADAT,"I")
...
LDL ;
 ...
 S V=$$LDL^APCLD818(APCLPD,APCLBDAT,APCLADAT,"I")
...
HDL ;
 ...
 S V=$$HDL^APCLD818(APCLPD,APCLBDAT,APCLADAT,"I")
...
TRIG ;
 ...
 S V=$$TRIG^APCLD818(APCLPD,APCLBDAT,APCLADAT,"I")
...
SELF ;
 ...
 ;S V=$G(^XTMP("APCLDM81",APCLJOB,APCLBTH,"AUDIT",APCLPD,98))
...
SDM  ;
 ...
 ;S V=$G(^XTMP("APCLDM81",APCLJOB,APCLBTH,"AUDIT",APCLPD,100))
 ;S P=$S($E(V)="Y":3,1:4)
...
CESS(P,B,D) ;EP - find any cessation hf in 12 months before E
 ...
 ;I 'T Q "No"
 S (X,G)=0
 I T F  S X=$O(^ATXAX(T,21,X)) Q:X'=+X!(G)  I $$REFUSAL^APCLD817(P,9999999.09,$P(^ATXAX(T,21,X,0),U),B,D) S G=1
 I G Q "Refused"
 S X=$O(^AUTTEDT("C","TO-Q",0))
 I X,$$REFUSAL^APCLD817(P,9999999.09,X,B,D) Q "Refused"
