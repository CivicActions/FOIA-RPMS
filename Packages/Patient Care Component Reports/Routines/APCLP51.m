APCLP51 ; IHS/CMI/LAB - IHS PRE Diabetes Audit 2005 ;      [ 03/02/2007  1:34 PM ]
 ;;3.0;IHS PCC REPORTS;**16,17,19**;FEB 05, 1997
 ;
BEGIN ;EP - called from option
 D TAXCHK^APCLP519  ;LORI RECHECK THIS
 W:$D(IOF) @IOF
 W !!!,$$CTR("ASSESSMENT OF PREDIABETES/METABOLIC SYNDROME CARE, 2005")
...
TYPE ;
 S APCLSTP=0
 K ^XTMP("APCLP51",APCLJOB,APCLBTH),^TMP($J,"PATS")
 S APCLTYPE=""
...
ZIS ;
 S XBRP="^APCLP51P",XBRC="^APCLP510",XBRX="XIT^APCLP51",XBNS="APCL"
 ;I APCLPREP=2 S XBRP="",XBRC="^APCLP510",XBRX="XIT^APCLP51",XBNS="APCL"
 D ^XBDBQUE
 D XIT
 Q
P ;
 S APCLSTP=0 K ^XTMP("APCLP51",APCLJOB,APCLBTH),^TMP($J,"PATS")
P1 ;
 K DIC S DIC="^AUPNPAT(",DIC(0)="AEMQ" D ^DIC K DIC
 I Y=-1,'$D(^XTMP("APCLP51",APCLJOB,APCLBTH,"PATS")) W !,"No patients selected" S APCLSTP=1 Q
 I Y=-1 Q
 S ^XTMP("APCLP51",APCLJOB,APCLBTH,"PATS",+Y)=""
 G P1
 Q
S ; Get patient name or cohort
 K ^XTMP("APCLP51",APCLJOB,APCLBTH),^TMP($J,"PATS") S APCLSTP=0
...
RAND ;random sample or not
 ...
 G:$D(DIRUT) C
 I Y="A" S C=0 F  S C=$O(^TMP($J,"PATS",C)) Q:C'=+C  S X=$O(^TMP($J,"PATS",C,0)),^XTMP("APCLP51",APCLJOB,APCLBTH,"PATS",X)=""
 ...
 I $D(DIRUT) S APCLSTP=1 Q
 S C=0 F N=1:1:APCLCNT Q:C=Y  S I=$R(APCLCNT) I I,$D(^TMP($J,"PATS",I)) S X=$O(^TMP($J,"PATS",I,0)),^XTMP("APCLP51",APCLJOB,APCLBTH,"PATS",X)="",C=C+1 K ^TMP($J,"PATS",I,X)
 K ^TMP($J,"PATS")
 Q
...
XIT1 ;
 ...
 K ^XTMP("APCLP51",APCLJOB,APCLBTH),APCLJOB,APCLBTH
XIT ;
...
BDMG(APCLJOB,APCLBTH,APCLDMRG,APCLADAT,APCLTYPE,APCLSTMP,APCLPCP,APCLCOM,APCLRAND,APCLRCNT,APCLCMS,APCLSTAT,APCLPREP,APCLFILE,APCLDSP,BDMGIEN,APCLSDPI,APCLSDPG) ;PEP - gui call
 ...
 S APCLNOW=$G(%)
 N APCLOPT  ;maw
 S APCLOPT="DM 2005 Pre-Diabetes Audit"  ;maw
 K DD,D0,DIC
 ...
 S ZTIO="",ZTDTH=$$NOW^XLFDT,ZTRTN="GUIEP^APCLP51",ZTDESC="GUI DM AUDIT" D ^%ZTLOAD
 D XIT
 Q
GUIEP ;EP - called from taskman
 ...
 D GUIR^XBLM("^APCLP51P","^TMP($J,""APCLP51"",")
 Q:$G(APCLDSP)  ;quit if to screen
 S X=0,C=0 F  S X=$O(^TMP($J,"APCLP51",X)) Q:X'=+X  S ^APCLGUIR(APCLIEN,11,X,0)=^TMP($J,"APCLP51",X),C=C+1
...
TEST ;
 ...
 F X=1:1:10 S ^XTMP("APCLP51",APCLJOB,APCLBTH,"PATS",X)=""
 D BDMG^APCLP51(APCLJOB,APCLBTH,1,DT,"P","","","","","","","",4,"TESTEPI",.APCLIEN)
 Q
