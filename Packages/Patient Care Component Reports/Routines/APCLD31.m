APCLD31 ; IHS/CMI/LAB - IHS Diabetes Audit 2003 ;     [ 03/02/2007  1:35 PM ]
 ;;3.0;IHS PCC REPORTS;**13,16,17,19**;FEB 05, 1997
 ;
BEGIN ;EP - called from option
 D TAXCHK^APCLD319
 W:$D(IOF) @IOF
 W !!!,$$CTR("ASSESSMENT OF DIABETES CARE, 2003")
...
TYPE ;
 S APCLSTP=0
 K ^XTMP("APCLDM31",APCLJOB,APCLBTH),^TMP($J,"PATS")
 S APCLTYPE=""
...
ZIS ;
 I APCLPREP'=2 S XBRP="^APCLD31P",XBRC="^APCLD310",XBRX="XIT^APCLD31",XBNS="APCL"
 I APCLPREP=2 S XBRP="",XBRC="^APCLD310",XBRX="XIT^APCLD31",XBNS="APCL"
 D ^XBDBQUE
 D XIT
 Q
P ;
 S APCLSTP=0 K ^XTMP("APCLDM31",APCLJOB,APCLBTH),^TMP($J,"PATS")
P1 ;
 K DIC S DIC="^AUPNPAT(",DIC(0)="AEMQ" D ^DIC K DIC
 I Y=-1,'$D(^XTMP("APCLDM31",APCLJOB,APCLBTH,"PATS")) W !,"No patients selected" S APCLSTP=1 Q
 I Y=-1 Q
 S ^XTMP("APCLDM31",APCLJOB,APCLBTH,"PATS",+Y)=""
 G P1
 Q
S ; Get patient name or cohort
 K ^XTMP("APCLDM31",APCLJOB,APCLBTH),^TMP($J,"PATS") S APCLSTP=0
...
RAND ;random sample or not
 ...
 G:$D(DIRUT) C
 I Y="A" S C=0 F  S C=$O(^TMP($J,"PATS",C)) Q:C'=+C  S X=$O(^TMP($J,"PATS",C,0)),^XTMP("APCLDM31",APCLJOB,APCLBTH,"PATS",X)=""
 ...
 I $D(DIRUT) S APCLSTP=1 Q
 S C=0 F N=1:1:APCLCNT Q:C=Y  S I=$R(APCLCNT) I I,$D(^TMP($J,"PATS",I)) S X=$O(^TMP($J,"PATS",I,0)),^XTMP("APCLDM31",APCLJOB,APCLBTH,"PATS",X)="",C=C+1 K ^TMP($J,"PATS",I,X)
 K ^TMP($J,"PATS")
 Q
...
XIT1 ;
 ...
 K ^XTMP("APCLDM31",APCLJOB,APCLBTH),APCLJOB,APCLBTH
XIT ;
...
WRITEF ;EP write flat file
 ...
 K ^XTMP("APCLDM31",APCLJOB,APCLBTH),APCLJOB,APCLBTH
 Q
BDMG(APCLJOB,APCLBTH,APCLDMRG,APCLADAT,APCLTYPE,APCLSTMP,APCLPCP,APCLCOM,APCLRAND,APCLRCNT,APCLCMS,APCLSTAT,APCLPREP,APCLFILE,APCLDSP,BDMGIEN) ;PEP - gui call
 ...
 N APCLOPT  ;maw
 S APCLOPT="2003 Diabetes Program Audit"
 D NOW^%DTC
 S APCLNOW=$G(%)
 K DD,D0,DIC
 ...
 S ZTIO="",ZTDTH=$$NOW^XLFDT,ZTRTN="GUIEP^APCLD31",ZTDESC="GUI DM AUDIT" D ^%ZTLOAD
 D XIT
 Q
GUIEP ;EP - called from taskman
 ...
 D GUIR^XBLM("^APCLD31P","^TMP($J,""APCLDM31"",")
 Q:$G(APCLDSP)  ;quit if to screen
 S X=0,C=0 F  S X=$O(^TMP($J,"APCLDM31",X)) Q:X'=+X  S ^APCLGUIR(APCLIEN,11,X,0)=^TMP($J,"APCLDM31",X),C=C+1
...
TEST ;
 ...
 F X=1:1:10 S ^XTMP("APCLDM31",APCLJOB,APCLBTH,"PATS",X)=""
 D BDMG^APCLD31(APCLJOB,APCLBTH,1,DT,"P","","","","","","","",4,"TESTEPI",.APCLIEN)
 Q
