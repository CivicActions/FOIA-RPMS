APCLD316 ;IHS/CMI/LAB - 2003 DIABETES AUDIT;     [ 07/14/03  5:43 PM ]
 ;;3.0;IHS PCC REPORTS;**13,14,20**;FEB 05, 1997
 ;
...
TBCODE(P,EDATE,R) ;EP
 ...
 I X]"" Q 2
 I $$PPD^APCLD318(P,EDATE)["POS" D  Q APCLJ
 .I $$TBTX^APCLD312(P)["TX COMPLETE" S APCLJ=1 Q
 .S APCLJ=2
 .Q
 I $$PPD^APCLD318(P,EDATE)["NEG" S APCLJ=4 D  Q APCLJ
 .I $$DODX(P,R,"I")="" S APCLJ=4 Q
 .S D=$$DODX(P,R,"I"),E=$$PPD^APCLD318(P,EDATE,"I") S APCLJ=$S(D>E:4,1:3)
 .Q
 S APCLJ=5
 Q APCLJ
 ;;
...
DIAMEAN(P,BDATE,EDATE) ;EP
 ...
 Q C\3
PPDDATE(P,EDATE) ;EP
 NEW X S X=$$LASTNP^APCLD318(P,EDATE)
 Q X
LIPID(P,BDATE,EDATE) ;EP
 ...
 I 'T Q "None"
 S X=0 F  S X=$O(^ATXAX(T,21,X)) Q:X'=+X!(G)  S G=$$REFUSAL^APCLD317(P,50,$P(^ATXAX(T,21,X,0),U),BDATE,EDATE)
 I G,$P(G,U,2)'="N" Q "Refused"
 I G Q "None - Not Medically Indicated"
 NEW T S T=$O(^ATXAX("B","DM AUDIT STATIN DRUGS",0))
 I 'T Q "None"
 S (X,G)=0 F  S X=$O(^ATXAX(T,21,X)) Q:X'=+X!(G)  S G=$$REFUSAL^APCLD317(P,50,$P(^ATXAX(T,21,X,0),U),BDATE,EDATE)
 I G,$P(G,U,2)'="N" Q "Refused"
 I G Q "None - Not Medically Indicated"
 ;
 Q "None"
...
ACE(P,BDATE,EDATE) ;EP
 ...
 I 'T Q "No"
 S (G,X)=0 F  S X=$O(^ATXAX(T,21,X)) Q:X'=+X!(G)  S G=$$REFUSAL^APCLD317(P,50,$P(^ATXAX(T,21,X,0),U),BDATE,EDATE)
 I G,$P(G,U,2)'="N" Q "Refused"
...
SELF(P,BDATE,EDATE) ;EP
 ...
 S E=$$LASTHF^APCLD319(P,"DIABETES SELF MONITORING",BD,ED,"F")
...
SDM(P,BDATE,EDATE) ;EP
 ...
 S X=EDATE,%DT="P" D ^%DT S ED=Y
 S X=BDATE,%DT="P" D ^%DT S BD=Y
 S E=$$LASTHF^APCLD319(P,"STAGED DIABETES MANAGEMENT",BD,ED)
 I E Q "Yes"
...
THERAPY(P,BD,EDATE) ;EP - therapy code for epi
 ...
 S STR="",TNAME=""
 S X=$$INSULIN^APCLD312(P,BD,EDATE)
 I X]"" S STR=STR_"2"
 S X=$$SULF^APCLD312(P,BD,EDATE)
 I X]"" S STR=STR_3
 S X=$$MET^APCLD312(P,BD,EDATE)
 I X]"" S STR=STR_4
 S X=$$ACAR^APCLD312(P,BD,EDATE)
 I X]"" S STR=STR_5
 S X=$$TROG^APCLD312(P,BD,EDATE)
 I X]"" S STR=STR_"6"
 I STR]"" Q STR
 Q 1
 ;
...
DODX(P,R,F) ;EP - date of dx for epi file
 ...
 I $G(R) S DATE=$$CMSFDX^APCLD313(P,R,"ID")
 I DATE]"" S EARLY=DATE
 S DATE=$$PLDMDOO^APCLD313(P,"I")
