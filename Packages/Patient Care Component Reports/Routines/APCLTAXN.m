APCLTAXN ; IHS/CMI/LAB - CODE RANGES IN TABLE ; [ 02/04/97 1:17 PM ]
...
ADD8 ;;
 ...
 Q
ADD1 ;;
 I $O(^ICPT("BA",APCL("HI")))=APCLVAL,$E(APCLVAL)=$E(APCL("HI"))!(APCLVAL&APCL("HI")) S APCLTBLE(APCL("LOW"))=APCLTBLE(APCLVAL) K APCLTBLE(APCLVAL) S APCLDONE=1
 Q
ADD13 ;;
 I $O(^ICPT("BA",APCLTBLE(APCLVAL)))=APCL("LOW"),$E(APCLVAL)=$E(APCL("LOW"))!(APCLVAL&APCL("LOW")) S APCL("LOW")=APCLVAL K APCLTBLE(APCLVAL)
 ; continue
 Q
 ;
SETRANGE ;
...
DEL2 ;;
 ...
 E  S APCLTBLE($O(^ICPT("BA",APCL("HI"))))=APCLTBLE(APCLVAL) K APCLTBLE(APCLVAL)
 S APCLDONE=1
 Q
 ;
DEL3 ;;
DEL6 ;;
 S APCLTBLE($O(^ICPT("BA",APCL("HI"))))=APCLTBLE(APCLVAL) K APCLTBLE(APCLVAL)
 S APCLDONE=1
 Q
 ;
DEL4 ;;
...
DEL9 ;;
 D @$S($E(APCLTBLE(APCLVAL))?1N:"NUMBER",1:"LETTER")
 S APCLTBLE($O(^ICPT("BA",APCL("HI"))))=APCLTBLE(APCLVAL)
 S APCLTBLE(APCLVAL)=APCL("NEWHI")
 S APCLDONE=1
 Q
 ;
...
CASEC ;
 ...
 ;
NUMBER ;
 S APCL("CODE")=APCL("LOW")-5 F  Q:APCL("LOW")]$O(^ICPT("BA",APCL("CODE")_" "))  S APCL("CODE")=APCL("CODE")-5
 S APCL("CODE")=$O(^ICPT("BA",APCL("CODE")_" ")) F  S APCL("NEWHI")=APCL("CODE"),APCL("CODE")=$O(^ICPT("BA",APCL("CODE"))) Q:APCL("CODE")=APCL("LOW")
 Q
 ;
LETTER ;
 S APCL("LET")=$E(APCL("LOW"))_" " F  S APCL("NEWHI")=$S($L(APCL("LET"))>2:APCL("LET"),1:APCL("LOW")),APCL("LET")=$O(^ICPT("BA",APCL("LET"))) Q:APCL("LET")=APCL("LOW")
 Q
EOJ ;
