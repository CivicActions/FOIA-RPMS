APCLD817 ;IHS/CMI/LAB - 2008 DIABETES AUDIT;      [ 02/12/2008  2:08 PM ]
...
EXEDUC(P,BDATE,EDATE) ;EP
 ...
 I $D(APCL(1)) Q "Yes"
 K APCL
 S X=P_"^ALL EDUC;DURING "_BDATE_"-"_EDATE S E=$$START1^APCLDF(X,"APCL(")
 S X=0,G=0 F  S X=$O(APCL(X)) Q:X'=+X!(G)  S I=+$P(APCL(X),U,4),I=$P($G(^AUPNVPED(I,0)),U),T=$P($G(^AUTTEDT(I,0)),U,2) I $P(T,"-",2)="EX" S G=1
 I G=1 Q "Yes"
 K APCL
 S X=P_"^LAST DX V65.41;DURING "_BDATE_"-"_EDATE S E=$$START1^APCLDF(X,"APCL(")
 I $D(APCL(1)) Q "Yes"
 S G=0
...
DFE(P,BDATE,EDATE) ;EP
 ...
 NEW X,Y,R S (X,Y)=0 F  S X=$O(APCL(X)) Q:X'=+X!(Y)  S R=$$PRIMPROV^APCLV($P(APCL(X),U,5),"D") I (R=33!(R=25)!(R=84)),'$$DNKA($P(APCL(X),U,5)) S Y=1_U_$P(APCL(X),U)
 I Y Q "Yes - Podiatrist Visit "_$$FMTE^XLFDT($P(Y,U,2),5)
 S X=0,Y=0 F  S X=$O(APCL(X)) Q:X'=+X!(Y)  S C=$$CLINIC^APCLV($P(APCL(X),U,5),"C") I C=65!(C="B7"),'$$DNKA($P(APCL(X),U,5)) S Y=1_U_$P(APCL(X),U)
 I Y Q "Yes - Podiatry Cln visit "_$$FMTE^XLFDT($P(Y,U,2),5)
...
EYE(P,BDATE,EDATE) ;EP
 NEW APCL,%,E,X K APCL S %=P_"^LAST EXAM DIABETIC EYE EXAM;DURING "_BDATE_"-"_EDATE,E=$$START1^APCLDF(%,"APCL(")
