...
START ;
 ...
 K ^XTMP("APCLAUD2",APCLJOB,APCLBT)
...
ICDB101 ;
 ;W !!,$S(APCLPSD="P":"Includes PRIMARY Diagnoses only.",1:"Includes PRIMARY and SECONDARY diagnoses.")
 ...
 Q
ICDB11 ;
 K ^XTMP("APCLAUD",APCLJOB,APCLBT,"ICDDFN")
 S DIR(0)="Y",DIR("A")="Do you want all ICD Coded diagnoses in the report",DIR("B")="Y" D ^DIR K DIR S:$D(DUOUT) DIRUT=1
 I $D(DIRUT) G BD
 I Y=1 S ^XTMP("APCLAUD",APCLJOB,APCLBT,"ICDDFN","ALL")="" G PRIM
ICDB2 D ^APCLAUD5
PRIM ;only primary dx?
 S APCLPSD=""
 S DIR(0)="S^P:PRIMARY Diagnosis Only;B:Both PRIMARY and SECONDARY Diagnoses",DIR("A")="Include which Diagnoses in this search",DIR("B")="B" KILL DA D ^DIR KILL DIR
 I $D(DIRUT) G ICDB11
 S APCLPSD=Y
ICDE1 W:$D(IOF) @IOF
 G ^APCLAUD0
