...
PR10 I $D(^XTMP("APCLAUD2",APCLJOB,APCLBT,APCLPNO,APCLIRNG,0)) S APCLCNT=^(0) D SETCAR S APCLCNTR=0 D ICDLN^APCLAUD4 Q:$D(APCLQ)
 S APCLICDC="" F  S APCLICDC=$O(^XTMP("APCLAUD2",APCLJOB,APCLBT,APCLPNO,APCLIRNG,APCLICDC)) Q:APCLICDC=""!($D(APCLQ))  D
 .S APCLINO=0
 .F  S APCLINO=$O(^XTMP("APCLAUD2",APCLJOB,APCLBT,APCLPNO,APCLIRNG,APCLICDC,APCLINO)) Q:APCLINO=""!($D(APCLQ))  D PR11
 Q
PR11 ;
 ;S APCLICNO=$P(^ICD9(APCLINO,0),"^"),APCLVNO=0
 S APCLICNO=$P($$ICDDX^ICDEX(APCLINO),"^",2),APCLVNO=0
 F  S APCLVNO=$O(^XTMP("APCLAUD2",APCLJOB,APCLBT,APCLPNO,APCLIRNG,APCLICDC,APCLINO,APCLVNO)) Q:APCLVNO=""!($D(APCLQ))  D PR12
 Q
...
PR120 S APCLVDT=$P($P(^AUPNVSIT(APCLVNO,0),"^"),"."),Y=APCLVDT X ^DD("DD") S APCLVDT=Y,APCLPTNM=$P(^AUPNVSIT(APCLVNO,0),"^",5),APCLHRN=$S($D(^AUPNPAT(APCLPTNM,41,APCLSITE,0)):$P(^(0),"^",2),1:"")
 ...
 S APCLPNO1=$P(^XTMP("APCLAUD2",APCLJOB,APCLBT,APCLPNO,APCLIRNG,APCLICDC,APCLINO,APCLVNO),U),APCLPOVD=$P(^(APCLVNO),U,2)
...
HEAD1 W:$D(IOF) @IOF
 ...
 W !!,"HRCN",?10,"Visit Date",?24,"Primary Provider",?45,"Patient Name",?70,"DOB",!?10,"ICD CODE",?24,"DIAGNOSIS"
 W !,APCL80D
 Q
SETCAR K APCLCAR
