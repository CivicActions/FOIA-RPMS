APCLD51E ; IHS/CMI/LAB - IHS Diabetes Audit 2005 ;      [ 03/02/2007  1:36 PM ]
 ;;3.0;IHS PCC REPORTS;**16,17,19**;FEB 05, 1997
 ;
BEGIN ;EP - called from option
 D TAXCHK^APCLD519
 W:$D(IOF) @IOF
REGASK ;
 W !,$$CTR("ASSESSMENT OF DIABETES CARE, 2005")
 W !,$$CTR("PCC DIABETES E-AUDIT")
 W !!,"This option is used to run the 2005 Electronic Diabetes Audit for a"
...
ZIS ;
 I APCLPREP'=2 S XBRP="^APCLD51P",XBRC="EAUDIT^APCLD510",XBRX="XIT^APCLD51",XBNS="APCL"
 I APCLPREP=2 S XBRP="",XBRC="EAUDIT^APCLD510",XBRX="XIT^APCLD51",XBNS="APCL"
 D ^XBDBQUE
 D XIT
 Q
P ;
 S APCLSTP=0 K ^XTMP("APCLDM51",APCLJOB,APCLBTH),^TMP($J,"PATS")
P1 ;
 K DIC S DIC="^AUPNPAT(",DIC(0)="AEMQ" D ^DIC K DIC
 I Y=-1,'$D(^XTMP("APCLDM51",APCLJOB,APCLBTH,"PATS")) W !,"No patients selected" S APCLSTP=1 Q
 I Y=-1 Q
 S ^XTMP("APCLDM51",APCLJOB,APCLBTH,"PATS",+Y)=""
 G P1
 Q
S ; Get patient name or cohort
 K ^XTMP("APCLDM51",APCLJOB,APCLBTH),^TMP($J,"PATS") S APCLSTP=0
...
C ;get register, status, random or not
 K ^XTMP("APCLDM51",APCLJOB,APCLBTH),^TMP($J,"PATS")
 S APCLCMS="",APCLSTP=0
...
RAND ;random sample or not
 ...
 G:$D(DIRUT) C
 I Y="A" S C=0 F  S C=$O(^TMP($J,"PATS",C)) Q:C'=+C  S X=$O(^TMP($J,"PATS",C,0)),^XTMP("APCLDM51",APCLJOB,APCLBTH,"PATS",X)=""
 ...
 I $D(DIRUT) S APCLSTP=1 Q
 S C=0 F N=1:1:APCLCNT Q:C=Y  S I=$R(APCLCNT) I I,$D(^TMP($J,"PATS",I)) S X=$O(^TMP($J,"PATS",I,0)),^XTMP("APCLDM51",APCLJOB,APCLBTH,"PATS",X)="",C=C+1 K ^TMP($J,"PATS",I,X)
 K ^TMP($J,"PATS")
 Q
...
WRITEF ;EP write flat file
 ...
 S I=$O(^APCLRECD("B","DM AUDIT 2005 EPI REC 1",0))
 ...
 S XBMED="F",XBFN=APCLFILE,XBTLE="SAVE OF DM AUDIT 2005 EPI INFO RECORDS GENERATED BY -"_$P(^VA(200,DUZ,0),U)
...
BDMG(APCLJOB,APCLBTH,APCLDMRG,APCLADAT,APCLTYPE,APCLSTMP,APCLPCP,APCLCOM,APCLRAND,APCLRCNT,APCLCMS,APCLSTAT,APCLPREP,APCLFILE,APCLDSP,BDMGIEN,APCLSDPI,APCLSDPG) ;PEP - gui call
 ...
 N APCLOPT  ;maw
 S APCLOPT="Run the 2005 Audit w/predefined set of Pts"  ;maw
 D NOW^%DTC
 S APCLNOW=$G(%)
 K DD,D0,DIC
 ...
 S ZTIO="",ZTDTH=$$NOW^XLFDT,ZTRTN="GUIEP^APCLD51E",ZTDESC="GUI DM AUDIT" D ^%ZTLOAD
 D XIT
 Q
GUIEP ;EP - called from taskman
 ...
 D GUIR^XBLM("^APCLD51P","^TMP($J,""APCLDM51"",")
 Q:$G(APCLDSP)  ;quit if to screen
 S X=0,C=0 F  S X=$O(^TMP($J,"APCLDM51",X)) Q:X'=+X  S ^APCLGUIR(APCLIEN,11,X,0)=^TMP($J,"APCLDM51",X),C=C+1
...
TEST ;
 ...
 F X=1:1:10 S ^XTMP("APCLDM51",APCLJOB,APCLBTH,"PATS",X)=""
 D BDMG^APCLD51(APCLJOB,APCLBTH,1,DT,"P","","","","","","","",4,"TESTEPI",.APCLIEN)
 Q
