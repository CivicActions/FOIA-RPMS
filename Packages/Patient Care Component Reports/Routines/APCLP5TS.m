APCLP5TS ; IHS/CMI/LAB - DISPLAY IND LISTS ;   
...
EN ;EP -- main entry point for 
 D EN^VALM("APCLP5 TAXONOMY UPDATE")
 D CLEAR^VALM1
 D FULL^VALM1
 W:$D(IOF) @IOF
 D EOJ
...
ADD ;EP - add an item to the selected list - called from a protocol
 ...
 I APCLTAXT="L",$P(^ATXLAB(APCLTAXI,0),U,22) W !!,"The ",$P(^ATXLAB(APCLTAXI,0),U)," is READ ONLY.",!,"You can not update it." D PAUSE G ADDX
 I APCLTAXT="T",$P(^ATXAX(APCLTAXI,0),U,22) W !!,"The ",$P(^ATXAX(APCLTAXI,0),U)," is READ ONLY.",!,"You can not update it." D PAUSE G ADDX
 D FULL^VALM1 W:$D(IOF) @IOF
 D EP^APCLP5TL(APCLTAXI)
ADDX ;
 D BACK
 Q
