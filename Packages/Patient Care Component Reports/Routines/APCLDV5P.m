...
RES ;
 ...
 K APCLRES S X=0 F  S X=$O(^XTMP("APCLDV5",APCLJ,APCLH,"VSTS",X)) Q:X'=+X  D
 .S APCLY=^XTMP("APCLDV5",APCLJ,APCLH,"VSTS",X)
...
DATE ;
 ...
 K ^TMP($J) S X=0 F  S X=$O(^XTMP("APCLDV5",APCLJ,APCLH,"VSTS",X)) Q:X'=+X  D
 .S APCLY=^XTMP("APCLDV5",APCLJ,APCLH,"VSTS",X)
...
PTOT() ;
 NEW C,X
 S C=0
 S X=0 F  S X=$O(^XTMP("APCLDV5",APCLJ,APCLH,"PTS",X)) Q:X'=+X  S C=C+1
 Q C
TOT() ;
 NEW C,X
 S C=0
 S X=0 F  S X=$O(^XTMP("APCLDV5",APCLJ,APCLH,"VSTS",X)) Q:X'=+X  S C=C+1
 Q C
HEADER ;EP
 G:'APCLPG HEADER1
...
HEADER1 ;
 ...
 W !,$$CTR("***  IPV SCREENING VISIT TALLY"_$S(APCLLIST:" AND VISIT LISTING",1:"")_"  ***",80),!
...
LIST ;EP - called from xbdbque
 ...
 K ^TMP($J)
 ;resort by sort item
 S APCLX=0 F  S APCLX=$O(^XTMP("APCLDV5",APCLJ,APCLH,"VSTS",APCLX)) Q:APCLX'=+APCLX  S APCLY=^XTMP("APCLDV5",APCLJ,APCLH,"VSTS",APCLX) D
 .S DFN=$P(APCLY,U,9)
 .D @APCLSORT
 ...
 .Q:$D(^XTMP("APCLDV5",APCLJ,APCLH,"PTS",DFN))
 .Q:$$END
