ACHSSVRQ ; IHS/ADC/GTH - SELECT AND PRINT AO SPECIAL VENDOR REPORT ; [ 10/22/1999  2:01 PM ]
 ;;3.0;CONTRACT HEALTH MGMT SYSTEM;**9,12**;SEP 17, 1997
 ;;ACHS*3*9 IV&V FIX: 2 DIGIT DATE COMPAIRISON FAILURE IN 2000
 ;;ACHS*3*12;IHS/DSD/CS;2990929;XXX-0999-200030; CORRECTIONS OF VARIABLE MISSPELLINGS
 ;
A2 ;
...
ZTLOAD ;
 S ZTRTN="PRINT^ACHSSVRQ",ZTDESC="PRINT SPECIAL AO VENDOR REPORTS",ZTIO=IO
...
INDEXC ;
 ...
 S ACHSADT=$S($E(ACHSADT,1,2)<84:20000000,1:19000000) ;ACHS*3*9
 ;BEGIN OLD CODE FOR XXX-0999-200030
 ;S X=$P(^ACHSSVR(ACHSIEN,0),U,1) ;ACHS*3*9
 ;END OLD CODE FOR XXX-0999-200030
 ;BEGIN NEW CODE FOR XXX-0999-200030
 ...
 ;BEGIN OLD CODE FOR XXX-0999-200030
 ;S:ACHSADT<X $P(^ACHSSVR(ACHSIEN,0),U,1)=$E(ACHSADT,3,8)
 ...
 S X=$P(^ACHSSVR(ACHSEIN,0),U,2) ; XXX-0999-200030
 ;END NEW CODE FOR XXX-0999-200030
 S X=$S($E(X,1,2)<84:20000000,1:19000000)+X ;ACHS*3*9
