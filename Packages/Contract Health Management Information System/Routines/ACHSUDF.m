ACHSUDF ; IHS/ITSC/PMF - FORMAT DOCUMENT DATA FOR PRINT/DISPLAY(1/2) ;    [ 01/10/2005  9:14 AM ]
 ;;3.1;CONTRACT HEALTH MGMT SYSTEM;**7,11,12,15,16,19,21,27,29,30**;JUNE 11,2001;Build 39
 ;ITSC/SET/JVK ACHS*3.1*7 - USE E-SIG IN DOCUMENT GLOBAL
 ;ITSC/SET/JVK ACHS*3.1*11 MODIFIED TO DISPLAY MEDICARE PROVIDER
 ;ITSC/SET/JV*29K ACHS*3.1*12- 1.4.04 MOD FOR PAWNEE BEN PKG
 ...
 ;ACHS*3.1*29 07/01/2021 IHS.OIT.FCJ No longer printing SSN on PO;ADD PREFERRED NAME FOR SCREEN DISPLAYS
 ;ACHS*3.1*30 4.28.22 IHS.OIT.FCJ CHG DUNS TO UEI and Add PNN API
 ;
PTA ;
...
P4 ;
 ...
 ;ACHS*3.1*16 11/12/2009 IHS.OIT.FCJ CHNG THE DISPLAY OF SSN;ACHS*3.1*29 NO LONGER PRINTING SSN
...
PRO3 ;
 ...
 ;ACHS*3.1*16 11.12.2009 IHS.OIT.FCJ TEST FOR DUNS PARAMETER PRINT DUNS INSTEAD OF UPIN;;ACHS*3.1*30- CHG TO UEI
 ;I $$PARM^ACHS(2,13)="Y" S D(8)=$P(^AUTTVNDR(ACHSPROV,0),U,7)
 I $$PARM^ACHS(2,13)="Y" S D(8)=$P(^AUTTVNDR(ACHSPROV,0),U,10)
...
ALL ;EP.
 ...
 D PTA,FAC,PRO
 ;S:$D(DFN) A(2)=$E($$GETPREF^AUPNSOGI(DFN,"E"),1,37) ;ACHS*3.1*29 10.1.2021 IHS.OIT.FCJ
 S:$D(DFN) A(2)=$E($$GETPREF^AUPNSOGI(DFN,"E",1),1,37) ;ACHS*3.1*30 5.3.22 TEST FOR REG PARM
 G UDF1
 ;
PRT ;EP.
 D PTA:DFN]"",PRO
