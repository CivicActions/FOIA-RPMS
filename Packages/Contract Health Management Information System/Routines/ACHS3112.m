ACHS3112 ;IHS/ITSC/JVK - ACHS 3.1 PATCH 12 ;  [ 03/24/2005  8:31 AM ]
 ;;3.1;CONTRACT HEALTH MGMT SYSTEM;**12**;JUNE 11, 2001
 ;3.1*12 12/23/2004;IHS/ITSC/JVK - ORIG RTN P10
 ;3.1*11 8/25/2004;IHS/ITSC/FCJ;ORIGINAL RTN FR P10 
 ;
 I '$G(IOM) D HOME^%ZIS
 ...
 ;
 I $$VCHK("ACHS","3.1",2,"'=")
 I '$$INSTALLD("AUT*98.1*15") D
 .W !,$$CJ^XLFSTR("AUT*98.1*15 IS REQUIRED"_IORVOFF,IOM)
 .S XPDQUIT=2
 ;
 NEW DA,DIC
 ...
 ;
 D HELP^XBHELP("INTROE","ACHS3112")
 I '$$DIR^XBDIR("E","","","","","",1) D SORRY(2) Q
 ;
 I $G(XPDENV)=1 S (XPDDIQ("XPZ1"),XPDDIQ("XPZ2"))=0 D HELP^XBHELP("INTROI","ACHS3111") I '$$DIR^XBDIR("E","","","","","",1) D SORRY(2)
 ;
 Q
 ;
SORRY(X) ;
...
VCHK(ACHSPRE,ACHSVER,ACHSQUIT,ACHSCOMP) ; Check versions needed.
 ...
 ;
PRE ;EP - From KIDS.
 I $$NEWCP^XPDUTL("PRE1","AUDS^ACHS3111")
 Q
 ;
POST ;EP - From KIDS.
 ...
 ;
 I $$NEWCP^XPDUTL("POS16","TEST^ACHSPQTS")
 ;
 S %="HHS^ACHS3112"
...
MAIL ;
 ...
 KILL ^TMP("ACHS3112",$J)
 D RSLT(" --- ACHS v 3.1 Patch 12, has been installed into this uci ---")
 F %=1:1 D RSLT($P($T(GREET+%),";",3)) Q:$P($T(GREET+%+1),";",3)="###"
 S %=0
 F  S %=$O(^XTMP("XPDI",XPDA,"BLD",XPDBLD,1,%)) Q:'%   D RSLT(^(%,0))
 S XMSUB=$P($P($T(+1),";",2)," ",3,99),XMDUZ=$S($G(DUZ):DUZ,1:.5),XMTEXT="^TMP(""ACHS3111"",$J,",XMY(1)="",XMY(DUZ)=""
 ...
 Q
 ;
RSLT(%) S ^(0)=$G(^TMP("ACHS3112",$J,0))+1,^(^(0))=%
 Q
 ;
...
INTROE ; Intro text during KIDS Environment check.
 ...
 ;;(1) Routines:ACHSRP1 fix undefined medicare participating
 ;;provider, exclude E-signature test on special local
 ;;obligations printing.    
 ;;
 ;;(2) Routines: ACHSSIGA and ACHSSIGB fix queue problem.
 ;;    
 ;;(3) Routines:ACHSSIG1 and ACHSSIG2 modified to not Kill "EQ" 
 ;;index unless E-sig applied.
 ;;    
 ;;(4) Routine:ACHSUUP test on transactions types "I" and
 ;;"S".
 ;;    
 ;;(5) Routine: ACHSVDC patched to sort correctly. 
 ;;
 ;;(6) Routines:ACHSRP,ACHSRP1,ACHSAV modified for display/printing of 
 ;;DHHS number.
 ;;
 ;;(7)Dictionary ACHSCTYP added for IHS sites to select procurement
 ;;instrument type. Routine ACHSA7 modified to prompt for procurment
 ;;type.
 ;;
 ;;###
 ;
...
AUDS ;EP - From KIDS.
 ...
 S ^XTMP("ACHS3112",0)=$$FMADD^XLFDT(DT,10)_"^"_DT_"^"_$P($P($T(+1),";",2)," ",3,99)
 NEW ACHS
 S ACHS=0
 F  S ACHS=$O(^XTMP("XPDI",XPDA,"FIA",ACHS)) Q:'ACHS  D
 . I '$D(^XTMP("ACHS3112",ACHS,"DDA")) S ^XTMP("ACHS3112",ACHS,"DDA")=$G(^DD(ACHS,0,"DDA"))
 . D MES^XPDUTL(" File "_$$RJ^XLFSTR(ACHS,12)_" - "_$$LJ^XLFSTR(^XTMP("XPDI",XPDA,"FIA",ACHS),30)_"- DD audit was '"_$G(^XTMP("ACHS3112",ACHS,"DDA"))_"'"),MES^XPDUTL($$RJ^XLFSTR("Set to 'Y'",69))
 . S ^DD(ACHS,0,"DDA")="Y"
 .Q
...
AUDR ; Restore the file data audit values to their original values.
 ...
 NEW ACHS
 S ACHS=0
 F  S ACHS=$O(^XTMP("ACHS3111",ACHS)) Q:'ACHS  D
 . S ^DD(ACHS,0,"DDA")=^XTMP("ACHS3111",ACHS,"DDA")
...
HHS ;
 ...
 N ACHSI
 S ACHSI=0
 F  S ACHSI=$O(^ACHSF(ACHSI)) Q:'ACHSI   I $P(^ACHSF(ACHSI,0),U,8)'="Y" D  Q
