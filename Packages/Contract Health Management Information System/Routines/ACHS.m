ACHS ;IHS/ITSC/PMF - CHS SUB-ROUTINES ; [ 01/18/2005  1:14 PM ]
 ;;3.1;CONTRACT HEALTH MGMT SYSTEM;**2,4,5,7,12,17,18,22,28,30,32**;JUNE 11,2001;Build 39
 ;ACHS*3.1*30 5.18.22 IHS.OIT.FCJ ADD E-SIG alert
 ;ACHS*3.1*32 12.2.24 IHS.OIT.FCJ CHG LOCK TIME/RESPONSE
 ;
...
CLEAN(FROM) ;EP fr ACHSAVAR-clean err glb>90 days
 ...
 .K ^ACHSERR(FROM)
 Q
 ;IF USER-MGR-WARN THEM OF ERR MESS IN ^ACHSERR-CALLED AT THE ENTRY ACT FOR OPT
...
HRN(P,L) ;EP -Ret HRN for DFN-P DUZ(2)-L
 ;ITSC/SET/JVK ACHS*3.1*12 ADD COMMENT FOR IHS/OKCAO/POC PAWNEE BEN. PKG.
...
LOCK(V,M) ;EP - LOCK var V, mode M (+/-).
 ...
 N C,L
 S C=0,L=15     ;ACHS*3.1*32 CHANGE L FROM 10 TO 15
 G:M="-" LOCK1
...
LOCK2 ;
 ;E  W:'$D(ZTQUEUED) *7,*7,!,$S(M="-":"UN",1:""),"LOCK OF '",V,"' FAILED.",!!,"NOTIFY PROGRAMMER IMMEDIATELY." D RTRN I 0   ;ACHS*3.1*32
 E  W:'$D(ZTQUEUED) *7,*7,!,$S(M="-":"UN",1:""),"LOCK OF '",V,"' FAILED.",!!,"Data entry in Progress, try again." D RTRN I 0  ;ACHS*3.1*32
 Q $T
 ;
LOCKMSG ;
 S C=C+1
...
LOGO ;EP - Dis logo-main menu
 ...
 ;S Y=$$FMTE^XLFDT($P($G(^DIC(9.4,I,22,A,0)),U,2))   ;'DATE DISTIBUTED' ;ACHS*3.1*28 3.25.20 IHS.OIT.FCJ NO LONGER PRINTING DT
 ...
 ;W !?L,"*",$$C^XBFUNC("Version "_V_" Patch "_P1_", "_Y,N),?R,"*",!,$$C^XBFUNC($$REPEAT^XLFSTR("*",D))  ;ACHS*3.1*28 3.25.20 IHS.OIT.FCJ NO LONGER PRINTING DT
 W !?L,"*",$$C^XBFUNC("Version "_V_" Patch "_P1,N),?R,"*",!,$$C^XBFUNC($$REPEAT^XLFSTR("*",D))  ;ACHS*3.1*28 3.25.20 IHS.OIT.FCJ NO LONGER PRINTING DT
 W !!,$$C^XBFUNC($$LOC())
 Q
ESIG ;TEST FOR DEN DOCS WAITING TO BE PRINTED OR SIGNED  ACHS*3.1*30 NEW SECTION
 Q:'$G(ACHSDESG)
 N ACHSNSCT,ACHSSGCT
 S ACHSNSCT=$P(^ACHSDENR(DUZ(2),0),U,14)
 S ACHSSGCT=$P(^ACHSDENR(DUZ(2),0),U,15)
 W !,"ALERT: Denial letters ("_ACHSSGCT_") have been e-signed and are ready for print."
 W !,"ALERT: There are ("_ACHSNSCT_") outstanding denials to be signed."
 Q
 ;
