ACHSUF ; IHS/ITSC/PMF - SET CHS FACILITY VARS, CHECK DATA INTEGRITY ;   [ 10/16/2001   8:16 AM ]
 ...
 ;ACHS*3.1*29 7.1.2021 IHS.OIT.FCJ AUTO ADD NEW FY
 ;                        Fix Global structure error rather then displaying an error to user
 ;                        Add call to save 10 year data;EOBR cleanup;setup FY cans
 ;
...
S16 ;
 ...
 ;ACHS*3.1*29 IHS.OIT.FCJ;Start of changes;AUTO SETUP NEW FY,saving global,updating cans and setting up advice of allowance
 W !!,"SETTING UP NEW FISCAL YEAR.  PLEASE WAIT........",!
 ;TEST FOR THE PARAMETER.... IF NOT SET DO THE FOLLOWING...
 S XBUF=$P(^ACHSF(DUZ(2),1),U,7),Y=0
 I XBUF="" D
 .W !,$$C^XBFUNC("***ATTENTION***",80)
 .W !,"The ANNUAL GLOBAL SAVE DIRECTORY is not set.  To complete automated setup,"
 .W !,"including saving and deleting the 10 year files (ACHSF, ACHSCHEF), specify"
 .W !,"the directory in the CHS Facility File.",!
 .S Y=$$DIR^XBDIR("Y","Do you want to exit to complete the directory","Y")
 I +Y>0 S ACHSERR=1 G K
 ;
 S ACHS("YR")=ACHSACFY,ACHSAUTO="",Y=1
 D AUTO^ACHSNEW
 ;S Y=$$DIR^XBDIR("Y","SHALL I SET IT UP FOR YOU NOW","NO","","","^D HELP^ACHS(""H"",""ACHSUF"")")
 ;I $D(DUOUT)!$D(DTOUT)!('Y) S ACHSERR=1 G K
 ;CHECK FOR OVER 10 YEARS OF DATA and DIRECTORY TO SAVE GLOBAL....
 I $D(ACHSFYWK(DUZ(2),ACHSCFY-10)),XBUF'="" S ACHSNEW=1,ACHSFY=ACHSCFY-10 D NEWFY^ACHSYFYD
 D EOBR^ACHSNEW
 D CANS^ACHSNEW
 I $D(ACHSERR),ACHSERR=1 G ERR1
 K ACHSFY,ACHSFYWK D ^ACHSVAR
 D C0SUB
 W !!,"We will set up the Advice of Allowance...."
 S ACHSNEW=1 D ^ACHSALUP
 U IO(0)
 W *7,*7,!!,"A NEW FISCAL YEAR "_ACHSCFY_" HAS BEEN SET UP.",!
...
C3 ;
 ...
 ;ACHS*3.1*29 IHS.OIT.FCJ FIX GLOBAL STRUCTURE ERROR
 ;I ACHSTCNT'=$P(^ACHS(9,DUZ(2),"FY",ACHSRX,"W",0),U,4) S ACHSERR=1
 ;I ACHSSRXX'=$P(^ACHS(9,DUZ(2),"FY",ACHSRX,"W",0),U,3) S ACHSERR=1
 S:ACHSTCNT'=$P(^ACHS(9,DUZ(2),"FY",ACHSRX,"W",0),U,4) $P(^(0),U,4)=ACHSTCNT
 S:ACHSSRXX'=$P(^ACHS(9,DUZ(2),"FY",ACHSRX,"W",0),U,3) $P(^(0),U,3)=ACHSSRXX
 G C1
 ;
CEND ;
