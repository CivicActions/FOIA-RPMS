ACHSEOB ;IHS/ITSC/PMF - PROCESS EOBRS (1/6) - READ IN, PROCESS ; 22 Feb 2016  11:50 AM
 ;;3.1;CONTRACT HEALTH MGMT SYSTEM;**5,6,21,22,23,28,32**;JUNE 11, 2001;Build 39
 ...
 ;ACHS*3.1*28 11.18.19 IHS.OIT.FCJ ONLY PRINT SUMMARY DURING FAC EOBR PROC
 ;ACHS*3.1*32 11.18.24 IHS.OIT.FCJ TEST FOR ACTIVE JOBS
...
S1 ;
 ...
 I ACHSISAO D REPORT^ACHSEOB0 G ENDX:$G(ACHSQUIT)!$D(DUOUT)!$D(DTOUT)!$D(DIRUT),S2  ;ACHS*3.1*28 ADDED ACHSQUIT
 ;
 S %ZIS="OPQ",%ZIS("A")="SELECT PRINTER FOR PROCESSING REPORT: "  ;ACHS*3.1*28 RMVD EOBR COMMENT
 D ^%ZIS
...
SUF ;
 ...
 .N Y S ACHSREP=$$DIR^XBDIR("Y","Do you wish to reprocess the file","N")
...
RDHDR ;EP
 ...
 ;
XIT ;
 ;S ACHSRPT=2  ;ACHS*3.1*28 NO LONGER PRINT THE ERROR EOBR'S
 S ACHSRPT=1   ;ACHS*3.1*28 NO LONGER PRINT THE ERROR EOBR'S
 ...
 ;
 ;DO ERROR REPORT;WILL ONLY PRINT SUMMARY BECAUSE PATCH 28 CHANGED REPORT TO SUMMARY ONLY ABOVE
...
ENDX ;EP
 ...
 W !!
 D RTRN^ACHS
 I ACHSISAO,'$D(ACHSAEND),$G(ACHSISAC),'$G(ACHSQUIT) D ^ACHSEOB8 ;ACHS*3.1*21;ACHS*3.1*28 ADDED ACHSQUIT
 Q
 ;
ABEND ;EP
 G ENDX
