ACHSEOB8 ; IHS/ITSC/TPF/PMF - AREA WRITE EOBR FILES FOR FACILITIES (1/2) ;  
 ;;3.1;CONTRACT HEALTH MGMT SYSTEM;**18,19,21,22,25,28**;JUN 11, 2001;Build 86
 ;ACHS*3.1*18 PATCH FOR FT DEF
 ;ACHS*3.1*19 2 NEW LINES FOR S UTE
 ;ACHS*3.1*21 CHANGED TO USE PARM INSTEAD OF RPMS SITE FOR FILE SAVES
 ;ACHS*3.1*22 CHANGES FOR THE FI ICD9 FILE
 ;ACHS*3.1*25 ADDED TO CHECK FOR ZISH PARAMETER
 ;ACHS*3.1*28 ZISH CHANGE TO USE "ACHS"_AUTTLOC
 ;
...
SEND ;
 ...
 ;ACHS*3.1*21 TST TUC LEGACY SYS
 ...
 ;ACHS*3.1*25 ADDED TO CHECK FOR ZISH PARAMETER;ACHS*3.1*28 REPLACED ACHSSYS FOR "ACHS"_AUTTLOC
 S ACHSZPAR=""
 I $D(^%ZIB(9888888.93,"B","ACHS"_$P(^AUTTLOC(ACHSPFAC,0),U,10))) D
 .S X="",X=$O(^%ZIB(9888888.93,"B","ACHS"_$P(^AUTTLOC(ACHSPFAC,0),U,10),X))
