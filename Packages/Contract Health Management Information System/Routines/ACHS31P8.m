ACHS31P8 ;IHS/SET/JVK - ACHS 3.1 PATCH 7 ;  [ 02/18/2004  12:00 PM ]
 ;;3.1;CONTRACT HEALTH MGMT SYSTEM;**8**;JUNE 11, 2001
 ; IHS/SET/JVK ACHS*3.1*8 2/23/2004;ORIGINAL MOD FOR P8
 ;
 I '$G(IOM) D HOME^%ZIS
 ...
 ;
 D HELP^XBHELP("INTROE","ACHS31P8")
 I '$$DIR^XBDIR("E","","","","","",1) D SORRY(2) Q
 ;
 I $G(XPDENV)=1 S (XPDDIQ("XPZ1"),XPDDIQ("XPZ2"))=0 D HELP^XBHELP("INTROI","ACHS31P8") I '$$DIR^XBDIR("E","","","","","",1) D SORRY(2)
 ;
 Q
 ;
SORRY(X) ;
...
VCHK(ACHSPRE,ACHSVER,ACHSQUIT,ACHSCOMP) ; Check versions needed.
 ...
 ;
PRE ;EP - From KIDS.
 I $$NEWCP^XPDUTL("PRE1","AUDS^ACHS31P8")
 Q
 ;
POST ;EP - From KIDS.
...
MAIL ;
 ...
 KILL ^TMP("ACHS31P8",$J)
 D RSLT(" --- ACHS v 3.1 Patch 8, has been installed into this uci ---")
 F %=1:1 D RSLT($P($T(GREET+%),";",3)) Q:$P($T(GREET+%+1),";",3)="###"
 S %=0
 F  S %=$O(^XTMP("XPDI",XPDA,"BLD",XPDBLD,1,%)) Q:'%   D RSLT(^(%,0))
 S XMSUB=$P($P($T(+1),";",2)," ",3,99),XMDUZ=$S($G(DUZ):DUZ,1:.5),XMTEXT="^TMP(""ACHS31P8"",$J,",XMY(1)="",XMY(DUZ)=""
 ...
 Q
 ;
RSLT(%) S ^(0)=$G(^TMP("ACHS31P8",$J,0))+1,^(^(0))=%
 Q
 ;
...
INTROE ; Intro text during KIDS Environment check.
 ...
 ;;(1) Routine ACHSSIG1,ACHSSIG2 modified for special local list
 ;;(2) Routine ACHSAC modified to cancel esig queued docs.
 ;;(3) Routine ACHSUUP modified to store initial docs in EQ only
 ;;(4) Fixed forms to display correctly.
 ;;(5) Routine ACHSMD0 removed Q at A10+5 
 ;;###
 ;
...
AUDS ;EP - From KIDS.
 ...
 S ^XTMP("ACHS31P8",0)=$$FMADD^XLFDT(DT,10)_"^"_DT_"^"_$P($P($T(+1),";",2)," ",3,99)
 NEW ACHS
 S ACHS=0
 F  S ACHS=$O(^XTMP("XPDI",XPDA,"FIA",ACHS)) Q:'ACHS  D
 . I '$D(^XTMP("ACHS31P8",ACHS,"DDA")) S ^XTMP("ACHS31P8",ACHS,"DDA")=$G(^DD(ACHS,0,"DDA"))
 . D MES^XPDUTL(" File "_$$RJ^XLFSTR(ACHS,12)_" - "_$$LJ^XLFSTR(^XTMP("XPDI",XPDA,"FIA",ACHS),30)_"- DD audit was '"_$G(^XTMP("ACHS31P8",ACHS,"DDA"))_"'"),MES^XPDUTL($$RJ^XLFSTR("Set to 'Y'",69))
 . S ^DD(ACHS,0,"DDA")="Y"
 .Q
...
AUDR ; Restore the file data audit values to their original values.
 ...
 NEW ACHS
 S ACHS=0
 F  S ACHS=$O(^XTMP("ACHS31P8",ACHS)) Q:'ACHS  D
 . S ^DD(ACHS,0,"DDA")=^XTMP("ACHS31P8",ACHS,"DDA")
