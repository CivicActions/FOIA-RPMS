ACHSDNL ; IHS/ITSC/PMF - DENIAL LTR/FS (OPTS) (1/6) ;   [ 10/31/2003  11:44 AM ]
 ;;3.1;CONTRACT HEALTH MGMT SYSTEM;**4,6,24,30**;JUNE 11, 2001;Build 39
...
SEL ;
 D QSEL
 ;S Y=$$DIR^ACHS("N^1:2:0","Select",1,"","^D QSEL^ACHSDNL",2)  ;ACHS*3.1*30
 I 'ACHSDESG S Y=$$DIR^ACHS("N^1:2:0","Select",1,"","^D QSEL^ACHSDNL",2)
 I ACHSDESG S Y=$$DIR^ACHS("N^1:4:0","Select",3,"","^D QSEL^ACHSDNL",2)
 ;
 S ACHDLTYP=Y    ;ACHS*3.1*30
 G PAT:Y=1             ;PATIENT INPUT AND LOOKUP
 ;G BDT:+Y=2           ;DATE INPUT
 I ACHDLTYP=2 G BDT     ;DATE INPUT ;ACHS*3.1*30
 I ACHDLTYP=3 G ESIG   ;E-SIG QUE  ;ACHS*3.1*30 
 I ACHDLTYP=4 D ESIGR G:$G(ACHSQ) END G:ACHDRPRT=2 PAT G BDT
 D END
 Q
 ;
PAT ; --- Select Denial
 ...
 I $$DN^ACHS(0,8)="Y",ACHDLTYP=4,$$DN^ACHS(0,11)'?1N.N W !!!,*7,*7,?15,"Document has NOT been Electronically signed",!! S ACHDLTYP=1 G P4
 I ACHDLTYP=4,$$DN^ACHS(0,11)'?1N.N W !!!,*7,*7,?15,"Document has NOT been Electronically signed",!! H 2 G END  ;ACHS*3.1*30
P4 ;
 ;
...
EDT ; --- Input end date
 ...
 I ACHDLTYP=4 D ESIGRC G:ACHSDLCT=0 END  ;ACHS*3.1*30
 G CPY   ;E-SIG LTR REPRINT  ;ACHS*3.1*30
 Q  ;ACHS*3.1*30
 ;
ESIG ;PRINT SIGNED DENIALS  ;ACHS*3.1*30 NEW SECTION
 S ACHSSGCT=$P(^ACHSDENR(DUZ(2),0),U,15)
 I ACHSSGCT<1 W !!,"There are not any Denial Letters signed and ready for printing.",! D RTRN^ACHS D END Q
 E  G CPY
 Q
 ;
ESIGR ;REPRINT SIGNED DENIALS  ;ACHS*3.1*30 NEW SECTION
 S ACHSQ=""
 I '$D(^ACHSDEN("AP",DUZ(2),"P")) W !!,"There are not any Denial Letters signed and ready for Reprinting.",! S ACHSQ=1 D RTRN^ACHS Q
 D QSEL2 S ACHDRPRT=$$DIR^ACHS("N^1:2:0","Select",1,"","^D QSEL2^ACHSDNL",2)
 S:$D(DUOUT)!$D(DTOUT) ACHSQ=1
 Q
ESIGRC ;TEST FOR REPRINT COUNT ;ACHS*3.1*30 NEW SECTION
 S ACHSBX=ACHDBDT-1,ACHSDLCT=0
 F  S ACHSBX=$O(^ACHSDEN(DUZ(2),"D","AISSUE",ACHSBX)) Q:(ACHSBX="")!(ACHSBX>ACHDEDT)  D
 .S ACHSA=0 F  S ACHSA=$O(^ACHSDEN(DUZ(2),"D","AISSUE",ACHSBX,ACHSA)) Q:ACHSA'?1N.N  D
 ..Q:$$DN^ACHS(0,8)="Y"
 ..Q:$E($$DN^ACHS(0,1))="#"
 ..Q:$$DN^ACHS(0,11)'?1N.N    ;TEST FOR SIGNED DOC
 ..Q:$$DN^ACHS(0,13)'?1N.N   ;TEST FOR PRINTED DOC
 ..S ACHSDLCT=ACHSDLCT+1
 I ACHSDLCT=0 W !!,"There are not any Denial Letters to reprint for E-Sig Date Range.",! D RTRN^ACHS
 Q
 ;
...
END ;EP
 ...
 K DTOUT,DUOUT,DIW,DIWL,DIWR,DIWT,ZTSK,ACHDRPRT,ACHSQ,ACHSBX
 K ACHDALT,ACHDNAMP,ACHDONE,ACHDPRE,ACHSCNT,ACHSDBCN,ACHSDBCP,ACHSIII,ACHSNFAC,ACHSQUIT,ACHSST,ACHSVPT,ACHDLTYP,ACHSDLCT ;ACHS*3.1*24&30
 Q
 ;
Q1 ;EP - From DIR.
 ...
 ;
QSEL ;EP - From DIR.
 ;ACHS*3.1*30 MOD FOR ESIG TST
 I 'ACHSDESG W !!?20,"1) Print individual ltrs & fact sheet",!!?20,"2) Print range by Issue Date" Q
 I ACHSDESG D
 .W !!?20,"1) Print individual ltrs & fact sheet (no E-Sig)",!!?20,"2) Print range by Issue Date (no E-Sig)"
 .W !!?20,"3) Print with E-Signature",!!?20,"4) Reprint with E-Signature"
 Q
 ;
QSEL2 ;EP - From DIR.
 ;ACHS*3.1*30 NEW SECTION FOR ESIG TST
 W !!?20,"1) Issued Date Range",!!?20,"2) Individual" Q
 Q
 ;
NAMERR ;
