ACHSDST ; IHS/ADC/GTH - DOCUMENT STATUS REPORT ; [ 05/25/1999  11:36 AM ]
 ;;3.0;CONTRACT HEALTH MGMT SYSTEM;**9**;SEP 17, 1997
 ;
 ;;ACHS*3*9;CS;MODIFIED FOR CONSISTANT DISPLAY OF FROM/TO DATES
 ;
...
DEV ;
 ...
 ;S ZTRTN="PRINT^ACHSDST",ZTIO="",ZTDESC=$P($T(TITLE),";",3)_", Type "_ACHSRPT_", "_$$FMTE^XLFDT(ACHSBDT)_" to "_$E(ACHSEDT,2,7),ACHSQIO=ION_";"_IOST_";"_IOM_";"_IOSL
 S ZTRTN="PRINT^ACHSDST",ZTIO="",ZTDESC=$P($T(TITLE),";",3)_", Type "_ACHSRPT_", "_$$FMTE^XLFDT(ACHSBDT)_" to "_$$FMTE^XLFDT(ACHSEDT),ACHSQIO=ION_";"_IOST_";"_IOM_";"_IOSL ;ACHS*3*9
 F ACHS="ACHSQIO","ACHSBDT","ACHSEDT","ACHSRPT" S ZTSAVE(ACHS)=""
 D ^%ZTLOAD
 G:'$D(ZTSK) DEV
K ;
 KILL ACHS,ACHSIO,ACHSQIO,ACHSBDT,ACHSEDT,ACHSRPT,ZTIO,ZTSK
 D ^%ZISC
 Q
 ;
PRINT ;EP - From TaskMan.
 ;
 D FC^ACHSUF
 I $D(ACHSERR),ACHSERR=1 KILL ZTSK G KILL
...
KILL ; Do ERPT, kill vars, quit.
 I $D(ZTQUEUED) KILL ACHSFC
 D ERPT^ACHS
 KILL ACHSDOC,ACHSDOC1,ACHSDOC2,ACHSBLNK,ACHSCNX,ACHSDOS,ACHSTYPE,ACHSVNDR,ACHSEIN,ACHSOPEN,ACHSNAME,ACHSSTS,ACHSTOS,ACHSTOTP,ACHSVPTR
 KILL DA,DFN,X2,X3
 Q
 ;
