ACHSUSC ; IHS/ITSC/PMF - DISPLAY DOCUMENT CANCEL/SUPPLEMENTAL INFO ;   [ 10/31/2003  12:13 PM ]
 ;;3.1;CONTRACT HEALTH MGMT SYSTEM;**6,17,28,29**;JUNE 11, 2001;Build 86
 ;3.1*6 6/13/03 IHS/SET/FCJ 3RD PARTY NOT PRINTING
 ;ACHS*3.1*28 03.05.2020 IHS.OIT.FCJ DISPLAY CANCELLATION COMMENTS
 ;ACHS*3.1*29 07.01.2021 IHS.OIT.FCJ ADDED TEST TO DISPLAY DATA FOR THE EXPORT DISPLAY DOCUMENT OPTION
 ;
...
A0 ;
 ...
 ;D ^ACHSAV                           ;INIT VARS, OPTS  ;ACHS*3.1*29 IHS.OIT.FCJ
 I $G(ACHSTXR) D ^ACHSADEX                              ;ACHS*3.1*29 IHS.OIT.FCJ
 E  D ^ACHSAV                                           ;ACHS*3.1*29 IHS.OIT.FCJ
 ;
A0A ;EP.
...
BC6 ;
 ...
 ;ACHS*3.1*28 NEW SECTION FOR DISPLAYING CANCELLATION COMMENT
 Q:'$D(^ACHSF(DUZ(2),"D",ACHSDIEN,"A"))
 I $$EXTSET^XBFUNC(9002080.01,63,$$DOC^ACHS(2,8))="OTHER" D
 .S R1=$P(^ACHSF(DUZ(2),"D",ACHSDIEN,"T",R,0),U)
 .F  S R1=$O(^ACHSF(DUZ(2),"D",ACHSDIEN,"A","B",R1)) Q:R1=""  S R2=R1
 .S R1=$O(^ACHSF(DUZ(2),"D",ACHSDIEN,"A","B",R2,0))
 .W:$D(^ACHSF(DUZ(2),"D",ACHSDIEN,"A",R1,2)) !,"CANCELLATION COMMENT: ",($P(^(2),U))
 G ENDA
 ;
C1 ;
 Q:$D(ACHSVIEW)
