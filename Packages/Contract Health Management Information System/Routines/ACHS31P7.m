ACHS31P7 ;IHS/SET/JVK - ACHS 3.1 PATCH 7 ;  [ 12/12/2003  8:54 AM ]
 ...
 ; IHS/SET/JVK ACHS*3.1*7 8.22.03 ;ORIGINAL MOD FOR P7
 ;
 I '$G(IOM) D HOME^%ZIS
 ;
 ...
 ;
 D HELP^XBHELP("INTROE","ACHS31P7")
 I '$$DIR^XBDIR("E","","","","","",1) D SORRY(2) Q
 ;
 I $G(XPDENV)=1 S (XPDDIQ("XPZ1"),XPDDIQ("XPZ2"))=0 D HELP^XBHELP("INTROI","ACHS31P7") I '$$DIR^XBDIR("E","","","","","",1) D SORRY(2)
 ;
 Q
 ;
SORRY(X) ;
...
VCHK(ACHSPRE,ACHSVER,ACHSQUIT,ACHSCOMP) ; Check versions needed.
 ...
 ;
PRE ;EP - From KIDS.
 I $$NEWCP^XPDUTL("PRE1","AUDS^ACHS31P7")
 Q
 ;
POST ;EP - From KIDS.
 ; ---Patches 3,4,5 & 6 Checks installs are done in Install Questions.
...
MAIL ;
 ...
 KILL ^TMP("ACHS31P7",$J)
 D RSLT(" --- ACHS v 3.1 Patch 7, has been installed into this uci ---")
 F %=1:1 D RSLT($P($T(GREET+%),";",3)) Q:$P($T(GREET+%+1),";",3)="###"
 S %=0
 F  S %=$O(^XTMP("XPDI",XPDA,"BLD",XPDBLD,1,%)) Q:'%   D RSLT(^(%,0))
 S XMSUB=$P($P($T(+1),";",2)," ",3,99),XMDUZ=$S($G(DUZ):DUZ,1:.5),XMTEXT="^TMP(""ACHS31P7"",$J,",XMY(1)="",XMY(DUZ)=""
 ...
 Q
 ;
RSLT(%) S ^(0)=$G(^TMP("ACHS31P7",$J,0))+1,^(^(0))=%
 Q
 ;
...
INTROE ; Intro text during KIDS Environment check.
 ...
 ;;(1) Test for OpenM-Cache set into routine ACHS
 ;;(2) Fixed ACHSTCK1 to allow open of device on Cache Systems
 ;;(3) Fixed Spelling in entry of ACHSDENA global
 ;;(4) Set screen on DD in CHS Facility file subfield document, variable
 ;;    pointer CPT/REV/ADA. Where CPT may not be used on dental
 ;;    purchase orders. 
 ;;(5) Fixed ACHSARCH so Read will work correctly for Cache.
 ;;(6) Fix UNDEF of DIWL in ACHSDNL5 
 ;;(7) Add Electronic Signature Capability for 843 Purchase Order.
 ;;(8) Add Report for documents pending electronic signature.
 ;;(9) Add Report for approved documents with electronic signature.
 ;;(10) Remove dictionaries that were left behind from V1.45.
 ;;(11) Add four fields (Authorizing and Ordering Officals E-SIG,E-SIG
 ;;     Date) to CHS Facility Document Sub-file.
 ;;(12) Add the CHS E-Sig Authority file which includes a parameters.
 ;;(13) Fix call to Fileman Multiple in ACHSPAM. Result of version 22.0 
 ;;     changes.
 ;;(14) Add document count of paid PO's to Service Class Code report.
 ;;(15) Add export of Service Class Code to NPIRS exports.
 ;;(16) Fix ACHSDNS routine for additional report printing.
 ;;###
 ;
...
AUDS ;EP - From KIDS.
 ...
 S ^XTMP("ACHS31P7",0)=$$FMADD^XLFDT(DT,10)_"^"_DT_"^"_$P($P($T(+1),";",2)," ",3,99)
 NEW ACHS
 S ACHS=0
 F  S ACHS=$O(^XTMP("XPDI",XPDA,"FIA",ACHS)) Q:'ACHS  D
 . I '$D(^XTMP("ACHS31P7",ACHS,"DDA")) S ^XTMP("ACHS31P7",ACHS,"DDA")=$G(^DD(ACHS,0,"DDA"))
 . D MES^XPDUTL(" File "_$$RJ^XLFSTR(ACHS,12)_" - "_$$LJ^XLFSTR(^XTMP("XPDI",XPDA,"FIA",ACHS),30)_"- DD audit was '"_$G(^XTMP("ACHS31P7",ACHS,"DDA"))_"'"),MES^XPDUTL($$RJ^XLFSTR("Set to 'Y'",69))
 . S ^DD(ACHS,0,"DDA")="Y"
 .Q
...
AUDR ; Restore the file data audit values to their original values.
 ...
 NEW ACHS
 S ACHS=0
 F  S ACHS=$O(^XTMP("ACHS31P7",ACHS)) Q:'ACHS  D
 . S ^DD(ACHS,0,"DDA")=^XTMP("ACHS31P7",ACHS,"DDA")
