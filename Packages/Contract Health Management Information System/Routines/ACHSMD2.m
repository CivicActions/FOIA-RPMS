ACHSMD2 ; IHS/ADC/GTH - PRINT DENIAL LISTING BY PROVIDER BY DATE OF SERVICE ; [ 09/17/97   9:12 AM ]
 ;;3.0;CONTRACT HEALTH MGMT SYSTEM;**9**;SEP 17, 1997
 ;;ACHS*3*9; ECHO BACK DATE INPUT
 ;
 I DT>2970131 G A0
...
B1 ; 
 ...
 G B0:ACHSZRR=""
 S X=0
 S:$D(^ACHSDEN(ACHSZRR,100)) X=$P(^ACHSDEN(ACHSZRR,100),U,2)
 I X=0 G B1
 I X'=ACHSVPTR G B1
 S ACHSX=^ACHSDEN(ACHSZRR,0)
 I $E(ACHSX)="#" G B1
 G B3
 ;
B2 ;
...
B3 ;
 ...
 I +DFN=0 G B5
 W $J($$HRN^ACHS(DFN,ACHSFAC),6),?7,$E($P(^DPT(DFN,0),U,1),1,29)
 S X=$P(^DPT(DFN,0),U,3)
 W ?38,$E(X,4,5),"-",$E(X,6,7),"-",$E(X,2,3),?49,$P(^DPT(DFN,0),U,2),?53,$P(^AUTTTRI($P(^AUPNPAT(DFN,11),U,8),0),U,2)
 G B10
 ;
B5 ;
...
B11 ;
 ...
 S Y="",X=0,ACHS=0
 F ACHSF=1:1 S X=$O(^ACHSDEN(ACHSZRR,700,X)) Q:+X=0  S Y=^ACHSDEN(ACHSZRR,700,X,0) I Y?7N.1" ".2N S ACHS=$L(Y) Q
 I ACHS=0 G B25
 S X=$P(Y," ",1)
 W ?121,$E(X,1,7)
 S X=$P(Y," ",2)
