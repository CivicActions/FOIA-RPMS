ACHSTX2 ; IHS/ITSC/PMF - EXPORT DATA (3/9) - RECORD 2(DHR), SET GLOBALS FOR OTHER RECORD TYPES ;JUL 10, 2008
 ;;3.1;CONTRACT HEALTH MGMT SYSTEM;**7,13,14,15,16,22,28,29**;JUN 11,2001;Build 86
 ...
 ;ACHS*3.1*28 3.5.2020 IHS/OIT/FCJ Multiple changes for direct export and FI/UFMS only changes
 ;ACHS*3.1*29 4.7.2021 IHS/OIT/FCJ Modified if re-export the next export was picking up from the wrong beginning date
 ;
 ...
 K ACHSCHSS
 D KILLGLBS^ACHSTX
 ;TRIBAL SITE CLOSED REG; ACHS*3.1*28
 I $$PARM^ACHS(0,8)="Y" D  I ACHSDCR1=0 D ERR Q
 .S ACHSDCR=0,ACHSDCR1=0 F  S ACHSDCR=$O(^ACHS(9,DUZ(2),"FY",ACHSCFY,"W",ACHSDCR)) Q:ACHSDCR=""  S ACHSDCR1=ACHSDCR
 .I $P(^ACHS(9,DUZ(2),"FY",ACHSCFY,"W",ACHSDCR1,0),U,2)'?1N.N S ACHSDCR1=0
 S (J,ACHSDCR,ACHSEDT,ACHSBDT,P)=0,ACHSRR=""
 ...
 ;NEED TO TEST FOR STANDARD EXPORT.... ACHS*3.1*28
 ;F I=1:1 S J=$O(^ACHSTXST(DUZ(2),1,J)) Q:+J<1  S P=J
 ;S ACHSBDT=$P(^ACHSTXST(DUZ(2),1,P,0),U,3),N=9999998-DT,DA=N
 ;NEED TO CHECK FOR RE-EXPORT....TYPE OF NOT A RE-EXPORT FOR TRIBAL RUNNING FI EXPORTS*********
 ;F I=1:1 S J=$O(^ACHSTXST(DUZ(2),1,J)) Q:+J<1  I $P(^ACHSTXST(DUZ(2),1,J,0),U,20)="S" S P=J   ;ACHS*3.1*29 IHS/OIT/FCJ
 F I=1:1 S J=$O(^ACHSTXST(DUZ(2),1,J)) Q:+J<1  I $P(^ACHSTXST(DUZ(2),1,J,0),U,20)="S",$P(^(0),U,21)="N" S P=J  ;ACHS*3.1*29 IHS/OIT/FCJ ADDED TEST FOR NOT A REXPORT
 I +P<1 S ACHSBDT=$E(DT,1,3)_$P(^ACHSF(DUZ(2),0),U,6),N=9999998-DT,DA=N-1
 S ACHSBDT=$P(^ACHSTXST(DUZ(2),1,P,0),U,3),N=9999998-DT,DA=N  ;TRIBAL ST DATE IS BY CLOSED DCR, WILL NOT NEED THE TIME
 S DA=DA-1
S1 ;
...
S2 ;EP - For export Re-Generation.
 ...
 S:ACHSREEX ACHSFDT=ACHSFDT+1 ;ACHS*3.1*28 ADDED ACHSREEX TEST
...
S3 ;
 ...
 ;S:ACHSRCT=0 ACHSFDT=ACHSBDT ;ACHS*3.1*28 DO NOT RESET START DT VAR
 S ACHSTY=""
S4 ;
...
S8 ;
 ...
 G S6   ;ACHS*3.1*28 NO LONGER CREATING TYPE 2 RECORD FOR CORE
 ...
 ;I ACHSRCT=1 S ACHSFDT=ACHSBDT W !!,"NUMBER OF RECORDS PROCESSED = ",!! ;ACHS*3.1*28 DO NOT RESET START DT VAR
 I ACHSRCT=1 W !!,"NUMBER OF RECORDS PROCESSED = ",!!
