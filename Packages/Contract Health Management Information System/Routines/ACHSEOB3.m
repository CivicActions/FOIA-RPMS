ACHSEOB3 ; IHS/ITSC/PMF - PROCESS EOBRS (4/6) - UPDATE DOCUMENT(1/2) ;   [ 09/17/2004  11:42 AM ]
 ;;3.1;CONTRACT HEALTH MGMT SYSTEM;**4,11**;JUN 11, 2001;Build 39
...
P2 ;
 ...
 I $D(ACHSEOBR("I")) S ACHSPROVI=ACHSPROV   ;ACHS*3.1*32
 ;
 K ACHSBLKF
...
PROCESS ; Process the adjustment or payment.
 ...
 ;check for pay node-if there then a final pay already done and eobr posts as adjustment
 I $D(^ACHSF(DUZ(2),"D",ACHSDIEN,"PA")) D  Q:ACHSERRA>0  G PROCESS2
 ...
 .; S ACHSIPA=ACHSEOBR("I",12)+ACHSEOBR("I",13)  ;ACHS*3.1*32
 .; S ACHSIPA=$E(ACHSIPA,1,$L(ACHSIPA)-2)_"."_$E(ACHSIPA,$L(ACHSIPA)-1,$L(ACHSIPA))  ;ACHS*3.1*32
 . D IVAR  ;ACHS*3.1*32
 . D A4A^ACHSAJ
 . K ACHSIPAI        ;ACHS*3.1*32 S ACHSIPAI FOR INTEREST TYPE ADJUSTMENT
 ;
 ;ACHS*3.1*23 CHG "E" TO VAR ACHSREJ FOR REC E OR J
 ;PROCESS 0 PAY?
 ;FINAL PAY
 ;I +ACHSEOBR(ACHSREJ,8)=0,+ACHS3RDP=0,ACHSPIND="F" D  G INTEREST ;ACHS*3.1*32 CHECK TO POST INTEREST
 I +ACHSEOBR(ACHSREJ,8)=0,+ACHS3RDP=0,ACHSPIND="F" D  G V         ;ACHS*3.1*32 CHECK TO POST INTEREST
 .S X=0
 .S X1=0
 .D TRAN                     ;
 ...
 .Q
 ;
 ;SET INTEREST VAR FOR "IP" TYPE PAYMENTS  ;ACHS*3.1*32
 S ACHSIPI=ACHSEOBR("I",12)+ACHSEOBR("I",13),ACHSIPI=$E(ACHSIPI,1,$L(ACHSIPI)-2)_"."_$E(ACHSIPI,$L(ACHSIPI)-1,$L(ACHSIPI)) ;ACHS*3.1*32
 D A3^ACHSPA      ;AUTOMATIC EOBR PROCESSING
 Q:ACHSERRA>0
 I ACHSPIND="I",+ACHSIPI D AUTO^ACHSPAI K ACHSIPI G PROCESS2   ;ACHS*3.1*32
 ;
...
V ;
 ;ACHS*3.1*32 REWROTE INTEREST SECTION
 I $G(ACHSEOBR("I",12))!$G(ACHSEOBR("I",13)) D  Q:ACHSERRA>0
 . D IVAR
 . D A4A^ACHSAJ        ;AUTOMATIC ADJUSTMENT
 . D AUTO^ACHSPAI
 . K ACHSIPAI
 ...
 ;
PROCESS2 ; FINISH PROCESS ;ACHS*3.1*32 RENAMED AND MODIFIED AS INTEREST WAS NOT POSTING MOVED INTEREST CALL
...
IVAR ;INTEREST VARS
 ...
 S ACHSIPAI=1
 S ACHSPROV=ACHSPROVI
 ;ACHS3TAJ-THIRD PARTY PAY AMOUNT;ACHS3RDS-THIRD PARTY SOURCE
 S:'$G(ACHS3TAJ) ACHS3TAJ=0 S:'$G(ACHS3RDS) ACHS3RDS=""   ;These are being killed when posting other payment
 Q
 ;
