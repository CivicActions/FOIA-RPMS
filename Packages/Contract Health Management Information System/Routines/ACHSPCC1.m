ACHSPCC1 ; IHS/ITSC/TPF/PMF - CHS AREA SPLITOUT (1/5) ; JUL 10, 2008
 ;;3.1;CONTRACT HEALTH MGMT SYSTEM;**14,21,32**;JUN 11,2001;Build 39
 ...
 . D JOBABEND^ACHSPCC4
 .Q
 ;S X=$O(^AUTTTEL(DUZ(2),1,"B","PCC",""))            ;ACHS*3.1*32
 ;I +X<1 D ERR("PCC") Q                              ;ACHS*3.1*32
 ;S ACHSPRN=$P(^AUTTTEL(DUZ(2),1,X,0),U,2)   ;RJE#   ;ACHS*3.1*32
 ;I +ACHSPRN<1 D ERR("RJEPCC") Q                     ;ACHS*3.1*32
 ;I '$D(^AUTTTEL(DUZ(2),1,"B","BCS")) D ERR("BCS") Q ;ACHS*3.1*32
 ;S X=$O(^AUTTTEL(DUZ(2),1,"B","BCS",""))            ;ACHS*3.1*32
 ;I '$D(^AUTTTEL(DUZ(2),1,X,0)) D ERR("BCS") Q       ;ACHS*3.1*32
 ;S ACHSFIRN=$P(^AUTTTEL(DUZ(2),1,X,0),U,2)  ;RJE#   ;ACHS*3.1*32
 ;I +ACHSFIRN<1 D ERR("RJEBCS") Q                    ;ACHS*3.1*32
 I +ACHSPFX>0,(+ACHSPFX<300!(+ACHSPFX>399)) D CPFXERR Q   ;CAN NUMBER PREFIX
