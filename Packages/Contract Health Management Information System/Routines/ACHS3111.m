ACHS3111 ;IHS/ITSC/FCJ - ACHS 3.1 PATCH 11 ;  [ 09/20/2004  11:20 AM ]
...
MAIL ;
 ...
 Q
 ;
RSLT(%) S ^(0)=$G(^TMP("ACHS3111",$J,0))+1,^(^(0))=%
 Q
 ;
...
INTROE ; Intro text during KIDS Environment check.
 ...
 ;;(1) Routines: ACHSBMC, and ACHSDN1
 ;;    modified to allow interface of V2 of RCIS and test for 180
 ;;    days before requiring a referral.
 ;;(2) Routines: ACHSTXP and ACHSTXP2
 ;;    modified to create export from Oct 1, 2000 thru Sept 30, 2004
 ;;(3) Routines: ACHSTX7, ACHSTX7A, ACHSPCC3
 ;;    modified to export additional ICD9, Revenue and PO information 
 ;;(4) Routines modified for Medicare like rates display and entry:
 ;;    ACHSVDV,ACHSVDVA,ACHSVDV1,ACHSA3,ACHSUDF,ACHSRP2,ACHSRPU
 ;;(5) Routine modified for additional sort: ACHSVDC
 ;;(6) Routines modified for test for Esig before printing:
 ;;    ACHSRP,ACHSRP1
 ;;###
 ;
...
AUDS ;EP - From KIDS.
 ...
 S ^XTMP("ACHS3111",0)=$$FMADD^XLFDT(DT,10)_"^"_DT_"^"_$P($P($T(+1),";",2)," ",3,99)
 NEW ACHS
 S ACHS=0
 F  S ACHS=$O(^XTMP("XPDI",XPDA,"FIA",ACHS)) Q:'ACHS  D
 . I '$D(^XTMP("ACHS3111",ACHS,"DDA")) S ^XTMP("ACHS3111",ACHS,"DDA")=$G(^DD(ACHS,0,"DDA"))
 . D MES^XPDUTL(" File "_$$RJ^XLFSTR(ACHS,12)_" - "_$$LJ^XLFSTR(^XTMP("XPDI",XPDA,"FIA",ACHS),30)_"- DD audit was '"_$G(^XTMP("ACHS3111",ACHS,"DDA"))_"'"),MES^XPDUTL($$RJ^XLFSTR("Set to 'Y'",69))
 . S ^DD(ACHS,0,"DDA")="Y"
 .Q
