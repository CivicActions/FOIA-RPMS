ACHSA7 ; IHS/ITSC/PMF - ENTER DOCUMENTS (8/8)-(CONFIRM & RECORD) ;     [ 01/18/2005  11:51 AM ]
 ;;3.1;CONTRACT HEALTH MGMT SYSTEM;**1,11,12,28,32**;JUN 11, 2001;Build 39
 ...
 ;ACHS*3.1*28 IHS.OIT.FCJ FIXED SKIPPING DOC #
 ;ACHS*3.1*32 6.20.24 IHS.OIT.FCJ MED PRI CHANGE
 ;
A1 ;
 K ACHSDIEN
...
75 ;
 ;S ^ACHSF(DUZ(2),"D",ACHSDIEN,3)=ACHSFDT_U_ACHSTDT_$S((ACHSTYP=2)!$D(ACHSBLKF)!$D(ACHSSLOC):"",1:U_U_U_ACHSRPHY_U_ACHSRMPC_U_$P(ACHSRCOI,U)_U_ACHSRALR)  ;ACHS*3.1*32
 S ^ACHSF(DUZ(2),"D",ACHSDIEN,3)=ACHSFDT_U_ACHSTDT_$S((ACHSTYP=2)!$D(ACHSBLKF)!$D(ACHSSLOC):"",1:U_U_U_ACHSRPHY_U_ACHSRMP_U_$P(ACHSRCOI,U)_U_ACHSRALR)  ;ACHS*3.1*32
...
SBKILL ;
 ...
 I '$$LOCK^ACHS("^ACHS(9,DUZ(2),""FY"",ACHSACFY,""C"")","-")  ;ACHS*3.1*28 UNLOCK IF RESET DOC NUMBER
 Q
 ;
ERRCHK ;
 K ACHSOUT
...
ERRCHK1 ;
 ...
 . ;I '$$LOCK^ACHS("^ACHS(9,DUZ(2),""FY"",ACHSACFY,""C"")","-")  ;ACHS*3.1*28 DO NOT UNLOCK UNTIL EXIT
 ;
 ;
