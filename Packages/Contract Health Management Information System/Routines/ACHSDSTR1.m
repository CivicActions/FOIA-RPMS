ACHSDSTR1 ;IHS/OIT/FCJ;RTN 2 OF 2 - DOCUMENT STATUS REPORT BY FY; [ 02/09/2001  10:54 AM ]
 ;;3.1;CONTRACT HEALTH MGMT SYSTEM;**26,30**;JUN 11, 2001;Build 39
 ;ACHS*3.1*26  IHS/OIT/FCJ NEW RTN FR A 1.6 VERSION
 ;ACHS*3.1*30 4.29.22 IHS.OIT.FCJ CHANGED SSN TO DOB
 ;
ST ;
...
TYPE ;
 ...
 ;S ACHSNAME="",ACHSSSN="",A=$O(^ACHSF(DUZ(2),"TB",ACHSBDT,ACHSTYPE,ACHSDIEN,0))     ;ACHS*3.1*30 SSN TO DOB
 S ACHSNAME="",ACHSDOB="",A=$O(^ACHSF(DUZ(2),"TB",ACHSBDT,ACHSTYPE,ACHSDIEN,0))      ;ACHS*3.1*30 SSN TO DOB
 ;I +A>0,$D(^ACHSF(DUZ(2),"D",ACHSDIEN,"T",A,0)) S A("DFN")=$P(^(0),U,3) I +A("DFN")>0,$D(^DPT(A("DFN"),0)) S ACHSNAME=$P(^(0),U),ACHSSSN=$P($G(^DPT(A("DFN"),0)),U,9)  ;ACHS*3.1*30 SSN TO DOB
 I +A>0,$D(^ACHSF(DUZ(2),"D",ACHSDIEN,"T",A,0)) S A("DFN")=$P(^(0),U,3) I +A("DFN")>0,$D(^DPT(A("DFN"),0)) S ACHSNAME=$P(^(0),U),ACHSDOB=$$DOB^AUPNPAT(A("DFN"),"S")  ;ACHS*3.1*30 SSN TO DOB
...
P2 ;
 ;I ACHSRTYP=1 W !,A("DOC"),"(",ACHSCLRK,")",?25,ACHSHRN,?35,$E(ACHSSSN,6,9),?52,ACHSFDT I +TOS W ?63,$S(+TOS=1:"43",+TOS=2:"57",+TOS=3:"64",1:"") ;ACHS*3.1*30
 I ACHSRTYP=1 W !,A("DOC"),"(",ACHSCLRK,")",?25,ACHSHRN,?35,ACHSDOB,?52,ACHSFDT I +TOS W ?63,$S(+TOS=1:"43",+TOS=2:"57",+TOS=3:"64",1:"")    ;ACHS*3.1*30
 ;S:ACHSFIL ^TMP($J,"ACHSDSTR",ACHSDIEN)=^TMP($J,"ACHSDSTR",ACHSDIEN)_U_$S(+TOS=1:"43",+TOS=2:"57",+TOS=3:"64",1:"")_U_A("DOC")_"("_ACHSCLRK_")"_U_ACHSHRN_U_$E(ACHSSSN,6,9)_U_ACHSFDT_U ;ACHS*3.1*30
 S:ACHSFIL ^TMP($J,"ACHSDSTR",ACHSDIEN)=^TMP($J,"ACHSDSTR",ACHSDIEN)_U_$S(+TOS=1:"43",+TOS=2:"57",+TOS=3:"64",1:"")_U_A("DOC")_"("_ACHSCLRK_")"_U_ACHSHRN_U_ACHSDOB_U_ACHSFDT_U  ;ACHS*3.1*30
 ;I ACHSRTYP=2 W !,A("DOC"),"(",ACHSCLRK,")",?25,EIN,?52,$E(ACHSSSN,6,9) I +TOS W ?63,$S(+TOS=1:"43",+TOS=2:"57",+TOS=3:"64",1:"")   ;ACHS*3.1*30
 I ACHSRTYP=2 W !,A("DOC"),"(",ACHSCLRK,")",?25,EIN,?52,ACHSDOB I +TOS W ?63,$S(+TOS=1:"43",+TOS=2:"57",+TOS=3:"64",1:"")    ;ACHS*3.1*30
...
END ;
 D:ACHSFIL FILSAV
 D ERPT^ACHS
 K ZTSK,A,AG,AGT,ACHS,ACHSHAT,BDATE,CNX,EDATE,EIN,ACHSLOC,OPEN,ACHSNAME,ACHSHRN,ACHSTYPE,ACHSDOB,ACHSOCC,ACHSQUIT,ACHSCLRK
...
HEAD ;HEADING FOR TOTAL ONLY REPORT
 ...
 .;I ACHSRTYP=1 W !,"Document #/(Clerk)",?25,"Chart#",?35,"Last 4SSN",?52,"Auth Date",?63,"Type"  ;ACHS*3.1*30
 .I ACHSRTYP=1 W !,"Document #/(Clerk)",?25,"Chart#",?37,"DOB",?52,"Auth Date",?63,"Type"   ;ACHS*3.1*30
 .;E  W !,"Document #/(Clerk)",?25,"EIN Number",?52,"Last 4SSN",?63,"Type"  ;ACHS*3.1*30
 .E  W !,"Document #/(Clerk)",?25,"EIN Number",?54,"DOB",?63,"Type"   ;ACHS*3.1*30
 W !,ACHS("-"),!
 I ACHSFIL D
 ...
 .;S:TOTONLY=2 ^TMP($J,"ACHSDSTR",7)="Patient Name"_U_"Provider of Service"_U_"Issue date"_U_"OCC"_U_"Amount"_U_"Type"_U_"Document #/(Clerk)"_U_"Chart#"_U_"Last 4SSN"_U_"Auth Date"  ;ACHS*3.1*30
 .S:TOTONLY=2 ^TMP($J,"ACHSDSTR",7)="Patient Name"_U_"Provider of Service"_U_"Issue date"_U_"OCC"_U_"Amount"_U_"Type"_U_"Document #/(Clerk)"_U_"Chart#"_U_"DOB"_U_"Auth Date"  ;ACHS*3.1*30
 Q
