ACHSTX7 ;IHS/ITSC/PMF - EXPORT DATA (8/9) - RECORD 7(638 STATISTICAL DATA FOR NPIRS) ;JUL 10, 2008
 ;;3.1;CONTRACT HEALTH MGMT SYSTEM;**2,3,5,7,11,12,14,15,21,23,29,32**;JUN 11,2001;Build 39
 ...
 ;ACHS*3.1*29 07.01.21 IHS.OIT.FCJ TEST FOR TRIBAL SITES USING THE FI;DO NOT CREATE A STAT FILE FOR SITES USING THE FI
 ;ACHS*3.1*32 11.14.24 IHS.OIT.FCJ SETTING PO ENTRIES TO THE EXPORT FILE;CHANGED LINE LABELS-VIOLATES SAC;ADA CODE CHANGE
 ;
 ;
 ;IHS/ITSC/PMF  
 ...
 I $$PARM^ACHS(2,11)="Y",$$PARM^ACHS(0,8)="Y" G ^ACHSTX8   ;ACHS*3.1*29 IF TRIBAL AND USING FI
...
GOTSOME ;
 ...
 ;ACHS*3.1*21;ADD PARM TEST TO NEXT LINE
...
PROC ;
 ...
 ;
 S ^TMP("ACHSTX",$J,ACHSDIEN,DA)=""    ;ACHS*3.1*32
...
ICD1043 ;43-ICD10 RCORD
 ...
 Q
 ;
RE43 ;END OF 43 RECORD   ;ACHS*3.1*32 CHANGED LINE LABEL FROM 43RE
...
57 ;dental service processed here
 ...
 ;
 ;S ACHSDATA=25_$P(^AUTTLOC(DUZ(2),0),U,10)_$E(ACHSEIN,2,10)_$S(ACHSSEX=1:"M",1:"F")_ACHSDOB_ACHSSSN_$P(ACHS,U)_$P(ACHS,U,2)_$S($P(ACHSTRAN,U,10):17000000+$P(ACHSTRAN,U,10),1:$J("",8))   ;ACHS*3.1*32
 S ACHSDATA=25_$P(^AUTTLOC(DUZ(2),0),U,10)_$E(ACHSEIN,2,10)_$S(ACHSSEX=1:"M",1:"F")_ACHSDOB_ACHSSSN_$E($P(ACHS,U),1,60)_$P(ACHS,U,2)   ;ACHS*3.1*32
 S ACHSDATA=ACHSDATA_$S($P(ACHSTRAN,U,10):17000000+$P(ACHSTRAN,U,10),1:$J("",8))   ;ACHS*3.1*32
...
ICD1064 ;AFTER ICD10 IMPLEMENTATION
 ...
 Q
 ;
RE64 ;END RECORD     ;ACHS*3.1*32 CHNG LINE LABEL FROM 64RE
 S ACHS=$$CPT^ACHSTX7A(DUZ(2),ACHSDIEN)
...
ENDREC ;do the end of the record here, for all types of service
 ...
 .;I (DT<$$PARM^ACHS(0,18))!(ACHSTOS2=57) S ^ACHSDATA(ACHSRCT)="7X"_ACHSAGID_$J(ACHSDIEN,20)_$J("",42) Q  ;ACHS*3.1*32
 .I (DT<$$PARM^ACHS(0,18))!(ACHSTOS2=57) S ^ACHSDATA(ACHSRCT)="7X"_ACHSAGID_$J(ACHSDIEN,20)_$E($P(ACHS,U),61,75)_$J("",27) Q   ;ACHS*3.1*32
