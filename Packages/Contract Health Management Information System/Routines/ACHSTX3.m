ACHSTX3 ; IHS/ITSC/PMF - EXPORT DATA (4/9) - RECORD 3(PATIENT FOR AO/FI) ;
 ;;3.1;CONTRACT HEALTH MGMT SYSTEM;**13,19,28**;JUN 11,2001;Build 86
...
A5 ;
 ...
 ;ACHS*3.1*19 CHANGED NXT LINE TO SEND THE DATABASE ID INSTEAD OF THE DOC DEST;ACHS*3.1*28 ADDED ^ACHSBCBS
 ...
 S (^ACHSBCBS(ACHSRCT),^ACHSDATA(ACHSRCT))="3A"_ACHSFAC_$E(ACHSHRN+1000000,2,7)_ACHSDOB_ACHSSEX_ACHSTRIB_ACHSNAME_ACHSCOV_ACHSCOMM_ACHSSSN_ACHSUPDT_$E($$SSV(ACHSR)_" ",1)_ACHSDB
 ;
 S ACHSRCT=ACHSRCT+1,ACHSRTYP(3)=ACHSRTYP(3)+1
 ;ACHS*3.1*28 ADDED ^ACHSBCBS
 I '$D(^DPT(ACHSR,.11)) S (^ACHSBCBS(ACHSRCT),^ACHSDATA(ACHSRCT))="3B"_$J("",78) G A7
...
A6 ;
 ;ACHS*3.1*28 ADDED ^ACHSBCBS
 S (^ACHSBCBS(ACHSRCT),^ACHSDATA(ACHSRCT))="3B"_ACHSADDR_ACHSCITY_ACHSST_ACHSZIP_ACHSINSR_ACHSDEST,$P(^AUPNPAT(ACHSR,0),U,15)=DT
