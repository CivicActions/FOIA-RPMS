ACHSTX5 ; IHS/ITSC/PMF - EXPORT DATA (6/9) - RECORD 5(DOCUMENT FOR AO/FI) ;JUL 10, 2008
 ;;3.1;CONTRACT HEALTH MGMT SYSTEM;**4,6,13,14,16,19,28,32**;JUN 11,2001;Build 39
 ...
 ;ACHS*3.1*28 12.2.20 IHS.OIT.FCJ ADDED ACHSBCBS GLOBAL FOR DIRECT EXPORTS
 ;ACHS*3.1*32 11.8.24 Add test for Tribal sites to set tmp global for achstxst export file
 ;
...
C6 ;
 ...
 ;ACHS*3.1*28 ADDED ^ACHSBCBS
 S (^ACHSBCBS(ACHSRCT),^ACHSDATA(ACHSRCT))="5A"_ACHSDOCN_ACHSTOS2_ACHSFAC_ACHSBIND_ACHSEIN_ACHSHRN_ACHSCN_ACHSCAN_ACHSOBJC_$E($P(ACHSDOCR,U,2),2,7)_ACHSSTS_ACHSIPA_ACHSESDA_ACHSFED
 ;
C7 ; Build Patient name & 3rd party info for 5B record.
 ...
 D ADD5B
 ;ACHS*3.1*28 ADDED ^ACHSBCBS
 S (^ACHSBCBS(ACHSRCT),^ACHSDATA(ACHSRCT))="5B"_ACHSREFT_ACHSEDOS_ACHSFRDT_ACHSTODT_ACHSPATN_ACHS5IN1_ACHS5IN2_ACHSTRIB_ACHSCOMM
 ;
 S ACHSRCT=ACHSRCT+1,ACHSRTYP(5)=ACHSRTYP(5)+1
 ;
 ...
 ;ACHS*3.1*28 ADDED ^ACHSBCBS
 S (^ACHSBCBS(ACHSRCT),^ACHSDATA(ACHSRCT))=$E(ACHS5C,1,80)  ;  ACHS*3.1*4
 I $$PARM^ACHS(0,8)="Y",$$PARM^ACHS(2,11)="Y" S ^TMP("ACHSTX",$J,R,1)=""  ;ACHS*3.1*32 ADD FOR TRIBAL SITES EXPORTING FI RECORDS
 S ACHSBCBS("COUNT")=ACHSBCBS("COUNT")+1,ACHSBCBS("TOT")=ACHSBCBS("TOT")+ACHSIPA   ;ACHS*3.1*28
