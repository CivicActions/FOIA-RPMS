ACHSC6P ; IHS/ADC/GTH - PRINT EXPENDITURE LIST BY PATIENT/COMMUNITY ; [ 05/25/1999  11:43 AM ]
 ;;3.0;CONTRACT HEALTH MGMT SYSTEM;**9**;SEP 17, 1997
 ;
 ;;ACHS*3*9;CS;MODIFIED DISPLAY OF DOB
 ;
 S $P(^TMP("ACHSC6",$J,0),U,1,2)="^ACHSC6P"
 I $D(ACHSQIO) F  S IOP=ACHSQIO D ^%ZIS Q:'POP  H 30
 K ^TMP("ACHSC6",$J,0,"ACHSQIO")
 S ACHSLOC=$$LOC^ACHS,ACHSTY="CHS EXPENDITURE REPORT BY "_$S(ACHSRPT=2:"COMMUNITY",ACHSRPT=5:"TRIBE",1:"PATIENT")
 D CITYST
...
A1 ;
 ...
 I $D(^TMP("ACHSC6",$J,"P",ACHSNAME,43)) S X=^(43),DFN=$P(X,U),ACHSCOMN=$P(X,U,2),ACHSN43=$P(X,U,3),ACHSESDA(43)=$P(X,U,4),ACHST43=$P(X,U,5),ACHST433=$P(X,U,6)
 I $D(^TMP("ACHSC6",$J,"P",ACHSNAME,64)) S X=^(64),DFN=$P(X,U),ACHSCOMN=$P(X,U,2),ACHSN64=$P(X,U,3),ACHSESDA(64)=$P(X,U,4),ACHST64=$P(X,U,5),ACHST643=$P(X,U,6)
 I $D(^TMP("ACHSC6",$J,"P",ACHSNAME,57)) S X=^(57),DFN=$P(X,U),ACHSCOMN=$P(X,U,2),ACHSN57=$P(X,U,3),ACHST57=$P(X,U,5),ACHST573=$P(X,U,6)
 S ACHSTT3P=ACHST573+ACHST433+ACHST643,ACHSTOA=ACHST43+ACHST64+ACHST57+ACHSTT3P
 ;I ACHSRPT=1 S ACHSDOB="" I DFN S ACHSDOB=$P(^DPT(DFN,0),U,3),ACHSDOB=$E(ACHSDOB,4,5)_"-"_$E(ACHSDOB,6,7)_"-"_$E(ACHSDOB,2,3)
 I ACHSRPT=1 S ACHSDOB="" I DFN S ACHSDOB=$P(^DPT(DFN,0),U,3),ACHSDOB=$E(ACHSDOB,4,5)_"-"_$E(ACHSDOB,6,7)_"-"_($E(ACHSDOB,1,3)+1700) ;ACHS*3*9
WRITE ;
 G:ACHSRPT=4 RPT4
...
RPT4 ;
 S ACHSTN43=ACHSTN43+ACHSN43,ACHSTN64=ACHSTN64+ACHSN64,ACHSESDA("T43")=ACHSESDA("T43")+ACHSESDA(43),ACHSESDA("T64")=ACHSESDA("T64")+ACHSESDA(64)
 S ACHST43("$")=ACHST43("$")+ACHST43,ACHST64("$")=ACHST64("$")+ACHST64
 S ACHST57("$")=ACHST57("$")+ACHST57,ACHSTN57=ACHSTN57+ACHSN57,ACHSGTOT("$")=ACHSGTOT("$")+ACHSTOA,ACHST3P("$")=ACHST3P("$")+ACHSTT3P
 G A1
 ;
A2 ;
...
COMP ;EP. Compressed print for printer.
 ...
 KILL ACHS("PRINT")
 S:$D(^%ZIS(2,IOST(0),12.1)) ACHS("PRINT",16)=^(12.1) ; Code for compressed print.
 I $D(^%ZIS(2,IOST(0),5)),$P(^(5),U)]"" S ACHS("PRINT",10)=$P(^(5),U) ; Code for standard print.
 I $D(ACHS("PRINT",16)),$D(ACHS("PRINT",10)) S IOP=IO_";132" U IO W @ACHS("PRINT",16) ; Set for compressed print.
 Q
 ;
CITYST ;EP.
 S ACHSCITY=""
 I $D(^AUTTLOC(DUZ(2),0)) S ACHSCITY=$P(^AUTTLOC(DUZ(2),0),U,13)_", "_$P(^DIC(5,+$P(^AUTTLOC(DUZ(2),0),U,14),0),U) Q
 I $D(^DIC(4,DUZ(2),1)) S ACHSCITY=$P(^DIC(4,DUZ(2),1),U,3)_", "_$P(^DIC(5,+$P(^DIC(4,DUZ(2),0),U,2),0),U)
 Q
 ;
