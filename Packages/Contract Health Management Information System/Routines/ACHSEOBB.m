ACHSEOBB ; IHS/ITSC/TPF/PMF - PROCESS EOBRS (2/6) CONTINUATION ;    
 ;;3.1;CONTRACT HEALTH MGMT SYSTEM;**21,22,23,32**;JUN 11, 2001;Build 39
 ...
 ; ACHS*3.1*32 8.8.24 IHS.OIT.FCJ DISPLAY FIFO ORDER;DELETE FILE AFTER PROCESSING
M1 ;EP
 ;
...
FAC ;EP - Update CHS FACILITY with EOB Seq # and Process date.
 ...
 ;ACHS*3.1*32 ST OF CHANGE TO DELETE PROCESSED FILES
 I '$$DEL^%ZISH($$IM^ACHS,ACHSMEDA) U IO(0) W !!?10,ACHSMEDA,"  has been Processed and Deleted"
 ;check for any files already processed and left in the directory that need to be removed
 Q:'$O(ACHSDFLS(0))
 S I=0 F  S I=$O(ACHSDFLS(I)) Q:I'?1N.N  D
 .S ACHSDFLS=$P(ACHSDFLS(I)," ")
 .I '$$DEL^%ZISH($$IM^ACHS,ACHSDFLS) U IO(0) W !!?10,ACHSDFLS,"  has been removed from the directory"
...
C01 ;
 ...
 E  G ABEND^ACHSEOB
 D ^%ZISC
 ;I $E(X,1,2)'="$$" S $P(ACHSUFLS(ACHSUFLS)," ",5)="BAD" G C01  ;ACHS*3.1*32
 I $E(X,1,2)'="$$" S ACHSUFLS(ACHSUFLS)=ACHSUFLS(ACHSUFLS)_" "_"DEL",ACHSDFLS(ACHSUFLS)=ACHSUFLS(ACHSUFLS) K ACHSUFLS(ACHSUFLS) G C01   ;ACHS*3.1*32
 I $E(X,3,7)?7N S ACHSUFLS(ACHSUFLS)=ACHSUFLS(ACHSUFLS)_" "_"DEL",ACHSDFLS(ACHSUFLS)=ACHSUFLS(ACHSUFLS) K ACHSUFLS(ACHSUFLS) G C01   ;ACHS*3.1*32
 I $D(^ACHSF(DUZ(2),17,$E(X,3,9))) S ACHSUFLS(ACHSUFLS)=ACHSUFLS(ACHSUFLS)_" "_"DEL",ACHSDFLS(ACHSUFLS)=ACHSUFLS(ACHSUFLS) K ACHSUFLS(ACHSUFLS) G C01   ;ACHS*3.1*32
...
C0END ;
 S IONOFF=""
 D ^%ZISC
 Q:'$O(ACHSUFLS(0))   ;ACHS*3.1*32
 S ACHSUFLS="",CT=0 F  S ACHSUFLS=$O(ACHSUFLS(ACHSUFLS)) Q:ACHSUFLS'?1N.N  S CT=CT+1,ACHSFLS(CT)=ACHSUFLS(ACHSUFLS) K ACHSUFLS(ACHSUFLS)    ;REODER FILES ;ACHS*3.1*32  
 F I=1:1:CT S ACHSUFLS(I)=ACHSFLS(I)   ;ACHS*3.1*32
 ;D FILDEL^ACHSEOBC  ACHS*3.1*32 NO LONGER AUTO DELETING FILES, ONLY DELETE AFTER PROCESSING
C1 ;
 U IO(0)
...
C2 ;
 ;ACHS*3.1*32 REWROTE C2,C3 TO DISPLAY IN FIFO ORDER
 F  S ACHSI=$O(ACHSUFLS(ACHSI)) Q:ACHSI'?1N.N  D
 .I $P(ACHSUFLS(ACHSI)," ",2)="DEL" K ACHSUFLS(ACHSI) Q
...
C5 ;
 ...
 ;
KILL ;EP
 ;ACHS*3.1*21 REMOVED ACHSISAO;ACHS*3.1*32 ADDED ACHSDFLS,ACHSFLS,I,CT
 ...
 K ACHSSEQN,ACHSEDXT,ACHSDFLS,ACHSFLS,I,CT
 K ^ACHSUSE("EOBR")
 Q
 ;
