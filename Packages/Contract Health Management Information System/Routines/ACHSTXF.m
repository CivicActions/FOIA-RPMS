ACHSTXF ; IHS/OIT/FCJ - EXPORT DATA - RECORD 2(UFMS);
 ;;3.1;CONTRACT HEALTH MGMT SYSTEM;**13,14,17,28,29**;JUN 11,2001;Build 86
 ;ACHS*3.1*13 IHS/OIT/FCJ NEW ROUTINE
 ;ACHS*3.1*28 IHS.OIT.FCJ Multiply changes for direct export from sites and specific type of export
 ;ACHS*3.1*29 07.01.2021 IHS.OIT.FCJ Added Mod for UEI and EFT and 5 dig can; 
 ; Added ACHSUFMT variable that is set in the menu option to test the format of the ufms record
 ...
 K ACHSCHSS
 D KILLGLBS^ACHSTX
 S (J,ACHSDCR,ACHSBDT)=0,(ACHSPEX,ACHSRR)=""
 S ACHSEDT=DT
S1 ;Set beginning date AND ein of prev export
 S ACHSBDT=$P(^ACHSF(DUZ(2),0),U,14)    ;date of last UFMS export DT-TIME   
 ...
 ;ACHS*3.1*28 BEGDT FOR REC DOES NOT REQUIRE THE DT-TIME; TEST BELOW WILL BE ON TRANSACTION
 S ACHSBDT=$P(ACHSBDT,".")-1
 ;
S2 ;
...
S6 ;
 ...
 G S6:$P(ACHSDOCT,U,32)>0  ;ACHS*3.1*28 TEST TO SEE IF PREVIOUSLY EXPORTED
 ;Has TRNS been previously sent-no longer based on DCR;PRIOR TO SETTING TX DT IN TRNS REC
...
S7 ;EP; FROM RE-EXPORT SINGLE TRANSACTIONS OR BATCHES - ACHSTXF1
 ...
 D CANOBJ^ACHSTX8
 I ACHSUFMT="N" S X=$L(ACHSOBJC) I X'=5 F I=1:1:5-X  S ACHSOBJC=" "_ACHSOBJC ;ACHS*3.1*29 INCREASED FROM 4 TO 5
 ...
 S ACHSEFT=$P(^AUTTVNDR(ACHSPROV,0),U,9) S X=$L(ACHSEFT) I X'=4 F I=1:1:4-X  S ACHSEFT=" "_ACHSEFT ;ACHS*3.1*29
 S ACHSUEI=$P(^AUTTVNDR(ACHSPROV,0),U,10) S X=$L(ACHSUEI) I X'=12 F I=1:1:12-X  S ACHSUEI=" "_ACHSUEI ;ACHS*3.1*29
S8 ;
 ...
 I ACHSREEX,ACHSREXT="F" G S9 ;ACHS*3.1*28;RE-EXPORT FI ONLY G S9
 S ACHSRTYP(8)=ACHSRTYP(8)+1
 ;ACHS*3.1*29 ST OF CHANGES; REMOVED GEO LOC AND 1 DIG FY IN NX LINE
 ...
 ;ACHS*3.1*17 1.9.2010 IHS.OIT.FCJ MOD NXT LINE TO SEND BUD FY;ACHS*3.1*29 BUD FY NO LONGER USED
 ...
 ;ACHS*3.1*29 5.1.2021 IHS.OIT.FCJ MOD NXT LINE TO SEND 5 DIG OBJ CLASS AND UEI MODS
 ...
 I ACHSUFMT="N" D
 .S ACHSRECL=177
 .S ^ACHSDATA(ACHSRCT)="U2"_ACHSEFDT_ACHSCDE_$S(ACHSTOS=1:323,ACHSTOS=2:324,ACHSTOS=3:325,1:"")_"HHSI"_ACHSARCO_ACHSDOCN_$J("",5)
 .S ^ACHSDATA(ACHSRCT)=^ACHSDATA(ACHSRCT)_ACHSCAN_ACHSOBJC_ACHSIPA_" "_ACHSEIN_$J("",54)_ACHS2FY_ACHSDEST_$J("",4)_ACHSDUNS_$J("",10)_ACHSUEI_ACHSEFT
 E  S ACHSRECL=161 D
 .S ^ACHSDATA(ACHSRCT)="U2"_ACHSEFDT_ACHSCDE_$S(ACHSTOS=1:323,ACHSTOS=2:324,ACHSTOS=3:325,1:"")_"HHSI"_ACHSARCO_ACHSDOCN_ACHSCTYP_$J("",3)_"1"_X1
 .S ^ACHSDATA(ACHSRCT)=^ACHSDATA(ACHSRCT)_ACHSCAN_ACHSOBJC_ACHSIPA_ACHSFED_ACHSEIN_$J("",54)_ACHS2FY_ACHSDEST_" "_ACHSXLOC_ACHSDUNS_$J("",10)
 S ^ACHSUFMS(ACHSRCT)=$E(^ACHSDATA(ACHSRCT),2,ACHSRECL),ACHSUFMS("COUNT")=ACHSUFMS("COUNT")+1,ACHSUFMS("TOT")=ACHSUFMS("TOT")+ACHSIPA
 I $L(^ACHSDATA(ACHSRCT))'=ACHSRECL W !!,*7,*7,"A DHR RECORD WAS PRODUCED THAT WAS NOT "_ACHSRECL_" CHARACTERS IN LENGTH:",!!,^(ACHSRCT),!,*7,*7 G ERROR^ACHSTX
 ;ACHS*3.1*29 END OF CHANGES
 I ACHSRCT=1 S ACHSFDT=ACHSBDT W !!,"NUMBER OF RECORDS PROCESSED = ",!!
 I ACHSRCT#25=0 W $J(ACHSRCT,8)
S9 ;ACHS*3.1*28 ADDED LINE, IF RE-EXPORT AND NOT UFMS
 I ACHSDEST="F" D
 .I ACHSREEX,ACHSREXT="U" Q   ;ACHS*3.1*28 If re-export and Ufms type only
 ...
 S ^TMP("ACHSTX",$J,P,DA)=""
 I ACHSREEX,ACHSREXT="F" Q    ;ACHS*3.1*28
...
CVTEND1 ;
 ...
 K ACHSX,ACHSDUNS,X1,ACHSDFY,ACHSXLOC,ACHSCTYP,ACHSFED,ACHSOBJC,ACHSTOS,DA,ACHSTY,ACHSEIN,ACHSSKIP,ACHS2FY,ACHSUEI,ACHSEFT
