ACHSTX6 ; IHS/ADC/GTH - EXPORT DATA (7/9) - RECORD 6(PAY FOR AO) ; [ 09/17/97   9:12 AM ]
 ;;3.0;CONTRACT HEALTH MGMT SYSTEM;**9**;SEP 17, 1997
 ;;ACHS*3*9 IV&V DATE FIX (USE FILEMAN FORMAT INTERNALLY)
 ;
...
A1 ;
 S R=$O(^ACHSTXPD(R))
 G END:'R
 S ACHSDOCR=^ACHSF(DUZ(2),"D",R,0),ACHSRR=0
A2 ;
 S ACHSRR=$O(^ACHSTXPD(R,ACHSRR))
 G A1:'ACHSRR
 S ACHSTRAN=^ACHSF(DUZ(2),"D",R,"T",ACHSRR,0)
 I $P(ACHSTRAN,U,2)'="P" G A2
 S ACHSDOCN=$P(ACHSDOCR,U,14)_ACHSFC_$E($P(ACHSDOCR,U)+100000,2,6)
 D TOS^ACHSTX8
 S ACHSFAC=ACHSAFAC,(X,X1)=$P(ACHSDOCR,U,8),ACHSEIN=$P(^AUTTVNDR(X,11),U)_$P(^(11),U,2),ACHSEIN=$E(ACHSEIN_$J("",12),1,12),ACHSFED=$S($P(^AUTTVNDR(X1,11),U,10)=2:2,1:1)
 S ACHSPTYP="  ",X=$P(^AUTTVNDR($P(ACHSDOCR,U,8),11),U,3)
 ...
 ;S ^ACHSDATA(ACHSRCT)="6A"_ACHSDOCN_ACHSTOS2_$E($P(ACHSDOCR,U,2),2,7)_"6"_ACHSFAC_ACHSHRN_ACHSEIN_ACHSPTYP_ACHSFED_ACHSCN_ACHSCAN_ACHSOBJC_ACHSDCR_$J("",10) ;ACHS*3*9 IV&V DATE FIX
 S ^ACHSDATA(ACHSRCT)="6A"_ACHSDOCN_ACHSTOS2_$P(ACHSDOCR,U,2)_"6"_ACHSFAC_ACHSHRN_ACHSEIN_ACHSPTYP_ACHSFED_ACHSCN_ACHSCAN_ACHSOBJC_ACHSDCR_$J("",10) ;ACHS*3*9 IV&V DATE FIX
...
A5 ;
 ...
 ;
END ;
 KILL ACHSCN,ACHSDCR,ACHSDOCN,ACHSDOS,ACHSFNAM,ACHSFULP,ACHSHRN,ACHSIPA,ACHSLNAM,ACHSOAMT,ACHSSCC,ACHSTRAN,ACHSWKLD,ACHSTOS,ACHSPTYP
 S ACHSROUT=ACHSRCT
 G ^ACHSTX7
 ;
