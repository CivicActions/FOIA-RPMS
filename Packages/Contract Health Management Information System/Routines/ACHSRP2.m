ACHSRP2 ; IHS/ITSC/PMF - PRINT CHS FORMS - INIT NAMED VARS, CALL FORM ROUTINE ;  [ 01/21/2005  3:50 PM ]
 ;;3.1;CONTRACT HEALTH MGMT SYSTEM;**3,5,7,11,16,28**;JUNE 11,2001;Build 86
 ...
 ;ACHS*3.1*16 10/26/2009 IHS.OIT.FCJ MULTIPLE COPIES FOR ZUNI
 ;ACHS*3.1*28 3/06/2020 IHS.OIT.FCJ ADDED REF#
 ;
 ...
 ;D INIT,REF:(($$PARM^ACHS(2,16)="Y")&(ACHSTYP'=2))  ;ACHS*3.1*28 DO NOT NEED THE TESTS
 D INIT,REF ;ACHS*3.1*28
 D SB1^ACHSRP1
 ;
...
REF ; Set Referral Physician and Medical Priority into print vars.
 ...
 ;
 ;
PROC ; Set Referral Procedure Narrative into print vars for OLD Forms.
...
DIAG ; Set Referral Diagnosis Narrative into print vars for regular Form.
 G:'$D(^ACHSF(DUZ(2),"D",ACHSDIEN,5)) REFN   ;ACHS*3.1*28 CHANGE EXT TO REFN
...
REFN ;REFERRAL NUMBER;ACHS*3.1*28
 ...
 I +ACHSREF S R("N",1)="a. "_$P(^BMCREF(ACHSREF,0),U,2)_$P($G(^BMCREF(ACHSREF,1)),U)
EXT ;
 K ACHSDX,ACHSPX,X,N
 Q
 ;
...
DIAG1 ; Set Referral Diagnosis Narrative into print vars for Universal Form.
 G:'$D(^ACHSF(DUZ(2),"D",ACHSDIEN,5)) REFN1 S ACHSDX=$G(^ACHSF(DUZ(2),"D",ACHSDIEN,5)) ;ACHS*3.1*28 CHG EXT1 TO REFN1
 I $L(ACHSDX)>72 S R("D",1)=$E(ACHSDX,1,22),N=23
 I $L(ACHSDX)<72 S R("D",1)="",N=1
 F X=2:1:3 S R("D",X)=$E(ACHSDX,N,N+36),N=N+37
REFN1 ;REFERRAL NUMBER;ACHS*3.1*28
 S ACHSREF=$P($G(^ACHSF(DUZ(2),"D",ACHSDIEN,2)),U,7)
 I +ACHSREF S R("N",1)="a. "_$P(^BMCREF(ACHSREF,0),U,2)_$P($G(^BMCREF(ACHSREF,1)),U)
EXT1 ;
 K ACHSDX,ACHSPX,X,N
