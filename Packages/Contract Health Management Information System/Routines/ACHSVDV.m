ACHSVDV ; IHS/ITSC/PMF - 1/2 VIEW/EDIT PROVIDER-VENDOR FILE ;
 ;;3.1;CONTRACT HEALTH MGMT SYSTEM;**6,11,13,19,23,24,29**;JUN 11, 2001;Build 86
 ...
 ;ACHS*3.1*13 11/29/06 IHS/OIT/FCJ FORMAT CHANGES AND ADDED DUNS NUMBER
 ;ACHS*3.1*23 IHS.OIT.FCJ ADDED DIRECT PARTICIPANT AND DIRECT EMAIL ADDRESS
 ;ACHS*3.1*29 5.1.2021 IHS.OIT.FCJ ADDED UEI AND EFT
 ;
A1 ;EP
 D LINES^ACHSFU,HDR
...
A2 ;EP
 Q:'$D(ACHSPROV)
 S ACHS0=$G(^AUTTVNDR(ACHSPROV,0)),ACHS11=$S($D(^AUTTVNDR(ACHSPROV,11)):^(11),1:""),ACHS13=$S($D(^AUTTVNDR(ACHSPROV,13)):^(13),1:"")
 S ACHS17=$S($D(^AUTTVNDR(ACHSPROV,17)):^(17),1:""),ACHS21=$S($D(^AUTTVNDR(ACHSPROV,21)):^(21),1:"")
 ...
 W !,"11) E-Mail: ",$P(ACHS21,U,3)               ;ACHS*3.1*19
 W ?46,"12) DUNS: ",$P(ACHS0,U,7)
 W !,"13) Direct Participant: ",$P(ACHS21,U,4)        ;ACHS*3.1*23 IHS.OIT.FCJ
 W ?46,"14) UEI: ",$P(ACHS0,U,10)                     ;ACHS*3.1*29 IHS.OIT.FCJ
 W !,"15) Direct E-Mail: ",$E($P(ACHS21,U,5),1,27)    ;ACHS*3.1*23 IHS.OIT.FCJ;ACHS*3.1*29
 W ?46,"16) EFT: ",$P(ACHS0,U,9)                      ;ACHS*3.1*29 IHS.OIT.FCJ
 W !!,"**** MAILING/BILLING ADDRESS ****",?44,"**** PROVIDER LOCATION ADDRESS ****"
 W !,"17) Street: ",$P(ACHS13,U),?46,"18) Street: ",$E($P(ACHS13,U,6),1,17)
 ...
 W ?46,"20) Fed/Non-Fed: ",$S($P(ACHS11,U,10)=1:"NON-FED",$P(ACHS11,U,10)=2:"FED",$P(ACHS11,U,10)="":"")
 W !,"21) Specialty: ",$S($P(ACHS11,U,4):$P(^DIC(7,$P(ACHS11,U,4),0),U),1:"")
 ;IHS/SET/JVK ACHS*3.1*6 ADD GEOGRAPHICAL LOCATION 4/15/2003     
 W ?46,"22) Geographic Loc: ",$S($P(ACHS11,U,25)="":"",$P(ACHS11,U,25):$P(^AUTTGL($P(ACHS11,U,25),0),U))
 ...
 ;ITSC/SET/JVK ACHS*3.1*11 CHG 12 TO 13;ACHS*3.1*13 IHS/OIT/FCJ CHG 14;ACHS*3.1*22 CHG 15;ACHS*3.1*24 CHG 17;;ACHS*3.1*29 CHG 19;
 ;I ACHSVT="" W *7,!!,"MUST HAVE VENDOR TYPE." D 12^ACHSVDVA G A2:'$D(Y),A1
 I ACHSVT="" W *7,!!,"MUST HAVE VENDOR TYPE." D 19^ACHSVDVA G A2:'$D(Y),A1
A3 ;
