ACHSUUP ; IHS/ITSC/PMF - UPDATE OBLIGATION BALANCE/DCR ACCOUNTS/QUEUE DOCUMENTS ;   [ 02/23/2005  11:39 AM ]
 ;;3.1;CONTRACT HEALTH MGMT SYSTEM;**7,8,9,12,28**;JUNE 11, 2001;Build 86
 ...
 ;ACHS*3.1*28 IHS.OIT.FCJ AUTO FIX OUT OF BALANCE REG
 ;
 D CKB,SBA:'$D(ACHSCNC)
 Q
 ;
...
SBA ;
 ...
 I '$$LOCK^ACHS("^ACHS(9,DUZ(2))","+") W !,"LOCK OF '^ACHS(9,",DUZ(2),") FAILED AT SBA^ACHSUUP." S ACHSCNC=1 Q   ;ACHS*3.1*28 ADDED ACHSCNC
 ;
...
CKB ;EP - CK for "OUT-OF-BALANCE" condition.
 ...
 ;W *7,!!!!,ACHS,!!,ACHS,!!!?22,$G(IORVON),"THE REGISTERS ARE OUT OF BALANCE",$G(IORVOFF),!!?26,$G(IORVON),"CONTACT YOUR SITE MANAGER",$G(IORVOFF),!!!,ACHS,!!,ACHS,!   ;ACHS*3.1*28
 ;D RTRN^ACHS            ;PRESS RETURN TO CONTINUE   ;ACHS*3.1*28
 ;I $D(^XUSEC("ACHSZMGR",DUZ)),$$DIR^XBDIR("Y","Want me to fix it for you","N","","","^D HELP^ACHSUUP",1) D FIX^ACHSBRF(ACHSACFY,ACHSACWK),RTRN^ACHS G CKB   ;ACHS*3.1*28
 W *7,!!!!,ACHS,!!,ACHS,!!!?22,$G(IORVON),"THE REGISTERS ARE OUT OF BALANCE",$G(IORVOFF),!!!,ACHS,!!,ACHS,!   ;ACHS*3.1*28
 W !!,"FIXING BALANCES",! D FIX^ACHSBRF(ACHSACFY,ACHSACWK),RTRN^ACHS G CKB  ;ACHS*3.1*28
 Q
 ;
HELP ;EP - From DIR.
