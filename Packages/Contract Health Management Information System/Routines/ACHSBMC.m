ACHSBMC ; IHS/ITSC/PMF - RCIS INTERFACE SUBROUTINES ;
 ;;3.1;CONTRACT HEALTH MGMT SYSTEM;**5,10,11,13,16,22,23,31,32**;JUN 11,2001;Build 39
 ...
 ;ACHS*3.1*31 IHS.OIT.FCJ CLOSING INPATIENT DENIAL REFERRALS
 ;ACHS*3.1*32 6/13/24 IHS.OIT.FCJ NEW STANDARD MED PRIORITIES;REMV TEST FOR CLOSED REF ON DENIAL ENTRY
 ;
...
GETREF4 ;ACHS*3.1*10 4.19.04 IHS/ITSC/FCJ ADD LN LABEL NXT SEC
 ...
 ;ACHS*3.1*32 ST OF CHG;REMV TEST FOR CLOSE REF ON DENIAL
 ...
 I $G(ACHD("FAC"))'="",($G(ACHS(.04))="I")!($G(ACHS(.04))="N") D  G GETREF0
 .W !!,"     This must be a Referral that is 'CHS FACILITY' or 'OTHER'."
 .W !,"You have selected a Referral that is '",$$EXTSET^XBFUNC(90001,.04,$G(ACHS(.04))),"'.",!
 .S ACHS=0
 ;ACHS*3.1*32;END OF CHG 
 ;ACHS*3.1*10 4.19.04 IHS/ITSC/FCJ END OF CHANGES
 ;ACHS*3.1*22 TEST FOR CLOSED REF CHCK TO CONTINUE;ACHS*3.1*32 ADDED CLOSED REF TST FOR DENIAL
 ;I $G(ACHD("FAC"))="",($G(ACHS(.15))="C1")!($G(ACHS(.15))="X") D  G:Y=0 GETREF1  ;ACHS*3.1*32
 I ($G(ACHS(.15))="C1")!($G(ACHS(.15))="X") D  G:Y=0 GETREF1    ;ACHS*3.1*32
 ...
 ;ACHS*3.1*32 TEST FOR NEW PRI GLB
 S ACHSRMPC="",ACHSRMP=0
 I $D(^BMCMPRI(0)) D
 .;I ACHS(.32)'="" S ACHSRMPC=$P($G(^BMCMPRI(ACHS(.32),0)),U) I ACHSRMPC'="",'$D(^ACHSMPR("B",ACHSRMPC)) S ACHSRMPC=""
 .Q:ACHS(.32)=""  S ACHSRMPC=$P($G(^BMCMPRI(ACHS(.32),0)),U)
 .I ACHSRMPC'="",'$D(^ACHSMPR("B",ACHSRMPC)) S ACHSRMPC="" Q
 .S ACHSRMP=$O(^ACHSMPR("B",ACHSRMPC,ACHSRMP))
 E  S ACHSRMPC=$S($G(ACHS(.32))=1:"I",$G(ACHS(.32))=2:"II",$G(ACHS(.32))=3:"III",$G(ACHS(.32))=4:"IV",1:"") ;ACHS*3.1*10
...
STAT(S) ;EP - Update Referral status
 ...
 .S ACHS(.15)=$S((ACHS(.14)="O")!(ACHS(.14)="I"):"C1",1:"A")     ;ACHS*3.1*31 CLOSING INP REF
