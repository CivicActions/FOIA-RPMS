ACHSPCC4 ; IHS/ITSC/PMF - CHS AREA SPLITOUT (4/5)(EOJ) ; [ 12/06/2002  10:36 AM ]
 ;;3.1;CONTRACT HEALTH MGMT SYSTEM;**5,21,23,32,33**;JUN 11,2001;Build 39
 ;IHS/SET/GTH ACHS*3.1*5 12/06/2002 - Remove direct ref to non-package global.
 ;ACHS*3.1*32 12.3.24 IHS.OIT.FCJ REMV UNUSED REF TO IHS COMMUNICATIONS PARAMETER FILE AND changed to CRV004 NDW version
 ;ACHS*3.1*33 3.7.25 IHS.OIT.FCJ FIXED TRAILER RECORD FOR THE FI
 ;
END ;EP
...
PCCHJCL ;EP - Generate Head JCL for Parklawn Computer Center.
 Q      ;ACHS*3.1*32  NO LONGER SENDING DATA TO PCC
...
PCCTJCL ;EP - Generate Tail JCL for Parklawn Computer Center.
 ;Q      ;ACHS*3.1*32  NO LONGER SENDING DATA TO PCC;ACHS*3.1*33 THIS IS REQUIRED FOR THE FI trailer record
...
PROD1 ;S X="//IHS001 JOB (1103,SBSP),'PRODUCTION',CLASS=Q,MSGCLASS=H" D PADWRITE^ACHSPCC3 ;ACHS*3.1*32
PROD2 ;S X="//STEP01 EXEC IHS001,AREA=RMT"_$E(1000+ACHSFIRN,2,4) D PADWRITE^ACHSPCC3      ;ACHS*3.1*32
PROD3 ;S X="//STEP010.IHSODOC DD *" D PADWRITE^ACHSPCC3                                   ;ACHS*3.1*32
...
DPSHJCL ;EP - Generate Head JCL for Data center.
 ...
 I ACHSGLBL="^ACHSPG2" S X=X_" CRV004"   ;ACHS*3.1*32
 D PADWRITE^ACHSPCC3
 Q
 ;
