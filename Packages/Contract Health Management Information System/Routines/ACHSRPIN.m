ACHSRPIN ; IHS/ITSC/PMF - retrieve ALL insurances, display, choose  ;
 ;;3.1;CONTRACT HEALTH MGMT SYSTEM;**3,13,21,27,28,31,32**;JUN 11,2001;Build 39
 ...
 ;ACHS*3.1*28 12/12/2019 IHS.OIT.FCJ ADDED ENTRY PT COMMENT
 ;ACHS*3.1*31 8.15.23 IHS.OIT.FCJ FIXED RRE CHECK FOR DOS
 ;
 ;INPUT:     DFN
 ;
 ;OUTPUT:    INS array, list all insurances sorted by DOS
 ;
 ;
GET ;EP- FROM ACHSDN2A, DOC ENTRY-ACHSA1, VURS REPORT-ACHSVUR2
 K INS
 ;
...
MCD ;
 ...
 ;ACHS*3.1*32;REWROTE SECTION-ST OF CHNG, DROPPING IF BEGIN INSURANCE DATE WAS THE SAME
 ...
 S I=0,CT=0 F  S I=$O(^AUPNMCD("B",DFN,I)) Q:I'?1N.N  S JJ=0 F  S JJ=$O(^AUPNMCD(I,11,JJ)) Q:JJ'?1N.N  D
 . S CT=CT+1
 . S ^TMP("ACHSRP31",$J,"MCD",CT)=$G(^AUPNMCD(I,11,JJ,0))
 . S $P(^TMP("ACHSRP31",$J,"MCD",CT),U,4,6)=$P($G(^AUPNMCD(I,0)),U,2,4)
 . S $P(^TMP("ACHSRP31",$J,"MCD",CT),U,7,8)=I_U_JJ
 ...
 S JJ=0,ACHS=0 F  S ACHS=ACHS+1,JJ=$O(^TMP("ACHSRP31",$J,"MCD",JJ)) Q:JJ'?1N.N  I $P(^TMP("ACHSRP31",$J,"MCD",JJ),U,6)]"",$D(^DIC(5,$P(^(JJ),U,6),0)) S $P(^TMP("ACHSRP31",$J,"MCD",JJ),U,6)=$P(^(0),U,2)
 S I=0,ACHS=0 F  S ACHS=ACHS+1,I=$O(^TMP("ACHSRP31",$J,"MCD",I)) Q:I'?1N.N  D
...
RRE ;
 ...
 .S DAT=^AUPNRRE(DFN,11,JJ,0)         ;ACHS*3.1*31
 .I $D(ACHDDOS),ACHDDOS<$P(DAT,U) Q   ;ACHS*3.1*21
 .I $D(ACHDDOS),$P(DAT,U,2)'="",ACHDDOS>$P(DAT,U,2) Q  ;ACHS*3.1*21
