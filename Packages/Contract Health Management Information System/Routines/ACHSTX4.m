...
B2 ;
 ...
 ;
 S ACHSRCT=ACHSRCT+1,ACHSRTYP(4)=ACHSRTYP(4)+1
 ;ACHS*3.1*28 ADDED ^ACHSBCBS
 S (^ACHSBCBS(ACHSRCT),^ACHSDATA(ACHSRCT))="4A"_ACHSEIN_ACHSNAME_ACHSPTYP_ACHSFED_ACHSFAC_ACHSDAP_ACHSUPDT_ACHSCN_ACHSDEST
 ;
 ...
 ;ACHS*3.1*28 ADDED ^ACHSBCBS
 S (^ACHSBCBS(ACHSRCT),^ACHSDATA(ACHSRCT))=^ACHSDATA(ACHSRCT)_ACHSST_$S($D(^AUTTVNDR(R,13)):$E($P($G(^AUTTVNDR(R,13)),U,4)_$J("",9),1,9),1:"")_ACHS1099_ACHSFONE_ACHSAPN_ACHSUPDT("CC")_" "_ACHSDEST
 ;
