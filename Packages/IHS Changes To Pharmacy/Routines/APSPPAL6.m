APSPPAL6 ;IHS/MSC/PLS - Pickup Activity Log SPEED Support ;16-Sep-2024 15:34;DU
 ;;7.0;IHS PHARMACY MODIFICATIONS;**1035**;Sep 23, 2004;Build 39
 ;             - 09/06/2024 - FID 99319
SPEED ;-Main entry - Called from APSP SPEED PICKUP Protocol
 N RXIEN,APSPPOP,ACTDT,DFN,APSPVAL,APSPZPAL,ARY,APSPUSE,APSPERR,APSPSKIP
 S APSPPOP=0,APSPZPAL=0,APSPUSE=0,APSPERR=0
 ...
 ..S APSPSKIP=0
 ..W !,"Processing SEQ number: "_$P(LST,",",ORD)_"   (Rx#: "_$P($G(^PSRX($P(PSOLST(ORN),"^",2),0)),U)_")"
 ..;W !,"Rx#:"_$P($G(^PSRX($P(PSOLST(ORN),U,2),0)),U)
 ...
 ..I APSPUSE=1 D  Q:$G(APSPSKIP)  ;Auto fill
 ...S APSPSKIP=$$GETFILL($P(PSOLST(ORN),"^",2)) ;prescription not eligible or user didn't set pickup time.
 ...;I GF S APSPSKIP=1  ; no dispenses available skip prescription
 ...I 'APSPSKIP D  Q
 ....N DATA
 ...
 ...E  W !,"A Pickup Date/Time was not entered."
 ..E  D SINGLERX
 ..I $L(LST,",")>2&'APSPUSE,'APSPSKIP D
 ...Q:ORD+1=$L(LST,",")  ;No reason to prompt if processing last item in list
 ...W !
...
SINGLERX ;
 ...
 S APSPPOP=0
 D GETPUP1(RXP)
 I APSPSKIP S VALMBCK="R",VALMSG="" Q
...
GETPUP1(RXN) ;-
 ...
 ;
 S Y=$$GETDPITM(RXN)
 I Y=-3 S APSPSKIP=1 Q
 I Y=-2 D  Q
 .S APSPSKIP=1
 .;W !,"Rx#:"_$P(^PSRX(RXN,0),U)
 .W $C(7),!,?5,"All dispenses have a pickup log entry. Use the Prescription Pickup"
 .W !,?5,"Log Edit and/or Delete actions to manage entries as necessary."
 ...
 D ^DIR
 I Y<1 S APSPSKIP=1 Q
 I Y<EDT S APSPSKIP=1 D  Q
...
GETDPITM(RX) ; Return a dispense
 ...
 ..I LP<LAST D
 ...S DIR("L",LP+2)="  "_LP_" - "_$P(LST(LP),U,2)_" on "_$$FMTE^XLFDT($P(LST(LP),U,3),"5Z")
 ..S DIR("L",2)="The following dispenses are selectable for pickup:"
 ..S DIR("L",1)="Rx#:"_$P(^PSRX(RX,0),U)
