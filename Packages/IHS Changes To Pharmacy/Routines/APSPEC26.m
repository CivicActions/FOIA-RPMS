APSPEC26 ;IHS/CIA/PLS - APSP ENVIRONMENT CHECK ROUTINE ;19-Aug-2020 07:31;DU
 ;;7.0;IHS PHARMACY MODIFICATIONS;**1026**;Sep 23, 2004;Build 47
 ;
ENV ;EP
 ;
...
PRE ;EP - Pre-init
 N IEN
 D RENAME^XPDMENU("APSP SS REFILL REQUEST","APSP SS RENEW REQUEST")
 ;Fix OO entry
 S IEN=$$FIND1^DIC(9009033.7,,"X","OO","B")
 I IEN D
 .N FDA,IENS,ERR
 .S FDA(9009033.7,"279,",.01)="00"
...
POST ;EP
 N ARY,ERR,IDX,LP
 D ADD^XPAR("SYS","APSP SS NDC SOURCE",,"N")
 D GETLST^XPAR(.ARY,"SYS","APSP RESTRICTED LIQUID TERMS","I",.ERR)
 S LP=0 F  S LP=$O(ARY(LP)) Q:'LP  I ARY(LP)="TSPS" S IDX=LP
 I '$G(IDX) D
 .S IDX=+$O(ARY($C(1)),-1)+1
 .D ADD^XPAR("SYS","APSP RESTRICTED LIQUID TERMS",IDX,"TSPS")
 ;D FIXNCPDP
 Q
FIXNCPDP ;-
 N DA,DIK
 S DA=220
 S $P(^APSPNCP(9009033.7,DA,0),U,4)="C120216"
 S DIK="^APSPNCP(9009033.7,"
 S DIK(1)=3
 D EN^DIK
 ;
 Q
