APSPRRTS ; IHS/MSC/PLS - RETURN TO STOCK REPORT ;31-Aug-2022 11:27;DU
 ;;7.0;IHS PHARMACY MODIFICATIONS;**1006,1007,1008,1009,1031**;Sep 23, 2004;Build 15
 ;
 ; IHS/MSC/MIR 73988 05/24/2022 Fixed date range 2 years back from report date
 ;                              EN+6 to EN+9
 ;
EN ;EP
 ...
 ;IHS/MSC/MIR 73988 05/24/2022 Fixed date range 2 years back from report date
 ;IHS/MSC/PLS 73988 Restored date range prompt per Latona 8-24-2022
 D ASKDATES^APSPUTIL(.APSPBD,.APSPED,.APSPQ,DT,DT)
 Q:APSPQ
 ;S APSPED=DT,APSPBD=DT-20000
...
PRINT1 ;EP
 ...
 ....S SUB3=0 F  S SUB3=$O(^TMP($J,"XREF",DIV,"DRUG",SUB1,SUB2,SUB3)) Q:'SUB3  S VAL=^(SUB3) D
 .....;S VAL=^(SUB3)
 .....D PRINT2(^TMP($J,"DATA",VAL))
 .....S DFLG=1
 .E  I 'APSPDSUB D
 ..S SUB1=0 F  S SUB1=$O(^TMP($J,"XREF",DIV,"RTS",SUB1)) Q:'SUB1  D
 ...S SUB2=0 F  S SUB2=$O(^TMP($J,"XREF",DIV,"RTS",SUB1,SUB2)) Q:'SUB2  S VAL=^(SUB2) D
 ....;S VAL=^(SUB2)
 ....D PRINT2(^TMP($J,"DATA",VAL))
 ....S DFLG=1
 .E  I APSPRTYP=2 D
 ...
 .....S SUB4=0 F  S SUB4=$O(^TMP($J,"XREF",DIV,"DCLS",SUB1,SUB2,SUB3,SUB4)) Q:'SUB4  S VAL=^(SUB4) D
 ......;S VAL=^(SUB4)
 ......D PRINT2(^TMP($J,"DATA",VAL))
 ......S DFLG=1
 Q
