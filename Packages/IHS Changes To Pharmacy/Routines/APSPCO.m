APSPCO ; IHS/MSC/PLS - List Manager Complete Orders ;04-May-2023 12:57;DU
 ;;7.0;IHS PHARMACY MODIFICATIONS;**1013,1014,1015,1016,1023,1025,1028,1030,1034**;Sep 23, 2004;Build 37
 ; Modified - IHS/MSC/PB - 09/28/2012 - OERR+26 to remove patient language flag
 ;            IHS/MSC/PLS - 02/25/2022 - OERR+13
 ;=================================================================
 ;IHS/MSC/MGH Patch 1016 ADDITEM +2,6,11 Added visual for a flagged order
 ;Modified EPCS ADDITEM+8,ADDPT+8,TOGORPT+28,ADDITEM+5
 ;         TOGORPT+28 - p1034
...
ADDITEM(IEN,INST) ;EP-
 N NOD0,LINE,COM,PTLOCK,FLAG,CS,DIG,DRUG,HC,EDT
 S FLAG=0,HC=0,DIG=0
 S NOD0=$G(^PS(52.41,IEN,0))
 Q:'NOD0
 S EDT=$$VALUE^ORCSAVE2(+NOD0,"EARLIEST")
 S PICKUP=$$VALUE^ORCSAVE2(+NOD0,"PICKUP")
 Q:+$P(EDT,".")>$$DT^XLFDT()  ;EPCS
 I LOCFLG Q:'$D(APSPLOCS(+$P(NOD0,U,13)))  ;IHS/MSC/PLS 1/15/2020
 I $P($G(^PS(52.41,IEN,0)),"^",23)=1 S FLAG=1
 S DRUG=$P(NOD0,U,9),HC=0
 ;IHS/GDIT/MSC/MGH EPCS changes
 I +DRUG D
 ...
 S STR=$S($$PTLOCK($P(NOD0,U,2)):"*",1:"")_$S(HC=1:"+",+DIG:"$",1:"")
 ;IHS/MSC/PLS - p1034
 ;S LINE=$$SETFLD^VALM1(STR_$$GET1^DIQ(2,$P(NOD0,U,2),.01),LINE,"PATIENT")
 S LINE=$$SETFLD^VALM1(STR_$$GETPREF^AUPNSOGI($P(NOD0,U,2),"E",1),LINE,"PATIENT")
...
SORTLST ;EP-
 ...
 .Q:'NOD0
 .;IHS/MSC/PLS - p1034
 .;S IDX=$S(CL=1:+$$HRN^AUPNPAT($P(NOD0,U,2),$S(APSPINS:APSPINS,1:DUZ(2))),CL=2:$$GET1^DIQ(2,$P(NOD0,U,2),.01),CL=3:$$DOB^AUPNPAT($P(NOD0,U,2)),CL=5:$$LOCNM($P(NOD0,U,13)),1:$$SRTDT())  ;1:$P(NOD0,U,6))
 .S IDX=$S(CL=1:+$$HRN^AUPNPAT($P(NOD0,U,2),$S(APSPINS:APSPINS,1:DUZ(2))),CL=2:$$GETPREF^AUPNSOGI($P(NOD0,U,2),"E",1),CL=3:$$DOB^AUPNPAT($P(NOD0,U,2)),CL=5:$$LOCNM($P(NOD0,U,13)),1:$$SRTDT())  ;1:$P(NOD0,U,6))
 .S CNT=CNT+1
...
LOCNM(IEN) ;EP-
 N LOC
 S LOC=$$GET1^DIQ(44,IEN,.01)
 Q $S(LOC="":"UNKNOWN",1:LOC)
...
ADDPT(DFN,INST,ORDCNT,LOCNM) ;EP-
 D ADDPT^APSPCO2(DFN,INST,ORDCNT,LOCNM) ;IHS/MSC/MGH Moved due to size issues
 Q
...
OERR(Y) ;EP-Copied from PSORX1
 ...
 ;IHS/MSC/PLS - 02/25/2022
 I $G(PSOFIN) S SSN=$P(^DPT(PSODFN,0),"^",9) W !!?10,$C(7),PSORX("NAME")_" (XXX-XX-XXX) " K SSN  ;_$E(SSN,1,3)_"-"_$E(SSN,4,5)_"-"_$E(SSN,6,9)_")" K SSN
