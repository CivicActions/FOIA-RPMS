APSPEC35 ;IHS/CIA/PLS - APSP ENVIRONMENT CHECK ROUTINE ;01-Jul-2024 14:29;DU
...
REMXPAR(PAR) ;Remove values stored for a given parameter
 ...
 Q
POST ;EP
 D REGMENU^BEHUTIL("APSP PRESCRIPTION PICKUP LOG",,"PLOG","APSP MAIN MENU")
 D REGMENU^BEHUTIL("APSP SPI PROVIDER RPT",4,"RPT","APSP SPI NCPDP MENU")
 D REGMENU^BEHUTIL("APSP WKLD RPT UD/IV ORDERS",,"PWKI","APSP MAIN MENU")
 D REGNMSP^CIAURPC("APSP","CIAV VUECENTRIC")
 ;Build "AC" xref of "ZPAL" multiple
 N RX,DA,DIK
 S RX=0 F  S RX=$O(^PSRX(RX)) Q:'RX  S DA(1)=RX,DIK="^PSRX("_DA(1)_",""ZPAL"",",DIK(1)=".02^AC" D ENALL^DIK
 Q
 ; Initialize APSP PICKUP FIELDS PROXY xpar
POPXPAR ;-
 N LST,L
 D GETLST^XPAR(.LST,"ALL","APSP PICKUP FIELDS PROXY","I")
 Q:$G(LST)
 F L="L","F","D","A1","A2","C","S","Z","P" D
 .D ADD^XPAR("SYS","APSP PICKUP FIELDS PROXY",L,0)
 Q
