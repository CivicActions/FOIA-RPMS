APSPCCN ; IHS/DSD/ENM - CREATE PCC NEW RX LINKAGE ;  [ 05/02/2003  2:41 PM ]
 ;;6.0;IHS PHARMACY MODIFICATIONS;**4**;11/17/2002
 ; NOTE: CALLED FROM PSONEW3,PSORENW1,APSPCCE
 ;
EN ; NORMAL ENTRY POINT FROM PHARMACY ROUTINES -- TASKMAN SETUP
 S APSRX0=^PSRX(APSRX,0)
 ;G ZTSK ;ABCD
 K ZTSAVE F %="APSRX","APSRX0","APCDALVR(" S ZTSAVE(%)=""
 S ZTRTN="ZTSK^APSPCCN",ZTDESC="NEW PRESCRIPTION LINK TO PCC FROM PHARMACY",ZTIO="",ZTDTH=DT D ^%ZTLOAD K ZTSK
 K APSRX,APSRX0,ZTRTN,ZTDESC,ZTIO,ZTDTH
 Q
 ;
...
RX ;S:$D(^PSRX(APSRX)) ^PSRX(APSRX,999999911)=APCDALVR("APCDADFN")
 I $D(^PSRX(APSRX)) NEW DIE,DR,DA S DIE="^PSRX(",DR="9999999.11////"_APCDALVR("APCDADFN"),DA=APSRX D ^DIE K DA,DIE,DR  ;IHS/OHPRD/JCM 6/11/90
 D K^APSPCCLQ  ; REMOVE QUEUE ENTRY ABCD
 S:$D(ZTQUEUED) ZTREQ="@"
EXIT K %,APSRX,APSRX0,APCDALVR
 Q
 ;
GVISIT ;
...
GVMED ;
 ...
 S X=$P(%,U,10),APCDALVR("APCDTSIG")=$S($L(X)>145:$E(X,1,144)_"~",1:X)  ;IHS/OKCAO/POC 11/17/2002 DONT TRUNCATE UNTIL 145 THE SIZE OF SIG IN VMED
 ;S APCDALVR("APCDTQTY")=+$P(%,U,7)\1
 S APCDALVR("APCDTQTY")=+$P(%,U,7) ;IHS/ITSC/ENM 05/02/03
