APSPEC25 ;IHS/CIA/PLS - APSP ENVIRONMENT CHECK ROUTINE ;03-Jun-2020 15:06;DU
 ;;7.0;IHS PHARMACY MODIFICATIONS;**1025**;Sep 23, 2004;Build 33
 ;
ENV ;EP
 ;
 S X=$$GET1^DIQ(200,DUZ,.01)
 W !!,$$CJ^XLFSTR("Hello, "_$P(X,",",2)_" "_$P(X,","),IOM)
 W !!,$$CJ^XLFSTR("Checking Environment for "_$P($T(+2),";",4)_" V "_$P($T(+2),";",3)_", Patch 1024.",IOM)
...
MES(TXT,QUIT) ;EP
 ...
 ;
PRE ;EP - Pre-init
 ; Populate the Request Type field with a default R if empty
 ;N LP
 ;S LP=0 F  S LP=$O(^APSPRREQ(LP)) Q:'LP  D
 ;.S:$P($G(^APSPRREQ(LP,0)),U,12)="" $P(^APSPRREQ(LP,0),U,12)="R"
 Q
...
ONEZC(ZC) ;EP
 ...
 Q
ADDITEMS ;Add items to files for eRX without structured sigs
 N FIL,FDA,ERR,IEN,ITEM,IENS,ERR
 S ITEM="AS WRITTEN"
 S CHK=$O(^PS(51.1,"B",ITEM,""))
 I 'CHK D
 .S FIL=51.1,IEN="+1,"
 .S FDA(FIL,IEN,.01)=ITEM
 .S FDA(FIL,IEN,4)="PS"
 .S FDA(FIL,IEN,5)="C"
 ...
 S FIL=51.2,IEN="+1,"
 S ITEM="SEE SIG"
 S CHK=$O(^PS(51.2,"B",ITEM,""))
 I 'CHK D
 .S FIL=51.2,IEN="+1,"
 .S FDA(FIL,IEN,.01)=ITEM
 .S FDA(FIL,IEN,1)="SS"
 .S FDA(FIL,IEN,3)=1   ;"ALL PACKAGES"
 .S FDA(FIL,IEN,4)="See Sig"
 .D UPDATE^DIE(,"FDA","IENS","ERR")
 .K IENS,ERR,FDA
 Q
 ;
GENSSAC ; Generate New AC XREF
 N DIK
 S DIK="^APSPRREQ(",DIK(1)=.12 D ENALL^DIK
 Q
 ;Unlock schedule restriction
UNLOCK ;EP-
 D UPDPAR("SYS","APSP AUTO RX SCHEDULE RESTRICT")
 Q
 ;
UPDPAR(ENT,PARAM,VAL,LCK) ;EP-
 ...
 S $P(^XTV(8989.51,IEN,0),U,6)=$G(LCK,0)
 Q
PAR ;Set system level for new parameters
 N LP,SEQ,TYPE,ERR
 D NDEL^XPAR("SYS","APSP RESTRICTED LIQUID TERMS",.ERR)
 D NDEL^XPAR("SYS","APSP LIQUID METRIC UNITS",.ERR)
 F LP=0:1 S SEQ=$P($T(UNIT+LP),";;",2) Q:'$L(SEQ)  D
 .S TYPE=$P($T(UNIT+LP),";;",3)
 .D ADD^XPAR("SYS","APSP RESTRICTED LIQUID TERMS",SEQ,TYPE)
 F LP=0:1 S SEQ=$P($T(LIQUID+LP),";;",2) Q:'$L(SEQ)  D
 .S TYPE=$P($T(LIQUID+LP),";;",3)
 .D ADD^XPAR("SYS","APSP LIQUID METRIC UNITS",SEQ,TYPE)
 Q
ADDMENU ;Add items to a menu
 ;D REGMENU^BEHUTIL("APSP MU MENU",,"MU","APSP MAIN MENU")
 D REGMENU^BEHUTIL("APSP EPCS PHARMACY REPORTS",,"EPCS","APSP MAIN MENU")
...
LNGNMX ;-
 ...
 Q
 ;
REPOINT ;Redo the DEA status field
 N IEN,SIEN,DEA,CREF,DIK,FDA,ERR,FNUM,AIEN
 S NIEN=0
 S IEN=0 F  S IEN=$O(^APSPCSA(IEN)) Q:'+IEN  D
 .S SIEN=0 F  S SIEN=$O(^APSPCSA(IEN,1,SIEN)) Q:'+SIEN  D
 ..S AIEN=SIEN_","_IEN_","
 ..S DEA=$P($G(^APSPCSA(IEN,1,SIEN,0)),U,6)
 ..I DEA'="" S NIEN=$O(^APSPAUDF("B",DEA,""))
 ..I +NIEN D
 ...S FDA(9009036.01,AIEN,.06)=NIEN
 ...D FILE^DIE("","FDA","ERR")
 Q
SPECREDO ;Repoint the Pharmacy specialty types from Set of Codes to Pointer
 N IEN,SIEN,TYP,CREF,DIK,FDA,ERR,FNUM,AIEN
 S NIEN=0
 S IEN=0 F  S IEN=$O(^APSPOPHM(IEN)) Q:'+IEN  D
 .S SIEN=0 F  S SIEN=$O(^APSPOPHM(IEN,8,SIEN)) Q:'+SIEN  D
 ..S AIEN=SIEN_","_IEN_","
 ..S TYP=$G(^APSPOPHM(IEN,8,SIEN,0))
 ..I TYP'="" S NIEN=$O(^APSPNCP(9009033.76,"D",TYP,""))
 ..I +NIEN D
 ...S FDA(9009033.98,AIEN,.01)=NIEN
 ...D FILE^DIE("","FDA","ERR")
 Q
 ;
 ;List of restricted units for liquids
UNIT ;;1;;TBS
 ;;2;;TABLESPOON
 ;;3;;TABLESPOONS
 ;;4;;TEASPOON
