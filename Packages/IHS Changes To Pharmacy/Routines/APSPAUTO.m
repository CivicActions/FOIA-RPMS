APSPAUTO ;IHS/CIA/PLS - Auto Release Prescription ;25-Feb-2025 16:30;DU
 ;;7.0;IHS PHARMACY MODIFICATIONS;**1002,1006,1008,1013,1014,1015,1017,1018,1021,1023,1036**;Sep 23, 2004;Build 17
...
BC1 ;
 ;IHS/MSC/PLS - 11/14/2017 - Comment next 3 line
...
EXPDT(RX,AUTO,RDT) ;EP-
 ...
 N RES,NREF,RX0,DS,RFCNT,EXTEXP,DE,CS,OEXPDT,ISSDT,CSEXP
 S CS=0,CSEXP=0
 S AUTO=$G(AUTO,0)
 ...
 S NREF=+$P(RX0,U,9)
 S DS=+$P(RX0,U,8)
 ;IHS/MSC/MGH use parameter for expiration dates for C2s
 I $P(CS,U,2) D
 .S CSEXP=+$$GET^XPAR("ALL","APSP CS II RX EXPIRE DAYS")
 S EXTEXP=$$GET1^DIQ(50,$P(RX0,U,6),9999999.08)
 S X2=$S(EXTEXP:EXTEXP,+$P(CS,U,2)&CSEXP:CSEXP,1:+$$EXPDATE^PSOUTLA2(CS,ISSDT))  ;CS:184,1:366)  ;p1036 ensure CSEXP has a value FID 114104
 S OEXPDT=$$FMADD^XLFDT(ISSDT,X2)
 S DS=$S(EXTEXP:EXTEXP,1:DS)
 I +$P(CS,U,2) S RES=1
 E  I $$FMADD^XLFDT(RDT,DS)'<(OEXPDT+1) S RES=0
 E  I 'NREF S RES=1
 E  D
 .S RFCNT=$O(^PSRX(RX,1,$C(1)),-1)
 .S RES=$S(RFCNT=NREF:1,1:0)  ; not eligible for change in expiration date
 Q $S(+$P(CS,U,2)&(RES):OEXPDT,RES:$$FMADD^XLFDT(RDT,DS),1:0)
