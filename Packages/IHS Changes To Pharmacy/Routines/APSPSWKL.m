APSPSWKL ; IHS/MSC/PLS - PHARMACY STAFF WORKLOAD REPORT ;21-Apr-2022 21:35
 ;;7.0;IHS PHARMACY MODIFICATIONS;**1013,1015,1028,1031**;Sep 23, 2004;Build 15
 ;     EN+27  Apr 21, 2022  IHS/MIR Feature 76885: Select Pharmacy Users regardless of Class
EN ;EP
 ...
 ..; MIR Feature 76885 Apr 21, 2022
 ..;S APSPCLAS=$$GET1^DIQ(200,APSPUSR,53.5)
 ..;I APSPCLAS="PHARMACIST"!(APSPCLAS="PHARMACY TECHNICIAN")!(APSPCLAS="PHARMACY PRACTITIONERS")!(APSPCLAS="CLINICAL PHARMACY SPECIALIST") D
 ..S APSPDARY(APSPUSR)=APSPNAME
 ..S APSPCNT=APSPCNT+1
 ..;E  W !,APSPNAME_" is not a pharmacy user."
...
NEW(RXIEN,IEN) ;Find new prescriptions
 N PHARM
 S PHARM=+$P($G(^PSRX(RXIEN,2)),U,3)  ;P1028
...
SETNEW(PHARM) ;Set the pharmacist data
 ...
 E  S PHARNAME=$S($P(^PSRX(RXIEN,0),U,11)="M":"MAIL",1:"UNKNOWN")
...
REFILL(RXIEN,IEN) ;Find refills
 N PHARM
 S PHARM=+$P($G(^PSRX(RXIEN,1,IEN,0)),U,5)  ;P1028
...
SETRFILL(PHARM) ;Set provider data
 N PHARNAME,GT,RT,NT,NP,RP,TP,DATA
 I +PHARM>0 S PHARNAME=$$GET1^DIQ(200,PHARM,.01)
 E  S PHARNAME=$S($P(^PSRX(RXIEN,1,IEN,0),U,2)="M":"MAIL",1:"UNKNOWN")
 I $D(^TMP("APSPW",$J,PHARNAME)) D
 .S DATA=$G(^TMP("APSPW",$J,PHARNAME))
 .S NP=$P(DATA,U,1),RP=$P(DATA,U,2),TP=$P(DATA,U,3)
 .S RP=RP+1,TP=TP+1
