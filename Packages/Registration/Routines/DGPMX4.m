DGPMX4 ; GENERATED FROM 'DGPM CHECK-IN LODGER' INPUT TEMPLATE(#2130), FILE 405;02/09/12
...
BEGIN S DNM="DGPMX4",DQ=1
 ...
 M DIEZAR=^DIE(2130,"AR") S DICRREC="TRIG^DIE17"
 S:$D(DTIME)[0 DTIME=300 S D0=DA,DIIENS=DA_",",DIEZ=2130,U="^"
...
C9F1 Q
X9 S DIC("S")="I $P($G(^BDGWD(+Y,0)),U,3)'=""I""" D W^DGPMVDD I $D(X) D ^DIC K DIC S DIC=DIE,X=+Y K:Y<0 X I $D(X) D WARD^DGPMVDD K:$D(DGOOS) X K DGOOS
 Q
 ;
...
X11 S:($P(DGPMP,"^",18)=$P(^DGPM(DA,0),"^",18)) Y="@42"
 Q
12 D:$D(DG)>9 F^DIE17,DE S DQ=12,DW="0;5",DV="R*P9999999.91'",DU="",DLB="TRANSFER FACILITY",DIFLD=.05
 S DE(DW)="C12^DGPMX4"
 S DU="AUTTTFAC("
 S Y="@"
 S X=Y,DB(DQ)=1 G:X="" N^DIE17:DV,A I $D(DE(DQ)),DV["I"!(DV["#") D E^DIE0 G A:'$D(X)
...
C12S S X="" G:DG(DQ)=X C12F1 K DB
 ...
 I $P(^DGPM(DA,0),"^",2)=3 S A1B2TAG="ADM" D ^A1B2XFR
C12F1 Q
X12 S DIC("S")="I $P($G(^AUTTTFAC(+Y,0)),U,2)=""""" D ^DIC K DIC S DIC=DIE,X=+Y K:Y<0 X
 Q
 ;
...
X13 S Y="@42"
 Q
14 S DQ=15 ;@41
15 D:$D(DG)>9 F^DIE17,DE S DQ=15,DW="0;5",DV="R*P9999999.91'",DU="",DLB="TRANSFER FACILITY",DIFLD=.05
 S DE(DW)="C15^DGPMX4"
 S DU="AUTTTFAC("
 G RE
...
C15S S X="" G:DG(DQ)=X C15F1 K DB
 ...
 I $P(^DGPM(DA,0),"^",2)=3 S A1B2TAG="ADM" D ^A1B2XFR
C15F1 Q
X15 S DIC("S")="I $P($G(^AUTTTFAC(+Y,0)),U,2)=""""" D ^DIC K DIC S DIC=DIE,X=+Y K:Y<0 X
 Q
 ;
...
X24 I DGPMP=^DGPM(DA,0) S Y=""
 Q
25 S DW="USR;3",DV="RP200'a",DU="",DLB="LAST EDITED BY",DIFLD=102
 S DE(DW)="C25^DGPMX4"
 S DU="VA(200,"
 S X=DUZ
 S Y=X
...
27 D:$D(DG)>9 F^DIE17,DE S DQ=27,DW="USR;4",DV="RD",DU="",DLB="LAST EDITED ON",DIFLD=103
