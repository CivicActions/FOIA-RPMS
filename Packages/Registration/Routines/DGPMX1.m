DGPMX1 ; GENERATED FROM 'DGPM ADMIT' INPUT TEMPLATE(#2127), FILE 405;02/09/12
...
BEGIN S DNM="DGPMX1",DQ=1
 ...
 M DIEZAR=^DIE(2127,"AR") S DICRREC="TRIG^DIE17"
 S:$D(DTIME)[0 DTIME=300 S D0=DA,DIIENS=DA_",",DIEZ=2127,U="^"
...
X15 I $P(^DGPM(DA,0),"^",18)'=9 S Y=.1
 Q
16 D:$D(DG)>9 F^DIE17,DE S DQ=16,DW="0;5",DV="R*P9999999.91'",DU="",DLB="TRANSFER FACILITY",DIFLD=.05
 S DE(DW)="C16^DGPMX1"
 S DU="AUTTTFAC("
 G RE
...
C16S S X="" G:DG(DQ)=X C16F1 K DB
 ...
 I $P(^DGPM(DA,0),"^",2)=3 S A1B2TAG="ADM" D ^A1B2XFR
C16F1 Q
X16 S DIC("S")="I $P($G(^AUTTTFAC(+Y,0)),U,2)=""""" D ^DIC K DIC S DIC=DIE,X=+Y K:Y<0 X
 Q
 ;
...
C21F1 Q
X21 S DIC("S")="I $P($G(^BDGWD(+Y,0)),U,3)'=""I""" D W^DGPMVDD I $D(X) D ^DIC K DIC S DIC=DIE,X=+Y K:Y<0 X I $D(X) D WARD^DGPMVDD K:$D(DGOOS) X K DGOOS
 Q
 ;
...
X28 I DGPMP=^DGPM(DA,0) S Y=""
 Q
29 S DW="USR;3",DV="RP200'a",DU="",DLB="LAST EDITED BY",DIFLD=102
 S DE(DW)="C29^DGPMX1"
 S DU="VA(200,"
 S X=DUZ
 S Y=X
...
31 D:$D(DG)>9 F^DIE17,DE S DQ=31,DW="USR;4",DV="RD",DU="",DLB="LAST EDITED ON",DIFLD=103
