...
DE S DIE="^DGPT(D0,""M"",",DIC=DIE,DP=45.02,DL=2,DIEL=1,DU="" K DG,DE,DB Q:$O(^DGPT(D0,"M",DA,""))=""
 I $D(^(0)) S %Z=^(0) S %=$P(%Z,U,26) S:%]"" DE(28)=%,DE(31)=% S %=$P(%Z,U,27) S:%]"" DE(34)=%,DE(37)=% S %=$P(%Z,U,28) S:%]"" DE(40)=%,DE(43)=% S %=$P(%Z,U,29) S:%]"" DE(50)=%,DE(53)=% S %=$P(%Z,U,30) S:%]"" DE(56)=%,DE(60)=%
...
X27 I '$D(DGEXQ(1)) S Y="@910"
 Q
28 S DW="0;26",DV="SX",DU="",DLB="WAS TREATMENT RELATED TO AGENT ORANGE EXPOSURE?",DIFLD=26
 S DU="Y:YES;N:NO;"
 G RE
X28 S DGFLAG=1 D 501^DGPTSPQ K:DGER X K DGER,DGFLAG
 Q
 ;
...
X29 S Y="@915"
 Q
30 S DQ=31 ;@910
31 S DW="0;26",DV="SX",DU="",DLB="TREATED FOR AO CONDITION",DIFLD=26
 S DU="Y:YES;N:NO;"
 S Y="@"
 S X=Y,DB(DQ)=1 G:X="" N^DIE17:DV,A I $D(DE(DQ)),DV["I"!(DV["#") D E^DIE0 G A:'$D(X)
 G RD
X31 S DGFLAG=1 D 501^DGPTSPQ K:DGER X K DGER,DGFLAG
 Q
 ;
32 S DQ=33 ;@915
33 D:$D(DG)>9 F^DIE17,DE S Y=U,DQ=33 D X33 D:$D(DIEFIRE)#2 FIREREC^DIE17 G A:$D(Y)[0,A:Y=U S X=Y,DIC(0)="F",DW=DQ G OUT^DIE17
X33 I '$D(DGEXQ(2)) S Y="@920"
 Q
34 S DW="0;27",DV="SX",DU="",DLB="WAS TREATMENT RELATED TO IONIZING RADIATION EXPOSURE?",DIFLD=27
 S DU="Y:YES;N:NO;"
 G RE
X34 S DGFLAG=2 D 501^DGPTSPQ K:DGER X K DGER,DGFLAG
 Q
 ;
...
X35 S Y="@925"
 Q
36 S DQ=37 ;@920
37 S DW="0;27",DV="SX",DU="",DLB="TREATED FOR IR CONDITION",DIFLD=27
 S DU="Y:YES;N:NO;"
 S Y="@"
 S X=Y,DB(DQ)=1 G:X="" N^DIE17:DV,A I $D(DE(DQ)),DV["I"!(DV["#") D E^DIE0 G A:'$D(X)
 G RD
X37 S DGFLAG=2 D 501^DGPTSPQ K:DGER X K DGER,DGFLAG
 Q
 ;
38 S DQ=39 ;@925
39 D:$D(DG)>9 F^DIE17,DE S Y=U,DQ=39 D X39 D:$D(DIEFIRE)#2 FIREREC^DIE17 G A:$D(Y)[0,A:Y=U S X=Y,DIC(0)="F",DW=DQ G OUT^DIE17
X39 I '$D(DGEXQ(3)) S Y="@930"
 Q
40 S DW="0;28",DV="SX",DU="",DLB="WAS TREATMENT RELATED TO SERVICE IN SW ASIA?",DIFLD=28
 S DU="Y:YES;N:NO;"
 G RE
X40 S DGFLAG=3 D 501^DGPTSPQ K:DGER X K DGER,DGFLAG
 Q
 ;
...
X41 S Y="@935"
 Q
42 S DQ=43 ;@930
43 S DW="0;28",DV="SX",DU="",DLB="EXPOSED TO ENVIR CONTAMINANTS",DIFLD=28
 S DU="Y:YES;N:NO;"
 S Y="@"
 S X=Y,DB(DQ)=1 G:X="" N^DIE17:DV,A I $D(DE(DQ)),DV["I"!(DV["#") D E^DIE0 G A:'$D(X)
 G RD
X43 S DGFLAG=3 D 501^DGPTSPQ K:DGER X K DGER,DGFLAG
 Q
 ;
44 S DQ=45 ;@935
45 D:$D(DG)>9 F^DIE17,DE S Y=U,DQ=45 D X45 D:$D(DIEFIRE)#2 FIREREC^DIE17 G A:$D(Y)[0,A:Y=U S X=Y,DIC(0)="F",DW=DQ G OUT^DIE17
X45 I '$D(DGEXQ(7)) S Y="@940"
 Q
...
X49 I '$D(DGEXQ(4)) S Y="@950"
 Q
50 S DW="0;29",DV="S",DU="",DLB="WAS TREATMENT RELATED TO MILITARY SEXUAL TRAUMA?",DIFLD=29
 S DU="Y:YES;N:NO;"
 G RE
X50 Q
...
X51 S Y="@955"
 Q
52 S DQ=53 ;@950
53 S DW="0;29",DV="S",DU="",DLB="TREATMENT FOR MST",DIFLD=29
 S DU="Y:YES;N:NO;"
 S Y="@"
 S X=Y,DB(DQ)=1 G:X="" N^DIE17:DV,A I $D(DE(DQ)),DV["I"!(DV["#") D E^DIE0 G A:'$D(X)
 G RD
