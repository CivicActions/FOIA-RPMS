...
DE S DIE="^DGPT(",DIC=DIE,DP=45,DL=1,DIEL=0,DU="" K DG,DE,DB Q:$O(^DGPT(DA,""))=""
 ...
 I $D(^(300)) S %Z=^(300) S %=$P(%Z,U,2) S:%]"" DE(29)=% S %=$P(%Z,U,3) S:%]"" DE(34)=% S %=$P(%Z,U,4) S:%]"" DE(40)=%,DE(44)=% S %=$P(%Z,U,5) S:%]"" DE(49)=% S %=$P(%Z,U,6) S:%]"" DE(54)=% S %=$P(%Z,U,7) S:%]"" DE(59)=%
 K %Z Q
 ;
W W !?DL+DL-2,DLB_": "
 Q
...
6 D:$D(DG)>9 F^DIE17,DE S DQ=6,DW="70;24",DV="*P80'X",DU="",DLB="SECONDARY DIAGNOSIS 9",DIFLD=79.24
 S DE(DW)="C6^DGPTX71",DE(DW,"INDEX")=1
 S DU="ICD9("
 G RE
...
11 D:$D(DG)>9 F^DIE17,DE S DQ=11,DW="71;1",DV="*P80'X",DU="",DLB="SECONDARY DIAGNOSIS 10",DIFLD=79.241
 S DE(DW)="C11^DGPTX71",DE(DW,"INDEX")=1
 S DU="ICD9("
 G RE
...
16 D:$D(DG)>9 F^DIE17,DE S DQ=16,DW="71;2",DV="*P80'X",DU="",DLB="SECONDARY DIAGNOSIS 11",DIFLD=79.242
 S DE(DW)="C16^DGPTX71",DE(DW,"INDEX")=1
 S DU="ICD9("
 G RE
...
21 D:$D(DG)>9 F^DIE17,DE S DQ=21,DW="71;3",DV="*P80'X",DU="",DLB="SECONDARY DIAGNOSIS 12",DIFLD=79.243
 S DE(DW)="C21^DGPTX71",DE(DW,"INDEX")=1
 S DU="ICD9("
 G RE
...
X28 I $P(DG701,U,2)]"" S Y="@820"
 Q
29 D:$D(DG)>9 F^DIE17,DE S DQ=29,DW="300;2",DV="SX",DU="",DLB="SUICIDE/SELF INFLICT INDICATOR",DIFLD=300.02
 S DU="1:Attempted Suicide;2:Accomplished Suicide;3:Self Inflicted Injury;"
 G RE
X29 S DGFLAG=2 D 701^DGPTSC01 K:DGER X K DGER,DGFLAG
 Q
 ;
...
X32 I '$D(DGBPC(3))!(DGDUP(3)) S Y="@840"
 Q
33 D:$D(DG)>9 F^DIE17,DE S Y=U,DQ=33 D X33 D:$D(DIEFIRE)#2 FIREREC^DIE17 G A:$D(Y)[0,A:Y=U S X=Y,DIC(0)="F",DW=DQ G OUT^DIE17
X33 I $P(DG701,U,3)]"" S Y="@840"
 Q
34 S DW="300;3",DV="SX",DU="",DLB="LEGIONNAIRE'S DISEASE",DIFLD=300.03
 S DU="1:Yes;2:No;"
 G RE
X34 S DGFLAG=3 D 701^DGPTSC01 K:DGER X K DGER,DGFLAG
 Q
 ;
...
X42 S Y="@860"
 Q
43 S DQ=44 ;@850
44 S DW="300;4",DV="P45.61'X",DU="",DLB="SUBSTANCE ABUSE",DIFLD=300.04
 S DU="DIC(45.61,"
 S X=DGTX
 S Y=X
 G Y
X44 S DGFLAG=4 D 701^DGPTSC01 K:DGER X K DGFLAG,DGER
 Q
 ;
...
X52 I '$D(DGBPC(6))!(DGDUP(6)) S Y="@880"
 Q
53 D:$D(DG)>9 F^DIE17,DE S Y=U,DQ=53 D X53 D:$D(DIEFIRE)#2 FIREREC^DIE17 G A:$D(Y)[0,A:Y=U S X=Y,DIC(0)="F",DW=DQ G OUT^DIE17
X53 I $P(DG701,U,6)]"" S Y="@880"
 Q
54 S DW="300;6",DV="NJ2,0X",DU="",DLB="CURRENT FUNCTIONAL ASSESSMENT",DIFLD=300.06
 G RE
X54 S DGFLAG=6 D 701^DGPTSC01 S:DGER X="" K DGER,DGFLAG K:+X'=X!(X>90)!(X<1)!(X?.E1"."1N.N) X
 Q
 ;
...
X57 I '$D(DGBPC(7))!(DGDUP(7)) S Y="@890"
 Q
58 D:$D(DG)>9 F^DIE17,DE S Y=U,DQ=58 D X58 D:$D(DIEFIRE)#2 FIREREC^DIE17 G A:$D(Y)[0,A:Y=U S X=Y,DIC(0)="F",DW=DQ G OUT^DIE17
X58 I $P(DG701,U,7)]"" S Y="@890"
 Q
59 S DW="300;7",DV="NJ2,0X",DU="",DLB="HIGH LEVEL PSYCH CLASS",DIFLD=300.07
 G RE
X59 S DGFLAG=7 D 701^DGPTSC01 S:DGER X="" K DGER,DGFLAG K:+X'=X!(X>90)!(X<1)!(X?.E1"."1N.N) X
 Q
 ;
