...
DE S DIE="^DPT(",DIC=DIE,DP=2,DL=1,DIEL=0,DU="" K DG,DE,DB Q:$O(^DPT(DA,""))=""
 ...
 I  S %=$P(%Z,U,9) S:%]"" DE(21)=% S %=$P(%Z,U,10) S:%]"" DE(11)=% S %=$P(%Z,U,11) S:%]"" DE(22)=%
...
C1F1 N X,X1,X2 S DIXR=27 D C1X1(U) K X2 M X2=X D C1X1("O") K X1 M X1=X
 D
...
C6F1 Q
X6 Q
7 D:$D(DG)>9 F^DIE17,DE S DQ=7,DW=".21;1",DV="FXa",DU="",DLB="K-NAME OF PRIMARY NOK",DIFLD=.211
...
C7F1 N X,X1,X2 S DIXR=297 D C7X1(U) K X2 M X2=X D C7X1("O") K X1 M X1=X
 I $G(X(1))]"" D
...
17 D:$D(DG)>9 F^DIE17,DE S DQ=17,DW=".21;5",DV="FX",DU="",DLB="K-STREET ADDRESS [LINE 3]",DIFLD=.215
 S DE(DW)="C17^DGRPTX5",DE(DW,"INDEX")=1
 G RE
...
18 D:$D(DG)>9 F^DIE17,DE S DQ=18,DW=".21;6",DV="FX",DU="",DLB="K-CITY",DIFLD=.216
 S DE(DW)="C18^DGRPTX5",DE(DW,"INDEX")=1
 G RE
...
19 D:$D(DG)>9 F^DIE17,DE S DQ=19,DW=".21;7",DV="P5'X",DU="",DLB="K-STATE",DIFLD=.217
 S DE(DW)="C19^DGRPTX5",DE(DW,"INDEX")=1
 S DU="DIC(5,"
 G RE
...
20 D:$D(DG)>9 F^DIE17,DE S DQ=20,DW=".22;7",DV="FOX",DU="",DLB="K-ZIP+4",DIFLD=.2207
 ...
 S DE(DW)="C20^DGRPTX5",DE(DW,"INDEX")=1
 G RE
...
X20 K:X[""""!($A(X)=45) X I $D(X) S DFN=DA D K1^DGLOCK2 I $D(X) K:$L(X)>15!($L(X)<5) X I $D(X) D ZIPIN^VAFADDR
 ...
 Q
 ;
21 D:$D(DG)>9 F^DIE17,DE S DQ=21,DW=".21;9",DV="FXa",DU="",DLB="K-PHONE NUMBER",DIFLD=.219
 S DE(DW)="C21^DGRPTX5"
 G RE
...
C21S S X="" G:DG(DQ)=X C21F1 K DB
 D ^DGRPTX12
C21F1 Q
X21 K:$L(X)>20!($L(X)<4) X I $D(X) S DFN=DA D K1^DGLOCK2
 I $D(X),X'?.ANP K X
 Q
 ;
22 D:$D(DG)>9 F^DIE17,DE S DQ=22,DW=".21;11",DV="F",DU="",DLB="K-WORK PHONE NUMBER",DIFLD=.21011
 G RE
...
X24 I $S('$D(^DPT(DFN,.21)):1,$P(^(.21),U,1)="":1,1:0) S Y=.331
 Q
25 D:$D(DG)>9 F^DIE17 G ^DGRPTX13
