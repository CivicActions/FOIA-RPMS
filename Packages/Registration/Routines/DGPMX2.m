DGPMX2 ; GENERATED FROM 'DGPM TRANSFER' INPUT TEMPLATE(#2451), FILE 405;02/09/12
...
DE S DIE="^DGPM(",DIC=DIE,DP=405,DL=1,DIEL=0,DU="" K DG,DE,DB Q:$O(^DGPM(DA,""))=""
 ...
 I $D(^("USR")) S %Z=^("USR") S %=$P(%Z,U,3) S:%]"" DE(42)=% S %=$P(%Z,U,4) S:%]"" DE(44)=%
 K %Z Q
 ;
W W !?DL+DL-2,DLB_": "
 Q
...
BEGIN S DNM="DGPMX2",DQ=1
 ...
 M DIEZAR=^DIE(2451,"AR") S DICRREC="TRIG^DIE17"
 S:$D(DTIME)[0 DTIME=300 S D0=DA,DIIENS=DA_",",DIEZ=2451,U="^"
...
X27 S Y="@25"
 Q
28 S DQ=29 ;@21
29 S DW="0;5",DV="R*P9999999.91'",DU="",DLB="ASIH FACILITY",DIFLD=.05
 S DE(DW)="C29^DGPMX2"
 S DU="AUTTTFAC("
 G RE
...
C29S S X="" G:DG(DQ)=X C29F1 K DB
 ...
 I $P(^DGPM(DA,0),"^",2)=3 S A1B2TAG="ADM" D ^A1B2XFR
C29F1 Q
X29 S DIC("S")="I $P($G(^AUTTTFAC(+Y,0)),U,2)=""""" D ^DIC K DIC S DIC=DIE,X=+Y K:Y<0 X
 Q
 ;
...
C38S S X="" G:DG(DQ)=X C38F1 K DB
 D ^DGPMX21
C38F1 Q
X38 S DIC("S")="I $P($G(^BDGWD(+Y,0)),U,3)'=""I""" D W^DGPMVDD I $D(X) D ^DIC K DIC S DIC=DIE,X=+Y K:Y<0 X I $D(X) D WARD^DGPMVDD K:$D(DGOOS) X K DGOOS
 Q
 ;
...
X41 I DGPMP=^DGPM(DA,0) S Y=""
 Q
42 D:$D(DG)>9 F^DIE17,DE S DQ=42,DW="USR;3",DV="RP200'a",DU="",DLB="LAST EDITED BY",DIFLD=102
 S DE(DW)="C42^DGPMX2"
 S DU="VA(200,"
 S X=DUZ
 S Y=X
