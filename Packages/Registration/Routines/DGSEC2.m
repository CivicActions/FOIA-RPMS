DGSEC2 ;ALB/RMO - Display User Access to Patient Record ; 22 JUN 87 1:00 pm
 ;;5.3;Registration;**391,1013,1015,1022**;MAY 28, 2004;Build 18
...
ASKUSR K DGUSR W !!,"Do you want to see when a select user accessed this record" S %=2 D YN^DICN G Q:%<0 S:%=2 DGUSR="ALL" I '% W !!,"Enter 'YES' to display a select user, or 'NO' to display all users." G ASKUSR
 ...
 ;
START  ;
 ;20230621 97824 maw p1022 replace DGNAM with PPN 
 ;U IO S DGX="",(DGCNT,DGPGE)=0 S:$D(^DPT(DFN,0)) DGNAM=$E($P(^(0),"^",1),1,30),DOB=$P(^(0),"^",3),SSN=$P(^(0),"^",9) D SELUSR:DGUSR,ALLUSR:DGUSR="ALL"
 U IO S DGX="",(DGCNT,DGPGE)=0 S:$D(^DPT(DFN,0)) DOB=$P(^(0),"^",3),SSN=$P(^(0),"^",9),DGNAM=$$GETPREF^AUPNSOGI(DFN,"E",1) D SELUSR:DGUSR,ALLUSR:DGUSR="ALL"
 ;
