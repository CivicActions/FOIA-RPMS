...
DE S DIE="^DGPT(D0,""M"",",DIC=DIE,DP=45.02,DL=2,DIEL=1,DU="" K DG,DE,DB Q:$O(^DGPT(D0,"M",DA,""))=""
 I $D(^(0)) S %Z=^(0) S %=$P(%Z,U,2) S:%]"" DE(5)=% S %=$P(%Z,U,3) S:%]"" DE(8)=% S %=$P(%Z,U,4) S:%]"" DE(11)=% S %=$P(%Z,U,5) S:%]"" DE(25)=% S %=$P(%Z,U,6) S:%]"" DE(29)=% S %=$P(%Z,U,7) S:%]"" DE(33)=% S %=$P(%Z,U,8) S:%]"" DE(37)=%
 I  S %=$P(%Z,U,9) S:%]"" DE(41)=% S %=$P(%Z,U,10) S:%]"" DE(2)=% S %=$P(%Z,U,18) S:%]"" DE(16)=%,DE(19)=%
 I $D(^(300)) S %Z=^(300) S %=$P(%Z,U,2) S:%]"" DE(48)=% S %=$P(%Z,U,3) S:%]"" DE(52)=%
 K %Z Q
 ;
W W !?DL+DL-2,DLB_": "
 Q
...
C2F1 S DIEZRXR(45.02,DIIENS)=$$OREF^DILF($NA(@$$CREF^DILF(DIE)))
 F DIXR=283,284,285,286,287,288,289,290,291,292 S DIEZRXR(45.02,DIXR)=""
 Q
...
C25F1 S DIEZRXR(45.02,DIIENS)=$$OREF^DILF($NA(@$$CREF^DILF(DIE)))
 F DIXR=283 S DIEZRXR(45.02,DIXR)=""
 Q
...
C29F1 S DIEZRXR(45.02,DIIENS)=$$OREF^DILF($NA(@$$CREF^DILF(DIE)))
 F DIXR=284 S DIEZRXR(45.02,DIXR)=""
 Q
...
X32 S DGNFLD="@50"
 Q
33 D:$D(DG)>9 F^DIE17,DE S DQ=33,DW="0;7",DV="*P80'X",DU="",DLB="ICD 3",DIFLD=7
 S DE(DW)="C33^DGX5F1",DE(DW,"INDEX")=1
 S DU="ICD9("
 G RE
...
C33S S X="" G:DG(DQ)=X C33F1 K DB
 ...
 S X=DG(DQ),DIC=DIE
 X "N X S X=""DGRUDD01"" X ^%ZOSF(""TEST"") Q:'$T  N DG1 S DG1=+$P(^DGPT(DA(1),0),""^"",1) D:(DG1>0) ADGRU^DGRUDD01(DG1)"
C33F1 S DIEZRXR(45.02,DIIENS)=$$OREF^DILF($NA(@$$CREF^DILF(DIE)))
 F DIXR=285 S DIEZRXR(45.02,DIXR)=""
 Q
X33 D ICDEN^DGPTF5 Q:X=""  K K S DIC("S")="S DGI=7 D EN^DGPTFJC I 'DGER" S DIC(0)=$P(DIC(0),"E")_$P(DIC(0),"E",2) D ^DIC K DIC S DIC=DIE,X=+Y K:Y<0 X
 Q
 ;
34 D:$D(DG)>9 F^DIE17,DE S Y=U,DQ=34 D X34 D:$D(DIEFIRE)#2 FIREREC^DIE17 G A:$D(Y)[0,A:Y=U S X=Y,DIC(0)="F",DW=DQ G OUT^DIE17
X34 I X K DGPTIT S DGNFLD="@50",Y="@800",DGPTIT(X_$C(59)_"ICD9(")=""
 Q
35 S DQ=36 ;@50
36 D:$D(DG)>9 F^DIE17,DE S Y=U,DQ=36 D X36 D:$D(DIEFIRE)#2 FIREREC^DIE17 G A:$D(Y)[0,A:Y=U S X=Y,DIC(0)="F",DW=DQ G OUT^DIE17
X36 S DGNFLD="@60"
 Q
37 D:$D(DG)>9 F^DIE17,DE S DQ=37,DW="0;8",DV="*P80'X",DU="",DLB="ICD 4",DIFLD=8
 S DE(DW)="C37^DGX5F1",DE(DW,"INDEX")=1
 S DU="ICD9("
 G RE
...
C37S S X="" G:DG(DQ)=X C37F1 K DB
 ...
 S X=DG(DQ),DIC=DIE
 X "N X S X=""DGRUDD01"" X ^%ZOSF(""TEST"") Q:'$T  N DG1 S DG1=+$P(^DGPT(DA(1),0),""^"",1) D:(DG1>0) ADGRU^DGRUDD01(DG1)"
C37F1 S DIEZRXR(45.02,DIIENS)=$$OREF^DILF($NA(@$$CREF^DILF(DIE)))
 F DIXR=286 S DIEZRXR(45.02,DIXR)=""
 Q
X37 D ICDEN^DGPTF5 Q:X=""  K K S DIC("S")="S DGI=8 D EN^DGPTFJC I 'DGER" S DIC(0)=$P(DIC(0),"E")_$P(DIC(0),"E",2) D ^DIC K DIC S DIC=DIE,X=+Y K:Y<0 X
 Q
 ;
38 D:$D(DG)>9 F^DIE17,DE S Y=U,DQ=38 D X38 D:$D(DIEFIRE)#2 FIREREC^DIE17 G A:$D(Y)[0,A:Y=U S X=Y,DIC(0)="F",DW=DQ G OUT^DIE17
X38 I X K DGPTIT S DGNFLD="@60",Y="@800",DGPTIT(X_$C(59)_"ICD9(")=""
 Q
39 S DQ=40 ;@60
40 D:$D(DG)>9 F^DIE17,DE S Y=U,DQ=40 D X40 D:$D(DIEFIRE)#2 FIREREC^DIE17 G A:$D(Y)[0,A:Y=U S X=Y,DIC(0)="F",DW=DQ G OUT^DIE17
X40 S DGNFLD="@70"
 Q
41 D:$D(DG)>9 F^DIE17,DE S DQ=41,DW="0;9",DV="*P80'X",DU="",DLB="ICD 5",DIFLD=9
 S DE(DW)="C41^DGX5F1",DE(DW,"INDEX")=1
 S DU="ICD9("
 G RE
...
X51 I '$D(DGBPC(3))!(DGDUP(3)) S Y="@830"
 Q
52 S DW="300;3",DV="SX",DU="",DLB="LEGIONNAIRE'S DISEASE",DIFLD=300.03
 S DU="1:Yes;2:No;"
 G RE
X52 S DGFLAG=3 D 501^DGPTSC01 K:DGER X K DGER,DGFLAG Q
 Q
 ;
