DGPMX3 ; GENERATED FROM 'DGPM DISCHARGE' INPUT TEMPLATE(#2128), FILE 405;02/09/12
...
BEGIN S DNM="DGPMX3",DQ=1
 ...
 M DIEZAR=^DIE(2128,"AR") S DICRREC="TRIG^DIE17"
 S:$D(DTIME)[0 DTIME=300 S D0=DA,DIIENS=DA_",",DIEZ=2128,U="^"
...
X8 I $P(^DGPM(DA,0),"^",18)'=10 S Y="@31"
 Q
9 D:$D(DG)>9 F^DIE17,DE S DQ=9,DW="0;5",DV="R*P9999999.91'",DU="",DLB="TRANSFER FACILITY",DIFLD=.05
 S DE(DW)="C9^DGPMX3"
 S DU="AUTTTFAC("
 G RE
...
C9S S X="" G:DG(DQ)=X C9F1 K DB
 ...
 I $P(^DGPM(DA,0),"^",2)=3 S A1B2TAG="ADM" D ^A1B2XFR
C9F1 Q
X9 S DIC("S")="I $P($G(^AUTTTFAC(+Y,0)),U,2)=""""" D ^DIC K DIC S DIC=DIE,X=+Y K:Y<0 X
 Q
 ;
...
X20 I DGPMP=^DGPM(DA,0) S Y=""
 Q
21 D:$D(DG)>9 F^DIE17,DE S DQ=21,DW="USR;3",DV="RP200'a",DU="",DLB="LAST EDITED BY",DIFLD=102
 S DE(DW)="C21^DGPMX3"
 S DU="VA(200,"
 S X=DUZ
 S Y=X
...
24 D:$D(DG)>9 F^DIE17,DE S DQ=24,DW="USR;4",DV="RD",DU="",DLB="LAST EDITED ON",DIFLD=103
