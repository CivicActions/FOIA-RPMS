...
DE S DIE="^DGPT(D0,""M"",",DIC=DIE,DP=45.02,DL=2,DIEL=1,DU="" K DG,DE,DB Q:$O(^DGPT(D0,"M",DA,""))=""
 I $D(^(0)) S %Z=^(0) S %=$P(%Z,U,3) S:%]"" DE(5)=% S %=$P(%Z,U,4) S:%]"" DE(8)=% S %=$P(%Z,U,5) S:%]"" DE(22)=% S %=$P(%Z,U,6) S:%]"" DE(27)=% S %=$P(%Z,U,7) S:%]"" DE(32)=% S %=$P(%Z,U,8) S:%]"" DE(37)=% S %=$P(%Z,U,9) S:%]"" DE(42)=%
 I  S %=$P(%Z,U,18) S:%]"" DE(10)=%,DE(13)=%
 I $D(^(300)) S %Z=^(300) S %=$P(%Z,U,2) S:%]"" DE(49)=% S %=$P(%Z,U,3) S:%]"" DE(53)=% S %=$P(%Z,U,4) S:%]"" DE(58)=%
 K %Z Q
 ;
W W !?DL+DL-2,DLB_": "
 Q
...
X31 S DGNFLD="@60"
 Q
32 D:$D(DG)>9 F^DIE17,DE S DQ=32,DW="0;7",DV="*P80'X",DU="",DLB="ICD 3",DIFLD=7
...
37 D:$D(DG)>9 F^DIE17,DE S DQ=37,DW="0;8",DV="*P80'X",DU="",DLB="ICD 4",DIFLD=8
 S DE(DW)="C37^DGPTX51",DE(DW,"INDEX")=1
 S DU="ICD9("
 G RE
...
X52 I '$D(DGBPC(3))!(DGDUP(3)) S Y="@820"
 Q
53 S DW="300;3",DV="SX",DU="",DLB="LEGIONNAIRE'S DISEASE",DIFLD=300.03
 S DU="1:Yes;2:No;"
 G RE
X53 S DGFLAG=3 D 501^DGPTSC01 K:DGER X K DGER,DGFLAG Q
 Q
 ;
...
X56 I '$D(DGBPC(4))!(DGDUP(4)) S Y="@830"
 Q
57 D:$D(DG)>9 F^DIE17,DE S Y=U,DQ=57 D X57 D:$D(DIEFIRE)#2 FIREREC^DIE17 G A:$D(Y)[0,A:Y=U S X=Y,DIC(0)="F",DW=DQ G OUT^DIE17
X57 D DRUG^DGPTSC01 I $D(DGTX) S Y="@825"
 Q
58 S DW="300;4",DV="P45.61'X",DU="",DLB="SUBSTANCE ABUSE",DIFLD=300.04
 S DU="DIC(45.61,"
 G RE
X58 S DGFLAG=4 D 501^DGPTSC01 K:DGER X K DGER,DGFLAG
 Q
 ;
