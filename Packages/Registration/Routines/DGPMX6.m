DGPMX6 ; GENERATED FROM 'DGPM SPECIALTY TRANSFER' INPUT TEMPLATE(#2452), FILE 405;02/09/12
...
BEGIN S DNM="DGPMX6",DQ=1
 ...
 M DIEZAR=^DIE(2452,"AR") S DICRREC="TRIG^DIE17"
 S:$D(DTIME)[0 DTIME=300 S D0=DA,DIIENS=DA_",",DIEZ=2452,U="^"
...
X22 S:DGPMP=^DGPM(DA,0) Y=""
 Q
23 S DW="USR;3",DV="RP200'a",DU="",DLB="LAST EDITED BY",DIFLD=102
 S DE(DW)="C23^DGPMX6"
 S DU="VA(200,"
 S X=DUZ
 S Y=X
...
C23S S X="" G:DG(DQ)=X C23F1 K DB
 I $D(DE(23))'[0!(^DD(DP,DIFLD,"AUDIT")'="e") S X=DG(DQ),DIIX=3_U_DIFLD D AUDIT^DIET
C23F1 Q
X23 Q
24 D:$D(DG)>9 F^DIE17,DE S Y=U,DQ=24 G A
25 D:$D(DG)>9 F^DIE17,DE S DQ=25,DW="USR;4",DV="RD",DU="",DLB="LAST EDITED ON",DIFLD=103
