AUM21051 ;IHS/SET/GTH - SCB UPDATE 2002 FEB 07 ; [ 02/08/2002   4:14 PM ]
...
SVARS ;;A,C,E,F,L,M,N,O,P,R,S,T,V;Single-character work variables.
 ...
 ;
 D RSLT($J("",5)_$P($T(UPDATE^AUM2105A),";",3))
 F L="GREET","INTROE","INTROI" F %=1:1 D RSLT($P($T(@L+%^AUM2105),";",3)) Q:$P($T(@L+%+1^AUM2105),";",3)="###"
 D DASH,LOCNEW,DASH,LOCMOD,DASH,COMMNEW,DASH,ICD9MOD,DASH
 Q
 ; -----------------------------------------------------
...
DIE NEW @($P($T(SVARS),";",3))
 ...
 Q
E(L) Q $P($P($T(@L^AUM2105A),";",3),":",1)
IEN(X,%,Y) ;
 S Y=$O(@(X_"""C"",%,0)"))
 I 'Y S Y=$$VAL^AUM2105M(X,%) I Y D  S:Y<0 Y=""
 . NEW %,@($P($T(SVARS),";",3))
 . S L=Y
...
MODOK D RSLT($J("",5)_"Changed : "_L)
 Q
RSLT(%) S ^(0)=$G(^TMP("AUM2105",$J,0))+1,^(^(0))=% D MES(%)
 Q
...
LOCNEW ;
 ...
 F T=1:1 S L=$T(LOCNEW+T^AUM2105A) Q:$P(L,";",3)="END"  D ADDLOC
 Q
ADDLOC ;
...
LOCMOD ;
 ...
 F T=1:2 S L=$T(LOCMOD+T^AUM2105A) Q:$P(L,";",3)="END"  S L("TO")=$T(LOCMOD+T+1^AUM2105A) D
 ...
 D DASH,RSLT($$LOCMOD^AUMXPORT("AUM2105A")_" patients marked for export because of the Location Code changes.")
 Q
 ; -----------------------------------------------------
COMMNEW ;
 D RSLT($$E("COMMNEW"))
 ...
 F T=1:1 S L=$T(COMMNEW+T^AUM2105A) Q:$P(L,";",3)="END"  D ADDCOMM
 Q
 ;
ADDCOMM ;
...
ICD9MOD ;
 D RSLT($$E("ICD9MOD"))
 D RSLT($$RJ^XLFSTR("CODE  USE ONLY WITH SEX  MAJOR DIAGNOSTIC CATEGORY",58))
 D RSLT($$RJ^XLFSTR("----  -----------------  -------------------------",58))
 F T=1:2 S L=$T(ICD9MOD+T^AUM2105A) Q:$P(L,";",3)="END"  S L=$T(ICD9MOD+T+1^AUM2105A) D
 . S L=$P(L,U,2,99),A=$P(L,U),S=$P(L,U,2),F=$P(L,U,3)
 . S DA=$O(^ICD9("AB",A,0))
 . I 'DA D RSLT(" CODE '"_A_"' is not in your ICD DIAGNOSIS file.") Q
 . S DR="9.5///"_S_";5///"_F_";2100000///"_DT,DIE="^ICD9("
 . D DIE
 . I $D(Y) D RSLT("ERROR:  Edit of fields for CODE '"_A_"' FAILED.") Q
 . D RSLT($J("",8)_A_$J("",20)_F)
 .Q
 Q
 ; -----------------------------------------------------
 ;
