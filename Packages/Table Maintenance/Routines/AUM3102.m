AUM3102 ;IHS/SET/GTH - SCB UPDATE 2002 OCT 04 ; [ 10/11/2002  12:51 PM ]
 ...
 D HELP^XBHELP("INTROE","AUM3102")
 ;
 I $G(XPDENV)=1 D
 . S (XPDDIQ("XPZ1"),XPDDIQ("XPZ2"))=0
 . D HELP^XBHELP("INTROI","AUM3102")
 .Q
 ;
...
POST ;EP - From KIDS.
 ...
 KILL ^TMP("AUM3102",$J)
 D AUDS,START^AUM31021,AUDR
 S XMSUB=$P($P($T(+1),";",2)," ",3,99),XMDUZ=$G(DUZ,.5),XMTEXT="^TMP(""AUM3102"",$J,",XMY(1)="",XMY(DUZ)=""
 ...
 Q:'$L($T(+1^AUMDELR))
 S ZTRTN="DEL^AUMDELR(""AUM3102"")",ZTDESC="Delete routines in the 'AUM3102' namespace.",ZTDTH=$$HADD^XLFDT($H,0,0,30,0),ZTIO="",ZTPRI=1
 D ^%ZTLOAD
...
AUDS ; Save current settings, and SET data auditing 'on'.
 S ^XTMP("AUM3102",0)=$$FMADD^XLFDT(DT,56)_"^"_DT_"^"_"2002 OCT 04 STANDARD TABLE UPDATES"
 NEW G,P
 F %=1:1 S G=$P($T(AUD+%),";",3) Q:G="END"  D
 . S P=$P(@(G_"0)"),"^",2)
 . I '$D(^XTMP("AUM3102",G)) S ^XTMP("AUM3102",G)=P
 . S:'(P["a") $P(@(G_"0)"),"^",2)=P_"a"
 . D AUDF(+P)
 .Q
 Q
AUDF(F) ; Process all fields for file F, including sub-files.
 ...
 . I '$D(^XTMP("AUM3102",G)) S ^XTMP("AUM3102",G)=P
 . I '$D(^XTMP("AUM3102",G,"AUDIT")) S ^XTMP("AUM3102",G,"AUDIT")=$G(@(G_"""AUDIT"")"))
 . S:'(P["a") $P(@(G_"0)"),"^",2)=P_"a"
 . S ^DD(F,D,"AUDIT")="y"
 .Q
 Q
...
AUDR ; Restore the file data audit values to their original values.
 NEW G,P
 S G=0
 F  S G=$O(^XTMP("AUM3102",G)) Q:'$L(G)  D
 . S $P(@(G_"0)"),"^",2)=^XTMP("AUM3102",G)
 . Q:'(G["^DD(")
 . S (@(G_"""AUDIT"")"))=^XTMP("AUM3102",G,"AUDIT")
 . K:@(G_"""AUDIT"")")="" @(G_"""AUDIT"")")
 .Q
 Q
 ; -----------------------------------------------------
...
INTROE ; Intro text during KIDS Environment check.
 ;;This distribution:
 ;;(1) Implements SCB mods for 04 Oct 2002,
 ;;(2) Corrects spelling of one entry in CLINIC STOP.
 ;;###
 ;
...
GREET ;;EP - To add to mail message.
 ...
 ;;"hqwhd@mail.ihs.gov".  Please refer to patch "AUM*3.1*2".
 ;;  
