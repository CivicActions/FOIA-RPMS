AUM21021 ; IHS/ASDST/GTH - STANDARD TABLE UPDATES, 2001 NOV 02 ; [ 11/15/2001  12:00 PM ]
 ;;02.1;TABLE MAINTENANCE;**2**;SEP 28,2001
 ;
START ;EP
 ;
 NEW A,C,DIC,DIE,DINUM,DLAYGO,DR,E,F,L,M,N,O,P,R,S,T
 ;
 D RSLT($J("",5)_$P($T(UPDATE^AUM2102A),";",3))
 F L="GREET","INTROE","INTROI" F %=1:1 D RSLT($P($T(@L+%^AUM2102),";",3)) Q:$P($T(@L+%+1^AUM2102),";",3)="###"
 D DASH,LOCNEW,DASH,LOCMOD,DASH,LOCINACT,DASH,COMMNEW,DASH,COMMMOD,DASH,COMMDEL,DASH,TRIBMOD,DASH
 Q
 ;
 ; -----------------------------------------------------
...
IEN(X,%,Y) ;
 ...
 Q +Y
DIK NEW A,C,E,F,L,M,N,O,P,R,S,T D ^DIK KILL DIK
 Q
FILE NEW A,C,E,F,L,M,N,O,P,R,S,T K DD,DO S DIC(0)="L" D FILE^DICN KILL DIC
 Q
M(%) Q $S(%=0:"ERROR : ",%=1:"NOT ADDED : ",1:"")
MODOK D RSLT($J("",5)_"Changed : "_L)
 Q
RSLT(%) S ^(0)=$G(^TMP("AUM2102",$J,0))+1,^(^(0))=% D MES(%)
 Q
MES(%) NEW A,C,E,F,L,M,N,O,P,R,S,T D MES^XPDUTL(%)
 Q
 ; -----------------------------------------------------
...
LOCNEW ;
 ...
 F T=1:1 S L=$T(LOCNEW+T^AUM2102A) Q:$P(L,";",3)="END"  D ADDLOC
 Q
ADDLOC ;
...
LOCMOD ;
 ...
 F T=1:2 S L=$T(LOCMOD+T^AUM2102A) Q:$P(L,";",3)="END"  S L("TO")=$T(LOCMOD+T+1^AUM2102A) D
 ...
 D DASH,RSLT($$LOCMOD^AUMXPORT("AUM2102A")_" patients marked for export because of the Location Code changes.")
 Q
 ; -----------------------------------------------------
LOCINACT ;
 S E=$$E("LOCINACT")
 D RSLT(E)
 D RSLT($$RJ^XLFSTR("AA SU FA NAME",33))
 D RSLT($$RJ^XLFSTR("-- -- -- ----",33))
 F T=1:1 S L=$T(LOCINACT+T^AUM2102A) Q:$P(L,";",3)="END"  D
 . S L=$P(L,";;",2),A=$P(L,U),S=$P(L,U,2),F=$P(L,U,3),N=$P(L,U,4),P=$P(L,U,5)
 . S L=A_" "_S_" "_F_" "_N_" "_P
 . S %=A_S_F,%=$O(^AUTTLOC("C",%,0))
 . I '% D RSLT($J("",5)_"ASUFAC "_A_S_F_" not found (OK).") Q
 . S DIE="^AUTTLOC(",DA=%,DR=".27////"_DT_";.28////"_DT
 . D DIE
...
COMMNEW ;
 ...
 F T=1:1 S L=$T(COMMNEW+T^AUM2102A) Q:$P(L,";",3)="END"  D ADDCOMM
 Q
 ;
ADDCOMM ;
...
COMMMOD ;
 D RSLT($$E("COMMMOD"))
 D RSLT($$RJ^XLFSTR("ST CT COM NAME",29)_$$RJ^XLFSTR("AA SU",33))
 D RSLT($$RJ^XLFSTR("-- -- --- ----",29)_$$RJ^XLFSTR("-- --",33))
 F T=1:2 S L=$T(COMMMOD+T^AUM2102A) Q:$P(L,";",3)="END"  S L("TO")=$T(COMMMOD+T+1^AUM2102A) D
 ...
 D DASH,RSLT($$COMMMOD^AUMXPORT("AUM2102A")_" patients marked for export because of the Community Code changes.")
 Q
 ; -----------------------------------------------------
COMMDEL ;
 D RSLT($$E("COMMDEL"))
 D RSLT($$RJ^XLFSTR("ST CT COM NAME",29)_$$RJ^XLFSTR("AA SU",33))
 D RSLT($$RJ^XLFSTR("-- -- --- ----",29)_$$RJ^XLFSTR("-- --",33))
 F T=1:1 S L=$T(COMMDEL+T^AUM2102A) Q:$P(L,";",3)="END"  D
 . S L=$P(L,";;",2),S=$P(L,U),O=$P(L,U,2),C=$P(L,U,3),N=$P(L,U,4),A=$P(L,U,5),V=$P(L,U,6),L=S_" "_O_" "_C_" "_N_" "_A_" "_V
 . S P=$O(^AUTTCOM("C",S_O_C,0))
 . I 'P D RSLT("COMMUNITY NOT FOUND (That's OK) => "_L) Q
 . S DIE="^AUTTCOM(",DA=P,DR=".14///<INACTIVE>;.18////"_DT
 . D DIE
 . I $D(Y) D RSLT(E(0)_" : EDIT FAILED => "_L) Q
 . D RSLT("SHORT NAME Marked '<INACTIVE>' :")
 . D RSLT($J("",15)_S_" "_O_" "_C_" "_N_$J("",32-$L(N))_A_" "_V)
 .Q
 Q
 ; -----------------------------------------------------
TRIBNEW ;
 ...
 F T=1:1 S L=$T(TRIBNEW+T^AUM2102A) Q:$P(L,";",3)="END"  D ADDTRIB
 Q
 ;
ADDTRIB ;
...
TRIBMOD ;
 ...
 F T=1:2 S L=$T(TRIBMOD+T^AUM2102A) Q:$P(L,";",3)="END"  S L("TO")=$T(TRIBMOD+T+1^AUM2102A) D
