AUM21091 ;IHS/SET/GTH - SCB UPDATE 2002 AUG 01 ; [ 08/13/2002   9:03 AM ]
...
SVARS ;;A,C,E,F,L,M,N,O,P,R,S,T,V;Single-character work variables.
 ...
 ;
 D RSLT($J("",5)_$P($T(UPDATE^AUM2109A),";",3))
 F L="GREET","INTROE","INTROI" F %=1:1 D RSLT($P($T(@L+%^AUM2109),";",3)) Q:$P($T(@L+%+1^AUM2109),";",3)="###"
 D DASH,LOCNEW,DASH,LOCMOD,DASH,LOCINACT,DASH,COMMNEW,DASH,CLINNEW,DASH,ICD9MOD,DASH,CLINMOD,DASH
 Q
 ; -----------------------------------------------------
...
DIE NEW @($P($T(SVARS),";",3))
 ...
 Q
E(L) Q $P($P($T(@L^AUM2109A),";",3),":",1)
IEN(X,%,Y) ;
 S Y=$O(@(X_"""C"",%,0)"))
 I 'Y S Y=$$VAL^AUM2109M(X,%) I Y D  S:Y<0 Y=""
 . NEW %,@($P($T(SVARS),";",3))
 . S L=Y
...
MODOK D RSLT($J("",5)_"Changed : "_L)
 Q
RSLT(%) S ^(0)=$G(^TMP("AUM2109",$J,0))+1,^(^(0))=% D MES(%)
 Q
...
LOCNEW ;
 ...
 F T=1:1 S L=$T(LOCNEW+T^AUM2109A) Q:$P(L,";",3)="END"  D ADDLOC
 Q
ADDLOC ;
...
LOCMOD ;
 ...
 F T=1:2 S L=$T(LOCMOD+T^AUM2109A) Q:$P(L,";",3)="END"  S L("TO")=$T(LOCMOD+T+1^AUM2109A) D
 ...
 D DASH,RSLT($$LOCMOD^AUMXPORT("AUM2109A")_" patients marked for export because of the Location Code changes.")
 Q
 ; -----------------------------------------------------
LOCINACT ;
 D RSLT($$E("LOCINACT"))
 D RSLT($$RJ^XLFSTR("AA SU FA NAME",26)_$$RJ^XLFSTR("PSEUDO",34))
 D RSLT($$RJ^XLFSTR("-- -- -- ----",26)_$$RJ^XLFSTR("------",34))
 F T=1:1 S L=$T(LOCINACT+T^AUM2109A) Q:$P(L,";",3)="END"  D
 . S L=$P(L,";;",2),A=$P(L,U),S=$P(L,U,2),F=$P(L,U,3),N=$P(L,U,4),P=$P(L,U,5)
 . S L=A_" "_S_" "_F_" "_N_$J("",32-$L(N))_P
 . S %=A_S_F,%=$O(^AUTTLOC("C",%,0))
...
COMMNEW ;
 ...
 F T=1:1 S L=$T(COMMNEW+T^AUM2109A) Q:$P(L,";",3)="END"  D ADDCOMM
 Q
 ;
ADDCOMM ;
...
CLINNEW ;
 ...
 F T=1:1 S L=$T(CLINNEW+T^AUM2109A) Q:$P(L,";",3)="END"  D ADDCLIN
 KILL DLAYGO
 Q
 ;
ADDCLIN ;
...
ICD9MOD ;
 ...
 NEW AUMI,DA,DIE,DR
 F AUMI=1:1 S L=$P($T(ICD9MOD+AUMI^AUM2109A),";;",2) Q:L="END"  D
 . S C=$P(L,U,1),Y=$$IXDIC("^ICD9(","ILX","AB",C,80)
 . I Y=-1 D RSLT("ERROR:  Lookup/Add of CODE '"_C_"' FAILED.") Q
 . S DA=+Y,DR="3///"_$E($P(L,U,2),1,30)_";5///"_$P(L,U,3)_";10///"_$E($P(L,U,4),1,245)_";100///@;102///@;2100000///"_DT,DIE="^ICD9("
 . D DIE
...
CLINMOD ;
 ...
 F T=1:1 S L=$T(CLINMOD+T^AUM2109A) Q:$P(L,";",3)="END"  D
 . S C=$P($P(L,";;",2),U,1)
 . I '$D(^DIC(40.7,"C",C)) D ADDCLIN Q
 . S L=$P(L,";;",2),C=$P(L,U),N=$P(L,U,2),A=$P(L,U,3),P=$P(L,U,5),L=C_" "_N_$J("",(32-$L(N)))_$$LJ^XLFSTR(A,8)_P
 . S DA=$O(^DIC(40.7,"C",C,0)),DIE="^DIC(40.7,",DR=".01///"_N_";999999901///"_A_";90000.01///"_P
 . D DIE
