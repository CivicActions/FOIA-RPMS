AUM21041 ;IHS/ASDST/GTH - SCB UPDATE 2001 DEC 14 ; [ 01/29/2002   4:01 PM ]
...
SVARS ;;A,C,E,F,L,M,N,O,P,R,S,T,V;Single-character work variables.
 ...
 ;
 D RSLT($J("",5)_$P($T(UPDATE^AUM2104A),";",3))
 F L="GREET","INTROE","INTROI" F %=1:1 D RSLT($P($T(@L+%^AUM2104),";",3)) Q:$P($T(@L+%+1^AUM2104),";",3)="###"
 D DASH,COMMMOD,DASH,CLINNEW,DASH,LOCMOD,DASH
 Q
 ; -----------------------------------------------------
...
DIE NEW @($P($T(SVARS),";",3))
 ...
 Q
E(L) Q $P($P($T(@L^AUM2104A),";",3),":",1)
IEN(X,%,Y) ;
 S Y=$O(@(X_"""C"",%,0)"))
 I 'Y S Y=$$VAL^AUM2104M(X,%) I Y D  S:Y<0 Y=""
 . NEW %,@($P($T(SVARS),";",3))
 . S L=Y
...
LOCNEW ;
 ...
 F T=1:1 S L=$T(LOCNEW+T^AUM2104A) Q:$P(L,";",3)="END"  D ADDLOC
 Q
ADDLOC ;
...
LOCMOD ;
 ...
 F T=1:2 S L=$T(LOCMOD+T^AUM2104A) Q:$P(L,";",3)="END"  S L("TO")=$T(LOCMOD+T+1^AUM2104A) D
 ...
 D DASH,RSLT($$LOCMOD^AUMXPORT("AUM2104A")_" patients marked for export because of the Location Code changes.")
 Q
 ; -----------------------------------------------------
CLINNEW ;
 D RSLT($$E("CLINNEW"))
 ...
 F T=1:1 S L=$T(CLINNEW+T^AUM2104A) Q:$P(L,";",3)="END"  D ADDCLIN
 KILL DLAYGO
 Q
 ;
ADDCLIN ;
...
COMMMOD ;
 ...
 F T=1:2 S L=$T(COMMMOD+T^AUM2104A) Q:$P(L,";",3)="END"  S L("TO")=$T(COMMMOD+T+1^AUM2104A) D
 ...
 D DASH,RSLT($$COMMMOD^AUMXPORT("AUM2104A")_" patients marked for export because of the Community Code changes.")
 Q
 ; -----------------------------------------------------
 ;
ADDCOMM ;
