AUM21011 ; IHS/ASDST/GTH - STANDARD TABLE UPDATES, 2001 SEP 28 ; [ 10/31/2001   3:40 PM ]
...
START ;EP
 ...
 ;
 D RSLT($J("",5)_$P($T(UPDATE^AUM2101A),";",3))
 F L="GREET","INTROE","INTROI" F %=1:1 D RSLT($P($T(@L+%^AUM2101),";",3)) Q:$P($T(@L+%+1^AUM2101),";",3)="###"
 D DASH,LOCNEW,DASH,LOCMOD,DASH,STATENEW,DASH,COMMNEW,DASH,COMMMOD,DASH
 Q
 ;
 ; -----------------------------------------------------
...
IEN(X,%,Y) ;
 S Y=$O(@(X_"""C"",%,0)"))
 I 'Y S Y=$$VAL^AUM2101M(X,%) I Y D  S:Y<0 Y=""
...
LOCNEW ;
 ...
 F T=1:1 S L=$T(LOCNEW+T^AUM2101A) Q:$P(L,";",3)="END"  D ADDLOC
 Q
ADDLOC ;
...
LOCMOD ;
 ...
 F T=1:2 S L=$T(LOCMOD+T^AUM2101A) Q:$P(L,";",3)="END"  S L("TO")=$T(LOCMOD+T+1^AUM2101A) D
 ...
 D DASH,RSLT($$LOCMOD^AUMXPORT("AUM2101A")_" patients marked for export because of the Location Code changes.")
 Q
 ; -----------------------------------------------------
COMMNEW ;
 D RSLT($$E("COMMNEW"))
 ...
 D RSLT($J("",13)_"-- -- --- ----"_$J("",28)_"-- --")
 F T=1:1 S L=$T(COMMNEW+T^AUM2101A) Q:$P(L,";",3)="END"  D ADDCOMM
 Q
 ;
ADDCOMM ;
...
COMMMOD ;
 ...
 D RSLT($J("",15)_"-- -- --- ----"_$J("",28)_"-- --")
 F T=1:2 S L=$T(COMMMOD+T^AUM2101A) Q:$P(L,";",3)="END"  S L("TO")=$T(COMMMOD+T+1^AUM2101A) D
 ...
 D DASH,RSLT($$COMMMOD^AUMXPORT("AUM2101A")_" patients marked for export because of the Community Code changes.")
 Q
 ; -----------------------------------------------------
STATENEW ;
 D RSLT($$E("STATENEW"))
 ...
 F T=1:1 S L=$T(STATENEW+T^AUM2101A) Q:$P(L,";",3)="END"  D ADDSTATE
 Q
 ;
ADDSTATE ;
