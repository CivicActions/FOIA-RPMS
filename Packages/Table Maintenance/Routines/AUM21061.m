AUM21061 ;IHS/SET/GTH - SCB UPDATE 2002 APR 02 ; [ 04/05/2002  11:37 AM ]
...
SVARS ;;A,C,E,F,L,M,N,O,P,R,S,T,V;Single-character work variables.
 ...
 D DASH,SUNEW,DASH,SUMOD,DASH,LOCNEW,DASH,LOCMOD,DASH
 Q
 ; -----------------------------------------------------
...
IEN(X,%,Y) ;
 S Y=$O(@(X_"""C"",%,0)"))
 I 'Y S Y=$$VAL^AUM2106M(X,%) I Y D  S:Y<0 Y=""
 . NEW %,@($P($T(SVARS),";",3))
 . S L=Y
...
SUNEW ;
 ...
 D RSLT($J("",13)_"-- -- ----")
 F T=1:1 S L=$T(SUNEW+T^AUM2106A) Q:$P(L,";",3)="END"  D ADDSU
 Q
 ; -----------------------------------------------------
...
SUMOD ;
 ...
 D RSLT($J("",15)_"-- -- ----")
 F T=1:2 S L=$T(SUMOD+T^AUM2106A) Q:$P(L,";",3)="END"  S L("TO")=$T(SUMOD+T+1^AUM2106A) D
...
LOCNEW ;
 ...
 F T=1:1 S L=$T(LOCNEW+T^AUM2106A) Q:$P(L,";",3)="END"  D ADDLOC
 Q
ADDLOC ;
...
LOCMOD ;
 ...
 F T=1:2 S L=$T(LOCMOD+T^AUM2106A) Q:$P(L,";",3)="END"  S L("TO")=$T(LOCMOD+T+1^AUM2106A) D
 ...
 D DASH,RSLT($$LOCMOD^AUMXPORT("AUM2106A")_" patients marked for export because of the Location Code changes.")
 Q
 ; -----------------------------------------------------
COMMNEW ;
 D RSLT($$E("COMMNEW"))
 ...
 F T=1:1 S L=$T(COMMNEW+T^AUM2106A) Q:$P(L,";",3)="END"  D ADDCOMM
 Q
 ;
ADDCOMM ;
