AUM31031 ;IHS/SET/GTH - SCB UPDATE 2002 NOV 07 ; [ 11/15/2002   7:56 AM ]
...
SVARS ;;A,C,E,F,L,M,N,O,P,R,S,T,V;Single-character work variables.
 ...
 ;
 D RSLT($J("",5)_$P($T(UPDATE^AUM3103A),";",3))
 F L="GREET","INTROE","INTROI" F %=1:1 D RSLT($P($T(@L+%^AUM3103),";",3)) Q:$P($T(@L+%+1^AUM3103),";",3)="###"
 D DASH,SUMOD,DASH,LOCMOD,DASH,COMMNEW,DASH,COMMMOD,DASH,CLINMOD,DASH
 Q
 ; -----------------------------------------------------
...
DIE NEW @($P($T(SVARS),";",3))
 ...
 Q
E(L) Q $P($P($T(@L^AUM3103A),";",3),":",1)
IEN(X,%,Y) ;
 S Y=$O(@(X_"""C"",%,0)"))
 I 'Y S Y=$$VAL^AUM3103M(X,%) I Y D  S:Y<0 Y=""
 . NEW %,@($P($T(SVARS),";",3))
 . S L=Y
...
MODOK D RSLT($J("",5)_"Changed : "_L)
 Q
RSLT(%) S ^(0)=$G(^TMP("AUM3103",$J,0))+1,^(^(0))=% D MES(%)
 Q
...
SUNEW ;
 ...
 F T=1:1 S L=$T(SUNEW+T^AUM3103A) Q:$P(L,";",3)="END"  D ADDSU
 Q
 ; -----------------------------------------------------
...
SUMOD ;
 ...
 D RSLT($J("",15)_"-- -- ----")
 F T=1:2 S L=$T(SUMOD+T^AUM3103A) Q:$P(L,";",3)="END"  S L("TO")=$T(SUMOD+T+1^AUM3103A) D
...
LOCNEW ;
 ...
 F T=1:1 S L=$T(LOCNEW+T^AUM3103A) Q:$P(L,";",3)="END"  D ADDLOC
 Q
ADDLOC ;
...
LOCMOD ;
 ...
 F T=1:2 S L=$T(LOCMOD+T^AUM3103A) Q:$P(L,";",3)="END"  S L("TO")=$T(LOCMOD+T+1^AUM3103A) D
 ...
 D DASH,RSLT($$LOCMOD^AUMXPORT("AUM3103A")_" patients marked for export because of the Location Code changes.")
 Q
 ; -----------------------------------------------------
COMMNEW ;
 D RSLT($$E("COMMNEW"))
 ...
 F T=1:1 S L=$T(COMMNEW+T^AUM3103A) Q:$P(L,";",3)="END"  D ADDCOMM
 Q
 ;
ADDCOMM ;
...
COMMMOD ;
 ...
 F T=1:2 S L=$T(COMMMOD+T^AUM3103A) Q:$P(L,";",3)="END"  S L("TO")=$T(COMMMOD+T+1^AUM3103A) D
 ...
 D DASH,RSLT($$COMMMOD^AUMXPORT("AUM3103A")_" patients marked for export because of the Community Code changes.")
 Q
 ; -----------------------------------------------------
ADDCLIN ;
...
CLINMOD ;
 ...
 F T=1:1 S L=$T(CLINMOD+T^AUM3103A) Q:$P(L,";",3)="END"  D
 . S C=$P($P(L,";;",2),U,2)
