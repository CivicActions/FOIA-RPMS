AUM2107 ;IHS/SET/GTH - SCB UPDATE 2002 MAY 10 ; [ 05/21/2002  11:27 AM ]
 ...
 D HELP^XBHELP("INTROE","AUM2107")
 ;
 I $G(XPDENV)=1 D
 . ; The following line prevents the "Disable Options..." and "Move
 . ; Routines..." questions from being asked during the install.
 . S (XPDDIQ("XPZ1"),XPDDIQ("XPZ2"))=0
 . D HELP^XBHELP("INTROI","AUM2107")
 .Q
 ;
...
POST ;EP - From KIDS.
 ...
 KILL ^TMP("AUM2107",$J)
 D AUDS,START^AUM21071,START^AUM21072,AUDR
 S XMSUB=$P($P($T(+1),";",2)," ",3,99),XMDUZ=$G(DUZ,.5),XMTEXT="^TMP(""AUM2107"",$J,",XMY(1)="",XMY(DUZ)=""
 ...
 Q:'$L($T(+1^AUMDELR))
 S ZTRTN="DEL^AUMDELR(""AUM2107"")",ZTDESC="Delete routines in the 'AUM2107' namespace.",ZTDTH=$$HADD^XLFDT($H,0,0,30,0),ZTIO="",ZTPRI=1
 D ^%ZTLOAD
...
AUDS ; Save current settings, and SET data auditing 'on'.
 S ^XTMP("AUM2107",0)=$$FMADD^XLFDT(DT,56)_"^"_DT_"^"_"2002 MAY 10 STANDARD TABLE UPDATES"
 NEW G,P
 F %=1:1 S G=$P($T(AUD+%),";",3) Q:G="END"  D
 . S P=$P(@(G_"0)"),"^",2)
 . I '$D(^XTMP("AUM2107",G)) S ^XTMP("AUM2107",G)=P
 . S:'(P["a") $P(@(G_"0)"),"^",2)=P_"a"
 . Q:'(G["^DD(")
 . I '$D(^XTMP("AUM2107",G,"AUDIT")) S ^XTMP("AUM2107",G,"AUDIT")=$G(@(G_"""AUDIT"")"))
 . S (@(G_"""AUDIT"")"))="y"
 .Q
 Q
 ; -----------------------------------------------------
AUDR ; Restore the file data audit values to their original values.
 NEW G,P
 F %=1:1 S G=$P($T(AUD+%),";",3) Q:G="END"  D
 . S $P(@(G_"0)"),"^",2)=^XTMP("AUM2107",G)
 . Q:'(G["^DD(")
 . S (@(G_"""AUDIT"")"))=^XTMP("AUM2107",G,"AUDIT")
 . K:@(G_"""AUDIT"")")="" @(G_"""AUDIT"")")
 .Q
 Q
 ; -----------------------------------------------------
...
INTROE ; Intro text during KIDS Environment check.
 ...
 ;;(2) 4 new HEALTH FACTORS approved by CIMTAC,
 ;;(3) "Primary Care Clinic" defined in CLINIC STOP file and populated.
 ;;###
 ;
...
GREET ;;EP - To add to mail message.
 ...
 ;;"hqwhd@mail.ihs.gov".  Please refer to patch "AUM*2.1*7".
 ;;  
