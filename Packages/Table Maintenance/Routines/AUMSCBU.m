AUMSCBU ;IHS/OIT/NKD - SCB UPDATE - UTILITY 12/07/2012 ;
 ...
 ; 08/28/19 - Screen inactive entries when matching State codes
 ; 11/25/19 - Modified Inactivate processing for Health Factors
 ; 05/18/20 - Added Race/Ethnicity Tables
 ; AUM*21.0*2 - Changes related to display processing
 ; AUM*21.0*3 - Added Admission Source Pre
 ;            - Modified Ina/Post processing for Patient Status Code
 ; AUM*24.0*5 - Added Pronouns Pre processing
...
STPRE ; EP - STATE - LOOKUP
 ...
 I 'AUMI  D
 . K AUMR
 . D FIND^DIC(5,"","@;.01","PX",P3,,"C","I $P(^(0),U,3)=P3",,"AUMR") ; IHS/OIT/NKD AUM*19.0*4 - SCREEN INACTIVE ENTRIES
 . D FIND^DIC(5,"","@;.01","PX",P3,,"C","I $P(^(0),U,3)=P3,$P($G(^(9999999.01)),U)",,"AUMR") ; SCREEN TO REMOVE ENTRIES THAT DON'T MATCH
 . S CNT=$P($G(AUMR("DILIST",0)),U,1)
 . S AUMI=$S(CNT=1:$P($G(AUMR("DILIST",1,0)),U,1),CNT>1:$P($G(AUMR("DILIST",$P($G(AUMR("DILIST",0)),U,1),0)),U,1),1:"")
 Q
HFPRE ; EP - HEALTH FACTOR - INPUT TRANSFORMS
 N CNT,AUMR
 S P7=$S(P7]"":P7-17000000,1:"") ; IHS/OIT/NKD AUM*20.0*1 - MODIFIED INACTIVE PROCESSING
 S:P7]"" AUMA="INA",INA=1 ; IHS/OIT/NKD AUM*20.0*1 - MODIFIED INACTIVE PROCESSING
...
PSCPRE ; EP - PATIENT STATUS CODE - INPUT TRANSFORMS
 ...
 S:P7]"" AUMA="INA",INA=1  ; IHS/OIT/NKD AUM*21.0*3 - INACTIVATE PROCESSING
 Q
...
REVPRE ; EP - REVENUE CODES - CREATE REVENUE CODES ENTRY
 ...
 S:P7 AUMA="INA",INA=1
 Q
 ; IHS/OIT/NKD AUM*20.0*3 - ADDED RACE PRE
RACEPRE ; EP - RACE - INPUT TRANSFORMS
 S P7=$S(P7]"":P7-17000000,1:"")
 S:P7]"" AUMA="INA",INA=1
 Q
 ; IHS/OIT/NKD AUM*20.0*3 - ADDED ETHNICITY PRE
ETHNPRE ; EP - ETHNICITY - INPUT TRANSFORMS
 S P7=$S(P7]"":P7-17000000,1:"")
 S:P7]"" AUMA="INA",INA=1
 Q
 ; IHS/OIT/NKD AUM*21.0*3 - ADDED ADMISSION SOURCE PRE
ASRCPRE ; EP - AREA - INPUT TRANSFORMS
 S P7=$S(P7]"":P7-17000000,1:"")
 S:P7]"" AUMA="INA",INA=1
 Q
 ; IHS/OIT/NKD AUM*24.0*5 - ADDED PRONOUNS PRE
PRNPRE ; EP - PRONOUNS - INPUT TRANSFORMS
 N CNT
 F CNT=1:1:5 S @("P"_CNT_"A")=$P(P3,",",CNT)
 S P7=$S(P7]"":P7-17000000,1:"")
 S:P7]"" AUMA="INA",INA=1
 Q
 ; IHS/OIT/NKD AUM*25.0*4 - ADDED PCLAS PRE
PCLASPRE ; EP - PROVIDER CLASS - INPUT TRANSFORMS
 S P7=$S(P7]"":P7-17000000,1:"")
 S:P7]"" AUMA="INA",INA=1
 Q
 ; IHS/OIT/NKD AUM*25.0*4 - ADDED IND PRE
INDPRE ; EP - PATIENT INDUSTRY - INPUT TRANSFORMS
 N CNT
 S P7=$S(P7]"":P7-17000000,1:"")
 S:P7]"" AUMA="INA",INA=1
 I '$L(P3) D ERR^AUMSCBD("Invalid EXT CODE: "_P3) Q
 I '$L(P4) D ERR^AUMSCBD("Invalid EXT CODE OID: "_P4) Q
 S CNT=0 F  S CNT=$O(^AUTTIND("C",P3,CNT)) Q:'CNT!AUMI  D
 . Q:$P($G(^AUTTIND(CNT,.8)),U,2)'=P4
 . S AUMI=CNT
 Q
 ; IHS/OIT/NKD AUM*25.0*4 - ADDED OCC PRE
OCCPRE ; EP - PATIENT OCCUPATION - INPUT TRANSFORMS
 N CNT
 S P7=$S(P7]"":P7-17000000,1:"")
 S:P7]"" AUMA="INA",INA=1
 I '$L(P3) D ERR^AUMSCBD("Invalid EXT CODE: "_P3) Q
 I '$L(P4) D ERR^AUMSCBD("Invalid EXT CODE OID: "_P4) Q
 S CNT=0 F  S CNT=$O(^AUTTOCC("C",P3,CNT)) Q:'CNT!AUMI  D
 . Q:$P($G(^AUTTOCC(CNT,.8)),U,2)'=P4
 . S AUMI=CNT
 Q
 ; CUSTOM NEW-ROUTINES
...
COMPOST ; EP - COMMUNITY - UPDATE PATIENT FILE CURRENT COMMUNITY IF NAME CHANGE
 ...
 ;I $D(AUMD("DSP")) D DISP^AUMSCBD  ; IHS/OIT/NKD AUM*21.0*2
...
LOCPOST ; EP - LOCATION - UPDATE INSTITUTION NAME
 ...
 I $D(@AUMD@("DSP")) D DISP^AUMSCBD
 Q
 ; IHS/OIT/NKD AUM*21.0*3 - REMOVED POST - START OLD CODE
 ...
 ;Q
 ; IHS/OIT/NKD AUM*21.0*3 - END OLD CODE
