AUM31041 ;IHS/SET/GTH - SCB UPDATE 2002 DEC 02 ; [ 12/05/2002   9:27 AM ]
...
SVARS ;;A,C,E,F,L,M,N,O,P,R,S,T,V;Single-character work variables.
 ...
 ;
 D RSLT($J("",5)_$P($T(UPDATE^AUM3104A),";",3))
 F L="GREET","INTROE","INTROI" F %=1:1 D RSLT($P($T(@L+%^AUM3104),";",3)) Q:$P($T(@L+%+1^AUM3104),";",3)="###"
 D DASH,SUMOD,DASH,LOCNEW,DASH,LOCMOD,DASH,COMMNEW,DASH,COMMMOD,DASH,TRIBMOD,DASH
 Q
 ; -----------------------------------------------------
...
DIE NEW @($P($T(SVARS),";",3))
 ...
 Q
E(L) Q $P($P($T(@L^AUM3104A),";",3),":",1)
IEN(X,%,Y) ;
 S Y=$O(@(X_"""C"",%,0)"))
 I 'Y S Y=$$VAL^AUM3104M(X,%) I Y D  S:Y<0 Y=""
 . NEW %,@($P($T(SVARS),";",3))
 . S L=Y
...
MODOK D RSLT($J("",5)_"Changed : "_L)
 Q
RSLT(%) S ^(0)=$G(^TMP("AUM3104",$J,0))+1,^(^(0))=% D MES(%)
 Q
...
SUNEW ;
 ...
 F T=1:1 S L=$T(SUNEW+T^AUM3104A) Q:$P(L,";",3)="END"  D ADDSU
 Q
 ; -----------------------------------------------------
...
SUMOD ;
 ...
 D RSLT($J("",15)_"-- -- ----")
 F T=1:2 S L=$T(SUMOD+T^AUM3104A) Q:$P(L,";",3)="END"  S L("TO")=$T(SUMOD+T+1^AUM3104A) D
...
LOCNEW ;
 ...
 F T=1:1 S L=$T(LOCNEW+T^AUM3104A) Q:$P(L,";",3)="END"  D ADDLOC
 Q
ADDLOC ;
...
LOCMOD ;
 ...
 F T=1:2 S L=$T(LOCMOD+T^AUM3104A) Q:$P(L,";",3)="END"  S L("TO")=$T(LOCMOD+T+1^AUM3104A) D
 ...
 D DASH,RSLT($$LOCMOD^AUMXPORT("AUM3104A")_" patients marked for export because of the Location Code changes.")
 Q
 ; -----------------------------------------------------
COMMNEW ;
 D RSLT($$E("COMMNEW"))
 ...
 F T=1:1 S L=$T(COMMNEW+T^AUM3104A) Q:$P(L,";",3)="END"  D ADDCOMM
 Q
 ;
ADDCOMM ;
...
COMMMOD ;
 ...
 F T=1:2 S L=$T(COMMMOD+T^AUM3104A) Q:$P(L,";",3)="END"  S L("TO")=$T(COMMMOD+T+1^AUM3104A) D
 ...
 D DASH,RSLT($$COMMMOD^AUMXPORT("AUM3104A")_" patients marked for export because of the Community Code changes.")
 Q
 ; -----------------------------------------------------
ADDTRIB ;
...
TRIBMOD ;
 ...
 F T=1:2 S L=$T(TRIBMOD+T^AUM3104A) Q:$P(L,";",3)="END"  S L("TO")=$T(TRIBMOD+T+1^AUM3104A) D
