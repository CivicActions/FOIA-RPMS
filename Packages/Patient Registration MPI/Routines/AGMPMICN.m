AGMPMICN ;IHS/SD/TPF - MPI MISSING ICN BACKGROUND TASK; MAY 20, 2010
...
MISSEUID ;EP - PROCESS MISSING EUID
 I '$$ENABLED^AGMPOPT('$D(ZTQUEUED)) Q  ; CHECK WHETHER MPI HAS BEEN DISABLED
 ...
 ;If running in foreground, let user pick maximum number of records to extract
 ...
 . S MAX=Y
 S DFNIEN=0
 ;Update the MISSING ICN TASK to use the TREATING FACILITY LIST.
 F CNT=0:1 S DFNIEN=$O(^DPT(DFNIEN)) Q:'DFNIEN  D
 .I $D(^DPT(DFNIEN,-9)) S MERGED=MERGED+1 Q  ;DON'T ATTEMPT FOR MERGED RECORDS
 .;I '$$XMITPAT^AGMPSMSG(DFNIEN) Q  ;DEMO PATIENT CHECK
 .I $$DEMOPAT^AGMPSMSG(DFNIEN) Q  ;Demo Patient Check
 ...
 .I PICN]"" K ^XTMP("AGMPICN",DFNIEN) Q  ;OK. Does not need to be tracked.
 .I $G(^XTMP("AGMPICN",DFNIEN))+DELAY>+$H S QUEUED=QUEUED+1 Q  ;Wait at least 'DELAY' days before trying again.
 .S DUZ(2)=0
 .F  S DUZ(2)=$O(^AUPNPAT(DFNIEN,41,DUZ(2))) Q:DUZ(2)=""  D  Q:DUZ(2)=""
 ..I $P($G(^AGFAC(DUZ(2),0)),U,21)'="Y" S NONORF=NONORF+1 Q  ;ONLY "OFFICIAL REGISTERING FACILITIES"
 ..S MISSING=MISSING+1
 ..;Only add to queue if under the maximum number of attempts per execution
 ..I ADDED<MAX D ATTEMPT(DFNIEN,.ADDED,.ERRORS)
 ..S DUZ(2)=""  ;Send only one message for this patient
 ...
 .W !,"ERRORS: ",ERRORS
 .W !,"MESSAGES QUEUED: ",ADDED I ADDED>MAX W "   *** Per execution maximum of ",MAX," exceeded ***"
 Q
 ;
ATTEMPT(DFN,ADDED,ERRORS) ;EP - TRY TO ADD PTS WITH MISSING EUIDs
 N BUSALOG
 D CREATMSG^AGMPHLO(DFN,"A28","",.SUCCESS)  ;DO THE A28
 I 'SUCCESS D  Q
 .S ERRORS=ERRORS+1
 .S ERR="CANNOT CREATE A28 DURING 'AGMP MPI MISSING ICN TSK'"
 .D NOTIF^AGMPHLO(DFN,ERR)
 ;If successful, update counter, add to the temporary global, and log
 S ADDED=ADDED+1
 ;S ^XTMP("AGMPICN",DFNIEN,DUZ(2))=+$H
 S ^XTMP("AGMPICN",DFNIEN)=+$H
 S BUSALOG(1)=DFN
 D LOG^BUSAAPI("O","P","A","AGMP MPI MISSING ICN TSK","Generated A28 MPI message","BUSALOG")
...
BATCHOUT() ;EP - THIS ENTRY POINT WILL COLLECT A28 MESSAGE FOR PTS WITH MISSING ICNS
 ...
 .D NOTIF^AGMPHLO(0,"Unable to create batch."_$S($D(ERR):" ERR:"_$G(ERR),1:""))
 S (DFNIEN,QUIT)=0
 ...
 ...S QUIT=1
 ...D NOTIF^AGMPHLO(0,"UNABLE TO ADD MESSAGE TO BATCH."_$S($D(ERR):" ERR:"_$G(ERR),1:""))
 ..M HLST=HLMSTATE
 ..D EVN^AGMPHLO(HLPM("EVENT"))
 ..I '$D(ERR) D PID^AGMPHL1(DFNIEN)
 ..I '$D(ERR) D ZPD^AGMPHL1(DFNIEN)
 ..I $D(ERR) D NOTIF^AGMPHLO(0,"UNABLE TO ADD MSG TO BATCH."_$S($D(ERR):" ERR:"_$G(ERR),1:""))
 ...
 .D NOTIF^AGMPHLO(0,"UNABLE TO SEND BATCH."_$S($D(ERR):" ERR:"_$G(ERR),1:""))
 K BATCH
 Q 1
 ;
SET ;EP - INIT VARS FOR BATCH
 ...
 ;S APPARMS("ACCEPT ACK TYPE")="NE"  ;(FIELD 15) Commit ACK type ;TPF - CHANGED TO THIS BECASUE OF "SE" ERRORS APPARMS("APP ACK RESPONSE")="AACK^AGMPHLO"  ;Callback when 'application ACK' is received
 S APPARMS("ACCEPT ACK RESPONSE")="CACK^AGMPHLO"  ;Callback when 'commit ACK' is received
