AGMPRPT2 ;IHS/SD/TPF - MPI REPORTS; 12/15/2007
 ;;1.0;AGMP;**2,4**;Apr 30, 2021;Build 55
 Q
MSGSTAT ;UNSUCCESSFUL STATUS REPORT
 N MSGIEN,STATUS,STATTYP,CLEAR,EESC,TOTMSG,MSGDATE,TOTCLEAR,DATES,TYPEEVNT
ASKFROM ;EP
 K DIR
 S DIR(0)="DO^::E"
...
QUEVSENT ;EP - MSGS IN QUEUE VS TOTAL MSGS
 N HLOG,PORT,MPILINK,REC,OUTCOUNT,TOTCOUNT,INCOUNT,FROM,MPIIEN,ESC
 S ESC=0
 ...
 .S MPIIEN=""
 .F  S MPIIEN=$O(^HLB("QUEUE","IN",FROM,"RPMS-MPI",MPIIEN)) Q:MPIIEN=""  D
 ..S INCOUNT=INCOUNT+1
 S MPIIEN=""
...
HDR ;EP - PRINT MENU HEADER
 N XPOS,ZIS
 ;NEEDED? D:'$D(AGOPT) ^AGVAR
 I $D(X) S X=$P(^DIC(19,$O(^DIC(19,"B",X,0)),0),U,2)
 I '$D(IOF) D    ; defensive for menu jumping
 . S IOP="HOME"
 . D ^%ZIS
 S XPOS=$X
 S ZIS=$P($G(^%ZIS(2,+$G(IOST(0)),1)),"^",2)
 I ZIS="" S ZIS="*0"
 W @ZIS
 S $X=XPOS
 W !!?30,"MASTER PATIENT INDEX"
 I $G(DUZ(2))'="",$D(^DIC(4,DUZ(2),0))#2 W !!?40-($L($P(^DIC(4,DUZ(2),0),U))\2),$P(^(0),U)
 I $D(X) W !!,$J("",80-$L(X)\2),X,!
 Q:$D(AGMP("RPT"))
