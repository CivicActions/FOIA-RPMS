AGMPHL1 ;IHS/SD/TPF - MPI HLO Interface ext ALL HLO MESSAGES;JAN 07, 2011
...
PID(DFN) ;EP
 ...
 I '$G(DFN) S ERR="DFN UNDEF IN PID^AGMPHL1" Q
 I $G(^DPT(DFN,0))="" S ERR="DPT 0 NODE FOR "_DFN_" NOT DEFINED IN PID^AGMPHL1" Q
 I $P($G(^DPT(DFN,0)),U)="" S ERR="DPT 0 NODE P^1 "_DFN_"NOT POPULATED IN PID^AGMPHL1" Q
 ...
 K PID,ERR
 D BLDPID^AGMPPID(DFN,1,"ALL",.PID,.HL1,.ERR)
...
CONTPID ;EP -
 ...
 I $D(ERR) D NOTIF^AGMPHLO(DFN,ERR)
 Q
 ;
...
ZPD(DFN) ;EP - RPMS SPECIFIC DATA
 N TRIBEPTR,TRIBECOD,ELIGSTAT,BENPTR,BENCLASS,QUANTUM,GUID
 ...
 S @("GUID=##class(%SYS.System).InstanceGUID()")  ;INSTANCE GUID
 D MYSET(.ARY,"ZPD",0)
 D MYSET(.ARY,1,1,1,1,1)  ;ALWAYS 1
...
SCHART(IEN) ;EP - GET ACTIVE CHARTS
 ...
 .S:DATEINAC'="" DATEINAC=$$CONDT^AGMPSMSG(DATEINAC)  ;CHANGE FM DATE TO SQL NEXTGATE DATE
