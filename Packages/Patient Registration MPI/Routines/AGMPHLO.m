AGMPHLO ;IHS/SD/TPF - MPI HLO Interface ALL HLO MESSAGES; MAY 20, 2010
...
NEWMSG(BPMRY) ;EP - FOR PATIENT MERGE
 ...
 N AGMPFR,AGMPTO
 S AGMPTO=$O(@BPMRY@(0))
 I '$G(AGMPTO) S A40ERR=1 D NOTIF(AGMPTO,"Unable to build HL7 message. ERR:A40: NO TO DFN FOR A40 EVENT") Q
 I '$D(^DPT(AGMPTO,0)) S A40ERR=1 D NOTIF(AGMPTO,"Unable to build HL7 message. ERR:A40: NO DPT ENTRY FOR DFN "_AGMPTO) Q
 ;
 S AGMPFR=$O(@BPMRY@(AGMPTO,0))
 I '$G(AGMPFR) S A40ERR=1 D NOTIF(AGMPFR,"Unable to build HL7 message. ERR:A40: NO DFN2 FOR A40 EVENT") Q
 I '$D(^DPT(AGMPFR,0)) S A40ERR=1 D NOTIF(AGMPFR,"Unable to build HL7 message. ERR:A40: NO DPT ENTRY FOR DFN2 "_AGMPFR) Q
 D CREATMSG(AGMPFR,"A40",AGMPTO,.SUCCESS)
 Q
 ;
...
CREATMSG(DFN,EVNTTYPE,DFN2,SUCCESS) ;EP - START FOR MOST PAT REG TRIGGERS
 ...
 ;S AGMPSTOP=1
 ; VQQ and A40 messages have been disabled
 I $G(EVNTTYPE)="VTQ"!($G(EVNTTYPE)="VQQ")!($G(EVNTTYPE)="A40") S SUCCESS=0 Q
 I "0"[DUZ(2) S SUCCESS=0 D NOTIF("","Site '"_DUZ(2)_"' is not valid.") Q
 ...
 ;
 ;Commented out - demo checks are now handled elsewhere
 ;I '$$XMITPAT^AGMPSMSG($G(DFN)) S SUCCESS=0 Q  ;DEMO PATIENT CHECK
 ;I $G(DFN2)'="",'$$XMITPAT^AGMPSMSG(DFN2) S SUCCESS=0 Q  ;DEMO PATIENT CHECK
 N A40ERR
 S SUCCESS=1
 S A40ERR=0
 ...
 ;
 I '$D(ERR),($G(EVNTTYPE)'="A40") D ZPD^AGMPHL1(DFN)  ;RPMS SPECIFIC DATA
 ;
 I '$D(ERR),($G(EVNTTYPE)="A40") D MRG^AGMPHL1(DFN2) ; DO A40 MERGE SEGMENT
...
DOSEND ;EP
 ...
 .E  D
 ..S APPARMS("APP ACK RESPONSE")="AACK^AGMPHLO"  ;Callback when 'application ACK' is received
 ...
 .S HLST("SYSTEM","PROCESSING ID")=$S($$PROD^XUPROD():"P",1:"T")  ;PROD/TEST FLAG
 ...
 .;I EVNTTYPE="A40" D NOTIF(DFN,"AN A40 MESSAGE HAS BEEN CREATED "_SUCCESS)  ; NOTIF sets SUCCESS to 0 and we don't need this call.
 .I 'SUCCESS D
 ..D NOTIF(DFN,"Unable to create HL7 message."_$S($D(ERR):" ERR:"_$G(ERR),1:""))
 I $D(ERR) D
 .I ERR[("NOT DEFINED IN PID^AGMPHL1") S SUCCESS=1_U_ERR Q  ;THIS IS A TIMING ISSUE THAT SEEMS TO RESOLVE ITSELF FOR A08
 .S SUCCESS=0
...
CACK ; EP - Commit ACK callback - called when CA, CE or CR is received.
 ...
 ;        MSG = Main message
NOTIF(DFN,MSG) ;EP
 N PNAM,XQA,XQAID,XQADATA,XQAMSG
 S SUCCESS=0
 S PNAM=""
