PSORDAYS ;IHS/ASDS/ENM/POC - PROGRAM TO CALCULATE DEFAULT DAYS SUP [ 02/20/2001  3:46 PM ]
 ;;6.0;OUTPATIENT PHARMACY;**3**;01/02/01
 ;W !,"ENTER THE STRING  "
 ;R X:30
 ;I (X="")!(X["^") QUIT
...
END ;CLEAN UP
 D EN^XBVK("PSOEX")
 K X,I,J,K,P,PART,QUIT,REPLANEW,SIG,TEXT,TIMES,TR,TRTO
 K CHECK
 Q
 ;
...
CHANGEN ;CHANGE TEXT TO NUMBERS
 ...
 S STRING=PSOEXSIG
 ;S STRING=X
 F I=1:1 S TEXT=$P($T(NTEXT+I^PSORDAY),";;",2) Q:TEXT="QUIT"  D
 .S LOOK=$P(TEXT,";",1)
 .S REPLACE=$P(TEXT,";",2)
...
CHANGET ;CHANGE TIME AROUND
 S STRING=PSOEXSIG
 ;S STRING=X
 F I=1:1 S TEXT=$P($T(TTEXT+I^PSORDAY),";;",2) Q:TEXT="QUIT"  D
 .S LOOK=$P(TEXT,";",1)
 .S REPLACE=$P(TEXT,";",2)
 .S NUMBER=$L(STRING,LOOK)-1
 .Q:NUMBER<1
...
CHANGEH ;CHANGE TIME TO NUMBERS 
 ...
 ;PSOEXSIG DEFINED
 ;NOTE FIRST HALF OF SIG
 I $L(PSOEXSIG,"ZGLOB")>1 S DELIMIT="ZGLOB",MULT=1 G A
 I $L(PSOEXSIG,"TEASPOON")>1 S DELIMIT="TEASPOON",MULT=5 G A
 I $L(PSOEXSIG,"BIGSPOON")>1 S DELIMIT="BIGSPOON",MULT=15 G A
 I $L(PSOEXSIG,"UNIT")>1 S DELIMIT="UNIT",MULT=.01 G A
 G KILLH
A S STRING(1)=$P(PSOEXSIG,DELIMIT),STRING(2)=$P(PSOEXSIG,DELIMIT,2)
 F Q=1:1:20 D
 .F P=1:1:20 D
...
B ;B ADDED 3/10/98 NEXT 7 LINES
 ...
 F I=1:1 S TEXT=$P($T(HTEXT+I^PSORDAY),";;",2) Q:TEXT="QUIT"!QUIT(2)  D
...
DAY ;CALCULATE THE DAYS SUPPLY
 IF 'PSOEXMX W !,*7,*7,"I DONT KNOW THE DAYS SUPPLY-GO FIGURE!" S PSOZDAY="" Q
 ;QTY IS PSOEXQTY
 S PSOZDAY=PSOEXQTY\PSOEXMX
 ;W !,"DAYS SUPPLY= ",PSOZDAY ;****DELETE
 Q
