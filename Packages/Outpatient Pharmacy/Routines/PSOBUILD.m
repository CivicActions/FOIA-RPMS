PSOBUILD ;IHS/DSD/JCM - BUILD ARRAY OF PATIENTS CURRENT MEDS;11-Dec-2024 13:39;DU
 ;;7.0;OUTPATIENT PHARMACY;**23,82,119,132,1006,1013,235,206,1010,1015,1023,1024,1030,1031,1036**;DEC 1997;Build 17
 ...
 ;            IHS/MSC/MGH - 02/23/18 - Added Controlled substance indicator
 ;            IHS/MSC/PLS - 03/04/22 - Inverted comparison logic at BARC+2,3
 ;            IHS/MSC/PLS - 05/03/22 - Added display order (APSP PPP DISPLAY ORDER XPAR)
 ;                        - 12/11/24 - BARC+6  FID 99323
 ; Input variables: PSODFN,DT,PSODTCUT
START N ORD,APSPCS,APSPDIG,APSPHC,APSPPICK,CS1 K PSOSD I '$D(PSODFN)!('$D(DT)) G END
 N DRGZ  ;IHS/MSC/PLS - 11/03/2011
 N LBLARY,LBLACT,LPOS  ;p1031
 S (APSPHC,APSPDIG)=0
 ...
 S STA=$$LBLADJ("ACTIVE")_U_$$LBLADJ("NON-VERIFIED")_U_$$LBLADJ("REFILL")_U_$$LBLADJ("HOLD")_U_$$LBLADJ("NON-VERIFIED")_U_$$LBLADJ("ACTIVE")_"^^^^^^"
 S STA=STA_$$LBLADJ("ACTIVE")_U_$$LBLADJ("DISCONTINUED")_U_U_$$LBLADJ("DISCONTINUED")_U_$$LBLADJ("DISCONTINUED")_U_$$LBLADJ("HOLD")
 ...
 .I $D(PSOSD($$LBLADJ("PENDING"),DRG)) S DRG=DRG_"^"_PEN
 .;IHS/MSC/MGH added indicator for controlled substances patch 1023
 .S APSPCS=$P($G(^PSDRUG(PSODD,0)),U,3)
 .S APSPDIG=$P($G(^PS(52.41,PEN,0)),U,24)
 .S APSPPICK=$P($G(^PS(52.41,PEN,0)),U,17)
 .I +APSPCS>1&(+APSPCS<6)&(+APSPDIG=0)&(APSPPICK'="C") S APSPHC=1
 .S PSOSD($$LBLADJ("PENDING"),DRG)="*****^17^Z^Z^"_$S(PSODD:$P(^PSDRUG(PSODD,0),"^",2),1:"")_"^"_$P(^PS(52.41,PEN,0),"^",11)_"^"_$S($G(^PSDRUG(PSODD,"ND"))]"":+^("ND")_"A"_$P(^("ND"),"^",3),1:"")
 .S PSOSD($$LBLADJ("PENDING"),DRG)=PSOSD($$LBLADJ("PENDING"),DRG)_"^"_$P(ORD,"^",10)_"^"_$P(ORD,"^",6)_"^"_PEN_"^"_$S($G(PSODD):$G(PSODD),1:""),PSOSD=+$G(PSOSD)+1 K PSOOI,PSODD
 .;IHS/MSC/MGH added line for controlled substances patch 1023
 .S CS1=$S(+APSPHC:"+",+APSPDIG:"$",1:" ")
 .S PSOSD($$LBLADJ("PENDING"),DRG)=PSOSD($$LBLADJ("PENDING"),DRG)_"^"_CS1
 .I +APSPCS=2 D
 ..S EARLY=$$VALUE^ORCSAVE2(+ORD,"EARLIEST")
 ..I +EARLY S PSOSD($$LBLADJ("PENDING"),DRG)=PSOSD($$LBLADJ("PENDING"),DRG)_"^"_EARLY
 .S PSOSD($$LBLADJ("PENDING"),DRG,"IHS")=DRGZ
 ...
 ;..K PSOSD("ACTIVE",DRG)
 ;IHS/MSC/PLS = 7/25/2022 - Sort order
 S LBLACT=$$LBLADJ("ACTIVE")
 S LPOS=+$$LBLADJ("ACTIVE",1)
 S DRG="" F  S DRG=$O(PSOSD(LBLACT,DRG)) Q:DRG=""  D
 .I $P($G(^PSRX(+PSOSD(LBLACT,DRG),999999921)),U,3) D
 ..S PSOSD($$LBLADJP("ACTIVE OTHER PHARMACY",$S('LPOS:0,1:$$LBLADJ("ACTIVE",1)+1)),DRG)=PSOSD(LBLACT,DRG)
 ..S PSOSD($$LBLADJP("ACTIVE OTHER PHARMACY",$S('LPOS:0,1:$$LBLADJ("ACTIVE",1)+1)),DRG,"IHS")=PSOSD(LBLACT,DRG,"IHS")
 ..K PSOSD(LBLACT,DRG)
END D EOJ
 Q
INIT ;
...
BARC S PSORFRM=$P(PSORX0,"^",9) F PSOJ=0:0 S PSOJ=$O(^PSRX(PSOBUILD("RX"),1,PSOJ)) Q:'PSOJ  S PSORFRM=PSORFRM-1
 ...
 ;S PSODRUGN=$P(PSODRUG0,"^") I $D(PSOSD(PSODRUGN)),PSOST0>10 Q:$P(PSOSD(PSODRUGN),"^",2)<11  Q:$P(PSOSD(PSODRUGN),"^",2)>10&($P(PSORX0,"^",13)'>$P(^PSRX(+$P(PSOSD(PSODRUGN),"^"),0),"^",13))  ;p1030  FID 75449
 ...
 ;S PSODRUGN=$$UP^XLFSTR($P(PSODRUG0,"^")) I $D(PSOSD(PSODRUGN)),PSOST0>10 Q:$P(PSOSD(PSODRUGN),"^",2)<11  Q:$P(PSOSD(PSODRUGN),"^",2)>10&($P(PSORX0,"^",13)'>$P(^PSRX(+$P(PSOSD(PSODRUGN),"^"),0),"^",13))  ;p1030 FID 75449
 N APSPQ S APSPQ=0  ;p1036
 S PSODRUGN=$$UP^XLFSTR($P(PSODRUG0,"^")) I $D(PSOSD(PSODRUGN)) D  Q:$G(APSPQ)
 .I PSOST0>10 D  Q:$G(APSPQ)
 ..I $P(PSOSD(PSODRUGN),"^",2)<11 S APSPQ=1 Q
 ..I $P(PSORX0,"^",13)'>$P(^PSRX(+$P(PSOSD(PSODRUGN),"^"),0),"^",13)&(+PSOBUILD("RX")<+$P(PSOSD(PSODRUGN),"^")) S APSPQ=1 Q  ;p1036 FID 99323
 ;..PSOSD(PSODRUGN) - holds existing IEN, PSOBUILD("RX") - holds current IEN
 S:'$D(PSOSD(PSODRUGN)) PSOBUILD("COUNT")=PSOBUILD("COUNT")+1
 ;Patch 1024 IHS/MSC/MGH Get CS data
 S APSPDIG=0,APSPHC=0
 S APSPCS=$P($G(^PSDRUG(PSODRG,0)),U,3)
 I $P($G(^PSRX(PSOBUILD("RX"),"PKI")),U,1)=1 S APSPDIG=1
 I $P($G(^PSRX(PSOBUILD("RX"),999999941)),U,2)=1 S APSPHC=1
 S CS1=$S(+APSPHC:"+",+APSPDIG:"$",1:" ")
 ...
 .S PSOSD(PSODRUGN_"^"_PSOBUILD("RX"))=PSOBUILD("RX")_"^"_PSOST0_"^"_PSOSTN_"^"_PSOSTF_"^"_PSOVACL_"^"_PSORFRM_"^"_PSONDF_"^"_PSODYS_"^^"_CS1,PSOBUILD("COUNT")=PSOBUILD("COUNT")+1
 .S PSOSD(PSODRUGN_U_PSOBUILD("RX"),"IHS")=$P(PSODRUG0,U)
 E  D
 .S PSOSD(PSODRUGN)=PSOBUILD("RX")_"^"_PSOST0_"^"_PSOSTN_"^"_PSOSTF_"^"_PSOVACL_"^"_PSORFRM_"^"_PSONDF_"^"_PSODYS_"^^"_CS1
...
LBLADJ(LBL,FLG) ;
 ...
 S FLG=$G(FLG,0)
 I '$D(LBLARY) D
 .D GETLST^XPAR(.LST,"ALL","APSP PPP DISPLAY ORDER","E","ERR")
 .N LP S LP=0 F  S LP=$O(LST(LP)) Q:'LP  D
 ..S LBLARY($P(LST(LP),U,2))=$P(LST(LP),U,1,2)_U_$$LBLADJP($P(LST(LP),U,2),LST(LP))
 S RES=$S($D(LBLARY(LBL)):$S(FLG:+LBLARY(LBL),1:$P(LBLARY(LBL),U,3)),1:LBL)
 Q RES
LBLADJP(VAL,L) ;
 Q:'L VAL
 Q $E("---------",1,9-L)_VAL_$E("---------",1,9-L)
