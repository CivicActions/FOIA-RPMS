PSOPKIV1 ;BHAM ISC/MHA - validate PKI cert. ;18-Apr-2018 13:35;DU
 ;;7.0;OUTPATIENT PHARMACY;**131,146,223,148,249,391,426,1023**;DEC 1997;Build 121
 ;Ref. to ^ORDEA is supported by DBIA 5709
 ;Ref. to ^ORB supported by DBIA 1362
 ;Ref. to ^XUSSPKI supported by DBIA 3539
 ;Modified - IHS/MSC/MGH  10/25/2017 L1+1 updated for EPCS
 ;                             11/1/2017 CER to use new routine
 ;                             11/11/2012 DCP+6
CER ;
 N PKIRT,PORDER,DEA
 ;IHS/MSC/MGH changes to use new routine for hash
 ;D VERIFY(.PKIRT,ORD)
 S PKIRT=$$RXVER^BEHOEPS(ORD)   ;EPCS new routine used
 S PKI=+PKIRT
 I PKI=1!(PKI=89802020) D  Q
 . S PKI1=1,VALMSG="Digitally Signed Order",PKIE="Processing "_VALMSG
 . I PKI=89802020 S PKIE=PKIE_": "_$P($T(@($E(PKI,7,8))),";;",2)
 . S PORDER=$P($G(^PS(52.41,ORD,0)),U,1)
 . D EDIT^APSPCSA(+PORDER,"VF")
 I PKI=0 D
 .S PORDER=$P($G(^PS(52.41,ORD,0)),U,1)
 .I $P(PKIRT,U,2)<0 D
 ..S PKIE=$P(PKIRT,U,3)
 ..S VALMSG="Dig Signed Order is not verifiable at this time"
 .E  D
 ..S PKIE=$P(PKIRT,U,3)_" - Order Auto Discontinued"
 ..S VALMSG="Digitally Signed Order Failed Verification"
 .D EDIT^APSPCSA(+PORDER,"FP")   ;Update audit log
 ...
 S PKI1=$S((PKI=0)&($P(PKIRT,U,2)=0):2,PKI=0:0,1:1)
 I PKI1=2 D DCP^PSOPKIV1 S VALMBCK="R"    ;Discontinue tampered order
 Q
L1 ;
...
ACT(DA) ;
 Q:'DA
 N I,J D AR
 ;IHS/MSC/MGH for EPCS
 ;S ^PSRX(DA,"A",0)="^52.3DA^"_J_"^"_J,^PSRX(DA,"A",J,0)=%_"^K^"_DUZ_"^0^INVALID PKI CERT. "_PKI
 S ^PSRX(DA,"A",0)="^52.3DA^"_J_"^"_J,^PSRX(DA,"A",J,0)=%_"^K^"_DUZ_"^0^INVALID PKI CERT. "_PKI1
...
DCP ;
 ...
 W !,"Order has been modified or tampered and will be discontinued"
 W *7
 ...
 ;IHS/MSC/MGH changed for EPCS to send alert
 ;S PKIOR=$E(PKI,7,8)
 S PKIOR=16
 ...
 ;IHS/MSC/MGH Use new mailman message
 ;D ^PSOPKIV2
 D MAIL^PSOPKIV2(ORD)
 Q
 ;
DCV ;
 W ! D KV^PSOVER1 K PKIR S DIR(0)="Y",DIR("B")="N",DIR("A",1)="Digitally signed Rx cannot be deleted but it has been tampered, so it can must be D/Ced."
