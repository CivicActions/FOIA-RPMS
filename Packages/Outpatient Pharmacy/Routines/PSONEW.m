PSONEW ;IHS/DSD/JCM - NEW RX ORDER MAIN DRIVER [ 02/20/2001  3:39 PM ]
 ;;6.0;OUTPATIENT PHARMACY;**3**;09/03/97
 ;;6.0;OUTPATIENT PHARMACY;**22,100,118**;09/03/97
 ;---------------------------------------------------------------
START ;
 D EOJ
...
IHSV ; hook to store data for patient in PCC parameter array for later use
 ...
 I PSONEW("QFLG") G END
 I PSONEW("DFLG") W !,*7,"RX DELETED",! G START
IHSP ;---- ---- ---- ---- ---- ----
 ...
 I PSONEW("DFLG")!PSONEW("QFLG") D DEL G START
...
EXD ;IHS/DSD/ENM 4/20/94 Check %APSITE P11 for Exp Date
 ...
 D ^PSONEW2 I PSONEW("DFLG") D DEL G START ; Asks if correct
...
DISPLAY ;
 ...
 ;
ASK ;
 S DIR(0)="SB^Y:YES;N:NO;P:PROFILE;R:REFILL;A:ALLERGIES/REACTIONS;G:GOTO FAST",DIR("B")="Y"
 S DIR("A")="NEW RX FOR "_$G(PSORX("NAME"))_" ?"
 ...
 ;I "Gg"[Y G FAST^PSORX ;IHS/OKCAO/POC FAST OPTION
 ;I "Gg"[Y S APSQFAST=1,PSONEW("QFLG")=1 ;IHS/OKCAO/POC FAST OPTION
 I "Gg"[Y S APSQFAST=1,PSONEW("QFLG")=1,PSOFROM="FAST" ;IHS/OKCAO/POC FAST OPTION
 I "Pp"[Y D ^PSOBUILD,^PSODSPL G ASK
 I "Rr"[Y S (PSORX("DO REFILL"),PSONEW("QFLG"))=1
 ;I "Aa"[Y D GMRA^PSODEM G ASK
 ;ALSO MODIFIED GMRAPEM0 AT ABOUT LINE 6 IHS/OKCAO/POC
 I "Aa"[Y D  G ASK  ;ADD A SHORT CUT IHS/OKCAO/POC 7/2/98
 .D GMRA^PSODEM
 .S DIR("A")="WANT TO ENTER ALLERGY INFORMATION  ",DIR("B")="NO",DIR(0)="Y" D ^DIR
 .K DIR
 .I Y=1 D EN2^GMRAPEM0
 .W ! ;G ASK LET'S NOT GET CAUGHT IN THIS LOOP STUFF AGAIN
ASKX ;
...
EOJ ;
 ...
 K PSONEW,PSODRUG,ANQDATA,LSI,C,MAX,MIN,NDF,REF,SIG,SER,PSOFLAG,PSOHI,PSOLO,APSPZRD
