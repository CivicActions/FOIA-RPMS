PSOORUT1 ;BIR/SAB - Utility routine for oerr interface ;18-Jan-2024 13:35;DU
 ;;7.0;OUTPATIENT PHARMACY;**1,14,30,46,132,1005,1006,148,233,274,225,305,289,1015,1016,1024,1031,1034**;DEC 1997;Build 37
 ...
 ; Modified - IHS/MSC/MGH - 02/23/18 - PEN+7,10,13
 ;                        - 01/23/20 - DISPL+1
 ;            IHS/MSC/PLS - 11/21/22 - DISPL+44, PENX
 ;                        - 04/21/23 - DISPL several lines adjusted to fit year on Issue Date row
...
SHOW ;
 ...
 . . S PSOCNT=PSOCNT+1 I PSOSTA=$$LBLADJ^PSOBUILD("PENDING") D PEN Q
...
SHOWX K DIRUT,DTOUT,DUOUT,DIROUT,PSODRG
 Q
 ;
DISPL S IEN=IEN+1 N PSOID,PSOCMOP,STATLTH,ECME,APSPCS
 ;IHS/MSC/MGH Patch 1024 changes made to move items over 2 spaces.
 N PSODRGX
 S PSODRGX=$E($P(PSODRG,"^"),1,37)  ;p1034
 K PSOLNT,PSOQTL,PSOLSP S PSOLRX=$S($G(^PSRX(+PSODATA,"IB")):11,1:12)-$L($P(^PSRX(+PSODATA,0),"^")),$P(PSOLNT," ",PSOLRX)=" ",PSODQL=$L($P(PSODRGX,"^"))+$L($P(^PSRX(+PSODATA,0),"^",7))
 ;Patch 1024 add in EPCS flag
 S APSPCS=$P(PSODATA,U,10)
 I PSODQL<37 S $P(PSOQTL," ",(37-PSODQL))=" " ;p1034
 E  S $P(PSOQTL," ",(50-$L($P(^PSRX(+PSODATA,0),"^",7))))=" ",$P(PSOLSP," ",(38-$L($P(PSODRGX,"^"))))=" "  ;p1034
 S ECME=$$ECME^PSOBPSUT(+PSODATA) I ECME'="" S PSOLNT=$E(PSOLNT,1,$L(PSOLNT)-1)
 ;IHS/MSC/PLS - 10/11/07
 ;S ^TMP("PSOPF",$J,IEN,0)=$J(PSOCNT,2)_$S($L(PSOCNT)<3:" ",1:"")_$P(^PSRX(+PSODATA,0),"^")_$S($G(^PSRX(+PSODATA,"IB")):"$",1:"")_ECME_PSOLNT_$P(PSODRGX,"^")_$S(PSODQL<37:PSOQTL_$P(^PSRX(+PSODATA,0),"^",7)_" ",1:$G(PSOLSP))
 S ^TMP("PSOPF",$J,IEN,0)=$S(PSOSTA="ACTIVE OTHER PHARMACY":"  ",1:$J(PSOCNT,2))_$S($L(PSOCNT)<3:" ",1:"")_$P(^PSRX(+PSODATA,0),"^")_$S($G(^PSRX(+PSODATA,"IB")):"$",1:"")_ECME_PSOLNT_$P(PSODRGX,"^")_" "
 S ^TMP("PSOPF",$J,IEN,0)=^TMP("PSOPF",$J,IEN,0)_$S(PSODQL<37:PSOQTL_APSPCS_$P(^PSRX(+PSODATA,0),"^",7)_" ",1:$G(PSOLSP))
 ;
 ...
 S STATLTH=$L(STAPRT)
 I PSODQL>36 S STAPRT=" "_STAPRT
 S ^TMP("PSOPF",$J,IEN,0)=^TMP("PSOPF",$J,IEN,0)_STAPRT_$S(STATLTH=0:"   ",STATLTH=1:"  ",STATLTH=2:" ",1:"")
 S PSOID=$P(^PSRX(+PSODATA,0),"^",13),PSOLF=+$G(^(3)),^TMP("PSOPF",$J,IEN,0)=^TMP("PSOPF",$J,IEN,0)_$E(PSOID,4,5)_"-"_$E(PSOID,6,7)_"-"_$E(PSOID,2,3)_" "  ;p1034
 ...
 S PSOLF=$S($G(PSOLF):$E(PSOLF,4,5),1:"  ")_"-"_$S($G(PSOLF):$E(PSOLF,6,7),1:"  ")_$S($P(PSOLF,"^",2)="R":"R ",1:"  ")  ;p1034
 S PSOLRD=$S($G(PSOLRD):$E(PSOLRD,4,5),1:"   ")_"-"_$S($G(PSOLRD):$E(PSOLRD,6,7),1:"   ")_$S($P(PSOLRD,"^",2)="R":"R ",1:"  ")  ;p1034
 ...
 I PSODQL>36 S IEN=IEN+1 D
 .N QTY
 .S QTY=$P(^PSRX(+PSODATA,0),"^",7)
 .S ^TMP("PSOPF",$J,IEN,0)=$E(PSOQTL,1,$L(PSOQTL)-(1+$E("210",$L(QTY))))_"Qty:"_$S($L(APSPCS):APSPCS,1:" ")_$J(QTY,3)  ;p1034
 K PSOLNT,PSOQTL,PSOLSP,PSOLRX,PSODQL
 S PSOLST(PSOCNT)="52^"_+PSODATA_"^"_$$STRIP^XLFSTR(PSOSTA,"-")  ;P1031
...
STA N LABEL,LINE,POS
 ...
 Q
PENX S PSOLST(PSOCNT)="52.41^"_$P(PSODATA,"^",10)_"^"_$$STRIP^XLFSTR(PSOSTA)  ;P1031
 K PSODATA,PSOLF,RN,PSOLSP,PSOQTL,PSOLNT
 Q
PEN ;
 N PSOQTL,PSOLNT,PSOLNTZ,PSOQTLX,PSCMOPF,SPACEZ,APSPCS
 ...
 ;IHS/MSC/MGH Patch 1023 Added line to get controlled substance indicator
 S APSPCS=$P(PSODATA,"^",12)
 S APSPED=$P(PSODATA,"^",13)
 S $P(PSOQTLX," ",(11-PSOLNTZ))=" "
 ;Break into 2 lines if earliest date
 I APSPED'="" D  G PENX
 .S IEN=IEN+1,$P(SPACEZ," ",32)=" "
 .S ISDT=$S('$P(PSODATA,"^",9):"     ",1:$E($P(PSODATA,"^",9),4,5)_"-"_$E($P(PSODATA,"^",9),6,7))
 .S EADT=$E(APSPED,4,5)_"-"_$E(APSPED,6,7)
 .S ^TMP("PSOPF",$J,IEN,0)=SPACEZ_$G(PSOQTL)_"  "_"QTY: "_$P(PSODATA,"^",8)_$G(PSOQTLX)_" "_APSPCS_" ISDT: "_ISDT_" EFDT: "_EADT_$S($G(PSCMOPF):"> ",1:"   ")
 ;IHS/MSC/MGH Patch 1023 change length to 35
 S:PSOLNT<35 $P(PSOQTL," ",(35-PSOLNT))=" "  ;P1034 changed 37-PSOLNT to 35-PSOLNT
 I PSOLNT<36 D  G PENX
 .I PSOLNT=35 S PSOQTL=""
 ...
 .;IHS/MSC/MGH added in CS indicator patch 1023
 ...
 .S ^TMP("PSOPF",$J,IEN,0)=^TMP("PSOPF",$J,IEN,0)_$G(PSOQTL)_"  "_"QTY: "_$P(PSODATA,"^",8)_$G(PSOQTLX)_" "_APSPCS_" ISDT: "_$S('$P(PSODATA,"^",9):"     ",1:$E($P(PSODATA,"^",9),4,5)_"-"_$E($P(PSODATA,"^",9),6,7))_$S($G(PSCMOPF):"> ",1:"  ")
 .S ^TMP("PSOPF",$J,IEN,0)=^TMP("PSOPF",$J,IEN,0)_"REF: "_$S($L($P(PSODATA,"^",6))>1:"",1:" ")_$P(PSODATA,"^",6)
 S IEN=IEN+1,$P(SPACEZ," ",42)=" "
 S PSOQTLX=$E($G(PSOQTLX),1,$L($G(PSOQTLX))-2) ;p1034
 ...
 S ^TMP("PSOPF",$J,IEN,0)=SPACEZ_"QTY: "_$P(PSODATA,"^",8)_$G(PSOQTLX)_" "_APSPCS_" ISDT: "_$S('$P(PSODATA,"^",9):"     ",1:$E($P(PSODATA,"^",9),4,5)_"-"_$E($P(PSODATA,"^",9),6,7))_$S($G(PSCMOPF):"> ",1:"  ")
 S ^TMP("PSOPF",$J,IEN,0)=^TMP("PSOPF",$J,IEN,0)_"REF: "_$S($L($P(PSODATA,"^",6))>1:"",1:" ")
