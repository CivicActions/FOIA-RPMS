PSORN52C ;BIR/SAB-files renewal entries con't ;28-Jan-2020 08:56;DU
 ;;7.0;OUTPATIENT PHARMACY;**1,7,11,27,46,75,87,100,111,124,117,131,146,1003,1011,148,200,225,1015,1017,1023,1024**;DEC 1997;Build 68
 ...
 ;            IHS/MSC/MGH - 05/18/17 - ENT+1 Changed for EPCS
 ;            IHS/MSC/MGH - 04/10/18 - Special fields added
 ;            IHS/MSC/MGH - 07/08/19 - Adding logic for hardcopy
...
ENT S ^PSRX(PSOX("IRXN"),"POE")=1,^PSRX(PSOX("IRXN"),"INS")=$G(PSOX("INS"))
 ...
 I $P($G(^PSRX(PSOX("IRXN"),999999941)),U,2)=1 D HARDCPY^APSPFNC6(PSOX("IRXN"),OR0)
 ;IHS/MGH/MGH Exclude hardcopy types from backdoor
 I $P($G(PSOX("CS")),"^"),'+$P($G(^PSRX(PSOX("IRXN"),"PKI")),"^"),'$G(PSOX("HARDCPY")) S $P(^PSRX(PSOX("IRXN"),"PKI"),"^",2)=1
 ;IHS/MSC/MGH 9/24/2019 Added in to keep clinic orders from being considered
 I $G(OR0),$P(OR0,U,17)="C" S $P(^PSRX(PSOX("IRXN"),"PKI"),"^",2)=""
...
ORC ;
 ...
 I $G(PSOFDR) D
 .;I $G(PKI1)=1,$G(PKIR)]"" D ACT^PSOPKIV1(PSOX("IRXN")) ;IHS/MGH COMMENTED OUT PATCH 1023
...
PRVDATA ; EP -
 ...
 ;IHS/MSC/MGH fields added for EPCS
 S $P(PSORN52(PSOX("IRXN"),999999941),U,1)=$G(PSOX("DETOX#"))
 S $P(PSORN52(PSOX("IRXN"),999999941),U,2)=$G(PSOX("HARDCPY"))
 S $P(PSORN52(PSOX("IRXN"),999999941),U,3)=$G(PSOX("MPSUP"))
 S $P(PSORN52(PSOX("IRXN"),999999941),U,4)=$G(PSOX("EARLY"))
 ;IHS/MSC/MGH patch 1017 Add RxNorm
 S $P(PSORN52(PSOX("IRXN"),999999921),U,7)=PSOX("RXNORM")
 Q
 ;IHS/MSC/MGH - new API for EPCS
