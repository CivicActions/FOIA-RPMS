PSOR52 ;IHS/DSD/JCM - FILES REFILL ENTRIES IN PRESCRIPTION FILE [ 01/15/2003  1:37 PM ]
 ;;6.0;OUTPATIENT PHARMACY;**3,4**;11/11/2002
 ;;6.0;OUTPATIENT PHARMACY;**22,115**;09/03/97
 ...
 ;
 ;IHS/SD/lwj 6/24/03 (patch 4) DUR POS segment
 ;---------------------------------------------------------   
EN(PSOX) ;Entry Point
START ;
...
INIT ;
 S PSOR52("QFLG")=0
 ;S PSOX("AWP")=$S($D(^PSDRUG($P(PSOX("RX0"),"^",6),999999931)):$P(^(999999931),"^",2),1:"")  ;IHS/OKCAO/POC GETTING READY FOR AWP 11/12/98 8/18/2000
 ;S PSOX("NDC")=$S($D(^PSDRUG($P(PSOX("RX0"),"^",6),2)):$P(^(2),"^",4),1:"") ;IHS/OKCAO/POC SET THE NDC ON REFILLS 11/12/98 COMMENT OUT SEE PSOREF1 IHS/OKCAO/POC 8/18/2000
 S PSOX("QTY")=$P(PSOX("RX0"),"^",7),PSOX("DAYS SUPPLY")=$P(PSOX("RX0"),"^",8)
 ;S:$G(^PSDRUG($P(PSOX("RX0"),"^",6),660))]"" PSOX("COST")=$P(^PSDRUG($P(PSOX("RX0"),"^",6),660),"^",6) ;COMMENTED OUT SET IN PSOREF2 IHS/OKCAO/POC 1/21/2001 IMPROVE COST
 S PSOX("LOGIN DATE")=DT
 ...
 I PSOX("FILL DATE")>DT,$P(PSOPAR,"^",6) S PSOX("OLD MAIL/WINDOW")=PSOX("MAIL/WINDOW"),PSOX("MAIL/WINDOW")="M"
...
FILE ;     
 ...
 K PSOX1
 S:$P(PSOX("RX0"),"^",15)=6 $P(^PSRX(PSOX("IRXN"),0),"^",15)=0
...
FINISH ;
 ...
 . Q
 ;
 I $G(PSORX("PSOL",1))']"" S PSORX("PSOL",1)=PSOX("IRXN")_"," G FINISHX
