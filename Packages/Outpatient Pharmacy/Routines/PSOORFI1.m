PSOORFI1 ;BIR/SAB - finish OP orders from OE/RR continued ;01-Mar-2019 14:39;DU
 ;;7.0;OUTPATIENT PHARMACY;**7,15,23,27,32,44,51,46,71,90,108,131,152,1003,1005,1006,1008,1013,186,210,222,258,260,225,1015,1016,1017,1023**;DEC 1997;Build 121
 ...
 ;            IHS/MSC/MGH   05/17/17 - DRG+5,DS+4,PST+37,PST+63 Changed for EPCS
 S SIGOK=1
DSPL ;Display items
 K ^TMP("PSOPO",$J),CLOZPAT,PSOPRC,PSODSPL
...
DRG I $P($G(^PSDRUG(+$G(PSODRUG("IEN")),"CLOZ1")),"^")="PSOCLO1" D CLOZ^PSOORFI2
 ...
 .S PSODRUG("DIG")=$P(OR0,"^",24)         ;MGH EPCS
...
ISSDT S (PSOID,Y,PSONEW("ISSUE DATE"))=$S($G(PSONEW("ISSUE DATE")):PSONEW("ISSUE DATE"),$P($G(OR0),"^",6):$E($P(OR0,"^",6),1,7),1:DT)
 ...
 S PSONEW("EARLY")=$S($L($$VALUE^ORCSAVE2(+OR0,"EARLIEST")):$$VALUE^ORCSAVE2(+OR0,"EARLIEST"),1:"")  ;IHS/MSC/MGH add earliest fill date
...
DS S:$D(CLOZPAT) PSONEW("DAYS SUPPLY")=$S(CLOZPAT=2&(PSONEW("DAYS SUPPLY")>28):28,CLOZPAT=1&(PSONEW("DAYS SUPPLY")>14):14,'CLOZPAT&(PSONEW("DAYS SUPPLY")>7):7,1:PSONEW("DAYS SUPPLY"))
 ...
 ;IHS/MSC/MGH EPCS
 I $G(PKI1)!($G(PKI)=1)!($G(PKI)=10) D L1^PSOPKIV1 ;K:$G(PKI)=1!($G(PKI)=10) PKI
...
PST D DOSE^PSOORFI4 K PSOINSFL
 ...
 ;I $P(OR0,"^",24) S PSOACT=$S($D(^XUSEC("PSDRPH",DUZ)):"DEFX",1:"") D
 I +$G(PSODRUG("DEA"))>1,+$G(PSODRUG("DEA"))<6 S PSOACT=$S($D(^XUSEC("PSDRPH",DUZ)):"DEFX",1:"") D
...
ULK S X=PAT_";DPT(" D ULK^ORX2 S:$G(PSOQUIT) POERR("QFLG")=1 ; not called anymore
 Q
LOCK1 ;IHS/MSC/MGH modified for EPCS
...
EX K DRET,SIG,PSODRUG,PRC,PHI
 ...
 K PSOFDR,PSOQUIT,PSOFIN,^TMP("PSOAO",$J),^TMP("PSODA",$J),^TMP("PSOPO",$J),^TMP("PSOPF",$J),^TMP("PSOPI",$J),^TMP("PSOHDR",$J),MEDA,MEDP,APSPVER
