PSOPKIV2 ;BIR/MHA - Dig Signed Pending order Auto-DC message ;09-Jul-2018 14:50;DU
 ;;7.0;OUTPATIENT PHARMACY;**391,1023**;DEC 1997;Build 121
 ;IHS/MSC/MGH added for EPCS
 ;                 PRV+6 removed detox number
 ;                 MAIL entry point added for sending mail message
 ;
ADCMAIL ;
 ...
 Q
MAIL(PORD) ;EP
 N XX,QQ,ZZ,DATA,MSG,AIEN,DFN
 S ZZ="PSOPODC"
 K ^TMP(ZZ,$J)
 Q:'+PORD
 S AIEN=PORD_","
 D GETS^DIQ(52.41,AIEN,".01;1;1.1;5;11;12;13","E","DATA","MSG")
 S PAT=DATA(52.41,AIEN,1,"E")
 S DFN=$$GET1^DIQ(52.41,AIEN,1,"I")
 S PROV=$$GET1^DIQ(52.41,AIEN,5,"I")
 S XMSUB=$P(^PS(59,PSOSITE,0),"^",6)_" - DIGITALLY SIGNED "_$S($P(PORD,"^",3)="RNW":"RE",1:"")_"NEW ORDER AUTO DISCONTINUED",XMDUZ=.5
 S LC=1,^TMP(ZZ,$J,LC)="",LC=LC+1
 S ^TMP(ZZ,$J,LC)="Following order was auto discontinued when finishing a pending order due to a tampered order",LC=LC+1
 S ^TMP(ZZ,$J,LC)="",LC=LC+1
 S ^TMP(ZZ,$J,LC)="Division      : "_$P(^PS(59,PSOSITE,0),"^"),LC=LC+1
 S ^TMP(ZZ,$J,LC)="CPRS Order #  : "_$G(DATA(52.41,AIEN,.01,"E")),LC=LC+1
 S ^TMP(ZZ,$J,LC)="Patient       : "_PAT,LC=LC+1
 D PATAD
 S ^TMP(ZZ,$J,LC)="Drug          : "_DATA(52.41,AIEN,11,"E"),LC=LC+1
 S ^TMP(ZZ,$J,LC)="Quantity      : "_DATA(52.41,AIEN,12,"E")
 N TLC K TMP("ZZ") S XX=0,TLC=1,TMP("ZZ",1,0)="SIG           : "
 F  S XX=$O(^PS(52.41,PORD,"SIG",XX)) Q:'XX  D
 .S QQ=^PS(52.41,PORD,"SIG",XX,0)
 .D WORDWRAP^PSOUTLA2(QQ,.TLC,$NA(TMP("ZZ")),15)
 S XX=0 F  S XX=$O(TMP("ZZ",XX)) Q:'XX  S ^TMP(ZZ,$J,LC+1)=TMP("ZZ",XX,0)
 S LC=LC+1
 S PRV=DATA(52.41,AIEN,5,"E")
 S ^TMP(ZZ,$J,LC)="Provider      : "_PRV,LC=LC+1
 S DEA=$$DEA^XUSER(0,PROV)
 S DEA=$S(DEA["-":"VA#           : ",1:"DEA#          : ")_DEA
 S ^TMP(ZZ,$J,LC)=DEA,LC=LC+1
 S ^TMP(ZZ,$J,LC)="Location      : "_DATA(52.41,AIEN,1.1,"E"),LC=LC+1
 S LC=LC+1,^TMP(ZZ,$J,LC)=""
 S XMY("G.BEHO EPCS INCIDENT RESPONSE")=""
 S XMY(DUZ)="",XMTEXT="^TMP(ZZ,$J," N DIFROM
 D ^XMD K XMDUZ,XMTEXT,XMSUB
 K ^TMP(ZZ,$J)
 Q
...
PRV ;
 ...
 S DEA=$S(DEA["-":"VA#           : ",1:"DEA#          : ")_DEA
 S ^TMP(ZZ,$J,LC)=DEA
 ;IHS/MGH/MGH ihs not using detox numbers
