PSOORNE2 ;BIR/SAB-display finished orders from backdoor ;26-Jun-2019 11:11;DU
 ;;7.0;OUTPATIENT PHARMACY;**11,21,23,27,32,37,46,84,103,117,131,146,1004,1008,156,210,148,222,238,264,281,289,1015,1016,1017,1018,1023**;DEC 1997;Build 121
 ;^PSDRUG( -  221
 ;^YSCL(603.01 - 2697
 ;^PS(50.606 - 2174
 ;^PS(50.7 - 2223
 ...
 ;            IHS/MSC/MGH   05/17/17 - ACT+15,PTST+30 changed for EPCS
 ;            IHS/MSC/MGH   12/19/17 - ACT+33,NEWSEL+1
 ;            IHS/MSC/MGH   03/16/17 - ACT+20,24,30,32
...
NEWSEL N ORN,ORD D K2^PSOORNE6
 ...
 Q
 ;
ACT N REF,RPHKEY,PKIND,ORDZ K ^TMP("PSOAO",$J),PCOMX,PDA,PHI,PRC,ACOM,ANS,PSOFDR,CLOZPAT,ANQREM,DUR,DRET
 ...
 ;add a hardcopy RX to the list
 S PKIND=$D(^PSRX(RXN,"PKI"))!(+$P($G(^PSRX(RXN,999999941)),U,2))
 S RPHKEY=$S('PKIND&($D(^XUSEC("PSORPH",DUZ))):1,PKIND&($D(^XUSEC("PSDRPH",DUZ))):1,1:0)
 ;I $D(^XUSEC("PSORPH",DUZ)) S RPH=1 D
 I RPHKEY S RPH=1 D
 .;Disallow partials for controlled substances IHS/MSC/MGH
 .I +PKIND S PSOACT=$S('ST&($G(INDT)]"")&(DT>$G(INDT)):"DHPLAC",ST=1:"DVE",ST=4:"DV",ST=3:"DU",ST=5:"ELD",ST=11:"EDPCL",ST=12&EXDT:"EDCL",ST=12&'EXDT:"ECL",(ST=14!(ST=15))&'EXDT:"ECL",ST=13:"L",ST=16:"DL",1:"DHPEACL")  ;MGH
 .E  S PSOACT=$S('ST&($G(INDT)]"")&(DT>$G(INDT)):"DHPLATC",ST=1:"DVE",ST=4:"DV",ST=3:"DU",ST=5:"ELTD",ST=11:"ETDPCL",ST=12&EXDT:"EDCL",ST=12&'EXDT:"ECL",(ST=14!(ST=15))&'EXDT:"ECL",ST=13:"L",ST=16:"DL",1:"DHPEATCL")
 .;D GET^PSOORNE5 S PSOACT=PSOACT_$S(ACTREN:"N",1:""),PSOACT=PSOACT_$S(ACTREF:"R",1:"")
 .D GET^PSOORNE5
 .S PSOACT=PSOACT_$S(ACTREN:"N",1:"")
 .S PSOACT=PSOACT_$S(ACTREF:"R",1:"")
 .I ST=5 D
 ..I PKIND S SURX=$O(^PS(52.5,"B",RXN,0)) I SURX,$P($G(^PS(52.5,SURX,0)),"^",7)="L" S PSOACT="L" K SURX Q  ;MGH
 ..E  S SURX=$O(^PS(52.5,"B",RXN,0)) I SURX,$P($G(^PS(52.5,SURX,0)),"^",7)="L" S PSOACT="TL" K SURX Q
 .;end changes
 ...
 E  D
 .I ST=5 D
 ..;IHS/MSC/MGH Start Mod
 ..I +PKIND S SURX=$O(^PS(52.5,"B",RXN,0)) I SURX,$P($G(^PS(52.5,SURX,0)),"^",7)="L" S PSOACT="TL" Q  ;MGH
 ..E  S SURX=$O(^PS(52.5,"B",RXN,0)) I SURX,$P($G(^PS(52.5,SURX,0)),"^",7)="L" S PSOACT="TL" Q
 .E  D
 ..I +PKIND S PSOACT=$S(ST'<1&(ST'>4)!(ST>12):"",ST=12&EXDT&($P($G(PSOPAR),"^",2)):"CDPL",1:"CPL")   ;MGH
 ..E  S PSOACT=$S(ST'<1&(ST'>4)!(ST>12):"",ST=12&EXDT&($P($G(PSOPAR),"^",2)):"CDPLT",1:"CPLT")
 .;D GET^PSOORNE5 S PSOACT=PSOACT_$S(ACTREN:"N",1:""),PSOACT=PSOACT_$S(ACTREF:"R",1:"")
 .D GET^PSOORNE5
 .;Add renewal and refill only if not CS medication
 .I +PKIND S PSOACT=PSOACT_$S(ACTREF&($D(^XUSEC("PSORPH",DUZ))):"N",1:"")
 .E  S PSOACT=PSOACT_$S(ACTREN:"N",1:"")
 .I +PKIND S PSOACT=PSOACT_$S(ACTREF&($D(^XUSEC("PSORPH",DUZ))):"R",1:"")
 .E  S PSOACT=PSOACT_$S(ACTREF:"R",1:"")
 .;End Mods
...
PTST ;
 ...
 I +$P($G(^PSDRUG($P(RX0,"^",6),0)),"^",3)>1,+$P($G(^PSDRUG($P(RX0,"^",6),0)),"^",3)<6 D
 .S ORDZ=$P(RXOR,"^",2)
 .D PRV^PSOORNE5
