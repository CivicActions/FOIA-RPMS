PSOBKDED ;BIR/SAB - Edit backdoor Rx Order entry ;15-May-2024 13:49;DU
 ;;7.0;OUTPATIENT PHARMACY;**11,46,91,78,99,117,133,143,268,1025,1035**;DEC 1997;Build 39
 ...
 ;Ref PS(55 IA 2228
 ;Modified - IHS/MSC/MGH - 11/27/19 - Check for liquids GD+3,ASK1+4
 ;         - IHS/MSC/PLS - 05/14/2024 - SCH+1 - FID 103810
...
RTE I $G(DRET) S PSORXED("ROUTE",ENT)=""
 ...
 Q
ASK K JUMP,UNITN,DOSE,SDOSE,LIQUID D KV D DOSE^PSSORPH(.DOSE,PSODRUG("IEN"),"O",PSODFN)
...
ASK1 S STRE=$P($G(DOSE("DD",PSODRUG("IEN"))),"^",5),UNITN=$P($G(DOSE("DD",PSODRUG("IEN"))),"^",6),DOSE("LD")=$P($G(DOSE("DD",PSODRUG("IEN"))),"^",11)
 ...
 ;IHS/MSC/MGH Liquid check
 S PSSLIQ=$$LIQUID^APSPLIQ(PSODRUG("OI"))
...
DOS I Y="^" Q
 W " "_$S($E($G(DOSE),1)="."&($G(UNITN)'=""):"0",1:"")_$G(DOSE) W:$G(UNITN)'="" UNITN
 ;W ! K DIR,DIRUT S DIR(0)="Y",DIR("A")="You entered "_$S($E(DOSE,1)="."&($G(UNITN)'=""):"0",1:"")_DOSE_$S($G(UNITN)'="":UNITN,1:"")_" is this correct",DIR("B")="Yes"
 W ! K DIR,DIRUT S DIR(0)="Y",DIR("A")="You entered "_DOSE_" is this correct",DIR("B")="Yes"
 D ^DIR I 'Y D KX K DOSE,UNITS,PSORXTE,PSOINDT,PSSLIQ,PSSMET G ASK
 N ERR
 D ZEROS^APSPLIQ(.ERR,DOSE)
 I $D(ERR)>1 D
 .N LOOP
 .F LOOP=1:1:ERR W !,$G(ERR(LOOP))
 ;IHS/MSC/MGH Check for liquid units
 S PSSMET=0
 I +PSSLIQ D
 .S PSSMET=$$PARSE^APSPLIQ(SDOSE)
 .I +PSSMET W !,"Dosage units for Oral Liquids must be standard metric units."
 I +PSSMET!($D(ERR)>1) G ASK
...
SCH D KX
 ...
 S DIR("?")="^D SCHLP^PSOORED4",DIR("A")="Schedule: ",DIR(0)="FA^1:24^I X[""""""""!(X?.E1C.E)!($A(X)=45)!($L(X)>24)!($L(X)<1) K X"
