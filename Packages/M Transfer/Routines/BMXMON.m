BMXMON ; IHS/OIT/HMW - BMXNet MONITOR ; 04 Jun 2010  3:10 PM
 ;;4.0;BMX;**2,5**;OCT 27, 2011;Build 3
 ;
...
RELOAD C BMXDEV ;IHS/OIT/HMW SAC Exemption Applied For ; IHS/OIT/GIS 10/4/2011
 ...
 Q
CONNECT ;
 ; IHS/OIT/KJH 06/14/2019 - Added a new error trap.
 ; Port scanners may open and close a port causing a <READ> error. This needs to be trapped or the listener stops.
 ; NOTE: This code was previously part of the RELOAD tag as a "dotted DO".
 ;IHS/OIT/HMW SAC Exemption Applied For
 N $ESTACK S $ETRAP="D CONNERR^BMXMON"
 R BMXACT#5:BMXDTIME ;Read first 5 chars from TCP buffer, timeout=BMXDTIME ;IHS/OIT/HMW SAC Exemption Applied For
 S BMXTERM=$ZA\2#2 ; Save the value of $ZA for later use so we can distinguish between a timeout and a NULL value.
 ;I BMXACT="" S BMXQUIT=1 Q  ; LISTENER TIMED OUT AFTER 999999 SECONDS (10 DAYS) ; IHS/OIT/GIS 10/4/2011
 I BMXACT="",BMXTERM=1 S BMXQUIT=1 Q  ; LISTENER TIMED OUT AFTER 999999 SECONDS (10 DAYS) ; IHS/OIT/GIS 10/4/2011
 I BMXACT="",BMXTERM=0 S BMXQUIT=2 Q  ; NO DATA RECEIVED FROM CONNECTION. MOST LIKELY A PORT SCANNER. NEED TO CLEAN OUT TCP BUFFER AND START OVER.
 ...
 Q
 ;
CONNERR ; On trapped <READ> or similar error, set variable to recycle the listener process.
 ...
 I (BMXERC["READ")!(BMXERC["WRITE")!(BMXERC["SYSTEM-F") S BMXQUIT=2 ; Restart the process on known errors.
 E  S BMXQUIT=1 ; Otherwise, unknown error. Don't attempt to restart the process.
 Q
 ; 
SESSION(BMXWIN) ;EP
