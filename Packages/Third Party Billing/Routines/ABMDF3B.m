ABMDF3B ; IHS/ASDST/DMJ - Set HCFA1500 Print Array PART 2 ;   [ 02/26/2003  12:21 PM ]
 ;;2.5;IHS 3P BILLING SYSTEM;**3,8,9,10**;APR 05, 2002
 ;Original;TMD;
 ;
...
BNODES S ABM("B5")=$G(^ABMDBILL(DUZ(2),ABMP("BDFN"),5)),ABM("B6")=$G(^(6)),ABM("B7")=$G(^(7)),ABM("B8")=$G(^(8)),ABM("B9")=$G(^(9)),ABM("B10")=$G(^(10))
 ;I $P(ABM("B5"),U,8)]"" S $P(ABMF(33),U,3)=$P(ABM("B5"),U,8)  ;abm*2.5*10 IM21068
 I $P(ABM("B5"),U,12)]"" S $P(ABMF(33),U,3)=$P(ABM("B5"),U,12)  ;abm*2.5*10 IM21068
 I $P($G(^DIC(40.7,ABMP("CLN"),0)),U)="AMBULANCE" S $P(ABMF(33),U,3)=$E($P($G(^ABMDBILL(DUZ(2),ABMP("BDFN"),12)),U,6),1,5)  ;IHS/SD/SDR 7/22/05 task 6
 ;start new code abm*2.5*10 IM21068
 I +$O(^ABMDBILL(DUZ(2),ABMP("BDFN"),37,0))'=0 D
 .;I $P($G(^ABMDBILL(DUZ(2),ABMP("BDFN"),8)),U)'="" S $P(ABMF(33),U,3)=$P($G(^ABMRLABS($P($G(^ABMDBILL(DUZ(2),ABMP("BDFN"),9)),U,23),0)),U,2)  ;abm*2.5*10 IM21468
 .I +$P($G(^ABMDBILL(DUZ(2),ABMP("BDFN"),8)),U)'=0,($P($G(^ABMDBILL(DUZ(2),ABMP("BDFN"),9)),U,23)'="") S $P(ABMF(33),U,3)=$P($G(^ABMRLABS($P($G(^ABMDBILL(DUZ(2),ABMP("BDFN"),9)),U,23),0)),U,2)  ;abm*2.5*10 IM21468
 .;E  S $P(ABMF(33),U,3)=$P($G(^ABMDBILL(DUZ(2),ABMP("BDFN"),9)),U,22)  ;default in-house  ;abm*2.5*10 IM21468
 .I +$P($G(^ABMDBILL(DUZ(2),ABMP("BDFN"),8)),U)=0 S $P(ABMF(33),U,3)=$P($G(^ABMDBILL(DUZ(2),ABMP("BDFN"),9)),U,22)  ;default in-house  ;abm*2.5*10 IM21468
...
REFR S $P(ABMF(27),U)=$P(ABM("B8"),U,8),$P(ABMF(27),U,2)=$P(ABM("B8"),U,11)
 ; start old code IHS/SD/SDR NDA-0402-180192
BLK19 ;S $P(ABMF(29),"^",1)=$P(ABM("B10"),"^",1)
 ; end old code IHS/SD/SDR NDA-0402-180192
 ; start new code IHS/SD/SDR NDA-0402-180192
 S ABMBLK19=$$SDT^ABMDUTL($P(ABM("B9"),U,11))  ;date last seen
 S ABMBLK19=ABMBLK19_" "_$P(ABM("B9"),U,24)  ;supervising prov UPIN abm*2.5*9 IM19291
 S ABMBLK19=ABMBLK19_" "_$P(ABM("B9"),U,12)  ;supervising prov
 ;S ABMBLK19=ABMBLK19_" "_$P(ABM("B9"),U,13)  ;date last x-ray  abm*2.5*10
 S ABMBLK19=ABMBLK19_" "_$$SDT^ABMDUTL($P(ABM("B9"),U,13))  ;last x-ray  ;abm*2.5*10
 ...
 K ABMBLK19
 ; end new code IHS/SD/SDR NDA-0402-180192
