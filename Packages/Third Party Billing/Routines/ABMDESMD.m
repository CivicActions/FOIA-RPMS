ABMDESMD ; IHS/SD/SDR - Summarized Claim Info - DENTAL ;    
 ;;2.6;IHS 3P BILLING SYSTEM;**28,29**;NOV 12, 2009;Build 562
 ;
 ;IHS/ASDS/LSL 09/21/00 V2.4 Patch 3 NOIS HQW-0900-100053 Some payers require a prefix of S, D, or 0 to ADA code
 ;
 ;IHS/SD/SDR 10/03/02 V2.5 P2 NHA-0302-180192 Modified routine to get the units instead of hardset to 1 and
 ;     to calculate charges based on units
 ;IHS/SD/EFG V2.5 P8 IM16385 Remove quit for ADA-90/ADA-94 formats
 ;IHS/SD/SDR v2.5 p13 NO IM Change to print provider# on dental line items
 ;
 ;IHS/SD/SDR v2.6 CSV
 ;IHS/SD/SDR 2.6*28 CR8340 Added 3 modifiers to ABMS array
...
DENU S ABMX("R")=$P(ABMX(0),U,2) Q:ABMX("R")=""
 ...
 S $P(ABMS(ABMX("R")),U,2)=+$P(ABMS(ABMX("R")),U,2)+ABMUNIT  ;abm*2.6*29 IHS/SD/SDR CR10888
 ;start new abm*2.6*29 IHS/SD/SDR CR10410
 S ABMCNDCK=U_$P(ABMX(0),U,13)_U_$P(ABMX(0),U,14)_U_$P(ABMX(0),U,15)
 D CNCD21CK^ABMDESM1
...
DENH S ABMS(ABMS("I"))=ABMX("SUB")
 ...
 F ABMI=13,14,15 D
 .S $P(ABMS(ABMS("I")),U,4)=$P(ABMS(ABMS("I")),U,4)_$S($P(ABMX(0),U,ABMI)'="":" "_$P(ABMX(0),U,ABMI),1:"")
