ABMEF22 ; IHS/ASDST/DMJ - Electronic 837 version 4010 Professional ;      
 ;;2.6;IHS Third Party Billing System;**2,3,8,10**;NOV 12, 2009;Build 43
 ;
 ; IHS/SD/SDR - v2.5 p8 - IM15585
 ;    Added code to check when patient changes, not just
 ;    subscriber change
 ;
 ; IHS/SD/SDR - v2.5 p10 - COB issue
 ;   Moved where ABMP array was getting killed; it was
 ;   happening too soon and data wasn't there for 2nd claim
 ;   for same patient if there was such an animal
...
CLAIM ;one claim
 ...
 ;D BILLSTAT^ABMDREEX(DUZ(2),ABMP("BDFN"),ABMPXMIT,"O",$P($G(^ABMDTXST(DUZ(2),ABMPXMIT,1)),U,6))  ;abm*2.6*2 5PMS10005
 D BILLSTAT^ABMDREEX(DUZ(2),ABMP("BDFN"),ABMPXMIT,"O",ABMGCN)  ;abm*2.6*2 5PMS10005
