ABMERSND ; IHS/ASDST/DMJ - RE-SEND A BATCH OF BILLS ELECTRONIC FORMAT ; 
 ;;2.6;IHS Third Party Billing System;**2,6,8,10,35,37**;NOV 12, 2009;Build 739
 ;Original;DMJ;
 ;IHS/SD/SDR 2.6*2 5PMS10005 - Set var for 3P Bill EXPORT NUMBER RE-EXPORT multiple
 ;IHS/SD/SDR 2.6*6 5010 - Changes for clearinghouse
 ;IHS/SD/SDR 2.6*35 ADO60705 Fixed <SUBSCR>BATCHLST+19^ABMERSND when recreating a batch
 ;IHS/SD/SDR 2.6*37 ADO75923 Fixed file lookup to use GCN; was using GCN and IEN without forcing the lookup to use
 ;   the right xref
 ;
START ;START HERE   
 W !
 ...
 S DIC(0)="AEMQ"
 S DIC("S")="I $L($P($G(^(1)),""^"",4))"
 ;D ^DIC  ;abm*2.6*37 IHS/SD/SDR ADO89299
 S D="EGCN^B^D"  ;abm*2.6*37 IHS/SD/SDR ADO89299
 D MIX^DIC1  ;abm*2.6*37 IHS/SD/SDR ADO89299
 K DIC
 Q:Y<0
 S ABMP("XMIT")=+Y
...
LIST ; EP
 ...
 I '$D(ABMT("XLIST")) D
 .F  S ABMT("XMIT")=$O(^ABMDTXST(DUZ(2),"C",ABMT("ORIGGCN"),ABMT("XMIT"))) D  Q:'+ABMT("XMIT")
 ..I $G(ABMT("GCN"))="" S ABMT("GCN")=$P($G(^ABMDTXST(DUZ(2),ABMT("XMIT"),1)),U,6)
 ..Q:(ABMT("GCN")'=ABMT("ORIGGCN"))
 ..S ABMT("CHKLIST",ABMT("XMIT"))=1
 ...
 ..D BATCHLST
 ..K DIR,DIE,DIC,X,Y,DA
 ..;S DIR(0)="NO^1:"_(ABMT("XCNT")-1)  ;abm*2.6*10 HEAT68097
 ..S DIR(0)="NO^1:"_(ABMT("XCNT"))  ;abm*2.6*10 HEAT68097
 ..S DIR("A")="Select"
 ..D ^DIR K DIR
 ..I +Y=0 S ABMQUIT=1
 ..S ABMSEL=Y
 ...
 .K ABMT("XLIST",ABMSEL)  ;abm*2.6*10 HEAT68097
...
BATCHLST ;
 W !
 S ABME("XCNT")=0
 ;F  S ABME("XCNT")=$O(ABMP("XLIST",ABME("XCNT"))) Q:'ABME("XCNT")  D  ;abm*2.6*10 HEAT68097
 F  S ABME("XCNT")=$O(ABMT("XLIST",ABME("XCNT"))) Q:'ABME("XCNT")  D  ;abm*2.6*10 HEAT68097
 .S ABMP("XMIT")=0
 .;F  S ABMP("XMIT")=$O(ABMP("XLIST",ABME("XCNT"),ABMP("XMIT"))) Q:'ABMP("XMIT")  D  ;abm*2.6*10 HEAT68097
 .F  S ABMP("XMIT")=$O(ABMT("XLIST",ABME("XCNT"),ABMP("XMIT"))) Q:'ABMP("XMIT")  D  ;abm*2.6*10 HEAT68097
 ..;I $D(ABMP("XLIST",ABME("XCNT"),ABMP("XMIT")))<11 D  Q  ;this is for entries created prior to p6 install  ;abm*2.6*10 HEAT68097
 ..I $D(ABMT("XLIST",ABME("XCNT"),ABMP("XMIT")))<11 D  Q  ;this is for entries created prior to p6 install  ;abm*2.6*10 HEAT68097
 ...
 ...;W ABME("XCNT"),?3,$$BDT^ABMDUTL($P($G(^ABMDTXST(DUZ(2),ABMP("XMIT"),0)),U)),?23,ABMP("GCN")  ;abm*2.6*10 HEAT68097
 ...W ABME("XCNT"),?3,$$BDT^ABMDUTL($P($G(^ABMDTXST(DUZ(2),ABMP("XMIT"),0)),U)),?23,ABMT("GCN")  ;abm*2.6*10 HEAT68097
 ...W ?30,$P($G(^ABMDEXP($P($G(^ABMDTXST(DUZ(2),ABMP("XMIT"),0)),U,2),0)),U)
 ...W ?47,ABMP("ITYP")
 ...;W:$P($G(^ABMDTXST(DUZ(2),ABMP("XMIT"),3,ABMP("SIEN"),0)),U,7) ?59,$P($G(^ABMRECVR($P($G(^ABMDTXST(DUZ(2),ABMP("XMIT"),3,ABMP("SIEN"),0)),U,7),0)),U)  ;abm*2.6*10 HEAT68097
 ...W:$P($G(^ABMDTXST(DUZ(2),ABMP("XMIT"),3,ABMT("SIEN"),0)),U,7) ?59,$P($G(^ABMRECVR($P($G(^ABMDTXST(DUZ(2),ABMP("XMIT"),3,ABMT("SIEN"),0)),U,7),0)),U)  ;abm*2.6*10 HEAT68097
 ...W !?23,$P($G(^AUTNINS($P($G(^ABMDTXST(DUZ(2),ABMP("XMIT"),0)),U,4),0)),U)
 ...;W ?46,$P($G(^VA(200,$P($G(^ABMDTXST(DUZ(2),ABMP("XMIT"),0)),U,5),0)),U)  ;abm*2.6*35 IHS/SD/SDR ADO60705
 ...W:$P($G(^ABMDTXST(DUZ(2),ABMP("XMIT"),0)),U,5) ?46,$P($G(^VA(200,$P($G(^ABMDTXST(DUZ(2),ABMP("XMIT"),0)),U,5),0)),U)  ;abm*2.6*35 IHS/SD/SDR ADO60705
 ...;W:$P($G(^ABMDTXST(DUZ(2),ABMP("XMIT"),3,ABMP("SIEN"),0)),U,6) ?70,"ST02: ",$$FMT^ABMERUTL($P($G(^ABMDTXST(DUZ(2),ABMP("XMIT"),3,ABMP("SIEN"),0)),U,6),"4NR")  ;abm*2.6*10 HEAT68097
 ...W:$P($G(^ABMDTXST(DUZ(2),ABMP("XMIT"),3,ABMT("SIEN"),0)),U,6) ?70,"ST02: ",$$FMT^ABMERUTL($P($G(^ABMDTXST(DUZ(2),ABMP("XMIT"),3,ABMT("SIEN"),0)),U,6),"4NR")  ;abm*2.6*10 HEAT68097
 ...W !
 ..S ABMP("SIEN")=0
