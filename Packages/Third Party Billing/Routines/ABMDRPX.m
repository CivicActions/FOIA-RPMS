ABMDRPX ; IHS/ASDST/DMJ - CPT Summary Report ; 
 ;;2.6;IHS Third Party Billing;**1,37**;NOV 12, 2009;Build 739
 ;Original;TMD;10/20/95 3:37 PM
 ;
 ;IHS/SD/SDR 2.5*2 10/21/02 UXX-1002-170028 Modified so report would print data second time if same session
 ;
 ;IHS/SD/SDR 2.6 CSV
 ;IHS/SD/SDR 2.6*1 HEAT4716 Include NDC on RXs
 ;IHS/SD/SDR 2.6*37 ADO75981 Included ambulance (47 multiple)
 ;
...
DATA ;
 ...
 I +$G(ABM("ALL"))=1 K ABM("SUB")  ;abm*2.6*37 IHS/SD/SDR ADO75981
 S ABMP("HIT")=0
 ...
 ;I +ABM("SUB") D ONE  ;abm*2.6*37 IHS/SD/SDR ADO75981
 I +$G(ABM("SUB")) D ONE  ;abm*2.6*37 IHS/SD/SDR ADO75981
 I ((+$G(ABM("ALL"))=0)&(+$G(ABM("SUB"))=43)) S ABM("SUB")=47 D ONE  ;abm*2.6*37 IHS/SD/SDR ADO75981
 I $G(ABM("ALL")) D
 .;F ABM("SUB")=21,23,25,27,33,35,37,39,43 D  ;abm*2.6*37 IHS/SD/SDR ADO75981
 .F ABM("SUB")=21,23,25,27,33,35,37,39,43,47 D  ;abm*2.6*37 IHS/SD/SDR ADO75981
...
ONE ;
 ; ONE CATEGORY
 ;I $D(ABMY("PX")),"23^25^33^43"[ABM("SUB") Q  ;abm*2.6*37 IHS/SD/SDR ADO75981
 I $D(ABMY("PX")),"23^25^33^43^47^"[ABM("SUB") Q  ;abm*2.6*37 IHS/SD/SDR ADO75981
 S ABM("PX")=0
 ...
 .;I ABM("SUB")=35!(ABM("SUB")=37)!(ABM("SUB")=43) S ABM("FEE")=$P(ABM(0),U,3)*$P(ABM(0),U,4)  ;abm*2.6*37 IHS/SD/SDR ADO75981
 .I ABM("SUB")=35!(ABM("SUB")=37)!(ABM("SUB")=43)!(ABM("SUB")=47) S ABM("FEE")=$P(ABM(0),U,3)*$P(ABM(0),U,4)  ;abm*2.6*37 IHS/SD/SDR ADO75981
 .I ABM("SUB")=39 D
...
TL ;
 ...
 ;S $P(^TMP("ABM-PX",$J),U,2)=$P(^TMP("ABM-PX",$J),U,2)+ABM("FEE")  ;abm*2.6*37 IHS/SD/SDR ADO75981
 S $P(^TMP("ABM-PX",$J),U,2)=$P($G(^TMP("ABM-PX",$J)),U,2)+ABM("FEE")  ;abm*2.6*37 IHS/SD/SDR ADO75981
