ABMAROLL ; IHS/ASDST/DMJ - A/R ROLL OVER ;     
 ;;2.6;IHS 3P BILLING SYSTEM;**10,34,37,42**;NOV 12, 2009;Build 789
 ;Original;DMJ;
 ;
 ;IHS/DSD/LSL 03/30/98 Don't complete a cancelled bill. This will cause errors on productivity
 ;   report and previous payments of bills.
 ;IHS/DSD/DMJ 07/23/99 NOIS PEB-0799-90049 Patch 3 #8 Changed from the DIC("DR") method to updating
 ;   each field individually. Apparently if one field in the string fails, fileman doesn't file anything.
 ;IHS/ASDS/SDH 2.4*9 08/14/01 NOIS NDA-1199-180065 Modified routine to subtract refund instead
 ;   of add (two negatives makes a positive)
 ;
 ;IHS/SD/SDR 2.5*2 5/28/02 NOIS HQW-1201-100023 Modified so that the export date will not be deleted
 ;    anymore.  This was causing problems with some reports not showing correct data because of this date missing.
 ;IHS/SD/SDR 2.5*8 6/6/05 Made LKUP and FILE published entry points per A/R
 ;IHS/SD/SDR 2.5*9 IM19622 Fixed so replacement insurer is checked when updating status
 ...
 ;IHS/SD/SDR 2.6*37 ADO76009 Added call to populate ACTIVE INSURER PI MULTIPLE to keep track of which insurer in case pt has same insurer twice
 ;IHS/SD/SDR 2.6*42 ADO107388 During rollback, if claim is already 'In Edit Mode' leave the insurer status alone
 ;
 ; *********************************************************************
 ;
...
REST ;
 ...
 .I $P(^ABMDCLM(DUZ(2),ABMP("CDFN"),0),"^",4)="E" Q  ;claim is back in edit mode already so leave it  ;abm*2.6*42 IHS/SD/SDR ADO107388
...
UNBILL ;
 ...
 ..;Q:$P($G(^AUTNINS(ABM("INSCO"),2)),U)="I"  ;abm*2.6*10 HEAT73780
 ..Q:$$GET1^DIQ(9999999.181,$$GET1^DIQ(9999999.18,ABM("INSCO"),".211","I"),1,"I")="I"  ;abm*2.6*10 HEAT73780
 ...
 K ABM,ABMP
 S Y=$$ACTINSM^ABMDE2E(DA(1),DA)  ;abm*2.6*37 IHS/SD/SDR ADO76009
 Q
 ;
ADD ;
...
OCLM ;
 ...
 ;I $P($G(^ABMNINS(DUZ(2),ABMP("INS"),0)),U,11)="Y"&($P($G(^AUTNINS(ABM(1),2)),U)="R") D  ;abm*2.6*10 HEAT73780
 I $P($G(^ABMNINS(DUZ(2),ABMP("INS"),0)),U,11)="Y"&($$GET1^DIQ(9999999.181,$$GET1^DIQ(9999999.18,ABMP("INS"),".211","I"),1,"I")="R") D  ;abm*2.6*10 HEAT73780
 .K DIC,DIE,X,Y,DA
 .S DA(1)=ABMP("CDFN")
