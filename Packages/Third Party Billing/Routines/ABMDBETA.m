ABMDBETA ; IHS/ASDST/DMJ - Routine to Update Beta Sites ;   
 ;;2.6;IHS 3P BILLING SYSTEM;;NOV 12, 2009;Build 43
 ;
 I '$D(DUZ(2)) D ^XBKVAR
 W !!!,"Wait, processing..."
 ;S DA=$O(^AUTNINS("B","BENEFICIARY PATIENT (INDIAN)","")) I DA,$P($G(^AUTNINS(DA,2)),U)'="I" S DR=".21////I",DIE="^AUTNINS(" D ^ABMDDIE Q:$D(ABM("DIE-FAIL"))  ;abm*2.6*10 HEAT73780
 S DA=$O(^AUTNINS("B","BENEFICIARY PATIENT (INDIAN)","")) I DA,$$GET1^DIQ(9999999.181,$$GET1^DIQ(9999999.18,DA,".211","I"),1,"I")'="I" S DR=".211////I",DIE="^AUTNINS(" D ^ABMDDIE Q:$D(ABM("DIE-FAIL"))  ;abm*2.6*10 HEAT73780
...
UPDT S DIE="^ABMDERR(",DR=".03////E;.05////1"
 ...
 ;I DA S DIE="^AUTNINS(",DR=".17////2;.21////W;.41////WORKMEN'S COMPENSATION" D ^ABMDDIE Q:$D(ABM("DIE-FAIL"))  ;abm*2.6*10 HEAT73780
 I DA S DIE="^AUTNINS(",DR=".17////2;.211////W;.41////WORKMEN'S COMPENSATION" D ^ABMDDIE Q:$D(ABM("DIE-FAIL"))  ;abm*2.6*10 HEAT73780
