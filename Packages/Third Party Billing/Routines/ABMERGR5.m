ABMERGR5 ; IHS/SD/SDR - GET ANCILLARY SVCS REVENUE CODE INFO ; 
 ...
 ;IHS/SD/SDR 2.6*28 CR8340 Added 3 modifiers to print on 837I for dental (33 mult)
...
27 ;EP - Medical Procedures
 ...
 .;S $P(ABMRV(+ABM(2),ABM(1),ABMLCNT),U,6)=(ABM(3)*ABM(4))  ;cumulative charges  ;abm*2.6*29 IHS/SD/SDR CR10410
...
33 ;EP - Dental
 ...
 .;F J=1,2,7,8,9 S ABM(J)=$P(^ABMDBILL(DUZ(2),ABMP("BDFN"),33,DA,0),"^",J)  ;abm*2.6*28 IHS/SD/SDR CR8340
 .F J=1,2,7,8,9,13,14,15 S ABM(J)=$P(^ABMDBILL(DUZ(2),ABMP("BDFN"),33,DA,0),"^",J)  ;abm*2.6*28 IHS/SD/SDR CR8340
 .S:'+ABM(9) ABM(9)=1
 ...
 .S $P(ABMRV(+ABM(2),+ABM("DCODE"),ABMLCNT),U,3)=ABM(13)  ;modifier  ;abm*2.6*28 IHS/SD/SDR CR8340
 .S $P(ABMRV(+ABM(2),+ABM("DCODE"),ABMLCNT),U,4)=ABM(14)  ;2nd modifier  ;abm*2.6*28 IHS/SD/SDR CR8340
 .S $P(ABMRV(+ABM(2),+ABM("DCODE"),ABMLCNT),U,5)=ABM(9)  ;units
 .;S $P(ABMRV(+ABM(2),+ABM("DCODE"),ABMLCNT),U,6)=(ABM(8)*ABM(9))  ;charges  ;abm*2.6*29 IHS/SD/SDR CR10410
 .S $P(ABMRV(+ABM(2),+ABM("DCODE"),ABMLCNT),U,9)=$P(^AUTTADA(ABM(1),0),U,2)  ;ADA Desc
 .S $P(ABMRV(+ABM(2),+ABM("DCODE"),ABMLCNT),U,10)=ABM(7)  ;DoS
 .S $P(ABMRV(+ABM(2),+ABM("DCODE"),ABMLCNT),U,12)=ABM(15)  ;3rd modifier  ;abm*2.6*28 IHS/SD/SDR CR8340
 .D GYCHK^ABMDESM1(+ABM(2),+ABM("DCODE"),ABMLCNT,ABM(8),ABM(9))  ;covered vs non-covered charges  ;abm*2.6*29 IHS/SD/SDR CR10410
