...
OVER ;get override values from 3p insurer file
 ;S ABMOLN=0 F  S ABMOLN=$O(^ABMNINS(DUZ(2),ABMP("INS"),2,"AOVR",3,ABMOLN)) Q:'ABMOLN  D  ;abm*2.6*10 HEAT53137
 S ABMOLN=0 F  S ABMOLN=$O(^ABMNINS(ABMP("LDFN"),ABMP("INS"),2,"AOVR",3,ABMOLN)) Q:'ABMOLN  D  ;abm*2.6*10 HEAT53137
 .;S ABMOPC=0 F  S ABMOPC=$O(^ABMNINS(DUZ(2),ABMP("INS"),2,"AOVR",3,ABMOLN,ABMOPC)) Q:'ABMOPC  D  ;abm*2.6*10 HEAT53137
 .S ABMOPC=0 F  S ABMOPC=$O(^ABMNINS(ABMP("LDFN"),ABMP("INS"),2,"AOVR",3,ABMOLN,ABMOPC)) Q:'ABMOPC  D  ;abm*2.6*10 HEAT53137
 ..K ABMOVTYP
 ..;I $D(^ABMNINS(DUZ(2),ABMP("INS"),2,"AOVR",3,ABMOLN,ABMOPC,0)) S ABMOVTYP=0  ;abm*2.6*10 HEAT53137
 ..I $D(^ABMNINS(ABMP("LDFN"),ABMP("INS"),2,"AOVR",3,ABMOLN,ABMOPC,0)) S ABMOVTYP=0  ;abm*2.6*10 HEAT53137
 ..;I $D(^ABMNINS(DUZ(2),ABMP("INS"),2,"AOVR",3,ABMOLN,ABMOPC,ABMP("VTYP"))) S ABMOVTYP=ABMP("VTYP")  ;abm*2.6*10 HEAT53137
 ..I $D(^ABMNINS(ABMP("LDFN"),ABMP("INS"),2,"AOVR",3,ABMOLN,ABMOPC,ABMP("VTYP"))) S ABMOVTYP=ABMP("VTYP")  ;abm*2.6*10 HEAT53137
 ..Q:'$D(ABMOVTYP)
 ..;S ABMVALUE=^ABMNINS(DUZ(2),ABMP("INS"),2,"AOVR",3,ABMOLN,ABMOPC,ABMOVTYP)  ;abm*2.6*10 HEAT53137
 ..S ABMVALUE=^ABMNINS(ABMP("LDFN"),ABMP("INS"),2,"AOVR",3,ABMOLN,ABMOPC,ABMOVTYP)  ;abm*2.6*10 HEAT53137
