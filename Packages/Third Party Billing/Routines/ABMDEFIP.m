...
DBFX(X,Y)         ;EP FIX BILL INSURANCE MULTIPLE IF BROKEN POINTER MEDICAID
 ...
 S:'$G(ABMDUZ2) ABMDUZ2=DUZ(2)  ;abm*2.5*10 IM20247
 N ABMP
 S ABMP("D0")=X,ABMP("D1")=Y
 ;start old code abm*2.5*10 IM20247
 ;S ABMP("ZERO")=^ABMDBILL(DUZ(2),ABMP("D0"),13,ABMP("D1"),0)
 ;S ABMP("PDFN")=$P(^ABMDBILL(DUZ(2),ABMP("D0"),0),"^",5)
 ;S ABMP("VDT")=$P(^ABMDBILL(DUZ(2),ABMP("D0"),7),"^",1)
 ;end old code start new code IM20247
 ;S ABMP("ZERO")=^ABMDBILL(ABMDUZ2,ABMP("D0"),13,ABMP("D1"),0)  ;abm*2.5*11 IM23904
 S ABMP("ZERO")=$G(^ABMDBILL(ABMDUZ2,ABMP("D0"),13,ABMP("D1"),0))  ;abm*2.5*11 IM23904
 S ABMP("PDFN")=$P(^ABMDBILL(ABMDUZ2,ABMP("D0"),0),U,5)
 S ABMP("VDT")=$P(^ABMDBILL(ABMDUZ2,ABMP("D0"),7),U)
 ;end new code abm*2.5*10 IM20247
 D MGET
 ;I $G(ABMP(1)) S $P(^ABMDBILL(DUZ(2),ABMP("D0"),13,ABMP("D1"),0),"^",6)=ABMP(1),$P(^(0),"^",7)=ABMP(2)  ;abm*2.5*10 IM20247
 I $G(ABMP(1)) S $P(^ABMDBILL(ABMDUZ2,ABMP("D0"),13,ABMP("D1"),0),U,6)=ABMP(1),$P(^(0),U,7)=ABMP(2)  ;abm*2.5*10 IM20247
 Q
DCFX(X,Y)        ;EP FIX CLAIM INSURANCE MULTIPLE IF BROKEN POINTER, MEDICAID
 N ABMP
 S ABMP("D0")=X,ABMP("D1")=Y
 S ABMP("ZERO")=^ABMDCLM(DUZ(2),ABMP("D0"),13,ABMP("D1"),0)
 S ABMP("PDFN")=$P(^ABMDCLM(DUZ(2),ABMP("D0"),0),"^",1)
...
MGET ;GET NEW POINTER
 S ABMP("INSCO")=$P(ABMP("ZERO"),"^",1)
 Q:$P($G(^AUTNINS(ABMP("INSCO"),2)),"^",1)'="D"
 S ABMP("PTR")=$P(ABMP("ZERO"),"^",6)
 S DFN=ABMP("PDFN")
 Q:$P($G(^AUPNMCD(+ABMP("PTR"),0)),"^",1)=DFN
 S ABMPVDT=ABMP("VDT")
