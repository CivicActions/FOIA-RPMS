ABMDE4A ; IHS/ASDST/DMJ - PAGE 4 - PROVIDERS VIEW ; 
 ;;2.5;IHS Third Party Billing System;**10,11**;APR 24, 2006
 ;
...
ATTN I '$D(ABM("A")) G OPER
 ...
 W !,"Discipline.: "
 W $P($G(^DIC(7,+$P($G(^VA(200,$P(ABM("A"),U,2),"PS")),U,5),0)),"^",1)
...
DD I $D(^VA(200,$P(ABM("A"),U,2),9999999)),$P(^(9999999),U)]"" S ABMA("Y")=$P(^(9999999),U)
 ...
 ;start new code abm*2.5*11 NPI
 S ABMNPI=$P($$NPI^XUSNPI("Individual_ID",$P(ABM("A"),U,2)),U)
 W !,"NPI.........: ",$S(+ABMNPI>0:ABMNPI,1:"")
 ;end new code abm*2.5*11 NPI
 ;start new code abm*2.5*10 IM20776
 ;W !?50,"Provider Taxonomy:",$$PTAX^ABMEEPRV($P(ABM("A"),U,2))  ;abm*2.5*11 NPI
 W ?50,"Provider Taxonomy:",$$PTAX^ABMEEPRV($P(ABM("A"),U,2))  ;abm*2.5*11 NPI
 ;end new code IM20776
 W !
OPER I '$D(ABM("O")) Q
 W !,"Oper Prov..: ",$P(ABM("O"),U)
 W ?50,"Phone #....: " I $D(^VA(200,$P(ABM("O"),U,2),.13)) W $P(^(.13),U)
 W !,"Discipline.: "
 W $P($G(^DIC(7,+$P($G(^VA(200,$P(ABM("O"),U,2),"PS")),U,5),0)),"^",1)
 I "RD"[$P($G(^AUTNINS(ABMP("INS"),2)),U),$P(^(2),U)]"" W ?50,"MCR/MCD #..: "
 E  W ?50,"Licensure #: "
...
OPNUM W $$SLN^ABMERUTL($P(ABM("O"),"^",2),ABMA("ST"))
 ...
 ;end new code abm*2.5*11 NPI
 ;start new code abm*2.5*10 IM20776
 ;W !?50,"Provider Taxonomy:",$$PTAX^ABMEEPRV($P(ABM("O"),U,2))  ;abm*2.5*11 NPI
 W ?50,"Provider Taxonomy:",$$PTAX^ABMEEPRV($P(ABM("O"),U,2))  ;abm*2.5*11 NPI
 ;end new code IM20776
 Q
 ;
...
PCCPRV I $D(^AUPNVPRV(ABMA("V"),0)) S ABMA(0)=^(0)
 ...
 .S ABMA("PRV")=$P($G(^DIC(16,+ABMA(0),0)),"^",1)
 .S ABMA("DISC")=$P($G(^DIC(6,+ABMA(0),0)),"^",4)
