ABMERGR6 ; IHS/SD/SDR - GET ANCILLARY SVCS REVENUE CODE INFO ; 
 ;;2.6;IHS Third Party Billing;**1,3,6,8,9,14,21,23,29,31**;NOV 12, 2009;Build 615
 ...
 ;IHS/SD/SDR 2.6*31 CR10421 Fixed <MAXNUM>43+25^ABMERGR6; occurs when Immunization Lot/Batch# contains an 'E'
 ;
43 ;EP - Miscellaneous Services
 ...
 .;S:+($P($G(^ABMDBILL(DUZ(2),ABMP("BDFN"),43,DA,1)),U,5)) $P(ABMRV(+ABM(2),ABM(1),ABMLCNT),U,37)=$P($G(^ABMDBILL(DUZ(2),ABMP("BDFN"),43,DA,1)),U,5)  ;immun. batch  ;abm*2.6*31 IHS/SD/SDR CR10421
 .S:($P($G(^ABMDBILL(DUZ(2),ABMP("BDFN"),43,DA,1)),U,5)'="") $P(ABMRV(+ABM(2),ABM(1),ABMLCNT),U,37)=$P($G(^ABMDBILL(DUZ(2),ABMP("BDFN"),43,DA,1)),U,5)  ;immun. batch  ;abm*2.6*31 IHS/SD/SDR CR10421
 .;end new code 5010
 .S $P(ABMRV(+ABM(2),ABM(1),ABMLCNT),U,38)=$P($G(^ABMDBILL(DUZ(2),ABMP("BDFN"),43,DA,2)),U)  ;abm*2.6*8 5010 LICN
 ...
 .S $P(ABMRV(+ABM(2),ABM(1),ABMLCNT),U,27)=$P($G(^ABMDBILL(DUZ(2),ABMP("BDFN"),43,DA,0)),U,12)  ;abm*2.6*21 HEAT120880
