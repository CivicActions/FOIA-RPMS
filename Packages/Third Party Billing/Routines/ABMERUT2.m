...
POS(X) ;EP - Point of Service ; parameter X apparently unused - comment said
 ...
 ;S X=$G(^ABMNINS(DUZ(2),ABMP("INS"),2,"AOVR",3,37,3,ABMP("VTYP")))  ;abm*2.6*10 HEAT53137
 S X=$G(^ABMNINS(ABMP("LDFN"),ABMP("INS"),2,"AOVR",3,37,3,ABMP("VTYP")))  ;abm*2.6*10 HEAT53137
 D POSOK
 ;I X="" S X=$G(^ABMNINS(DUZ(2),ABMP("INS"),2,"AOVR",3,37,3,0))  ;abm*2.6*10 HEAT53137
 I X="" S X=$G(^ABMNINS(ABMP("LDFN"),ABMP("INS"),2,"AOVR",3,37,3,0))  ;abm*2.6*10 HEAT53137
 D POSOK
 ;I X="" S X=$G(^ABMNINS(DUZ(2),ABMP("INS"),2,"AOVR",14,37,3,ABMP("VTYP")))  ;abm*2.6*10 HEAT53137
 I X="" S X=$G(^ABMNINS(ABMP("LDFN"),ABMP("INS"),2,"AOVR",14,37,3,ABMP("VTYP")))  ;abm*2.6*10 HEAT53137
 D POSOK
 ;I X="" S X=$G(^ABMNINS(DUZ(2),ABMP("INS"),2,"AOVR",14,37,3,0))  ;abm*2.6*10 HEAT53137
 I X="" S X=$G(^ABMNINS(ABMP("LDFN"),ABMP("INS"),2,"AOVR",14,37,3,0))  ;abm*2.6*10 HEAT53137
 D POSOK
 I X'="" Q X
