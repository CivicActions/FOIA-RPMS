ABME8L6 ; IHS/ASDST/DMJ - Header [ 03/09/2004  11:13 AM ]
 ;;2.5;IHS 3P BILLING SYSTEM;**1,5,6,10,12,15**;APR 05, 2002
 ;Header Segments
 ;
...
START ;START HERE
 ...
 ; D PXSET^ABMUTL8(ABMP("BDFN"))
 S ABMP("FLAT")=$$FLAT^ABMDUTL(ABMP("INS"),ABMP("VTYP"),ABMP("VDT"))
 ;I '$D(ABMP("FLAT")) D PXSET^ABMUTL8(ABMP("BDFN"))  ;abm*2.5*10
 ;D PXSET^ABMUTL8(ABMP("BDFN"))  ;abm*2.5*10  ;abm*2.5*12 IM24245
 ;I '$D(ABMP("FLAT")),($G(ABMP("BTYP"))'=131),($P($G(^AUTNINS(ABMP("INS"),0)),U)="MEDICAID EXEMPT") D PXSET^ABMUTL8(ABMP("BDFN"))  ;abm*2.5*12 IM24245  ;abm*2.5*15 IM30188
 I $P($G(^ABMNINS(ABMP("LDFN"),ABMP("INS"),1,ABMP("VTYP"),1)),U,9)'="N" D PXSET^ABMUTL8(ABMP("BDFN"))  ;abm*2.5*15 IM30188
 ...
 .D WR^ABMUTL8("QTY")
 I $P(ABMB7,"^",3) D
 .; start new code IHS/SD/SDR 7/16/04 v2.5 p6
 .; QUIT if Medicare and not inpatient
 .I $P($G(^AUTNINS(ABMP("INS"),2)),U)="R",(ABMP("VTYP")'=111) Q
 .; end new code IHS/SD/SDR 7/16/04 v2.5 p6
 .D EP^ABME8QTY("CA")
 .D WR^ABMUTL8("QTY")
 Q
