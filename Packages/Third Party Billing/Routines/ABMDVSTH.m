ABMDVSTH ; IHS/ASDST/DMJ - PCC Visit Stuff - PART 5 (HOSPITALIZATION) CONTD ;  
 ;;2.6;IHS Third Party Billing System;**2,4,36,37**;NOV 12, 2009;Build 739
 ;IHS/SD/SDR 2.5*10 IM20022 Use AOB/ROI multiples
 ;
 ;IHS/SD/SDR 2.6*2 3PMS10003A modified to call ABMFEAPI
 ;IHS/SD/SDR 2.6*4 HEAT15806 admit date/time not showing due to ABMFEAPI quit; moved up or made conditional
 ...
 ;IHS/SD/SDR 2.6*37 ADO75502 Standardized Assignment of Benefits (AoB) so the CG and the CE work the same
 ;
...
BENE ;EP - ASSIGNMENT OF BENEFITS
 ;S DR=".75////N"  ;abm*2.6*37 IHS/SD/SDR ADO75502
 S DR=".75////N;.712////@"  ;abm*2.6*37 IHS/SD/SDR ADO75502
 ;I ($D(^AUPNPAT(ABMP("PDFN"),71,0)))>10,($O(^AUPNPAT(ABMP("PDFN"),71,"B",9999999),-1)<ABMP("VDT")) S DR=".75////Y;.712////"_$O(^AUPNPAT(ABMP("PDFN"),71,"B",9999999),-1)  ;abm*2.6*37 IHS/SD/SDR ADO75502
 ;start new abm*2.6*37 IHS/SD/SDR ADO75502
 S ABMT("AOBDT")=9999999999
 S ABMAOBF=0
 F  S ABMT("AOBDT")=$O(^AUPNPAT(ABMP("PDFN"),71,"B",ABMT("AOBDT")),-1) Q:('ABMT("AOBDT")!(ABMAOBF=1))  D  Q:ABMAOBF
 .I ABMT("AOBDT")>ABMP("VDT") Q
 .S ABMAOBF=1
 I ABMAOBF=1 S DR=".75////Y;.712////"_ABMT("AOBDT")
 ;end new abm*2.6*37 IHS/SD/SDR ADO75502
 D ^DIE K DR
 K ABMT("AOBDT"),ABMAOBF
 Q
