ABMDF37B ; IHS/SD/SDR - ADA 2024 Dental Export -part 3 ;    
 ;;2.6;IHS Third Party Billing;**39,40**;NOV 12, 2009;Build 785
 ;IHS/SD/SDR 2.6*39 ADO99168 new routine
 ;IHS/SD/SDR 2.6*40 ADO111599 Print the VA Contract# in box 35
 ;
INS ;Ins Info
 I +$G(ABMSINS) D
 ...
 S $P(ABMF(2),U)="X"  ;stmt/actual svcs (1)
 I $P($G(^AUTNINS(ABMP("INS"),2)),"^")="D"&($P($G(^ABMDVTYP(ABMP("VTYP"),0)),U)["EPSDT") S $P(ABMF(2),U,2)="X"  ;EPSDT/Title 19(1)
BNODES ; Bill nodes
 ...
 .;S ABMF(41)=$P($G(^ABMDPARM(ABMP("LDFN"),1,3)),U,13)  ;VA contract# (35)  ;abm*2.6*40 IHS/SD/SDR ADO111599
 ;start new abm*2.6*40 IHS/SD/SDR ADO111599
 I ($P($G(^AUTNINS(ABMP("INS"),0)),U)="VA MEDICAL BENEFIT (VMBP)") D
 .S ABMF(40)=$P($G(^ABMDBILL(DUZ(2),ABMP("BDFN"),9)),U,27)  ;VA Contract# (35)
...
FSYM I $P(ABM("B7"),U,4)="Y" D  ;ROI
 ...
 S $P(ABMF(41),U)=$$POS^ABMERUTL  ;Place of Treatment (38)
 I (+$P($G(ABM("B4")),U,3)+$P($G(ABM("B9")),U,18)+$P($G(ABM("B9")),U,19))'=0 S $P(ABMF(41),U,2)="Y"  ;Enclosures (39)
 I (+$P($G(ABM("B4")),U,3)+$P($G(ABM("B9")),U,18)+$P($G(ABM("B9")),U,19))=0 S $P(ABMF(41),U,2)="N"  ;Enclosures (39)
 I (+$P($G(ABM("B9")),U,26)'=0) S $P(ABMF(42),U)=$$SDT^ABMDUTL($P($G(ABM("B9")),U,26))  ;Date Last SRP (39a)
XRAY ;#/X-rays included
ORTHO ;Ortho. Related?
 S $P(ABMF(44),U,$S($P(ABM("B4"),U,4):3,1:2))="X"  ;(40)
 ; Ortho. Placement Dt
 I $P(ABM("B4"),U,4) S $P(ABMF(44),U,3)=$P(ABM("B4"),U,5)  ;(41)
 I $P(ABM("B4"),U,13) S $P(ABMF(46),U)=$P(ABM("B4"),U,13)  ;(42)
PROTH ;Proth. Included?
 S $P(ABMF(46),U,$S($P(ABM("B4"),U,6):3,1:2))="X"  ;(43)
 ; Prior Placement Dt
 I $P(ABM("B4"),U,6) S $P(ABMF(46),U,4)=$P(ABM("B4"),U,7)  ;(44)
 ;
 ...
 S $P(ABMF(22),U,6)=ABMBIL_"-"_ABMSFX_" "_ABMHRN  ;Pt ID(23)
...
PAT ;
 ...
 S $P(ABMF(19),U,2)=$P(ABM("P11"),U,4)  ;Mailing-city(20)
 S $P(ABMF(19),U,2)=$P(ABMF(19),U,2)_", "_$P(^DIC(5,$P(ABM("P11"),U,5),0),U,2)  ;Mailing-St(20)
 S $P(ABMF(19),U,2)=$P(ABMF(19),U,2)_"  "_$P(ABM("P11"),U,6)  ;Mailing-Zip(20)
 S $P(ABMF(22),U,2)=$P(ABM("P0"),U,3)  ;dob(21)
 I $P(ABM("P0"),U,2)="M" S $P(ABMF(22),U,3)="X"  ;sex-male(22)
 I $P(ABM("P0"),U,2)="F" S $P(ABMF(22),U,4)="X"  ;sex-female(22)
 I ("^F^M^"'[("^"_$P(ABM("P0"),U,2)_"^")) S $P(ABMF(22),U,5)="X"  ;sex-unknown(22)
 K ABM("P0"),ABM("P11")
...
PRV ;
 ...
 .D SELBILL^ABMDE4X
 .D PAYED^ABMERUTL
 .S $P(ABMF(52),U)=$P(ABM("A"),U)  ;(53)
 .I $P($G(^AUTTLOC(ABMP("LDFN"),0)),U,2)="AIDC",($P($G(^AUTNINS(ABMP("INS"),0)),U)["DELTA DENTAL") S $P(ABMF(52),U)=""
 .S $P(ABMF(52),U,2)=$S($G(ABMP("PRINTDT"))="O":$P($G(^ABMDTXST(DUZ(2),$P($G(^ABMDBILL(DUZ(2),ABMP("BDFN"),1)),U,7),0)),U),1:DT)  ;(53)
 .S $P(ABMF(55),U,2)=$S($P($$NPI^XUSNPI("Individual_ID",$P(ABM("A"),U,2)),U)>0:$P($$NPI^XUSNPI("Individual_ID",$P(ABM("A"),U,2)),U),1:"")  ;Dent NPI (54)
 .I $P($G(^AUTTLOC(ABMP("LDFN"),0)),U,2)="AIDC",((ABMP("INS")=1722)!($P($G(^AUTNINS(ABMP("INS"),0)),U)["DELTA DENTAL")) S $P(ABMF(55),U,2)=""
 .I $P($G(^AUTNINS(ABMP("INS"),0)),U)["MEDICAID UTAH" S $P(ABMF(55),U,2)=$S($P($$NPI^XUSNPI("Organization_ID",ABMP("LDFN")),U)>0:$P($$NPI^XUSNPI("Organization_ID",ABMP("LDFN")),U),1:"")  ;Fac NPI for UTAH MEDICAID (54)
 .S $P(ABMF(58),U,2)=$TR($$SLN^ABMEEPRV($P(ABM("A"),U,2)),"-")  ;Dent Lic(55)
 .I $P($G(^ABMDBILL(DUZ(2),ABMP("BDFN"),41,ABM("X"),0)),U,4)="Y" S $P(ABMF(54),U,2)="X" ;Locum Tenens
 .I $P($G(^AUTTLOC(ABMP("LDFN"),0)),U,2)="AIDC" D
 ..I ((ABMP("INS")=1722)!($P($G(^AUTNINS(ABMP("INS"),0)),U)["DELTA DENTAL")) S $P(ABMF(57),U,2)=""
 ..S $P(ABMF(58),U,2)=""
 ..I ABMP("INS")=5 S $P(ABMF(58),U,2)="NM008A76"
 ..I $P($G(^AUTNINS(ABMP("INS"),0)),U)["UNITED CONCORDIA" S $P(ABMF(58),U,2)=601046
 ..I $P($G(^AUTNINS(ABMP("INS"),0)),U)["DELTA DENTAL" S $P(ABMF(58),U,2)=8886
 .S $P(ABMF(55),U,3)=$TR($$SLN^ABMEEPRV($P(ABM("A"),U,2)),"-")  ;(55)
 .I $P($G(^AUTTLOC(ABMP("LDFN"),0)),U,2)="AIDC" D
 ..I (ABMP("INS")=1722)!($P($G(^AUTNINS(ABMP("INS"),0)),U)["DELTA DENTAL") S $P(ABMF(55),U,3)=$S(ABMP("INS")=5:"NM008A76",ABMP("INS")["UNITED CONCORDIA":601046,($P($G(^AUTNINS(ABMP("INS"),0)),U)["DELTA DENTAL"):8886,1:"")
 ...
 .S $P(ABMF(56),U,2)=$$PTAX^ABMEEPRV($P(ABM("A"),U,2))  ;specialty(tax. code)(56a)
 .I $P($G(^AUTTLOC(ABMP("LDFN"),0)),U,2)="AIDC" D
 ..I $P($G(^AUTNINS(ABMP("INS"),0)),U)["DELTA DENTAL" S $P(ABMF(57),U,2)=""
 Q
POL ;
 N I
 S I=0
 ...
 K ABMCSZ
 I ($P(ABMV("X2"),U,6)="M") S $P(ABMF(10),U,3)="X"  ;Sex-male (14)
 I ($P(ABMV("X2"),U,6)="F") S $P(ABMF(10),U,4)="X"  ;Sex-female (14)
 I (("^F^M^")'[("^"_$P(ABMV("X2"),U,6)_"^")) S $P(ABMF(10),U,5)="X"  ;Sex-unknown (14)
 S $P(ABMF(10),U,6)=$P(ABMV("X1"),U,4)  ;Emp. id(15)
 S $P(ABMF(10),U,2)=$P(ABMV("X2"),U,7)  ;dob(13)
 S $P(ABMF(10),U)=$$RCID^ABMUTLP(ABMP("INS")) ;Payer ID (3a)
