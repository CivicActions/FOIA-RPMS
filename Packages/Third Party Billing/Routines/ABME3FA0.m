ABME3FA0 ; IHS/ASDST/DMJ - HCFA-1500 NSF 3.01  RECORD FA0 (Claim Root Segment) ;     [ 10/30/2003  3:15 PM ]
 ;;2.5;IHS 3P BILLING SYSTEM;**2,3,10**;APR 05, 2002
 ;
...
50 ;40-47 Service Date From
 ...
 S ABMR(61,50)=$P(ABMRV(J,K,M),U,10)  ;abm*2.5*10 IM20395
...
60 ;48-55 Service Date To
 ; start old code V2.5 P3
 ;S ABMR(61,60)=$P(ABMRV(J,K),"^",15)
 ;S:ABMR(61,60)="" ABMR(61,60)=$P(ABMRV(J,K),"^",10)
 ; end old code V2.5 P3
 ; start new code V2.5 P3
 ;S ABMR(61,60)=$P(ABMRV(J,K),"^",10)  ;abm*2.5*10 IM20395
 S ABMR(61,60)=$P(ABMRV(J,K,M),U,10)  ;abm*2.5*10 IM20395
 ; end new code V2.5 P3
...
120 ;69-70 Modifier 3
 ...
 S ABMR(61,120)=$P(ABMRV(J,K,M),U,12)  ;abm*2.5*10 IM20395
...
130 ;71-77 Line Charges
 ...
 S ABMR(61,130)=$P(ABMRV(J,K,M),U,6)  ;abm*2.5*10 IM20395
...
140 ;78-78 Diag Code Pointer 1
 ...
 S ABMCDX=$P(ABMRV(J,K,M),U,11)  ;abm*2.5*10 IM20395
 S:ABMCDX="" ABMCDX=1
...
180 ;82-85 Units of Service
 ...
 S ABMR(61,180)=$P(ABMRV(J,K,M),U,5)  ;abm*2.5*10 IM20395
 S ABMR(61,180)=$$FMT^ABMERUTL(ABMR(61,180),"4NRJ1")
 Q
190 ;86-89 Anesthesia/Oxygen Min
 S ABMR(61,190)=""
 ;S:J=39 ABMR(61,190)=$P(ABMRV(J,K),"^",16)  ;abm*2.5*10 IM20395
 S:J=39 ABMR(61,190)=$P(ABMRV(J,K,M),U,16)  ;abm*2.5*10 IM20395
...
230 ;93-107 Rendering Prov ID     
 ;S ABMPROV=$P(ABMRV(J,K),"^",13)  ;abm*2.5*10 IM20395
 S ABMPROV=$P(ABMRV(J,K,M),U,13)  ;abm*2.5*10 IM20395
...
400 ;193-193 Hospice Employed Prov Ind
 ;S ABMR(61,400)=""  ;IHS/SD/SDR NDA-0402-180192
 S ABMR(61,400)=$P($G(^ABMDBILL(DUZ(2),ABMP("BDFN"),9)),"^",15)  ;IHS/SD/SDR NDA-0402-180192
