ABMDF36A ; IHS/SD/SDR - ADA 2019 Dental Export -part 2 ;    
 ...
 ;  NEWed ABMSINS to stop <SUBSCR>SEL+3^ABMDE2X error
 ;
...
BADDR ;
 ...
 ..;S $P(ABMF(16),U,4)=$P($G(^AUPN3PPH(+ABMX("PH"),0)),U,4)  ;Pol#(8)  ;abm*2.6*30 IHS/SD/SDR CR11171
 ...
 ..;S:(+$P($G(^ABMDBILL(DUZ(2),ABMP("BDFN"),13,ABMSINS,0)),U,6)'=0) $P(ABMF(16),U,4)=$P($G(^AUPNMCD($P(^ABMDBILL(DUZ(2),ABMP("BDFN"),13,ABMSINS,0),U,6),0)),U,3)  ;Policy#(8)  ;abm*2.6*30 IHS/SD/SDR CR11171
 ..S:(+$P($G(^ABMDBILL(DUZ(2),ABMP("BDFN"),13,ABMSINS,0)),U,6)'=0) $P(ABMF(16),U,5)=$P($G(^AUPNMCD($P(^ABMDBILL(DUZ(2),ABMP("BDFN"),13,ABMSINS,0),U,6),0)),U,3)  ;Policy#(8)  ;abm*2.6*30 IHS/SD/SDR CR11171
