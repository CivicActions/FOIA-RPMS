ABMUTLP2 ; IHS/SD/SDR - PAYER UTILITIES ;      
 ;;2.6;IHS 3P BILLING SYSTEM;**10,11,21,30,31,37**;NOV 12, 2009;Build 739
 ...
 ;IHS/SD/SDR 2.6*30 CR9115 Mapped remaining insurer types to 837 codes
 ;IHS/SD/AML,SDR 2.6*31 CR10374 Added code for AMT*EAF segment; also fixed tribal self-insured so CAS*OA*96 prints for 837P, CAS*CO*45 prints for 837I
 ;IHS/SD/SDR 2.6*31 CR8944 Correction for <UNDEF>PAYED+66^ABMUTLP2 error; also made change for CAS*OA*0 showing as the first CAS*OA adjustment
 ;IHS/SD/SDR 2.6*37 ADO76009 Updated pymt to be insurer specific; same insurer could be on twice
 ;
SOP ;EP - source of pay (claim filing indicator)
 ...
 ;start new abm*2.6*30 IHS/SD/SDR CR9115
 I ABMTYP="T" S ABMP("SOP",ABMI)=15
 I ABMTYP="FPL" S ABMP("SOP",ABMI)=11
 I ABMTYP="G" S ABMP("SOP",ABMI)="ZZ"
 I ABMTYP="MMC" S ABMP("SOP",ABMI)=16
 I ABMTYP="MC" S ABMP("SOP",ABMI)="OF"
 I ABMTYP="N" S ABMP("SOP",ABMI)="ZZ"
 I ABMTYP="SEP" S ABMP("SOP",ABMI)="CI"
 I ABMTYP="TSI" S ABMP("SOP",ABMI)=11
 ;end new abm*2.6*30 IHS/SD/SDR CR9115
...
PAYED ; EP  ;abm*2.6*19 IHS/SD/SDR HEAT168248 - split from ABMUTLP
 ...
 K ABMP("PAYED")
 ;S (ABMOACNT,ABMPRCNT)=1  ;abm*2.6*31 IHS/SD/SDR CR8944
 S ABMCOCNT=1
 ...
 .S M=$S(+$P($G(^ABMDBILL(ABMDUZ2,I,0)),U,26)'=0:$P($G(^ABMDBILL(ABMDUZ2,I,0)),U,26),1:1)   ;PI multiple  ;abm*2.6*37 IHS/SD/SDR ADO76009
 .S (ABMOACNT,ABMPRCNT)=1  ;abm*2.6*31 IHS/SD/SDR CR8944
 .N J
 .S J=0
 ...
 ..S ABMP("PAYED",K,M)=+$G(ABMP("PAYED",K,M))+ABMPAY  ;Add amt paid per ins
 ..S $P(ABMP("PAYED",K,M),"^",2)=$P(^ABMDBILL(ABMDUZ2,I,3,J,0),U)
 ..S ABMP("PAYED")=+$G(ABMP("PAYED"))+ABMPAY  ;Add amt paid
 ...
 ..;I ("^13^14^"[("^"_ABMADJC_"^")) S ABMP(K,"EAF")=+$G(ABMP(K,"EAF"))+ABMAMT  ;abm*2.6*31 IHS/SD/SDR CR10374  ;abm*2.6*37 IHS/SD/SDR ADO76009
 ..I ("^13^14^"[("^"_ABMADJC_"^")!($P($G(^ABMDBILL(ABMDUZ2,I,3,J,0)),U,18)="Y")) S ABMP(K,M,"EAF")=+$G(ABMP(K,M,"EAF"))+ABMAMT  ;abm*2.6*37 IHS/SD/SDR ADO76009
 ...
 ...;S ABMP(K,"OA",ABMOACNT)="96^"_$S((ABMP("EXP")>30):ABMP("BILL"),1:$P($G(^ABMDBILL(ABMDUZ2,I,2)),U))_"^1"  ;abm*2.6*31 IHS/SD/SDR CR10374
 ...;start old abm*2.6*37 IHS/SD/SDR ADO76009
 ...;S:(ABMP("EXP")=32) ABMP(K,"OA",ABMOACNT)="96^"_ABMP("BILL")_"^1"  ;abm*2.6*31 IHS/SD/SDR CR10374
 ...;S:(ABMP("EXP")=31) ABMP(K,"CO",ABMCOCNT)="45^"_ABMP("BILL")_"^1"  ;abm*2.6*31 IHS/SD/SDR CR10374
 ...;S (ABMP("PAYED"),$P(ABMP("PAYED",K),"^",0,2))=0
 ...;S ABMP(K,"EAF")=$S((ABMP("EXP")>30):ABMP("BILL"),1:$P($G(^ABMDBILL(ABMDUZ2,I,2)),U)) ;abm*2.6*31 IHS/SD/SDR CR10374
 ...;end old start new abm*2.6*37 IHS/SD/SDR ADO76009
 ...S:(ABMP("EXP")=32) ABMP(K,M,"OA",ABMOACNT)="96^"_ABMP("BILL")_"^1"  ;abm*2.6*31 IHS/SD/SDR CR10374
 ...S:(ABMP("EXP")=31) ABMP(K,M,"CO",ABMCOCNT)="45^"_ABMP("BILL")_"^1"  ;abm*2.6*31 IHS/SD/SDR CR10374
 ...S (ABMP("PAYED"),$P(ABMP("PAYED",K,M),"^",0,2))=0
 ...S ABMP(K,M,"EAF")=$S((ABMP("EXP")>30):ABMP("BILL"),1:$P($G(^ABMDBILL(ABMDUZ2,I,2)),U)) ;abm*2.6*31 IHS/SD/SDR CR10374
 ...;end new abm*2.6*37 IHS/SD/SDR ADO76009
 ..;end abm*2.6*10 COB billing
 ..;I ($G(ABMSAR)="A2") S ABMP(K,"CO",ABMCOCNT)=ABMSAR_"^"_ABMAMT_"^1",ABMCOCNT=ABMCOCNT+1 Q  ;abm*2.6*31 IHS/SD/SDR CR10374
 ..;I ("^A2^45^139^249^278^279^281^P16^P24^P25^P26^"[("^"_ABMSAR_"^")) S ABMP(K,"CO",ABMCOCNT)=ABMSAR_"^"_ABMAMT_"^1",ABMCOCNT=ABMCOCNT+1 Q  ;abm*2.6*31 IHS/SD/AML,SDR CR10374 1/19/18 TSI but for MSP  ;abm*2.6*37 IHS/SD/SDR ADO76009
 ..I ("^A2^45^139^249^278^279^281^P16^P24^P25^P26^"[("^"_ABMSAR_"^")) S ABMP(K,M,"CO",ABMCOCNT)=ABMSAR_"^"_ABMAMT_"^1",ABMCOCNT=ABMCOCNT+1 Q  ;abm*2.6*31 IHS/SD/AML,SDR CR10374 1/19/18 TSI but for MSP  ;abm*2.6*37 IHS/SD/SDR ADO76009
 ..;I $G(ABMSAR)=1!($G(ABMSAR)=2)!($G(ABMSAR)=3) S ABMP(K,"PR",ABMPRCNT)=ABMSAR_"^"_ABMAMT_"^1",ABMPRCNT=ABMPRCNT+1  ;abm*2.6*37 IHS/SD/SDR ADO76009
 ..I $G(ABMSAR)=1!($G(ABMSAR)=2)!($G(ABMSAR)=3) S ABMP(K,M,"PR",ABMPRCNT)=ABMSAR_"^"_ABMAMT_"^1",ABMPRCNT=ABMPRCNT+1  ;abm*2.6*37 IHS/SD/SDR ADO76009
 ..;I $G(ABMSAR)'=1&($G(ABMSAR)'=2)&($G(ABMSAR)'=3) S ABMP(K,"OA",ABMOACNT)=ABMSAR_"^"_ABMAMT_"^1",ABMOACNT=ABMOACNT+1  ;abm*2.6*9 NOHEAT
 ..;I +$G(ABMSAR)'=0&($G(ABMSAR)'=1)&($G(ABMSAR)'=2)&($G(ABMSAR)'=3) S ABMP(K,"OA",ABMOACNT)=ABMSAR_"^"_ABMAMT_"^1",ABMOACNT=ABMOACNT+1  ;abm*2.6*9 NOHEAT  ;abm*2.6*31 IHS/SD/SDR CR10374
 ..;
 ..;I +$G(ABMSAR)'=0&($G(ABMSAR)'=1)&($G(ABMSAR)'=2)&($G(ABMSAR)'=3) S ABMP(K,"OA",ABMOACNT)=ABMSAR_"^"_ABMAMT_"^1",ABMOACNT=ABMOACNT+1  ;abm*2.6*9 NOHEAT ;abm*2.6*31 IHS/SD/AML CR10374 1/19/18 TSI but for MSP
 ..;start new abm*2.6*31 IHS/SD/AML CR10374
 ..;I +$G(ABMSAR)'=0&("^1^2^3^45^139^249^278^281^P16^P24^P25^P26^"'[("^"_ABMSAR_"^")) S ABMP(K,"OA",ABMOACNT)=ABMSAR_"^"_ABMAMT_"^1",ABMOACNT=ABMOACNT+1  ;abm*2.6*37 IHS/SD/SDR ADO76009
 ..I ($G(ABMSAR)'="")&("^1^2^3^45^139^249^278^281^P16^P24^P25^P26^"'[("^"_ABMSAR_"^")) S ABMP(K,M,"OA",ABMOACNT)=ABMSAR_"^"_ABMAMT_"^1",ABMOACNT=ABMOACNT+1  ;abm*2.6*37 IHS/SD/SDR ADO76009
 ..;end new abm*2.6*31 IHS/SD/SR CR10374
 ..; 
 .;start new abm*2.6*19 IHS/SD/SDR HEAT168248
 .K ABMTP,ABMTI,ABMTSAR,ABMTREAL
 .;start old abm*2.6*31 IHS/SD/SDR CR8944
 .;S ABMTI=0
 ...
 .;...I '$D(ABMP(ABMTSAR)) S ABMTP(ABMTSAR,ABMTT)=""
 .;K ABMTP,ABMTI,ABMTSAR,ABMTREAL
 .;S ABMTI=0
 .;F  S ABMTI=$O(ABMP(ABMTI)) Q:'ABMTI  D
 ...
 .;...S ABMCNT=ABMCNT+1
 .;end old start new abm*2.6*31 IHS/SD/SDR CR8944
 .S ABMTI=K
 .F ABMTCD="OA","CO","PR" D
 ..S ABMTT=0
 ..F  S ABMTT=$O(ABMP(ABMTI,ABMTCD,ABMTT)) Q:'ABMTT  D
 ...S ABMTSAR=$P(ABMP(ABMTI,ABMTCD,ABMTT),U)
 ...I $D(ABMTP(ABMTI,ABMTSAR)) D
 ....S ABMTREAL=$O(ABMTP(ABMTI,ABMTSAR,0))
 ....S $P(ABMP(ABMTI,ABMTCD,ABMTREAL),U,2)=$P(ABMP(ABMTI,ABMTCD,ABMTREAL),U,2)+$P(ABMP(ABMTI,ABMTCD,ABMTT),U,2)
 ....K ABMP(ABMTI,ABMTCD,ABMTT)
 ...I '$D(ABMP(ABMTI,ABMTSAR)) S ABMTP(ABMTI,ABMTSAR,ABMTT)=""
 .K ABMTP,ABMTSAR,ABMTREAL
 .F ABMTTTYP="OA","CO","PR" D
 .M ABMTP(ABMTI,ABMTTTYP)=ABMP(ABMTI,ABMTTTYP)
 .K ABMP(ABMTI,ABMTTTYP)
 .S ABMCNT=1
 ...
 ...S ABMP(ABMTI,ABMTTTYP,ABMCNT)=$G(ABMTP(ABMTI,ABMTTTYP,ABMTT))
 ...S ABMCNT=ABMCNT+1
 .;end new abm*2.6*31 IHS/SD/SDR CR8944
