...
90 ;68-82 Ordering Provider ID
 ...
 S ABMR(62,90)=$P(ABMRV(J,K,L),U,16)  ;abm*2.5*10 IM20395
...
150 ;110-120 National Drug Code
 ;S ABMR(62,150)=$P(ABMRV(J,K),"^",15)  ;abm*2.5*10 IM20395
 S ABMR(62,150)=$P(ABMRV(J,K,L),U,15)  ;abm*2.5*10 IM20395
 S ABMR(62,150)=$$FMT^ABMERUTL(ABMR(62,150),11)
 Q
160 ;121-127 National Drug Units
 ;S ABMR(62,160)=$P(ABMRV(J,K),"^",5)  ;abm*2.5*10 IM20395
 S ABMR(62,160)=$P(ABMRV(J,K,L),U,5)  ;abm*2.5*10 IM20395
 S ABMR(62,160)=$$FMT^ABMERUTL(ABMR(62,160),"7NR")
 Q
170 ;128-142 Prescription #
 ;S ABMR(62,170)=$P(ABMRV(J,K),"^",13)  ;abm*2.5*10 IM20395
 S ABMR(62,170)=$P(ABMRV(J,K,L),U,13)  ;abm*2.5*10 IM20395
 S ABMR(62,170)=$$FMT^ABMERUTL(ABMR(62,170),15)
 Q
180 ;143-150 Prescription Date
 ;S ABMR(62,180)=$P(ABMRV(J,K),"^",10)  ;abm*2.5*10 IM20395
 S ABMR(62,180)=$P(ABMRV(J,K,L),U,10)  ;abm*2.5*10 IM20395
