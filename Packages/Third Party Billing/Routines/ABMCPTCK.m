ABMCPTCK ; IHS/SD/SDR - Claim Summary-CPT check ;   [ 01/09/2006  7:30 AM ]
 ;;2.5;IHS 3P BILLING SYSTEM;**9,10**;JAN 09, 2006
 ;
...
CPTCHK(ABMVDFN,ABMLIST) ;PEP-Check if CPT codes exist on visit
 ...
 Q ABMCFLG
 ;start new code abm*2.5*10 IM20329
CPTLIST(ABMPCDFN) ;PEP-create list of CPTs on visit
 ...
 .D LINEITEM
 Q ABMCPTS
 ;end new code abm*2.5*10 IM20329
VISIT ;  note: must be in range 99201-99499 (E&M codes)
 S ABMCFLG=0
 Q:$P($G(^AUPNVSIT(ABMVDFN,0)),U,17)=""
 S ABMICPT=$P($G(^AUPNVSIT(ABMVDFN,0)),U,17)
 I $D(ABMLIST($P(^ICPT(ABMICPT,0),U))) S ABMCFLG=1
 I $G(ABMCPTS)=1 S ABMCPTS($P(^ICPT(ABMICPT,0),U))=""  ;abm*2.5*10 IM20329
 Q
PROC ;
 S ABMCFLG=0
 S ABMIEN=0
