ABMDF37X ; IHS/SD/SDR - ADA-2024 FORM ;   
 ;;2.6;IHS 3P BILLING SYSTEM;**39,40**;NOV 12, 2009;Build 785
 ;IHS/SD/SDR 2.6*39 ADO99168 New Routine
 ;IHS/SD/SDR 2.6*40 ADO111599 Fix for VA Contract# to print
 ;************************************************************************************
 ;
MARG ;Set left and top margins
 S U="^",(ABM("LM"),ABM("TM"),ABM("LN"))=0
 I $D(^ABMDEXP(37,0)) S ABM("TM")=$P(^(0),U,3),ABM("LM")=$P(^(0),U,2)
 W $$EN^ABMVDF("IOF")
...
LOOP ;
 ...
 ;I ABM("LN")>38,ABM("LN")<42 S ABM("FL")=39 ;Lines 39 thru 42 are same  ;abm*2.6*40 IHS/SD/SDR ADO111599
 I ABM("LN")>38,ABM("LN")<40 S ABM("FL")=39 ;Lines 39 thru 40 are same  ;abm*2.6*40 IHS/SD/SDR ADO111599
  ;
...
OVER ;GET OVRRIDE VALUES FROM 3P INSURER FILE
 S ABMOLN=0
 F  S ABMOLN=$O(^ABMNINS(ABMP("LDFN"),ABMP("INS"),2,"AOVR",37,ABMOLN)) Q:'ABMOLN  D
 .S ABMOPC=0
 .F  S ABMOPC=$O(^ABMNINS(ABMP("LDFN"),ABMP("INS"),2,"AOVR",37,ABMOLN,ABMOPC)) Q:'ABMOPC  D
 ..K ABMOVTYP
 ..I $D(^ABMNINS(ABMP("LDFN"),ABMP("INS"),2,"AOVR",37,ABMOLN,ABMOPC,0)) S ABMOVTYP=0
 ..I $D(^ABMNINS(ABMP("LDFN"),ABMP("INS"),2,"AOVR",37,ABMOLN,ABMOPC,ABMP("VTYP"))) S ABMOVTYP=ABMP("VTYP")
 ..Q:'$D(ABMOVTYP)
 ..S ABMVALUE=^ABMNINS(ABMP("LDFN"),ABMP("INS"),2,"AOVR",37,ABMOLN,ABMOPC,ABMOVTYP)
...
8 ;;5;;34
9 ;;5;;34
10 ;;10^42^55^57^59^64;;20^10D^1^1^1^15
...
21 ;;1;;30
 ;22 ;;1;;30
22 ;;10^42^55^57^59^63;;20^10D^1^1^1^16
