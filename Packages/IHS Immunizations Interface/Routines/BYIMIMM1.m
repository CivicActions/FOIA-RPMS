BYIMIMM1 ;IHS/CIM/THL - IMMUNIZATION DATA EXCHANGE;
 ;;2.0;BYIM IMMUNIZATION DATA EXCHANGE;**3,4,5,6,7,8,9,10**;JUL 11, 2017;Build 456
 ;
...
ID ;ID PATIENT
 ...
 S (NAME,FULLNAME)=X
 I FACILITY]"",$O(^AUTTLOC("C",FACILITY,0)) S Z=2,OLDDUZ(Z)=$$DUZ^BYIMIMM(),DUZ(Z)=$O(^AUTTLOC("C",FACILITY,0))
 ...
 ..S DA(1)=$$DUZ^BYIMIMM()
 ..S DIC="^BYIMPARA("_$$DUZ^BYIMIMM()_",4,"
 ..S DIC(0)="LZ"
 ..S X=FULLNAME
 ...
 ..S Y=-1
 S DFN=+Y
 I '$D(^AUPNPAT(DFN,41,$$DUZ^BYIMIMM(),0)) D
 .K OLDDUZ
 .N XX
 .S XX=0
 .F  S XX=$O(^AUPNPAT(DFN,41,XX)) Q:'XX!$G(OLDDUZ(2))  D
 ..S Z=2,OLDDUZ(Z)=$$DUZ^BYIMIMM()
 ..S Z=2,DUZ(Z)=XX
 ..K BYIMQUIT
...
PADD ;EP;TO ADD PARAMETER SITE
 ...
 S DIC("B")=$P(^DIC(4,$$DUZ^BYIMIMM(),0),U)
 W @IOF
 W !!,"EDIT Data Exchange Parameters"
 W !
 D ^DIC
 K DIE,DIC,DINUM,DR,DA,DD,DO,DIK,DLAYGO
 S (BYIMDA,DA)=+Y
...
LOCIN(LOC) ;PROCESS LOCATION DATA FOR FACILITY NAME OR CODE
 ...
 I $P(^DIC(4,+LOCDA,0),U)="OTHER",$P($G(^BISITE($S($G(OLDDUZ(2)):OLDDUZ(2),1:$$DUZ^BYIMIMM()),0)),U,3),LOCDA'=$P(^(0),U,3) S LOCDA=$P(^(0),U,3)
 Q:$P(^DIC(4,+LOCDA,0),U)'="OTHER"
 ;DETERMINE STATE FOR 'OTHER' FACILITY
 N X,Y,Z
 S X=$P($G(^DIC(4,$$DUZ^BYIMIMM(),0)),U,2)
 Q:'X
 S Z=0
 S Y=0
