BIVWPXRM ;IHS/CMI/MWR - Immunizations Forecasting Utilities ; 25 Jul 2016  4:56 PM
...
EN(DFN,REMNDR,ARY,ATTR,RCODE,RNAME) ; extrinsic returns true if the reminder evaluates true
 ...
 ;
 ; Q 0  ; GPL BYPASS FOR INITIAL TESTING 3/9/2016
 ;D MAIN(11,264,0)
 D MAIN(DFN,REMNDR,0)
...
TEST ; try out the above
 ...
 ;S RCODE="070.30"
 ;S RNAME="Hep B"
 ;I $$EN^PXVWPXRM(DFN,IENHEPB,.RETURN,"hadHepB",RCODE,RNAME) S N=N+1 S HEPB=1
 ;I $G(^TMP("PXRHM",$J,IENHEPB,"VIMM-HEPATITIS B DIAGNOSIS"))["DUE NOW" S N=N+1 S HEPB=1
 ;W "HEPB=",HEPB,!
 D EN^PXVWTAX(DFN,"BKM HEP B DXS",.RETURN) I $G(RETURN("findings","B","hadHepB",1))=1 S N=N+1,HEPB=1
 K ^TMP("PXRHM",$J)
 Q
 ...
 S RCODE="072.9"
 S RNAME="Mumps"
 I $$EN^BIVWPXRM(DFN,IENMUMPS,.RETURN,"hadMumps",RCODE,RNAME) S N=N+1 S IENMUMPS=1
