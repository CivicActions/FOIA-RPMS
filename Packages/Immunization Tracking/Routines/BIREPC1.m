BIREPC1 ;IHS/CMI/MWR - REPORT, COVID IMM; OCT 15,2010
 ;;8.5;IMMUNIZATION;**24**;OCT 24,2011;Build 15
 ;;* MICHAEL REMILLARD, DDS * CIMARRON MEDICAL INFORMATICS, FOR IHS *
 ;;  VIEW OR PRINT COVID IMMUNIZATION REPORT.
 ;
 ;----------
START(BIX) ;EP
 ...
 ;     6 - BIPOP  (ret) BIPOP=1 if error.
 ;
 ;---> Check for required Variables.
 ;I '$G(BIYEAR) D ERROR(679) D RESET^BIREPC Q
 I '$G(BIQDT) D ERROR(622) D RESET^BIREPC Q
 I '$D(BICC) D ERROR(614) D RESET^BIREPC Q
 I '$D(BIHCF) D ERROR(625) D RESET^BIREPC Q
 I '$D(BICM)  D ERROR(615) D RESET^BIREPC Q
 I '$D(BIBEN) D ERROR(662) D RESET^BIREPC Q
 S:$G(BIUP)="" BIUP="u"
 ;
 D SETVARS^BIUTL5 N VALMCNT
 I $G(BIX)="PRINT" D PRINT,RESET^BIREPC Q
 ;
 ...
 S BIAG="ALL",BIRPDT=$G(DT),BIRTN="BIREPC1",BITITL="COVID"
 D EN
 D RESET^BIREPC
 Q
 ;
...
PRINT ;EP
 ...
 ;
 ;---> Prepare report.
 K ^TMP("BIREPC1",$J),^TMP("BIDUL",$J)
 N VALM,VALMHDR
 D HDR,START^BIREPC2(BIQDT,.BICC,.BIHCF,.BICM,.BIBEN,BIUP)
 ;
 D PRTLST^BIUTL8("BIREPC1")
 D EXIT,RESET^BIREPC
 Q
 ;
 ;
 ;----------
EN ;EP
 ;---> Main entry point for List Template BI REPORT COVID IMM1.
 D EN^VALM("BI REPORT COVID IMM1")
 Q
 ;
 ;
 ;----------
HDR ;EP
 ;---> Header code
 D HEAD^BIREPC2(BIQDT,.BICC,.BIHCF,.BICM,.BIBEN,BIUP)
 Q
 ;
 ;
 ;----------
INIT ;EP
 ...
 W !!?10,"This may take some time.  Please hold on...",!
 D START^BIREPC2(BIQDT,.BICC,.BIHCF,.BICM,.BIBEN,BIUP)
...
HELP ;EP
 ...
 D TITLE^BIUTL5("COVID REPORT - HELP, page 1 of 1")
 D TEXT1,DIRZ^BIUTL3()
 D:BIX'="??" RE^VALM4
 Q
 ;
...
TEXT1 ;EP
 ...
 ;;          or "NOT up-to-date" with their COVID immunizations,
 ;;          according to recommendeded guidelines for their age.
 ;;
 ;;Enter "C" to view the list of Patients who were CURRENT or
 ;;          "up-to-date" with their COVID immunizations, according
...
DEQUEUE ;EP
 ...
 K VALMHDR,^TMP("BIREPC1",$J)
 ;
 D HDR^BIREPC1,START^BIREPC2(BIQDT,.BICC,.BIHCF,.BICM,.BIBEN,BIUP)
 ;
 D PRTLST^BIUTL8("BIREPC1"),EXIT
 Q
 ;
 ;
 ;----------
