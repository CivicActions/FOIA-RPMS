BIREPF4 ;IHS/CMI/MWR - REPORT, FLU IMM; OCT 15, 2010
 ;;8.5;IMMUNIZATION;**15,26**;OCT 24,2011;Build 33
...
CHKSET(BIDFN,BICC,BIHCF,BICM,BIBEN,BIAGRP,BIQDT,BIFH,BIYEAR,BIUP) ;EP
 ...
 ;
 ;---> Add refusals, if any.
 ;N Z D CONTRA^BIUTL11(BIDFN,.Z,1) I $O(Z(0)) D   ;IHS/CMI/LAB - look at refusals from NMI file only
 N Z D REFUSAL^BIUTL13(BIDFN,.Z) I $O(Z(0)) D
 ...
 .N I,% S I=0 F  S I=$O(^AUTTIMM(I)) Q:I'=+I  I $$VAL^XBDIQ1(9999999.14,I,.09)="FLU" S %=$$VAL^XBDIQ1(9999999.14,I,.03) I $D(Z(%)) S BITMP("REFUSALS",BIDFN)=""
 .;**********
 ;
