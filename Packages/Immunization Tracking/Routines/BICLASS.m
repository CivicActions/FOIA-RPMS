BICLASS ;GDIT/HS/BEE-IMMUNIZATION Cache Class Compiler ; 02 Mar 2021  9:53 AM
...
EXPGO ;
 ; Create new record in 9002084.71 with IEN of REC
 ; Populate Name (not sure what it should be), Date/Time, and set status to I
 K DO,DA S DIC=9002084.71,DLAYGO=9002084.71,DIC(0)="L",X=$S($G(NAME)'="":NAME,1:"New Record for "_$H)
 ...
 . S B64=$TR(B64,$C(10))
 . F I=1:1 SET XML=$P(B64,$C(13),I) Q:XML=""  D POPULATE^BICLASS(REC,XML) Q:ERR
 . I ERR D ERROR
 . D POPULATE^BICLASS(REC,"------------------------- SEGMENT END ------------------------")
 S DIE="^BICLASS("
 K DA S DA=REC
 ; Set Status to READY
 S DR="1.02////R"
...
IMPORT(REC,ERR) ;
 ...
 ; Change the value of field RPMS STATUS in 9002084.71 to "I"
 K DA S DA=REC,DIE="^BICLASS(",DR="1.02////I" D ^DIE
 ;
 ; Create a new global-based character stream
 ...
 F  D  Q:B64=""
 . S B64=""
 . F  S I=$O(^BICLASS(REC,10,I)) Q:'I  S STR=^BICLASS(REC,10,I,0) Q:STR["SEGMENT END"  S B64=B64_STR_$C(13,10)
 . I B64="" Q
 ...
 . S DIC="^BICLASS("_REC_",11,",DIC(0)="L",DLAYGO=1
 . S DA(1)=REC
 . S X=CLASS
 . D ^DIC
 ; Change the value of the field RPMS STATUS in 9002084.71 to "R"
 ; and populate RPMS DATE/TIME INSTALLED
 S DIE="^BICLASS("
 K DA S DA=REC
 ; Set Status to READY
 W !,"Updating 9002084.71 Record"
 D NOW^%DTC
 S DR="1.02////C;1.03////"_%
 D ^DIE
EXIT ;
...
POPULATE(REC,XML) ;
 S DA(1)=REC
 K DIC S DIC="^BICLASS("_DA(1)_",10," ;XML Subfile
...
BGERROR ;
 ...
 ; Change the value of the field RPMS STATUS in 9002084.71 to "E"
 ; S ERRTEXT=""
