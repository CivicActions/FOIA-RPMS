BIVWVPRD ;IHS/CMI/MWR -- Serve VistA data as XML via RPC ;8/2/11  15:29
 ;;8.5;IMMUNIZATION;**18**;JUN 15,2020;Build 28
 ;;1.0;VIRTUAL PATIENT RECORD;**1,2,4,5**;Sep 01, 2011;Build 4
...
GET(VPR,DFN,TYPE,START,STOP,MAX,ID,FILTER) ; -- Return search results as XML in @VPR@(n)
 ...
 ;
 ;---> * * * All this stuff below is adding XML tags:
 ;---> All this this TYPE code can be eliminated and just hard code for the
 ;---> Imm routine below in RTN: "BIVWVPIM"
 ;
 ; extract data
 N VPRTYPE,VPRP,VPRHDR,VPRTAG,VPRTN
 ;
 ;---> ADD adds a line to the XML with the content in parentheses:
 ;
 ...
 . ;
 . ;---> Basically call BIVWVPIM:
 . ;D @(VPRTN_"(DFN,START,STOP,MAX,ID)")
 . D EN^BIVWVPIM(DFN,START,STOP,MAX,ID)
 . S @VPR@(VPRHDR)=@VPR@(VPRHDR)_" total='"_+$G(VPRTOTL)_"' >" D ADD("</"_VPRTAG_">")
 ;
 ;---> Above lines are adding data to @VPR (^TMP("BI_VPR",$J)
 ;---> Next line closes off "results" subset of imms that will be eventually passed
 ;---> to the Input XML:
 D ADD("</results>")
 ;
 ;---> At this point immunizations data is in ^TMP("BI_VPR",$J)
 ;b
GTQ ; end
 K ^TMP("BIVPRD",$J)
 Q
...
RTN(X) ; -- Return name of VPRDxxxx routine for clinical domain X
 ...
 I X["immunization" S Y="BIVWVPIM",X="immunizations"
...
PROVSPC(NP) ; -- Return contact & specialty info for provider NP
 ; save strings in ^TMP("BIVPRD",$J,NP) for efficiency
 N X,Y,I,CLS,RES,X13,X15 S NP=+$G(NP) ;protect I for calling routine
 S RES=$G(^TMP("BIVPRD",$J,NP)) I $L(RES) Q RES
...
TEST(DFN,TYPE,ID,START,STOP,MAX,TEXT,IN) ; -- test GET, write results to screen
 ...
 ;
 S IDX=OUT
 F  S IDX=$Q(@IDX) Q:IDX'?1"^TMP(""BI_VPR"","1.N.E  Q:+$P(IDX,",",2)'=$J  W !,@IDX
 Q
