BIREPE3 ;IHS/CMI/MWR - REPORT, VAC ELIGIBILITY; MAY 10, 2010 [ 01/31/2025  10:23 AM ]
 ;;8.5;IMMUNIZATION;**3,26,29,30**;OCT 24,2011;Build 125
 ...
 ;;  PATCH 26 change call to ELIGC^BIELIG to pass 99 instead of 6 due to codes added
 ;
 ;----------
GETIMMS(BIBEGDT,BIENDDT,BICC,BIHCF,BICM,BIBEN,BIHIST,BIVT,BIU19) ;EP
 ...
 N VDT,VDA,IDA
 S VDT=BIBEGDT-.9999
 F  S VDT=$O(^AUPNVIMM("ADT",VDT)) Q:(VDT>(BIENDDT+.9999)!('VDT))  D
 .S VDA=0
 .F  S VDA=$O(^AUPNVIMM("ADT",VDT,VDA)) Q:'VDA  D
 ..S IDA=0
 ..F  S IDA=$O(^AUPNVIMM("ADT",VDT,VDA,IDA)) Q:'IDA  D
 ...D CHKSET(VDT,VDA,IDA,.BICC,.BIHCF,.BICM,.BIBEN,BIHIST,.BIVT,BIU19)
 Q
 ;
 ;
 ;----------
CHKSET(BIDATE,BIVIEN,BIIIEN,BICC,BIHCF,BICM,BIBEN,BIHIST,BIVT,BIU19) ;EP
 ...
 ;20230309 maw p26 mod next 2 lines for new code added
 ;N BIELIGC S BIELIGC=$$ELIGC^BIELIG(BIELIG,6)
 N BIELIGC S BIELIGC=$$ELIGC^BIELIG(BIELIG,99)
