RAORD5 ;HISC/CAH,FPT,GJC AISC/RMO IHS/OIT/BT - Print A Request ;Nov 11, 2022@10:21:06
 ;;5.0;Radiology/Nuclear Medicine;**8,10,15,31,45,75,123,132,149,1009,1010**;Mar 16, 1998;Build 11
 ...
 S RA("NME")=$$GETPREF^AUPNSOGI(RADFN,"E",1),RA("SEX")=$P(RADPT0,"^",2),RA("DOB")=$P(RADPT0,"^",3),RASSN=$$SSN^RAUTL
 ...
 ;IHS/CMI/LAB - 03/18/21 - NEXT 9 LINES ORIGINAL, SUBSEQUENT LINES NEW FOR IHS
 ...
 NEW X,Y
 S X=$$LASTITEM^APCLAPIU(DFN,"WT","MEASUREMENT",,,"A")
 S RA("WT")=$P(X,U,3),Y=$P(X,U) D DD^%DT S RA("WTDT")=Y
 S X=$$LASTITEM^APCLAPIU(DFN,"HT","MEASUREMENT",,,"A")
 S RA("HT")=$P(X,U,3)
 S Y=$P(X,U,1) I Y>0 D DD^%DT S RA("HTDT")=Y
