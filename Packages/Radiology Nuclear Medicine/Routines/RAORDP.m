RAORDP ;HISC/CAH,FPT AISC/DMK IHS/OIT/BT - Log of Pending/Hold Requests ;11/15/2022  16:49
 ;;5.0;Radiology/Nuclear Medicine;**15,133,1009,1010**;Mar 16, 1998;Build 11
 ;
...
WRT ;
 ...
 ;IHS/CMI/LAB - 03/18/21 - PATCH 1009, CR #10995 BEGIN MODS
 ;W !,$S($D(^DPT(RADFN,0)):$E($P(^(0),"^"),1,19),1:"Unknown"),?21,$E($P(^DPT(RADFN,0),"^",9),6,9),?27,$S($D(^RAMIS(71,RAPR,0)):$E($P(^(0),"^"),1,24),1:"Unknown"),?53,$P(RADD,"@") ;IHS/CMI/LAB - ORIGINAL LINE
 N RANAM S RANAM=$$GETPREF^AUPNSOGI(RADFN,"E",1) S:RANAM="" RANAM=$S($D(^DPT(RADFN,0)):$E($P(^(0),"^"),1,19),1:"Unknown")
 W !,RANAM W:$L(RANAM)>20 ! W ?20,$$HRN^AUPNPAT(RADFN,DUZ(2)),?27,$S($D(^RAMIS(71,RAPR,0)):$E($P(^(0),"^"),1,24),1:"Unknown"),?53,$P(RADD,"@") ;IHS/CMI/LAB - NEW LINE FOR HRN
 ;IHS/CMI/LAB - 03/18/21 - PATCH 1009, CR #10995 END MODS
...
HDR ; header
 ...
 ;IHS/CMI/LAB - 03/18/21 - PATCH 1009, CR #10995 BEGIN MODS
 ;W !,"PATIENT NAME",?21,"SSN",?27,"PROCEDURE",?53,"DATE DESIRED",?67,"DATE ORDERED"   ;IHS/CMI/LAB - original line
 W !,"PATIENT NAME",?20,"HRN",?27,"PROCEDURE",?53,"DATE DESIRED",?67,"DATE ORDERED"    ;IHS/CMI/LAB - replaced SSN with HRN
 ;IHS/CMI/LAB - 03/18/21 - PATCH 1009, CR #10995 END MODS
