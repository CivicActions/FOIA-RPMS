RAORDS ;HISC/CAH,DAD AISC/RMO IHS/OIT/BT - Select Patient's Requests ;11/11/2022 13:05
 ;;5.0;Radiology/Nuclear Medicine;**15,21,132,1009,1010**;Mar 16, 1998;Build 11
 ...
 ;IHS/CMI/LAB - PATCH 1009 CR# 8992 reorder by date
 I $G(BRALS)'="D" F RAOURG=0:0 S RAOURG=$O(^TMP($J,"RAORDS",RAOURG)) Q:'RAOURG!($D(RAOSEL))  F RAODTI=0:0 S RAODTI=$O(^TMP($J,"RAORDS",RAOURG,RAODTI)) Q:'RAODTI!($D(RAOSEL))  D CHKORD
 G:$G(BRALS)'="D" Q
 ;reorder by date
 ;if sort by date, reorder ^TMP by switching RAOURG and RAODTI
 K ^TMP($J,"RAORDS REORDER BY DATE")
 M ^TMP($J,"RAORDS REORDER BY DATE")=^TMP($J,"RAORDS")
 K ^TMP($J,"RAORDS")
 F RAOURG=0:0 S RAOURG=$O(^TMP($J,"RAORDS REORDER BY DATE",RAOURG)) Q:'RAOURG  D
 .F RAODTI=0:0 S RAODTI=$O(^TMP($J,"RAORDS REORDER BY DATE",RAOURG,RAODTI)) Q:'RAODTI  D
 ..F RAOIFN=0:0 S RAOIFN=$O(^TMP($J,"RAORDS REORDER BY DATE",RAOURG,RAODTI,RAOIFN)) Q:'RAOIFN  D
 ...F RACNT1=0:0 S RACNT1=$O(^TMP($J,"RAORDS REORDER BY DATE",RAOURG,RAODTI,RAOIFN,RACNT1)) Q:'RACNT1  D
 ....S ^TMP($J,"RAORDS",RAODTI,RAOURG,RAOIFN,RACNT1)=^TMP($J,"RAORDS REORDER BY DATE",RAOURG,RAODTI,RAOIFN,RACNT1)
 K ^TMP($J,"RAORDS REORDER BY DATE")
 F RAODTI=0:0 S RAODTI=$O(^TMP($J,"RAORDS",RAODTI)) Q:'RAODTI!($D(RAOSEL))  F RAOURG=0:0 S RAOURG=$O(^TMP($J,"RAORDS",RAODTI,RAOURG)) Q:'RAOURG!($D(RAOSEL))  D CHKORD
 K BRALS
...
SETUTL ; Check if option is to be screened.  If yes, apply the screen.
 ...
 Q
 ;
CHKORD ;IHS/CMI/LAB - 04/15/2021 patch 1009 CR# 8982 begin mods
 ;IHS/CMI/LAB - commented out line below, replaced with new lines
 ;F RAOIFN=0:0 S RAOIFN=$O(^TMP($J,"RAORDS",RAOURG,RAODTI,RAOIFN)) Q:'RAOIFN!($D(RAOSEL))  F RACNT1=0:0 S RACNT1=$O(^TMP($J,"RAORDS",RAOURG,RAODTI,RAOIFN,RACNT1)) Q:RACNT1'>0!($D(RAOSEL))  S RAORD0=^(RACNT1) D PRTORD
 I $G(BRALS)'="D" D  Q
 .F RAOIFN=0:0 S RAOIFN=$O(^TMP($J,"RAORDS",RAOURG,RAODTI,RAOIFN)) Q:'RAOIFN!($D(RAOSEL))  F RACNT1=0:0 S RACNT1=$O(^TMP($J,"RAORDS",RAOURG,RAODTI,RAOIFN,RACNT1)) Q:RACNT1'>0!($D(RAOSEL))  S RAORD0=^(RACNT1) D PRTORD
 F RAOIFN=0:0 S RAOIFN=$O(^TMP($J,"RAORDS",RAODTI,RAOURG,RAOIFN)) Q:'RAOIFN!($D(RAOSEL))  F RACNT1=0:0 S RACNT1=$O(^TMP($J,"RAORDS",RAODTI,RAOURG,RAOIFN,RACNT1)) Q:RACNT1'>0!($D(RAOSEL))  S RAORD0=^(RACNT1) D PRTORD
 ;IHS/CMI/LAB - end mods 
 Q
 ;
PRTORD D HD:'(RASEQ#8) Q:$D(RAOSEL)  S RASEQ=RASEQ+1,RAOIFNS(RASEQ)=RAOIFN,RAPRC=$S($D(^RAMIS(71,+$P(RAORD0,"^",2),0)):$P(^(0),"^"),1:"Unknown"),RAODTE=9999999.9999-RAODTI
 ...
 Q
 ;
HD D ASKSEL:RASEQ Q:$D(RAOSEL)
 N REHDR S REHDR="**** Requested Exams for "_RANME_" **** "_RACNT
 S REHDR=REHDR_$S($L(REHDR)<71:" Requests",1:" Reqs")
 N XLOC S XLOC=79-$L(REHDR)\2
 W @IOF,!?XLOC,REHDR
