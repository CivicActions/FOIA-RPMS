RAZWH ; IHS/ADC/PDW - RADIOLOGY WOMEN'S HEALTH LINK ;    [ 11/01/2001  12:15 PM ]
 ...
 ;;  CALLED BY ^RAZEXPT WHEN EXPORTING HISTORICAL MAMS TO WOMEN'S HEALTH
 ;
 ;G LOOP
 ...
 ;--->                     THESE IF CALLED FROM ^RAZEXPT ROUTINE.
 ;
 ;---> GENERATED VARIBLES:
...
CREATE(DFN,DATE,CASE) ;EP
 ...
 N RACASE,RACPT,RAERR,RAZIRAD  ;,RAEXAM0  ;IHS/HQW/SCR 10/29/01 **11** 
 ;
 ...
 S RAZIRAD=$P(RAEXAM0,U,15)   ;IHS/HQW/SCR 10/29/01 **11**
 ;
 S RACPT=$P(^RAMIS(71,$P(RAEXAM0,U,2),0),U,9)
 Q:(RACPT'=76090)&(RACPT'=76091)&(RACPT'=76092)
 ;
 ...
 Q:'$P(Y,U,10)
 ;
 ;---> SET RAZSTAT =THE STATUS (OPEN OR CLOSED) IN WOMEN'S HEALTH.
 ;---> THAT MAMMOGRAMS SHOULD RECEIVE WHEN COPIED OVER FROM RADIOLOGY.
 S RAZSTAT=$P(Y,U,23) S:RAZSTAT="" RAZSTAT="o"
 ;
...
PROC ;---> CREATE MAMMOGRAM PROCEDURE IN BW PROCEDURE FILE #9002086.1.
 ...
 S RADR=RADR_";.12////"_RADATE_";.14////"_RAZSTAT_";.15////"_RACASE
 ;
 ;S RADR=RADR_";.18////.5;.19////"_DT  ;Cmnt'd out to add Interpreting Radiologist field IHS/HQW/SCR 10/29/01 **11**
 S RADR=RADR_";.18////.5;.19////"_DT_";.35////"_RAZIRAD  ;IHS/HQW/SCR 10/29/01 **11**
 ;
...
LOOP ;EP
 ...
 Q
 S RAZDT1=2951231,RAZCOUNT=0
 F  S RAZDT1=$O(^RADPT("AR",RAZDT1)) Q:'RAZDT1  D
 .S RAZDFN=0
 .F  S RAZDFN=$O(^RADPT("AR",RAZDT1,RAZDFN)) Q:'RAZDFN  D
 ..S RAZDT2=0
 ..F  S RAZDT2=$O(^RADPT("AR",RAZDT1,RAZDFN,RAZDT2)) Q:'RAZDT2  D
 ...Q:'$D(^RADPT(RAZDFN,"DT",RAZDT2))
 ...S RAZCS=0
 ...F  S RAZCS=$O(^RADPT(RAZDFN,"DT",RAZDT2,"P",RAZCS)) Q:'RAZCS  D
 ....Q:'$D(^RADPT(RAZDFN,"DT",RAZDT2,"P",RAZCS,0))
 ....S RAZ0=^RADPT(RAZDFN,"DT",RAZDT2,"P",RAZCS,0)
 ....;---> QUIT IF THIS PROCEDURE WAS NOT A MAMMOGRAM, SCREENING.
 ....Q:$P(^RAMIS(71,$P(RAZ0,U,2),0),U,9)'=76092
 ....;---> QUIT IF THIS EXAM DOES NOT HAVE A STATUS OF COMPLETE.
 ....Q:$P(RAZ0,U,3)'=2
 ....;W !,^RADPT(RAZDFN,"DT",RAZDT2,"P",RAZCS,0)
 ....D CREATE(RAZDFN,RAZDT2,RAZCS)
 ....S RAZCOUNT=RAZCOUNT+1
 W !?5,RAZCOUNT," procedures passed to Women's Health."
 Q
