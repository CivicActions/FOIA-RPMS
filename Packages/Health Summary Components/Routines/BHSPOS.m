BHSPOS ;IHS/MSC/MGH - Routine to select meds for selected med health summary compoenent;30-Jan-2025 14:43
 ...
 ;returns data in ^TMP("BHS",$J,Inverse last fill date,0)
 ;data is ISSUE DATE^LAST FILL DATE^DRUG^PROVIDER^STATUS^RX#^QTY^#REFILLS^IFN^COST/FILL^EXP/CANC DATE
 ;and ^TMP("BHS",$J,Inverse last fill date,n,0)=SIG
 ;NON-VA Meds: ^TMP("BHS",$J,"NVA",n,0)=orderable item_" "_dose form^status (active or discontinued)^start date(fm format)^cprs order # (ptr to 100)^date/time documented (fm format)^documented by (ptr to 200_";"_.01)^dc date/time(fm format)
 ;^TMP("BHS",$J,"NVA",n,1,0)=dosage^med route^schedule^drug (file #50_";"_.01)^clinic (file #44_";"_.01)
 ;^TMP("BHS",$J,"NVA",n,"DSC",nn,0)=statement/explanation/comments
 ;
 ...
 ;
SEL(MEDSEG) ;Use selected meds only
 ...
 .S ^TMP("BHS",$J,"NVA",PSONV,1,0)=$P(NVA,"^",3)_"^"_$P(NVA,"^",4)_"^"_$P(NVA,"^",5)_"^"_$S($P(NVA,"^",2):$P(NVA,"^",2)_";"_$P(^PSDRUG($P(NVA,"^",2),0),"^"),1:"")_"^"
 .S ^TMP("BHS",$J,"NVA",PSONV,1,0)=^TMP("BHS",$J,"NVA",PSONV,1,0)_$S($D(^SC(+$P(NVA,"^",12),0)):$P(NVA,"^",12)_";"_$P(^SC($P(NVA,"^",12),0),"^"),1:"")
 .F S=0:0 S S=$O(^PS(55,DFN,"NVA",I,"DSC",S)) Q:'S  S ^TMP("BHS",$J,"NVA",PSONV,"DSC",S,0)=^PS(55,DFN,"NVA",I,"DSC",S,0)
 ;
END K PSODT,PSOST,PSORXX,PSO0,PSO2,PSOIDD,PSOFD,PSOSEL,PSODR,PSOPR,PSOREF,PSORFL,PSOI,PSOJ,PSOX,PSOCF,I,PSONV,NVA,PSONDF,INS1
 Q
CHECK ;
 Q
 ;
GET N PSOSEL,PSONDF,PSORSN
 ...
 F I=0:0 S I=$O(^PSRX(PSORXX,"SIG1",I)) Q:'I  S ^TMP("BHS",$J,PSOJ,I,0)=^PSRX(PSORXX,"SIG1",I,0)
 Q
SIG ;formats backdoor SIG
 S X=$P(^PSRX(PSORXX,"SIG"),"^") D SIGONE^PSOHELP S SIG=$E($G(INS1),2,250),ENT=1
 F SG=1:1:$L(SIG) S:$L($G(^TMP("BHS",$J,PSOJ,ENT,0))_" "_$P(SIG," ",SG))>80 ENT=ENT+1 S:$P(SIG," ",SG)'="" ^TMP("BHS",$J,PSOJ,ENT,0)=$G(^TMP("BHS",$J,PSOJ,ENT,0))_" "_$P(SIG," ",SG)
 K SIG,ENT,SG,X Q
