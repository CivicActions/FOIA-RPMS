BHSPSO7A ; IHS/MSC/MGH - Health summary selected for medications ;30-Jan-2025 13:39
 ;;1.0;HEALTH SUMMARY COMONENTS;**3,21,22**;March 17,2006;Build 47
 ...
 ;   DBIA  10011  ^DIWP
 ;
 ; IHS/MSC/MIR  11/18/2024  WRT+12 has been updated for "Hold" status Feature 115724 (patch 21)
 ;              01/30/2025  PSONDF and GENERIC(PSONDF) got removed MAIN+9 and MAIN+10 (not used) (patch 22)
 ;              01/30/2025  MAIN+14 was fixed to print NVA as well (patch 22)
MAIN ; OP Rx HS Component
 ...
 S GMTSEL=0 F  S GMTSEL=$O(GMTSEG(GMTSEGN,50,GMTSEL)) Q:'GMTSEL  D
 ...
 S GMTSLO=GMTSLO+3
 S (GMTOP,GMX,IX)=0
 F  S IX=$O(^TMP("BHS",$J,IX)) Q:IX'="NVA"&'IX  S GMR=$G(^(IX,0)) D WRT
 S GMTSLO=GMTSLO-3
...
WRT ; Writes OP Pharmacy Segment Record
 ...
 W !,?22,$P(GMR,U,6),?35,$S($P($P(GMR,U,5),";")="H":"A",1:$P($P(GMR,U,5),";")),?39,$P(GMR,U,7),?54,ID,?65,LFD,?76,"("_$P(GMR,U,8)_")",!
 ...
 W ! W:$L($P(GMR,U,12)) ?2,"Reason: ",$P(GMR,U,12),! S GMTOP=0
 Q
HEAD ; Prints Header
