BHSMEDR ;IHS/MSC/MGH - Health Summary for MED RECONCILIATION ;18-Nov-2024 11:12;DU
 ;;1.0;HEALTH SUMMARY COMPONENTS;**4,6,8,9,21**;March 17, 2006;Build 45
 ;;---------------------------------------------------------------
 ; IHS/CMI/LAB - APCHS7R -- SUMMARY PRODUCTION COMPONENTS ;
 ;;2.0;IHS PCC SUITE;**2**;MAY 14, 2009
 ;
 ;Patch 6 for non-VA meds
 ; IHS/MSC/MIR  11/18/2024  CONT+31 has been updated for "Hold" status Feature 115724 (BHS patch 21)
SET ;
 S BHSRX=P,BHSREF=0 D REF
 S BHSTAT(C,N,D,(9999999-F),M)=$S(P:$P(^PSRX(P,0),U),1:"")_U_BHSREF
 S $P(BHSTAT(C,N,D,(9999999-F),M),U,10)=P S:$G(SIG)'="" $P(BHSTAT(C,N,D,(9999999-F),M),U,11)=SIG
 S BHMDSP(D,N,F)=""
 Q
...
CONT ; <SETUP>
 N BHSPAT,BHSQ
 S BHSPAT=DFN
 I '$D(^AUPNVMED("AC",BHSPAT)),'$D(^PS(52.41,"P",BHSPAT)),'$D(^PS(55,BHSPAT,"P","A")) Q
 ...
 .I S=3 S C="HOLD MEDICATIONS" D SET Q
...
GETPEND ;
 ...
 D ADDHOLD  ;Add Medications on HOLD - IHS/MSC/MIR patch 21 Feature 115724
DISP ;DISPLAY MEDS
 ...
 D CKP^GMTSUP Q:$D(GMTSQIT)
OUT I '$D(BHSTAT("OUTSIDE PHARMACY MEDS")) G OUT2
...
MEDDSP ;DISPLAY MEDICATION
 N BHSRX,BHSORD
 S BHSN=$S(BHM'=9999999:^AUPNVMED(BHM,0),1:"")
 S BHS11=$S(BHM'=9999999:^AUPNVMED(BHM,11),1:"")
 S BHCNT=BHCNT+1
 D CKP^GMTSUP Q:$D(GMTSQIT)
 W BHCNT,".",?6,BHN,?40,"Rx #:",$P(BHZ,U,1),?60,"Refills left: ",$S('$P(BHZ,U,2):"NONE",1:$P(BHZ,U,2)),!
 D CKP^GMTSUP Q:$D(GMTSQIT)
 S BHSIG=$P(BHSN,U,5) D SIG
 S X=BHSSGY I X="" S X=$P(BHZ,U,11)
 K ^UTILITY($J,"W") S DIWL=0,DIWR=60 D ^DIWP
 D CKP^GMTSUP Q:$D(GMTSQIT)
 W ?6,"Directions: "_$S($L($G(^UTILITY($J,"W",0,1,0)))>1:$G(^UTILITY($J,"W",0,1,0)),$L($G(^UTILITY($J,"W",0,1,0)))=1:"No directions on file",1:" "),!
 I $G(^UTILITY($J,"W",0))>1 F F=2:1:$G(^UTILITY($J,"W",0)) Q:$D(GMTSQIT)  D
 ...
 K ^UTILITY($J,"W")
 D CKP^GMTSUP Q:$D(GMTSQIT)
 I BHT=1!(BHT=6) W ?6,"Last Filled: ",$$D(9999999-BHDT) D  W !
 ...
 I BHT=3 W ?6,"Reason: " I $P(BHZ,U,10) S BHSRX=$P(BHZ,U,10) W $$VAL^XBDIQ1(52,BHSRX,99)_" - "_$$VAL^XBDIQ1(52,BHSRX,99.1)_" ("_$$VAL^XBDIQ1(52,BHSRX,99.2)_")",!
...
CTR(X,Y) ;EP - Center X in a field Y wide.
 ...
 ;
ADDHOLD  ;Add Medications on HOLD - IHS/MSC/MIR patch 21 Feature 115724
 N IN,P,F,M,C,N,D,SIG,I S F=3000000,M=9999999
 S IN=0 F  S IN=$O(^PS(55,BHSPAT,"P",IN)) Q:'IN  D
 .S P=$G(^PS(55,BHSPAT,"P",IN,0)) Q:'P  Q:$$GET1^DIQ(52,P,100)'="HOLD"
 .S C="HOLD MEDICATIONS",D=$P(^PSRX(P,0),U,6),N=$P(^PSDRUG(D,0),U)
 .S SIG="" I $D(^PSRX(P,"SIG1",1)) S I=0 F  S I=$O(^PSRX(P,"SIG1",I)) Q:'I  S SIG=SIG_$G(^PSRX(P,"SIG1",I,0))_" "
 .D SET
 Q
