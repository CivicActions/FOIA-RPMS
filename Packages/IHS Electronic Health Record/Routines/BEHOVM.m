BEHOVM ;MSC/IND/DKM - Cover Sheet: Vital Measurements ;09-Feb-2021 11:34;PLS
 ;;1.1;BEH COMPONENTS;**001003,001004,001005,001006,001009,001010,001013**;Sep 18, 2007
 ;=================================================================
...
TEMPLATE(DATA,DFN,VSTR,METRIC) ;EP
 ...
 D GRID(.DATA,DFN,,,RMAX,.VITS,VSTR,.METRIC,.SD)
 Q
...
QUERY(RTN,CNT,SD) ;
 ...
 .S VCTL=+VITS(SEQ)
 .D TYPEINFO(.VCTL,.VNAM,.VABR,.VUNT,VMSR,.VTYP,.SD)
 .;I VCTL'>0!(VTYP'>0) K VITS(SEQ) Q
 .S:DEFUNT METRIC=$$DEFUNIT(VCTL,VUNT)
 .K QARY D GETCATS^BEHOVM2(.QARY,VABR)
 .S VITS(SEQ)=VCTL_U_VTYP_U_VNAM_U_VABR_U_VUNT(METRIC)_U_$S($O(^BEHOVM(90460.01,VCTL,3,0)):"BEHOVM PCTILE",1:"")_U_($D(@(QARY))=10)_U_SD
...
TYPEINFO(VCTL,VNAM,VABR,VUNT,VMSR,VTYP,VSD) ;EP
 N X
 S VCTL=$$VCTL(VCTL)
...
VALIDATE(DATA,VCTL,METRIC,X) ;EP
 N VABR,VUNT,VMSR,LP,UNIT,SD
 D TYPEINFO(.VCTL,,,.VUNT,,,.SD)
 ...
 I X'[U,UNIT'=METRIC S X=$$CONVERT(X,UNIT,SD)
 S DATA=X
 Q
