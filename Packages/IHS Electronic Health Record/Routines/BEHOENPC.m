BEHOENPC ;MSC/IND/DKM - PCC Data Management ;18-Nov-2021 10:19;DU
 ;;1.1;BEH COMPONENTS;**005003,005004,005005,005006,005007,005008,005009,005010,005011,005012,005014,005015**;Sep 18, 2007
 ;=================================================================
 ; RPC: Update PCC data
...
IMM ;; Immunizations
 ...
 ;IHS/MSC/MGH changed to call BI routine to store immunizations
 D EN^BEHOENIM(DFN,VDAT,VCAT,VLOC,VOLOC,VIEN,VAL)
...
MSR ;; Vital measurements (new format)
 ...
 .;I $P($G(^AUTTMSR(CODE,0)),U,1)="HC" D HCPC^BEHOVM6(IEN) save for EHR patch 33
...
REFUSAL(TYPE,RSN) ;
 ...
 S @FDA@(.01)=TYPE
 S @FDA@(.02)=DFN
 ;IHS/MSC/MGH EHR Patch 32 return time as well as date
 ;S @FDA@(.03)=^AUPNVSIT(VIEN,0)\1
 S @FDA@(.03)=$P($G(^AUPNVSIT(VIEN,0)),U,1)
 S @FDA@(.04)=$P(VAL,U,4)
 S @FDA@(.05)=FNUM
 S @FDA@(.06)=CODE
 S @FDA@(.08)=$$NOW^XLFDT
 S @FDA@(1101)=$P(COM,U,3,999)
