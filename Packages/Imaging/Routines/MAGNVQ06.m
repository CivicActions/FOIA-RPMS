MAGNVQ06 ;WOIFO/NST,BT - List images for Reports ; 9 Oct 2024 11:25 AM
 ;;3.0;IMAGING;**185,197,BMAG196**;Mar 19, 2002;Build 4525;May 01, 2013
 ...
 ;                or
 ;               RPT^CPRS^29027^RA^79029185.9998-*123' (return StudyIen 123)
 ;                or
...
IMAGEC(MAGZRY,DATA,IMGLESS,RARPT) ;A copy from MAGGTRAI
 ...
 S SSIEN=$E(ENT)="*" ;search StudyIen
 I SSIEN S IMGIEN=$P(ENT,"*",2),ENT=$$FNDENT(IMGIEN,DFN,INVDTTM)
 I SSIEN,'ENT S @MAGZRY@(0)="0^INVALID Data : Attempt to access Exam with Study IEN ("_STUDYIEN_") failed." Q
 S ENT=+ENT
 S INVDT=$P(INVDTTM,".",1)
 S INVTM=$P(INVDTTM,".",2)
 F  Q:($L(INVDT)<8)  S INVDT=$E(INVDT,2,$L(INVDT))
 ...
 Q
 ;
FNDENT(IMGIEN,DFN,INVDTTM) ;Find entry for study ien
 N ENT,RARPT,FOUND,STUDYIEN
 N OI,MAGIEN1
 ;
 S STUDYIEN=$P($G(^MAG(2005,IMGIEN,0)),U,10)
 I 'STUDYIEN S STUDYIEN=IMGIEN
 S ENT=0,FOUND=0
 F  S ENT=$O(^RADPT(DFN,"DT",INVDTTM,"P",ENT)) Q:'ENT  D
 . S RARPT=$P(^RADPT(DFN,"DT",INVDTTM,"P",ENT,0),U,17)
 . I RARPT D
 . . S OI=0
 . . F  S OI=$O(^RARPT(RARPT,2005,OI)) Q:'OI  D  Q:FOUND
 . . . S MAGIEN1=$P(^RARPT(RARPT,2005,OI,0),U)
 . . . ; Assure magdfn = rarpt dfn
 . . . ; Assure magien1 is pointing to this rarpt
 . . . I MAGIEN1=STUDYIEN,$P($G(^RARPT(RARPT,0)),U,2)=$P($G(^MAG(2005,MAGIEN1,0)),U,7),$P($G(^MAG(2005,MAGIEN1,2)),U,7)=RARPT S FOUND=ENT
 Q FOUND
 ;
