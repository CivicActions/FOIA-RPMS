BGOVSTR1 ;IHS/MSC/JS - Utility calls for V STROKE ;11-Nov-2021 08:26;MGH
...
SETREF(DFN,REFRES,REFDT,VFNEW) ; EP
 ...
 ;S DTDONE=$P(REFDT,".",1)
 S DTDONE=REFDT    ;IHS/MSC/MGH Allow for time on refusal patch 30
...
DELREF(VFIEN) ; EP
 ...
 I DNIRDT="" S DNIRDT=$P(NOD0,"^",15)
 ;I $G(DFN)=""!($G(DNIRDT)="")!($G(DNIRDUZ)="") Q RET
 I $G(DFN)=""!($G(DNIRDT)="") Q RET
 ;S INVDATE=9999999-DNIRDT
 S INVDATE=9999999-$P(DNIRDT,".",1)_$S($P(DNIRDT,".",2)'="":"."_$P(DNIRDT,".",2),1:"") ;P30 IHS/MSC/MGH add time
 S CPT=$$GET^XPAR("ALL","BGO STROKE TROMBO NOT DONE",1,"E")
...
GETVM(VIEN) ;
 ...
 .S DAT=VITALDT,USR=+$P($G(^AUPNVMSR(VMIEN,12)),U,4)
 .S TYPE=+X
