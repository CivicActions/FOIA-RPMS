...
PID10(CS)  ;--race
 ...
 ;
PID11(CS,DF)  ;-- address
 N PID11,ADD1,ADD2,COM,CTYI,CTY
 S ADD1=$G(^DPT(DF,.11))
 S ADD2=$G(^DPT(DF,.121))
 S COM=$P($G(^AUPNPAT(DF,11)),U,17)  ;2015 chit
 I COM S CTYI=$P(^AUTTCOM(COM,0),U,2)  ;2015 chit
 ;I CTYI S CTY=$P(^AUTTCTY(CTYI,0),U,2)_$P(^AUTTCTY(CTYI,0),U,7)  ;2015 chit
 I $G(CTYI) S CTY=$P(^AUTTCTY(CTYI,0),U,3)_$P(^AUTTCTY(CTYI,0),U,7)  ;2015 chit needs to be 5 characters
 S PID11=$P(ADD1,U)_CS_$P(ADD1,U,2)_CS_$P(ADD1,U,4)_CS_$$GET1^DIQ(5,$P(ADD1,U,5),1)_CS_$P(ADD1,U,6)_CS_"USA"_CS_"H"_CS_CS_$G(CTY)  ;2015 chit $$LZERO($$GET1^DIQ(5,$P(ADD1,U,5),2)_$P(ADD1,U,7),5)
