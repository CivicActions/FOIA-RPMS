LA7CORU ;VA/DALOI/JMC - Builder of HL7 Lab Results OBR/OBX/NTE ; 22-Oct-2013 09:22 ; MAW
 ;;5.2;AUTOMATED LAB INSTRUMENTS;**1002,27,46,61,1018,64,71,1027,68,1033,1047**;NOV 01, 1997;Build 21
 ;
 ; Cloned from LA7VORU
 ;
...
ORC ; Build ORC segment
 ;
 I $G(LA7INPT),$G(LA7ADDON) Q  ;maw 01132020 2015 chit no second ORC for reflex
 ...
 K LA7X
 M LA7X=LA("RUID")
 S ORC(2)=$$ORC2^LA7CORC(.LA7X,LA7FS,LA7ECH)
 ;
 ; Host UID
 K LA7X
 M LA7X=LA("HUID")
 ...
 . Q:'$G(LA7ORCSN)
 . S $P(ORC(3),$E(LA7ECH))=$S(LA7ORCSN>1:LA7UID_$$SUFFIX^LA7CORUB(LA7ORCSN),1:LA7UID)
 . S $P(ORC(3),$E(LA7ECH),2)="LR"
 . I $G(LA7INPT),$G(LRSS)'="MI" S $P(ORC(3),$E(LA7ECH),3)="2.16.840.1.113883.3.72.5.24"
 . I $G(LA7INPT),$G(LRSS)'="MI" S $P(ORC(3),$E(LA7ECH),4)="ISO"
 I $G(LA7INPT),$G(LA7ADDON) D
 . Q:'$G(LA7ORCSN)
 . S $P(ORC(3),$E(LA7ECH))=$S(LA7ORCSN>1:LA7UID_$$SUFFIX^LA7CORUB(LA7ORCSN),1:LA7UID)
 . S $P(ORC(3),$E(LA7ECH),2)="LR"
 ;S ORC(3)=$$ORC3^LA7CORC(.LA7X,LA7FS,LA7ECH)
 S ORC(4)=ORC(3)  ;ihs/cmi/maw mu2
 ;
 ...
 S ORC(12)=$$ORC12^LA7CORC(LA7X,LA7Y,LA7FS,LA7ECH,2)
 S $P(ORC(12),$E(LA7ECH),9)="NPI"_$E(LA7ECH,4)_"2.16.840.1.113883.4.6"_$E(LA7ECH,4)_"ISO"
 S $P(ORC(12),$E(LA7ECH),10)="L"
 S $P(ORC(12),$E(LA7ECH),13)="NPI"
 S $P(ORC(12),$E(LA7ECH),14)=LA7FAC_$E(LA7ECH,4)_"2.16.840.1.113883.3.72.5.26"_$E(LA7ECH,4)_"ISO"
 S $P(ORC(12),$E(LA7ECH),21)=$P(ORC(12),$E(LA7ECH),6)
 I $G(LA7INPT) S $P(ORC(12),$E(LA7ECH),7)=""  ;mu2 inpatient
 ;
 ;provider call back number
 ;S ORC(14)=$$ORC14^LA7CORC(LA7X,LA7Y,LA7FS,LA7ECH)
 S ORC(14)=$$OBR17^LA7COBR(LA7FS,LA7ECH)  ;MU2 order call back number
 ;
 ; Enterer's location
 ;not used in MU2
 ;S LA7X=""
 ...
 I LA7X D
 . S ORC(21)=$$ORC21^LA7CORC(LA7X,LA7FS,LA7ECH)
 . S $P(ORC(21),$E(LA7ECH,1),6)="NIST Sending App"_$E(LA7ECH,4)_"2.16.840.1.113883.3.72.5.21"_$E(LA7ECH,4)_"ISO"
 . S $P(ORC(21),$E(LA7ECH,1),10)=$P($G(^DIC(4,LA7X,99)),U)
 . S ORC(22)=$$ORC22^LA7CORC(LA7X,$P(LA763(0),"^"),LA7FS,LA7ECH)
 ;
 S ORC(23)=$$ORC23^LA7CORC(DUZ(2),DT,LA7FS,LA7ECH)
 S ORC(24)=$$ORC24^LA7CORC(LA7OP,DT,LA7FS,LA7ECH)
...
OBX ;Observation/Result segment for Lab Results
 ...
 . ; Build OBX segment
 . K LA7DATA
 . D OBX^LA7COBX(LA("LRDFN"),LA("SUB"),LA("LRIDT"),$P(LA7VT,"^",1,2),.LA7DATA,.LA7OBXSN,LA7FS,LA7ECH,$G(LA7NVAF))
...
TQ1(FS,ECH,ACC,LDFN,LIDT) ; build the TQ1 segment for MU2
 ...
 S TQ1="TQ1"
 S TQ1(1)=1
 S TQ1(7)=$$FMTHL7^XLFDT($P($G(^LR(LDFN,LRSS,LIDT,0)),U))
 S TQ1(8)=$$FMTHL7^XLFDT($P($G(^LR(LDFN,LRSS,LIDT,0)),U,3))
 F J=1:1:14 S $P(TQ1,FS,J+1)=TQ1(J)
 Q TQ1
 ;
SPM(FS,ECH,ACC) ; build the SPM segment for MU2
 ...
 S SPM="SPM"
 S SPM(1)=1
 S SPM(2)=$$SPM2^LA7MUSPM(CS,SC,ACC)
 S SPM(4)=$$SPM4^LA7MUSPM(CS,ACC,LRSPEC)
 N ROI
 S ROI=$O(^BLRRLO("ACC",ACC,0))
 I ROI S SPM(5)=$$SPM5^LA7MUSPM(CS,ACC,LRSPEC,ROI)  ;TODO
 I $G(LRSAMP) S SPM(6)=$$SPM6^LA7MUSPM(CS,ACC,LRSPEC)  ;TODO  LOOK AT FILE 62
 I $G(LRSAMP) S SPM(7)=$$SPM7^LA7MUSPM(CS,ACC,LRSPEC)  ;TODO  LOOK AT FILE 62
 I $G(LRSAMP) S SPM(8)=$$SPM8^LA7MUSPM(CS,ACC,LRSPEC)  ;TODO  LOOK AT FILE 62
 I ROI S SPM(9)=$$SPM9^LA7MUSPM(CS,ACC,LRSPEC,ROI)  ;TODO
 S SPM(11)=$$SPM11^LA7MUSPM(CS,ACC,LRSPEC)  ;TODO  FILE 61 TIME ASPECT FIELD
 S SPM(12)=$$SPM12^LA7MUSPM(CS,SC,ACC,LRSPEC)  ;TODO  LOOKS LIKE HARD CODE RIGHT NOW
 S SPM(17)=$$SPM17^LA7MUSPM(CS,LRDFN,LRIDT)
 S SPM(18)=$$SPM18^LA7MUSPM(CS,LRDFN,LRIDT)
 S SPM(21)=$$SPM21^LA7MUSPM(CS,ACC,LRSPEC)
 F J=1:1:29 S $P(SPM,FS,J+1)=SPM(J)
 Q SPM
 ;
