PSIVLABR ;BIR/PR-REPRINT LABELS ;05-Feb-2025 13:58;DU
 ...
 ;
 ; Modified - IHS/CIA/PLS - 03/31/04 - Line DEM+4
...
START S PSIV1=1,LINE=0 D RE D
 ...
 D:'$D(PSIVCT) ^PSIVSTAT
Q K PSIV,PSIVDOSE,PSIVCT,PSIVWD,P16,LINE,MESS,PSIV2,PSIVFLAG,PSIVRM,PSIV1,PDOSE,PDATE,XX1,XX2,BAG,CX,PSIMESS,TEXT1 Q
RE ;
 ;NEED THE CODE BELOW?
 ...
 .N EXDT,XX1
 .;IHS/MSC/PB - 2/13/13 modified the line below to set TEXT1 = "________" if the offset is '>0
 .;Q:$P($G(^PS(55,DFN,"IV",+ON,9999999)),"^")'>0
...
INF S X=$P(P(8),"@") D:X]"" P
 ...
 ;S X=PSIVBAG D P
 ;IHS KCF VAOIT  modified the following line to print the expiration date to print on the iv label IF EXP DATE PRINTS TO RIGHT OF LABEL COUNT
 S X=PSIVBAG I $G(TEXT1)'="" D OFFSET^PSIVLABL(X,"Do Not Use After: "_TEXT1) S X=$G(PRTLINE)
 D P
 ;IHS/MSC/PB - 04/25/12 - modified the following line to print the expiration date to print on the iv label
 ;IHS/MSC/PB - 12/10/12 - modified the line below to check for TEXT1 to exist.
 ;D OFFSET^PSIVLABL("Do Not Use After:",TEXT1) S X=$G(PRTLINE) D P
 ;I $G(TEXT1)'="" D OFFSET^PSIVLABL("Do Not Use After:",TEXT1) S X=$G(PRTLINE) D P
 ;MSC/IHS/PB - 3/1/13 end of modification
 Q
