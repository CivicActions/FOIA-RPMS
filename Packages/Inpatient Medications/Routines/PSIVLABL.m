PSIVLABL ;BIR/PR-PRINT OUT LABELS ;05-Feb-2025 06:52;NIC
 ;;5.0; INPATIENT MEDICATIONS ;**58,82,104,127,178,184,1010,1015,1035**;16 DEC 97;Build 39
 ;
...
DEM ;Get demographics and see if label is example only
 ...
 D ENIV^PSJAC,NOW^%DTC
 S PSIVNOW=$$ENDTC^PSGMI(%),VADM(2)=$G(VA("BID"))
 D SETP
 S PSIVWD=$S(+VAIN(4):$P(VAIN(4),U,2),1:$$GETHLOC^PSIVLABR(.P))
 I $D(PSIVEXAM) G ENX
 ;
 G:PSIVNOL<1 Q D SETP
 S PSIVRM=$P(PSIVSITE,U,13),P16=$P($G(^PS(55,DFN,"IV",+ON,9)),U,3) S:PSIVRM<1 PSIVRM=30 I $D(PSIVCT),PSIVCT'=1 K PSIVCT
