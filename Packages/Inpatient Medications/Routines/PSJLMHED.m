PSJLMHED ;BIR/MLM-BUILD LM HEADERS ;27-Nov-2023 15:11;DU
 ;;5.0; INPATIENT MEDICATIONS ;**4,58,85,110,148,1015,1034**;16 DEC 97;Build 37
 ;
 ; Reference to ^PS(55 is supported by DBIA 2191.
 ; Reference to CWAD^ORQPT2 is supported by DBIA 2831.
 ; Reference to ^SC is supported by DBIA 10040.
 ; Modified - IHS/CIA/PLS - 12/05/03 - Line HDRO+8
 ;            IHS/MSC/PLS - 04/17/23 - Line HDRO+4
 ;                          11/21/23 - Shifted Ward, Room-bed
 ;                          11/27/23 - Removed LAST output for previously admitted patients
...
HDRO(DFN) ; Standardized part of profile header.
 N PSJCLIN,PSJAPPT,PSJCLINN,RMORDT,INPT S (PSJCLIN,PSJAPPT)=0,(RMORDAT,PSJCLINN)="" I $G(PSJORD) D
 ...
 ;IHS/MSC/PLS - p1034
 S INPT=$$ISINPT^APSPFNC1(DFN)
 ;K VALMHDR I PSJCLINN]"" S PSJ=VADM(1),PSJ=$$SETSTR^VALM1("   Clinic: "_PSJCLINN,PSJ,28,26)
 K VALMHDR I PSJCLINN]"" S PSJ=$$GETPREF^AUPNSOGI(DFN,"E",1),PSJ=$$SETSTR^VALM1("   Clinic: "_PSJCLINN,PSJ,28,26)
 ;I PSJCLINN="" S PSJ=VADM(1),PSJ=$$SETSTR^VALM1($S('PSJPDD:"     ",1:"Last ")_"Ward: "_PSJPWDN,PSJ,30,18)
 I PSJCLINN="" S PSJ=$$GETPREF^AUPNSOGI(DFN,"E",1)
 ;S PSJ=$$SETSTR^VALM1($S('PSJPDD:"     ",1:"Last ")_"Ward: "_PSJPWDN,PSJ,30,18)
 S X=$$CWAD^ORQPT2(DFN)
 ...
 ;IHS/MSC/PLS - 11/21/2023 - P1034
 S:INPT PSJ=$$SETSTR^VALM1("Ward: "_PSJPWDN,PSJ,29,25)
 S RMORDT=$S(INPT:"Room-Bed: "_$G(PSJPRB),1:"")
 ...
 S VALMHDR(2)=$$SETSTR^VALM1("Ht(cm): "_PSJPHT_" "_PSJPHTD,PSJ,55,25)
 ;IHS/MSC/PLS - 11/21/2023 - P1034
 ;S PSJ="   DOB: "_$P($P(PSJPDOB,U,2)," ")_" ("_PSJPAGE_")",VALMHDR(3)=$$SETSTR^VALM1("Wt(kg): "_PSJPWT_" "_PSJPWTD,PSJ,55,25)
 S PSJ="   DOB: "_$P($P(PSJPDOB,U,2)," ")_" ("_PSJPAGE_") "
 S PSJ=$$SETSTR^VALM1(RMORDT,PSJ,25,28)
 S VALMHDR(3)=$$SETSTR^VALM1("Wt(kg): "_PSJPWT_" "_PSJPWTD,PSJ,55,25)
 Q
 ;
