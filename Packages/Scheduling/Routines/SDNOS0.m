SDNOS0 ;ALB/LDB - NO SHOW REPORT ; 07 May 99 10:21 AM
 ;;5.3;PIMS;**20,194,410,517,523,1015,1016,1022**;MAY 28, 2004;Build 18
...
SET ;S:$P(SDCL1,U,15)&SDDIVO&SDV1 SDDIV=$P(SDCL1,U,15) S ^UTILITY($J,"SDNO",SDDIV,$P(SDCL1,U),SDBEG1,$P(^DPT(SDPAT,0),U),+$P(^(0),U,9))=SD2_U_SD10_U_SD12  ;IHS/ANMC/LJF 11/03/2000
 S:$P(SDCL1,U,15)&SDDIVO&SDV1 SDDIV=$P(SDCL1,U,15) S ^UTILITY($J,"SDNO",SDDIV,$P(SDCL1,U),SDBEG1,$$GETPREF^AUPNSOGI(SDPAT,"E",1),+$$HRCN^BDGF2(SDPAT,+$G(DUZ(2))))=SD2_U_SD10_U_SD12_U_SDPAT  ;IHS/ANMC/LJF 11/03/2000
