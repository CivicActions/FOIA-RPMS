BGP5NPL ; IHS/CMI/LAB - national patient list ; 20 Dec 2004  9:24 AM
 ;;5.0;IHS CLINICAL REPORTING;**1**;OCT 15, 2004
 ;
 ;
 ;
 W:$D(IOF) @IOF
 W !,$$CTR("IHS GPRA Performance Report Patient List",80)
 W !,$$CTR("CRS 2005, Version 5.0, Patch 1",80)
INTRO ;
 D XIT
 W !!,"This will produce a list of patients who either met, did not meet"
 W !,"or to list both those that met and did not meet a National indicator."
 W !,"You will be asked to select one or more indicator topics and then to choose"
 W !,"which indicator numerators you would like to report on.",!!
...
TP ;get time period
 D XIT
 S BGPRTYPE=1,BGP5RPTH="",BGPNPL=1,BGPINDT="G",BGP5GPU=1
SI ;
 K DIRUT
 K BGPIND
 D EN^BGP5DSI
 I '$D(BGPIND) W !!,"No indicators selected." H 3 D XIT Q
SI1 ;NOW SELECT ONE OR MORE W/IN THE TOPIC
 ...
 .K BGPX S X=0,BGPC=0 F  S X=$O(^BGPVNPL("B",BGPIND,X)) Q:X'=+X!($D(DIRUT))  S BGPX(X)="",BGPC=BGPC+1
 .;display the choices
 .W !!!,"Please select one or more of these report choices within the",!,$P(^BGPINDV(BGPIND,0),U,3)," indicator topic.",!
 .K BGPY S X=0,BGPC=0 F  S X=$O(BGPX(X)) Q:X'=+X!($D(DIRUT))  S BGPC=BGPC+1 W !?10,BGPC,")",?14,$P(^BGPVNPL(X,0),U,3) S BGPY(BGPC)=X
 ...
 ;get report type
 I $D(DIRUT) G TP
 D RT^BGP5DSL I '$D(BGPLIST)!($D(BGPQUIT)) G SI1
 ;BEGIN TEST STUFF
...
SUM ;display summary of this report
 ...
 ;W !!,"All AREA DIRECTOR CLINICAL indicators will be calculated."
 D PT^BGP5DSL
 I BGPROT="" G BEN
ZIS ;call to XBDBQUE
 D REPORT^BGP5UTL
...
DRIVER ;
 ...
 ;
NODEV ;
 S XBRP="",XBRC="NODEV1^BGP5NPL",XBRX="XIT^BGP5NPL",XBNS="BGP"
 D ^XBDBQUE
 Q
 ;
NODEV1 ;
...
TSKMN ;EP ENTRY POINT FROM TASKMAN
 ...
 S ZTCPU=$G(IOCPU),ZTRTN="DRIVER^BGP5NPL",ZTDTH="",ZTDESC="NATIONAL GPRA REPORT 05" D ^%ZTLOAD D XIT Q
 Q
 ;
XIT ;
 D ^%ZISC
