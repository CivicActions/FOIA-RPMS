BGP8AUUP ; IHS/CMI/LAB - UPLOAD SITE FILES, CREATE EXCEL FILES ;
...
EN(BGPBEGDT,BGPENDDT,BGPMAN) ;EP -- MAIN ENTRY POINT
 ...
 ;quit trying to process after 5 tries
 ...
 . I $G(BGPMAN) D  Q
 . . W !,BGPERR
 . . D MSG1^BGP8AU1M(BGPBEGDT,BGPENDDT,.BGPFACS)
 ...
 Q:BGPOUT
 ;
 ;upload all files into the BGP 18 DATA* files
 D UPLOAD(BGPPATH,BGPBEGDT,BGPENDDT,.BGPFACS)
 ;
 ;create the 5 excel files
 D EN^BGP8AU1A(BGPBEGDT,BGPENDDT,.BGPEND,.BGPF)
 ;
 ...
 ;
 ;
 D EN^BGP8AU1D(BGPBEGDT,BGPEND,.BGPDFILE)
 ;
 ;
 ...
 S ZTDTH=BGPX
 ;S ZTSAVE("BGP*")=""
 S ZTRTN="AUTO^BGP8AUUP"
 S ZTDESC="BGP8 AUTO GPRA AREA AGGREGATE"
 S ZTIO=""
 D ^%ZTLOAD
 Q
...
CLEAN(BGPBEGDT,BGPENDDT) ;
 ...
 S BGPD0=0 F  S BGPD0=$O(^BGPGPDCR("B",BGPBEGDT,BGPD0)) Q:'BGPD0  D
 . S BGPASU=$P(^BGPGPDCR(BGPD0,0),U,9)
 . I $G(BGPFACS(BGPENDDT,BGPASU))="" Q
 . Q:$P(^BGPGPDCR(BGPD0,0),U,12)'=1  ;DON'T DELETE OTHER NATIONAL, GPU, ETC.
 . S DA=BGPD0
 . S DIK="^BGPGPDCR("
 . D ^DIK
 ;
 S BGPD0=0 F  S BGPD0=$O(^BGPGPDPR("B",BGPBEGDT,BGPD0)) Q:'BGPD0  D
 . S BGPASU=$P(^BGPGPDPR(BGPD0,0),U,9)
 . I $G(BGPFACS(BGPENDDT,BGPASU))="" Q
 . Q:$P(^BGPGPDPR(BGPD0,0),U,12)'=1  ;DON'T DELETE OTHER NATIONAL, GPU, ETC.
 . S DA=BGPD0
 . S DIK="^BGPGPDPR("
 . D ^DIK
 ;
 S BGPD0=0 F  S BGPD0=$O(^BGPGPDBR("B",BGPBEGDT,BGPD0)) Q:'BGPD0  D
 . S BGPASU=$P(^BGPGPDBR(BGPD0,0),U,9)
 . I $G(BGPFACS(BGPENDDT,BGPASU))="" Q
 . Q:$P(^BGPGPDBR(BGPD0,0),U,12)'=1  ;DON'T DELETE OTHER NATIONAL, GPU, ETC.
 . S DA=BGPD0
 . S DIK="^BGPGPDBR("
 . D ^DIK
 Q
...
READ(BGPPATH,BGPFILE,BGPOUT) ;
 ;----- READ CONTENTS OF DATA FILE AND PUT INTO ^TMP
 ;
...
ALERTMM1(BGPMSG,BGPBEGDT,BGPENDDT,BGPFACS,BGPF,BGPDFILE,BGPUSR) ;
 ...
 .;S C=C+1,BGPTEXT(C)=BGPFGNT4
 .;S C=C+1,BGPTEXT(C)=BGPFGNT5
 .;S C=C+1,BGPTEXT(C)=BGPFGNT6
