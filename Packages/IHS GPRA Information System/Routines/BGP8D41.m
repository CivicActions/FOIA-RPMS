...
I0302 ;EP
 ...
 I BGPACTCL,$$CHD^BGP8D729(DFN,BGP365,BGPEDATE) S BGPIHD=1,BGPD3=1
...
BPCV ;EP - called from elder care routine
 ...
 I BGPVALUE["unknown" S BGPVALUE=$$BPCPT^BGP8D22(DFN,BGPBDATE,BGPEDATE) I BGPVALUE]"" S BGPN1=1,BGPVALUE=$P(BGPVALUE,U,2) G V  ;CPTS COUNT IN NUMERATOR 1 ONLY
...
DEP(P,BDATE,EDATE) ;EP
 S X=$$DEP^BGP8D25(P,BDATE,EDATE) I $P(X,U) Q X
 S X=$$DEPSCR^BGP8D25(P,BDATE,EDATE) I $P(X,U) Q X
 Q ""
DEPCHD(P,BDATE,EDATE) ;EP
 S X=$$DEP^BGP8D25(P,BDATE,EDATE) I $P(X,U) Q X
 S X=$$DEPSCR^BGP8D25(P,BDATE,EDATE) I $P(X,U) Q X
 S X=$$DEPSUIC^BGP8D25(P,BDATE,EDATE) I $P(X,U) Q X
 Q ""
LIFE(P,BDATE,EDATE) ;EP
 S X=$$MEDNUTR^BGP8D711(P,BDATE,EDATE) I X]"" Q 1_U_X
 S X=$$SPECNUTR^BGP8D711(P,BDATE,EDATE) I X]"" Q 1_U_X
 S X=$$SPECEX^BGP8D711(P,BDATE,EDATE) I X]"" Q 1_U_X
 S X=$$OTHREL^BGP8D711(P,BDATE,EDATE) I X]"" Q 1_U_X
 Q ""
LIFED(P,BDATE,EDATE) ;EP
 S X=$$MEDNUTRD^BGP8D711(P,BDATE,EDATE) I X]"" Q 1_U_X
 S X=$$SPECNUTR^BGP8D711(P,BDATE,EDATE) I X]"" Q 1_U_X
 S X=$$SPECEX^BGP8D711(P,BDATE,EDATE) I X]"" Q 1_U_X
 S X=$$OTHREL^BGP8D711(P,BDATE,EDATE) I X]"" Q 1_U_X
 Q ""
TOBACCO(P,BDATE,EDATE) ;EP
 S X=$$TOBACCO^BGP8D7(P,BDATE,EDATE) I X]"" Q 1_U_X
 S X=$$TOBDX(P,BDATE,EDATE) I X]"" Q 1_U_X
 S X=$$PED^BGP8D7(P,BDATE,EDATE) I X]"" Q 1_U_X
 S X=$$DENT^BGP8D7(P,BDATE,EDATE) I X]"" Q 1_U_X
 S X=$$CPTSM^BGP8D7(P,BDATE,EDATE) I X]"" Q 1_U_X
 Q ""
...
TOBDX(P,BDATE,EDATE) ;EP
 ...
 .I $E($P($$ICDDX^BGP8UTL2(Y),U,2),1,5)'="305.1",$P($$ICDDX^BGP8UTL2(Y),U,2)'="V15.82" Q
 .S G=$P($$ICDDX^BGP8UTL2(Y),U,2)_" PL"
 .Q
 Q G
