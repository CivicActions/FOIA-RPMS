BGPLPC12 ; IHS/CMI/LAB - measure I2 ; 02 Feb 2019  11:25 AM
 ;;19.1;IHS CLINICAL REPORTING;**1**;MAY 30, 2019;Build 7
 ;
DFU S (BGPN1,BGPD1)=0
 S BGPFDV="" K BGPDEPV,BGPV,BGPDOV
 ;
 I BGPAGEB<12 S BGPSTOP=1 Q  ;12 or greater during time period
 ;
...
DFU1 ;
 S BGPD1=1
 S BGPVALUE=""
 S BGPVALUE="ENC "_$$DATE^BGPLUTL((9999999-BGPFDV))_"|||"  ;hit denominator
 I BGPN1 S BGPVALUE=BGPVALUE_"*** "_$$DATE^BGPLUTL($P(G,U,2))_" "_$P(G,U,4)_" "_$P(G,U,6)_" "_$P(G,U,7)
 I 'BGPN1 S BGPVALUE=BGPVALUE_$$DATE^BGPLUTL($P(G,U,2))_" "_$P(G,U,4)_" "_$P(G,U,6)_" "_$P(G,U,7)
 ;
DFUE ;
 K D,V,X,Y,BGPV,BGPDEPV,BGPFDV,BGPDOV,G
 Q
DEPV(V) ;EP
 ;V19.1 PATCH 1 ADDED V POV CHECK BECAUSE SOME OF THE SNOMEDS IN THIS SUBSET ARE DX RELEATED (E.G. BREAST EXAM)
 NEW A,B,C
 S A=0,B=""
...
HASDX(P,BDATE,EDATE) ;
 NEW X,Y,Z
 I $$PLTAXND^BGPLDU(P,"BGP IPC DEPRESSION DIAG DXS",EDATE,1) Q 1
 I $$IPLSNOND^BGPLDU(P,"PXRM BGP IPC DEPRESSION DX",EDATE,1) Q 1
 I $$LASTDX^BGPLUTL1(P,"BGP IPC DEPRESSION DIAG DXS",BDATE,EDATE) Q 1
 I $$PLTAXND^BGPLDU(P,"BGP IPC BIPOLAR DIAG DXS",EDATE,1) Q 1
 I $$IPLSNOND^BGPLDU(P,"PXRM BGP IPC BIPOLAR DX",EDATE,1) Q 1
 I $$LASTDX^BGPLUTL1(P,"BGP IPC BIPOLAR DIAG DXS",BDATE,EDATE) Q 1
...
BHDX(P,BDATE,EDATE) ;has a bh dx in either taxonomy BDATE to EDATE?
 ...
 .I 'Z Q
 .I $$ICD^BGPLUTL2(Z,$O(^ATXAX("B","BGP IPC BIPOLAR DIAG DXS",0)),9) S G=1 Q
 .I $$ICD^BGPLUTL2(Z,$O(^ATXAX("B","BGP IPC DEPRESSION DIAG DXS",0)),9) S G=1 Q
 .Q
 Q G
...
FUPLAN(P,BDATE,EDATE) ;
 ...
 .S S=$P(T,"-",1)
 .S C=$$ICDDX^BGPLUTL2(S)
 .I $P(C,U,1)'="-1",$$ICD^BGPLUTL2($P(C,U,1),$O(^ATXAX("B","BGP IPC DEPRESSION DIAG DXS",0)),9) S G=1_U_"F/U Pt Ed "_T Q
 .I $P(C,U,1)'="-1",$$ICD^BGPLUTL2($P(C,U,1),$O(^ATXAX("B","BGP IPC BIPOLAR DISORDER DXS",0)),9) S G=1_U_"F/U Pt Ed "_T Q
 .;is it a snomed?
 ...
 I G Q G
 ;v med first
 D GETMEDS^BGPLUTL2(P,BDATE,EDATE,"BGP IPC DEPRESSION MEDS","",,,.BGPMEDS1,"BGP IPC DEPRESSION RXNORM")
 ...
 S TAX1=$O(^BGPSNOML("B","BGP IPC DEPRESSION RXNORM",0))
 ...
 .I B]"",$D(^BGPSNOML(TAX1,11,"B",B)) S M=1
 .Q:'M
 .;ORDER
