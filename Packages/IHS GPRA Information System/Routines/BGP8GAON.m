BGP8GAON ; IHS/CMI/LAB - AREA OTHER NATIONAL MEASURES GPRA REPORT 01 Jul 2010 1:10 PM ;
...
TESTNTL ;
 ...
 S BGPSUL(1)=""
 S BGPSUL(2)=""
 D EP(.ERR,1,2522,"BGP 18 AREA OTHER NATIONAL MEASURES REPORT","A",.BGPSUL,1,3030000,3000000,1,"B",$$NOW^XLFDT)
 W !,ERR
 Q
EP(BGPRET,BGPUSER,BGPDUZ2,BGPOPTN,BGPRPTT,BGPSUL,BGPQTR,BGPPER,BGPVDT,BGPBEN,BGPROT,BGPRTIME,BGPHC,BGPFILE,BGPDNT) ;EP - called from GUI to produce national gpra report (AO-AGP)
 ...
 ;  BGPSUL  - array containing Iens from BGP 18 ONM DATA CURRENT that the user selected
...
EP1 ;
 ...
 S BGPSUCNT=0
 S BGPSUCNT=C
 I BGPSUCNT=1 S Y=$O(BGPSUL(0)),X=$P(^BGPGPDCR(Y,0),U,9),X=$O(^AUTTLOC("C",X,0)) I X S BGPSUNM=$P(^DIC(4,X,0),U)
 ...
 S BGPFONN1="CRSONMNT1"_$P(^AUTTLOC(DUZ(2),0),U,10)_2010063000000000_$$D^BGP8UTL(BGPNOW)_"_"_$$LZERO^BGP8UTL(BGPC,6)_".TXT"
 S BGPFONN2="CRSONMNT2"_$P(^AUTTLOC(DUZ(2),0),U,10)_2010063000000000_$$D^BGP8UTL(BGPNOW)_"_"_$$LZERO^BGP8UTL(BGPC,6)_".TXT"
 S BGPFONN3="CRSONMNT3"_$P(^AUTTLOC(DUZ(2),0),U,10)_2010063000000000_$$D^BGP8UTL(BGPNOW)_"_"_$$LZERO^BGP8UTL(BGPC,6)_".TXT"
 S BGPFONN4="CRSONMNT4"_$P(^AUTTLOC(DUZ(2),0),U,10)_2010063000000000_$$D^BGP8UTL(BGPNOW)_"_"_$$LZERO^BGP8UTL(BGPC,6)_".TXT"
 ;S BGPFONN5="CRSONMNT5"_$P(^AUTTLOC(DUZ(2),0),U,10)_2010063000000000_$$D^BGP8UTL(BGPNOW)_"_"_$$LZERO^BGP8UTL(BGPC,6)_".TXT"
 ;create entry in GUI file
 D ^XBFMK
 S X=BGPFILE
 S DIC="^BGPGUIR(",DIC(0)="L",DIADD=1,DLAYGO=90560.19
 S DIC("DR")=".02////"_BGPUSER_";.03////"_$S(BGPRTIME]"":BGPRTIME,1:$$NOW^XLFDT)_";.05///"_BGPOPTN_";.06///R;.07///"_$G(BGPROT)_";1///"_$G(BGPFONN1)_", "_$G(BGPFONN2)_", "_$G(BGPFONN3)_", "_$G(BGPFONN4)  ;_", "_$G(BGPFONN5)
 ...
 D TSKMN
 S BGPRET=BGPGIEN
 S $P(BGPRET,"~",3)=$G(BGPFONN1)_","_$G(BGPFONN2)_","_$G(BGPFONN3)_","_$G(BGPFONN4) ;_","_$G(BGPFONN5)
 Q
 ;
TSKMN ;
 S ZTIO=""
 K ZTSAVE S ZTSAVE("*")=""
 S ZTCPU=$G(IOCPU),ZTRTN="AONM^BGP8GAON",ZTDTH=$S(BGPRTIME]"":BGPRTIME,1:$$NOW^XLFDT),ZTDESC="GUI AREA ONM REPORT" D ^%ZTLOAD
...
AONM ;
 ...
 D GUIR^BGPXBLM("PRINT^BGP8PARQ","^TMP($J,""BGPGUI"",")
