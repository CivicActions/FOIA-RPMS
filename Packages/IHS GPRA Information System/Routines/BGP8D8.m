BGP8D8 ; IHS/CMI/LAB - measure C 11 Feb 2018 3:35 PM ;
 ;;18.0;IHS CLINICAL REPORTING;**1**;NOV 21, 2017;Build 62
 ;
IE2 ;EP
 S (BGPN1,BGPN2,BGPN3,BGPN4,BGPN5,BGPN6,BGPN7,BGPN8,BGPD1,BGPD2,BGPD3,BGPD4,BGPD5,BGPD6,BGPD7,BGPD8,BGPD9,BGPD10,BGPD11,BGPD12,BGPD13,BGPD14,BGPD15,BGPD16,BGPD17,BGPD18,BGPD19)=0
 ...
 I BGPACTCL,BGPSEX="F",$$PREG^BGP8D7(DFN,$$FMADD^XLFDT(BGPEDATE,-608),BGPEDATE,1,1,"",BGPBDATE,BGPEDATE),BGPNO2="" S BGPD1=1
 ;I BGPACTUP,BGPSEX="F",$$PREG^BGP8D7(DFN,$$FMADD^XLFDT(BGPEDATE,-608),BGPEDATE,1,1),BGPNO2="" S BGPD18=1
 ...
 ;
 I BGPACTUP,BGPAGEB>12,BGPAGEB<65,BGPDXB="" S BGPD2=1
 ...
 I $P(^DPT(DFN,0),U,2)="M",BGPAGEB>24,BGPAGEB<46,BGPDXB="" S BGPD19=1
 ...
 S BGPCD4=""
 I BGPFIRST D
 .S D=$P(BGPFIRST,U,3) S BGPCD4=$$CD4RES^BGP8D841(DFN,$$FMADD^XLFDT(D,-90),$$FMADD^XLFDT(D,90),1)
 .Q:'BGPCD4
 .S BGPN9=1
 .S R=$P(BGPCD4,U,3)
 .I R="" S BGPN14=1 Q
 ...
 S BGPN7=0
 ;
 S BGPHIVR=$$HIVTEST1^BGP8D812(DFN,BGPBDATE,$S(BGPHIVD:BGPHIVD,1:BGPEDATE))
 S BGPDAFT=""
...
V ;
 ...
 I BGPD1 D
 .I BGPN1 S BGPVAL=$P(BGPHIVG,U,2)_$S(BGPD1:" (20 months)",1:"") Q
 I BGPD2,'BGPN1,BGPHIVE S X=$P(BGPHIVE,U,2),L=$L(X,","),BGPVAL=$P(X,",",L)_" "
 ...
 S %="",D="",R=""
 I BGPHIV S %=" 1yr ",D=$P($P(BGPHIV,U,4)," ")_" ",E=""
 ...
 I BGPCD4 S:BGPVALUD'["|||" BGPVALUD=BGPVALUD_"|||" S:$P(BGPVALUD,"|||",2)]"" BGPVALUD=BGPVALUD_";" S BGPVALUD=BGPVALUD_"CD4="_$S($P(BGPCD4,U,3)]"":$P(BGPCD4,U,3),1:"No Result")
...
IE1 ;EP
 ...
 S BGPCD4=$$CD4^BGP8D812(DFN,BGPBDATE,BGPEDATE)
 S BGPPCR=$$PCR^BGP8D811(DFN,BGPBDATE,BGPEDATE)
 S BGPRX=$$ART^BGP8D811(DFN,BGPBDATE,BGPEDATE)
 I BGPCD4,'BGPPCR S BGPN1=1
 I BGPPCR,'BGPCD4 S BGPN2=1
 I BGPPCR,BGPCD4 S BGPN3=1
 I (BGPN1+BGPN2+BGPN3) S BGPN4=1
 I BGPRX S BGPN5=1
 S BGPVALUE="UP|||"_$S(BGPN3:"BOTH: ",1:"")_$S(BGPCD4:"CD4: "_$$DATE^BGP8UTL($P(BGPCD4,U,2))_" "_$P(BGPCD4,U,3),1:"")
 I $P(BGPVALUE,"|||",2)]"" S BGPVALUE=BGPVALUE_"; "
 S BGPVALUE=BGPVALUE_$S(BGPPCR:"Viral Load: "_$$DATE^BGP8UTL($P(BGPPCR,U,2))_" "_$P(BGPPCR,U,3),1:"")
 I $P(BGPVALUE,"|||",2)]"" S BGPVALUE=BGPVALUE_"; "
...
PHYACT ;EP
 ...
 I '(BGPACTUP+BGPACTCL) S BGPSTOP=1 Q
 I BGPACTCL,BGPAGEB>4,BGPAGEB<12 S BGPD3=1
 I BGPACTCL,BGPAGEB>11,BGPAGEB<20 S BGPD4=1
 I BGPACTCL,BGPAGEB>19,BGPAGEB<25 S BGPD5=1
 I BGPACTCL,BGPAGEB>24,BGPAGEB<35 S BGPD6=1
 ...
 I BGPEXER S BGPN2=1,BGPVALUE=BGPVALUE_"; EXER ED: "_$$DATE^BGP8UTL($P(BGPEXER,U,1))_" "_$P(BGPEXER,U,2)
 S BGPGOAL="" I BGPPHYA S BGPGOAL=$$GOAL(DFN,$P(BGPPHYA,U,2),BGPEDATE)
 I BGPGOAL S BGPN3=1,BGPVALUE=BGPVALUE_"; GOAL SET: "_$$DATE^BGP8UTL($P(BGPGOAL,U,1)) ;_" "_$P(BGPGOAL,U,2)
...
EXER(P,BDATE,EDATE) ;
 ...
 I $D(BGPG(1)) Q $P(BGPG(1),U)_U_$P(BGPG(1),U,2)_U_$$DATE^BGP8UTL($P(BGPG(1),U,1))
 S Y="BGPALLED("
 ...
 .S S=$P(T,"-",1)
 .S S=$$ICDDX^BGP8UTL2(S)
 .I $P(S,U,1)'="-1",$$ICD^BGP8UTL2($P(S,U,1),$O(^ATXAX("B","BGP EXERCISE COUNSELING DXS",0)),9) S %=$P(BGPALLED(X),U)_U_T Q
 Q %
FIRSTHF(P,BDATE,EDATE,CAT) ;EP
 ...
 S D=$O(O(0))
 I D="" Q D
 Q 1_U_D_U_$$DATE^BGP8UTL(D)_U_$$VAL^XBDIQ1(9000010.23,O(D),.01)
 ;
HIVDX1(P,EDATE,BDATE) ;EP - any HIV dx
 ...
 .Q
 Q G
HIVDX(P,EDATE,BDATE,RPE) ;EP - any HIV dx ever or PL
 NEW BGPG,G,Y,X,T,E
 K BGPG
 S Y="BGPG("
 S BDATE=$G(BDATE)
 I BDATE="" S BDATE=$P(^DPT(P,0),U,3)
 I $G(RPE)="" S RPE=$G(BGPEDATE)
 S X=P_"^LAST DX [BGP HIV/AIDS DXS;DURING "_$$FMTE^XLFDT(BDATE)_"-"_$$FMTE^XLFDT(EDATE) S E=$$START1^APCLDF(X,Y)
 I $D(BGPG(1)) Q $P(BGPG(1),U)
 S T=$O(^ATXAX("B","BGP HIV/AIDS DXS",0))
 S X=0,G="" F  S X=$O(^AUPNPROB("AC",P,X)) Q:X'=+X!(G]"")  D
 .Q:$P(^AUPNPROB(X,0),U,12)="D"
 .I $P(^AUPNPROB(X,0),U,13)="" Q:$P(^AUPNPROB(X,0),U,8)>$S(RPE:RPE,1:EDATE)
 .I $P(^AUPNPROB(X,0),U,13)]"",$P(^AUPNPROB(X,0),U,13)>EDATE Q
 .S Y=$P(^AUPNPROB(X,0),U)
 .I $$ICD^BGP8UTL2(Y,T,9) S G=$P(^AUPNPROB(X,0),U,13) S:G="" G=$P(^AUPNPROB(X,0),U,8) Q
 .S S=$$VAL^XBDIQ1(9000011,X,80001)
 .I S]"",$D(^XTMP("BGPSNOMEDSUBSET",$J,"PXRM HIV",S)) S G=$P(^AUPNPROB(X,0),U,13) S:G="" G=$P(^AUPNPROB(X,0),U,8) Q
 .Q
 Q G
HIVTEST(P,BDATE,EDATE) ;EP
 ...
 ....I '$D(BGPA((9999999-D))) S BGPC=BGPC+1,BGPC(BGPC)=1_U_$$DATE^BGP8UTL((9999999-D))_" Lab"_U_$P(^AUPNVLAB(X,0),U,4)_U_D,BGPA((9999999-D))="" Q
 ...Q:'T
 ...S J=$P($G(^AUPNVLAB(X,11)),U,13) Q:J=""
 ...Q:'$$LOINC^BGP8D21(J,T)
 ...Q:$D(BGPA((9999999-D)))
 ...S BGPC=BGPC+1,BGPC(BGPC)=1_U_$$DATE^BGP8UTL((9999999-D))_"  Lab "_$P($G(^LAB(95.3,J,0)),U)_"-"_$P($G(^LAB(95.3,J,0)),U,15)_U_$P(^AUPNVLAB(X,0),U,4)_U_D,BGPA((9999999-D))=""
 ...Q
 ..Q
 .;
 .S L=0 F  S L=$O(^AUPNVLAB("AE",P,D,L)) Q:L'=+L  D
 ...
 ....I $$ICD^BGP8UTL2($P(^AUPNVCPT(X,0),U),T,1) I '$D(BGPA((9999999-$P(ED,".")))) S BGPC=BGPC+1,BGPC(BGPC)=1_U_$$DATE^BGP8UTL((9999999-$P(ED,".")))_" CPT "_$$VAL^XBDIQ1(9000010.18,X,.01)_U_U_$P(ED,"."),BGPA((9999999-$P(ED,".")))=""
 ....Q
 ...Q
 ..Q
...
FIRSTDX(P,BDATE,EDATE) ;EP - any HIV dx
 ...
 .S G=1_U_"First DX: "_$$DATE^BGP8UTL($P(BGPG(1),U,1))_U_$P(BGPG(1),U,1)
 S T=$O(^ATXAX("B","BGP HIV/AIDS DXS",0))
 S X=0,G="" F  S X=$O(^AUPNPROB("AC",P,X)) Q:X'=+X!(G]"")  D
 .Q:$P(^AUPNPROB(X,0),U,12)="D"
 .Q:$P(^AUPNPROB(X,0),U,12)="I"
 ...
 .S Y=$P(^AUPNPROB(X,0),U)
 .I $$ICD^BGP8UTL2(Y,T,9) S G=$P(^AUPNPROB(X,0),U,13) S:G="" G=$P(^AUPNPROB(X,0),U,8) S G=G_U_$$VAL^XBDIQ1(9000011,X,.01) Q
 .S S=$$VAL^XBDIQ1(9000011,X,80001)
 .I S]"",$D(^XTMP("BGPSNOMEDSUBSET",$J,"PXRM HIV",S)) S G=$P(^AUPNPROB(X,0),U,13) S:G="" G=$P(^AUPNPROB(X,0),U,8) S G=G_U_$$VAL^XBDIQ1(9000011,X,.01) Q
 Q G
