BGPLPC13 ; IHS/CMI/LAB - measure I2 ; 31 Aug 2018  9:19 AM
 ;;19.1;IHS CLINICAL REPORTING;**1**;MAY 30, 2019;Build 7
 ;
DRM ;EP - CALLED FROM IPC REPORT
 S (BGPN1,BGPD1,BGPD2,BGPD3)=0
 ...
 S BGPXBD=$$FMADD^XLFDT(BGPBDATE,-(428))
 S BGPXED=$$FMADD^XLFDT(BGPBDATE,-61)
...
N1 I 'BGPISD S BGPSTOP=1 G DRME
 ...
 I A<12 S BGPSTOP=1 G DRME
 ;
 ;now what about exclusions?
 ;might as well process exclusions first;
 ;palliative care/long term care
 I $$PALLCARE(DFN,$$DOB^AUPNPAT(DFN),$$FMADD^XLFDT(BGPISD,425)) S BGPSTOP=1 G DRME
 ;
 ;long term care services (combined with palliative care)
 ;I $$CARESERV(DFN,$$DOB^AUPNPAT(DFN),$$FMADD^XLFDT(BGPISD,425)) S BGPSTOP=1 G DRME
 ;
 ;has bipolar or personality disorder before end of report period
 I $$HASBPPD(DFN,$$DOB^AUPNPAT(DFN),$$FMADD^XLFDT(BGPISD,425)) S BGPSTOP=1 G DRME
 ;
 ;dod prior to
 S D=$$DOD^AUPNPAT(DFN)
 I D,D<$$FMADD^XLFDT(BGPISD,425) S BGPSTOP=1 G DRME
 ;
 ...
 I A<18 S BGPD2=1
 I A>17 S BGPD3=1
 S G=$$PHQ9N(DFN,$$FMADD^XLFDT($P(BGPISD,U,1),306),$$FMADD^XLFDT($P(BGPISD,U,1),425))
 I G S BGPN1=1 G DRM1
 S G=$$PHQ9NBH(DFN,$$FMADD^XLFDT($P(BGPISD,U,1),306),$$FMADD^XLFDT($P(BGPISD,U,1),425))
 I G S BGPN1=1
DRM1 ;
 S BGPVALUE=""
 S BGPVALUE="ENC "_$$DATE^BGPLUTL($P(BGPISD,U,1))_"|||"  ; PHQ9: "_$$VAL^XBDIQ1(9000010.01,$P(BGPISD,U,2),.04)_"|||"  ;hit denominator
 I BGPN1 S BGPVALUE=BGPVALUE_"*** "_$$DATE^BGPLUTL($P(G,U,2))_" "_$P(G,U,3)_": "_$P(G,U,4)
DRME ;
...
V ;
 ...
 ;S Y=$$DEPV2(V) I Y S BGPISD=Q Q  ;PATCH 1 V19.1
 Q
BH ;
 ;is there a PHQ9 > 9?  if not, don't bother???
 S Q=$$PHQ9BH(V)
 ...
 .I Y,$$ICD^ATXAPI(Y,$O(^ATXAX("B","BGP IPC DEP REM OFFICE CPTS",0)),1) S BGPO=1 Q
...
BHN ;
 ...
 ..I C="" Q
 ..S C=+$$CODEN^BGPLUTL2(C,80)
 ..I 'C Q
 ..I $$ICD^BGPLUTL2(C,$O(^ATXAX("B","BGP IPC MAJOR DEPRESSION DXS",0)),1) S G=1,BGPISD=Q Q
 ..I $$ICD^BGPLUTL2(C,$O(^ATXAX("B","BGP IPC DYSTHYMIA DXS",0)),1) S G=1,BGPISD=Q Q
 .I G Q
 .I $$PLTAXND^BGPLDU(DFN,"BGP IPC MAJOR DEPRESSION DXS",BGPDD,1) S G=1,BGPISD=Q Q
 .I $$IPLSNOND^BGPLDU(DFN,"PXRM BGP IPC MAJOR DEP",BGPDD,1) S G=1,BGPISD=Q Q
 .I $$PLTAXND^BGPLDU(DFN,"BGP IPC DYSTHYMIA DXS",BGPDD,1) S G=1,BGPISD=Q Q
 .I $$IPLSNOND^BGPLDU(DFN,"PXRM BGP IPC DYSTHYMIA",BGPDD,1) S G=1,BGPISD=Q Q
 I BGPP S G=0 D  Q
 ...
 .S C=+$$CODEN^BGPLUTL2(C,80)
 .I 'C Q
 .I C,$$ICD^BGPLUTL2(C,$O(^ATXAX("B","BGP IPC MAJOR DEPRESSION DXS",0)),1) S G=1,BGPISD=Q Q
 .I C,$$ICD^BGPLUTL2(C,$O(^ATXAX("B","BGP IPC DYSTHYMIA DXS",0)),1) S G=1,BGPISD=Q
 Q
PHQ9(VST) ;
 NEW X,Y,Z
...
PHQ9NBH(P,BDATE,EDATE) ;
 ...
 Q Y
PALLCARE(P,BDATE,EDATE) ;EP - palliative care, long term care
 NEW X,Y,Z,BGPV,G,A,B,I
 ;problem list first
 S (X,Y,I,G)=0
 F  S X=$O(^AUPNPROB("AC",P,X)) Q:X'=+X!(G)  D
 .Q:'$D(^AUPNPROB(X,0))
 .Q:$P(^AUPNPROB(X,0),U,12)="D"
 .Q:$P(^AUPNPROB(X,0),U,12)="I"
 .S Y=$P(^AUPNPROB(X,0),U)
 .I EDATE,$P(^AUPNPROB(X,0),U,13)>EDATE Q  ;if there is a doo and it is after report period skip
 .I $P(^AUPNPROB(X,0),U,13)="",EDATE,$P(^AUPNPROB(X,0),U,8)>EDATE Q  ;entered after report period, skip
 .I $$ICD^BGPLUTL2(Y,$O(^ATXAX("B","BGP IPC PALLIATIVE CARE DXS",0)),9) S G=1 Q
 .S S=$$VAL^XBDIQ1(9000011,X,80001)
 .I S]"",$D(^XTMP("BGPSNOMEDSUBSET",$J,"PXRM BGP IPC PALLIATIVE",S)) S G=1 Q
 .I S]"",$D(^XTMP("BGPSNOMEDSUBSET",$J,"PXRM BGP IPC LONG TERM CARE",S)) S G=1 Q
 I G Q G
 ;HOW ABOUT V POV DX
 I $$LASTDX^BGPLUTL1(P,"BGP IPC PALLIATIVE CARE DXS",BDATE,EDATE) Q 1
 ...
 ..I $D(^XTMP("BGPSNOMEDSUBSET",$J,"PXRM BGP IPC LONG TERM CARE",A)) S G=1 Q
 .I G Q
 .S A=0
 ...
 ..Q:S=""
 ..I $D(^XTMP("BGPSNOMEDSUBSET",$J,"PXRM BGP IPC PALLIATIVE",S)) S G=1 Q
 ..I $D(^XTMP("BGPSNOMEDSUBSET",$J,"PXRM BGP IPC LONG TERM CARE",S)) S G=1 Q
 .I G Q
 .;is .17 a cpt we want?
 .S Y=$$VALI^XBDIQ1(9000010,V,.17)
 .I Y,$$ICD^ATXAPI(Y,$O(^ATXAX("B","BGP IPC LONG TERM CARE CPTS",0)),1) S G=1 Q
 .;now check all V CPTs
...
DEPV1(V) ;EP
 ...
 S G=0
 S A=0,B="",BGPISD=""
 ;V19.1 PATCH 1 GET RID OF ALL SNOMED CODES
 ...
 ;is .17 a cpt we want?
 S Y=$$VALI^XBDIQ1(9000010,V,.17)
 I Y,$$ICD^ATXAPI(Y,$O(^ATXAX("B","BGP IPC DEP REM OFFICE CPTS",0)),1) G DEPV1N
 ;now check all V CPTs
 S Z=0 F  S Z=$O(^AUPNVCPT("AD",V,Z)) Q:Z'=+Z!(B)  D
 .S Y=$P($G(^AUPNVCPT(Z,0)),U,1)
 .I Y,$$ICD^ATXAPI(Y,$O(^ATXAX("B","BGP IPC DEP REM OFFICE CPTS",0)),1) S B=1 Q
 I B G DEPV1N
 Q ""
DEPV1N ;
 ;now check for active problem list or primary POV 
...
DEPV2N ;
 ...
 .I $$ICD^BGPLUTL2(C,$O(^ATXAX("B","BGP IPC DYSTHYMIA DXS",0)),1) S G=1,BGPISD=Q Q
 .S Y="",I=""
 .I $P(^AUPNVSIT(V,0),U,7)="H" F  S Y=$O(^AUPNVPOV("AD",V,Y)) Q:Y'=+Y  I $P(^AUPNVPOV(Y,0),U,12)="P" S I=Y
 .I $P(^AUPNVSIT(V,0),U,7)'="H" S Y=$O(^AUPNVPOV("AD",V,0)) I Y S I=Y
 .Q:'Y
...
HASBPPD(P,BDATE,EDATE) ;
 NEW X,Y,Z,G
 I $$PLTAXND^BGPLDU(P,"BGP IPC BIPOLAR DISORDER DXS",EDATE,1) Q 1
 I $$IPLSNOND^BGPLDU(P,"PXRM BGP IPC BIPOLAR DX",EDATE,1) Q 1
 I $$LASTDX^BGPLUTL1(P,"BGP IPC BIPOLAR DISORDER DXS",BDATE,EDATE) Q 1
 I $$PLTAXND^BGPLDU(P,"BGP IPC PERSONALITY DIS DXS",EDATE,1) Q 1
 I $$IPLSNOND^BGPLDU(P,"PXRM BGP IPC PERSONAL DIS",EDATE,1) Q 1
 I $$LASTDX^BGPLUTL1(P,"BGP IPC PERSONALITY DIS DXS",BDATE,EDATE) Q 1
 I $$PLTAXND^BGPLDU(P,"BGP IPC SCHIZO PSYCHOTIC DXS",EDATE,1) Q 1
 I $$IPLSNOND^BGPLDU(P,"PXRM BGP IPC SCHIZO DIS",EDATE,1) Q 1
 I $$LASTDX^BGPLUTL1(P,"BGP IPC SCHIZO PSYCHOTIC DXS",BDATE,EDATE) Q 1
 I $$PLTAXND^BGPLDU(P,"BGP IPC PERVASIVE DEVELOP DXS",EDATE,1) Q 1
 I $$IPLSNOND^BGPLDU(P,"PXRM BGP IPC PERVASIVE DEV",EDATE,1) Q 1
 I $$LASTDX^BGPLUTL1(P,"BGP IPC PERVASIVE DEVELOP DXS",BDATE,EDATE) Q 1
 ...
 I G Q G
 ;
 S T="PXRM BGP IPC PERVASIVE DEV"
 S S=0 F  S S=$O(^XTMP("BGPSNOMEDSUBSET",$J,T,S)) Q:S=""!(G)  D
 .Q:'$D(^AUPNVPOV("ASNC",P,S))
 .S D=0 F  S D=$O(^AUPNVPOV("ASNC",P,S,D)) Q:D=""!(G)  D
 ..S Y=9999999-D
...
BHDX(P,BDATE,EDATE) ;has a bh dx in either taxonomy BDATE to EDATE?
 ...
 .I I="" Q
 .S Z=+$$CODEN^BGPLUTL2(I,80)
 .I 'Z Q
 .I $$ICD^BGPLUTL2(Z,$O(^ATXAX("B","BGP IPC BIPOLAR DISORDER DXS",0)),9) S G=1 Q
 .I $$ICD^BGPLUTL2(Z,$O(^ATXAX("B","BGP IPC PERSONALITY DIS DXS",0)),9) S G=1 Q
 .I $$ICD^BGPLUTL2(Z,$O(^ATXAX("B","BGP IPC SCHIZO PSYCHOTIC DXS",0)),9) S G=1 Q
 .I $$ICD^BGPLUTL2(Z,$O(^ATXAX("B","BGP IPC PERVASIVE DEVELOP DXS",0)),9) S G=1 Q
 .Q
 Q G
