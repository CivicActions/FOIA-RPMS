...
SP(RETVAL,BGPSTR) ;-- get the site parameter entry
 ...
 S RETVAL="^BGPTMP("_$J_")"
 S BGPI=0
 S ^BGPTMP($J,BGPI)="T00010BMXIEN^T00050Location^T00050Home Location^T00050Community Taxonomy^T00001EISS^T00001CHS^T00001HeightWeight^T00050Location Taxonomy^T00050Demo Template"_$C(30)
 S BGPDA=0 F  S BGPDA=$O(^BGPSITE("B",BGPSE,BGPDA)) Q:'BGPDA  D
 . N BGPDATA,BGPLI,BGPL,BGPLA,BGPHI,BGPH,BGPHA,BGPTX,BGPTXI,BGPEISS,BGPCHS,BGPMTAX,BGPMTAXI,BGPHTWT,BGPDEMOI,BGPDEMO
 ...
 . S ^BGPTMP($J,BGPI)=^BGPTMP($J,BGPI)_U_$S($G(BGPDEMOI):BGPDEMOI_"~"_$G(BGPDEMO),1:"")_$C(30)
 S ^BGPTMP($J,BGPI+1)=$C(31)_$G(BGPERR)
 Q
 ;
...
SITE(RETVAL,BGPSTR) ;-- save the site parameters
 S X="MERR^BGPGU",@^%ZOSF("TRAP") ; m error trap
 N BGPI,BGPJ,BGPDATA,BGPDA,BGPHTWT,P,BGPERR,BGPFAC,BGPTAX,BGPHLOC,R,BGPEISS,BGPCHS,BGPDEMO
 S P="|",R="~"
 S BGPI=0
 S BGPERR=""
