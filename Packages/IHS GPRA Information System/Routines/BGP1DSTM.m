BGP1DSTM ; IHS/CMI/LAB - national patient list 20 Dec 2004 9:24 AM 07 Mar 2010 2:31 PM ; 29 Mar 2011  10:05 AM
 ...
 ;
 W:$D(IOF) @IOF
 W !,$$CTR("IHS GPRA & PART Performance Patient Search Template Creation",80)
 W !,$$CTR($$RPTVER^BGP1BAN,80)
INTRO ;
 D XIT
 W !!,"This will produce a search template of patients who either met or did not meet"
 W !,"a National GPRA & PART Report performance measure.  You will be asked to select "
...
TP ;get time period
 D XIT
 S BGPRTYPE=1,BGP1RPTH="",BGPNPL=1,BGPINDB="G",BGP1GPU=1,BGP1NPLT=1
SI ;
 K DIRUT
 K BGPIND
 D EN^BGP1DSI
...
SI1 ;NOW SELECT ONE OR MORE W/IN THE TOPIC
 ...
 .K BGPX S BGPO=0,X=0,BGPC=0 F  S BGPO=$O(^BGPNPLB("AN",BGPIND,BGPO)) Q:BGPO'=+BGPO!($D(DIRUT))  D
 ...
 I $D(BGPQUIT) G TP
 I '$D(BGPINDL) G TP
 D RT^BGP1DSL I '$D(BGPLIST)!($D(BGPQUIT)) G SI1
...
SUM ;display summary of this report
 W:$D(IOF) @IOF
 W !,$$CTR("SUMMARY OF NATIONAL GPRA & PART SEARCH TEMPLATE TO BE GENERATED")
 W !,$$CTR($$RPTVER^BGP1BAN,80)
 W !!,"The date ranges for this report are:"
 W !?5,"Report Period: ",?31,$$FMTE^XLFDT(BGPBD)," to ",?31,$$FMTE^XLFDT(BGPED)
 W !?5,"Previous Year Period: ",?31,$$FMTE^XLFDT(BGPPBD)," to ",?31,$$FMTE^XLFDT(BGPPED)
 ...
 .W !?2,$P(^BGPNPLB(Y,0),U,3),":  ",$P(^DIBT(BGPINDL(X,Y,"TEMP"),0),U)
 D PT^BGP1DSL
 I BGPROT="" G COMM
ZIS ;call to XBDBQUE
 D REPORT^BGP1UTL
...
TSKMN ;EP ENTRY POINT FROM TASKMAN
 ...
 S ZTCPU=$G(IOCPU),ZTRTN="DRIVER^BGP1NPL",ZTDTH="",ZTDESC="NATIONAL GPRA & PART REPORT 06" D ^%ZTLOAD D XIT Q
 Q
 ;
XIT ;
 D ^%ZISC
...
EN1 ;EP Help
 ...
 S X=0 F  S X=$O(^BGPNPLB(BGPII,11,X)) Q:X'=+X  W !?3,^BGPNPLB(BGPII,11,X,0)
...
CT ;EP - create search templates and write message
 ...
 I BGPPTYPE="D" D W^BGP1DP(" ",0,2,BGPPTYPE)  ;S X=" " D S^BGP1NPLD(X,1,1) S X=" " D S^BGP1NPLD(X,1,1)
 S BGPORD=$P($G(^BGPINDB(BGPIC,12)),U,6)
 S O=0 F  S O=$O(^XTMP("BGP1DNP",BGPJ,BGPH,"LIST",O)) Q:O'=+O  D
 .S X=0 F  S X=$O(^XTMP("BGP1DNP",BGPJ,BGPH,"LIST",O,X)) Q:X'=+X  D
 ..S Y=0 F  S Y=$O(^XTMP("BGP1DNP",BGPJ,BGPH,"LIST",O,X,Y)) Q:Y'=+Y  D
 ...S T=$G(BGPINDL(X,Y,"TEMP"))
 ...I T="" Q
 ...I '$D(^DIBT(T,0)) Q
 ...L +^DIBT(T):10 I '$T Q
 ...S C=0
 ...S P=0 F  S P=$O(^XTMP("BGP1DNP",BGPJ,BGPH,"LIST",O,X,Y,P)) Q:P'=+P  D
 ....S C=C+1
 ....S ^DIBT(T,1,P)=""
 ....Q
 ...L -^DIBT(T)
 ...D W^BGP1DP("Search template: "_$P(^DIBT(T,0),U)_" created with "_C_" members.",0,2,BGPPTYPE)
 D W^BGP1DP("",0,1,BGPPTYPE)
 Q
