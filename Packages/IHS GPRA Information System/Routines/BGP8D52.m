BGP8D52 ; IHS/CMI/LAB - measure 31 ;
...
WASS ;
 ...
 ;PREGNANT QUIT
 I $$PREG^BGP8D715(DFN,BGPBDATE,BGPEDATE,1,1,"",BGPBDATE,BGPEDATE) S BGPSTOP=1 Q
 S BGPD1=1
 ...
 I BGPBMI="" S BGPBMI=$$LASTDX^BGP8UTL1(DFN,"BGP BMI DXS",BGPBDATE,BGPEDATE) I BGPBMI S BGPBMI=$P(BGPBMI,U,2)
 ...
 I BGPN3 D
 .I N]"" S N=N_"; "
 .S N=N_"NUTR: "_$$DATE^BGP8UTL($P(BGPNUTR,U))_" "_$P(BGPNUTR,U,2)
 I BGPN4 D
 .I N]"" S N=N_"; "
 .S N=N_"PHY: "_$$DATE^BGP8UTL($P(BGPPHY,U))_" "_$P(BGPPHY,U,2)
...
NUTR(P,BDATE,EDATE) ;EP
 ...
 S BGPG=$$CPT^BGP8DU(P,BDATE,EDATE,$O(^ATXAX("B","BGP CPT NUTRITION COUNSELING",0)),5)
 ...
 .I $P(S,U,1)'="-1",$$ICD^BGP8UTL2($P(S,U,1),$O(^ATXAX("B","BGP DIETARY SURVEILLANCE DXS",0)),9) S %=$P(BGPALLED(X),U)_U_"EDUC: "_T Q
 .S S=$P(T,"-",1),S=$$CPT^ICPTCOD(S)
 .I $P(S,U,1)'="-1",$$ICD^BGP8UTL2($P(S,U,1),$O(^ATXAX("B","BGP CPT NUTRITION COUNSELING",0)),1) S %=$P(BGPALLED(X),U)_U_"EDUC: "_T Q
 I %]"" Q %
 Q ""
PHY(P,BDATE,EDATE) ;EP
 ...
 I $D(BGPG(1)) Q $P(BGPG(1),U)_U_"DX "_$P(BGPG(1),U,2)
 S BGPG=$$CPT^BGP8DU(P,BDATE,EDATE,$O(^ATXAX("B","BGP CPT PHYSICAL ACTIVITY",0)),5)
 I BGPG Q $P(BGPG,U,1)_U_"CPT "_$P(BGPG,U,2)  ;BGPG
 S Y="BGPALLED("
 S X=P_"^ALL EDUC;DURING "_$$FMTE^XLFDT(BDATE)_"-"_$$FMTE^XLFDT(EDATE) S E=$$START1^APCLDF(X,Y)
 S (X,D)=0,%="",T="" F  S X=$O(BGPALLED(X)) Q:X'=+X!(%]"")  D
 ...
 .S S=$P(T,"-",1),S=$$CPT^ICPTCOD(S)
 .I $P(S,U,1)'="-1",$$ICD^BGP8UTL2($P(S,U,1),$O(^ATXAX("B","BGP CPT PHYSICAL ACTIVITY",0)),1) S %=$P(BGPALLED(X),U)_U_"EDUC: "_T Q
 I %]"" Q %
 Q ""
