BGP8PDLE ; IHS/CMI/LAB - print ind 03 Jul 2010 12:41 PM 07 Dec 2010 12:18 PM 09 Sep 2014 12:17 PM ; 24 Jan 2018  7:42 AM
 ;;18.0;IHS CLINICAL REPORTING;**1**;NOV 21, 2017;Build 62
 ;
 ;
I1AGE ;EP
 I BGPINDG="I" Q
 S BGPHD1="USER POP TOBACCO USERS/IN CESSATION",BGPHD2="USER POP Tobacco Users/In Cessation"
 ...
 D I1AGEP
 S BGPHD1="MALE USER POP TOBACCO USERS/IN CESSATION",BGPHD2="# Male UP Tobacco Users/in Cessation"
 D H3
 K BGPDAC,BGPDAP,BGPDAB S (C,D,E,F,G)=0 F BGPX="114.5.1","114.5.2","114.5.3" D I1AGE1,I1AGE2,I1AGE4
 D I1AGEP
FEM ;
 S BGPHD1="FEMALE USER POP TOBACCO USERS/IN CESSATION",BGPHD2="# Female UP Tobacco Users/in Cessation"
 D H3
...
I1AGEP ;
 ...
 S X="# w/ Tobacco Cessation Counseling or Rx for Cessation Aid or Quit-No Refusals" D S(X,1,1)
 F X=1:1:3 S V=$P(BGPDAC(X),U,2) S Y=V D S(Y,,X+1)
 S X="% w/ Tobacco Cessation Counseling or Rx for Cessation Aid or Quit-No Refusals" D S(X,1,1)
 F X=1:1:3 S V=$P(BGPDAC(X),U,3) S Y=$$SB($J(V,6,1)) D S(Y,,X+1)
 S X=" " D S(X,1,1) S X="# w/ Tobacco Cessation Counseling or Rx for Cessation Aid-No Refusals" D S(X,1,1)
 ...
 S X=" " D S(X,1,1) S X="# Quit" D S(X,1,1)
...
V(T,R,N,P) ;EP
 I $G(BGPAREAA) G VA
 NEW X
 I T=1 S X=$P($G(^BGPGPDCR(R,N)),U,P) Q $S(X]"":X,1:0)
 I T=2 S X=$P($G(^BGPGPDPR(R,N)),U,P) Q $S(X]"":X,1:0)
 I T=3 S X=$P($G(^BGPGPDBR(R,N)),U,P) Q $S(X]"":X,1:0)
 Q ""
VA ;
