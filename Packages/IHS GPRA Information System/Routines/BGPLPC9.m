BGPLPC9 ; IHS/CMI/LAB - measure I2 ; 02 Feb 2019  11:25 AM
...
BCS ;EP
 ...
 I BGPAGEB<51 S BGPSTOP=1 Q  ;18 or greater during time period
...
ENC9(P,BDATE,EDATE) ;EP  - have encounter per CMS122v6
 ...
 .I Y,$$OFFCPT9(Y) S G=1_U_$$DATE^BGPLUTL(D)_" CPT: "_$P($$CPT^ICPTCOD(Y),U,2) Q
...
MAS(P,EDATE) ;EP mastectomy before end of time frame
 ...
 K BGP
 ;V POV
 S BGP(1)=$$LASTDX^BGPLUTL1(P,"BGP IPC BILAT MASTECTOMY DXS",$$DOB^AUPNPAT(P),EDATE)
 I $P(BGP(1),U,1) Q 1
 ;problem list
 ...
 .I $P(^AUPNPROB(X,0),U,13),$P(^AUPNPROB(X,0),U,13)>EDATE Q  ;if there is a doo and it is after report period skip
 .I $P(^AUPNPROB(X,0),U,13)="",$P(^AUPNPROB(X,0),U,8)>EDATE Q  ;entered after report period, skip
 .I $$ICD^BGPLUTL2(Y,T,9) S BGPRL("RT")=1 Q
 .I $$ICD^BGPLUTL2(Y,T1,9) S BGPRL("LT")=1 Q
 .S S=$$VAL^XBDIQ1(9000011,X,80001)
 .I S=429242008!(I=137681000119108) S BGPRL("RT")=1 Q
 .I S=429009003!(I=137671000119105) S BGPRL("LT")=1 Q
 ...
 .I $$ICD^BGPLUTL2(Y,$O(^ATXAX("B","BGP IPC UNILAT MASTECTOMY DXS",0)),9),$$UP^XLFSTR(C)["LEFT" S BGPRL("LT")=1 Q
 .I $$ICD^BGPLUTL2(Y,$O(^ATXAX("B","BGP IPC UNILAT MASTECTOMY DXS",0)),9),$$UP^XLFSTR(C)["RIGHT" S BGPRL("RT")=1 Q
 .Q
 I $D(BGPRL("RT")),$D(BGPRL("LT")) Q 1
 ;NOW V POV
 S X=$$LASTDX^BGPLUTL1(P,"BGP IPC RT MASTECTOMY DXS",$$DOB^AUPNPAT(P),EDATE) I X S BGPRL("RT")=1
 S X=$$LASTDX^BGPLUTL1(P,"BGP IPC LT MASTECTOMY DXS",$$DOB^AUPNPAT(P),EDATE) I X S BGPRL("LT")=1
 I $D(BGPRL("RT")),$D(BGPRL("LT")) Q 1
 S S=429242008
 S D=0 F  S D=$O(^AUPNVPOV("ASNC",P,S,D)) Q:D=""!(G)  D
 .S Y=9999999-D
...
MAM(P,BDATE,EDATE) ;
 ...
 .I Y,$$ICD^BGPLUTL2(Y,$O(^ATXAX("B","BGP IPC MAMMOGRAM CPTS",0)),1) D  Q
