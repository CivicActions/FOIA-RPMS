BGP1DPA2 ; IHS/CMI/LAB - COMP NATIONAL GPRA FOR PTS W/APPT 09 Jan 2010 3:36 PM 26 Jun 2010 3:40 PM 03 Jun 2011 8:04 AM ;
...
I017 ;EP - colorectal 
 ...
 I $$CRC^BGP1D61(DFN,BGPEDATE) S BGPSKIP=1 Q
 I 'BGPACTUP S BGPSKIP=1 Q
 S (BGPIN,BGPIN1,BGPIN2,BGPIN3)=""
 S BGPLAST=""
 S BGPFOB=$$FOB^BGP1D61(DFN,$$DOB^AUPNPAT(DFN),BGPEDATE)  ;last FOB
 S D=$P(BGPFOB,U,2)
 I D]"",$$FMDIFF^XLFDT(DT,D)<365 S BGPIN=1
 I D>BGPLAST S BGPLAST=D
 S BGPSIG=$$SIG^BGP1D61(DFN,BGPEDATE,$$DOB^AUPNPAT(DFN))  ;last SIG
 ...
 I D>BGPLAST S BGPLAST=D
 S BGPCOLO=$$COLO^BGP1D61(DFN,BGPEDATE,$$DOB^AUPNPAT(DFN))  ;last COLO
 ...
 I D>BGPLAST S BGPLAST=D
 S BGPBE=$$BE^BGP1D61(DFN,BGPEDATE,$$DOB^AUPNPAT(DFN))  ;last BE
 ...
 ;S BGPREF=$$REF^BGP1D61(DFN,BGPBDATE,BGPEDATE) I BGPREF S BGPSKIP=1 Q  ;ref in past yr
 S BGPN1=0
 ...
 ;
 I BGPFOB]"" S BGPX=BGPX_$S(BGPX]"":"|",1:"") S BGPX=BGPX_"Last FOBT: "_$$DATE^BGP1UTL($P(BGPFOB,U,2))
 I BGPBE]"" S BGPX=BGPX_$S(BGPX]"":"|",1:"")_"Last DCBE: "_$P(BGPBE,U,2)
 I BGPX="" S BGPX="No Colonoscopy, Flex Sig, FOBT or DCBE documented."
 S BGPX=BGPX_"|May be overdue as of: "_$S(BGPLAST]"":$$DATE^BGP1UTL($$FMADD^XLFDT(BGPLAST,365)),1:$$DATE^BGP1UTL(BGPBD))
...
I019 ;EP - tobacco cess
 ...
 S BGPTOBP=$$TOBACCO^BGP1D7(DFN,$$DOB^AUPNPAT(DFN),$$FMADD^XLFDT(BGPBDATE,-1))
 ...
 S BGPN="",BGPT=""
 S BGPFOB=$$PED^BGP1D711(DFN,$$DOB^AUPNPAT(DFN),BGPEDATE,1)
 I BGPFOB="" S BGPISSV=$$TITLE(BGPGPRAI)_U_"Last Intervention: Never|Overdue as of: "_$$DATE^BGP1UTL(BGPBD) D I019X Q
 S D=$P(BGPFOB,U,1),BGPNFOB=$S($P(BGPFOB,U,1):$$FMADD^XLFDT(D,365),1:DT) I BGPNFOB'>BGPED S BGPIN1=1
 I 'BGPIN1 S BGPSKIP=1 Q
 S BGPISSV=$$TITLE(BGPGPRAI)_U_"Last Intervention: "_$$DATE^BGP1UTL($P(BGPFOB,U,1))_"|Overdue as of: "_$$DATE^BGP1UTL(BGPNFOB)
I019X ;
 ...
 Q
I020 ;EP - Alcohol Screen
 I 'BGPACTUP S BGPSKIP=1 Q
 I BGPSEX'="F" S BGPSKIP=1 Q
 I BGPAGEB<15 S BGPSKIP=1 Q
 I BGPAGEB>44 S BGPSKIP=1 Q
 S (BGPD,BGPN,BGPN1,BGPREF,BGPNS,BGPV)=""
 S BGPN1=$$ALHF^BGP1D5(DFN,$$DOB^AUPNPAT(DFN),BGPEDATE)
 S BGPD=$P(BGPN1,U,4),BGPV=$P(BGPN1,U,2)
 S BGPN1=$$ALDX^BGP1D5A(DFN,$$DOB^AUPNPAT(DFN),BGPEDATE)
 I BGPD<$P(BGPN1,U,4) S BGPD=$P(BGPN1,U,4),BGPV=$P(BGPN1,U,2)
 S BGPN1=$$ALPED^BGP1D5(DFN,$$DOB^AUPNPAT(DFN),BGPEDATE)
 I BGPD<$P(BGPN1,U,4) S BGPD=$P(BGPN1,U,4),BGPV=$P(BGPN1,U,2)
 S BGPNS=$S(BGPD:$$FMADD^XLFDT(BGPD,365),1:"")
 I BGPD="" S BGPISSV=$$TITLE(BGPGPRAI)_U_"Last Alcohol Screen: Never|Overdue as of: "_$$DATE^BGP1UTL(BGPBD) D I020X Q
 I BGPNS>BGPED S BGPSKIP=1 Q  ;not due
 S BGPISSV=$$TITLE(BGPGPRAI)
 I BGPV]"" S $P(BGPISSV,U,2)="Last Alcohol Screen: "_$$DATE^BGP1UTL(BGPD)
 ...
 S $P(BGPISSV,U,2)=$P(BGPISSV,U,2)_"|"_$S(BGPNS'>BGPED:"Alcohol Screening Overdue as of: ",1:"Alcohol Screening next due: ")_$$DATE^BGP1UTL(BGPNS)
I020X ;
...
IAA ;EP
 ...
 S BGPN1=$$DEP^BGP1D25(DFN,$$DOB^AUPNPAT(DFN),BGPEDATE)
 I BGPN1 NEW X S X=$P(BGPN1,U,4),%DT="" D ^%DT S BGPD=Y,BGPV="2 Mood Disorder DXs "
 S BGPN1=$$DEPSCR^BGP1D25(DFN,$$DOB^AUPNPAT(DFN),BGPEDATE)
 I BGPN1,BGPD<$P(BGPN1,U,4) S BGPD=$P(BGPN1,U,4),BGPV=$P(BGPN1,U,2)
 S BGPN1=$$DEPEDU^BGP1D25(DFN,$$DOB^AUPNPAT(DFN),BGPEDATE)
 I BGPN1,BGPD<$P(BGPN1,U,4) S BGPD=$P(BGPN1,U,4),BGPV=$P(BGPN1,U,2)
 I BGPD="" S BGPISSV=$$TITLE(BGPGPRAI)_U_"Last Depression Screen: Never|Overdue as of: "_$$DATE^BGP1UTL(BGPBD) D IAAX Q
 S BGPNS=$S(BGPD:$$FMADD^XLFDT(BGPD,365),1:"")
 I BGPNS>BGPED S BGPSKIP=1 D IAAX Q
 S BGPISSV=$$TITLE(BGPGPRAI)_U_"Last Depression Screen: "_$$DATE^BGP1UTL(BGPD)_"|Overdue as of: "_$$DATE^BGP1UTL(BGPNS)
 ;
IAAX ;
 K BGPD,BGPN,BGPN1,BGPREF,BGPNS
 Q
I014 ;EP
 ...
 S BGPISSV=$$TITLE(BGPGPRAI)_U_"Last Dental Sealant: "_$$DATE^BGP1UTL(BGPLDS)_"|"_BGPS
 Q
SEAL(P,BDATE,EDATE) ;
...
I016 ;EP
 ...
 S BGPISSV=$$TITLE(BGPGPRAI)_U_"Last Topical Fluoride: "_$$DATE^BGP1UTL(BGPLDS)_"|"_BGPS
 Q
TF(P,BDATE,EDATE) ;
 ...
 ..S A=$P($G(^AUPNVCPT(Y,0)),U)
 ..Q:'A
 ..I $$ICD^ATXCHK(A,$O(^ATXAX("B","BGP CPT TOPICAL FLUORIDE",0)),1) S T=$P($G(^AUPNVCPT(Y,0)),U,16) S:T=""!(T=0) T=1 I $$VD^APCLV(V)>BGPC S BGPC=$$VD^APCLV(V)
 .Q:G  ;one per visit
...
I021 ;EP
 ...
 S BGPN1=$$DVEX^BGP1D5(DFN,$$DOB^AUPNPAT(DFN),BGPEDATE)
 S BGPD=$P(BGPN1,U,2),BGPVA=$P(BGPN1,U,3)
 S BGPN1=$$DVDX^BGP1D5(DFN,$$DOB^AUPNPAT(DFN),BGPEDATE)
 I BGPD<$P(BGPN1,U,2) S BGPD=$P(BGPN1,U,2),BGPVA=$P(BGPN1,U,3)
 S BGPN1=$$DVPED^BGP1D5(DFN,$$DOB^AUPNPAT(DFN),BGPEDATE)
 I BGPD<$P(BGPN1,U,2) S BGPD=$P(BGPN1,U,2),BGPVA=$P(BGPN1,U,3)
 S BGPNS=$S(BGPD:$$FMADD^XLFDT(BGPD,365),1:"")
 I BGPNS="" S BGPISSV=$$TITLE(BGPGPRAI)_U_"Last DV Screen: Never|Overdue as of: "_$$DATE^BGP1UTL(BGPBD) D I021X Q
 I BGPNS>BGPED S BGPSKIP=1 D I021X Q
 S BGPISSV=$$TITLE(BGPGPRAI)_U_"Last DV Screen: "_$$DATE^BGP1UTL(BGPD)_"|Overdue as of: "_$$DATE^BGP1UTL(BGPNS)
I021X ;
 K BGPD,BGPN,BGPN1,BGPREF,BGPNS
 Q
IE2 ;EP
 ...
 I 'BGPN1 S $P(BGPISSV,U,2)="Overdue as of: "_$$DATE^BGP1UTL(BGPBD)
...
I0302A ;EP
 ...
 K ^TMP($J)
 I '$$FIRSTIHD^BGP1D721(DFN,BGPEDATE) S BGPSKIP=1 Q  ;first dx not prior to report period
 I '$$V2IHD^BGP1D721(DFN,$$DOB^AUPNPAT(DFN),BGPEDATE) S BGPSKIP=1 Q  ;at least 2 IHD dxs ever
 D I0302ASC^BGP1D41
 S BGPISSV=""
...
I031A ;EP
 ...
 S BGPBMI=$$BMIOR^BGP1D6(DFN,$$DOB^AUPNPAT(DFN),BGPEDATE,BGPAGEE)
 I $P(BGPBMI,U)="" S BGPISSV=$$TITLE(BGPGPRAI)_U_"Last BMI: Never|Overdue as of: "_$$DATE^BGP1UTL(BGPBD) D I031AX Q
 I $$FMADD^XLFDT($P(BGPBMI,U,2),365)<BGPED S BGPIN1=1
 S BGPOW=$$OW^BGP1D6(DFN,$P(BGPBMI,U),$$AGE^AUPNPAT(DFN,$P(BGPBMI,U,2))) I BGPOW S BGPN1=1
 S BGPOB=$$OB^BGP1D6(DFN,$P(BGPBMI,U),$$AGE^AUPNPAT(DFN,$P(BGPBMI,U,2))) I BGPOB S BGPN2=1
 S A=$$AGE^AUPNPAT(DFN,$P(BGPBMI,U,2))
 I 'BGPIN1,'BGPOB S BGPSKIP=1 D I031AX Q
 S BGPISSV=$$TITLE(BGPGPRAI)_U_"Last BMI: "_$P(BGPBMI,U)_":"_$$DATE^BGP1UTL($P(BGPBMI,U,2))_" Age at BMI: "_A_$S(BGPOW:" At Risk 85-94%",BGPOB:" OW 95%",1:"")
 I BGPIN1 S BGPISSV=BGPISSV_"|Overdue as of: "_$$DATE^BGP1UTL($$FMADD^XLFDT($P(BGPBMI,U,2),365))
I031AX ; 
...
BFR ;EP
 ...
 I '$D(BGPBFR) S BGPISSV=$$TITLE(BGPGPRAI)_U_"Last Infant Feeding Choice Screening: Never" D BFRX Q   ;|Overdue as of: "_$$DATE^BGP1UTL(DT) D BFRX Q
 ;GET LASTEST (highest days old)
 S X=0,Y="" F  S X=$O(BGPBFR(X)) Q:X'=+X  S Y=X
 S BGPISSV=$$TITLE(BGPGPRAI)_U_"Last Infant Feeding Choice Screening: "_$$DATE^BGP1UTL($P(BGPBFR(Y),U,2))  ;_"|Overdue as of: "_$$DATE^BGP1UTL(DT)
 ;
BFRX ;
 K BGPADAY,BGPBFR
 Q
