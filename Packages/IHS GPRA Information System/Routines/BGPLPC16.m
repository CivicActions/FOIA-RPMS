BGPLPC16 ; IHS/CMI/LAB - measure I2 ; 02 Feb 2019  11:25 AM
...
CBP ;EP
 ...
 I BGPAGEB>84 S BGPSTOP=1 Q  ;85 or less during time period
 ;
 ;hypertension?
 I '$$HTN(DFN,$$DOB^AUPNPAT(DFN),$$FMADD^XLFDT(BGPBDATE,180)) S BGPSTOP=1 G CBPX  ;no hypertension
 ;now what about exclusions?
 I $$HOSPIND^BGPLPC2(DFN,BGPBDATE,BGPEDATE) S BGPSTOP=1 G CBPX  ;no hospice pts
 ;
 I $$PREG(DFN,BGPBDATE,BGPEDATE) S BGPSTOP=1 G CBPX
 I $$ESRD(DFN,$$DOB^AUPNPAT(DFN),BGPEDATE) S BGPSTOP=1 G CBPX
 I $$CKD(DFN,$$DOB^AUPNPAT(DFN),BGPEDATE) S BGPSTOP=1 G CBPX
 I $$KIDTRAN(DFN,$$DOB^AUPNPAT(DFN),BGPEDATE) S BGPSTOP=1 G CBPX
 I $$CPT^BGPLDU(DFN,$$DOB^AUPNPAT(DFN),BGPEDATE,$O(^ATXAX("B","BGP IPC VASCULAR ACC DIAL CPTS",0)),5) S BGPSTOP=1 G CBPX
 I $$CPT^BGPLDU(DFN,$$DOB^AUPNPAT(DFN),BGPEDATE,$O(^ATXAX("B","BGP IPC ESRD OPT SRV CPTS",0)),5) S BGPSTOP=1 G CBPX
 I $$CPT^BGPLDU(DFN,$$DOB^AUPNPAT(DFN),BGPEDATE,$O(^ATXAX("B","BGP IPC KIDNEY TRANSPLANT CPTS",0)),5) S BGPSTOP=1 G CBPX
 I $$CPT^BGPLDU(DFN,$$DOB^AUPNPAT(DFN),BGPEDATE,$O(^ATXAX("B","BGP IPC DIALYSIS SRV CPTS",0)),5) S BGPSTOP=1 G CBPX
 ;ENCOUNTER
...
CBPV ;
 ...
 I BGPVAL]"" S BGPVALUE=BGPVALUE_$$DATE^BGPLUTL($P(BGPVAL,U,1))_" "_$P(BGPVAL,U,2)_"/"_$P(BGPVAL,U,3)
CBPX ;
...
ENC16(P,BDATE,EDATE) ;EP  - have encounter per CMS122v6
 ...
 .I Y,$$OFFCPT16(Y) S G=1_U_$$DATE^BGPLUTL(D)_" CPT: "_$P($$CPT^ICPTCOD(Y),U,2) Q
...
OFFCPT16(C) ;EP
 ...
 Q ""
HTN(P,BDATE,EDATE) ;
 I $$PLTAXND^BGPLDU(P,"BGP IPC ESSENTIAL HTN DXS",EDATE,1) Q 1
 I $$IPLSNOND^BGPLDU(P,"PXRM BGP IPC HTN",EDATE,1) Q 1
 I $$LASTDX^BGPLUTL1(P,"BGP IPC ESSENTIAL HTN DXS",BDATE,EDATE) Q 1
 Q ""
PREG(P,BDATE,EDATE) ;
 NEW X,Y,Z,G,A
 I $P(^DPT(P,0),U,2)'="F" Q ""
 ;check dx
 S X=$$LASTDX^BGPLUTL1(P,"BGP IPC PREGNANCY DXS",BDATE,EDATE) I X Q 1
...
PREGN .I $$ICD^ATXAPI($P(^AUPNPROB(X,0),U,1),T,9) S G=1 Q
 ...
 Q G
ESRD(P,BDATE,EDATE) ;
 I $$PLTAXND^BGPLDU(P,"BGP IPC ESRD DXS",EDATE,1) Q 1
 I $$LASTDX^BGPLUTL1(P,"BGP IPC ESRD DXS",BDATE,EDATE) Q 1
 NEW X,S,I
 S I=""
 ...
 Q ""
CKD(P,BDATE,EDATE) ;
 I $$PLTAXND^BGPLDU(P,"BGP IPC CKD STG 5 DXS",EDATE,1) Q 1
 I $$LASTDX^BGPLUTL1(P,"BGP IPC CKD STG 5 DXS",BDATE,EDATE) Q 1
 NEW X,S,I
 S I=""
 S X=0 F  S X=$O(^AUPNPROB("AC",P,X)) Q:X'=+X!(I)  D
 .Q:'$D(^AUPNPROB(X,0))
 ...
 I G Q 1
 Q ""
KIDTRAN(P,BDATE,EDATE) ;
 I $$PLTAXND^BGPLDU(P,"BGP IPC KIDNEY TRANSPLANT DXS",EDATE,1) Q 1
 I $$LASTDX^BGPLUTL1(P,"BGP IPC KIDNEY TRANSPLANT DXS",BDATE,EDATE) Q 1
 NEW X,S,I
 S I=""
 S X=0 F  S X=$O(^AUPNPROB("AC",P,X)) Q:X'=+X!(I)  D
 .Q:'$D(^AUPNPROB(X,0))
...
LASTBP(P,BDATE,EDATE) ;
 ...
 .Q:'$P(^AUPNVSIT(V,0),U,9)  ;no dependent entries
 .Q:$P(^AUPNVSIT(V,0),U,11)  ;deleted
 .Q:'$$BP(V)  ;no BPs  so quit v19
 .S D=$$VD^APCLV(V)
 .S Y=$$FTOF^BGPLPC2(V) I Y]"" S Z(D,V)="" Q
 .;is .17 a cpt we want?
...
BP(V) ;
 ...
 NEW X,Y,G
 S G=""
 S X=0 F  S X=$O(^AUPNVMSR("AD",V,X)) Q:X'=+X!(G)  D
 .Q:$$VAL^XBDIQ1(9000010.01,X,.01)'="BP"
 .Q:$P($G(^AUPNVMSR(X,2)),U,1)  ;entered in error
 .S G=1
 Q G
