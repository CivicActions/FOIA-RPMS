BGPLPC6 ; IHS/CMI/LAB - measure I2 ; 02 Feb 2019  11:25 AM
...
CIZ ;EP
 ...
 ;
 S BGPDTAP=$$DTAP(DFN)
 S BGPIPV=$$IPV^BGPLPC61(DFN)
 S BGPMMR=$$MMR^BGPLPC62(DFN)
 S BGPHIB=$$HIB^BGPLPC63(DFN)
 S BGPHEPB=$$HEPB^BGPLPC64(DFN)
 S BGPVAR=$$VZV^BGPLPC65(DFN)
 S BGPPNEU=$$PNEUMO^BGPLPC66(DFN)
 S BGPHEPA=$$HEPA^BGPLPC67(DFN)
 S BGPROTA=$$ROTA^BGPLPC68(DFN)
 S BGPFLU=$$FLU^BGPLPC69(DFN)
...
DTAP(P) ;
 ...
 .Q:'$$ICD^BGPLUTL2(Y,TCPT,1)  ;not a dtap cpt
 ...
 .S X=$$ANCONT^BGPLD31(P,BGPZ,A730)
...
ENCEPH(P,EDATE) ;
 ...
 NEW X,Y,Z,G,T,S,D
 I $$PLTAXND^BGPLDU(P,"BGP IPC IZ ENCEPHALOPATHY DXS",EDATE,0) Q 1
 I $$IPLSNOND^BGPLDU(P,"PXRM BGP IPC IZ ENCEPHAL",EDATE,0) Q 1
 I $$LASTDX^BGPLUTL1(P,"BGP IPC IZ ENCEPHALOPATHY DXS",$$DOB^AUPNPAT(P),EDATE) Q 1
 ;NOW V POV SNOMED
...
ENC6(P,BDATE,EDATE) ;EP  - have encounter per CMS117v6
 ...
 .S D=$$VD^APCLV(V)
 .S Y=$$SNOMEDV(V) I Y]"" S G=1_U_$$DATE^BGPLUTL(D)_" SNOMED: "_Y Q   ;ITEM 18
 .;is .17 a cpt we want?
 .S Y=$$VALI^XBDIQ1(9000010,V,.17)
 .I Y,$$OFFCPT6(Y) S G=1_U_$$DATE^BGPLUTL(D)_" CPT: "_$P($$CPT^ICPTCOD(Y),U,2) Q
