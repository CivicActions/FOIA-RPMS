BGP8D81 ; IHS/CMI/LAB - measure C 17 Oct 2014 8:51 AM ;
...
V2ASTH(P,BDATE,EDATE) ;EP
 ...
 .I S]"",$D(^XTMP("BGPSNOMEDSUBSET",$J,SNPA,S)) S Z="PL Snomed "_S G ASSEVNS
 .I $$ICD^BGP8UTL2(Y,T,9),$P(^AUPNPROB(X,0),U,15)]"",$P(^AUPNPROB(X,0),U,15)>1 S Z=$P(^AUPNPROB(X,0),U,15) G ASSEVNS
 .I $$ICD^BGP8UTL2(Y,T,9) S G=$$SEVPER^BGP8D22(X) I G S Z=G G ASSEVNS
 .I S]"",$D(^XTMP("BGPSNOMEDSUBSET",$J,SNA,S)),$P(^AUPNPROB(X,0),U,15)]"",$P(^AUPNPROB(X,0),U,15)>1 S Z=$P(^AUPNPROB(X,0),U,15) G ASSEVNS
 .I S]"",$D(^XTMP("BGPSNOMEDSUBSET",$J,SNA,S)) S G=$$SEVPER^BGP8D22(X) I G S Z=G G ASSEVNS
 .Q
ASSEVNS .;
 .S G=1_U_"PL "_$P(^ICD9(Y,0),U)_"=Pers: "_Z  ;$P(^AUPNPROB(X,0),U,15)
 .Q
 I G Q G
...
INHALED(P,BDATE,EDATE) ;EP - any inhaled steroid?
 ...
 ..S T=T+1
 ..I D="" S D=1_U_$$DATE^BGP8UTL($P($P(^AUPNVSIT(V,0),U),"."))_$P(^PSDRUG(Z,0),U) Q  ;it is an inhaled steroid that wasn't d/c'ed so 1 dispensing event
 ..S D=D_", "_$$DATE^BGP8UTL($P($P(^AUPNVSIT(V,0),U),"."))_" "_$P(^PSDRUG(Z,0),U)
 I T<2 Q ""
 Q D
VAPI(A,B) ;EP
 ;a is drug ien
...
ASTHTHER ;EP
 ...
 ;S BGPVAL=$$V2ASTH(DFN,BGPBDATE,BGPEDATE) I 'BGPVAL S BGPSTOP=1 Q  ;no asthma visits or persistent in ats
 I $$EMP^BGP8D213(DFN,$$DOB^AUPNPAT(DFN),BGPEDATE) S BGPSTOP=1 Q  ;has dx of emphysema
 I $$COPD^BGP8D22(DFN,$$DOB^AUPNPAT(DFN),BGPEDATE) S BGPSTOP=1 Q  ;has copd
 S (BGPASTH1,BGPASTH2)=$$ASSEV^BGP8D22(DFN,BGPEDATE)
 I BGPASTH1="" S BGPASTH1=$$PERASTH^BGP8D22(DFN,$$FMADD^XLFDT(BGPBDATE,-365),BGPBDATE)
 I BGPASTH2="" S BGPASTH2=$$PERASTH^BGP8D22(DFN,BGPBDATE,BGPEDATE)
...
SABA(P,BDATE,EDATE) ;EP - any SABA?
 ...
 K BGPMEDS1
 D GETMEDS^BGP8UTL2(P,BDATE,EDATE,"BGP PQA SABA MEDS","BGP PQA SABA NDC",,,.BGPMEDS1)
 ...
 K BGPMEDS1,^TMP($J,"A")
 D GETMEDS^BGP8UTL2(P,SD,E,"BGP PQA CONTROLLER MEDS","BGP PQA CONTROLLER NDC",,,.BGPMEDS1)
 ...
 .S V1D=$$VD^APCLV(V)
 .S C=1_U_"CONT THPY: "_$$DATE^BGP8UTL(V1D)_" "_$$VAL^XBDIQ1(9000010.14,M,.01)
 I 'C Q G
...
CHK90 ;
 ...
 S C=C+1,T(C)=$$DATE^BGP8UTL(SD)_U_$$VAL^XBDIQ1(9000010.14,M,.01)_U_"("_Q_")"
 ...
 .S C=C+1,T(C)=$$DATE^BGP8UTL(V1D)_U_$$VAL^XBDIQ1(9000010.14,M1,.01)_U_" ("_Q_")"
 .S TQ=TQ+Q
 Q:TQ<4
...
CONT(P,BDATE,EDATE) ;controller meds (at least 2)
 ...
 .S V1D=$$VD^APCLV(V)
 .S T=T+1
 .I C="" S C=1_U_""_$$DATE^BGP8UTL(V1D)_" "_$$VAL^XBDIQ1(9000010.14,M,.01) I 1
 .E  S C=C_", "_$$DATE^BGP8UTL(V1D)_" "_$$VAL^XBDIQ1(9000010.14,M,.01)
 I T>1 Q C
 Q ""
 ;
LABA(P,BDATE,EDATE) ;EP - any inhaled steroid?
 ...
 .I $D(^ATXAX(T1,21,"B",Z))!($$NDC(Z,T4)),'$$STATDC(Y) D
 ..S T=T+1
 ..I D="" S D=1_U_"LABA: "_$P(^PSDRUG(Z,0),U)_U_$$DATE^BGP8UTL($P($P(^AUPNVSIT(V,0),U),".")) Q  ;it is an inhaled steroid that wasn't d/c'ed so 1 dispensing event
 ..S D=D_", "_$P(^PSDRUG(Z,0),U)_U_$$DATE^BGP8UTL($P($P(^AUPNVSIT(V,0),U),"."))
 I T<2 Q ""
 Q D
