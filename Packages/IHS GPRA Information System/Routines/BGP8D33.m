...
OPV(P,EDATE) ;EP
 ...
 K BGPX D GETIMMS^BGP8D32(P,EDATE,C,.BGPX)
 ...
 ...S Y=$P(^AUPNVCPT(X,0),U),Y=$P($$CPT^ICPTCOD(Y),U,2) I Y=90712!(Y=90698)!(Y=90711)!(Y=90713)!(Y=90723)!(Y=90696)!(Y=90697) S BGPOPV(9999999-$P(ED,"."))="",BGPAPOV(9999999-$P(ED,"."))=""
 ..S X=0 F  S X=$O(^AUPNVTC("AD",V,X)) Q:X'=+X  D
 ...S Y=$P(^AUPNVTC(X,0),U,7) Q:'Y  S Y=$P($$CPT^ICPTCOD(Y),U,2) I Y=90712!(Y=90698)!(Y=90711)!(Y=90713)!(Y=90723)!(Y=90696)!(Y=90697) S BGPOPV(9999999-$P(ED,"."))="",BGPAPOV(9999999-$P(ED,"."))=""
 ...
 K BGPG ;D SETPRC^BGP8UTL1(P,$$DOB^AUPNPAT(P),EDATE,"BGP IPV IZ PROCS",.BGPG)
 ...
 I $$PLTAXND^BGP8DU(P,"BGP OPV EVID DISEASE",EDATE) Q 4_U_"Evid Polio"
 I $$IPLSNOND^BGP8DU(P,"PXRM BGP POLIO",EDATE) Q 4_U_"Evid Polio"
 F BGPZ=2,10,89,110,120,130,132,146 S X=$$ANNECONT^BGP8D31(P,BGPZ,EDATE) Q:X]""
 ...
 F BGPIMM=90712,90696,90698,90711,90713,90723,90697 D
 ...
 ;F BGPIMM=2,89,10,110,120,130,132,146 S R=$$IMMREF^BGP8D32(P,BGPIMM,$$DOB^AUPNPAT(P),EDATE)+R
...
REF ;
 ...
 I R Q $S(BGPNMI:4,1:3)_U_$S(BGPNMI:"NMI MMR",1:"Ref MMR")
 ;now check Refusals in imm pkg
 S R="" ;F BGPIMM=3,94 S R=$$IMMREF^BGP8D32(P,BGPIMM,$$DOB^AUPNPAT(P),EDATE)+R
 ;
MMRC ;
 S BGPZ=$$IMMUNO^BGP8D36(P,EDATE) I BGPZ Q 4_U_"Contra MMR"
 S BGPZ=$$HIVDX^BGP8D24A(P,$$DOB^AUPNPAT(P),EDATE) I BGPZ Q 4_U_"Contra MMR"
 S BGPZ=$$LYMPHO^BGP8D36(P,EDATE) I BGPZ Q 4_U_"Contra MMR"
 F BGPZ=3,94 S X=$$MMRCONT^BGP8D31(P,BGPZ,EDATE) Q:X]""
 I X]"" Q 4_U_"Contra MMR"
MRC ;
 F BGPZ=4 S X=$$ANCONT^BGP8D31(P,BGPZ,EDATE) Q:X]""
...
REFMR ;
 ...
 ;now check Refusals in imm pkg
 ;F BGPIMM=4 I $$IMMREF^BGP8D32(P,BGPIMM,$$DOB^AUPNPAT(P),EDATE) S BGPMR=3
RMC ;
 F BGPZ=38 S X=$$ANCONT^BGP8D31(P,BGPZ,EDATE) Q:X]""
...
REFRM I BGPRM=0 D
 ...
 ;F BGPIMM=38 I $$IMMREF^BGP8D32(P,BGPIMM,$$DOB^AUPNPAT(P),EDATE) S BGPRM=3
MEX ;
 ...
 I $$PLTAXND^BGP8DU(P,"BGP MEASLES EVIDENCE",EDATE) S BGPME=4
 I $$IPLSNOND^BGP8DU(P,"PXRM BGP MEASLES",EDATE) S BGPME=4
 I BGPME=0 D
 .S B=$$DOB^AUPNPAT(P),E=EDATE
 .F BGPIMM=5 D
 ..S I=$O(^AUTTIMM("C",BGPIMM,0)) Q:'I
 ...
 ;F BGPIMM=7 I $$IMMREF^BGP8D32(P,BGPIMM,$$DOB^AUPNPAT(P),EDATE) S BGPME=3
 F BGPZ=5 S X=$$MMRCONT^BGP8D31(P,BGPZ,EDATE) Q:X]""
 I X]"" S BGPME=4
MUX ;
 ...
 I $$PLTAXND^BGP8DU(P,"BGP MUMPS EVIDENCE",EDATE) S BGPMU=4
 I BGPMU=0 D
 .S B=$$DOB^AUPNPAT(P),E=EDATE
 .F BGPIMM=7 D
 ..S I=$O(^AUTTIMM("C",BGPIMM,0)) Q:'I
 ...
 ;now check Refusals in imm pkg
 ;F BGPIMM="7" I $$IMMREF^BGP8D32(P,BGPIMM,$$DOB^AUPNPAT(P),EDATE) S BGPMU=3
 F BGPZ=7 S X=$$ANCONT^BGP8D31(P,BGPZ,EDATE) Q:X]""
 I X]"" S BGPMU=4
RUBX ;
 ...
 I $$PLTAXND^BGP8DU(P,"BGP RUBELLA EVIDENCE",EDATE) S BGPRUB=4
 I $$IPLSNOND^BGP8DU(P,"PXRM BGP RUBELLA",EDATE) S BGPRUB=4
 I BGPRUB=0 D
 .S B=$$DOB^AUPNPAT(P),E=EDATE
 .F BGPIMM=6 D
 ..S I=$O(^AUTTIMM("C",BGPIMM,0)) Q:'I
 ...
 ;F BGPIMM=6 I $$IMMREF^BGP8D32(P,BGPIMM,$$DOB^AUPNPAT(P),EDATE) S BGPRUB=3
 F BGPZ=6 S X=$$ANCONT^BGP8D31(P,BGPZ,EDATE) Q:X]""
 I X]"" S BGPRUB=4
