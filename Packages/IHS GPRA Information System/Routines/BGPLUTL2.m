BGPLUTL2 ; IHS/CMI/LAB - UTILITIES 06 Jan 2017 9:49 AM ; 08 Jan 2019  7:50 AM
...
EDD(P,BDATE,EDATE) ;EP - find EDD
 ...
 S Y=$$LASTITEM^BGPLDU(P,BDATE,EDATE,"MEASUREMENT","EGA")
 I Y D  I EDD Q EDD
...
GETMEDS(P,BGPMBD,BGPMED,TAXM,TAXN,TAXC,BGPDNAME,BGPZ,TAXRXN) ;EP
 ...
 S T3="" I TAXRXN]"" S T3=$O(^BGPSNOML("B",TAXRXN,0))
 ...
 .S C=$$VAL^XBDIQ1(50,D,9999999.27)
 .I C]"",T3,$D(^BGPSNOML(T3,11,"B",C)) S G=1
...
RCIS(P,BDATE,EDATE,ICDC,CPTC) ;EP
 ...
 Q 1_"^"_$P($P(^BMCREF(G,0),U),".")_"^"_$$DATE^BGPLUTL($P($P(^BMCREF(G,0),U),"."))_"^"_"RCIS referral"_"^"_X_"^"_"90001"_"^"_G
 ;
...
UNFOLDTX ;EP
 ...
 S BGPDA=0 F  S BGPDA=$O(^BGPTAXL(BGPDA)) Q:'BGPDA  D
 . S BGPTAX=$P($G(^BGPTAXL(BGPDA,0)),U)
 . S BGPFL=$P($G(^BGPTAXL(BGPDA,0)),U,2)
...
SNOMED ;unfold all snomed subsets
 ...
 S BGPDA=0 F  S BGPDA=$O(^BGPSNOSL(BGPDA)) Q:BGPDA'=+BGPDA  D
 .S N=$P(^BGPSNOSL(BGPDA,0),U,1)  ;subset name
 .K ^TMP($J,"SUB")
 .S OUT=$NA(^TMP($J,"SUB"))
