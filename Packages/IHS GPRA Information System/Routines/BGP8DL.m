BGP8DL ;IHS/CMI/LAB - LOCAL COM REPORT;
 ;;18.0;IHS CLINICAL REPORTING;**1**;NOV 21, 2017;Build 62
 ;
 ;
 W:$D(IOF) @IOF
 W !!,$$CTR("IHS 2018 CRS - Clinical Performance Measure Report (Selected Measures)",80)
INTRO ;
 D XIT
...
COMM ;
 ...
 D ^XBFMK
 K DIR
 W ! S DIR(0)="Y",DIR("A")="Run Report for One Community",DIR("B")="N" KILL DA D ^DIR KILL DIR
 I $D(DIRUT) G BY
 I Y D  G:BGPCOMMI="" COMM  G:BGPCOMMI COMQ
...
SUM ;display summary of this report
 W:$D(IOF) @IOF
 W !,$$CTR("SUMMARY OF 2018 CLINICAL MEASURE PERFORMANCE REPORT TO BE GENERATED")
 W !!,"The date ranges for this report are:"
 W !?5,"Report Period: ",?31,$$FMTE^XLFDT(BGPBD)," to ",?31,$$FMTE^XLFDT(BGPED)
 W !?5,"Previous Year Period: ",?31,$$FMTE^XLFDT(BGPPBD)," to ",?31,$$FMTE^XLFDT(BGPPED)
 W !?5,"Baseline Period: ",?31,$$FMTE^XLFDT(BGPBBD)," to ",?31,$$FMTE^XLFDT(BGPBED)
 I BGPTAXI W !!,"The COMMUNITY Taxonomy to be used is: ",$P(^ATXAX(BGPTAXI,0),U)
 I BGPCOMMI W !!,"The COMMUNITY is: ",$P(^AUTTCOM(BGPCOMMI,0),U,1)
 I BGPHOME W !,"The HOME location is: ",$P(^DIC(4,BGPHOME,0),U)," ",$P(^AUTTLOC(BGPHOME,0),U,10)
 I 'BGPHOME W !,"No HOME Location selected."
 W !!,"These performance measures will be calculated: " S X=0 F  S X=$O(BGPIND(X)) Q:X'=+X  W $P(^BGPINDR(X,0),U,3)," ; "
 W !!,"Lists will be produced for these measures: "
 S X=0 F  S X=$O(BGPLIST(X)) Q:X'=+X  W $P(^BGPINDR(X,0),U,3)," ; "
 D TEXT^BGP8DSL
 I $D(DIRUT) G BEN
 D PT^BGP8DSL
 I BGPROT="" G BEN
...
NODEV1 ;
 ...
 Q
DI ;EP
 S X=0 F  S X=$O(^BGPINDRC("ADM",1,X)) Q:X'=+X  S BGPIND($P(^BGPINDRC(X,0),U,1))=""
 Q
CI ;EP
 S X=0 F  S X=$O(^BGPINDRC("ACARD",1,X)) Q:X'=+X  S BGPIND($P(^BGPINDRC(X,0),U,1))=""
 Q
AI ;EP
 S X=0 F  S X=$O(^BGPINDRC("AAST",1,X)) Q:X'=+X  S BGPIND($P(^BGPINDRC(X,0),U,1))=""
 Q
II ;EP
 S X=0 F  S X=$O(^BGPINDRC("AIPC",1,X)) Q:X'=+X  S BGPIND($P(^BGPINDRC(X,0),U,1))=""
 Q
PI ;EP
 S X=0 F  S X=$O(^BGPINDRC("APQA",1,X)) Q:X'=+X  S BGPIND($P(^BGPINDRC(X,0),U,1))=""
 Q
WI ;EP
 S X=0 F  S X=$O(^BGPINDRC("AWH",1,X)) Q:X'=+X  S BGPIND($P(^BGPINDRC(X,0),U,1))=""
 Q
EI ;EP
 S X=0 F  S X=$O(^BGPINDRC("AEL",1,X)) Q:X'=+X  S BGPIND($P(^BGPINDRC(X,0),U,1))=""
 Q
SI ;EP
 K BGPIND
 D EN^BGP8DSI
...
TSKMN ;EP ENTRY POINT FROM TASKMAN
 ...
 S ZTCPU=$G(IOCPU),ZTRTN="DRIVER^BGP8DL",ZTDTH="",ZTDESC="CRS 18 COM REPORT" D ^%ZTLOAD D XIT Q
 Q
 ;
NODEV ;
