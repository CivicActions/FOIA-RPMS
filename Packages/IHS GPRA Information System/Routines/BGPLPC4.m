BGPLPC4 ; IHS/CMI/LAB - measure I2 ; 02 Feb 2019  11:25 AM
...
FLU ;EP
 ...
 S A=$$M6BD(DFN)
 I A>BGPBDATE S BGPSTOP=1 Q  ;turned 6 months after end date of report period
 ;
 ...
 S BGPDV1=$$ENC42(DFN,$$FMADD^XLFDT(BGPBDATE,-92),$$FMADD^XLFDT(BGPBDATE,90)) I BGPDV1="" S BGPSTOP=1 Q
 ;
 ...
 ;
 S BGPD1=1
 S X=$$FLUVAC(DFN,$$FMADD^XLFDT(BGPBDATE,-153),$$FMADD^XLFDT(BGPBDATE,90))
 I $E(X)=1 S BGPN1=1
 I 'BGPN1 K BGPSTOP D  I $D(BGPSTOP) S BGPD1=0,BGPN1=0 Q
 .;now what about exceptions?
 .I $$FLUREF(DFN,$$FMADD^XLFDT(BGPBDATE,-153),$$FMADD^XLFDT(BGPBDATE,90)) S BGPSTOP=1 Q  ;refused
 .I $$FLUALG(DFN,$$DOB^AUPNPAT(DFN),$$FMADD^XLFDT(BGPBDATE,90),BGPEDATE) S BGPSTOP=1 Q  ;ALLERGY
 S BGPVALUE=""
 S BGPVALUE="ENC "_$P(BGPDV,U,2)_";"_$P(BGPDV1,U,2)_"|||"  ;hit denominator
...
FLUALG(P,BDATE,BDATE89,EDATE) ;
 ...
 ..I F=81 I '$$ICD^BGPLUTL2(I,TCPT,1) Q  ;cpt but not a flu cpt
...
ENC4(P,BDATE,EDATE) ;EP  - have encounter per CMS122v6
 ...
 .S D=$$VD^APCLV(V)
 .S Y=$$SNOMEDV(V) I Y]"" S G=1_U_$$DATE^BGPLUTL(D)_" SNOMED: "_Y Q  ;ITEM 1
 .;is .17 a cpt we want?
 .S Y=$$VALI^XBDIQ1(9000010,V,.17)
 .I Y,$$OFFCPT4(Y) S G=1_U_$$DATE^BGPLUTL(D)_" CPT: "_$P($$CPT^ICPTCOD(Y),U,2) Q
...
SNOMEDV(V) ;EP
 ...
 .I $D(^XTMP("BGPSNOMEDSUBSET",$J,"PXRM BGP IPC OP CONSULT",A)) S B=A_" OP CONSULT" Q
 .I $D(^XTMP("BGPSNOMEDSUBSET",$J,"PXRM BGP IPC LONG TERM CARE",A)) S B=A_" LONG TERM" Q
 .I $D(^XTMP("BGPSNOMEDSUBSET",$J,"PXRM BGP IPC HOME HEALTH",A)) S B=A_" HOME HEALTH" Q
 .I $D(^XTMP("BGPSNOMEDSUBSET",$J,"PXRM BGP IPC PAT PROV INT",A)) S B=A_" PROV INT" Q
 .I A=18170008 S B=A Q
 .I A=207195004 S B=A Q
 I B]"" Q B
 S A=0,B=""
...
ENC42(P,BDATE,EDATE) ;EP  - have encounter per CMS147
 ...
 .S D=$$VD^APCLV(V)
 .S Y=$$VALI^XBDIQ1(9000010,V,.17)
 .I Y,$$FLUCPT(Y) S G=1_U_$$DATE^BGPLUTL(D)_" CPT: "_$P($$CPT^ICPTCOD(Y),U,2) Q
 .;now check all V CPTs
 .S Z=0 F  S Z=$O(^AUPNVCPT("AD",V,Z)) Q:Z'=+Z!(G)  D
 ..S Y=$P($G(^AUPNVCPT(Z,0)),U,1)
 ..I Y,$$FLUCPT(Y) S G=1_U_$$DATE^BGPLUTL(D)_" CPT: "_$P($$CPT^ICPTCOD(Y),U,2) Q
 ...
 ..I $D(^XTMP("BGPSNOMEDSUBSET",$J,"PXRM BGP IPC FLU ENCOUNTER",A)) S G=1_U_$$DATE^BGPLUTL(D)_" SNOMED: "_A Q
 ...
 ..I A]"",$D(^XTMP("BGPSNOMEDSUBSET",$J,"PXRM BGP IPC FLU ENCOUNTER",A)) S G=1_U_$$DATE^BGPLUTL(D)_" SNOMED: "_A
 Q G
PROVINT(V) ;EP
 NEW A,B,C
 S A=0,B=""
...
FLUVAC(P,BDATE,EDATE) ;
 ...
 .Q:D<BDATE
 .Q:D>EDATE
 .S BGPLFLU=1_U_$$DATE^BGPLUTL(D)_U_"Imm "_CVX
 I BGPLFLU Q BGPLFLU
 ;CPT
 I TCPT D
 .S X=$$CPT^BGPLDU(P,BDATE,EDATE,TCPT,5) I X]"" Q
 .S X=$$TRAN^BGPLDU(P,BDATE,EDATE,TCPT,5)
 I X]"" Q 1_U_$$DATE^BGPLUTL($P(X,U,1))_U_"CPT: "_$P(X,U,2)
