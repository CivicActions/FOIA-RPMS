BGP8D74 ;IHS/CMI/LAB - MEASURE LOGIC;
...
IEDA ;EP
 ...
 S X="",D="",J="",C=0
 F  S X=$O(BGPDAE(X)) Q:X=""  D
 .S T=0
 .S D=0 F  S D=$O(BGPDAE(X,D)) Q:D'=+D  D
 ..S C=$P(BGPDAE(X,D),U,1),T=T+C
 ..I $P(BGPDAE(X,D),U,1)>0 S BGPN1=1
 ..I $P(BGPDAE(X,D),U,1)>1 S BGPN2=1
 ..I C>0 S J=J_$S(J]"":"; ",1:"")_X_" "_$P(^PSDRUG(D,0),U,1) ;_$S(C=1:" (1 fill)",1:" (2+ fills)")
 .I T=1 S J=J_" (1 fill)"
 .I T>1 S BGPN2=1 S J=J_" (2+ fills)"
 ;S C=C+1,J=J_$S(J]"":"; ",1:"")_$$DATE^BGP8UTL($P(BGPDAE(X),U,3))_" "_$P(BGPDAE(X),U,2)_" ("_$P(BGPDAE(X),U,4)_")"
 F X=1:1:18 I '$D(BGPNUMV(X)) S BGPNUMV(X)=0
 S BGPVALUE="UP"
 I BGPRTYPE=3!(BGPRTYPE=5) S BGPVALUE=""
 S BGPVALUE=BGPVALUE_$S(BGPD1&(BGPVALUE]""):",AC",BGPD1&(BGPVALUE=""):"AC",1:"")_"|||" I BGPN1 S BGPVALUE=BGPVALUE_J
...
IBFR ;EP
 ...
 .S BGPV2="2 MOS: "_X_" DO, "_$$DATE^BGP8UTL($P(Y,U,2))_" "_$$EXTSET^XBFUNC(9000010.44,.01,+Y)
 .S G=1
 D
 ...
 .S BGPV3="6 MOS: "_X_" DO, "_$$DATE^BGP8UTL($P(Y,U,2))_" "_$$EXTSET^XBFUNC(9000010.44,.01,+Y)
 .S G=1
 D
 ...
 .S BGPV4="9 MOS: "_X_" DO, "_$$DATE^BGP8UTL($P(Y,U,2))_" "_$$EXTSET^XBFUNC(9000010.44,.01,+Y)
 .S G=1
 D
 ...
 .S BGPV5="1 YR: "_X_" DO, "_$$DATE^BGP8UTL($P(Y,U,2))_" "_$$EXTSET^XBFUNC(9000010.44,.01,+Y)
 .S G=1
 ...
 .S X=$O(BGPBFR("")) S Y=BGPBFR(X) S BGPVALUE="Scrn: "_X_" DO, "_$$DATE^BGP8UTL($P(Y,U,2))_" "_$$EXTSET^XBFUNC(9000010.44,.01,+Y)
...
HOSPICE(P,BDATE,EDATE) ;ep hospice
 ...
 I $$CPT^BGP8DU(P,BDATE,EDATE,$O(^ATXAX("B","BGP CPT HOSPICE",0)),5) Q 1
 ;CHECK V POV FOR SNOMED CODES
 NEW S,Y,T,G,D,M,O
 S T=$O(^BGPSNOMR("B","HOSPICE CARE",0))
 S G=""
 S BD=9999999-BDATE
 S S=0 F  S S=$O(^BGPSNOMR(T,11,"B",S)) Q:S=""!(G)  D
...
GETDAE(P,BDATE,EDATE,BGPRET,BGPRET1) ;EP
 ...
 K BGPMEDS1 S BGPXXX="ANTICHOLINERGIC",BGPGOT=0 D GETMEDS^BGP8UTL2(P,BGPBDATE,BGPEDATE,"BGP HEDIS ANTICHOLINERGIC MEDS","BGP HEDIS ANTICHOLINERGIC NDC",,,.BGPMEDS1)
 D ADDINMED
 I BGPGOT S BGPRET1(1)=1  ;had 2 of this one
 K BGPMEDS1 S BGPXXX="ANTITHROMBOTIC",BGPGOT=0 D GETMEDS^BGP8UTL2(P,BGPBDATE,BGPEDATE,"BGP HEDIS ANTITHROMBOTIC MEDS","BGP HEDIS ANTITHROMBOTIC NDC",,,.BGPMEDS1)
 D ADDINMED
 I BGPGOT S BGPRET1(2)=1
 K BGPMEDS1 S BGPXXX="ANTI-INFECTIVE",BGPGOT=0 D GETMEDS^BGP8UTL2(P,BGPBDATE,BGPEDATE,"BGP HEDIS ANTI-INFECTIVE MEDS","BGP HEDIS ANTI-INFECTIVE NDC",,,.BGPMEDS1)
 D ADDINMED
 I BGPGOT S BGPRET1(3)=1
 K BGPMEDS1 S BGPXXX="CARDIOVASCULAR",BGPGOT=0 D GETMEDS^BGP8UTL2(P,BGPBDATE,BGPEDATE,"BGP HEDIS CARDIOVASCULAR MEDS","BGP HEDIS CARDIOVASCULAR NDC",,,.BGPMEDS1)
 D ADDINMED
 I BGPGOT S BGPRET1(4)=1
 K BGPMEDS1 S BGPXXX="CENTRAL NERVOUS",BGPGOT=0 D GETMEDS^BGP8UTL2(P,BGPBDATE,BGPEDATE,"BGP HEDIS CENTRAL NERVOUS MEDS","BGP HEDIS CENTRAL NERVOUS NDC",,,.BGPMEDS1)
 D ADDINMED
 I BGPGOT S BGPRET1(5)=1
 K BGPMEDS1 S BGPXXX="ENDOCRINE",BGPGOT=0 D GETMEDS^BGP8UTL2(P,BGPBDATE,BGPEDATE,"BGP HEDIS ENDOCRINE MEDS","BGP HEDIS ENDOCRINE NDC",,,.BGPMEDS1)
 D ADDINMED
 I BGPGOT S BGPRET1(6)=1
 K BGPMEDS1 S BGPXXX="PAIN",BGPGOT=0 D GETMEDS^BGP8UTL2(P,BGPBDATE,BGPEDATE,"BGP HEDIS PAIN MEDS","BGP HEDIS PAIN NDC",,,.BGPMEDS1)
 D ADDINMED
 I BGPGOT S BGPRET1(7)=1
 K BGPMEDS1 S BGPXXX="SKL MUSCLE",BGPGOT=0 D GETMEDS^BGP8UTL2(P,BGPBDATE,BGPEDATE,"BGP HEDIS SKL MUSCLE RELAX MED","BGP HEDIS SKL MUSCLE RELAX NDC",,,.BGPMEDS1)
 D ADDINMED
...
ADDINMED ;
 ...
 .S $P(BGPRET(BGPXXX,D),U)=$P($G(BGPRET(BGPXXX,D)),U)+1,BGPGOT=1
 .S $P(BGPRET(BGPXXX,D),U,2)=$P(^PSDRUG(D,0),U)
 .S $P(BGPRET(BGPXXX,D),U,3)=$P(BGPMEDS1(X),U,1)  ;LAST FILL
 .S $P(BGPRET(BGPXXX,D),U,4)=BGPXXX
 .Q
 Q
ANTIINF ;
...
FALLEX(P,BDATE,EDATE) ;EP
 ...
 I $D(BGPG(1)) Q 1_"^Ex 37^"_$$DATE^BGP8UTL($P(BGPG(1),U))
 S %=$$CPT^BGP8DU(P,BDATE,EDATE,$O(^ATXAX("B","BGP FALL RISK EXAM CPTS",0)),6)
 I % Q "1^CPT "_$P(%,U,3)_"^"_$$DATE^BGP8UTL($P(%,U,2))
 Q ""
DHF(P,BDATE,EDATE) ;EP
 ...
 I $D(BGPG(1)) Q 1_U_"POV "_$P(BGPG(1),U,2)_U_"Hx of Fall: "_$$DATE^BGP8UTL($P(BGPG(1),U))
 Q ""
FID(P,BDATE,EDATE) ;EP
 ...
 .S E=$P(^AUPNVPOV(Y,0),U,9) I E,$$ICD^BGP8UTL2(E,T,9) S G=1_U_"E-CODE "_$P($$ICDDX^BGP8UTL2(E),U,2)_U_"Fall Injury: "_$$DATE^BGP8UTL($P(BGPG(X),U,1)) Q
 .S E=$P(^AUPNVPOV(Y,0),U,18) I E,$$ICD^BGP8UTL2(E,T,9) S G=1_U_"E-CODE "_$P($$ICDDX^BGP8UTL2(E),U,2)_U_"Fall Injury: "_$$DATE^BGP8UTL($P(BGPG(X),U,1)) Q
 .S E=$P(^AUPNVPOV(Y,0),U,19) I E,$$ICD^BGP8UTL2(E,T,9) S G=1_U_"E-CODE "_$P($$ICDDX^BGP8UTL2(E),U,2)_U_"Fall Injury: "_$$DATE^BGP8UTL($P(BGPG(X),U,1)) Q
 I G Q G
 Q ""
AGM(P,BDATE,EDATE) ;EP
 S BGPG=$$LASTDX^BGP8UTL1(P,"BGP ABNORMAL GAIT OR MOBILITY",BDATE,EDATE)
 I $P(BGPG,U) Q 1_U_"POV "_$P(BGPG,U,2)_U_"Abnormal Gait: "_$$DATE^BGP8UTL($P(BGPG,U,3))
 Q ""
REFFRE(P,BDATE,EDATE) ;EP
 ;add Refusal for exam 37
 S G=$$REFUSAL^BGP8UTL1(P,9999999.15,$O(^AUTTEXAM("C",37,0)),$$FMADD^XLFDT(EDATE,-365),EDATE)
 I $P(G,U)=1 Q 1_"^Ex 37^"_"Refused "_$$DATE^BGP8UTL($P(G,U,2))
 Q ""
NDC(A,B) ;
 ;a is drug ien
 ;b is taxonomy ien
