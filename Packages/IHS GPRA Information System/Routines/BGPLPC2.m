BGPLPC2 ; IHS/CMI/LAB - measure I2 ; 02 Feb 2019  11:25 AM
 ;;19.1;IHS CLINICAL REPORTING;**1**;MAY 30, 2019;Build 7
 ;
DMG ;EP
 S (BGPN1,BGPD1)=0
 S BGPDV=""
 I BGPAGEB<18 S BGPSTOP=1 Q  ;18 or greater during time period
...
ENC1(P,BDATE,EDATE) ;EP  - have encounter per CMS122v6
 ...
 ;2.  CPT in BGP IPC PREVCARE EOV >=18 CPTS
 ;3.  CPT in BGP IPC PREVCARE IOV >=18 CPTS
 ;4.  CPT in BGP IPC HOMEHEALTH VISIT CPTS
 ;5.  CPT in BGP IPC ANNUAL WELLNESS CPTS
 NEW X,Y,Z,G,BGPV,D
 ...
 .S D=$$VD^APCLV(V)
 .;S Y=$$FTOF(V) I Y]"" S G=1_U_$$DATE^BGPLUTL(D)_" FTOF: "_Y Q
 .S Y=$$IPCOFFV(V) I Y]"" S G=1_U_$$DATE^BGPLUTL(D)_" SNOMED: "_Y Q
 .;is .17 a cpt we want?
 .S Y=$$VALI^XBDIQ1(9000010,V,.17)
 .I Y,$$OFFCPT(Y) S G=1_U_$$DATE^BGPLUTL(D)_" CPT: "_$P($$CPT^ICPTCOD(Y),U,2) Q
 .;now check all V CPTs
 .S Z=0 F  S Z=$O(^AUPNVCPT("AD",V,Z)) Q:Z'=+Z!(G)  D
 ..S Y=$P($G(^AUPNVCPT(Z,0)),U,1)
 ..I Y,$$OFFCPT(Y) S G=1_U_$$DATE^BGPLUTL(D)_" CPT: "_$P($$CPT^ICPTCOD(Y),U,2) Q
 .;HOME HEALTH SNOMED 19.1 PATCH 1
 .S Y=$$HOMESNOM(V) I Y]"" S G=1_U_$$DATE^BGPLUTL(D)_" HOME HEALTH: "_Y Q
 Q G
IPCOFFV(V) ;EP
 NEW A,B,C
 S A=0,B=""
 F  S A=$O(^AUPNVSIT(V,28,"B",A)) Q:A=""!(B]"")  D
 .I $D(^XTMP("BGPSNOMEDSUBSET",$J,"PXRM BGP IPC OFFICE VISIT",A)) S B=A_" OFF VISIT"
 I B]"" Q B
 S A=0,B=""
...
HOMESNOM(V) ;EP
 ...
 S A=0,B=""
 F  S A=$O(^AUPNVSIT(V,28,"B",A)) Q:A=""!(B]"")  D
 .I $D(^XTMP("BGPSNOMEDSUBSET",$J,"PXRM BGP IPC HOME HEALTH",A)) S B=A_" HOME HEALTH"
 I B]"" Q B
 S A=0,B=""
 F  S A=$O(^AUPNVSIT(V,26,"B",A)) Q:A=""!(B]"")  D
 .I $D(^XTMP("BGPSNOMEDSUBSET",$J,"PXRM BGP IPC HOME HEALTH",A)) S B=A_" HOME HEALTH"
...
DMRP(P,BDATE,EDATE) ;EP - DM DURING REPORT PERIOD
 NEW X,Y,Z
 I $$PLTAXND^BGPLDU(P,"BGP IPC DIABETES DXS",EDATE,1) Q 1
 I $$IPLSNOND^BGPLDU(P,"PXRM BGP IPC DIABETES",EDATE,1) Q 1
 I $$LASTDX^BGPLUTL1(P,"BGP IPC DIABETES DXS",BDATE,EDATE) Q 1
 Q ""
HOSPIND(P,BDATE,EDATE) ;EP - Hospice indicator IPC
 ...
 ..S G=1
 I G Q G
 I $$IPLSNOND^BGPLDU(P,"PXRM BGP IPC HOSPICE",EDATE,1) Q 1
 I G Q G
 ;check all V Hospitalization discharges during the report period
 S X=0,G=""
...
HGBA1C(P,BDATE,EDATE) ;EP
 ...
 ....S %=(9999999-$P(%,"."))_"."_(9999-$$RZERO^BGPLUTL($P(%,".",2),4)),%=+%
 ...
 ..I X="" Q
 ..I $E(X)[">" S G=1_U_$$DATE^BGPLUTL(9999999-$P(D,"."))_U_X Q
 ..I $E(X)["<" S G=1_U_$$DATE^BGPLUTL(9999999-$P(D,"."))_U_X Q
 ..I $E(X)'=+$E(X) Q
 ..S G=1_U_$$DATE^BGPLUTL(9999999-$P(D,"."))_U_X
