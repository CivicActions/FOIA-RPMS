...
PCR(P,BDATE,EDATE) ;EP
 ...
 S E=+$$CODEN^ICPTCOD(87536),%=$$CPTI^BGP8DU(P,BDATE,EDATE,E) I %]"" Q 1_U_$P(%,U,2)_"^87536"
 S E=+$$CODEN^ICPTCOD(87539),%=$$CPTI^BGP8DU(P,BDATE,EDATE,E) I %]"" Q 1_U_$P(%,U,2)_"^87539"
 S E=+$$CODEN^ICPTCOD("G9242"),%=$$CPTI^BGP8DU(P,BDATE,EDATE,E) I %]"" Q 1_U_$P(%,U,2)_"^G9242"
 S E=+$$CODEN^ICPTCOD("G9243"),%=$$CPTI^BGP8DU(P,BDATE,EDATE,E) I %]"" Q 1_U_$P(%,U,2)_"^G9243"
 S E=+$$CODEN^ICPTCOD(87536),%=$$TRANI^BGP8DU(P,BDATE,EDATE,E) I %]"" Q 1_U_$P(%,U,2)_"^87536 TRAN"
 S E=+$$CODEN^ICPTCOD(87539),%=$$TRANI^BGP8DU(P,BDATE,EDATE,E) I %]"" Q 1_U_$P(%,U,2)_"^87539 TRAN"
 S E=+$$CODEN^ICPTCOD("G9242"),%=$$TRANI^BGP8DU(P,BDATE,EDATE,E) I %]"" Q 1_U_$P(%,U,2)_"^G9242 TRAN"
 S E=+$$CODEN^ICPTCOD("G9243"),%=$$TRANI^BGP8DU(P,BDATE,EDATE,E) I %]"" Q 1_U_$P(%,U,2)_"^G9243 TRAN"
 ...
 .I $$LOINC^BGP8D21(J,T) S G=1_U_$$VD^APCLV($P(^AUPNVLAB(I,0),U,3))_U_$$VAL^XBDIQ1(9000010.09,I,.01)
 Q G
ART(P,BDATE,EDATE) ;EP -antiviral med?
 NEW A,C,G,BGPMEDS1,T,M
 K BGPMEDS1,^TMP($J,"A")
 D GETMEDS^BGP8UTL2(P,BDATE,EDATE,"BGP PQA ANTIRETROVIRAL MEDS","BGP PQA ANTIRETROVIRAL NDC",,,.BGPMEDS1)
 S C=""
...
MMR(P,EDATE) ;EP
 ...
 S BGPMMR=0
 S C="3^94"
 K BGPX D GETIMMS^BGP8D32(P,EDATE,C,.BGPMMR)
...
MUPV ;
 ...
 K BGPG ;D SETPRC^BGP8UTL1(P,$$DOB^AUPNPAT(P),EDATE,"BGP MUMPS IZ PROCS",.BGPG)
 ;S X=0 F  S X=$O(BGPG(X)) Q:X'=+X  S BGPMU($P(BGPG(X),U))=""
 S X="",Y="",C=0 F  S X=$O(BGPMU(X)) Q:X'=+X  S C=C+1 D
 .I C=1 S Y=X Q
 .I $$FMDIFF^XLFDT(X,Y)<11 K BGPMU(X) Q
...
RUBPV ;
 ...
 K BGPG ;D SETPRC^BGP8UTL1(P,$$DOB^AUPNPAT(P),EDATE,"BGP RUBELLA IZ PROCS",.BGPG)
 ;S X=0 F  S X=$O(BGPG(X)) Q:X'=+X  S BGPRUB($P(BGPG(X),U))=""
 S X="",Y="",C=0 F  S X=$O(BGPRUB(X)) Q:X'=+X  S C=C+1 D
 .I C=1 S Y=X Q
 .I $$FMDIFF^XLFDT(X,Y)<11 K BGPRUB(X) Q
...
MEX ;
 ...
 I $$PLTAXND^BGP8DU(P,"BGP MEASLES EVIDENCE",EDATE) S BGPME=4,BGPMEEV=1
 I $$IPLSNOND^BGP8DU(P,"PXRM BGP MEASLES",EDATE) S BGPME=4,BGPMEEV=1
 I BGPME=0 D
 .S B=$$DOB^AUPNPAT(P),E=EDATE
 .F BGPIMM=5 D
 ..S I=$O(^AUTTIMM("C",BGPIMM,0)) Q:'I
...
MUX ;
 ...
 I $$PLTAXND^BGP8DU(P,"BGP MUMPS EVIDENCE",EDATE) S BGPMU=4,BGPMUEV=1
 I BGPMU=0 D
 .S B=$$DOB^AUPNPAT(P),E=EDATE
 .F BGPIMM=7 D
 ..S I=$O(^AUTTIMM("C",BGPIMM,0)) Q:'I
...
RUBX ;
 ...
 I $$PLTAXND^BGP8DU(P,"BGP RUBELLA EVIDENCE",EDATE) S BGPRUB=4,BGPRUEV=1
 I $$IPLSNOND^BGP8DU(P,"PXRM BGP RUBELLA",EDATE) S BGPRUB=4,BGPRUEV=1
 I BGPRUB=0 D
 .S B=$$DOB^AUPNPAT(P),E=EDATE
 .F BGPIMM=6 D
 ..S I=$O(^AUTTIMM("C",BGPIMM,0)) Q:'I
