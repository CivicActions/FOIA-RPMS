BGP5DHE ; IHS/CMI/LAB - IHS GPRA 05 REPORT DRIVER ; 
 ;;5.0;IHS CLINICAL REPORTING;**1**;OCT 15, 2004
 ;
 ;
 W:$D(IOF) @IOF
 W !!,$$CTR("2005 HEDIS Clinical Performance Indicator Report",80)
INTRO ;
 D XIT
 W !!,"This will produce a HEDIS Indicator Report for all HEDIS indicators for a year"
 W !,"period you specify.  You will be asked to provide: 1) the"
...
SUM ;display summary of this report
 ...
 W:$D(IOF) @IOF
 W !,$$CTR("SUMMARY OF FY 05 HEDIS REPORT TO BE GENERATED")
 W !!,"The date ranges for this report are:"
 W !?5,"Report Period: ",?31,$$FMTE^XLFDT(BGPBD)," to ",?31,$$FMTE^XLFDT(BGPED)
 W !?5,"Previous Year Period: ",?31,$$FMTE^XLFDT(BGPPBD)," to ",?31,$$FMTE^XLFDT(BGPPED)
 W !?5,"Baseline Period: ",?31,$$FMTE^XLFDT(BGPBBD)," to ",?31,$$FMTE^XLFDT(BGPBED)
 W !!,"The COMMUNITY Taxonomy to be used is: ",$P(^ATXAX(BGPTAXI,0),U)
 W !!,"All HEDIS indicators will be calculated."
 D PT^BGP5DHSL
 I BGPROT="" G BEN
ZIS ;call to XBDBQUE
 D REPORT^BGP5HUTL
 ...
 I BGPRPT="" D XIT Q
 I BGPEXPT D
 .W !!,"A file will be created called BG05",$P(^AUTTLOC(DUZ(2),0),U,10)_".HE"_BGPRPT," and will reside",!,"in the ",BGPUF," directory.",!
 ...
 I POP W !,"Report Aborted" S DA=BGPRPT,DIK="^BGPHEDCV(" D ^DIK K DIK D XIT Q
 I POP W !,"Report Aborted" S DA=BGPRPT,DIK="^BGPHEDPV(" D ^DIK K DIK D XIT Q
 I POP W !,"Report Aborted" S DA=BGPRPT,DIK="^BGPHEDBV(" D ^DIK K DIK D XIT Q
 I $D(IO("Q")) G TSKMN
DRIVER ;
 D ^BGP5D1
 U IO
 ...
 ;
NODEV ;
 S XBRP="",XBRC="NODEV1^BGP5DHE",XBRX="XIT^BGP5DHE",XBNS="BGP"
 D ^XBDBQUE
 ;D XIT
 Q
 ;
...
TSKMN ;EP ENTRY POINT FROM TASKMAN
 ...
 S ZTCPU=$G(IOCPU),ZTRTN="DRIVER^BGP5DHE",ZTDTH="",ZTDESC="HEDIS 05 REPORT" D ^%ZTLOAD D XIT Q
 Q
 ;
XIT ;
 D ^%ZISC
...
LISTS ;any lists with indicators?
 ...
 I $D(BGPLIST) D RT^BGP5DHSL I '$D(BGPLIST)!($D(BGPQUIT)) G LISTS ;get report type for each list
 Q
CHKY ;
