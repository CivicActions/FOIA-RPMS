BGPLPC14 ; IHS/CMI/LAB - measure I2 ; 26 Jul 2019  3:40 PM
 ;;19.1;IHS CLINICAL REPORTING;**1**;MAY 30, 2019;Build 7
 ;
BMI ;EP - CALLED FROM IPC REPORT
 S (BGPN1,BGPD1)=0
 S BGPBMIE="",BGPBMI="",BGPFU="",BGPDVS=""
 ;
 ;
 ...
 S BGPDVS=$P(BGPBMIE,U,2),BGPBMIE=$P(BGPBMIE,U,1)
 ...
 ;palliative care
 I $$PALLCARE^BGPLPC13(DFN,$$DOB^AUPNPAT(DFN),BGPBMIE) S BGPSTOP=1 G BMIE
 ;
 ;pregnancy
 ...
 S BGPD1=1
 S D=$E(BGPBMIE,1,3)-1,D=D_$E(BGPBMIE,4,7) S D=$$FMADD^XLFDT(D,-29)
 S BGPBMI=$$MRBMI(DFN,D,BGPBMIE) I BGPBMI]"" S BGPBMI=$J(BGPBMI,5,2)
 I BGPBMI'<18.5,BGPBMI<25 S BGPN1=1 G BMI1
 S D=$E(BGPBMIE,1,3)-1,D=D_$E(BGPBMIE,4,7) S D=$$FMADD^XLFDT(D,-29)
 I BGPBMI'<25 S BGPFU=$$ANFU(DFN,D,BGPBMIE) S:BGPFU BGPN1=1 G BMI1
 S BGPFU=$$BLFU(DFN,D,BGPBMIE) S:BGPFU BGPN1=1 G BMI1
 ;MEDICAL REASON NOT DONE
 S D=$E(BGPBMIE,1,3)-1,D=D_$E(BGPBMIE,4,7) S D=$$FMADD^XLFDT(D,-29)
 I $$MRND(DFN,D,$$FMADD^XLFDT(BGPBMIE,-1)) S BGPD1=0,BGPSTOP=1 G BMIE
BMI1 ;
 S BGPVALUE=""
 S BGPVALUE="ENC "_$$DATE^BGPLUTL(BGPBMIE)_"|||"  ;hit denominator
 I BGPN1 S BGPVALUE=BGPVALUE_"*** BMI: "_BGPBMI I BGPFU S BGPVALUE=BGPVALUE_" F/U PLAN: "_$$DATE^BGPLUTL($P(BGPFU,U,2))_" "_$P(BGPFU,U,3)
...
MRBMIE(P,BDATE,EDATE) ;EP  - MOST RECENT BMI ENCOUNTER DURING BDATE TO EDATE
 ...
 .S D=$$VD^APCLV(V)
 .S Y=$$BMIENC(V) I Y]"" S G=D_"^SNOMED: "_Y Q   ;ITEM 18
 .;is .17 a cpt we want?
...
MRBMI(P,BDATE,EDATE) ;
 NEW X,Y,H,W,BMI
 S X=$$LASTITEM^BGPLDU(P,BDATE,EDATE,"MEASUREMENT","BMI")
 I X Q $P(X,U,4)
 ;get last ht within bdate and edate
 S X=$$LASTITEM^BGPLDU(P,BDATE,EDATE,"MEASUREMENT","WT")
 I X="" Q ""
 S W=$P(X,U,4)
 S X=$$LASTITEM^BGPLDU(P,BDATE,EDATE,"MEASUREMENT","HT")
 I X="" Q ""
 S H=$P(X,U,4)
 ;calculate BMI
...
BMIENC(V) ;EP
 ...
 I B]"" Q B
 S A=0,B="",C=""
 F  S A=$O(^AUPNVPOV("AD",V,A)) Q:A=""!(B]"")  D
 .S C=$$VAL^XBDIQ1(9000010.07,A,1101) I C]"",$D(^XTMP("BGPSNOMEDSUBSET",$J,"PXRM BGP IPC DEP SCRN ENC",C)) S B=C
 Q B
BMIECPT(C) ;
...
PREG(P,BDATE,EDATE) ;
 ...
 I $P(^DPT(P,0),U,2)'="F" Q ""
 ;check dx
 S X=$$LASTDX^BGPLUTL1(P,"BGP IPC PREGNANCY CMS69 DXS",BDATE,EDATE) I X Q 1
...
MRND(P,BDATE,EDATE) ;EP
 ...
 .I '$$ICD^BGPLUTL2(I,$O(^ATXAX("B","BGP IPC ABOVE NORMAL FU CPTS",0)),1),'$$ICD^BGPLUTL2(I,$O(^ATXAX("B","BGP IPC BELOW NORMAL FU CPTS",0)),1) Q
 .S ID=0 F  S ID=$O(^AUPNPREF("AA",P,F,I,ID)) Q:ID=""!(G)  D
 ..S D=9999999-ID
 ..Q:D<BDATE
 ..Q:D>EDATE
...
ANFU(P,BDATE,EDATE) ;EP
 NEW A,B,C,X,Y,Z,T,V,R,G,C
 ;is there a dx
 S X=$$LASTDX^BGPLUTL1(P,"BGP IPC ABOVE NORMAL FU DXS",BDATE,EDATE) I X Q 1_U_$P(X,U,3)_U_"DX: "_$P(X,U,2)
 ;now check cpts
 ...
 .;clinic code
 .S C=$$CLINIC^APCLV(Y,"C")
 .I C=67!(C=36) S G=1_U_$$VD^APCLV(Y)_U_"Clinic: "_C Q
 .;provider
 .S A=0 F  S A=$O(^AUPNVPRV("AD",Y,A)) Q:A'=+A!(G)  D
 ..S B=$P($G(^AUPNVPRV(A,0)),U,1)
 ..Q:'B
 ..S C=$$VALI^XBDIQ1(200,B,53.5)
 ..Q:'C
 ..S C=$P($G(^DIC(7,C,9999999)),U,1)
 ..I C="07"!(C=29) S G=1_U_$$VD^APCLV(Y)_U_"Prov: "_C Q
 ..Q
 .;PATIENT ED
 .S A=0 F  S A=$O(^AUPNVPED("AD",Y,A)) Q:A'=+A!(G)  D
 ..S T=$P($G(^AUPNVPED(A,0)),U,1)
 ..Q:'T
 ..S T=$P($G(^AUTTEDT(T,0)),U,2)
 ..I $P(T,"-",2)="EX"!($P(T,"-",2)="LA")!($P(T,"-",2)="N")!($P(T,"-",2)="MNT")!($P(T,"-")="Z71.3") S G=1_U_$$VD^APCLV(Y)_U_"Pt Ed: "_T Q
 .;REFERRAL
 ...
 ..Q:'R
 ..Q:'$D(^BMCREF(R,0))
 ..S A=0 F  S A=$O(^BMCREF(R,22,"B",A)) Q:A=""!(G)  I $D(^XTMP("BGPSNOMEDSUBSET",$J,"PXRM BGP IPC OVERWEIGHT",A)) S G=1_U_$$VD^APCLV(Y)_U_"Refer: "_S_"/"_A
 I G Q G
 S G=""
 ;
 D GETMEDS^BGPLUTL2(P,BDATE,EDATE,"BGP IPC ABOVE NORMAL MEDS","",,,.BGPMEDS1,"BGP IPC ABOVE NORMAL RXNORM")
...
BLFU(P,BDATE,EDATE) ;EP
 ...
 ;is there a dx
 S X=$$LASTDX^BGPLUTL1(P,"BGP IPC BELOW NORMAL FU DXS",BDATE,EDATE) I X Q 1_U_$P(X,U,3)_U_"DX: "_$P(X,U,2)
 ;now check cpts
 ...
 ;NOW MEDS
 S G=""
 D GETMEDS^BGPLUTL2(P,BDATE,EDATE,"BGP IPC BELOW NORMAL MEDS","",,,.BGPMEDS1,"BGP IPC BELOW NORMAL RXNORM")
 S X=0,T=0,W="" F  S X=$O(BGPMEDS1(X)) Q:X'=+X!(G)  D
 .S Y=$P(BGPMEDS1(X),U,4)  ;vmed ien
 .S V=$P(BGPMEDS1(X),U,5)
 .Q:'$D(^AUPNVMED(Y,0))
